 ...
 * @returns {string[]} Returns the file path and line number of every matched line.
 * @example
 *
 * // The contents of 'foo.js':
 *
 ...
 * ]
 *
 * dantil.pathAndLineNumbersOf(__filename, '\'ipsum\'')
 * // => [ '/Users/Danny/foo.js:3' ]
 *
 * dantil.pathAndLineNumbersOf(__filename, 'ipsum')
 * // => [ '/Users/Danny/foo.js:3', '/Users/Danny/foo.js:4' ]
 */
exports.pathAndLineNumbersOf = function (filePath, string) {
	var matches = []

	basePathAndLineNumbersOf(filePath, string, function (pathAndLineNumber) {
 ...
 * ]
 *
 * dantil.firstPathAndLineNumberOf(__filename, 'ipsum')
 * // => '/Users/Danny/foo.js:3'
 *
 * dantil.firstPathAndLineNumberOf(__filename, '\'ipsum\'')
 * // => '/Users/Danny/foo.js:'4
 */
exports.firstPathAndLineNumberOf = function (filePath, string) {
	var match
...
/**
 * The base implementation of `dantil.pathAndLineNumbersOf` and `dantil.firstPathAndLineNumberOf` to find occurrences of `string` in the source file at `filePath`. Invokes iteratee per matched line with the path and line number in the format `filePath:lineNumber` as the only argument.
 *
 * @private
 * @param {string} filePath The path of the source file to search.
 * @param {string} string The string to search for.
 * @param {Function} iteratee The function invoked per matched line. Return `true` to break out of the function.
 */
function basePathAndLineNumbersOf(filePath, string, iteratee) {
	var fileLines = fs.readFileSync(filePath, 'utf8').split('\n')
	var fileLinesLen = fileLines.length

	for (var l = 0; l < fileLinesLen; ++l) {
		if (fileLines[l].indexOf(string) !== -1) {
