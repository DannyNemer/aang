	...
	for (var prop in options) {
		...
		if (propSchemaVals) {
			...
			if (Array.isArray(optsVal) && propSchema.arrayType) {
				if (isIllFormedArray(options, prop, propSchema.arrayType)) {
					return true
				}
			}
		}
 ...
 * @private
 * @static
 * @param {Object} options The passed options object to inspect.
 * @param {string} prop The `options` property name that defines the array to inspect.
 * @param {Function|Function[]} arrayType The expected type (constructor), or array of expected types, of `options[prop]`.
 * @returns {boolean} Returns `true` if `options[prop]` contains an element not of type `arrayType`, else `false`.
 */
function isIllFormedArray(options, prop, arrayType) {
