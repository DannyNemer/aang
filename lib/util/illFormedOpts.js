	...
	for (var prop in options) {
		...
		if (propSchema.values) {
			...
			if (Array.isArray(optsVal)) {
				// Check if passed array is empty (if `propSchema.allowEmpty` is not truthy) or contains `undefined`.
				if (isIllFormedArray(options, prop, propSchema)) {
					return true
				}
...
/**
 * Checks if the array `options[prop]` is empty (if `propSchema.allowEmpty` is not truthy) or contains `undefined`.
 *
 * @private
 * @static
 * @param {Object} options The options object to inspect.
