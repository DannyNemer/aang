	...
	for (var prop in options) {
		...
		if (propSchemaVals) {
			...
			if (Array.isArray(optsVal) && propSchema.arrayType) {
				var arrayType = propSchema.arrayType
...
/**
 * Checks `options` for properties not defined in `schema`. If so, prints an error message with `schema` to display the acceptable properties.
 *
 * @private
 * @static
 * @param {Object} schema The definition of required and optional properties for `options`.
 * @param {Object} [options] The options object to check for conformity to `schema`.
 * @returns {boolean} Returns `true` if `options` has properties not defined in `schema`, else `false`.
 */
