...
/**
 * Checks if `options[prop]` is not a predefined, acceptable value in `acceptableVals`.
 *
 * @private
 * @static
 * @param {Object} options The options object to inspect.
 * @param {string} prop The `options` property name that defines the value to inspect.
 * @param {*[]} acceptableVals The predefined, acceptable values for `options[prop]`.
 * @returns {boolean} Returns `true` if `options[prop]` is not in `acceptableVals`, else `false`.
 */
function isUnrecognizedVal(options, prop, acceptableVals) {
	var optsVal = options[prop]

	// Check if passed value is not a predefined, acceptable value.
	if (acceptableVals.indexOf(optsVal) === -1) {
		var propQuoted = '\'' + prop + '\''
		util.logError('Unrecognized value for ' + propQuoted + ':', util.stylize(optsVal))
		util.log('       Acceptable values for ' + propQuoted + ':', acceptableVals)
		util.logPathAndObject(options)
		return true
	}
...
function isIncorrectType(options, prop, propType) {
	var optsVal = options[prop]
	var acceptableTypes = Array.isArray(propType) ? propType : [ propType ]

	if (acceptableTypes.indexOf(optsVal.constructor) === -1) {
		var propQuoted = '\'' + prop + '\''
		util.logError('Incorrect type for ' + propQuoted + ':', util.stylize(optsVal))
		util.log('       Acceptable types for ' + propQuoted + ':', concatConstructorNames(acceptableTypes))
		util.logPathAndObject(options)
		return true
	}
...
/**
 * Checks if the array `options[prop]` contains an element not of the acceptable type(s), `arrayType`.
 *
 * @private
 * @static
 * @param {Object} options The options object to inspect.
 * @param {string} prop The `options` property name that defines the array to inspect.
 * @param {Function|Function[]} arrayType The acceptable type(s) (constructor(s)) of elements in `options[prop]`.
 * @returns {boolean} Returns `true` if `options[prop]` contains an element not of type(s) `arrayType`, else `false`.
 */
