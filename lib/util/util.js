...
exports.logErrorWithNewLine = function () {
	return exports.logError.apply(null, Array.from(arguments).concat('\n'))
}
 ...
 * @returns {*} Returns the first printed value.
 */
function logWithColoredLabel(label, color) {
	var values = Array.from(arguments).slice(2)
	exports.log.apply(null, prependColoredLabel(label, color, values))
	return values[0]
}
 ...
 * @param {...*} [values] The values to print following `label`.
 * @returns {*} Returns the first value following `label`.
 */
...
/**
 * Imports methods for `dantil.time()` and `dantil.timeEnd()`.
 *
 * @private
 * @type {Object}
 */
...
/**
 * Imports methods for `dantil.count()`, `dantil.countEnd()`, and `dantil.countEndAll()`.
 *
 * @private
 * @type {Object}
 */
...
exports.clone = function (value) {
	 ...
	 * • Create the new object using `Object.create()` instead of `new value.constructor()` to avoid invoking any operations in the constructor of `value`.
	 * • Use `Object.assign()` to skip non-enumarable properties and properties inherited through the prototype chain.
	 */
 ...
 * @returns {Object} Returns the clone of `object` without `undefined` properties.
 */
exports.deleteUndefinedObjectProps = function (object) {
	var object = exports.clone(object)
	...
	for (var prop in object) {
		...
		// Use `typeof` instead of `instanceof` to omit `object` instance methods (of type `Function`).
 ...
 * @param {number} [subLength=0] The length of each sub-array (i.e., the second dimension).
