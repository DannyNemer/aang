 ...
 * @param {boolean} [collapsed] Specify putting ranges of lines without changes in a fold.
	...
	for (var p = 0, partsLen = diffParts.length; p < partsLen; ++p) {
		...
		if (part.added) {
			...
			if (partsLen > 1) {
				...
				if (p === 0) {
					if (part.count > unchangedFoldSize) {
						unchangedPart += diffBar(part, unchangedFoldSize) + '\n'
					}

					unchangedPart += lines.slice(-unchangedFoldSize - 1).join('\n')
				} else if (p === partsLen - 1) {
					unchangedPart += lines.slice(0, unchangedFoldSize).join('\n')

					if (part.count > unchangedFoldSize) {
						unchangedPart += '\n' + diffBar(part, unchangedFoldSize)
					}
				} else {
					unchangedPart += lines.slice(0, unchangedFoldSize).join('\n') + '\n'

					if (part.count > unchangedFoldSize * 2) {
						unchangedPart += diffBar(part, unchangedFoldSize * 2) + '\n'
					}
...
/**
 * Generates a bar depicting `numFoldedLines` lines without changes collapsed in a fold, for use by `dantil.diffObjects()`.
 *
 * @private
 * @static
 * @param {Object} part The part being collapsed.
 * @param {number} numFoldedLines The number of lines to collapse.
 * @returns {string} Returns the bar depicting the range of unchanged lines collapsed in fold.
 */
function diffBar(part, numFoldedLines) {
	var bar = '----' + (part.count - numFoldedLines) + ' lines'
