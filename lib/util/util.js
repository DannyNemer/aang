...
/**
 * Passes the structured stack trace as an array of `CallSite` objects to the provided `getFrameFunc(stack)`, which returns a single frame, for which this function returns in the format `filePath:lineNumber`.
 *
 * Skips stack frames for this module and native Node functions.
 *
 * @private
 * @static
...
function getPathAndLineNumberForFrame(getFrameFunc) {
	// Collect the stack trace and invoke `getFrameFunc` to get the desired stack frame.
	var frame = getFrameFunc(getStackTraceArray())

	// If `getFrameFunc` returns a frame, return the path and line number `frame` in the format `filePath:lineNumber`.
	if (frame) {
...
/**
 * Gets the first `CallSite` object in the stack trace, skipping frames for this module and native Node functions.
 *
 * @private
 * @static
 * @returns {CallSite} Returns the first `CallSite` object in the stack trace.
 */
function getTopStackFrame() {
	var topStackFrame
	getPathAndLineNumberForFrame(function (stack) {
		topStackFrame = stack[0]
	})
...
exports.logLine = function (label) {
	exports.log(stylizeStackFrame(getTopStackFrame(), label))
}
...
exports.prettifyStackTrace = function (removePathParens) {
	...
	Error.prepareStackTrace = function (error, structuredStackTrace) {
		var stack = structuredStackTrace.map(function (frame) {
			...
			var frameString = stylizeStackFrame(frame)
 ...
 */
function stylizeStackFrame(stackFrame, label) {
	var frameString = stackFrame.toString()

	// Colorize function name (or `label`, if provided).
	var lastSpaceIndex = frameString.lastIndexOf(' ')
	if (lastSpaceIndex !== -1) {
		return exports.colors.cyan(label || frameString.substring(0, lastSpaceIndex)) + frameString.substring(lastSpaceIndex)
	}
