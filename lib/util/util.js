...
exports.getPathAndLineNumber = function () {
	return getPathAndLineNumberForFrame(function (stack) {
 ...
 */
function getPathAndLineNumberForFrame(getFrameFunc) {
...
/**
 * Prints the provided values like `dantil.log()` prepended with red-colored "Error: ", followed by a trailing newline.
 *
...
exports.logError = function () {
	exports.logErrorSansNewLine.apply(null, arguments)
...
/**
 * Prints the provided values like `dantil.logError()`, without a trailing newline.
 *
 * @static
 * @memberOf dantil
 * @category Console
 ...
 * @example
 *
 * dantil.logErrorSansNewLine('Failed', numTestsFailed, 'of', tests.length, 'tests')
 * ```
 * Output:
 * <br><img src="https://raw.githubusercontent.com/DannyNemer/dantil/master/doc/img/dantil-logErrorSansNewLine-example.jpg" alt="dantil.logErrorSansNewLine() example output"/>
 * ```
 */
exports.logErrorSansNewLine = logWithColoredLabel.bind(null, 'Error', 'red')
...
function logColoredLabelAndPath(label, color, logThisLine) {
	...
	if (logThisLine !== true) {
		pathAndLineNumber = exports.getModuleCallerPathAndLineNumber()
	}
	...
	if (!pathAndLineNumber) {
		pathAndLineNumber = exports.getPathAndLineNumber()
	}
...
exports.logLine = function (label) {
	getPathAndLineNumberForFrame(function (stack) {
		var topStackFrame = stack[0]
		var frameString = topStackFrame.toString()
		var firstParenIndex = frameString.indexOf('(') - 1
		frameString = exports.colors.cyan(label || frameString.substring(0, firstParenIndex)) + frameString.substring(firstParenIndex)
 ...
 * @memberOf dantil
 * @category Console
 * @param {boolean} [removePathParens] Specify removing parentheses that surround file paths. This prevents parentheses from breaking iTerm's open-file-path shortcut in iTerm versions prior to v3.x.
 * @example
 *
...
exports.prettifyStackTrace = function (removePathParens) {
	// Collect 15 stacks frames instead of (the default) 10.
	...
	Error.prepareStackTrace = function (error, structuredStackTrace) {
		...
		// Color `Error` name (e.g., `TypeError`) red.
