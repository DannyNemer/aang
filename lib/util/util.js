...
exports.logError = function () {
	logColoredLabelAndArgs('Error', 'red', arguments)
}
...
exports.logWarning = function () {
	logColoredLabelAndArgs('Warning', 'yellow', arguments)
}
...
exports.logSuccess = function () {
	logColoredLabelAndArgs('Success', 'green', arguments)
}

/**
 * Prints `args to `stdout` like `dantil.log()` prepended with `label` (e.g., "Error") colored `color` (e.g., "Error: ").
 *
 * @private
 * @static
 * @param {string} label The label to prepend to `args` (e.g., "Error").
 * @param {string} color The color to stylize `label`.
 * @param {*[]} args The arguments array of values to follow `label`.
 */
function logColoredLabelAndArgs(label, color, args) {
	var args = prependColoredLabel(label, color, Array.from(args))
	exports.log.apply(null, args)
}

/**
 * Prepends `label` (e.g., "Error") colored `color` (e.g., "Error: ") to `values`. If the first character of the first value in `values` is '\n', moves the character to before `label`.
 *
 * @private
 * @static
