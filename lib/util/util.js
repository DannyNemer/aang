 ...
 */
exports.pathAndLineNumbersOf = function (filePath, value, stringify) {
	var matches = []

	basePathAndLineNumbersOf(filePath, value, stringify, function (pathAndLineNumber) {
 ...
 * // => '/Users/Danny/foo.js:4'
 */
exports.firstPathAndLineNumberOf = function (filePath, value, stringify) {
	var match
 ...
 */
function basePathAndLineNumbersOf(filePath, value, stringify, fromLine, iteratee) {
	// Check arity.
	if (typeof stringify === 'function') {
		iteratee = stringify
		fromLine = 0
		stringify = false
	} else if (typeof stringify === 'number') {
		iteratee = fromLine
		fromLine = stringify
		stringify = false
	} else if (typeof fromLine === 'function') {
		iteratee = iteratee
		fromLine = 0
	}
	...
	lineNumbersOf(filePath, value, fromLine, function (lineNumber) {
...
/**
 * Gets the line numbers of occurrences of `string` in the source file at `filePath`. Stops iteration once `iteratee` returns truthy. Invokes `iteratee` with one argument: (lineNumber).
 *
 * @private
 * @static
 * @param {string} filePath The path of the source file to search.
 ...
 */
function lineNumbersOf(filePath, string, fromLine, iteratee) {
	// Check arity.
	...
	// Find occurrences of `string` in `fileLines`.
	for (var l = fromLine, fileLinesLen = fileLines.length; l < fileLinesLen; ++l) {
