...
exports.getPathAndLineNumber = function () {
	...
	return getPathAndLineNumberForFrame(stack[0])
}
...
exports.getModuleCallerPathAndLineNumber = function () {
	...
	for (var f = 0, stackLen = stack.length; f < stackLen; ++f) {
		...
		// Get the module name of where this method was invoked.
		if (!thisModuleName) {
			thisModuleName = frame.getFileName()
			continue
		}

		// Find the frame of the most recent module after where this method was invoked.
		if (thisModuleName !== frame.getFileName()) {
			return getPathAndLineNumberForFrame(frame)
		}
	}

	// Return `undefined` if there is no other module in the stack below where this method was invoked.
}

/**
 * Gets the file path and line number of `stackFrame` in the format `filePath:lineNumber:columnNumber`.
 *
 * @private
 * @static
 * @param {CallSite} stackFrame The stack frame for which to get the file path and line number.
 * @returns {string} Returns the file path and line number of `stackFrame`.
 */
function getPathAndLineNumberForFrame(stackFrame) {
	var frameString = stackFrame.toString()
	var lastSpaceIndex = frameString.lastIndexOf(' ')
	return lastSpaceIndex === -1 ? frameString : frameString.substring(lastSpaceIndex)
}
...
exports.logLine = function (label) {
	var stack = getStackTraceArray()
	exports.log(stylizeStackFrame(stack[0], label))
}
