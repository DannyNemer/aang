 ...
 *
 * Hence, can not prevent paths where the start rule is recursive and contains multiple instances of the same rule.
 *
 ...
 * By default, when the `--find-all` option is set to `false`, the ambiguity check will only print one instance of ambiguity for each pair of rules from the root nonterminal symbol.
 *
 * This also exemplifies a single instance of a path being ambiguous with multiple paths from the same start rule. Hence, when `--find-all` is `true`, can not `break` after finding ambiguity with a pair of paths.
 *
 ...
 *
 * Both paths may find their other instance of ambiguity before the instance they share. Hence, can not stop a check for ambiguity after finding first instance of ambiguity; otherwise, that shared instance will not be reached.
 *
