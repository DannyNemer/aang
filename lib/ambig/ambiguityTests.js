 ...
 *
 * Used by `ambiguityCheck` to test its algorithm's coverage by reporting if
 * it correctly detects each ambiguous test.
 */
...
/**
 * Ambiguity created by terminal rules with the same number of terminal
 * symbols (i.e., branches).
 *
 * S -> XY -> 'x y'
 *   -> Z  -> 'z'
 * S -> X  -> 'x'
...
/**
 * Ambiguity created by terminal rules with a different number of terminal
 * symbols (i.e., branches).
 *
 * S -> X -> 'x'
 *   -> Y -> 'y'
...
/**
 * The same instance of ambiguity represented at different depths by multiple
 * pairs of paths.
 *
 * When `--find-all` is `true`, avoid printing different instances of the same
 * ambiguous relationship by removing a pair's identical rightmost symbols
 * (using `diffTrees()`) and comparing to previous pairs.
 *
...
/**
 * Ambiguity created by path built from a path used in an earlier instance of
 * ambiguity.
 *
 * Hence, a path can not be discarded after finding ambiguity.
 *
...
/**
 * Multiple instances of ambiguity created by same pair of start rules from
 * the root nonterminal symbol.
 *
 * By default, when the `--find-all` option is set to `false`, the ambiguity
 * check will only print one instance of ambiguity for each pair of rules from
 * the root nonterminal symbol.
 *
...
/**
 * Ambiguity created by a recursive rule (i.e., a rule whose RHS contains the
 * LHS).
 *
 * - The grammar enforces symbols with recursive rules to require a
 * non-recursive rule to be the stop case.
 *
 * - In the grammar, recursive rules are made with `<empty>` in
 * `createEditRules`; however, here they are made manually (without
 * `createEditRules`).
 *
 * S -> X
 *   -> S -> Y
...
/**
 * Ambiguity created by rules where one the rules' RHS contains an unambiguous
 * recursive symbol. Ambiguity occurs only in these rules, not in the
 * recursive symbol's rules alone.
 *
 * Hence, can not prevent recursive rules in path search.
 *
 * S -> recur -> X
 *            -> recur -> Y
 * S -> X
...
/**
 * Ambiguity created by a recursive start rule and a path using multiple
 * instances of a single rule.
 *
 * Hence, can not prevent paths where the start rule is recursive and contains
 * multiple instances of the same rule.
 *
...
/**
 * Multiple instances of ambiguity created by the same pair of start rules
 * from the root nonterminal symbol.
 *
 * The presence of multiple instances, as opposed to one, is caused by an
 * ambiguous symbol in one of the rules' RHS.
 *
 * By default, when the `--find-all` option is set to `false`, the ambiguity
 * check will only print one instance of ambiguity for each pair of rules from
 * the root nonterminal symbol.
 *
 * This also exemplifies a single instance of a path being ambiguous with
 * multiple paths from the same start rule. Hence, when `--find-all` is
 * `true`, can not `break` after finding ambiguity with a pair of paths.
 *
...
/**
 * Ambiguity created by a path with two other paths at different rightmost
 * symbols.
 *
 * Hence, parse trees must be cloned before removing their rightmost symbols
 * (when found ambiguous).
 *
 * In a binary rule, the first branch is reduced down to the terminal symbol.
 * That path can be found ambiguous and have rightmost symbols removed. Then,
 * it might be found ambiguous again, but using symbols that were previously
 * removed.
 *
...
/**
 * Ambiguity created by a pair of paths, each ambiguous with at least one
 * other path.
 *
 * Both paths may find their other instance of ambiguity before the instance
 * they share. Hence, can not stop a check for ambiguity after finding first
 * instance of ambiguity; otherwise, that shared instance will not be reached.
 *
 * - If only one of the pair of ambiguous paths has more than one instance of
 * ambiguity, then it can be stopped early because its other instances of
 * ambiguity will be reached by the other paths (which have one instance of
 * ambiguity).
 *
 * - Must be terminal rules, otherwise the ambiguity can be found at different
 * instances of the path (i.e., different lengths) rather than at a single
 * instance.
 *
 * The third pair of ambiguous paths will not be reached if both paths are
 * stopped earlier:
 * S -> 'x'         S -> 'x'         S -> X1 -> 'x'
...
/**
 * Ambiguity created by unconjugated display text objects. This requires a
 * deep comparison of the text object values to detect the ambiguity.
 *
 * XY -> pl   -> 'x'
 *    -> 3-sg -> 'y'
 * S -> XY1-pl -> 'x'|'y'
...
/**
 * Ambiguity created by inserted text across multiple insertions. This
 * requires concatenating the display text to detect the ambiguity.
 *
 * S -> X     -> 'x' 'x' 'y' -> 'x x y'
...
/**
 * Ambiguity created by semantics parsed in a different order. This requires
 * sorting semantic arguments to detect the ambiguity.
 *
...
for (var nontermSym in ruleSets) {
	if (reTest.test(nontermSym) && ruleSets[nontermSym].length < 2) {
		...
		util.log('  ' + NSymbol._defLines[nontermSym])
