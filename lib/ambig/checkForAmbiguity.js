...
function searchPaths(nontermSym) {
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (rule.insertionIdx === undefined && !rule.isTransposition) {
			...
			if (rule.isTerminal) {
				// Any semantic on a terminal rule is completely reduced (i.e., RHS).
				newPath.semantics = reduceSemanticTree(undefined, 0, rule.semantic)

				// Prepend with a space because all will begin with space.
				newPath.terminals += ' ' + RHS[0]

				if (rule.text) {
					newPath.text.push(rule.text)
				}
			} else {
				newPath.semantics = appendSemantic(undefined, 0, rule.semantic, rule.semanticIsRHS, rule.insertedSemantic)
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (rule.insertionIdx === undefined && !rule.isTransposition) {
			...
			if (rule.isTerminal) {
				...
				if (newPath.semantics === -1) continue
