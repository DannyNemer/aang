...
function searchPaths(nontermSym) {
	...
	var rootPath = {
		...
		nextSym: undefined,
		// The reverse linked list of the second symbols of previously incomplete binary rules and yet-to-be-conjugated text objects from insertion rules. After reaching a terminal symbol, inspect `nextSyms` to complete the binary rules and conjugate the text objects.
		nextSyms: undefined,
				...
				for (var p = 0, pathsALen = pathsA.length; p < pathsALen; ++p) {
					...
					for (var o = 0; o < pathsBLen; ++o) {
						...
						if (pathB.nextSym === nextSym && nextSymsEqual(pathB.nextSyms, nextSyms)) {
							// Completely reduce semantic trees irrespective of parsing states and semantic argument requirements.
							var pathASemanticString = completeSemanticTree(pathA.semantics)
							var pathBSemanticString = completeSemanticTree(pathB.semantics)

							// Check if semantic trees are identical.
							var semanticsEquivalent = pathASemanticString === pathBSemanticString

							// Check if display texts, including unconjugated text objects and insertion text of yet-to-be-reduced rules, are identical.
							var textEquivalent = textsEqual(pathA.text, pathB.text) && nextTextsEqual(pathB.nextSyms, nextSyms)
							...
							if (!argv.findAll || !pairExists(ambigPairs, treeA, treeB)) {
								...
								if (!argv.quiet) {
									...
									util.log('  Semantic:', pathASemanticString)
									util.log('  Text:', pathA.text)
									util.dir('  ', treeA)
									util.log()
									util.log('  Semantic:', pathBSemanticString)
...
/**
 * Compares two paths' `nextSyms` linked lists of symbols and yet-to-be-conjugated text, ignoring the instances of text, to determine if the symbols are equivalent.
 *
...
function nextSymsEqual(a, b) {
	// Ignore instances of yet-to-be-conjugated text.
	while (a && !a.sym) {
		a = a.prev
	}
	...
	// Reached end of list without finding a difference.
	if (!a && !b) return true
...
/**
 * Compares two paths' `nextSyms` linked lists of symbols and yet-to-be-conjugated text, ignoring the instances of symbols, to determine if the text values are equivalent.
 *
 * @param {Object} a The `nextSyms` list to compare.
 * @param {Object} b The other `nextSyms` list to compare.
...
function nextTextsEqual(a, b) {
	// Ignore instances of yet-to-be-reduced symbols.
	while (a && !a.text) {
		a = a.prev
	}
...
function textsEqual(a, b) {
	// Text items are identical strings or both `undefined`.
	if (a === b) return true
...
function reduceSemanticTree(prevSemantic, prevNextSymsLen, newSemantic) {
	...
	while (prevSemantic) {
		...
		// Stop at a LHS semantic whose parse node has yet-to-be-reduced child nodes.
		else {
			break
		}
...
function conjugateText(path, text) {
	...
	if (textConstructor === String) {
		...
		// `text` is an insertion's array of text strings and yet-to-be-conjugated text objects, which contain a term's inflected forms. The array never contains other arrays.
...
/**
 * Conjugates an array of text strings and yet-to-be-conjugated text objects from an earlier insertion at RHS index 1. Called after completing an insertion rule's parse that determines the person-number required for the nominative case in the second (inserted) branch (verb precedes subject).
 *
