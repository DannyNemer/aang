 ...
 *
 *   Ambiguity exists when there are multiple paths from a single nonterminal symbol to the same
 *   rightmost symbols. When ambiguity is found, parse trees demonstrating the ambiguity are
 *   printed to demonstrate the necessary changes to make to the grammar.
 *
 ...
 *                          determining the necessary changes to make to the grammar; though, can
 *                          be helpful as the change needed might be in a subsequent rule produced
 *                          by the rule at the root of an instance of ambiguity. For certain cases
 *                          with `--find-all`, such as recursive rules (i.e., a rule whose RHS
 *                          contains the LHS), an excessive number of ambiguity instances are
 *                          printed.                                                     [boolean]
			...
			for (var terminals in pathSetsA) {
				...
				for (var p = 0, pathsALen = pathsA.length; p < pathsALen; ++p) {
					...
					for (var o = 0; o < pathsBLen; ++o) {
						...
						// A pair of paths is ambiguous when `terminals`, `nextSym`, and `nextSyms` (which together represent the rightmost symbols of the root symbol) are identical.
						// If `--complete-trees` is `true`, then `nextSym` and `nextSyms` are always `undefined`.
							foundAmbiguity = true
