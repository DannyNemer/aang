	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (rule.insertionIdx === undefined && !rule.isTransposition) {
			...
			var newPath = {
				...
				terminals: '',
				// Number of symbols used by this path, to ensure below `--tree-syms-limit`.
				symsCount: 1 + RHSLen,
				// Linked list to convert path's rules (i.e., the sets of RHS symbols) to a parse tree.
				RHS: RHS,
				// Pointer to previous node for tree graph representation.
				prev: rootNode,
				...
				semantics: undefined,
				// Number of elements in `nextSyms`, excluding text to append.
			...
			if (rule.isTerminal) {
				// Prefix with a space because all will begin with space.
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (rule.insertionIdx === undefined && !rule.isTransposition) {
			...
			if (rule.isTerminal) {
				...
				// This prevents certain instances of ambiguity from being printed. Although those parse trees would also be rejected in `forestSearch()`, they are still being constructed by `Parser` and their offending rules should be examined.
