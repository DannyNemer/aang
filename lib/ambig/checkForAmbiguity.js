 ...
 * @returns {Array} Returns a copy of `path.text` with `text` appended and conjugated if the necessary grammatical property exists.
 */
function conjugateText(path, text) {
	...
	if (textConstructor === String) {
		...
		newTextArray.push(conjugateTextObject(path, text))
	} else {
		...
		// The array returned by `conjugateTextArray()` is flattened in `concatTextAndNextSyms()` (which yields fewer operations than if performed here.)
		newTextArray.push(conjugateTextArray(path, text))
	}
 ...
 * @returns {Array} Returns a copy of `textArray` with text objects conjugated if the necessary grammatical property exists.
 */
