...
// Do not inspect transpositions because any ambiguity they create is evident in the original rules from which they were derived. Remove rules to avoid checking each rule multiple times within the recrusive function, `buildPaths()`.
...
function searchPaths(nontermSym) {
	...
	for (var r = 0; r < rulesLen; ++r) {
		...
		var newPath = createPath(rootPath, rules[r])
...
function buildPaths(pathSets, prevPath) {
	...
	for (var r = 0; r < rulesLen; ++r) {
		// Create a new path by expanding `prevPath` with one of its productions, `rule`.
		var newPath = createPath(prevPath, rules[r])
 ...
 * @param {number} prevNextSymsCount The number of elements in the previous path's `nextSyms` excluding yet-to-conjugate text. Used to determine if the rule of a LHS semantic has been completely reduced and the semantic can be reduced.
 ...
 */
function appendSemantic(prevSemantic, prevNextSymsCount, ruleProps) {
	// If `ruleProps.insertedSemantic` exists, then it is a RHS semantic and `ruleProps.semantic` also exists and is a LHS semantic.
		...
		if (ruleProps.semanticIsRHS) {
			if (prevSemantic) {
				...
				if (!ruleProps.rhsCanProduceSemantic) {
					...
					return reduceSemanticTree(prevSemantic, prevNextSymsCount, ruleProps.semantic)
				}
			}
