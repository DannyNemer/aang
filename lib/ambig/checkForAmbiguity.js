...
function createPath(prevPath, rule) {
	...
	if (!rule.isTerminal) {
		...
		newPath.semantics = appendSemantic(prevPath.semantics, prevPath.nextSymsCount, rule.semantic, rule.semanticIsRHS, rule.insertedSemantic, rule.rhsCanProduceSemantic)
	...
	while (true) {
		...
		// Unary nonterminal rule.
			...
			// Insertion rule.
				...
				parNode.children = rule.insertionIdx === 1 ? [ node, insertion ] : [ insertion, node ]
			}
...
function nodesEqual(a, b) {
	...
	if (a.symbol !== b.symbol || a.insertedText !== b.insertedText) return false
 ...
 * @param {number} prevNextNodesCount The number of elements in the previous path's `nextSyms` excluding yet-to-conjugate text. Used to determine if the rule of a LHS semantic has been completely reduced and the semantic can be reduced.
 ...
 */
function appendSemantic(prevSemantic, prevNextSymsCount, newSemantic, semanticIsRHS, insertedSemantic, rhsCanProduceSemantic) {
	// If `insertedSemantic` exists, then it is a RHS semantic and `newSemantic` also exists and is a LHS semantic.
	if (insertedSemantic) {
		...
		return {
			...
			prev: {
				...
				nextSymsCount: prevNextSymsCount,
				prev: prevSemantic,
			}
		}
	}
		...
		if (semanticIsRHS) {
			if (prevSemantic && prevSemantic.isRHS) {
				...
				return reduceSemanticTree(prevSemantic, prevNextSymsCount, newSemantic)
			} else {
				return {
					semantic: newSemantic,
					isRHS: true,
 ...
 */
function reduceSemanticTree(prevSemantic, prevNextSymsCount, newSemantic) {
	...
	while (prevSemantic) {
		...
		else if (prevNextSymsCount <= prevSemantic.nextSymsCount) {
