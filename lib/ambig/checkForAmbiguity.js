				...
				for (var p = 0, pathsALen = pathsA.length; p < pathsALen; ++p) {
					...
					for (var o = 0; o < pathsBLen; ++o) {
						...
						if (pathB.nextSym === nextSym && nextSymsEqual(pathB.nextSyms, nextSyms)) {
							...
							var semanticA = completeSemanticTree(pathA.semantics)
							var semanticB = completeSemanticTree(pathB.semantics)

							// Check if semantic trees are identical.
							var semanticsEquivalent = semantic.arraysEqual(semanticA, semanticB)

							// Check if display texts, including unconjugated text objects and insertion text of yet-to-reduce rules, are identical.
							...
							if (!argv.findAll || !pairExists(ambigPairs, treeA, treeB)) {
								...
								if (!argv.quiet) {
									...
									util.log('  Text:', concatTextAndNextSyms(pathA))
									util.log('  Semantic:', semanticA === -1 ? -1 : (semanticA ? semantic.colorString(semantic.toString(semanticA)) : undefined))
									util.dir('  ', treeA)
									util.log()
									util.log('  Text:', concatTextAndNextSyms(pathB))
									util.log('  Semantic:', semanticB === -1 ? -1 : (semanticB ? semantic.colorString(semantic.toString(semanticB)) : undefined))
									util.dir('  ', treeB)
									util.log()
								}
...
/**
 * Concatenates a path's display text, yet-to-conjugate insertion text arrays, and yet-to-reduce nonterminal symbols. Merges adjacent strings, which is necessary for determining if two paths have identical display text; e.g., XY -> "x" "y" equals Z -> "x y".
 *
 * Includes yet-to-reduce nonterminal symbols to prevent merging non-adjacent insertion texts. These nonterminal symbols are already determined to be identical to the other path being compared with this path.
 *
...
function concatTextAndNextSyms(path) {
	...
	// Include yet-to-reduce nonterminal symbols to prevent merging non-adjacent insertion texts. These nonterminal symbols are already determined to be identical to the other path being compared with this path.
	if (path.nextSym) {
	...
	while (listItem) {
		...
		if (listItemText instanceof Array) {
			...
			// Include yet-to-reduce nonterminal symbols and yet-to-conjugate insertion text.
