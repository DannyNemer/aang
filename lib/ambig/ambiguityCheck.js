...
/**
 * Performs a comparison between two rules' texts to determine if they are
 * equivalent.
 *
 * The provided arguments are either a rule's text object to conjugate,
 * array of text strings and objects to conjugate, or text string not
 * needing conjugation.
 *
 * @private
 * @static
 ...
 * @returns {boolean} Returns `true` if the text `a` and `b` are equivalent,
 * else `false`.
 */
function textsEqual(a, b) {
	// Text items are identical strings, object or array references, or both
	// `undefined`.
	if (a === b) return true
	...
	// Compare contents of insertion text arrays (containing text objects and
	// strings).
...
/**
 * Converts a reverse linked list of path nodes, each containing the grammar
 * rule used in its construction, to a graph representation.
 *
 * @private
 * @static
...
/**
 * Removes the rightmost portions of a pair of trees that the pair have in
 * common. This trims the trees up to the portions created by the rules
 * causing the ambiguity.
 *
 * When `--find-all` is passed, prevents printing the same instance of
 * ambiguity multiple times when found in multiple pairs of trees. I.e., the
 * pairs are distinguished by rules that come after the rules creating
 * ambiguity.
 *
 * @private
 * @static
...
function diffTrees(a, b) {
	// Invert trees to an array of rightmost symbols that lead back to the root
	// node.
	var invertedTreeA = invertTree(a)
	var invertedTreeB = invertTree(b)

	/**
	 * If there is a different number of rightmost symbols (i.e., branches), use
	 * the minimum length to compare as much of the trees as possible.
	 *
	 * EX: X -> A -> "a"   X -> "a b"
	 *       -> B -> "b"
	 */
	...
	for (var n = 0; n < minTreeWidth; ++n) {
		...
		// Trees have a different number of branches; break when at end of
		// narrower tree.
...
/**
 * Inverts a parse tree `rulesToTree()` produced to an array of rightmost
 * nodes with pointers to their parent nodes, ending at the tree's root
 * node.
 *
 * @private
 * @static
 * @param {Object} tree The root node of the parse tree to invert.
 * @returns {Object[]} Returns the inverted tree as an array of rightmost
 * nodes with pointers to parents.
 */
...
/**
 * Checks if a pair of ambiguous trees exists in `ambigPairs`. Called when
 * `--find-all` is passed, after the trees have been processed by
 * `diffTrees()`, to prevent printing the same instance of ambiguity
 * multiple times when found in multiple pairs of trees.
 *
 * @private
 * @static
 * @param {Object[]} ambigPairs The array of distinct pairs of ambiguous
 * trees found so far.
 ...
 * @returns {boolean} Returns `true` if the pairs exists in `ambigPairs`,
 * else `false`.
 */
...
/**
 * Checks if nodes `a` and `b` are are equivalent, defined as such if they
 * have the same symbol and identical children (determined recursively).
 *
 * @private
 * @static
 * @param {Object} a The node to compare.
 * @param {Object} b The other node to compare.
 * @returns {boolean} Returns `true` if the nodes are equivalent, else
 * `false`.
 */
 ...
 * @param {number} nextSymCount The number of symbols in the previous path's
...
function appendSemantic(prevSemantic, nextSymCount, ruleProps) {
	...
	if (ruleProps.semantic) {
		...
		if (ruleProps.semanticIsReduced) {
			if (prevSemantic) {
				if (!ruleProps.rhsCanProduceSemantic) {
					 ...
					 * parsing the RHS nodes. This enables finding and discarding
					 * semantically illegal parses earlier than otherwise.
					 */
				...
				/**
				 * Check if reducing `ruleProps.semantic` with LHS `prevSemantic`
				 * and merging the resulting semantic with RHS `prevSemantic.prev`
				 * will produce an illegal semantic.
				 */
		 ...
		 *
		 * Discard new LHS semantic if `prevSemantic` is completely reduced
		 * (i.e., RHS), is identical to `ruleProps.semantic`, and multiple
		 * instances of the semantic are forbidden.
		 */
...
/**
 * Appends a terminal rule's RHS semantics, if any, to the previous path's
 * semantic list, and then reduces the path's semantic tree up to the first
 * incompletely reduced node.
 *
...
function reduceSemanticTree(prevSemantic, nextSymCount, newRHSSemantic) {
	...
	while (prevSemantic) {
		...
		// follows the semantic.
			...
			// A semantic function without arguments - currently can only be
			// `intersect()`.
		...
		/**
		 * Check if reducing `newRHSSemantic` with LHS `prevSemantic` and
		 * merging the resulting semantic with RHS `prevSemantic.prev` will
		 * produce an illegal semantic.
		 */
...
/**
 * Checks if reducing `newRHSSemantic` with LHS `prevSemantic` and merging the
 * resulting semantic with RHS `prevSemantic.prev` will produce an illegal
 * semantic.
 *
 * LHS `prevSemantic` can not reduce with `newRHSSemantic` because another
 * semantic can come and merge with `newRHSSemantic` as a RHS array. If
 * `prevSemantic` has a `maxParams` of `1`, and will be copied for each
 * semantic within the semantic array to contain `newRHSSemantic`, then checks
 * if that semantic will be illegal when merged with `prevSemantic.prev`.
 *
 * Example:
...
function isIllegalSemanticReduction(prevSemantic, newRHSSemantic) {
	...
	// Check if `prevSemantic` will be copied for each of its RHS semantics
	// (because it has a `maxParams` of `1`), and `prevSemantic.prev` is
	// reduced.
	if (prevSemanticNode.semantic.maxParams === 1 && prevSemantic.prev && prevSemantic.prev.isRHS) {
		...
		for (var s = 0; s < prevPrevSemanticArrayLen; ++s) {
			...
			// Check if `prevSemantic` and `prevSemantic.prev` have the same root
			// semantic function.
			if (prevSemanticNode.semantic === prevPrevSemanticNode.semantic) {
				/**
				 * Check if `prevSemantic` + `newRHSSemantic` and
				 * `prevSemantic.prev` will be semantically illegal.
				 *
				 * If not, can not reduce `prevSemantic` + `newRHSSemantic`, merge
				 * with `prevSemantic.prev`, and copy `prevSemantic` for the future
...
/**
 * Completely reduces a semantic tree irrespective of parsing state and
 * semantic argument requirements. Used to print the semantics of a pair of
 * ambiguous paths, which are not always completely reduced.
 *
 * @private
 * @static
 * @param {Object} semanticList The path's semantic linked list to
 * forcefully reduce.
 * @param {Object} path The path of `semanticList` for error reporting.
 * @returns {Object[]|number|undefined} Returns a semantic array of the
 * reduced semantic tree if parsing succeeds, `-1` if there is a semantic
 * error, or `undefined` if there is no `semanticList` to begin with.
 */
function completeSemanticTree(semanticList, path) {
	...
	// Forcefully reduce the semantic tree irrespective of whether RHS semantics
	// have been reduced.
	while (semanticList) {
		if (semanticList.isRHS) {
			...
			// Throw an exception if RHS is semantically illegal to report illegal
			// semantics that should be detected and discarded earlier.
			if (prevSemantic === -1) {
			...
			/**
			 * Discard if the RHS semantic is fewer semantic arguments than the
			 * LHS semantic's `minParams` requires, which is due to not completing
			 * the parse. Check this before `semantic.reduce()` which throws an
	...
	/**
	 * Sort semantics. This normally occurs in `semantic.reduce()` when
	 * parsing, however, here the necessary LHS semantic might occur before
	 * the paths' root nonterminal symbol. Hence, this semantic tree can be
	 * multiple RHS semantics.
	 */
 ...
 *
 * `text` can be a `string` not needing conjugation, an `Object` containing
 * a term's inflected forms, or an `Array` of text strings and conjugative
 * text objects.
 *
 * If `text` is an `Array`, then it is being conjugated after completing an
 * insertion rule's single branch which determines the person-number
 * required for the nominative case in the second (inserted) branch (verb
 * precedes subject).
 *
 * Conjugation will fail when the root symbol of `path` occurs after where
 * the necessary person-number property occurs, then returns `textObj`. This
 * does not occur in a normal parse which begins at the start symbol.
 *
 * @private
 * @static
 * @param {Object|string|Array} text The rule's text string not needing
 * conjugation, text object to conjugate, or array of text strings and
 * objects to conjugate.
 * @param {Object} personNumberList The reverse linked list of person-number
 * properties for conjugating `text`.
 * @param {Object} [gramProps] The previous path's (i.e., the parent
 * nonterminal rule's) grammatical properties (`case`, `tense`, and
 * `acceptedTense`) for conjugating `text`.
 * @param {string} [inputTense] The terminal symbol's `tense` property to
 * check against the previous path's (i.e., the parent nonterminal rule's)
 * `acceptedTense` property and determine if the input terminal symbol is an
 * (optionally) acceptable form of the associated verb, `text`.
 * @returns {Object|string|Array} Returns `text`, conjugated if the
 * necessary person-number property exists.
 */
function conjugateText(text, personNumberList, gramProps, inputTense) {
	...
	if (textConstructor === Object) {
		// Conjugate `text`, which contains a term's inflected forms, if the
		// necessary grammatical property exists.
		return conjugateTextObject(text, personNumberList, gramProps, inputTense)
	}

	// Conjugate `text`, which is an insertion's array of text strings and
	// conjugative text objects. This array will never contains other arrays.
	var textArray = []

	for (var t = 0, textLen = text.length; t < textLen; ++t) {
		// Do not pass `gramProps` or `inputTense`, which are only for terminal
		// rules (i.e., not insertions).
		textArray.push(conjugateText(text[t], personNumberList))
	}

	// This array is flattened in `concatTextAndNextSyms()` (which yields fewer
	// operations than if concatenated to `path.text` here).
	return textArray
}

/**
 * Conjugates `textObj` to a term's correct inflection according to the
 * parent rule's grammatical properties or the path's previous person-number
 * property.
 *
 * Conjugation will fail when the root symbol of `path` occurs after where
 * the necessary person-number property occurs, then returns `textObj`. This
 ...
 * @private
 * @static
 * @param {Object} textObj The text object, containing a term's different
 * inflected forms, to conjugate.
 * @param {Object} personNumberList The reverse linked list of person-number
 * properties for conjugating `textObj`.
 * @param {Object} [gramProps] The previous path's (i.e., the parent
 * nonterminal rule's) grammatical properties (`case`, `tense`, and
 * `acceptedTense`) for conjugating `textObj`.
 * @param {string} [inputTense] The terminal symbol's `tense` property to
 * check against the previous path's (i.e., the parent nonterminal rule's)
 * `acceptedTense` property and determine if the input terminal symbol is an
 * (optionally) acceptable form of the associated verb, `textObj`.
 * @returns {Object|string} Returns the conjugated string if the necessary
 * person-number property exists, else `textObj`.
 */
function conjugateTextObject(textObj, personNumberList, gramProps, inputTense) {
	if (gramProps) {
		...
		// Must occur before person-number check, otherwise "[have] [like]" yields
		// "have like" instead of "have liked".
		var tense = gramProps.tense
		if (tense && textObj[tense]) {
			return textObj[tense]
		}

		// If the input terminal symbol is of a tense which the parent rule
		// optionally accepts, then accept it. E.g., "(repos I) like/liked".
	...
	if (!personNumberList) {
		// No conjugation possible because the path's root symbol occurs after
		// where the necessary person-number property occurs.
		return textObj
	}

	// Conjugate for required person-number. E.g., "(repos I) like", "(repos
	// Danny) likes".
...
/**
 * Removes the transposition rules from the grammar's rules, `ruleSets`.
 *
 * Do not inspect transpositions because any ambiguity they create is
 * evident in the original rules from which they were derived.
 *
 * @private
 * @static
...
/**
 * Sorts and prints found instances of ambiguity. The sorting enables better
 * comparisons of output from multiple runs.
 *
 * @param {Object[]} ambiguityToPrint The instances of ambiguity
 * (accumulated in `findAmbiguity()`) to print.
 */
function printFoundAmbiguity(ambiguityToPrint) {
	ambiguityToPrint.map(JSON.stringify).sort().map(JSON.parse).forEach(function (ambigObj) {
		util.log(util.colors.yellow('Ambiguity') + ':', util.stylize(ambigObj.terminals))
		util.log('  Text:', ambigObj.textA)
		util.log('  Semantic:', ambigObj.semanticA)
		util.dir('  ', ambigObj.treeA)
		util.log()
		util.log('  Text:', ambigObj.textB)
		util.log('  Semantic:', ambigObj.semanticB)
		util.dir('  ', ambigObj.treeB)
		util.log()
	})
}