...
function searchPaths(nontermSym) {
	...
	var rootPath = {
		...
		nextSym: undefined,
		// The reverse linked list of the second symbols of previously incomplete binary rules and yet-to-conjugate text objects from insertion rules. After reaching a terminal symbol, inspect `nextItems` to complete the binary rules and conjugate the text objects.
		nextItems: undefined,
		// The number of symbols in the `nextItems` that can produce a semantic. This excludes other symbols and yet-to-conjugate text. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
		nextSymsCount: 0,
		// The reverse linked list of yet-to-reduce semantics.
		semanticList: undefined,
		// The path's display text.
		text: [],
		// The reverse linked list of grammatical properties for yet-to-conjugate text in `nextItems`.
...
function createPath(prevPath, rule) {
	...
	if (!rule.isTerminal) {
		...
		if (rule.insertionIdx === undefined) {
			if (rule.RHS.length === 2) {
				// If rule is binary, add the second RHS symbol to the list of `nextItems`. The rule will resume after completing the branch produced by the first RHS symbol.
				newPath.nextItems = {
					sym: rule.RHS[1],
					canProduceSemantic: rule.secondRHSCanProduceSemantic,
					prev: prevPath.nextItems,
				}
	...
	// Terminal rule.
		...
		var nextItems = newPath.nextItems
		while (nextItems) {
			var text = nextItems.text
		...
		if (nextItems) {
			...
			newPath.nextSym = nextItems.sym
			newPath.nextItems = nextItems.prev
			...
			if (nextItems.canProduceSemantic) {
				--newPath.nextSymsCount
			}
		} else {
			// No symbols remain.
			// `newPath.nextSymsCount` equals 0.
			newPath.nextItems = undefined
		}
	}
		...
		for (var b = a + 1; b < pathTabLen; ++b) {
			...
			for (var terminals in pathSetsA) {
				...
				for (var p = 0, pathsALen = pathsA.length; p < pathsALen; ++p) {
					...
					var nextItems = pathA.nextItems

					for (var o = 0; o < pathsBLen; ++o) {
						var pathB = pathsB[o]

						// `terminals`, `nextSym`, and `nextItems` together are the rightmost symbols of the root symbol. If no nonterminal symbols remain to be reduced (as is always true with `--complete-trees`), then the rightmost symbols form what would be the parse input.
						if (pathB.nextSym === nextSym && nextSymsEqual(pathB.nextItems, nextItems)) {
...
/**
 * Compares two paths' `nextItems` linked lists of symbols and yet-to-conjugate text, ignoring the instances of text, to determine if the symbols are equivalent.
 *
 * @param {Object} a The `nextItems` list to compare.
 * @param {Object} b The other `nextItems` list to compare.
...
/**
 * Compares two paths' `nextItems` linked lists of symbols and yet-to-conjugate text, ignoring the instances of symbols, to determine if the text values are equivalent.
 *
 ...
 * @param {number} nextSymsCount The number of symbols in the previous path's `nextItems` that can produce a semantic. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
