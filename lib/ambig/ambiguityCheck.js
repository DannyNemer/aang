...
function searchPaths(nontermSym) {
	...
	var rootPath = {
		...
		curSym: undefined,
		// The linked list of the second nodes of previously incomplete binary rules and yet-to-conjugate text objects from insertion rules. When `path.curSym` is `undefined` after reaching a terminal symbol, inspect `nextItemList` to complete the binary rules and conjugate the text objects.
...
function buildPath(pathSets, prevPath, rule) {
	...
	if (argv.semanticCheck && completeSemanticTree(newPath.semanticList) === -1) {
		logSemanticError(newPath)
		return
	}
...
function createPath(prevPath, rule) {
	...
	// Terminal rule.
		...
		// In `pfsearch`, this step occurs right before expanding a path instead of when the terminal rule is reached, as here, to avoid work on paths whose cost prevents them from ever being popped from the min-heap.
					...
					for (var o = 0; o < pathsBLen; ++o) {
						...
						if (pathB.curSym === curSym && nextSymsEqual(pathB.nextItemList, nextItemList)) {
							...
							if (!pathA.textAndSyms) {
								...
								pathA.semanticTree = completeSemanticTree(pathA.semanticList)

								// Discard if semantic is illegal.
								if (pathA.semanticTree === -1) {
									logSemanticError(pathA)
									continue
								}
							...
							if (!pathB.textAndSyms) {
								...
								pathB.semanticTree = completeSemanticTree(pathB.semanticList)

								// Discard if semantic is illegal.
								if (pathB.semanticTree === -1) {
									logSemanticError(pathB)
									continue
								}
							...
							// Check if semantic trees are defined and identical. Some ambiguity instances are correctly found when considering a pair of `undefined` semantics equivalent; however, this also reports incorrect instances. The former instances can be found by increasing `--tree-syms-limit`.
							var semanticsEquivalent = pathA.semanticTree !== undefined && semantic.arraysEqual(pathA.semanticTree, pathB.semanticTree)
 ...
 * @param {Object} prevSemantic The previous path's semantic linked list.
 * @param {number} nextSymsCount The number of symbols in the previous path's `nextItemList` that can produce a semantic. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
 * @param {Object[]} [newSemantic] The terminal rules' RHS semantic.
...
function reduceSemanticTree(prevSemantic, nextSymsCount, newSemantic) {
	...
	while (prevSemantic) {
		...
		// Stop at a LHS semantic whose parse node has yet-to-reduce child nodes.
			...
			// LHS `prevSemantic` cannot reduce with `newSemantic` because another semantic can come and merge with `newSemantic` as a RHS array. If `prevSemantic` has a `maxParams` of `1`, and will be copied for each semantic within the semantic array to contain `newSemantic`, then check if that semantic will be illegal when merged with `prevSemantic.prev`.
			//
			...
			if (newSemantic) {
				...
				if (prevSemanticNode.semantic.maxParams === 1 && prevSemantic.prev && prevSemantic.prev.isRHS) {
					var prevPrevSemanticNode = prevSemantic.prev.semantic[0]
					...
					if (prevSemanticNode.semantic === prevPrevSemanticNode.semantic) {
						// Check if `prevSemantic` + `newSemantic` and `prevSemantic.prev` will be semantically illegal.
						// If not, can not reduce `prevSemantic` + `newSemantic`, merge with `prevSemantic.prev`, and copy `prevSemantic` for the future semantics, as will eventually happen. This is because it is possible no semantics will come down this branch, which cannot be determined in advance, and the copied function will remain illegally empty.
						if (semantic.isIllegalRHS(prevPrevSemanticNode.children, newSemantic)) {
...
/**
 * Prints an error for a path with illegal semantics when forcefully and completely reduced (i.e., reduced irrespective of parsing state and semantic argument requirements). This exposes illegal semantics that should be detected and discarded earlier.
 *
 * @param {Object} path The path for which to print an error.
 */
function logSemanticError(path) {
