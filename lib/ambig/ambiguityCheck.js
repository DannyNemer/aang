...
function searchPaths(nontermSym) {
	...
	var rootPath = {
		...
		nextSym: undefined,
		// The reverse linked list of the second symbols of previously incomplete binary rules and yet-to-conjugate text objects from insertion rules. After reaching a terminal symbol, inspect `nextItemList` to complete the binary rules and conjugate the text objects.
...
function createPath(prevPath, rule) {
	...
	// Terminal rule.
		...
		while (nextItemList) {
			...
			nextItemList = nextItemList.prev
		}

		if (nextItemList) {
			// Get the second symbol of the most recent incomplete binary rule.
			newPath.nextSym = nextItemList.sym
			newPath.nextItemList = nextItemList.prev
			...
			for (var terminals in pathSetsA) {
				...
				for (var p = 0, pathsALen = pathsA.length; p < pathsALen; ++p) {
					...
					for (var o = 0; o < pathsBLen; ++o) {
						...
						// `terminals`, `nextSym`, and `nextItemList` together are the rightmost symbols of the root symbol. If no nonterminal symbols remain to be reduced (as is always true with `--complete-trees`), then the rightmost symbols form what would be the parse input.
						if (pathB.nextSym === nextSym && nextSymsEqual(pathB.nextItemList, nextItemList)) {
