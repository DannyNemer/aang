...
/**
 * Checks for ambiguity created by `nontermSym`'s rules. Compares paths created by each rule from `nontermSym` to paths created by `nontermSym`'s other rules. Does not compare paths that the same initial rule produces because if ambiguity exists there, then it is caused by another symbol.
 *
...
function searchPaths(nontermSym) {
	...
	// Exit if `nontermSym` produces only one (non-transposition) rule because at least two rules are required for ambiguity to exist.
	...
	var rootPath = {
		...
		curSym: undefined,
		// The linked list of yet-to-parse second nodes of previous binary rules and conjugative text objects of previous insertion rules. When `curSym` is `undefined` after reaching a terminal symbol, inspect `nextItemList` to complete the binary rules and conjugate the text objects.
	...
	for (var r = 0; r < rulesLen; ++r) {
		// Paths each root rule of `nontermSym` produces are stored in separate sets of paths. Those sets map strings of terminal symbols to arrays of paths.
		var pathSets = {}
...
/**
 * Recursively constructs all possible expansions of `prevPath` using `rule`, which its rightmost nonterminal symbol produced. Adds the new paths to `pathSets` for use by `findAmbiguity()` to search for ambiguous pairs.
 *
 * @private
 * @static
...
function createPath(prevPath, rule) {
	...
	if (!rule.isTerminal && !rule.rhsDoesNotProduceText) {
		...
		// The grammatical properties (`case`, `tense`, and `acceptedTense`) used to conjugate terminal rules `newPath.curNode` (and its sibling node if binary) produces. Only occurs on nonterminal rules.
		...
		if (rule.insertedSymIdx === undefined) {
			if (rule.rhs.length === 2) {
				// All binary rules are nonterminal rules. Prepend the second RHS symbol to `nextItemList`, and complete the rule after completing the branch the first RHS symbol produces.
				if (prevNextItemList) {
					newPath.nextItemList = {
						// The second symbol of this binary rule to parse after completing the first symbol's branch.
						sym: rule.rhs[1],
						// The path's grammatical properties (`case`, `tense`, and `acceptedTense`) used to conjugate terminal rules `node` produces.
						gramProps: newPath.gramProps,
						// The number of symbols in the `nextItemList` that can produce a semantic. This excludes other symbols and conjugative text. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
 ...
 * Ambiguity exists if multiple paths exist to the same rightmost symbols, and the resulting semantic trees and/or display texts are indistinguishable. If ambiguity is found among a pair of paths, prints the parse trees, semantic trees, and display texts for those paths to show the necessary changes to make to the grammar
 *
 * Compares paths that different root rules produces, where `nontermSym` is the root symbol. Does not compare paths from the same root rule, because any ambiguity that exists there is caused by a symbol other than this root symbol.
 *
 ...
 * @private
 * @static
 * @param {Object} pathTab The set of paths a single nonterminal symbol produces.
 */
				...
				for (var p = 0, pathsALen = pathsA.length; p < pathsALen; ++p) {
					...
					for (var o = 0; o < pathsBLen; ++o) {
						...
						if (pathB.curSym === curSym && nextSymsEqual(pathB.nextItemList, nextItemList)) {
							...
							// Check if display texts, including conjugative text objects and insertion texts of yet-to-reduce rules, are identical.
							...
							if (semanticsEquivalent || textsEquivalent) {
								...
								if (!argv.findAll || !pairExists(ambigPairs, treeA, treeB)) {
									if (!argv.quiet) {
										// Accumulate instances of ambiguity in `ambiguityToPrint` to allow for sorting all results before printing, which enables better comparisons of output from multiple runs.
...
/**
 * Checks if `nextItemList` lists `a` and `b` are equivalent by comparing the linked lists of symbols and conjugative text, ignoring the instances of text.
 *
 * @private
 * @static
...
/**
 * Compares two paths' `nextItemList` linked lists of symbols and conjugative text, ignoring the instances of symbols, to determine if the text values are equivalent.
 *
...
function concatTextAndNextSyms(path) {
	...
	// Merge adjacent strings. Can not use `Array.prototype.join()` because `items` can contain conjugative text objects.
...
/**
 * Inverts a parse tree `rulesToTree()` produced to an array of rightmost nodes with pointers to their parent nodes, ending at the tree's root node.
 *
 * @private
 * @static
...
/**
 * Checks if nodes `a` and `b` are are equivalent, defined as such if they have the same symbol and identical children (determined recursively).
 *
 * @private
 * @static
 ...
 *
 * LHS `prevSemantic` can not reduce with `newRHSSemantic` because another semantic can come and merge with `newRHSSemantic` as a RHS array. If `prevSemantic` has a `maxParams` of `1`, and will be copied for each semantic within the semantic array to contain `newRHSSemantic`, then checks if that semantic will be illegal when merged with `prevSemantic.prev`.
 *
 * Example:
 ...
 *
 * `text` can be a `string` not needing conjugation, an `Object` containing a term's inflected forms, or an `Array` of text strings and conjugative text objects.
 *
 ...
 * @private
 * @static
 * @param {Object|string|Array} text The rule's text string not needing conjugation, text object to conjugate, or array of text strings and objects to conjugate.
 ...
 * @returns {Object|string|Array} Returns `text`, conjugated if the necessary person-number property exists.
 */
function conjugateText(text, personNumberList, gramProps, inputTense) {
	...
	// Conjugate `text`, which is an insertion's array of text strings and conjugative text objects. This array will never contains other arrays.
	var textArray = []
 ...
 * @returns {Object|string} Returns the conjugated string if the necessary person-number property exists, else `textObj`.
 */
...
/**
 * Sorts and prints found instances of ambiguity. The sorting enables better comparisons of output from multiple runs.
 *
 * @param {Object[]} ambiguityToPrint The instances of ambiguity (accumulated in `findAmbiguity()`) to print.
 */
