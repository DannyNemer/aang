 ...
 *                          a greater `--tree-sym-limit` value than otherwise to find the same
...
function buildPath(pathSets, prevPath, rule) {
	...
	if (newPath.curSym && newPath.symsCount < argv.treeSymLimit) {
...
function createPath(prevPath, rule) {
		...
		if (rule.insertedSymIdx === undefined) {
			if (rule.RHS.length === 2) {
				...
				newPath.nextItemList = {
					...
					symCount: prevNextItemList ? prevNextItemList.symCount + Number(rule.secondRHSCanProduceSemantic) : Number(rule.secondRHSCanProduceSemantic),
		...
		// Insertion rule.
			...
			if (rule.insertedSymIdx === 1) {
				...
				newPath.nextItemList = {
					...
					symCount: prevNextItemList ? prevNextItemList.symCount : 0,
					// The number of items in `nextItemList`.
					size: prevNextItemList ? prevNextItemList.size + 1 : 1,
					// The next item that follows after completing this branch, created from the previous binary or insertion rule.
					next: prevNextItemList,
	...
	// Terminal rule.
		...
		if (nextItemList) {
			...
			// The path's grammatical properties (`tense`, `gramCase`, and `acceptedTense`) used to conjugate the second symbol of this binary rule, if applicable (i.e., a terminal rule).
