...
/**
 * Checks if `nextItemList` lists `a` and `b` are equivalent by comparing the linked lists of symbols and yet-to-conjugate text, ignoring the instances of text.
 *
 * @private
 * @static
...
function concatTextAndNextSyms(path) {
	...
	// Merge adjacent strings. Can not use `Array.prototype.join()` because `items` can contain yet-to-conjugate text objects.
 ...
 *
 * Later, when the size of the path's `nextItemList` is greater than or equal to the size associated with a property (following a branch's completion), then that property's subtree has completed and the person-number properties which followed it (via sub-nodes) are removed. That initial property remains at the front of `personNumberList` and can conjugate subsequent verbs outside its subtree (as intended).
 *
 * @private
 * @static
...
function unwindPersonNumberList(personNumberList, nextItemListSize) {
	...
	while (personNumberItem) {
		// If the current size of the path's `personNumberList` is greater than or equal to the size when at a person-number property's definition position in the parse tree, then the property's subtree has completed and the person-number properties which followed it (via sub-nodes) are removed. This person-number property remains and can conjugate subsequent verbs outside its subtree (as intended).
