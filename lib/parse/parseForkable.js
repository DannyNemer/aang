 ...
 *   benchmark and the parse results to the parent process, and then exits. This
 *   program must be run as a forked child process.
 *
...
var argv = yargs
	.usage([
		...
		'  Parses <query> and finds the k-best parse trees, sends a process duration benchmark and the parse results to the parent process, and then exits. This program must be run as a forked child process.',
		'',
	...
	.check(function (argv, options) {
		if (!process.connected) {
			throw 'Error: ' + argv.$0 + ' not ran as a forked child process'
		}
...
var parseResults = parse(query, argv.k)

// End timer.
var durationTuple = process.hrtime(startTime)
var duration = durationTuple[0] * 1e3 + durationTuple[1] / 1e6

// Send benchmark measurement and parse results to parent process, and then exit.
process.send({
	duration: duration,
