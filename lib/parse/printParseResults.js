...
function checkTree(tree, nextTree, options) {
	...
	checkSemanticParams(tree.semanticStr, tree, options)
	if (tree.ambiguousSemantics) {
		tree.ambiguousSemantics.forEach(function (semanticStr) {
			checkSemanticParams(semanticStr, tree, options)
		})
	}
}
 ...
 * @param {Object} options The module options object.
 */
function checkSemanticParams(semanticStr, tree, options) {
	checkEachSemanticNode(semanticStr, tree, options, 'illegal number of semantic arguments', function (semanticNode) {
		var semanticDef = semanticNode.semantic

		if (!semanticDef.isArg) {
