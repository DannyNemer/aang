 ...
 * @param {boolean} [options.quiet=false] Specify suppressing all output except errors for failed parse tree checks.
...
module.exports = function (parseResults, options) {
	...
	for (var t = 0, treesLen = trees.length; t < treesLen; ++t) {
		...
		if (!options.quiet) {
			// Print `tree` display text, semantic representation, and tree graph representation (if `options.trees` is `true`).
