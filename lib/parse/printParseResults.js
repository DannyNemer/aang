...
module.exports = function (trees, options) {
	...
	for (var t = 0, treesLen = trees.length; t < treesLen; ++t) {
		...
		if (t < treesLen - 1 && util.cleanFloat(tree.cost) > util.cleanFloat(trees[t + 1].cost)) {
			util.logError('Parse trees are not sorted by increasing cost:')
		...
		if (tree.cost !== tree.minCost) {
			util.logError('Costs incorrect:', 'cost: ' + tree.cost + ', minCost: ' + tree.minCost)
			util.log(tree.text)
			if (options.trees) util.dir(linkedListToGraph(tree, options))
			throw 'Costs error'
		}
		...
		if (/undefined/.test(tree.text)) {
			...
			util.logError('Unconjugated text:', text)
			if (options.trees) util.dir(linkedListToGraph(tree, options))
		...
		// Binary nonterminal rule.
			...
			nodesStack.push({
				...
				children: [
					...
					formatNode(path.nextItemList.node, options, nodesStack.pop(), path.nextItemList.minCost)
				],
			})
		}
