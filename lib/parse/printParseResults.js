 ...
 * @param {boolean} [options.quiet=false] Specify suppressing parse trees from output. If `true`, neither prevents parse tree checks nor `options.stats`.
...
var printOptionsSchema = {
	// Specify suppressing parse trees from output. If `true`, neither prevents parse tree checks nor `options.stats`.
	quiet: Boolean,
	...
	noSemantics: Boolean,
	// The input query that produced `parseResults.trees`, with which to stylize its differences with the top parse tree's display text.
...
/**
 * Prints `tree` (output by `pfsearch`).
 *
