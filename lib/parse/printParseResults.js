...
var printOptionsSchema = {
	...
	// The input query that produced `trees`, with which to stylize its differences with the top parse result's display text.
...
module.exports = function (trees, options) {
	...
	for (var t = 0, treesLen = trees.length; t < treesLen; ++t) {
		...
		if (t === 0 && options.diffInputQuery !== undefined && options.diffInputQuery !== tree.text) {
			var diff = util.diffStrings(options.diffInputQuery, tree.text)
			util.log(diff.expected)
			tree.text = diff.actual
		}
