...
/**
 * Prints parse results output by `Parser.prototype.parse()`, which includes the k-best parse trees output by `pfsearch`.
 *
 * @static
...
function checkTree(tree, nextTree, options) {
	...
	// Check for semantic arguments in `intersect()`, `union()`, and `not()`.
...
/**
 * Checks for semantic arguments in `intersect()`, `union()`, and `not()`, which is forbidden.
 *
 * Accepts a semantic string to convert to the original object array format instead of accepting the original semantic already in that format because the original format of the ambiguous semantics are not stored.
 *
 * @param {string} semanticString The semantic string to check.
 * @param {Object} tree The parse tree owner of `semanticString`.
 * @param {Object} options The module options object.
 */
function checkForbiddenSemanticArgs(semanticString, tree, options) {
	var semanticArray = semantic.stringToObject(semanticString)
	...
	for (var s = 0, stackLen = semanticStack.length; s < stackLen; ++s) {
		...
		// Check if a semantic argument exists in `intersect()`, `union()`, or `not()`.
		if (/\b(intersect|union|not)\b/.test(semanticNode.semantic.name)) {
			for (var c = 0, semanticChildrenLen = semanticChildren.length; c < semanticChildrenLen; ++c) {
				var semanticChildNode = semanticChildren[c]

				if (!semanticChildNode.children) {
					// Color violating semantic argument red for printing.
					semanticChildNode.semantic.name = util.colors.red(semanticChildNode.semantic.name)

					// Save violating semantic function names to indicate in error message.
					// Strip semantic name of any color when checking if the semantic was already identified because the violating semantics are also colorized for printing.
					if (semanticViolators.indexOf(util.colors.stripColor(semanticNode.semantic.name)) === -1) {
						semanticViolators.push(semanticNode.semantic.name)

						// Color violating semantic function cyan for printing.
						semanticNode.semantic.name = util.colors.cyan(semanticNode.semantic.name)
					}
				}
			}
		}

		// Check every semantic child.
		if (semanticChildren) {
	...
	if (semanticViolators.length > 0) {
		...
		util.logError('\n', semanticViolatorNames, semanticViolators.length === 1 ? 'contains' : 'contain', 'semantic arguments:')
