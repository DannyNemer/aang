	...
	for (var s = 0, stackLen = semanticStack.length; s < stackLen; ++s) {
		...
		if (/\b(intersect|union|not)\b/.test(semanticNode.semantic.name)) {
			...
			if (semanticHasArgs) {
				...
				var semanticName = thisSemantic.name

				// Save violating semantic function name to indicate in error message.
				...
				thisSemantic.name = util.colors.cyan(semanticName)
			}
		}
 ...
 * @param {Object} options The module options object.
 */
function checkSemanticOperatorsMinParams(semanticString, tree, options) {
	var semanticArray = semantic.stringToObject(semanticString)
	var semanticStack = semanticArray.slice()
	var semanticViolators = []
	...
	for (var s = 0, stackLen = semanticStack.length; s < stackLen; ++s) {
		...
		// Check instances of `intersect()` and `union()` have at least two arguments.
		if (/\b(intersect|union)\b/.test(semanticNode.semantic.name)) {
			if (semanticChildren.length < 2) {
				...
				// Colorize violating semantic function red for printing.
				thisSemantic.name = util.colors.red(semanticName)
			}
		}
	...
	if (semanticViolators.length > 0) {
		...
		util.logError('\n', semanticViolatorNames, semanticViolators.length === 1 ? 'contains' : 'contain', 'less than 2 semantic arguments:')
		util.log('  ', semantic.toString(semanticArray))
