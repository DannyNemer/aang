 ...
 *
 * Creates a min heap data structure created using a binary tree. All nodes are
 * less than or equal to each of its two children.
 *
 * @constructor
 */
function BinaryHeap() {
...
BinaryHeap.prototype.push = function (node) {
	...
	while (nodeIndex > 0) {
		...
		// Set the parent node to the new node's previous index and repeat the
		// previous step.
 ...
 * 2. If the new root's value is less than or equal to the value of both of its
 *    children, stop.
 * 3. If not, swap the node with its child of smallest value and return to the
 *    previous step.
 *
 * @memberOf BinaryHeap
...
BinaryHeap.prototype.pop = function () {
	...
	if (length > 0) {
		...
		while (true) {
			...
			if (child1Index < length) {
				...
				if (child1Cost < nodeCost) {
					...
					if (child2Index < length) {
						...
						// If the second child's value is less than the node's
						// value and the first child's value, swap its index
						// with the node's.
					...
					// If the first child's value is less than the node's value
					// and the second child's value, swap its index with the
					// node's.
					...
					// If the second child's value is less than the node's
					// value, swap its index with the node's.
			...
			// Set the formerly last node in the heap at the index where its
			// value is less than or equal to the value of both of its
			// children, and stop.
			this.content[nodeIndex] = node
			break
		}
