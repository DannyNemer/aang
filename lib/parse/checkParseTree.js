...
module.exports = function (tree, nextTree, options) {
	...
	// Check there are no multiple instances of a semantic marked `forbidsMultiple` within an `intersect()`.
...
/**
 * Checks for multiple instances of a semantic marked `forbidsMultiple` within an `intersect()`. If found, throws an exception.
 *
 * @private
 * @static
...
function checkForbiddenMultiple(semanticArray, tree, options) {
	checkEachSemanticNode(semanticArray, tree, options, 'multiple instances of a semantic marked `forbidsMultiple`', function (semanticNode) {
		// Check for multiple instances of a semantic marked `forbidsMultiple` within an `intersect()`.
		if (semanticNode.semantic.name === 'intersect') {
			...
			for (var a = 0, semanticChildrenLen = semanticChildren.length; a < semanticChildrenLen; ++a) {
				...
				if (childSemanticDef.forbidsMultiple) {
					for (var b = a + 1; b < semanticChildrenLen; ++b) {
						var childSemanticNodeB = semanticChildren[b]

						// Check for multiple instance of `childSemanticDef`, which is marked `forbidsMultiple`.
