...
/**
 * Checks `tree` (output by `pfsearch`) for errors. If found, throws an
 * exception.
 *
 ...
 * @param {Object|undefined} nextTree The next parse tree within the set, if
 * any (to check sorting).
...
module.exports = function (tree, nextTree, options) {
	...
	// Use `util.cleanFloat()` to handle cost differences due to JavaScript
	...
	// Check the number of semantic arguments conforms to the parameter bounds
	// defined in the grammar.
	checkSemanticParams(tree.semanticList.semantic, tree, options)

	// Check there are no multiple instances of a semantic marked
	// `forbidsMultipleIntersection` within an `intersect()`.
	checkForbiddenMultiple(tree.semanticList.semantic, tree, options)

	// Check instances of `intersect()`, `union()`, and `not()` do not contain
	// non-function semantic arguments (e.g., entities, `me`).
...
/**
 * Checks for multiple instances of a semantic marked
 * `forbidsMultipleIntersection` within an `intersect()`. If found, throws an
 * exception.
 *
 * @private
 * @static
...
function checkForbiddenMultiple(semanticArray, tree, options) {
	checkEachSemanticNode(semanticArray, tree, options, 'multiple instances of a semantic marked `forbidsMultipleIntersection`', function (semanticNode) {
		// Check for multiple instances of a semantic marked
		// `forbidsMultipleIntersection` within an `intersect()`.
		if (semanticNode.semantic.name === 'intersect') {
			...
			for (var a = 0, semanticChildrenLen = semanticChildren.length; a < semanticChildrenLen; ++a) {
				...
				if (childSemanticDef.forbidsMultipleIntersection) {
					for (var b = a + 1; b < semanticChildrenLen; ++b) {
						...
						// Check for multiple instance of `childSemanticDef`,
						// which is marked `forbidsMultipleIntersection`.
						if (childSemanticNodeB.semantic === childSemanticDef) {
							/**
							 * Clone and colorize violating child semantic
							 * within `intersect()` for printing, if not
							 * already colorized. (Cloning is necessary because
							 * the semantic definition object is shared.)
							 *
							 * Does not impact future semantic comparisons
							 * within this iteration because this does not
							 * mutate `childSemanticDef`.
							 */
			...
			if (hasForbiddenMultiple) {
				// Clone and colorize violating `intersect()` semantic function
				// for printing.
...
/**
 * Checks the number of semantic arguments within each semantic conforms to
 * that semantic's parameter bounds defined in the grammar. If not, throws an
 * exception.
 *
 * @private
 * @static
	...
	checkEachSemanticNode(semanticArray, tree, options, 'illegal number of semantic arguments', function (semanticNode) {
		if (semanticNode.children) {
			...
			if (childrenLen < semanticDef.minParams || childrenLen > semanticDef.maxParams) {
				...
				// (Cloning is necessary because the semantic definition object
				// is shared.)
				semanticNode.semantic = {
...
/**
 * Checks instances of `intersect()`, `union()`, and `not()` do not contain
 * non-function semantic arguments (e.g., entities, `me`). If found, throws an
 * exception.
 *
 * @private
 * @static
...
function checkForbiddenSemanticArgs(semanticArray, tree, options) {
	checkEachSemanticNode(semanticArray, tree, options, 'non-function semantic arguments', function (semanticNode) {
		// Check for non-function semantic arguments (e.g., entities, `me`) in
		// `intersect()`, `union()`, or `not()`.
		if (/\b(intersect|union|not)\b/.test(semanticNode.semantic.name)) {
			...
			for (var c = 0, semanticChildrenLen = semanticChildren.length; c < semanticChildrenLen; ++c) {
				...
				if (!childSemanticNode.children) {
					// Clone and colorize violating semantic argument for
					// printing. (Cloning is necessary because the semantic
					// definition object is shared.)
...
/**
 * Checks if `predicate` returns truthy for any semantic nodes in
 * `semanticArray`. If so, prints an error and throws an exception.
 *
 * The printed error includes the names of the semantics for which `predicate`
 * returned truthy in the format "${semanticViolator} contains
 * ${description}:", the entire semantic (preserving any colorization of
 * semantic names that occur in `predicate`), and associated `tree` properties.
 *
 * Iterates over the entire semantic, and invokes `predicate` with one
 * argument: (semanticNode).
 *
 * @private
 * @static
 ...
 * @param {Object} tree The parse tree owner of `semanticArray`.
 * @param {Object} options The module options object.
 * @param {string} description The check description included in the error
 * message, if any.
 ...
 * @param {Object} options The module options object.
 * @param {Function} customizer The function invoked after printing `tree.text`
 * (and `tree.cost` if `options.costs`) and before (printing the parse tree
 * graph if `options.trees` and) throwing an exception.
 * @param {boolean} [printSemanticTree] Specify printing the unformatted
 * semantic tree, `tree.semanticList.semantic`.
 */
