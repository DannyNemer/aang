...
function createTermSeqRuleProps(subNode, cost) {
	...
	return {
		...
		cost: cost,
		text: text,
		// If `parentGramProps` exists, do not save input tense because `parentGramProps` uses `tense` to conjugate `text` above.
...
function createTermSeqInsertionRuleProps(subNode, cost) {
	...
	return {
		 ...
		 *
		 * If `parentGramProps` exists, do not save input tense because `parentGramProps` uses `childSub.ruleProps.tense` to conjugate `text` above.
		 */
...
function getChildSub(subNode) {
	...
	if (childRuleProps.personNumber) {
	...
	// Check for the anaphoric grammatical person-number property on child subnodes because it has not yet been seen, though would be easy to support.
	if (childRuleProps.anaphoraPersonNumber) {
		util.logError('Term sequence child subnode has `anaphoraPersonNumber`:')
		util.log('  subnode:', subNode)
		util.log('  child:', childSub)
		throw new Error('Ill-formed term sequence')
	}
