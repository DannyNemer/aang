...
function createPartialTermSequenceRuleProps(subnode) {
	 ...
	 */
	var text
	var parentGramProps = subnodeRuleProps.gramProps && subnodeRuleProps.gramProps[termSequenceRHSIndex]
	if (termSequenceRHSIndex === 0 || (parentGramProps && (parentGramProps.form || (parentGramProps.acceptedTense && parentGramProps.acceptedTense === childSub.ruleProps.tense)))) {
		text = getSubnodeText(childSub, parentGramProps)
	} else {
	...
	return {
		 ...
		 * • No need to reference `subnodeRuleProps.secondRHSCanProduceSemantic` (i.e., if `termSequenceRHSIndex` is 0) because the term sequence subnode is guaranteed to not produce semantics, hence `subnodeRuleProps.rhsCanProduceSemantic` can only apply to the (remaining) non-sequence subnode.
		 */
