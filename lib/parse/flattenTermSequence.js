...
function createTermSeqRuleProps(subNode, cost) {
	...
	var text = getSubnodeText(leftSub, subNodeRuleProps.gramProps)
	...
	return {
		...
		cost: cost,
		text: text,
		// If `subNodeRuleProps.gramProps` exists, do not save input tense because `subNodeRuleProps.gramProps` uses `tense` to conjugate `text` above.
		tense: subNodeRuleProps.gramProps ? undefined : tense,
...
function createTermSeqInsertionRuleProps(subNode, cost) {
	...
	return {
		 ...
		 *
		 * If `subNodeRuleProps.gramProps` exists, do not save input tense because `subNodeRuleProps.gramProps` uses `childSub.ruleProps.tense` to conjugate `text` above.
		 */
		tense: subNodeRuleProps.gramProps ? undefined : childSub.ruleProps.tense,
		semantic: subNodeRuleProps.semantic,
	}
}
