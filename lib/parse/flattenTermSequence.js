...
function createTermSeqRuleProps(subNode, cost) {
	...
	var text = getSubnodeText(leftSub, parentGramProps)
	var tense = leftSub.ruleProps.tense
	...
	if (subNodeNext) {
		...
		var rightText = getSubnodeText(rightSub, parentGramProps)
		 ...
		 */
		if (!tense) {
			tense = rightSub.ruleProps.tense
		}
	}
	...
	return {
		...
		cost: cost,
		text: text,
		// If `parentGramProps` exists, do not save input tense because `parentGramProps` conjugated the verb in `text` to which `tense` applies above, whether matching `parentGramProps.acceptedTense` existed or not.
		tense: parentGramProps ? undefined : tense,
		semantic: subNodeRuleProps.semantic,
		/**
		 * Thus far, `personNumber` only exists for the following term sequence:
...
/**
 * Checks if `subNode`, which was passed to `createTermSeqRuleProps()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} subNode The `createTermSeqRuleProps()` term sequence subnode to inspect.
 * @returns {boolean} Returns `true` if `subNode` is ill-formed, else `false`.
 */
function isIllFormedTermSeq(subNode) {
	var subNodeRuleProps = subNode.ruleProps
...
function createTermSeqSubstitutionRuleProps(subNode, cost) {
	if (isIllFormedTermSeqSubstitution(subNode)) {
 ...
 * @returns {boolean} Returns `true` if `subNode` is ill-formed, else `false`.
 */
function isIllFormedTermSeqSubstitution(subNode) {
	var subNodeRuleProps = subNode.ruleProps
	if (!subNodeRuleProps.isTermSequence) {
...
function createTermSeqInsertionRuleProps(subNode, cost) {
	if (isIllFormedTermSeqInsertion(subNode)) {
	...
	var parentGramProps = subNodeRuleProps.gramProps
	var childSubText = getSubnodeText(childSub, parentGramProps)
	...
	return {
		 ...
		 *
		 * If `parentGramProps` exists, do not save input tense because `parentGramProps` conjugated the verb in `text` to which `childSub.ruleProps.tense` applies above, whether matching `parentGramProps.acceptedTense` existed or not.
		 */
		tense: parentGramProps ? undefined : childSub.ruleProps.tense,
		semantic: subNodeRuleProps.semantic,
	}
}

/**
 * Checks if `subNode`, which was passed to `createTermSeqInsertionRuleProps()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} subNode The `createTermSeqInsertionRuleProps()` term sequence insertion subnode to inspect.
 * @returns {boolean} Returns `true` if `subNode` is ill-formed, else `false`.
 */
function isIllFormedTermSeqInsertion(subNode) {
	var subNodeRuleProps = subNode.ruleProps
	if (!subNodeRuleProps.isTermSequence) {
...
function getCheapestChildSub(subNode) {
	...
	if (subNode.size === 1 || childSubs.every(childSub => childSub.ruleProps.cost < 1)) {
		...
		util.log('\nAmbiguous child subnodes:', ...childSubs)
		throw new Error('Ill-formed term sequence')
	}
 ...
 * @returns {string|Object} Returns the display text `subNode` produces, conjugated if possible.
 */
function getSubnodeText(subNode, parentGramProps) {
	var text = subNode.ruleProps.text
	if (text) {
		// Conjugate `text` if `parentGramProps` is defined and `subNode.ruleProps.text` is a conjugative text object or an array containing a conjugative text object.
		if (parentGramProps) {
			return conjugateText(text, parentGramProps, subNode.ruleProps.tense)
		}
...
function conjugateText(text, parentGramProps, inputTense) {
	var textConstructor = text.constructor

	// 87% (excluding array)
	if (textConstructor === Object) {
		return conjugateTextObject(text, parentGramProps, inputTense)
	}

	// Rare. for strings in arrays and where there is parentGram props, but applies to other branch
	if (textConstructor === String) {
		return text
	}

	// never has nested arrays
	var conjugatedText = ''
	for (var t = 0, textArrayLen = text.length; t < textArrayLen; ++t) {
		if (t > 0) {
			conjugatedText += ' '
		}
		conjugatedText += conjugateText(text[t], parentGramProps, inputTense)
	}
	return conjugatedText
}

/**
 * Conjugates text objects within `textArray` to strings and concatenates all included items into a string separated by spaces.
 *
 * @private
 * @static
 * @param {(Object|string)[]} textArray The display text array of invariable strings and conjugative objects to conjugate.
 * @param {Object} parentGramProps The `ruleProps.gramProps` of the parent subnode that produces the subnode that owns `textArray`.
 ...
 */
function conjugateTextArray(textArray, parentGramProps, inputTense) {
	var conjugatedText = ''
	for (var t = 0, textArrayLen = textArray.length; t < textArrayLen; ++t) {
		var textItem = textArray[t]
		if (textItem.constructor === Object) {
			conjugatedText += ' ' + conjugateTextObject(textItem, parentGramProps, inputTense)
		} else {
			conjugatedText += ' ' + textItem
		}
	}
	return conjugatedText.slice(1)
}

/**
 * Conjugates `textObj` to the term's correct inflection according to the parent subnode's `parentGramProps`, if applicable.
 *
 * @private
 * @static
 ...
 */
function conjugateTextObject(textObj, parentGramProps, inputTense) {
	/**
