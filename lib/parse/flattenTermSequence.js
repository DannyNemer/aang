...
function getSubnodeText(subNode, parentGramProps) {
	...
	if (text) {
		if (parentGramProps) {
			// Conjugate `text` if `parentGramProps` is defined and `subNode.ruleProps.text` is a conjugative display text object.
			if (text.constructor === Object) {
				return conjugateTextObject(text, parentGramProps, subNode.ruleProps.tense)
			}

			if (text.constructor === Array) {
				return conjugateTextArray(text, parentGramProps, subNode.ruleProps.tense)
			}
		}
