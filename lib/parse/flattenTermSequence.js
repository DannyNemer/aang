...
function getSubnodeText(subNode, parentGramProps) {
	...
	if (text) {
		...
		if (parentGramProps && text.constructor === Object) {
 ...
 * @param {string} [inputTense] The term sequence's input tense, defined by a verb terminal symbol's `tense` property within, to accept if matches `parentGramProps.acceptedTense`.
 * @returns {string} Returns the display text `subNode` produces.
 */
function conjugateTextObject(textObj, parentGramProps, inputTense) {
	/**
	 * Conjugate a child subnode within a term sequence that accepts a tense if matched in input.
	 *
