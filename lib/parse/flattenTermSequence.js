...
function createTermSeqRuleProps(subNode, cost) {
	...
	/**
	 * Only save input tense from `leftSub` if its text remains unconjugated, otherwise its `tense` was used in its conjugation.
	 *
	 * This check is identical to `!parentGramProps[0]`, becuase `conjugateTextObject()` throws an exception if `parentGramProps[0]` exists and it can not conjugate the display text.
	 *
	...
	if (subNodeNext) {
		...
		// Only save input tense from `rightSub` if its text remains unconjugated, otherwise its `tense` was used in its conjugation.
...
function createTermSeqInsertionRuleProps(subNode, cost) {
	...
	// Get the display text `childSub` produces, and conjugate with the grammatical properties specific to its RHS index, `parentGramProps[0]`, if any.
	var parentGramProps = subNodeRuleProps.gramProps
	var childSubText = getSubnodeText(childSub, parentGramProps && parentGramProps[0])
	...
	return {
		 ...
		 *
		 * Only save input tense from `childSub` if its text remains unconjugated, otherwise its `tense` was used in its conjugation.
		 */
 ...
 *
 * If `subNode.ruleProps.text` is a conjugative display text object or array but `parentGramProps` is `undefined`, returns the text object or array to be conjugated by a `gramProps` of an ancestor rule higher in its tree or a `personNumber` property in a preceding nominative rule in its tree.
 *
...
function conjugateText(text, parentGramProps, inputTense) {
	...
	// Conjugate text objects within a text array, and concatenate resulting strings with invariable strings in the array. Will never contain nested arrays.
	var conjugatedText = ''
