 ...
 *
 * Else if `insertedPersonNumber` is defined and `subnode.ruleProps.text` is a conjugative display text object or array, returns the conjugated text string. For use by `createTermSequenceInsertionRuleProps()` when the parent node of `subnode` is an insertion with `ruleProps.insertedSymIdx` of 0 and the inserted (leading) text was generated from a nominative subject. For example:
 *   subnode: `[have]`,
 *   personNumber: "oneSg",
 *     => "have"
 ...
 * @param {string} [insertedPersonNumber] The `ruleProps.personNumber` of the parent insertion subnode that has `ruleProps.insertedSymIdx` of 0. For use by `createTermSequenceInsertionRuleProps()` when the inserted (leading) text was generated from a nominative subject.
