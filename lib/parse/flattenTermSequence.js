 ...
 *
 * For use by `calcHeuristicCosts` while traversing the parse forest to calculate the A* heuristic cost estimates.
 *
 * @static
...
function createPartialTermSequenceRuleProps(subnode) {
	 ...
	 *
	 * Only pass the sequence's `gramProps` after checking here if conjugation will succeed because `conjugateText()` does not support failed conjugation attempts and returning a non-string.
	 * • If `termSequenceRHSIndex` is 1, conjugation may fail if the text requires a nominative `personNumber` property from the leading, non-sequence branch, which can not be determined until traversed by `pfsearch`. For example, the binary pair: `[nom-users+]` `[like]`.
	 * • Always attempt conjugation if `termSequenceRHSIndex` is 0 because any preceding `personNumber` property does not apply because it is outside its subtree.
	 */
	var text
	var parentGramProps = subnodeRuleProps.gramProps
	...
	// Temporarily save `gramProps` for the non-sequence subnode. Only exists for terminal rules that are not yet implemented as term sequences: `[follow]`.
	var newGramProps
	...
	/**
	 * Sum `subnodeRuleProps.cost` with the cost of the term sequence subnode's branch, which is its `minCost` because only the cheapest branch is used because term sequences are ambiguous (no semantics).
	 *
	 * Calculate before overwriting `subnode.node` below.
	 *
	 * Do not change the remaining subnode's `minCost`, because the value excludes the subnode's `ruleProps.cost` itself and is therefore unaffected.
	 */
	var newCost = subnodeRuleProps.cost + (termSequenceRHSIndex === 0 ? subnode.node : subnode.next.node).minCost

	/**
	 * If the left child subnode is the term sequence to insert, replace `subnode.node` -> `subnode.next.node` for `pfsearch` to only traverse the right child subnode.
	 *
	 * Need not delete `subnode.next` after flattening the subnode because `pfsearch` does not check for it when it sees `ruleProps.insertedSymIdx`.
	 */
	...
	/**
	 * Create a new `ruleProps` that inserts the term sequence's text at its original RHS index.
	 *
	 * Exclude the following properties:
	 * • rhsTermSequenceIndexes - No longer specify `subnode` is a partial term sequence. Necessary when reparsing (with all tokens marked deletable after the initial parse fails) to prevent `calcHeuristicCosts` from attempting to flatten the already flattened partial term sequence subnode.
	 * • gramProps - Only exists for term sequence subnodes, and because the remaining subnode is not a term sequence, then `gramProps` must be undefined for that RHS index.
	 * • secondRHSCanProduceSemantic - Only applicable to binary subnodes.
	 * • tense - Only applicate to terminal `ruleProps`. For use by `pfsearch` to conjugate non-insertion text, or by `flattenTermSequence` to transfer the property to parent sequences (which partial term sequences can not have).
	 */
	return {
		// Unlike other flattened `ruleProps`, partial term sequences rename nonterminal because they are insertions.
		isNonterminal: true,
		// The sum of `subnodeRuleProps.cost` and the (inserted) term sequence subnode's cost (`minCost`).
		cost: newCost,
		...
		// The semantic properties. `subnodeRuleProps.insertedSemantic` is guaranteed to be `undefined` because the insertion is a term sequence, which always lack semantics.
		semantic: subnodeRuleProps.semantic,
		// Likely always `true` because the remaining child subnode is not a term sequence. Can be falsey, however, if produces only terminal rules not yet implemented as term sequences.
		semanticIsReduced: subnodeRuleProps.semanticIsReduced,
		/**
		 * Specify the remaining RHS subnode can produce a semantic.
		 * • No need to check `subnodeRuleProps.secondRHSCanProduceSemantic` (i.e., if `termSequenceRHSIndex` is 0) because the term sequence subnode is guaranteed to not produce semantics, hence `subnodeRuleProps.rhsCanProduceSemantic` can only apply to the (remaining) non-sequence subnode.
		 * • Likely always `true` because the remaining child subnode is not a term sequence. Can be falsey, however, if produces only terminal rules not yet implemented as term sequences.
		 */
...
/**
 * Checks if `subnode`, which was passed to `createTermSequenceSubstitutionRuleProps()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} subnode The `createTermSequenceSubstitutionRuleProps()` term sequence substitution subnode to inspect.
 * @returns {boolean} Returns `true` if `subnode` is ill-formed, else `false`.
 */
