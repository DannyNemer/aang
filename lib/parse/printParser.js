...
/**
 * Sorts `nodeTabs` for inspecting and comparing output.
 *
 * @memberOf Parser
 * @returns {Object[][]} Returns `nodeTabs` for convenience.
 */
Parser.prototype.sortNodeTabs = function () {
	for (var t = 0; t < this.tokensLen; ++t) {
		var nodes = this.nodeTabs[t]

		// Sort nodes alphabetically (by `index`) then by increasing size (because larger nodes are added later in the parse). The sort function will never return 0 because each node is unique.
		nodes.sort(function (a, b) {
			var diff = a.sym.index - b.sym.index
		...
		for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
			nodes[n].subs.sort(function (a, b) {
