 ...
 * @private
 * @static
 * @param {Object[][]} nodeTabs The array of maps of symbol indexes to arrays of nodes at each query index to sort.
...
function sortNodeTabs(nodeTabs) {
	for (var t = 0, nodeTabsLen = nodeTabs.length; t < nodeTabsLen; ++t) {
		...
		// Nodes already sorted alphabetically (by nonterminal symbol `index`).
		for (var n = 0, nodeSetsLen = nodeSets.length; n < nodeSetsLen; ++n) {
			...
			// Sort subnodes alphabetically (by `index`). The `sort()` function will never return 0 because each subnode is unique.
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				nodes[i].subs.sort(function (a, b) {
					var diff = a.node.sym.index - b.node.sym.index
					if (diff !== 0) return diff

					if (a.next && b.next) {
						return a.next.node.sym.index - b.next.node.sym.index
					}
...
Parser.prototype.printStack = function () {
	...
	for (var i = 0, vertTabLen = this.vertTab.length; i < vertTabLen; ++i) {
		var vertices = this.vertTab[i]

		for (var v = 0, verticesLen = vertices.length; v < verticesLen; ++v) {
			var vertex = vertices[v]
			var zNodes = vertex.zNodes

			util.log(util.colors.yellow(vertex.startIdx + '_' + getShiftsStateIndex(this.stateTable, vertex.shifts)))
			...
			for (var z = 0, zNodesLen = zNodes.length; z < zNodesLen; ++z) {
				...
				for (var v2 = 0, subVerticesLen = subVertices.length; v2 < subVerticesLen; ++v2) {
					...
					util.log('     <= v_' + subVertex.startIdx + '_' + getShiftsStateIndex(this.stateTable, subVertex.shifts) + ' ' + zNodesStr)
				}
			}
		}
	}
