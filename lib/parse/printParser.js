...
function sortNodeTabs(nodeTabs) {
	for (var t = 0, nodeTabsLen = nodeTabs.length; t < nodeTabsLen; ++t) {
		var symIdToNodesMap = nodeTabs[t]

		// Nodes already sorted alphabetically (by nonterminal symbol `id`).
		for (var n = 0, maxSymId = symIdToNodesMap.length; n < maxSymId; ++n) {
			var nodes = symIdToNodesMap[n]
			if (!nodes) continue
			...
			// Sort sub-nodes alphabetically (by symbol `id`). The `sort()` function will never return 0 because each sub-node is unique.
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
	...
	while (stack.length > 0) {
		...
		for (var s = 0, subsLen = subs.length; s < subsLen; ++s) {
			...
			do {
				...
				if (subNode.subs) {
					// Copy `subs` array because the node for `[blank-inserted]` is saved to the `StateTable` prototype and shared for every parse.
