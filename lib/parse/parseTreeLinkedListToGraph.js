/**
 * Converts a reverse linked list `path` output by `pfsearch` when ran with
 * `buildTrees` as `true` (which links each `path` to the previous `path` and
 * its new `ruleProps`) to a graph representation.
 *
 * @static
 * @param {Object} path The parse tree to convert.
 * @param {Object} options The options object.
 * @param {boolean} [options.treeNodeCosts=false] Specify including each node's
 * path cost.
 * @param {boolean} [options.treeTokenRanges=false] Specify including the start
 * and end indexes of each node's token range.
...
module.exports = function (path, options) {
	...
	// The stack of previous nodes lower in the parse tree (which become child
	// nodes).
	var nodesStack = []

	if (!path.prev) {
		throw 'parseTreeLinkedListToGraph: pfsearch was not invoked with `options.buildTrees`'
	}
	...
	while (true) {
		...
		if (!node) {
			...
			if (!node) {
				...
				// Find last node, skipping the insertion text also stored in
				// `nextNodes`.
		...
		// Binary nonterminal rule.
			...
			nodesStack.push({
				...
				children: [
					...
					// Do not display the cost of the second node because its
					// `mincost` is included in the first node's `mincost`, and
					// after completing the parse of the first node's branch
					// and returning to parse this (second) node, there is no
					// updated `minCost` value that includes the previous
					// branch's total cost.
...
function formatNode(node, options, childNode, minCost) {
	...
	if (options.treeNodeCosts && minCost !== undefined) {
		// Include each node's path cost. I.e., the cost of the path so far +
		// the heuristic estimate of the cost remaining.
