 ...
 * 2. The merged `text` values of adjacent term sequences, created by `flattenTermSequence`.
 *
 * For use by `pfsearch`.
 *
 * @static
 ...
 * @returns {string} Returns the conjugated display text, prepended with a space.
 */
module.exports = function conjugateText(text, personNumberList, parentGramProps, inputTense) {
	...
	// Conjugate `text`, which is an array of text strings and conjugative text objects. Exists for insertions and merged `text` values for adjacent term sequences. This array will never contains nested arrays.
	var conjugatedText = ''
...
function conjugateTextObject(textObj, personNumberList, parentGramProps, inputTense) {
	...
	/**
	 * Conjugate verb according to grammatical person-number property in preceding nomniative rules. For example:
	 *   "(repos I) like"
	 *   "(repos Danny) likes".
	 */
