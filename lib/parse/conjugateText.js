 ...
 * @param {string} [inputTense] The verb terminal symbol's `tense` property to compare to `parentGramProps.acceptedTense`, if any, and determine if the input is an (optionally) acceptable form of the associated verb, `textObj`.
...
function conjugateTextObject(textObj, personNumberList, parentGramProps, inputTense) {
	if (parentGramProps) {
		/**
		 * If the input terminal symbol is of a tense which the parent rule optionally accepts, then accept it. For example: "repos I like/liked".
		 *
