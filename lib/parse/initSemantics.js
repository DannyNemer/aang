...
/**
 * Initializes the semantics of rules in the grammar for parsing by replacing
 * identical semantic functions, semantic nodes, and semantic arrays with
 * references to a single object. The former enables semantic equality checks
 * by object reference instead of checking their `name` properties. The latter
 * two enables equality checks of entire semantic trees and arrays by object
 * reference instead of having to traverse the objects.
 *
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 * @param {Object} semantics The map of the grammar's semantic names to semantics.
 */
module.exports = function (ruleSets, semantics) {
	// The semantic arrays used in `ruleSets`.
	var semanticArrayTab = []
	// The semantic nodes used in `ruleSets`.
	var semanticNodeTab = []

	// Initialize semantics on rules by replacing identical semantics with
	...
	// Initialize semantics on instances of the `requires` property by
	// replacing them with references to instances of the same semantic.
	...
	/**
	 * Gets a reference to a semantic array identical to `newSemanticArray` if
	 * it already exists in `ruleSets`, otherwise adds `newSemanticArray` to
	 * `semanticArrayTab`. Recursively replaces the nodes in `newSemanticArray`
	 * and the nodes' properties with references to identical instances within
	 * `ruleSets`.
	 *
	 ...
	 * @returns {Object[]} Returns a reference to the instance of `newSemanticArray` in `ruleSets`.
	 */
	...
	/**
	 * Gets a reference to a semantic node identical to `newSemanticNode` if it
	 * already exists in `ruleSets`, otherwise adds `newSemanticNode` to
	 * `semanticNodeTab`. Recursively replaces the node's semantic function and
	 * children array, if any, with references to identical instances within
	 * `ruleSets`.
	 *
	...
	function getSemanticNode(newSemanticNode) {
		// Replace each semantic function with references to the unique
		// instance of the semantic within the grammar.
		...
		// Get a reference to a matching unique instance of `newSemanticNode`
		// within the grammar, if it exists.
