...
/**
 * Initializes the semantics of rules in the grammar for parsing by replacing identical semantic functions, semantic nodes, and semantic arrays with references to a single object. The former enables semantic equality checks by object reference instead of checking their `name` properties. The latter two enables equality checks of entire semantic trees and arrays by object reference instead of having to traverse the objects.
 *
...
module.exports = function (ruleSets, semantics) {
	...
	/**
	 * Gets a reference to a semantic array identical to `newSemanticArray` if it already exists in `ruleSets`, otherwise adds `newSemanticArray` to `semanticArrayTab`. Recursively replaces the nodes in `newSemanticArray` and the nodes' properties with references to identical instances within `ruleSets`.
	 *
	...
	/**
	 * Gets a reference to a semantic node identical to `newSemanticNode` if it already exists in `ruleSets`, otherwise adds `newSemanticNode` to `semanticNodeTab`. Recursively replaces the node's semantic function and children array, if any, with references to identical instances within `ruleSets`.
	 *
