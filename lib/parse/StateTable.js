...
function StateTable(ruleSets, startSymbol, blankSymbol) {
	...
	// Generate the `Parser` node set for `blankSymbol` which `Parser` appends to the end of the array of nodes that produce the input query's matched terminal symbols. This enables `Parser` to accept insertion rules that are only recognized at the end of an input query.
	this.blankWordNodeArray = this.createBlankInsertedNodeArray(blankSymbol)
}
 ...
 * @memberOf StateTable
 * @param {string} blankSymbol The name of the blank terminal symbol of `ruleSets`.
 * @returns {Object[]} The array containing the nonterminal node that produces `blankSymbol`.
 */
StateTable.prototype.createBlankInsertedNodeArray = function (blankSymbol) {
