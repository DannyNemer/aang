...
function createRuleProps(rule) {
	...
	if (!rule.isTerminal) {
		...
		if (rule.rhsTermSequenceIndexes) {
			 ...
			 * `ruleProps.rhsTermSequenceIndexes` to distinguish the subnode as
			 * a partial term sequence.
			 *
			 ...
			 * enables nesting complete term sequences (which this is not)
			 * within other term sequences.
			 *
			 * Note: Might amend the grammar generator to assign this property
			 * to partial term sequence rules as done here, and extend the
			 * generator to distinguish partial sequence to prevent their
			 * nesting.
			 */
			ruleProps.isTermSequence = true
		}
	...
	/**
	 * `secondRHSCanProduceSemantic` is checked at binary, non-insertion rules.
	 * Convert to an integer so that it can properly increment
