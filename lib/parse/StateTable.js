...
function StateTable(grammar) {
	...
	// Generate the `Parser` node set for the `[blank-inserted]` symbol, which `Parser` appends to the end of the array of nodes that produce the input query's matched terminal symbols. This enables `Parser` to accept insertion rules that are only recognized at the end of an input query.
...
/**
 * Generates the `Parser` node set for the `[blank-inserted]` symbol, which `Parser` appends to the end of the array of nodes that produce the input query's matched terminal symbols. This enables `Parser` to accept insertion rules that are only recognized at the end of an input query.
 *
 * @memberOf StateTable
...
StateTable.prototype.print = function () {
	...
	for (var s = 0, statesLen = this.states.length; s < statesLen; ++s) {
		...
		for (var i = 0, symIdsLen = symIds.length; i < symIdsLen; ++i) {
			var symId = Number(symIds[i])
			util.log('\t' + this.idToSymbolName(symId), '=>', this.states.indexOf(shifts[symId]))
		}
	}
}
