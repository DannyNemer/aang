...
function StateTable(grammar) {
	...
	this.terminalSymTab = {}
	// The map of placeholder symbol names (e.g., entity categories) to their symbol object. Used by `Parser` for placeholder symbol lookup.
	this.placeholderSymTab = {}
	// The numbers of terminal and nonterminal symbols in this instance, for assigning symbol ids. Separate nonterminal and terminal symbols because `Parser` uses ids to map nonterminal symbols, and including the terminal ids would yield unnecessarily larger maps.
	...
	// Generate the `Parser` node set for the `[blank-inserted]` symbol, which `Parser` appends to the end of the array of nodes that produce the input query's matched terminal rules. This enables `Parser` to accept insertion rules that are only recognized at the end of an input query.
	...
	// The set of deletables for constructing additional parse trees with the specified input tokens deleted at a cost.
...
function createRuleProps(rule) {
	var ruleProps = {
		// The base rule cost. Including the epsilon rule cost, semantic cost (if any), and any edit cost penalties (for substitutions, transposition, or insertions).
		cost: rule.cost,
	}
	...
	if (rule.insertedSymIdx !== undefined) {
		...
		// If `buildGrammar` ran with '--trees' option, then include the insertion rules' parse trees (which form the insertions) if printing the graph representations of the `phsearch` parse trees.
		if (rule.tree) {
	...
	if (!rule.isTerminal) {
		...
		/**
		 * Specify nonterminal rules whose RHS produces neither display text nor semantics. This enables `flattenTermSequence` to reduce the associated node's subnode's costs (which include deletion costs on the terminal nodes) so that `pfsearch` does not wastefully traverse those subnodes. This includes nonterminal substitutions and stop-words created from regex-style terminal rules, and rules that produce only stop-words.
		 *
		 * In addition, this enables these nonterminal rules to share existing terminal rules that also have `text` values by only using their `text` value and not traversing those nodes.
		 */
		if (rule.rhsDoesNotProduceText) {
			ruleProps.rhsDoesNotProduceText = true
		}

		// Specify `rule.rhs` produces only terminal rule sets; i.e., consecutive single-token terminal rules.
		else if (rule.isTermSequence) {
			ruleProps.isTermSequence = true

			/**
			 * If the nonterminal term sequence has `insertedSymIdx`, then it is an insertion term sequence.
			 *
			 * This instructs `flattenTermSequence` to traverse the single child node `rule` produces to get the matched terminal rule `text`. `flattenTermSequence` merges this `text` with the insertion `text` according to `rule.insertedSymIdx`. For example:
			 ...
			 *
			 * `flattenTermSequence` also gets the input tense of any verb terminal rule its single child node (i.e., `rule.rhs`) produces to maintain optional tense if the parent rule of `rule` has matching `acceptedTense`. For example:
			 *   `[contribute-to]` -> `[contribute]`, text: "to"
			...
			/**
			 * If the nonterminal term sequence has `text` and no `insertedSymIdx`, then it is a multi-token substitution term sequence.
			 *
			 * This instructs `pfsearch` to use this rule's `text` property instead of the matched terminal rules it produces. For example:
			 *   `[contribute-to]` -> `[work-verb]` `[on]` (input) -> text: `[ {contribute-verb-forms}, "to" ]`
			 *
			 * `flattenTermSequence` will, however, traverse this rule's child nodes to get the input tense of any matched verb terminal rules to maintain tense for any verb substitutions in `text` if the parent rule  of `rule` has matching `acceptedTense` (i.e., maintain the tense if input, but do not conjugate to the tense if otherwise). For example:
			...
			/**
			 * If the nonterminal term sequence has no `text`, then it is a normal term sequence.
			 *
			 * Without `text` or `insertedSymIdx`, `isTermSequence` alone instructs `flattenTermSequence` to traverse this rule's child nodes and do the following:
			 * 1. Merge the `text` values of the matched terminal rules (i.e., child nodes).
			 * 2. Get the input tense of any matched verb terminal rules (for `text` conjugation).
			 * 3. Assign both the merged `text` (#1) and input tense (#2), if any, to this rule's node.
			 * 4. Mark this rule's node as terminal.
			 *
			 * This prevents `pfsearch` from traversing past this rule's (now terminal) node (to its child nodes), and instead use this node's merged `text` and input tense for display text.
			 *
			 * This operation reduces multiple instances of generating the same display text for the same subtree of adjacent terminal rules.
			 *
			 * This operation is also essential for conjugating verb phrases. Consider the following rules:
			 * 1. `[contribute-to]`   -> `[contribute-verb]` `[to]`
			 * 2. `[contribute-verb]` -> 'contributed', text: {contribute-verb-forms}
			 * The rule with the verb phrase `[contribute-to]` as a RHS symbol also has the necessary `gramProps` grammatical properties to conjugate the `text` object of `[contribute-verb]`. Hence, `flattenTermSequence` must bring the `[contribute-verb]` `text` object up one level (#2 -> #1 above).
			 *
			 * This rule can be unary and neither a substitution nor insertion. For example:
			 *   `[like]` -> `[love]` -> "love", "loves", "loved" text: `{love-verb-forms}`
			 */
		}

		/**
		 * Specify this binary nonterminal rule is a partial term sequence, where the symbol at the defined RHS index is a term sequence and the other is not. Instructs `flattenTermSequence` to flatten the binary node into an insertion node with the term sequence subnode's text as the insertion text.
		 */
		if (rule.rhsTermSequenceIndexes) {
			ruleProps.rhsTermSequenceIndexes = rule.rhsTermSequenceIndexes

			/**
			 * Assign `isTermSequence`, even though only one of the two child subnodes is a term sequence, to simplify the `calcHeuristicCosts` check for passing the subnode to `flattenTermSequence`. `flattenTermSequence` then properly checks for `ruleProps.rhsTermSequenceIndexes` to distinguish the subnode as a partial term sequence.
			 *
			 * In the grammar, partial term sequences lack this property because it would contradict the property's definition, which otherwise enables nesting complete term sequences (which this is not) within other term sequences.
			 *
			 * Note: Might amend the grammar generator to assign this property to partial term sequence rules as done here, and extend the generator to distinguish partial sequence to prevent their nesting.
			 */
			ruleProps.isTermSequence = true
		}

		// Specify instructing `flattenTermSequence` to discard the display text the RHS symbol(s) at the defined index(es) produces when `flattenTermSequence` creates a new `ruleProps` from this rule's term sequence node.
		if (rule.rhsNoTextIndexes) {
			ruleProps.rhsNoTextIndexes = rule.rhsNoTextIndexes
		}

		// Specify this nonterminal rule is a transposition edit, which instructs `Parser.prototype.reduce()` to swap the child nodes when reducing this rule's binary node.
		...
		// The map of RHS index to the grammatical properties with which to conjugate the term sequence at that RHS index.
		if (rule.gramProps) {
...
/**
 * Generates the `Parser` node set for the `[blank-inserted]` symbol, which `Parser` appends to the end of the array of nodes that produce the input query's matched terminal rules. This enables `Parser` to accept insertion rules that are only recognized at the end of an input query.
 *
 * @memberOf StateTable
...
StateTable.prototype.createBlankInsertedNodeArray = function (blankSymbolName) {
	...
	// Return the node in an array for use in every parse as the last token index's node set.
...
StateTable.prototype.generate = function (startSym) {
	...
	for (var s = 0; s < ruleSets.length; ++s) {
		...
		var newState = {
			reds: [],
			shifts: [],
			// The index of the state within the state table. Used by `Parser.prototype.addVertex()` to map state index to vertex zNodes for the current input query parse index.
	...
	// Extend `state.shifts` to map `symbol.id` to the next state for `symbol`, if any, instead of mapping to the next state's index.
	mapShiftsToStates(this.states)

	// Assign `cost` to `ruleProps` arrays (i.e., insertions) from its first element (which is the cheapest `ruleProps` because it is sorted in grammar generation) for use as its minimum cost in `calcHeuristicCosts`.
	cacheRulePropsArraysMinCost(this.nonterminalSymTab)
}

/**
 * Extends shifts to map `symbol.id` to the next state for `symbol`, if any, instead of mapping to the next state's index.
 *
 * @private
 * @static
...
/**
 * Assigns `cost` to `ruleProps` arrays (i.e., insertions) from its first element (which is the cheapest `ruleProps` because it is sorted in grammar generation) for use as its minimum cost in `calcHeuristicCosts`.
 *
 * @private
 * @static
 * @param {Object} nonterminalSymTab The map of nonterminal symbol names to their symbol object.
 */
 ...
 *
 * @memberOf StateTable
 * @param {boolean} [suppressStateIndexes] Specify suppressing state indexes from output. This is useful for comparing output for different grammar builds.
 */
