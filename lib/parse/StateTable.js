...
StateTable.prototype.generate = function (startSym) {
	...
	for (var s = 0, statesLen = this.states.length; s < statesLen; ++s) {
		...
		for (var i = 0, symIndexesLen = symIndexes.length; i < symIndexesLen; ++i) {
			...
			shifts[symIndex] = {
				state: this.states[shifts[symIndex]],
				symName: this.indexToSymbolName(symIndex),
				stateIndex: s,
			}
		}
	}

	// Determines the minimum number of additional shifts needed at each shift and reduction to reach the accepting state. This enables discarding nodes in `Parser` we know fail because too few input tokens remains. Also helps identify when a sequence of terminal nodes is impossible and requires a deletion.
	require('./calcMinShiftsToAccept')(this.states)
}
...
StateTable.prototype.print = function () {
	...
	this.states.forEach(function (state, s) {
		...
		state.reds.forEach(function (red) {
			...
			util.log(ruleStr, '$', red.minShiftsToAccept, red.ruleProps)
		})

		var shifts = state.shifts
		for (var symIndex in shifts) {
			symIndex = Number(symIndex)
			var shift = shifts[symIndex]
			util.log('\t' + shift.symName, '=>', shift.stateIndex, '$', shift.minShiftsToAccept)
		}
	}, this)
}
