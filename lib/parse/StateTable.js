...
function createRuleProps(rule) {
	...
	if (!rule.isTerminal) {
		...
		// Specifies nonterminal rules whose RHS produces neither display text nor semantics. This enables `calcHeuristicCosts` to reduce the associated node's subnode's costs (which include deletion costs on the terminal nodes), so that `pfsearch` does not wastefully traverse those subnode. This includes nonterminal substitutions where `text` is defined on a nonterminal rule, nonterminal stop-words, and rules that produce only stop-words.
		if (rule.isSubstitution || rule.isStopWord) {
			ruleProps.noText = true
		}
	}
