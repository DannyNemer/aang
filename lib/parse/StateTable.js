...
function createRuleProps(rule) {
	...
	if (rule.secondRHSCanProduceSemantic) {
		ruleProps.secondRHSCanProduceSemantic = true
	}
	...
	if (rule.gramProps) {
		if (rule.gramProps.personNumber) {
			// Separate `personNumber` from `gramProps` (below).
			ruleProps.personNumber = rule.gramProps.personNumber
			delete rule.gramProps.personNumber
		}
...
StateTable.prototype.generate = function (startSym) {
	...
	// Delete symbol properties only needed for `StateTable` initial generation.
	for (var symbolName in this.symbolTab) {
		var symbol = this.symbolTab[symbolName]

		if (!symbol.isTerminal) {
			// `rules` are not needed for nonterminal symbols.
			delete symbol.rules
