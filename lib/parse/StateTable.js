...
function createRuleProps(rule) {
	...
	if (!rule.isTerminal) {
		 ...
		 */
			...
			/**
			 * If the nonterminal binary term set has `insertedSymIdx`, then it is an insertion. This instructs `calcHeuristicCosts` to traverse the single child node `rule` produces to get the matched terminal rule `text`. `calcHeuristicCosts` merges this `text` with the insertion `text` according to `rule.insertedSymIdx`. For example:
			...
			/**
			 * If the nonterminal binary term set has `text` and no `insertedSymIdx`, then it is a multi-token substitution. This instructs `calcHeuristicCosts` to use this rule's `text` property instead of the matched terminal rules it produces. For example:
