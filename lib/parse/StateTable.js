...
StateTable.prototype.lookUp = function (symbolName, isTerminal, isPlaceholder) {
	...
	if (isPlaceholder) {
		...
		if (!symbol) {
			symbol = this.placeholderSymTab[symbolName] = this.newSymbol(symbolName, isTerminal)
		}
	} else if (isTerminal) {
		symbol = this.terminalSymTab[symbolName]

		if (!symbol) {
			symbol = this.terminalSymTab[symbolName] = this.newSymbol(symbolName, isTerminal)
		}
	} else {
		symbol = this.nonterminalSymTab[symbolName]

		if (!symbol) {
			symbol = this.nonterminalSymTab[symbolName] = this.newSymbol(symbolName, isTerminal)
		}
	}
...
StateTable.prototype.indexToSymbolName = function (index) {
	for (var symbolName in this.nonterminalSymTab) {
		var symbol = this.nonterminalSymTab[symbolName]
