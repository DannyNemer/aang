...
function createRuleProps(rule) {
	...
	if (!rule.isTerminal) {
		...
		// Check if `rule` is a binary term, which specifies it produces only terminal rule sets; i.e., consecutive single-token terminal rules.
		else if (rule.rhsIsBinaryTermSet) {
			ruleProps.rhsIsBinaryTermSet = true

			/**
			 * If the nonterminal binary term set has `insertedSymIdx`, then it is an insertion.
			 *
			 * This instructs `calcHeuristicCosts` to traverse the single child node `rule` produces to get the matched terminal rule `text`. `calcHeuristicCosts` merges this `text` with the insertion `text` according to `rule.insertedSymIdx`. For example:
			 ...
			 *
			 * `calcHeuristicCosts` also gets the input tense of any verb terminal rule its single child node (i.e., `rule.rhs`) produces to maintain optional tense if the parent rule of `rule` has matching `acceptedTense`. For example:
			 *   `[contribute-to]` -> `[contribute]`, text: "to"
			...
			if (rule.insertedSymIdx !== undefined) {
				if (!rule.text) {
					util.logError('Binary term set insertion lacks `text`:', rule)
					throw new Error('Ill-formed binary term set insertion')
				}
			...
			/**
			 * If the nonterminal binary term set has `text` and no `insertedSymIdx`, then it is a multi-token substitution.
			 *
			 ...
			 *
			 * `calcHeuristicCosts` will, however, traverse this rule's child nodes to get the input tense of any matched verb terminal rules to maintain tense for any verb substitutions in `text` if the parent rule  of `rule` has matching `acceptedTense` (i.e., maintain the tense if input, but do not conjugate to the tense if otherwise). For example:
			...
			/**
			 * If the nonterminal binary term set has no `text`, then it is a normal binary term set.
			 *
			 * This property instructs `calcHeuristicCosts` to traverse this rule's child nodes and do the following:
			 ...
			 * The rule with the verb phrase `[contribute-to]` as a RHS symbol also has the necessary `gramProps` grammatical properties to conjugate the `text` object of `[contribute-verb]`. Hence, `calcHeuristicCosts` must bring the `[contribute-verb]` `text` object up one level (#2 -> #1 above).
			 *
			 * This rule can be unary and not be a substitution. For example:
			 *   `[like]` -> `[love]` -> "love", "loves", "loved" text: `{love-verb-forms}`
			 */
			else {
				// Ensure non-substitution binary term sets have two RHS symbols. Nonterminal substitutions can have one RHS symbol when they are insertions for previously binary rules.
				if (rule.rhs.length !== 2) {
					util.logError('Binary term set does not have two RHS symbols:', rule)
