...
function createRuleProps(rule) {
	...
	if (!rule.isTerminal) {
		...
		/**
		 * When `rule.isTermSequence` is not truthy because not every RHS symbol is a term sequence, `rule.rhsTermSequenceIndexes` specifies the RHS indexes of which RHS symbol is a term sequence, if either. Only possible if `rule` is binary.
		 *
