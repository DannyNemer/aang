...
function createRuleProps(rule) {
	...
	if (!rule.isTerminal) {
		...
		// Check if `rule.rhs` is produces only terminal rule sets; i.e., consecutive single-token terminal rules.
		else if (rule.isTermSetSequence) {
			ruleProps.isTermSetSequence = true

			/**
			 * If the nonterminal term set sequence has `insertedSymIdx`, then it is an insertion.
			 *
			...
			if (rule.insertedSymIdx !== undefined) {
				if (!rule.text) {
					util.logError('Term set sequence insertion lacks `text`:', rule)
					throw new Error('Ill-formed term set sequence insertion')
				}
			...
			/**
			 * If the nonterminal term set sequence has `text` and no `insertedSymIdx`, then it is a multi-token substitution.
			 *
			...
			/**
			 * If the nonterminal term set sequence has no `text`, then it is a normal term set sequence.
			 *
