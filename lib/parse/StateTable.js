...
function createRuleProps(rule) {
	...
	if (!rule.isTerminal) {
		...
		// Check if `rule.rhs` is produces only terminal rule sets; i.e., consecutive single-token terminal rules.
		else if (rule.isTermSequence) {
			ruleProps.isTermSequence = true

			/**
			 * If the nonterminal term sequence has `insertedSymIdx`, then it is an insertion.
			 *
			...
			/**
			 * If the nonterminal term sequence has `text` and no `insertedSymIdx`, then it is a multi-token substitution.
			 *
		...
		/**
		 * When `rule.isTermSequence` is not truthy because not every RHS symbol is a term sequence, `rule.rhsTermSequenceIndexes` specifies the RHS indexes of which RHS symbol is a term sequence, if either. Only possible if `rule.rhs` is binary.
		 *
		...
		// Specify instructing `flattenTermSequence` to discard the display text the RHS symbol(s) at the defined index(es) produces when `flattenTermSequence` creates a new, terminal `ruleProps` from this rule's term sequence node.
		if (rule.noTextIndexes) {
			ruleProps.noTextIndexes = rule.noTextIndexes
		}
