		...
		for (var n = 0; n < maxSymId; ++n) {
			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					...
					/**
					 * Mark previously flattened term sequences to instruct `calcHeuristicCosts` to avoid double counting the cost of their child nodes, because the flattened term sequences' `ruleProps.cost` values already include the sum cost of their child nodes from the initial parse.
					 *
