 ...
 * @property {boolean} failedInitStartSym Indicates whether the parse initially failed to reach the start symbol (before marking all input tokens as deletable).
 * @property {boolean} failedInitLegalTrees Indicates whether the parse initially failed to generate any legal parse trees (due to illegal semantics before marking all input tokens as deletable).
...
Parser.prototype.parse = function (query, k, options) {
	...
	if (this.startNode) {
		...
		parseResults.pathCount = trees.pathCount,

		if (trees.length > 0) {
			parseResults.trees = trees
			parseResults.ambiguousTreeCount = trees.ambiguousTreeCount
	...
	if (this.startNode) {
		...
		// Return where `parseResults.trees` is `undefined` to indicate never reaching the start symbol.
		return parseResults
	}
}
