...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			 ...
			 *
			 * When re-parsing with all tokens marked deletable, there can also be duplicate instances of a non-entity terminal sub-nodes at the same index. Though, it is rare. For example:
			 *   "JavaScript Ruby repos mine mine"
	     * "mine" and "mine" will obviously be the same terminal rule. With all tokens marked deletable, there will be two sub-nodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine". `Parser.prototype.addTermRuleNodes()` could detect and prevent such duplicity, but the overhead is too great for its rarity.
