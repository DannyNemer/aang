...
Parser.prototype.createWords = function (terminalSym, endPos, semanticArg, text) {
	...
	if (this.position < this.tokensLen) {
		this.createDeletions(terminalNode, wordNodes, endPos)
	}
}
 ...
 * @param {number} endPos The end index of the terminal symbol in input.
 */
Parser.prototype.createDeletions = function (terminalNode, oldWordNodes, endPos) {
	var leadingDelLen = 1
	...
	while (this.deletableTokenSpans[startPos]) {
		...
		this.baseCreateDeletions(terminalNode, oldWordNodes, startPos, endPos, leadingDelLen)
	...
	var trailingDelLen = this.deletableTokenSpans[endPos + 1]
	endPos += trailingDelLen
	if (endPos === this.tokensLen - 1) {
		leadingDelLen = 0
		...
		do {
			this.baseCreateDeletions(terminalNode, oldWordNodes, startPos, endPos, trailingDelLen + leadingDelLen)
 ...
 * @param {number} endPos The end index of the span of the terminal symbol and deletables.
 * @param {number} costPenalty The cost penalty for the deletions, which is equal to 1 per deleted token.
 */
Parser.prototype.baseCreateDeletions = function (terminalNode, oldWordNodes, startPos, endPos, costPenalty) {
	// The nodes for terminal rules that produce the `terminalSym`.
	var wordNodes = []
	// The size (i.e., span) of the terminal symbol and surround deletions.
	var wordSize = endPos - startPos + 1
