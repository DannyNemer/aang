...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = lastIdx; n > -1; --n) {
		...
		if (node.sym === nontermSym && node.size === size) {
			...
			if (subIsNew(node.subs, sub, this.position,  this.tokensLen)) {
				node.subs.push(sub)
			}
 ...
 */
function subIsNew(existingSubs, newSub, pos, tokensLen) {
	var newSubNext = newSub.next

	var newSubRuleProps = newSub.ruleProps
	newSubRuleProps = newSubRuleProps.constructor === Array ? newSubRuleProps[0] : newSubRuleProps
	...
	for (var s = existingSubs.length - 1; s > -1; --s) {
		...
		var oldSubRuleProps = oldSub.ruleProps
		oldSubRuleProps = oldSubRuleProps.constructor === Array ? oldSubRuleProps[0] : oldSubRuleProps
		if (newSubRuleProps.origRHS ? util.arraysEqual(oldSubRuleProps.rhs, newSubRuleProps.origRHS) : util.arraysEqual(oldSubRuleProps.origRHS, newSubRuleProps.rhs)) {
			// we are still suggestions with this impelemntation. Why?
			if (pos <= tokensLen) {
				// newSub is an insertion with an arry
				if (newSub.ruleProps.constructor === Array) {
					// first insertion is a blank
					if (newSubRuleProps.insertedSymIdx === undefined) {
						// save only blank version - remove others
						newSub.ruleProps = newSubRuleProps
						return true
					}

					// no blanks - reject all insertions
					return false
				}
				// new sub is single insertion
				else if (newSubRuleProps.insertedSymIdx !== undefined) {
					return false
				}
				// oldSub is insertion arry
				else if (oldSub.ruleProps.constructor === Array) {
					// first insertion is a blank
					if (oldSubRuleProps.insertedSymIdx === undefined) {
						// save only blank version - remove others
						oldSub.ruleProps = oldSubRuleProps
						return true
					}

					// remove sub for insertions, add original rule
					existingSubs.splice(s, 1)
					return true
				}
				// oldSub is single insertion
				else if (oldSubRuleProps.insertedSymIdx !== undefined) {
					// remove sub for insertions, add original rule
					existingSubs.splice(s, 1)
					return true
				}
