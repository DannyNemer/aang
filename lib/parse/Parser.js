...
/**
 * Resets the `minCost` property of all nonterminal nodes and subnodes in `nodeTabs` to `undefined`.
 *
			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				for (var s = 0, subsLen = subs.length; s < subsLen; ++s) {
					...
					if (ruleProps.isNonterminal || ruleProps.constructor === Array) {
						sub.minCost = sub.node.minCost = undefined

						var subNext = sub.next
						if (subNext) {
							subNext.minCost = subNext.node.minCost = undefined
						}
					}
				}
			}
