 ...
 * @param {boolean} [options.printStats=false] Specify printing `pfsearch` performance statistics.
...
Parser.prototype.parse = function (query, k, options) {
	...
	if (this.startNode) {
		...
		if (trees.length > 0) {
			// Return trees if `pfsearch` successfully generated legal parse trees (i.e., no contradictory, illegal semantics).
			return trees
		} else {
	...
	// After failing to reach the start node or failing to generate legal parse trees (due to contradictory, illegal semantics) on the initial parse, as a last resort (and a significant performance hit), mark all tokens as deletable (except those already added to `this.deletions`) and add new nodes with those deletions.
	...
	if (this.startNode) {
		...
		if (trees) {
			// If re-parsing after initially failing to generate legal parse trees, include the `pathCount` from the first parse.
