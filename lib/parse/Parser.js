...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			 ...
			 * sequence node. The comparisons require completing all reductions of
			 * a subnode to know its cheapest subnode (i.e., a nested term
			 * sequence) before reducing with its parent term sequence node.
			 * However, `Parser` may reduce a parent node with a given node before
			 * reducing all of the latter node's child nodes. Moreover, `Parser`
			 * can not determine if all reductions for a node are complete until
			 * parsing completes. Hence, a comparison at this state might have an
			 * inaccurate minimum cost.
			 ...
			 * subnode comparison in `calcHeuristicCosts` had to have reached the
			 * start node.
			 */
			if (sub.node.subs) {
 ...
 * child nodes (which occurs when `minCost === undefined`) only to have their
 * cost sums discarded (because already included in term sequences'
 * `ruleProps.cost`).
 ...
 *   cheapest child of a terms sequence node (which here would be from the first
 *   parse and already flattened) because they are semantically identical.
 *   Requires `pfsearch` to stop at the parent node of the term sequence, which
 *   requires `calcHeuristicCosts` to further flatten term sequences into
 *   insertions on their parent rules. This is planned.
 *
 * @private
 * @static
