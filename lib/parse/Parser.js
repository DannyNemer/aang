			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					...
					for (var s = 0, subsLen = subs.length; s < subsLen; ++s) {
						 ...
						 *
						 * Finds these subnodes as such, instead of marking flattened `ruleProps` instances as `wasTermSeq`, which is more efficient, because this re-parsing is a fallback for rarely poorly formed input queries that should not introduce any additional overhead to all other parses.
						 */
						if (subnode.node.subs && !subRuleProps.isNonterminal && subRuleProps.text) {
							/**
							 * Can assign new propery to `subRuleProps` because `subRuleProps` is gauranteed to have been created by `calcHeuristicCosts` specifically for this parse; it is not one of the immutable `ruleProps` in the `StateTable`.
							 *
							 * Need not worry about seeing the same `subRuleProps` instance multiple times here, which would wastefully reassign the same property and value, because thie method iterates through the set of all nodes, unlike `calcHeuristicCosts` which traverses the pars forest from its root.
							 */
							subRuleProps.wasTermSeq = true
						}
					}
				}
			}
