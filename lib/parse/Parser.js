...
Parser.prototype.shiftReduce = function (termRuleMatchTab) {
	...
	// The vertices for each state at each token index. For accessing previous indexes for terminal rule shifting in `Parser.prototype.shiftTerminalRuleNodes()`.
	this.vertTabs = util.new2DArray(termRuleMatchTab.length + 1)
	...
	while (this.curIdx <= this.tokensLen) {
		...
		if (this.curIdx < this.tokensLen) {
			this.nodes = this.nodeTabs[this.curIdx]
		}

		// Set `vertices` for adding states for this index.
		this.vertices = this.vertTabs[++this.curIdx]
		// Empty map for new vertices at this index.
...
Parser.prototype.shiftTerminalRuleNodes = function (termRuleMatches) {
	for (var w = 0, termRuleMatchLen = termRuleMatches.length; w < termRuleMatchLen; ++w) {
		...
		var prevVertices = this.vertTabs[termRuleMatch.startIdx]
...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	var nodes = this.nodes[nontermSym.index]
	if (!nodes) {
		// Create new node.
		var newNode = {
			// The LHS of `sub`.
		...
		// Save new node.
		this.nodes[nontermSym.index] = [ newNode ]
...
Parser.prototype.getStartNode = function () {
	// Check the vertex zNodes for the last parse index (same zNodes as `this.vertTabs`) for the accepting state vertex (state table index of 1).
	...
	var prevVertices = this.vertTabs[this.vertTabs.length - 2]
