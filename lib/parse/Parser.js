...
/**
 * Gets the node for `nontermSym` if it exists, else creates a new node, and adds `sub` to the node.
 *
 * @memberOf Parser
...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = this.nodeTab.length - 1; n > -1; --n) {
		node = this.nodeTab[n]
		if (node.sym === nontermSym && node.size === size) break
	}
	...
	// Add subnode to existing node if unique.
	else if (subIsNew(node.subs, sub)) {
		node.subs.push(sub)
	}
...
/**
 * Creates a vertex for `state` if no vertex exists.
 *
 * @memberOf Parser
...
Parser.prototype.addNode = function (node, oldVertex) {
	...
	// The zNode which holds `node` and vertices that point to `node`.
	var zNode
