	...
	for (var t = 0; t < this.tokensLen; ++t) {
		...
		if (entitySet) {
			...
			for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
				...
				var deletionCost = 0
				var deletion
				while (deletion = this.deletions[--startIdx]) {
				...
				if (deletion = this.deletions[t + 1]) {
					var endIdx = t + deletion.length
...
/**
 * Searches input for deletables. Saves the length and cost of each continuous span of deletables that begins at each token index, if any, as an object in the array `this.deletions`.
 *
 * @memberOf Parser
 * @param {string[]} tokens The input tokens.
 */
Parser.prototype.deletablesLookup = function (tokens) {
	// The length and cost of continuous deletables beginning at each token index, if any.
	this.deletions = []

	// Assign the length and cost of each continuous span of deletables that begins at each index, if any.
	for (var t = this.tokensLen - 1; t > -1; --t) {
		if (deletables.indexOf(tokens[t]) !== -1) {
			...
			this.deletions[t] = {
				...
				followingCost: nextDeletion ? nextDeletion.cost + nextDeletion.followingCost : 0,
				// The length of continuous deletables beginning at this index.
				length: nextDeletion ? nextDeletion.length + 1 : 1,
			}
		}
	}
}
 ...
 * @memberOf Parser
 */
Parser.prototype.findDeletions = function () {
	...
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var w = 0, wordsLen = origWordTabLengths[endIdx]; w < wordsLen; ++w) {
			...
			this.baseFindDeletions(terminalNode, wordNodes, word.start, endIdx)
		}
	}
}

/**
 * Searches input for deletables that precede `terminalNode` and/or follow `terminalNode` if at the end of input, and creates new nodes based on `wordNodes` with spans and cost penalties that include the deletables.
 *
 ...
 */
Parser.prototype.baseFindDeletions = function (terminalNode, wordNodes, startIdx, endIdx) {
	// Add nodes (in `this.addSub()`) to the input index of the terminal symbol's last lexical token.
	this.nodeTab = this.nodeTabs[endIdx]
	...
	while (deletion = this.deletions[--delStartIdx]) {
		...
		// Create new nodes based on `wordNodes` for the span beginning with the deletable(s) at `delStartIdx` and ending with this terminal symbol. Each new node has a cost penalty equal to the number of deleted tokens.
		this.addDeletion(terminalNode, wordNodes, delStartIdx, endIdx, deletionCost)
	}
...
Parser.prototype.matchTerminalRules = function (query) {
	...
	// Initialize the possible deletions in input. Saves the length and cost of each continuous span of deletables that begins at each token index, if any, as an object in the array `this.deletions`.
	this.deletablesLookup(tokens)
