...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	var lastIdx = this.nodes.length - 1
...
Parser.prototype.addVertex = function (state) {
	...
	for (var v = lastIdx; v > -1; --v) {
		...
		if (vertex.state === state) {
			// Reorder to cache last call. Tests report a 93% hit rate (i.e., 93% of sought vertices that already exist are in the last index).
			if (v !== lastIdx) {
