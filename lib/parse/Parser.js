...
Parser.prototype.parse = function (query) {
	...
	var startNode = this.baseParse(wordTab)
	...
	if (!startNode) {
		...
		startNode = this.baseParse(wordTab)
	}
 ...
 *
 * Steps through `wordTab`, performing shift steps for an index's matched terminal nodes, followed by a series of reduce steps to apply completed grammar rules, and repeating for each successive index.
 *
 * @memberOf Parser
