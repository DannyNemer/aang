...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			 ...
			 * 2. It would be inefficient for `Parser.prototype.addSub()` to search for the cheapest term sequence for parser nodes that might never reach the start node. In contrast, every term sequence subnode comparison in `calcHeuristicCosts` had to have reached the start node.
			 */
			if (sub.node.subs) {
 ...
 * May search vertices at two query indexes:
 *
 ...
 *
 * The property `ruleProps.wasTermSequence` instructs `calcHeuristicCosts` to avoid double counting the cost of the subnode's child nodes, because the flattened term sequence's `ruleProps.cost` value already includes the sum cost of its child nodes from the initial parse.
 *
...
function markTermSequences(subnodes) {
	for (var s = 0, subnodesLen = subnodes.length; s < subnodesLen; ++s) {
		...
		/**
		 * Identify term sequences flattened by `calcHeuristicCosts` on the initial parse: `subnode` has child nodes yet is marked terminal and has display text.
		 *
		 * Manually searches for these subnodes, instead of having `calcHeuristicCosts` mark flattened `ruleProps` instances as `wasTermSequence`, which would be more efficient, because the entire reparsing process is a fall back for rarely poorly formed input queries and should not introduce any additional overhead to normal parses.
		 */
		if (subnode.node.subs && !subRuleProps.isNonterminal && subRuleProps.text) {
			 ...
			 *
			 * Need not worry about visiting a `subRuleProps` instance multiple times, which would wastefully reassign the same property and value, because this method iterates through the set of all nodes, unlike `calcHeuristicCosts` which traverses the parse forest from its root.
			 */
