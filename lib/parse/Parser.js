			...
			for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
				...
				if (deletion = this.deletions[t + 1]) {
					...
					if (endIdx === this.tokensLen - 1) {
						startIdx = t
						deletionsCost = deletion.followingCost
 ...
 */
Parser.prototype.baseFindDeletions = function (terminalNode, origWordNodes, origStartIdx, endIdx) {
	// Add nodes (in `this.addSub()`) to the input index of the terminal symbol's last lexical token.
	this.nodeTab = this.nodeTabs[endIdx]
	...
	while (deletion = this.deletions[--startIdx]) {
		...
		this.addDeletion(terminalNode, origWordNodes, startIdx, endIdx, deletionsCost)
	}
 ...
 */
Parser.prototype.addDeletion = function (terminalNode, origWordNodes, startIdx, endIdx, costPenalty) {
	// The nodes for terminal rules that produce the `terminalSym`.
	var wordNodes = []
