...
/**
 * Checks if an n-gram from the input query is an entity. This is a simple, temporary entity resolution implementation that will be replaced by language models for each category and Elasticsearch for lookup.
 *
 * @memberOf Parser
 * @param {number} endIdx The end index in the input query of `nGram`.
 * @param {string} nGram A token from the input query.
 */
Parser.prototype.entityLookup = function (endIdx, nGram) {
	...
	if (entityInstances) {
		for (var e = 0, entityInstancesLen = entityInstances.length; e < entityInstancesLen; ++e) {
			...
			var terminalSym = this.stateTable.terminalSymTab[entity.category]

			var entityId = entity.id
			var semanticArg = this.semanticArgTab[entityId] || (this.semanticArgTab[entityId] = [ { semantic: { name: entityId } } ])

			// Generate all nodes for rules that produce the terminal symbol.
			this.createWords(terminalSym, endIdx, semanticArg, entity.text)
		}
	}
}
...
Parser.prototype.intSymbolLookup = function (unigramNum) {
	...
	for (var i = 0, intSymbolsLen = intSymbols.length; i < intSymbolsLen; ++i) {
		...
		if (parsedFloat <= intSymbol.max) {
			var terminalSym = this.stateTable.terminalSymTab[intSymbol.name]
			...
			var semanticArg = this.semanticArgTab[unigramNum] || (this.semanticArgTab[unigramNum] = [ { semantic: { name: unigramNum } } ])
 ...
 */
Parser.prototype.createWords = function (terminalSym, endIdx, semanticArg, text) {
	...
	var terminalNode = {
		...
		size: endIdx - this.position + 1,
		start: this.position,
	}

	// Set `size` to zero for the `<blank>` symbol (the symbol inserted at the end of the query). This enables the parse forest to include both trees that end with the `<blank>` symbol and those without, which would otherwise be excluded because of a different size (and seemingly not spanning the entire input).
	...
	// Check for deletables for all tokens, but not for the `<blank>` symbol.
	if (this.position < this.tokensLen) {
		this.createDeletions(terminalNode, wordNodes, endIdx)
	}
}

/**
 * Searches input for deletables that precede `terminalNode` and/or follow `terminalNode` if at the end of input, and creates new nodes based on `oldWordNodes` with spans and cost penalties that include the deletables.
 *
 * @param {Object} terminalNode The node for the terminal symbol to check for adjacent deletables.
 * @param {Object[]} oldWordNodes The nodes for the terminal rules that produce `terminalNode`.
 * @param {number} endIdx The end index of the terminal symbol in input.
 */
Parser.prototype.createDeletions = function (terminalNode, oldWordNodes, endIdx) {
	var leadingDelLen = 1
	var startPos = this.position - leadingDelLen

	// Step backward checking for continuous spans of deletable tokens that end at the start of this terminal symbol.
	while (this.deletableTokenSpans[startPos]) {
		// Create new nodes based on `oldWordNodes` for the span beginning with the deletable(s) at `startPos` and ending with this terminal symbol. Each new node has a cost penalty equal to the number of deleted tokens.
		this.baseCreateDeletions(terminalNode, oldWordNodes, startPos, endIdx, leadingDelLen)
...
/**
 * Create new nodes based on `oldWordNodes` with spans and cost penalties that include adjacent deletables.
 *
 * @param {Object} terminalNode The node for the terminal symbol to check for adjacent deletables.
 * @param {Object[]} oldWordNodes The nodes for the terminal rules that produce `terminalNode`.
 * @param {number} startPos The start index of the span of the terminal symbol and deletables.
 ...
 */
Parser.prototype.baseCreateDeletions = function (terminalNode, oldWordNodes, startPos, endIdx, costPenalty) {
	// The nodes for terminal rules that produce the `terminalSym`.
	var wordNodes = []
	// The size (i.e., span) of the terminal symbol and surround deletions.
	var wordSize = endIdx - startPos + 1

	// For each terminal rule, create a new node with a deletion cost penalty and the new token span.
	for (var n = 0, oldWordNodesLen = oldWordNodes.length; n < oldWordNodesLen; ++n) {
...
Parser.prototype.matchTerminalRules = function (query) {
	...
	// An array of arrays for each input token index, each of which holds nodes for terminal rules that produce matched terminal symbols in `query`.
	this.wordTab = []
		...
		if (isNaN(nGram)) {
			...
			while (true) {
				...
				if (terminalSym && !terminalSym.isPlaceholder) {
					this.createWords(terminalSym, endIdx)
				}
	...
	// Add an array for nodes for the `<blank>` symbol index at the end of the input query.
	this.nodeTabs[this.position] = []

	// Input the `<blank>` symbol at the end of parsing the input query to enable insertion rules that are only recognized at the end of a query. The `<blank>` symbol also enables deletions (with costs) for input tokens that precede it (i.e., the last token(s) of input).
	this.createWords(this.stateTable.terminalSymTab[grammar.blankSymbol], this.position)
...
Parser.prototype.parse = function (query) {
	...
	while (true) {
		...
		// Use a separate `nodeTab` for each token index, but share the same `nodeTab` for the last input token and the `<blank>` symbol which follows it. This allows the new nodes for insertions that use `<blank>` to merge with the trees that do not and would otherwise have a different span (i.e., size). It also enables multiple instances of insertions that use `<blank>` within a single tree.
		if (this.position < this.tokensLen) {
	...
	// When no start symbol is found found in the last `vertTab` (above), which is for the inserted `<blank>` symbol, then there were no successful reductions (for insertions) that included `<blank>`. This prevented the nodes in the final input token index from being brought over to the final `vertTab`. Hence, search the `vertTab` for the final input token index.
