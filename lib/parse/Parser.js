...
Parser.prototype.matchTerminalRules = function (query) {
	...
	// Initialize an array for nodes for each input token index.
...
Parser.prototype.parse = function (query) {
	...
	while (true) {
		...
		// Use a separate `nodeTab` for each token index, but share the same `nodeTab` for the last input token and the `<blank>` symbol which follows it. This allows the new nodes for insertions that use `<blank>` to be merged with the trees that do not and otherwise have a different span (i.e., size).
		if (this.position < this.tokensLen) {
