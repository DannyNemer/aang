	...
	for (var w = 0, termRuleMatchLen = termRuleMatches.length; w < termRuleMatchLen; ++w) {
		...
		for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
			...
			for (var v = 0; v < prevVerticesLen; ++v) {
				...
				var nextState = prevVertex.shifts[nodeSymIdx]
				if (nextState) {
...
Parser.prototype.addSub = function (nontermSym, sub) {
 	...
 	if (nodes) {
 		...
 		// Implement map as `Object` instead of `Array`, even though keys are numbers, because most often there are only 1-2 nodes (of different sizes) at the same index for the same symbol. An `Array` map required allocating space for arrays far much larger than needed.
 		nodes = this.nodeSets[nontermSym.index] = {}
 	}

 	// Create and save new node in map of size to node.
 	return nodes[size] = {
 		// The LHS of `sub`.
 		sym: nontermSym,
			...
			for (var v = 0; v < verticesLen; ++v) {
				...
				for (var z = 0, vertexZNodesLen = vertexZNodes.length; z < vertexZNodesLen; ++z) {
					...
					for (var v2 = 0, zNodeVerticesLen = zNodeVertices.length; v2 < zNodeVerticesLen; ++v2) {
						...
						this.addNode(node, prevVertex, prevVertex.shifts[nodeSymIdx])
					}
				}
			}
		} else {
...
Parser.prototype.resetMinCosts = function () {
	...
	for (var t = 0; t < this.tokensLen; ++t) {
		...
		for (var n = 0; n < nodeSetsLen; ++n) {
			...
			var nodeSizes = Object.keys(nodes)
			for (var i = 0, nodeSizesLen = nodeSizes.length; i < nodeSizesLen; ++i) {
				var node = nodes[nodeSizes[i]]
				if (!node) continue
				var subs = node.subs
