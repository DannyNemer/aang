...
Parser.prototype.createDeletions = function (terminalNode, oldWordNodes, endIdx) {
	var leadingDelLen = 1
	var startIdx = this.position - leadingDelLen
	...
	while (this.deletableTokenSpans[startIdx]) {
		// Create new nodes based on `oldWordNodes` for the span beginning with the deletable(s) at `startIdx` and ending with this terminal symbol. Each new node has a cost penalty equal to the number of deleted tokens.
		this.baseCreateDeletions(terminalNode, oldWordNodes, startIdx, endIdx, leadingDelLen)
	...
	if (endIdx === this.tokensLen - 1) {
		...
		do {
			this.baseCreateDeletions(terminalNode, oldWordNodes, startIdx, endIdx, trailingDelLen + leadingDelLen)
 ...
 */
Parser.prototype.baseCreateDeletions = function (terminalNode, oldWordNodes, startIdx, endIdx, costPenalty) {
	// The nodes for terminal rules that produce the `terminalSym`.
	var wordNodes = []
