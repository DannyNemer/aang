 ...
 *
 * If the parse succeeds, the returned array has the following properties:
 * @param {boolean} failedStartSym `true` if the parse initially fails to reach the start node, else `false`.
 * @param {boolean} noLegalTrees `true` if the parse initially fails to generate legal parse trees (due to illegal semantics), else `false`.
...
Parser.prototype.parse = function (query, k, options) {
	// Track whether the parse initially failed to reach the start node.
	var failedStartSym = false
	// Track whether the parse initially failed to generate any legal parse trees (due to illegal semantics).
	var noLegalTrees = false
	...
	this.startNode = this.baseParse(wordTab)

	if (this.startNode) {
		// Use A* path search to determine the `k`-best parse trees in the parse forest, along with the trees' associated semantic trees and conjugated display texts.
		var trees = pfsearch(this.startNode, k, options)
	...
	// After failing to reach the start node or failing to generate legal parse trees (due to illegal semantics) on the initial parse, as a last resort (and a significant performance hit), mark all tokens as deletable (except those already added to `this.deletions`) and add new nodes with those deletions.
	...
	if (this.startNode) {
		...
		var newTrees = pfsearch(this.startNode, k, options)

		if (trees) {
			// If re-parsing after initially failing to generate legal parse trees, include the `pathCount` from the first parse forest search.
			newTrees.pathCount += trees.pathCount
		}

		newTrees.noLegalTrees = noLegalTrees
		newTrees.failedStartSym = failedStartSym
 ...
 */
Parser.prototype.baseParse = function (wordTab) {
	...
	while (true) {
		...
		if (!words) {
			...
			// No recognized token at index. This array will not be used, but creating and checking its length (which occurs anyway) is faster than always checking for if array exists.
			++this.position
