 ...
 *
 * Also identifies all `ruleProps` that are term sequences flattened by `calcHeuristicCosts` (and no longer have the `isTermSequence` property), which instructs `calcHeuristicCosts` to update their `cost` values (the sum of the subtree that comprises the flattened term sequence) following the introduction by additional deletables of new possible parse trees.
 *
 * @memberOf Parser
 */
		...
		for (var n = 0; n < maxSymId; ++n) {
			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					...
					// Mark previously flattened term sequences as such, to instruct `calcHeuristicCosts` to update their `cost` values following new possible trees with the additional deletables.
