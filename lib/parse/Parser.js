...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			...
			// Return `null` if `sub` produces a terminal node to indicate `node` already existed. This allows `Parser.prototype.addWordNodes()` to avoid adding duplicate instances of `node` to `termRuleMatches`.
...
Parser.prototype.addNode = function (node, prevVertex, nextState) {
	// Track number of `Parser.prototype.addNode()` invocations. This profiling adds ~50 ms to the total `benchmark` duration.
	util.count('addNode')
