...
const pfsearch = require('./pfsearch')
...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = lastIdx; n > -1; --n) {
		...
		if (node.sym === nontermSym && node.size === size) {
			...
			if (n !== lastIdx) {
				this.nodes[n] = this.nodes[lastIdx]
				this.nodes[lastIdx] = node
			}
...
Parser.prototype.addNode = function (node, oldVertex) {
	...
	const state = oldVertex.state.shifts[node.sym.index]
	...
	const vertexZNodes = this.addVertex(state)
...
Parser.prototype.reduce = function (redZNode, reds) {
	const vertices = redZNode.vertices
