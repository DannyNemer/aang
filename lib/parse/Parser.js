...
Parser.prototype.addNode = function (node, oldVertex) {
	...
	var shift = oldVertex.state.shifts[node.sym.index]
	// Fails only for shifts; never for reductions.
	if (!shift) return

	// Avoid shifts that require more shifts than the number of remaining input tokens.
	if (this.position <= this.tokensLen && this.tokensLen - this.position < shift.minShiftsToAccept) {
		return
	}
