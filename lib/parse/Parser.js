		...
		for (var n = 0; n < maxSymId; ++n) {
			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					 ...
					 * • This implementation fails because those child nodes, though children of term sequences here, can also be the root of another term sequence elsewhere in the parse forest. Hence, `pfsearch` will reference their node's `minCost` values when expanding the current path with that node, and therefore the `minCost` would be incorrect as 0.
					 */
