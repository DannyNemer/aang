...
Parser.prototype.shiftReduce = function (wordTab) {
	...
	this.vertTab = this.vertTabs[this.curIdx]
	...
	while (this.curIdx <= this.tokensLen) {
		...
		// Set `vertTab` for `Parser.prototype.addNode()` invocations.
		this.vertTab = this.vertTabs[++this.curIdx]
	...
	return getStartNode(this.vertTab) || getStartNode(this.vertTabs[this.vertTabs.length - 2])
}
...
Parser.prototype.shiftTerminalRuleNodes = function (words) {
	for (var w = 0, wordsLen = words.length; w < wordsLen; ++w) {
		...
		var prevVertTab = this.vertTabs[word.startIdx]
		var prevVertTabLen = prevVertTab.length
		...
		for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
			...
			for (var v = 0; v < prevVertTabLen; ++v) {
				// Add the terminal rule node (if there exists a shift from `prevVertTab`).
				this.addNode(node, prevVertTab[v])
			}
		}
	}
}
...
/**
 * Searches `vertTab` for the start node for the accepting parse state, otherwise the parse failed.
 *
