...
Parser.prototype.createDeletions = function (terminalNode, origWordNodes, origStartIdx, endIdx) {
	...
	while (deletion = this.deletions[--startIdx]) {
		...
		this.baseCreateDeletions(terminalNode, origWordNodes, startIdx, endIdx, deletionsCost)
	}
...
Parser.prototype.matchTerminalRules = function (query) {
	...
	for (var i = this.tokensLen - 1; i > -1; --i) {
		...
		// Assign the length and cost of the continuous span of deletables that begins at index, if any.
		if (deletables.indexOf(tokens[i]) !== -1) {
			var nextDeletion = this.deletions[i + 1]
			this.deletions[i] = {
