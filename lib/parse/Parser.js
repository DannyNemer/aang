		...
		for (var n = 0; n < maxSymId; ++n) {
			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					...
					// Mark previously flattened term sequences to instruct `calcHeuristicCosts` to avoid double counting the cost of their child nodes, because the flattened term sequences' `cost` values already include the sum cost of their child nodes from the initial parse.
					for (var s = 0, subsLen = subs.length; s < subsLen; ++s) {
						var subnode = subs[s]
						var subRuleProps = subnode.ruleProps

						/**
						 * Check `subnode` has child nodes yet is marked terminal and has display text. Such properties are unique to term sequences flattened by `calcHeuristicCosts`.
						 *
