			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					...
					for (var s = 0, subsLen = subs.length; s < subsLen; ++s) {
						 ...
						 *
						 * Manually searches for these subnodes, instead of having `calcHeuristicCosts` mark flattened `ruleProps` instances as `wasTermSequence`, which is more efficient, because this entire re-parsing process is a fall back for rarely poorly formed input queries and should not introduce any additional overhead to all other parses.
						 */
						if (subnode.node.subs && !subRuleProps.isNonterminal && subRuleProps.text) {
							/**
							 * Can assign new property to `subRuleProps` because `subRuleProps` is guaranteed to have been created by `calcHeuristicCosts` specifically for this parse; it is not an immutable `ruleProps` in the `StateTable` instance.
							 *
