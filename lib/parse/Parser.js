...
Parser.prototype.parse = function (query, k, options) {
	...
	if (this.startNode) {
		...
		if (pfsearchResults.trees.length > 0) {
			...
			// Reset the `minCost` property of all nonterminal sub-nodes in `nodeTabs` to `undefined`. Most of the nodes will be reused in the second parse, and therefore require their `minCost` value be set to `undefined` to enable `calcHeuristicCosts` to re-calculate the heuristic costs of the new parse forest.
			this.resetMinCosts()
 ...
 * @param {Object} sub The sub-node (i.e., RHS) `nontermSym` produces.
...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			/**
			 * If `sub` is nonterminal, compare to existing sub-nodes.
			 *
			 * 99.9% of sub-nodes at this closure are nonterminal.
			 *
			...
			/**
			 * `sub` is terminal. Do not compare to existing sub-nodes because `Parser.prototype.addTermRuleNodes()` ensures all passed terminal sub-nodes at each token index are unique.
			 *
			 * There can be multiple terminal sub-nodes for the same parent node for matches to different entities in the same entity category at the same token index. The sub-nodes' `ruleProps` distinguish their identical terminal nodes.
			 *
			 * If a terminal symbol is deleted in input, either because it is a grammar-defined deletable or when re-parsing with all tokens marked deletable, there can also be duplicate instances of a non-entity terminal sub-nodes at the same index. Though, it is rare. For example:
			 *   "JavaScript Ruby repos mine mine"
	     * "mine" and "mine" will obviously be the same terminal rule. With "mine" marked deletable, there will be two sub-nodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine". `Parser.prototype.addTermRuleNodes()` could detect and prevent such duplicity, but the overhead is too great for its rarity.
 ...
 * @private
 * @static
 * @param {Object[]} existingSubs The sub-nodes to check.
 * @param {Object} newSub The new nonterminal sub-node.
...
function isNewSub(existingSubs, newSub) {
	...
	// Tests report 1.15x more likely to find the sub-node faster by iterating backward.
 ...
 *
 * @memberOf Parser
 * @param {Object} redZNode The zNode from which to build sub-nodes using `reds`.
