 ...
 */
Parser.prototype.entityLookup = function (tokens) {
	...
	for (var s = 1; s < this.tokensLen; ++s) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			var prevMatchedEntities = matchedEntitySets[entityObj.start - 1]
			...
			for (var p = 0, prevMatchedEntitiesLen = prevMatchedEntities.length; p < prevMatchedEntitiesLen; ++p) {
				...
				if (prevEntity.id === entity.id) {
					...
					matchedEntities.push({
						entity: entity,
						matchedTokens: matchedTokens,
						start: prevEntityObj.start,
						size: prevEntityObj.size + entityObj.size,
	...
	// Move entity matches to the array index of the start of their span. This is necessary so that the terminal nodes are added at the correct index.
	var matchedEntitySetsTemp = []
	for (var s = 0; s < this.tokensLen; ++s) {
		matchedEntitySetsTemp[s] = []

		var matchedEntities = matchedEntitySets[s]
		if (matchedEntities) {
			for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
				var entityObj = matchedEntities[e]
				matchedEntitySetsTemp[entityObj.start].push(entityObj)
			}
		}
	}
	matchedEntitySets = matchedEntitySetsTemp

	// Generate all nodes for rules that produce the entity terminal symbols.
	for (var s = 0; s < this.tokensLen; ++s) {
		var matchedEntities = matchedEntitySets[s]
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			var entityObj = matchedEntities[e]
			var entity = entityObj.entity
			var endIdx = entityObj.start + entityObj.size + entityObj.deletions - 1

			// Add nodes (in `this.addSub()`) to the input index of the terminal symbol's last lexical token.
			this.nodeTab = this.nodeTabs[endIdx]

			// Create node for terminal symbol.
			var terminalNode = {
				sym: this.stateTable.placeholderSymTab[entity.category],
				size: entityObj.size + entityObj.deletions,
				start: entityObj.start,
			}

			// The nodes for terminal rules that produce the `terminalSym`.
			var wordNodes = []
			var rules = terminalNode.sym.rules

			// The number of lexical tokens in the entity match.
			var wordSize = terminalNode.size

			// +1.0 cost penalty per inserted entity token or deletion
			var costPenalty = (entity.size - entityObj.size) + entityObj.deletions
			...
			// Create a node for each terminal rule that produces the entity terminal symbol.
			for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
				...
				var sub = {
					...
					ruleProps: ruleProps = {
						...
						// Use the semantic argument created from input.
...
Parser.prototype.intSymbolLookup = function (unigramNum) {
	...
	for (var i = 0, intSymbolsLen = intSymbols.length; i < intSymbolsLen; ++i) {
		...
		if (parsedFloat <= intSymbol.max) {
			...
			var semanticArg = this.semanticArgTab[unigramNum] || (this.semanticArgTab[unigramNum] = [ {
				semantic: {
					name: unigramNum,
				}
			} ])
 ...
 * @param {Object[]} [semanticArg] The semantic argument if the terminal symbol is a placeholder (i.e., the semantic created from an integer in input).
...
Parser.prototype.createWords = function (terminalSym, endIdx, semanticArg, text) {
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (semanticArg) {
			...
			sub.ruleProps = {
				...
				// Use the semantic argument created from input (e.g., integer).
				semantic: origRuleProps.semantic ? semantic.reduce(origRuleProps.semantic, semanticArg) : semanticArg,
	...
	// Check for deletables that precede `terminalNode` and/or follow `terminalNode` if at the end of input.
