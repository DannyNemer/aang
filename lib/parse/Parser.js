...
Parser.prototype.parse = function (query) {
	...
	while (true) {
		...
		// Remove the terminal node if it cannot follow the previous node.
		if (redsIdx === this.reds.length) {
			var curPos = this.position - 1

			if (!this.deletions[curPos]) {
				util.log(util.colors.blue.bold('DID STUFF!'), query.split(' ')[curPos])

				// Add the current position as a deletable with cost = 10
				var nextDeletion = this.deletions[curPos + 1]
				if (nextDeletion) {
					this.deletions[curPos] = {
						cost: 10,
				...
				// no delettions already created here because no deletables at this index
				// apply deletions to next wordTab idx
				this.addWordTabDeletions(curPos + 1)
