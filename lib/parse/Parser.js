 ...
 * @property {boolean} failedInitStartSym Indicates the parse initially failed to reach the start symbol (which required marking all input tokens as deletable and reparsing).
 * @property {boolean} failedInitLegalTrees Indicates the parse initially failed to generate any legal parse trees due to illegal semantics (which required marking all input tokens as deletable and reparsing).
...
Parser.prototype.parse = function (query, k, options) {
	...
	// Do not reparse if `query` is only one token or all of `query` has already been marked deletable. (The second parse can not produce anything different.)
	if (this.tokensLen === 1 || (this.deletions[0] && this.deletions[0].length === this.tokensLen)) {
		return parseResults
	}

	// After failing to reach the start symbol or failing to generate legal parse trees (due to illegal semantics) on the initial parse, as a last resort (and a significant performance hit), reparse the input query with all tokens marked deletable (except those already added to `this.deletions`) and add new nodes with those deletions.
	termRuleMatchTab = this.addDeletablesForAllTokens()

	// With the same `nodeTabs` index (including both the existing and new terminal nodes, in addition the previously added nonterminal nodes), reparse the terminal rule matches. As a temporary solution, reset all other data sets, including `vertices` and `reds`, slightly slowing performance.
	this.startNode = this.shiftReduce(termRuleMatchTab)
	...
	if (this.startNode) {
		...
		// If reparsing after initially failing to generate legal parse trees, include the `pathCount` from the first `pfsearch` invocation.
		parseResults.pathCount += pfsearchResults.pathCount
		parseResults.ambiguousTreeCount = pfsearchResults.ambiguousTreeCount
...
Parser.prototype.shiftReduce = function (termRuleMatchTab) {
	...
	// Get the start node for the accepting parse state that spans the entire input query, if any.
 ...
 *
 * A node is created for each instance of a nonterminal symbol with a unique input token index or unique input token span. The grammar generator currently forbids recursive sequences of unary rules, preventing recursive nodes. See "Recursive Node Restriction" in `calcHeuristicCosts` for a detailed explanation.
 *
...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	if (!nodes) {
		...
		var newNode = {
			...
			subs: [ sub ],
			// The heuristic estimate of the minimum cost to complete the parse tree from this node, assigned in `calcHeuristicCosts`.
			minCost: undefined,
		}
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			 ...
			 *
			 * Can not extend this check to avoid multiple matches for the same term sequence, which `calcHeuristicCosts` currently handles, for the following reasons:
			 * 1. This check requires choosing the cheapest subnode for a term sequence node. The comparisons require completing all reductions of a subnode to know its cheapest subnode (i.e., a nested term sequence) before reducing with its parent term sequence node. However, `Parser` may reduce a parent node with a given node before reducing all of the latter node's child nodes. Moreover, `Parser` can not determine if all reductions for a node are complete until parsing completes. Hence, a comparison at this state might have an inaccurate minimum cost.
			 * 2. It would be inefficient for `Parser.prototype.addSub()` to search for the cheapest term sequence for parse nodes that might never reach the start node. In contrast, every term sequence subnode comparison in `calcHeuristicCosts` had to have reached the start node.
			 */
			if (sub.node.subs) {
			...
			/**
			 * `sub` is terminal. Do not compare to existing subnodes because `Parser.prototype.addTermRuleNodes()` ensures all passed terminal subnodes at each token index are unique.
			 *
			 * There can be multiple terminal subnodes for the same parent node for matches to different entities in the same entity category at the same token index. The subnodes' `ruleProps` distinguish their identical terminal nodes.
			 *
			 * If a terminal symbol is deleted in input, either because it is a grammar-defined deletable or when reparsing with all tokens marked deletable, there can also be duplicate instances of a non-entity terminal subnodes at the same index. Though, it is rare. For example:
			 *   "followers of mine mine"
	     * "mine" and "mine" will obviously be the same terminal rule. With "mine" marked deletable, there will be two subnodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine". `Parser.prototype.addTermRuleNodes()` could detect and prevent such duplicity, but the overhead is too great for its rarity.
	     ...
	     * • As is, `isSubNew()` would not detect these non-entity duplicate terminal nodes because they are different `Object` instances (i.e., requires comparing additional properties).
			 *
			 * Return `null` to indicate `node` existed before invoking this method to instruct `Parser.prototype.addWordNodes()` to not add multiple instances of `node` to `this.termRuleMatches`.
			 */
			node.subs.push(sub)
			return null
		}
...
function isNewSub(existingSubs, newSub) {
	...
	for (var s = existingSubs.length - 1; s > -1; --s) {
		...
		if (newSubNextNode && (oldSub = oldSub.next)) {
			...
			// Only occurs for the `<blank>` symbol, which has a `size` of 0 and can produce a binary reduction for a node with the same size as a non-binary reduction for the same node.
			continue
		}
...
Parser.prototype.addNode = function (node, prevVertex, nextState) {
	...
	// Get the zNode for `node` for `nextState`. All zNodes have the same `node.sym` but different `node.size`. Tests report optimal to iterate backward.
...
Parser.prototype.addVertex = function (state) {
	...
	this.vertices.push({
		...
		shifts: state.shifts,
		// The nodes that point this vertex. The nodes all have the same symbol, but different span/size (i.e., a different z-index position). For reducing binary rules in `Parser.prototype.reduce()`.
		zNodes: newZNodes,
	})
	...
	for (var r = 0, redsLens = reds.length; r < redsLens; ++r) {
		...
		if (red.isBinary) {
			...
			for (var v = 0; v < verticesLen; ++v) {
				// The zNodes that point to this vertex. I.e., every instance of the symbol at different heights within the tree (different spans).
...
Parser.prototype.getStartNode = function () {
	...
	if (acceptingStateZNodes) {
		// Get index 0 because there can be only instance (i.e., size/span) of the start symbol at each index.
...
/**
 * Resets the `minCost` property, which is the cost heuristic for parse forest search, of all nonterminal nodes in `nodeTabs` to `undefined`.
 *
 * After `pfsearch` fails to generate legal parse trees (due to illegal semantics), `Parser.prototype.parse()` marks all tokens as deletable and re-generates the parse forest. The nodes constructed during the first parse remain in `nodeTabs`, and most are reused and therefore require their `minCost` value be set to `undefined` to enable `calcHeuristicCosts` to re-calculate the heuristic costs of the new parse forest.
 *
 * Identifies all `ruleProps` that are term sequences flattened by `calcHeuristicCosts` (and no longer have the `isTermSequence` property), which instructs `calcHeuristicCosts` to avoid double counting the cost of their child nodes, because the flattened term sequences' `cost` values already include the sum cost of their child nodes from the initial parse.
 *
 * Invoke this method after `pfsearch` fails on the first parse and before reparsing input with all tokens marked as deletable. Ensures method only operates on nodes from the initial parse.
 *
 * @memberOf Parser
 */
		...
		for (var n = 0; n < maxSymId; ++n) {
			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					...
					// Mark previously flattened term sequences in `subs`, if any, by assigning the boolean property `wasTermSequence` to their `ruleProps`.
...
/**
 * Marks previously flattened term sequences in `subnodes`, if any, by assigning the boolean property `ruleProps.wasTermSequence`.
 *
 * The property `ruleProps.wasTermSequence` instructs `calcHeuristicCosts` to avoid double counting the cost of the subnode's term sequence child nodes, because the flattened term sequence's `ruleProps.cost` value already includes the sum cost of its child nodes from the initial parse.
 *
 * For use by `Parser.prototype.resetMinCosts()` after `pfsearch` fails to generate legal parse trees, and before `Parser.prototype.parse()` reparses input with all tokens marked deletable.
 *
 * Failed alternate implementation:
 * • Avoid double counting the cost of child nodes by setting their `minCost` values to 0. Then `calcHeuristicCosts` need not backtrack the double counting as the current implementation requires.
 * • Would also prevent `calcHeuristicCosts` from needlessly traversing these child nodes (which occurs when `minCost === undefined`) only to have their cost sums discarded (because already included in term sequences' `ruleProps.cost`).
 * • Fails: Those child nodes, though children of term sequences here, can also be the root of another term sequence elsewhere in the parse forest. Hence, `pfsearch` will reference their node's `minCost` values when expanding the current path with that node, and therefore the `minCost` would be incorrect as 0.
 *
 * Failed alternate implementation:
 * • Do not reset `minCost` values of nodes from the first parse, and manually invoke `calcHeuristicCosts` on each new node from the second parse (identified by `minCost === undefined`). As a result, `calcHeuristicCosts` will not see term sequences from the first parse because their parent nodes' `minCost` values are defined. This prevents these term sequences, with `cost` values that include the sum of their child nodes' costs, from contributing an incorrect value to their parent nodes' `minCost` that double counts their child nodes' costs.
 *   - All new nodes from the second parse added as subnodes to nodes from the first parse (and therefore skipped by this implementation) are more expensive than their sibling child nodes because they include deletion costs. Hence, they would not change the existing `minCost` value of their parent nodes from the first parse and can be skipped.
 * • Drawback: Invoking `calcHeuristicCosts()` on every node with `minCost === undefined` in this iteration wastefully operates on nodes unreachable from the start node (which can double the processing). Though, this is negligible considering this method is a fall back for rare cases.
 * • Fails: Without reseting `minCost` values of original nodes, `calcHeuristicCosts` will not flatten new term sequences added as child nodes to the initial parse's nodes because parent nodes are only passed if `minCost === undefined`. It is inconsequential that their cost values are excluded, because they are guaranteed to be ambiguous and more expensive, and therefore ultimately discarded. However, `pfsearch` will fail when encountering these non-flattened term sequences.
 *   – This would not occur if `pfsearch` only visited (or had access to) the cheapest child of a terms sequence node (which here would be from the first parse and already flattened) because they are semantically identical. Requires `pfsearch` to stop at the parent node of the term sequence, which requires `calcHeuristicCosts` to further flatten term sequences into insertions on their parent rules. This is planned.
 *
 * @private
 * @static
...
function markTermSequences(subnodes) {
	for (var s = 0, subnodesLen = subnodes.length; s < subnodesLen; ++s) {
		...
		/**
		 * Identify term sequences flattened by `calcHeuristicCosts` on the initial parse:
		 * 1. `subnode` has child nodes yet is marked terminal and has display text.
		 * 2. `subnode` is binary yet has `insertedSymIdx` (i.e., partial term sequences).
		 *
		 * Manually search for these subnodes, instead of having `calcHeuristicCosts` mark flattened `ruleProps` instances as `wasTermSequence` (which would be more effecient), because this entire reparsing process is a rare fall back for poorly formed input queries and should not introduce any additional overhead to normal parses.
		 */
		if ((subnode.node.subs && !subRuleProps.isNonterminal && subRuleProps.text) ||
				(subnode.next && subRuleProps.insertedSymIdx !== undefined)) {
			/**
			 * It is safe to assign a new property to `subRuleProps` because `subRuleProps` is guaranteed to have been created by `calcHeuristicCosts` specifically for this parse; it is not an immutable `ruleProps` in the `StateTable` instance.
			 *
			 * This iteration will not visit the same `subRuleProps` instance multiple times, which would wastefully reassign the same property and value, because this method iterates through the set of all nodes, unlike `calcHeuristicCosts` which traverses the parse forest from its root.
			 */
