...
Parser.prototype.parse = function (query, k, options) {
	...
	if (!this.startNode) {
		// After failing to reach the start node on the initial parse, as a last resort (and a significant performance hit), mark all tokens as deletable (except those already added to `this.deletions`) and add new nodes with those deletions.
	...
	if (this.startNode) {
		...
		return pfsearch(this.startNode, k, options)
	}
}
