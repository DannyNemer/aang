...
Parser.prototype.parse = function (query, k, options) {
	// Indicate whether the parse initially failed to reach the start symbol (before marking all input tokens as deletable).
	var failedInitStartSym = false
	// Indicate whether the parse initially failed to generate any legal parse trees (due to illegal semantics before marking all input tokens as deletable).
	var failedInitLegalTrees = false
	...
	if (this.startNode) {
		...
		if (trees.length > 0) {
			...
			this.resetMinCosts()
			failedInitLegalTrees = true
		}
	} else {
		failedInitStartSym = true
	}
	...
	if (this.startNode) {
		...
		newTrees.failedInitLegalTrees = failedInitLegalTrees
		newTrees.failedInitStartSym = failedInitStartSym

		return newTrees
	} else {
		// Return empty array if parse never reaches start symbol.
		var emptyArray = []
		emptyArray.failedInitStartSym = failedInitStartSym
		return emptyArray
	}
}
