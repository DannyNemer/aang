...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			...
			// Return `null` if `sub` produces a terminal node to indicate `node` existed before invoking this method. This allows `Parser.prototype.addWordNodes()` to avoid adding duplicate instances of `node` to `this.termRuleMatches`.
