 ...
 * @returns {Object|null} Returns the node to which `sub` was added if `sub` is nonterminal or the node did not already exist (in `nodeTabs` index), else `null`.
 */
Parser.prototype.addSub = function (nontermSym, sub) {
	var size = sub.size

	var nodes = this.nodeSets[nontermSym.index]
	if (!nodes) {
		// Create new node.
		var newNode = {
			// The LHS of `sub`.
		...
		// Save new node.
		this.nodeSets[nontermSym.index] = [ newNode ]
		...
		for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
			...
			if (node.size === size) {
				...
				// Return `null` if `sub` produces a terminal node. This enables `Parser.prototype.addWordNodes()` to avoid adding duplicate instances of `node` to `termRuleMatches`.
...
/**
 * Gets the next state for `node.sym` that follows `prevVertex.state`. Then, gets the vertex for that state if it exists, else creates a new vertex. Then, gets that vertex's zNode for `node` if it exists, else creates a new zNode. Then, adds `prevVertex` to that zNode if it is unique.
 *
 * @memberOf Parser
 * @param {Object} node The new node.
 * @param {Object} prevVertex The vertex that points to `node` (i.e., the previous state).
 */
Parser.prototype.addNode = function (node, prevVertex) {
	// Get the next state that follows `prevVertex.state` for `node.sym`, if any.
	var state = prevVertex.shifts[node.sym.index]
		...
		if (red.isBinary) {
			...
			for (var v = 0; v < verticesLen; ++v) {
				...
				for (var z = 0, vertexZNodesLen = vertexZNodes.length; z < vertexZNodesLen; ++z) {
					...
					// Add `node` for the shift from each preceding vertex. Create new vertex for each shift's (next) state with the preceding vertex pointing to it. (There will always be a shift for the reduction's LHS symbol.)
					var zNodeVertices = zNode.vertices
