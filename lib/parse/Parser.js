...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	for (var n = 0, nodesLen = nodes.length; n < nodesLen; ++n) {
		...
		if (node.size === size) {
			 ...
			 *
			 * If a terminal symbol is deleted in input, either because it is a grammar-defined deletable or when re-parsing with all tokens marked deletable, there can also be duplicate instances of a non-entity terminal subnodes at the same index. Though, it is rare. For example:
 ...
 *
 * Invoke this method after `pfsearch` fails on the first parse and before re-parsing input with all tokens marked as deletable. Ensures method only operates on nodes from the initial parse.
 *
 * @memberOf Parser
 */
 ...
 *
 * For use by `Parser.prototype.resetMinCosts()` after `pfsearch` fails to generate legal parse trees, and before `Parser.prototype.parse()` re-parses input with all tokens marked deletable.
 *
...
function markTermSequences(subnodes) {
	for (var s = 0, subnodesLen = subnodes.length; s < subnodesLen; ++s) {
		 ...
		 *
		 * Manually searches for these subnodes, instead of having `calcHeuristicCosts` mark flattened `ruleProps` instances as `wasTermSequence`, which would be more efficient, because the entire re-parsing process is a fall back for rarely poorly formed input queries and should not introduce any additional overhead to normal parses.
		 */
