			...
			for (var i = 0, nodesLen = nodes.length; i < nodesLen; ++i) {
				...
				if (subs) {
					...
					for (var s = 0, subsLen = subs.length; s < subsLen; ++s) {
						 ...
						 *
						 * Finds these subnodes as such, instead of marking flattened `ruleProps` instances as `wasTermSequence`, which is more efficient, because this re-parsing is a fallback for rarely poorly formed input queries that should not introduce any additional overhead to all other parses.
						 */
