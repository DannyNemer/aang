...
Parser.prototype.addSub = function (nontermSym, sub) {
	...
	// Add subnode to the existing node if unique.
...
Parser.prototype.addNode = function (node, oldVertex) {
	...
	for (var v = vertexZNodes.length - 1; v > -1; --v) {
		if (vertexZNodes[v].node === node) break
	}
	...
	if (v === -1) {
		...
		// Add `oldVertex` to the existing `zNode` vertices if unique, so that `oldVertex` points to `zNode`.
		var zNodeVertices = vertexZNodes[v].vertices
