...
/**
 * Calculates the heuristic estimate of the minimum cost of a complete subtree (i.e., reaches terminal nodes) that descends from `childSubs`, which is assigned to the corresponding parent node and subnode (of `childSubs`).
 *
 ...
 * @returns {number} Returns the minimum cost of a subtree that descends from `childSubs` to assign to the corresponding parent node and subnode (of `childSubs`).
 */
function calcMinCost(childSubs) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		// Only nonterminal nodes and subnodes have a `minCost` (which does not include the cost of the subnode itself).
		if (childSub.minCost === undefined) {
			// All nodes where `minCost` is `undefined` have subnodes.
			// Can leave property `undefined` while traversing because this sub-node cannot be a sub-node of itself because a unique node is created for each instance of a nonterminal symbol with a different size/span.
			...
			if (childSubNext) {
				// Do not inspect same subnode or node more than once (a subnode can belong to more than one node).
				if (childSubNext.minCost === undefined) {
					...
					if (childNodeNext.minCost === undefined) {
						...
						childSubNext.minCost = childNodeNext.minCost = calcMinCost(childNodeNext.subs)
					} else {
						childSubNext.minCost = childNodeNext.minCost
					}
				}
