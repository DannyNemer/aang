...
/**
 * Gets the cheapest subnode the term sequence `subNode` produces (i.e., in `subNode.node.subs`).
 *
 * @private
 * @static
 * @param {Object} subNode The term sequence subnode.
 * @returns {Object} Returns the cheapest subnode `subNode` produces.
 */
function getChildSub(subNode) {
	 ...
	 *
	 * 2. After `Parser` fails to reach the start node or `pfsearch` fails to generate legal parse trees (due to contradictory semantics) on the initial parse, as a last resort, `Parser` re-parses the input query with all tokens marked deletable. This enables the following possible instances of term sequence ambiguity:
	 ...
	 *      As shown, the term sequence `Y` recognizes the input "a b" and "a" with insertion text "b". Provided the input "a b", and with all input tokens marked deletable, `Y` has two matches to the same token span "a b", "a <b>" (where "b" is deleted).
	 ...
	 *      "mine" and "mine" will obviously be the same terminal rule. With all tokens marked deletable, there will be two subnodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine". This is unavoidable in the grammar.
	 *      Moreover, these duplicate instances of the same LHS symbol over the same span could be detected in `Parser.prototype.addTermRuleNodes()` with a terminal node table, similar to the nonterminal node table used in `Parser.prototype.addSub()`, but the overhead is too great for such rarity.
	 *
	 * Most instances of term sequence ambiguity can be caught when a term sequence is passed to `Parser.prototype.addSub()` from `Parser.prototype.addTermRuleNodes()`; however, it would be inefficient to search for the cheapest term sequence for parser nodes that might never reach the start node. In contrast, every term sequence subnode comparison in `calcHeuristicCosts` had to have reached the start node.
	 *
	 * Furthermore, these cost comparisons will not always work in `Parser.prototype.addSub()` because these comparison require completing all reductions of a subnode before reducing with its parent term sequence node. This is because `Parser` may reduce a parent node with a given node before reducing all of the latter node's child nodes. Moreover, `Parser` can not determine if all reductions for a node are complete until the end. For example:
	 *   X -> A -> "z", cost: 0.5
	 *   X -> B -> "z", cost: 1
	 *        B -> C -> "z", cost: 0
	 * Suppose the reduction for `X -> B -> "z"` is compared against `X -> A -> "z"`, found to be more expensive, and discarded. Later, the parser reduces the third rule sequence above, `B -> C -> "z"`, which makes the reduction `X -> B` cheaper than `X -> A`; however, it is too late because the `X -> B` reduction was already discarded and will not reappear.
	 */
