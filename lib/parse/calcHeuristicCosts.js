...
function calcMinCost(sub) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		if (childSub.minCost === undefined) {
			...
			// If a is nonterminal no produces neither display text nor semantics, then reduce its subnodes' costs (which include deletion costs on the terminal nodes), so that `pfsearch` does not wastefully traverse those subnodes. This includes nonterminal substitutions where `text` is defined on a nonterminal rule, nonterminal stop-words, and rules that produce only stop-words.
			if (childSubRuleProps.noText) {
