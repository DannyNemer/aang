...
/**
 * Creates a string representation of `binarySubNode` for printing to the console.
 *
 * Formats the node as follows: ''[rhs-a]' '[rhs-b]'.
 *
 * @private
 * @static
 * @param {Object} binarySubNode The binary subnode to stringify
 * @returns {string} Returns the binary subnode string representation.
 */
function stringifyBinarySubNode(binarySubNode) {
	var leftSymName = binarySubNode.node.sym.name
	var rightSymName = binarySubNode.next.node.sym.name
	return util.stylize(leftSymName) + ' ' + util.stylize(rightSymName)
}
