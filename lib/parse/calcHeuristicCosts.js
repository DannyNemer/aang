...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		 ...
		 * 2. text - Merge the `text` values of this subnode's child nodes (i.e., the matched terminal rules) for `pfsearch` to use as display text.
		 * 3. tense - Get the tense of any matched verb terminal rules `childSub` produces to maintain tense for any verb substitutions whose parent rule has matching `acceptedTense`.
		 * 4. semantic - Keep `childSub.ruleProps.semantic`, if any. The rules it produces can not have semantics.
		 * 5. cost - Use `cost`, the minimum cost for `childSub`, as the new `childSub.ruleProps.cost` because it is identical to the cumulative cost (including any deletion costs) of the subtree `childSub` produces, which will be inaccessible after assigning this new, terminal `ruleProps`. If there are multiple subtrees, then they are ambiguous because their text and semantics are defined here (identically), and the minimum cost subtree would be chosen anyway.
		 */
 ...
 * 2. text - Merges the `text` values of this subnode's child nodes (i.e., the matched terminal rules) for `pfsearch` to use as display text.
 * 3. tense - Gets the tense of any matched verb terminal rules `subNode` produces to maintain tense for any verb substitutions whose parent rule has matching `acceptedTense`.
 * 4. semantic - Keeps `subNode.ruleProps.semantic`, if any.
 * 5. cost - Uses `cost`, the cumuluative cost (including any deletion costs) of the subtree `subNode` produces, which will be inaccessible after assigning this new, terminal `ruleProps`
 *
 * @private
 * @static
