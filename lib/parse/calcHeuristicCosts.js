...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		 ...
		 *
		 * Note: In the grammar, partial term sequences lack the property `isTermSequence` and are instead only distinguished by the property `rhsTermSequenceIndexes`. `StateTable` extends partial term sequences with `isTermSequence` to simply the check below.
		 */
		 ...
		 *
		 * Even though reparsing with all tokens marked deletable introduces new parse trees to the parse forest, this will not change any previously matched term sequences because any new subtree for such sequences must be ambiguous (because all lack semantics). The new subtree is guaranteed to have deletion costs and can neither possibly be the cheapest subtree to contribute the `minCost` nor be used in `pfsearch`.
		 *
		 * Alternative implementation: Prevent traversal of already flattened term sequences on reparse.
		 * 1. Remove this block.
		 * 2. Add the following conditional around the `calcMinCost(childNode)` invocation:
		 *    `!(childSubRuleProps.wasTermSequence && childSubRuleProps.insertedSymIdx === undefined)`
		 * 3. Extend the conditional surround the `calcMinCost(childNodeNext)` invocation as such:
		 *    `childSubNext && !childSubRuleProps.wasTermSequence`
		 * Fails: This implementation is slower than the current implementation because it increases the number of conditional checks of all parses, even though it removes traversal of term sequences and `cost` reassignment during reparses. Reparses only account for 1% of term sequence flattening.
		 */
		else if (childSubRuleProps.wasTermSequence) {
			// Before this reassignment, `cost` is higher than `childSubRuleProps.cost` because `cost` is `childSubRuleProps.cost` plus the cost of its child nodes, though that cost is already in `childSubRuleProps.cost`. Reassigning here prevents double counting the cost of the term sequence child nodes.
			cost = childSubRuleProps.cost

			// If `childSub` is a partial term sequence, for which one of its two child subnodes is a term sequence and was flattened into an insertion, add the `minCost` of the non-sequence subnode whose cost was never added to `childSubRuleProps.cost` (unlike the term sequence child subnode).
			if (childSubNext && childSubRuleProps.insertedSymIdx !== undefined) {
				cost += childNode.minCost
			}

			// Need not mark `wasTermSequence` as `false` to prevent multiple assignments to the same `childSubRuleProps` instance because the `childNode.minCost` assignment above prevents multiple visits to the `childSub` that owns it `childSubRuleProps`.
		}
