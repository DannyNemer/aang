...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		// When re-parsing with all tokens marked deletable (after failing the initial parse), avoid double counting the cost of term sequence child nodes. On the first parse, `calcHeuristicCosts` adds the cost of the term sequences' child nodes to the flattened term sequences because the child nodes are not visited afterward. To avoid double counting the cost of those child nodes, overwrite `cost` with the preciously calculated cost of the term sequence. Even though re-parsing introduces new parse trees to the parse forest, this will not change any term sequences because any new term sequence sub-tree is ambiguous (because both lack semantics) the new sub-tree must have deletion costs, meaning it is not the cheapest version and would neither contribute the `minCost` nor be used in `pfsearch`.
...
function getChildSub(subNode) {
	...
	if (subNode.size === 1 || subNode.node.minCost < 1) {
		util.logError('Term sequence ambiguity caugsed by ill-formed grammar (i.e., not caused by deletion):', subNode)
		throw new Error('Ill-formed term sequence')
	}
	...
	for (var s = 1, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		var childSub = childSubs[s]
