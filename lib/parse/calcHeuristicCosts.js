...
function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		// Can leave `minCost` `undefined` while traversing because this node can not be a subnode of itself because a unique node is created for each instance of a nonterminal symbol with a different size/span. This is true as long as recursive insertion rules are forbidden.
