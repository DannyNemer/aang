...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		 ...
		 */
			...
			if (rule.text) {
				/**
				 * If `childSub` is a binary term set with text, then it is a multi-token substitution. Create a new `ruleProps` as explained above, extended as follows:
				 ...
				 * 5. tense - Get the tense of any matched verb terminal rules `childSub` produces to maintain tense for any verb substitutions whose parent rule has matching `acceptedTense`.
				 */
				 ...
				 * 4. text - Merge the `text` values of this subnode's child nodes (i.e., matched terminal rules) for `pfsearch` to use as display text.
				 * 5. tense - Get the tense of any matched verb terminal rules `childSub` produces to maintain tense if the parent rule has matching `acceptedTense`.
				 */
 ...
 * 4. text - Merges the `text` values of this subnode's child nodes (i.e., the matched terminal rules) for `pfsearch` to use as display text.
 * 5. tense - Gets the tense of any matched verb terminal rules `childSub` produces to maintain tense if the parent rule has matching `acceptedTense`.
 *
 * @private
 * @static
 ...
 * 5. tense - Gets the tense of any matched verb terminal rules `subNode` produces to maintain tense for any verb substitutions whose parent rule has matching `acceptedTense`.
 *
 * @private
 * @static
