...
function createBinaryTermSetRuleProps(subNode, cost) {
	...
	if (subNodeRuleProps.insertedSymIdx !== undefined) {
		util.logError('`createBinaryTermSetRuleProps()` invoked on subnode with `insertedSymIdx` (should use `createTermSetSeqInsertRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
...
function createTermSetSeqInsertRuleProps(subNode, cost) {
	...
	if (!subNodeRuleProps.text) {
		util.logError('`createTermSetSeqInsertRuleProps()` invoked on subnode without insertion text (should use `createBinaryTermSetRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
