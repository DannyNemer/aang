...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		 ...
		 * 4. Mark `childSub` as terminal to prevent `pfsearch` from traversing past `childSub`. Instead `pfsearch` uses `childSub.ruleProps.text` and `childSub.ruleProps.tense` for gnerating display text.
		 */
