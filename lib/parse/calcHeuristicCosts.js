...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		/**
		 * When re-parsing with all tokens marked deletable (after failing the initial parse), avoid double counting the cost of term sequence child nodes.
		 *
		 ...
		 *
		 * Even though re-parsing with all tokens marked deletable introduces new parse trees to the parse forest, this will not change any term sequences because any new term sequence sub-tree is ambiguous (because all lack semantics). The new sub-tree is guaranteed to have deletion costs and can neither possibly be the cheapest sub-tree to contribute the `minCost` nor be used in `pfsearch`.
		 */
