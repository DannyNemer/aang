...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		/**
		 * If `childSub` is a binary term set, traverse its childes and do the following:
		 * 1. Merge the `text` values of its child nodes (i.e., matched terminal rules).
		 * 2. Get the input tense of any matched verb terminal rules (for `text` conjugation).
		 * 3. Assign both the merged `text` (#1) and input tense (#2), if any, to `childSub.ruleProps`.
		 * 4. Mark `childSub` as terminal to prevent `pfsearch` from traversing past `childSub`. Instead `pfsearch` uses `childSub.ruleProps.text` and `childSub.ruleProps.tense` for generating display text.
		 */
