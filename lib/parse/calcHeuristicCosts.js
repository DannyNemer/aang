...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		if (childSubRuleProps.rhsDoesNotProduceText) {
			 ...
			 *
			 * Use `cost`, the minimum cost for `childSub`, as the new `childSub.ruleProps.cost` because it is identical to the cumulative cost of the subtree that will be inaccessible after assigning this new, terminal `ruleProps`. If there are multiple subtrees, then they are ambiguous because their text and semantics are defined here, and its minimum cost subtree will be chosen anyway.
			 */
			childSub.ruleProps = {
				cost: cost,
		...
		/**
		 * If `childSub` is a binary term set (without text), then create a new `ruleProps` for this subnode with the following properties:
		 * 1. terminal - Mark the new `ruleProps` as terminal so that `pfsearch` uses this node's `ruleProps` to generate display text and does not traverse its child nodes.
		 * 2. text - Merge the `text` values of this subnode's child nodes (i.e., the matched terminal rules).
		 * 3. tense - Get the tense of any matched verb terminal rules this node produces to maintain tense for any verb substitutions whose parent rule has matching `acceptedTense`.
		 * 4. semantic - Keep this rule's `ruleProps.semantic`, if any. The rules it produces can not have semantics.
...
/**
 * Traverses `subNode`'s child nodes and does the following:
 * 1. Merges the `text` values of its child nodes.
 * 2. Gets the input tense of the child nodes for any matched verb terminal rules.
 * 3. Creates a new `ruleProps` object for `subNode` with the following properties:
 *    (a) `cost`, the cumulative cost of the subtree `subNode` produces.
 *    (b) The merged `text` (from #1).
 *    (c) The input tense, if any (from #2).
 *    (d) The semantic `subNode.ruleProps.semantic`, if any.
 *
 * The resulting `ruleProps` is no longer marked `isNonterminal`, to prevent `pfsearch` from traversing past `subNode` and instead use `ruleProps.text` and `ruleProps.tense` to generate display text.
 *
 * @private
 * @static
 * @param {Object} subNode The binary term set subnode for which to generate a new, terminal `ruleProps` by merging the `text` and `tense` values of its child nodes.
 * @param {number} cost The cumulative cost of the subtree `subNode` produces.
 * @returns {Object} Returns the new, terminal `ruleProps` for `subNode`, generated from its child nodes.
 */
function createBinaryTermSetRuleProps(subNode, cost) {
	...
	return {
		...
		/**
		 * Currently, the grammar forbids a rule marked `rhsIsBinaryTermSet` to produce multiple verbs, which prevents multiple instances of `tense`. Hence, this ensures `pfsearch` references the correct `tense` for the correct verb.
		 *
