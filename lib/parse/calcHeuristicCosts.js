...
function createTermSetSeqRuleProps(subNode, cost) {
	...
	if (subNodeRuleProps.text) {
		util.logError('`createTermSetSeqRuleProps()` invoked on subnode with substitution text (should use `createBinaryTermSetSubstitutionRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
...
function createBinaryTermSetSubstitutionRuleProps(subNode, cost) {
	...
	if (!subNodeRuleProps.text) {
		util.logError('`createBinaryTermSetSubstitutionRuleProps()` invoked on subnode without substitution text (should use `createTermSetSeqRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}

	if (subNodeRuleProps.insertedSymIdx !== undefined) {
		util.logError('`createBinaryTermSetSubstitutionRuleProps()` invoked on subnode with `insertedSymIdx` (should use `createTermSetSeqInsertRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
...
function createTermSetSeqInsertRuleProps(subNode, cost) {
	...
	if (subNodeRuleProps.insertedSymIdx === undefined) {
		util.logError('`createTermSetSeqInsertRuleProps()` invoked on subnode without `insertedSymIdx` (should use `createBinaryTermSetSubstitutionRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
