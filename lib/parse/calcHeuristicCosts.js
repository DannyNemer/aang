...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		// Reassign `cost` value to flattened term sequences following re-parsing with all tokens marked deletable, which can change the `minCost` values by introducing new possible trees to the forest.
		else if (childSubRuleProps.wasTermSequence) {
			childSubRuleProps.cost = cost
			childSubRuleProps.wasTermSequence = false
		}
