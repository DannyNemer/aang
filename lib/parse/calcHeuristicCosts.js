...
function getChildSub(subNode) {
	 ...
	 *      "mine" and "mine" will obviously be the same terminal rule. With all tokens marked deletable, there will be two sub-nodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine". This is unavoidable in the grammar.
	...
	for (var s = 1; s < childSubsLen; ++s) {
		...
		// Compare `childSub.ruleProps.cost`, not `childSub.node.minCost`, because `calcHeuristicCosts` already traversed and flattened `subNode.node.subs`, merging their `minCost` with their `ruleProps.cost` on their new, terminal `ruleProps`.
