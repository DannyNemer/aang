...
function calcMinCost(sub) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		if (childSubRuleProps.constructor === Array) childSubRuleProps = childSubRuleProps[0]
		...
		if (childSub.minCost === undefined) {
			...
			// If a node has subnodes (and hence is in this closure), but `isNonterminal` is `false`, then the associated rule was originally a multi-token terminal symbol substitution where `text` is defined on this node. Reduce this node's subnodes' costs (which include deletion costs on the terminal nodes), so that `pfsearch` does not wastefully traverse those subnodes which produce neither semantics nor text.
			if (!childSubRuleProps.isNonterminal) {
