...
function createTermSeqRuleProps(subNode, cost) {
	...
	var leftRuleProps = leftSub.ruleProps
	var leftText = leftRuleProps.text
	if (!leftText) {
		printSubNodeErr(subNode, subNode.node.subs, 'Term sequence left subnode lacks `text`')
		throw new Error('Ill-formed term sequence')
	}
	...
	if (subNodeNext) {
		...
		var rightRuleProps = rightSub.ruleProps
		var rightText = rightRuleProps.text
		if (!rightText) {
			printSubNodeErr(subNode, subNodeNext.node.subs, 'Term sequence right subnode lacks `text`')
			throw new Error('Ill-formed term sequence')
		}
...
function createTermSeqInsertionRuleProps(subNode, cost) {
	...
	if (!childSubText) {
		printSubNodeErr(subNode, subNode.node.subs, 'Term sequence insertion subnode lacks `text`')
		throw new Error('Ill-formed term sequence insertion')
	}
...
/**
 * Gets the single subnode which the term sequence `subNode` produces (i.e., in `subNode.node.subs`).
 *
 * If `subNode` produces more than 1 subnode, throws an exception.
 *
 * @private
 * @static
 * @param {Object} subNode The term sequence subnode from which to retrieve its subnode.
 * @returns {Object} Returns the subnode which `subNode` produces.
 */
function getChildSub(subNode) {
	...
	printSubNodeErr(subNode, childSubs, 'Term sequence has > 1 subnodes')
	throw new Error('Ill-formed term sequence')
}
