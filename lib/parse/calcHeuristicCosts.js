	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		 ...
		 */
			...
			if (childSubRuleProps.insertedSymIdx !== undefined) {
				 ...
				 */
				childSub.ruleProps = createTermSetSeqInsertRuleProps(childSub, cost)
...
function createTermSetSeqRuleProps(subNode, cost) {
	...
	if (subNodeRuleProps.insertedSymIdx !== undefined) {
		util.logError('`createTermSetSeqRuleProps()` invoked on subnode with `insertedSymIdx` (should use `createTermSetSeqInsertRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
...
function createTermSetSeqSubstitutionRuleProps(subNode, cost) {
	...
	if (subNodeRuleProps.insertedSymIdx !== undefined) {
		util.logError('`createTermSetSeqSubstitutionRuleProps()` invoked on subnode with `insertedSymIdx` (should use `createTermSetSeqInsertRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
 ...
 * @returns {Object} Returns the new, terminal `ruleProps` for `subNode`.
 */
function createTermSetSeqInsertRuleProps(subNode, cost) {
	var subNodeRuleProps = subNode.ruleProps
	if (!subNodeRuleProps.isTermSetSequence) {
		util.logError('`createTermSetSeqInsertRuleProps()` invoked on subnode not defined as a binary term set:', subNode)
		throw new Error('Ill-formed binary term set')
	}

	if (!subNodeRuleProps.text) {
		util.logError('`createTermSetSeqInsertRuleProps()` invoked on subnode without insertion text (should use `createTermSetSeqRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}

	if (subNodeRuleProps.insertedSymIdx === undefined) {
		util.logError('`createTermSetSeqInsertRuleProps()` invoked on subnode without `insertedSymIdx` (should use `createTermSetSeqSubstitutionRuleProps()`):', subNode)
		throw new Error('Ill-formed binary term set')
	}
