...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		 ...
		 * 5. tense - See below.
		 *
		 * The new `ruleProps` excludes the following propeties which the original `ruleProps` can contain prior to flattening, because all are specific to nonterminal nodes:
		 * • isTermSequence - Used to mark `childNode` that hve yet to be flattened.
...
function createTermSeqRuleProps(subNode, cost) {
	...
	if (subNodeNext) {
		...
		return {
			...
			// Exclude `semanticIsReduced` property, if any, because `pfsearch` knows all semantics on terminal nodes are reduced.
 ...
 *
 * `subNode` almost always produces a single child node, but in the rare cases (documented below) of multiple children, the term sequence is ambiguous (because semantically identical) and the cheapest must be chosen.
 *
 * @private
 * @static
