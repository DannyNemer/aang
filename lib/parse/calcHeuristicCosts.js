 ...
 *
 * The cost of a subtree/path is the sum of the minimum cost of all node in the subtree, excluding the parent node's cost. The parent node's cost is excluded in the sum because there are several possible costs for the node (to add to the minimum cost of a path in `pfsearch`) when there is a `ruleProps` array (i.e., multiple insertions).
 *
 * @private
 * @static
...
function calcMinCost(childSubs) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		...
		if (childNode.minCost === undefined) {
			cost += childNode.minCost = calcMinCost(childNode.subs)
		} else {
		...
		if (childSubNext) {
			...
			if (childNodeNext.minCost === undefined) {
				cost += childNodeNext.minCost = calcMinCost(childNodeNext.subs)
			} else {
				cost += childNodeNext.minCost
			}
		}
