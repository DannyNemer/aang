...
module.exports = function calcMinCost(parentNode) {
	...
	for (var s = 0, childSubsLen = childSubs.length; s < childSubsLen; ++s) {
		 ...
		 * 5. tense - See below.
		 */
		else if (childSubRuleProps.isTermSetSequence) {
			if (childSubRuleProps.insertedSymIdx !== undefined) {
				 ...
				 */
				childSub.ruleProps = createTermSetSeqInsertionRuleProps(childSub, cost)
				 ...
				 */
				childSub.ruleProps = createTermSetSeqSubstitutionRuleProps(childSub, cost)
			} else {
				/**
				 ...
				 * 5. tense - Get the tense of any matched verb terminal rules `childSub` produces to maintain tense if the parent rule of `childSub` has matching `acceptedTense`.
				 */
				childSub.ruleProps = createTermSetSeqRuleProps(childSub, cost)
			}
		}
 ...
 */
function createTermSetSeqRuleProps(subNode, cost) {
	var subNodeRuleProps = subNode.ruleProps
	if (!subNodeRuleProps.isTermSetSequence) {
		util.logError('`createTermSetSeqRuleProps()` invoked on subnode without `isTermSetSequence`:', subNode)
		throw new Error('Ill-formed term set sequence')
	}

	if (subNodeRuleProps.insertedSymIdx !== undefined) {
		util.logError('`createTermSetSeqRuleProps()` invoked on subnode with `insertedSymIdx` (should use `createTermSetSeqInsertionRuleProps()`):', subNode)
		throw new Error('Ill-formed term set sequence')
	}

	if (subNodeRuleProps.text) {
		util.logError('`createTermSetSeqRuleProps()` invoked on subnode with substitution text (should use `createTermSetSeqSubstitutionRuleProps()`):', subNode)
		throw new Error('Ill-formed term set sequence')
	}

	var leftSubs = subNode.node.subs
	if (leftSubs.length > 1) {
		printSubNodeErr(subNode, leftSubs, 'Term set sequence has > 1 left subnodes')
		throw new Error('Ill-formed term set sequence')
	}
	...
	if (!leftText) {
		printSubNodeErr(subNode, leftSubs, 'Term set sequence left subnode lacks `text`')
		throw new Error('Ill-formed term set sequence')
	}
	...
	if (subNodeNext) {
		...
		if (rightSubs.length > 1) {
			printSubNodeErr(subNode, rightSubs, 'Term set sequence has > 1 right subnodes')
			throw new Error('Ill-formed term set sequence')
		}
		...
		if (!rightText) {
			printSubNodeErr(subNode, rightSubs, 'Term set sequence right subnode lacks `text`')
			throw new Error('Ill-formed term set sequence')
		}

		return {
			cost: cost,
			text: mergeTextPair(leftText, rightText),
			/**
			 * Currently, the grammar forbids a rule marked `isTermSetSequence` to produce multiple verbs, thereby preventing multiple instances of `tense`. Hence, this ensures `pfsearch` references the correct `tense` for the correct verb.
			 *
 ...
 * @returns {Object} Returns the new, terminal `ruleProps` for `subNode`.
 */
function createTermSetSeqSubstitutionRuleProps(subNode, cost) {
	var subNodeRuleProps = subNode.ruleProps
	if (!subNodeRuleProps.isTermSetSequence) {
		util.logError('`createTermSetSeqSubstitutionRuleProps()` invoked on subnode without `isTermSetSequence`:', subNode)
		throw new Error('Ill-formed term set sequence substitution')
	}

	if (!subNodeRuleProps.text) {
		util.logError('`createTermSetSeqSubstitutionRuleProps()` invoked on subnode without substitution text (should use `createTermSetSeqRuleProps()`):', subNode)
		throw new Error('Ill-formed term set sequence substitution')
	}

	if (subNodeRuleProps.insertedSymIdx !== undefined) {
		util.logError('`createTermSetSeqSubstitutionRuleProps()` invoked on subnode with `insertedSymIdx` (should use `createTermSetSeqInsertionRuleProps()`):', subNode)
		throw new Error('Ill-formed term set sequence substitution')
	}
 ...
 * @returns {Object} Returns the new, terminal `ruleProps` for `subNode`.
 */
function createTermSetSeqInsertionRuleProps(subNode, cost) {
	var subNodeRuleProps = subNode.ruleProps
	if (!subNodeRuleProps.isTermSetSequence) {
		util.logError('`createTermSetSeqInsertionRuleProps()` invoked on subnode without `isTermSetSequence`:', subNode)
		throw new Error('Ill-formed term set sequence insertion')
	}

	if (!subNodeRuleProps.text) {
		util.logError('`createTermSetSeqInsertionRuleProps()` invoked on subnode without insertion text (should use `createTermSetSeqRuleProps()`):', subNode)
		throw new Error('Ill-formed term set sequence insertion')
	}

	if (subNodeRuleProps.insertedSymIdx === undefined) {
		util.logError('`createTermSetSeqInsertionRuleProps()` invoked on subnode without `insertedSymIdx` (should use `createTermSetSeqSubstitutionRuleProps()`):', subNode)
		throw new Error('Ill-formed term set sequence insertion')
	}

	var childSubs = subNode.node.subs
	if (childSubs.length > 1) {
		printSubNodeErr(subNode, childSubs, 'Term set sequence has > 1 left subnodes')
		throw new Error('Ill-formed term set sequence insertion')
	}
	...
	if (!childSubText) {
		printSubNodeErr(subNode, childSubs, 'Term set sequence insertion subnode lacks `text`')
		throw new Error('Ill-formed term set sequence insertion')
	}
	...
	return {
		...
		text: subNodeRuleProps.insertedSymIdx === 1
			? mergeTextPair(childSubText, subNodeRuleProps.text)
			: mergeTextPair(subNodeRuleProps.text, childSubText),
		/**
 ...
 */
function mergeTextPair(leftText, rightText) {
