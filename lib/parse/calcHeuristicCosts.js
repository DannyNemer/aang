...
function getChildSub(subNode) {
	 ...
	 *      As shown, the term sequence `Y` recognizes the input "a b" and "a" with insertion text "b". Provided the input "a b", and with all input tokens marked deletable, `Y` will have two matches for the same token span: "a b", "a <b>" (where "b" is deleted).
	 ...
	 *      These duplicate instances of the same LHS symbol over the same span could be detected in `Parser.prototype.addTermRuleNodes()` with a terminal node table, similar to the nonterminal node table used in `Parser.prototype.addSub()`, but the overhead is too great for such rarity (even if the table is only used during the second parse).
	 *
