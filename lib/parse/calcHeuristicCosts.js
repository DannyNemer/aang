...
function createBinaryTermSetRuleProps(subNode, cost) {
	...
	if (subNodeNext) {
		...
		return {
			...
			/**
			 * Currently, the grammar forbids a rule marked `rhsIsBinaryTermSet` to produce multiple verbs, thereby preventing multiple instances of `tense`. Hence, this ensures `pfsearch` references the correct `tense` for the correct verb.
			 *
