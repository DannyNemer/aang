...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		if (ruleProps.insertedSymIdx === undefined) {
			...
			// Add text for multi-symbol text substitution; e.g., "about","which" -> "that". This node's subnodes do not produce any text, but rather uses this nonterminal rule's text.
			if (ruleProps.text) {
				newPath.text += ' ' + ruleProps.text
			}
		}
