...
/**
 * Uses A* path search to determine the `k`-best parse trees in the parse forest returned by `Parser`, along with the trees' associated semantic trees and conjugated display texts.
 *
 * @static
 ...
 * @param {boolean} [options.printAmbiguity=false] Specify printing instances of semantic ambiguity.
...
module.exports = function (startNode, k, options) {
	...
	while (heap.content.length > 0) {
		...
		if (curNode) {
			...
			if (nextItemList) {
				...
				// The path's grammatical properties (`case`, `tense`, and `acceptedTense`) used to conjugate the second node of this binary rule, if applicable (i.e., a terminal rule).
				...
				if (treeIsUnique(trees, path, options.printAmbiguity)) {
	...
	trees.pathCount = heap.pushCount
	trees.ambiguousTreeCount = ambiguousTreeCount
...
function createPath(prevPath, sub, ruleProps) {
	...
	var newPath = {
		...
		minCost: newCost + sub.minCost + (prevNextItemList ? prevNextItemList.minCost : 0),
	}
	...
	if (ruleProps.isNonterminal) {
		...
		// The next subnodes this path can expand from.
		...
		if (ruleProps.insertedSymIdx === undefined) {
			// All binary rules are nonterminal rules. Prepend the second subnode to `nextItemList`, and complete the rule after completing the branch produced by the first subnode.
			...
			if (subNext) {
				if (prevNextItemList) {
					newPath.nextItemList = {
						...
						minCost: prevNextItemList.minCost + subNext.minCost,
		...
		// Insertions only exist on nonterminal rules because they can only be built from binary rules. This might change if we enable terminal symbols to be in a RHS with another terminal or nonterminal symbol (or multiple).
			...
			if (ruleProps.insertedSymIdx === 1) {
				...
				newPath.text += conjugateText(ruleProps.text, newPath.personNumberList)
			}
		}
	}
 ...
 */
function appendSemantic(prevSemantic, nextNodeCount, ruleProps) {
	// If `ruleProps.insertedSemantic` exists, then it is a RHS semantic and `ruleProps.semantic` also exists and is a LHS semantic.
	...
	if (ruleProps.semantic) {
		...
		if (ruleProps.semanticIsReduced) {
			if (prevSemantic) {
				// Resolve anaphoric semantic arguments, if any.
				var newRHSSemantic = resolveAnaphora(prevSemantic, ruleProps.semantic)
				...
				if (!ruleProps.rhsCanProduceSemantic) {
					...
					return reduceSemanticTree(prevSemantic, nextNodeCount, newRHSSemantic)
				}

				if (prevSemantic.isRHS) {
					// Merge RHS (i.e., reduced) semantic with previous semantic if also reduced.
					var newRHSSemantic = semantic.mergeRHS(prevSemantic.semantic, newRHSSemantic)
				...
				if (isIllegalSemanticReduction(prevSemantic, newRHSSemantic)) {
					return -1
				}
			}
 ...
 */
function reduceSemanticTree(prevSemantic, nextNodeCount, newRHSSemantic) {
	if (newRHSSemantic && prevSemantic) {
		// Resolve anaphoric semantic arguments, if any.
		newRHSSemantic = resolveAnaphora(prevSemantic, newRHSSemantic)
		if (newRHSSemantic === -1) return -1
	}
	...
	while (prevSemantic) {
		...
		else if (nextNodeCount <= prevSemantic.nextNodeCount) {
...
/**
 * Performs anaphora resolution. Checks `newRHSSemantic` for anaphoric semantic arguments, and if any, returns a new RHS semantic with the anaphoric arguments replaced by the antecedent semantic; otherwise, returns the original semantic.
 *
 * @param {Object} prevSemanticList The previous path's semantic linked list.
 * @param {Object[]} newRHSSemantic The new RHS semantic.
 * @returns {Object[]|number} Returns `newRHSSemantic` with any anaphoric arguments replaced by its antecedent semantic if semantically legal, else `-1`.
 */
function resolveAnaphora(prevSemanticList, newRHSSemantic) {
	var antecedentSemantic

	for (var s = 0, newRHSSemanticLen = newRHSSemantic.length; s < newRHSSemanticLen; ++s) {
		var rhsSemanticNode = newRHSSemantic[s]

		// Does not traverse semantic children because any (unresolved) anaphoric semantics on grammar rules only exist as independent semantic arguments (i.e., are not nested within a semantic tree). If that changes, this function will need to be extended.
		if (rhsSemanticNode.semantic.anaphoric) {
			if (!antecedentSemantic) {
				// Replace anaphoric semantic with its antecedent semantic of the same person-number. Must create a new semantic because the RHS semantic is shared (and can be from the state table).
				antecedentSemantic = getAntecedentSemantic(prevSemanticList, rhsSemanticNode.semantic.anaphoraPersonNumber)

				// Exit if `newRHSSemantic` contains only one semantic or anaphor is illegal.
				if (newRHSSemanticLen === 1 || antecedentSemantic === -1) {
					return antecedentSemantic
				}

				// Clone `newRHSSemantic` if contains multiple semantics of which at least one is anaphoric (though this does not yet occur in the grammar).
				antecedentSemantic = antecedentSemantic[0]
				newRHSSemantic = newRHSSemantic.slice()
			}

			// Replace the anaphoric semantic with the antecedent semantic (when `newRHSSemantic` contains multiple semantics).
			newRHSSemantic[s] = antecedentSemantic
		}
	}

	// Return original semantic, or a new semantic with the anaphoric semantics replaces by the antecedent semantic (when `newRHSSemantic` contains multiple semantics).
	return newRHSSemantic
}

/**
 * Searches `semanticList` for the antecedent semantic of `anaphoraPersonNumber`, if any.
 *
 ...
 */
function getAntecedentSemantic(semanticList, anaphoraPersonNumber) {
	while (semanticList) {
		if (semanticList.isRHS) {
			// Check if semantic previously found.
			if (semanticList.antecedent) {
				return semanticList.antecedent
			}
			...
			if (antecedentSemantic) {
				// Save semantic for future invocations on the same semantic list.
				return semanticList.antecedent = [ antecedentSemantic ]
			}
...
function conjugateText(text, personNumberList, gramProps, inputTense) {
	...
	for (var t = 0, textLen = text.length; t < textLen; ++t) {
		...
		conjugatedText += conjugateText(text[t], personNumberList)
	}
...
function printAmbiguousPair(existingTree, newTree) {
	...
	if (newTree.prev) {
		require('./printParseResults')([ newTree ], { trees: true })
		util.log()
	}
}