...
function appendSemantic(prevSemantic, nextNodeCount, ruleProps) {
		...
		if (ruleProps.semanticIsReduced) {
			if (prevSemantic) {
				if (!ruleProps.rhsCanProduceSemantic) {
					...
					return reduceSemanticTree(prevSemantic, nextNodeCount, ruleProps.semantic)
				}
	...
	if (ruleProps.anaphoraPersonNumber) {
		// Resolve anaphora by copying the matching antecedent semantic. Only invoked here (not in `reduceSemanticTree()`) because `anaphoraPersonNumber` only occurs on nonterminal rules.
