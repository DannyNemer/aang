...
module.exports = function (startNode, k, buildTrees, printStats, printAmbiguity) {
	...
	while (heap.content.length > 0) {
		...
		if (!curNode) {
			// Previously reached a terminal symbol at a branch's end. Get the next node in `path.nextItemList` while conjugating any preceding inserted text objects.
...
/**
 * Expands `path`, the cheapest path in `heap`, by creating a new path for each of its subnodes, `subs` (i.e., its neighboring nodes).
 *
 * @static
 ...
 * @param {Object[]} subs The subnodes of `path.curNode`, with which to expand `path`.
 * @param {boolean} [buildTrees] Specify constructing parse trees, at expense of speed, for printing.
 */
...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		// Save the next subnodes this path can expand from.
