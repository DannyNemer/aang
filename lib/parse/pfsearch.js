...
function createPath(prevPath, sub, ruleProps) {
	...
	// Terminal rule.
		...
		if (ruleProps.text) {
			 ...
			 * Prepend `ruleProps.personNumber` to `prevPath.personNumberList` which conjugates display text of nominative verbs within this subtree that follow `newPath`.
			 *
			 * Only occurs on terminal rules for the following flattened term sequence:
			 *   `[nom-users]` -> `[users-term]`, `all(user)`, text: "people", personNumber: "pl"
			 * Does not conjugate `newPath.text`; rather, conjugates nominative verbs within this subtree that follow this rule.
			 */
			if (ruleProps.personNumber) {
				newPath.personNumberList = prependPersonNumber(prevPath, ruleProps.personNumber)
			}
...
function appendSemantic(semanticList, nextNodeCount, ruleProps) {
	...
	if (ruleProps.insertedSemantic) {
		 ...
		 */
		if (semanticList && semanticList.isRHS && semantic.isForbiddenMultiple(semanticList.prev.semantic, semanticList.semantic, ruleProps.semantic)) {
			return -1
		}
	...
	if (newSemantic) {
		...
		if (ruleProps.semanticIsReduced) {
			if (semanticList) {
				...
				if (semanticList.isRHS) {
					// Discard new RHS semantic, `newSemantic`, if `semanticList.prev.semantic` is `intersect()`, `semanticList.semantic` is completed reduced (i.e., RHS) and identical to `newSemantic`, and multiple instances of that identical semantic are forbidden within the arguments of `intersect()`.
					if (semantic.isForbiddenMultiple(semanticList.prev.semantic, semanticList.semantic, newSemantic)) {
						return -1
					}
		 ...
		 *
		 * Discard new LHS semantic, `newSemantic`, if `semanticList.prev.semantic` is `intersect()`, `semanticList.semantic` is completed reduced (i.e., RHS) and identical to `newSemantic`, and multiple instances of that identical semantic are forbidden within the arguments of `intersect()`.
		 *
		 ...
		 */
		else if (semanticList && semanticList.isRHS && semantic.isForbiddenMultiple(semanticList.prev.semantic, semanticList.semantic, newSemantic)) {
			return -1
		}
