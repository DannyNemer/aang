	...
	if (sub.node.subs) {
		...
		// Insertions only exist on nonterminal rules because they can only be built from binary rules. This might change if we enable terminal symbols to be in a RHS with another terminal or nonterminal symbol (or multiple).
			...
			if (ruleProps.insertedSymIdx === 1) {
				// Insertion rules that use the `<blank>` symbol (i.e., insertions that only occur at the end of input) have a second branch for that symbol. Do not traverse that branch (because it does not produce text), but add its cost, which can include deletion costs, to the path's cost.
				if (sub.next) {
					// The `minCost` is identical to the sum of the ancestors' costs because there is only one possible branch.
					newPath.cost += sub.next.minCost
				}
