...
/**
 * Uses A* path search to find the `k`-best parse trees in the parse forest returned by `Parser`, along with the trees' associated semantic trees and conjugated display texts.
 *
 * @static
...
module.exports = function (startNode, k, options) {
	...
	while (heap.content.length > 0) {
		...
		if (curNode) {
			...
			else if (treeIsUnique(trees, path, options.printAmbiguity)) {
 ...
 * @private
 * @static
 * @param {Object} prevSemantic The previous path's semantic linked list to append.
...
function appendSemantic(prevSemantic, nextNodeCount, ruleProps) {
	...
	if (ruleProps.semantic) {
		...
		if (ruleProps.semanticIsReduced) {
			if (prevSemantic) {
				if (!ruleProps.rhsCanProduceSemantic) {
					...
					return baseReduceSemanticTree(prevSemantic, nextNodeCount, ruleProps.semantic)
				}
				...
				// Check if reducing `ruleProps.semantic` with LHS `prevSemantic` and merging the resulting semantic with RHS `prevSemantic.prev` will produce an illegal semantic. These illegal semantics would otherwise be caught later.
		...
		else if (prevSemantic && prevSemantic.isRHS && semantic.isForbiddenMultiple(prevSemantic.semantic, ruleProps.semantic)) {
			return -1
		}
	...
	if (ruleProps.anaphoraPersonNumber) {
		...
		var newRHSSemantic = resolveAnaphora(prevSemantic, ruleProps.anaphoraPersonNumber)
		if (newRHSSemantic === -1) return -1
		...
		return baseReduceSemanticTree(prevSemantic, nextNodeCount, newRHSSemantic)
	}
 ...
 * Fails if resulting semantic is illegal (e.g., duplicates within a RHS semantic), and then returns `-1`.
 *
 * Only invoked from `createPath()`, because all other reductions know `newRHSSemantic` exists and use `baseReduceSemanticTree()`.
 *
 * @private
 * @static
 * @param {Object} prevSemantic The previous path's semantic linked list to append and reduce.
 * @param {number} nextNodeCount The number of nodes in the previous path's `nextItemList` that can produce a semantic. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
...
function reduceSemanticTree(prevSemantic, nextNodeCount, newRHSSemantic) {
	if (!newRHSSemantic) {
		if (prevSemantic.isRHS) {
			var prev = prevSemantic.prev

			// Instead of setting `newRHSSemantic` to `prevSemantic.semantic` and entering the loop below, check if the loop will fail to avoid unnecessarily recreating `prevSemantic` at the bottom of this function. This condition is true for 92%.
			if (!prev || nextNodeCount > prev.nextNodeCount) {
				return prevSemantic
			}

			// Reduce the semantic list up to the first incompletely reduced node.
			return baseReduceSemanticTree(prev, nextNodeCount, prevSemantic.semantic)
	...
	// Append `newRHSSemantic` to `prevSemantic` and reduce up to the first incompletely reduced node.
	return baseReduceSemanticTree(prevSemantic, nextNodeCount, newRHSSemantic)
}

/**
 * The base implementation of `reduceSemanticTree()`, without support for handling no `newRHSSemantic` argument, that appends a RHS semantic to the previous path's semantic list, and then reduces the path's semantic tree up to the first incompletely reduced node.
 *
 ...
 * @param {Object} prevSemantic The previous path's semantic linked list to append and reduce.
 * @param {number} nextNodeCount The number of nodes in the previous path's `nextItemList` that can produce a semantic. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
 * @param {Object[]} newRHSSemantic The RHS semantic.
 * @returns {Object|number} Returns the reduced semantic linked list if reduction is semantically legal, else `-1`.
 */
function baseReduceSemanticTree(prevSemantic, nextNodeCount, newRHSSemantic) {
	...
	while (prevSemantic) {
		...
		// Check if reducing `newRHSSemantic` with LHS `prevSemantic` and merging the resulting semantic with RHS `prevSemantic.prev` will produce an illegal semantic. These illegal semantics would otherwise be caught later.
		else if (isIllegalSemanticReduction(prevSemantic, newRHSSemantic)) {
			return -1
		}
 ...
 *
 * LHS `semanticList` cannot reduce with `rhsSemantic` because another semantic can come and merge with `rhsSemantic` as a RHS array. If `semanticList` has a `maxParams` of `1`, and will be copied for each semantic within the semantic array to contain `rhsSemantic`, then checks if that semantic will be illegal when merged with `semanticList.prev`.
 *
 * Example:
