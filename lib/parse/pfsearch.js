...
module.exports = function (startNode, k, buildTrees, printStats, printAmbiguity) {
	...
	while (heap.content.length > 0) {
		...
		// Check if just reached a terminal symbol at a branch's end.
		if (!curNode) {
			...
			if (!nextItemList) {
				...
				// Get the second node of the most recent incomplete binary rule.
				curNode = nextItemList.node
				path.nextItemList = nextItemList.next

				// Remove person-number properties for previously completed subtrees.
				path.personNumberList = unwindPersonNumberList(personNumberList, nextItemList.size)

				// The path's grammatical properties (`verbForm`, `gramCase`, and `acceptedTense`) used to conjugate the second node of this binary rule, if applicable (i.e., a terminal rule).
...
function createPath(prevPath, sub, ruleProps) {
	...
	var newPath = {
		...
		cost: newCost + sub.minCost + (prevPath.nextItemList ? prevPath.nextItemList.minCost : 0),
	}
	...
	if (sub.node.subs) {
		...
		// The grammatical properties (`verbForm`, `gramCase`, and `acceptedTense`) used to conjugate terminal rules produced by `newPath.curNode` (and its sibling node if this is a binary rule). Only occurs on nonterminal rules.
		...
		if (ruleProps.insertionIdx === undefined) {
			...
			if (subNext) {
				var nextItem = prevPath.nextItemList
				newPath.nextItemList = {
					// The second node of this binary rule to parse after completing the first node's branch.
					node: subNext.node,
					// The path's grammatical properties (`verbForm`, `gramCase`, and `acceptedTense`) used to conjugate terminal rules produced by `node`.
					gramProps: newPath.gramProps,
					// Specify if `node` can produce a semantic. Used to determine whether to decrement `path.nextNodesCount` when beginning to parse `node`.
					...
					minCost: subNext.minCost + (nextItem ? nextItem.minCost : 0),
					// The number of items in `nextItemList`.
					size: nextItem ? nextItem.size + 1 : 1,
		...
		// Insertions only exist on nonterminal rules because they can only be built from binary rules. This might iIf we enable terminal symbols to be in a RHS with another terminal or nonterminal symbol (or multiple).
			...
			if (ruleProps.insertionIdx === 1) {
				...
				newPath.nextItemList = {
					...
					minCost: nextItem ? nextItem.minCost : 0,
					// The number of items in `nextItemList`.
					size: nextItem ? nextItem.size + 1 : 1,
					// The next item that follows after completing this branch, created from the previous binary or insertion rule.
					next: nextItem,
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`verbForm`, `gramCase`, and `acceptedTense`) for conjugating `text`.
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`verbForm`, `gramCase`, and `acceptedTense`) for conjugating `textObj`.
...
function conjugateTextObject(textObj, personNumberList, gramProps, inputTense) {
	if (gramProps) {
		...
		// If the input terminal symbol is of a tense which the parent rule optionally accepts as a verb form, then accept it. E.g., "(repos I) like/liked".
