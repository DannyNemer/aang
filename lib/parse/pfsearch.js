	...
	while (heap.content.length > 0) {
		...
		if (curNode) {
			...
			if (nextItemList) {
				...
				// The path's grammatical properties (`gramCase`, `tense`, and `acceptedTense`) used to conjugate the second node of this binary rule, if applicable (i.e., a terminal rule).
...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		// The grammatical properties (`gramCase`, `tense`, and `acceptedTense`) used to conjugate terminal rules produced by `newPath.curNode` (and its sibling node if this is a binary rule). Only occurs on nonterminal rules.
		...
		if (ruleProps.insertedSymIdx === undefined) {
			...
			if (subNext) {
				newPath.nextItemList = {
					...
					node: subNext.node,
					// The path's grammatical properties (`gramCase`, `tense`, and `acceptedTense`) used to conjugate terminal rules produced by `node`.
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`gramCase`, `tense`, and `acceptedTense`) for conjugating `text`.
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`gramCase`, `tense`, and `acceptedTense`) for conjugating `textObj`.
