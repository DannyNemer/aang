...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		if (ruleProps.insertedSymIdx === undefined) {
			...
			// Append text for multi-symbol text substitution , if any, to the previous path's text, performing any necessary conjugation. E.g., "about","which" -> "that". This node's subnodes do not produce any text, but rather uses this nonterminal rule's text.
			if (ruleProps.text) {
