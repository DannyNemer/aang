	...
	for (var s = 0, newRHSSemanticLen = newRHSSemantic.length; s < newRHSSemanticLen; ++s) {
		...
		if (rhsSemanticNode.semantic.anaphoric) {
			if (!antecedentSemantic) {
				// Replace anaphoric semantic with its antecedent semantic. Must create a new semantic because the RHS semantic is shared (and can be from the state table).
				antecedentSemantic = getAntecedentSemantic(prevSemanticList)
...
/**
 * Searches `semanticList` for the antecedent semantic, if any.
 *
 ...
 */
function getAntecedentSemantic(semanticList) {
	while (semanticList) {
		if (semanticList.isRHS) {
			...
			for (var s = 0, stackLen = stack.length; s < stackLen; ++s) {
				...
				} else if (semanticNode.semantic.isPerson) {
					if (!antecedentSemantic) {
						// The antecedent.
