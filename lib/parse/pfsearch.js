			...
			if (subNext) {
				...
				if (prevNextItemList) {
					newPath.nextItemList = {
						 ...
						 * sibling node in this biinary rule (i.e., `sub.node`).
						 */
 ...
 * then reduces the path's semantic tree up to the first incompletely
 * reduced node.
 *
 * Fails if resulting semantic is illegal (e.g., duplicates within a RHS
 * semantic), and then returns `-1`.
 ...
 * display text.
 *
 * Trees discarded for ambiguity result from the grammar, howoever, this
 * does not mean the grammar should always be designed to avoid ambiguity.
 * Sometimes, the necessary rules required to disambiguate expands the
 * grammar (and state table) such that there is greater overhead for
 * `Parser` (i.e., the number of shift and reduce actions) than the
 * processing saved in `pfsearch`. In production, `Parser` consumes ~85% of
 * processing while `pfsearch` consumes ~10% (when `k` is 7).
 *
 * @private
 * @static
