...
module.exports = function (startNode, k, buildTrees, printStats, printAmbiguity) {
	...
	while (heap.content.length > 0) {
		...
		if (curNode) {
			...
			expandPath(heap, path, curNode.subs, buildTrees)
		} else {
			...
			if (nextItemList) {
				...
				// The path's grammatical properties (`tense`, `gramCase`, and `acceptedTense`) used to conjugate the second node of this binary rule, if applicable (i.e., a terminal rule).
				...
				expandPath(heap, path, nextItemList.node.subs, buildTrees)
			} else  {
	...
	if (printStats) {
		...
		util.log('Ambiguous trees:', ambiguousTreeCount)
	}
...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		// The grammatical properties (`tense`, `gramCase`, and `acceptedTense`) used to conjugate terminal rules produced by `newPath.curNode` (and its sibling node if this is a binary rule). Only occurs on nonterminal rules.
		...
		if (ruleProps.insertedSymIdx === undefined) {
			...
			if (subNext) {
				newPath.nextItemList = {
					...
					node: subNext.node,
					// The path's grammatical properties (`tense`, `gramCase`, and `acceptedTense`) used to conjugate terminal rules produced by `node`.
					...
					minCost: subNext.minCost + (prevNextItemList ? prevNextItemList.minCost : 0),
					...
					nodeCount: prevNextItemList ? prevNextItemList.nodeCount + ruleProps.secondRHSCanProduceSemantic : ruleProps.secondRHSCanProduceSemantic,
					// The number of items in `nextItemList`.
					size: prevNextItemList ? prevNextItemList.size + 1 : 1,
		...
		// Insertions only exist on nonterminal rules because they can only be built from binary rules. This might iIf we enable terminal symbols to be in a RHS with another terminal or nonterminal symbol (or multiple).
			...
			if (ruleProps.insertedSymIdx === 1) {
				...
				newPath.nextItemList = {
					...
					minCost: prevNextItemList ? prevNextItemList.minCost : 0,
					// The number of nodes in the `nextItemList` that can produce a semantic. This excludes other nodes and yet-to-conjugate text. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
					nodeCount: prevNextItemList ? prevNextItemList.nodeCount : 0,
					// The number of items in `nextItemList`.
					size: prevNextItemList ? prevNextItemList.size + 1 : 1,
					// The next item that follows after completing this branch, created from the previous binary or insertion rule.
					next: prevNextItemList,
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`tense`, `gramCase`, and `acceptedTense`) for conjugating `text`.
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`tense`, `gramCase`, and `acceptedTense`) for conjugating `textObj`.
