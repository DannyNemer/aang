 ...
 * @property {string} semanticStr The string representation of the semantic tree.
...
module.exports = function (startNode, k, options) {
	...
	heap.push({
		// The previously added node whose sub-nodes this path can expand from.
		curNode: startNode,
	...
	while (heap.content.length > 0) {
		...
		if (curNode) {
			// Expand `path`, the cheapest path in `heap`, by creating a new path for each of its sub-nodes, `subs` (i.e., its neighboring nodes).
			...
			if (nextItemList) {
				...
				// Expand the second node of the most recent incomplete binary rule by creating a new path for each of its sub-nodes, `subs` (i.e., its neighboring nodes).
...
/**
 * Expands `path`, the cheapest path in `heap`, by creating a new path for each of its sub-nodes, `subs` (i.e., its neighboring nodes), and adding them to `heap`.
 *
 * @private
 * @static
 ...
 * @param {Object[]} subs The sub-nodes of `paths`'s last node, `path.curNode`, with which to expand `path`.
 * @param {boolean} [buildTrees] Specify constructing parse trees for printing.
 */
function expandPath(heap, path, subs, buildTrees) {
	// Expand `path` by creating new paths from each of its sub-nodes.
...
/**
 * The base implementation of `expandPath()` which expands `path`, the cheapest path in `heap`, by creating a new path using one if its sub-nodes, `sub` (i.e., a neighboring node), and adding it to `heap`.
 *
 * @private
 * @static
 * @param {BinaryHeap} heap The min-heap of search paths, for which to add the new paths.
 * @param {Object} path The cheapest path in `heap`.
 * @param {Object} sub The sub-nodes of `paths`'s last node, `path.curNode`, with which to expand `path`.
 * @param {Object} ruleProps The rule properties of `sub`.
 * @param {boolean} [buildTrees] Specify constructing parse trees for printing.
 */
function baseExpandPath(heap, path, sub, ruleProps, buildTrees) {
	// Create a new path by expanding `path` with its sub-node, `sub`.
 ...
 * @param {Object} sub The sub-node of `prevPath`'s last node.
 * @param {Object} ruleProps The rule properties of `sub`.
...
function createPath(prevPath, sub, ruleProps) {
	...
	if (ruleProps.isNonterminal) {
		...
		if (ruleProps.insertedSymIdx === undefined) {
			// All binary rules are nonterminal rules. Prepend the second sub-node to `nextItemList`, and complete the rule after completing the branch that the first sub-node produces.
