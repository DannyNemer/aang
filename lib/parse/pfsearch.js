 ...
 * Fails if resulting semantic is illegal (e.g., duplicates within a RHS semantic), and then returns `-1`.
 *
 * Only invoked from `createPath()` on terminal nodes, because all other reductions know `rhsSemantic` exists and use `baseReduceSemanticTree()`.
 *
 * @private
 * @static
...
function reduceSemanticTree(semanticList, nextNodeCount, ruleProps) {
	...
	if (semanticList.isRHS) {
		...
		// Instead of setting `rhsSemantic` to `semanticList.semantic` and entering the loop in `baseReduceSemanticTree()`, check if the loop will fail to avoid unnecessarily recreating `semanticList` in `baseReduceSemanticTree()`. This condition is true for 92%.
