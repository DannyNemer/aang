...
function createPath(prevPath, sub, ruleProps) {
	...
	if (ruleProps.isNonterminal) {
		...
		// The grammatical properties (`form` and `acceptedTense`) used to conjugate terminal rules produced by `newPath.curNode` (and its sibling node if this is a binary rule). Only occurs on nonterminal rules.
			...
			if (subNext) {
				...
				if (prevNextItemList) {
					newPath.nextItemList = {
						...
						node: nextNode,
						// The path's grammatical properties (`form` and `acceptedTense`) used to conjugate terminal rules produced by `node`.
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`form` and `acceptedTense`) for conjugating `text`.
 ...
 * @param {Object} [gramProps] The previous path's (i.e, the parent nonterminal rule's) grammatical properties (`form` and `acceptedTense`) for conjugating `textObj`.
