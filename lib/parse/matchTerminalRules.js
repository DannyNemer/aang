		...
		for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
			...
			if (entity.size > 1) {
				var entityMatch = {
					...
					// E.g., a match to "Iroh" can be for either the alias "Iroh" or "General Iroh", both of which map to "Iroh".
					// Note: It is likely the overhead from this additional check for this obscure edge case is more detrimental to performance than having `Parser` and `pfsearch` absorb additional load, and allowing `pfsearch` to catch the ambiguity at the end.
					hasAmbigUniTokenAlias: entity.ambiguousUniTokenAliases && entity.ambiguousUniTokenAliases.indexOf(entToken) !== -1,
				}
				...
				matchedEntities.push(entityMatch)

				// Save the entity match for adding deletions in `Parser.prototype.addDeleltionToAdjacentSymbolMatches()`. Include `entityMatch` for multi-token entities so that expansions of this match with adjacent deletables are also added to `matchedEntities` (for use by `Parser.prototype.mergeEntityMatches()`).
				this.matchedSymbols.push({
					terminalSym: terminalSym,
					semanticArg: semanticArg,
					text: entity.text,
	...
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			// Do not add a parse node for a single-token entity match via a token for which a uni-token alias for the same entity exists and a parse node was already added (with a guaranteed smaller cost). I.e., avoids multiple matches for the same entity (id and display) via different names/aliases over the same one-token span.
 			// E.g., a match to "Iroh" can be for either the alias "Iroh" or "General Iroh", both of which map to "Iroh".
			if (entityObj.hasAmbigUniTokenAlias) continue
...
Parser.prototype.addDeleltionToAdjacentSymbolMatches = function (delStartIdx, delEndIdx, deletionCost, onlyNewDeletables) {
	...
	for (var s = 0, matchedSymbolsLen = this.matchedSymbols.length; s < matchedSymbolsLen; ++s) {
		...
		if (entityMatch) {
			...
			var newIdx = onlyNewDeletables ? getMatchedEntityIndex(matchedEntities, entityMatch.entity, delStartIdx, deletionCost) : matchedEntities.length
			if (newIdx !== -1) {
				matchedEntities[newIdx] = {
