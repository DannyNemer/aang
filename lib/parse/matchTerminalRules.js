...
Parser.prototype.matchTerminalRules = function (query) {
	...
	this.tokens = query.trim().toLowerCase().split(/\s+/)
...
Parser.prototype.entityLookup = function (entToken, tokenIdx) {
	...
	if (entitySet) {
		...
		for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
			...
			// Get the terminal symbol for the entity category. Terminal nodes for multiple entities in the same category use the same symbol, but use the node's parent sub-node's `ruleProps`, which contains the entities' semantic and display text, to distinguish them.
...
Parser.prototype.addTermRuleNodes = function (terminalSym, startIdx, endIdx, semanticArg, text, costPenalty) {
	 ...
	 *
	 * However, there can be multiple matches at the same index for the same entity category, which is the same terminal symbol, though for different entities within the category. The node's parent sub-node's `ruleProps` distinguishes these terminal nodes.
	 *
	 * If a terminal symbol is deleted in input, either because it is a grammar-defined deletable or when re-parsing with all tokens marked deletable, then there can be duplicate instances of a non-entity terminal node at the same index. Though, it is rare. For example:
	 *   "JavaScript Ruby repos mine mine"
	 * "mine" and "mine" will obviously be the same terminal rule. With "mine" marked deletable, there will be two sub-nodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine".
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		// If `node` did not already exists (for another sub-node at this index), then add it to the current index of `termRuleMatchTab`.
		if (node) {
