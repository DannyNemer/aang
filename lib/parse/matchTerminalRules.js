...
/**
 * Gets the index of where to add the entity with specified parameters in `matchedEntities` to avoid duplicates in `Parser.prototype.mergeEntityMatches()`.
 *
 * If `newDeletionsCost` is less than an existing match in `matchedEntities` for the same entity, then returns the existing match's index with which to overwrite. Else if `newDeletionsCost` is greater than or equal to an an existing match for the same entity, the returns `-1` to discard the new entity match. Else, the entity is unique and returns the length of `matchedEntities` with which to append the new entity.
 *
 * @private
 * @static
