...
Parser.prototype.createWords = function (terminalSym, startIdx, endIdx, semanticArg, text, costPenalty) {
	...
	// The number of tokens in the terminal symbol.
	...
	// Save word to the input query index of its last token.
...
Parser.prototype.entitiesLookup = function () {
	...
	for (var endIdx = 1; endIdx < this.tokensLen; ++endIdx) {
		var matchedEntities = matchedEntitySets[endIdx]
	...
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			this.createWords(entityObj.terminalSym, entityObj.startIdx, entityObj.endIdx, entityObj.semanticArg, entity.text, costPenalty)
		}
	}
}
