...
Parser.prototype.mergeEntityMatches = function (onlyNewDeletables) {
	...
	for (var endIdx = 1; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			var entityMatch = matchedEntities[e]
			...
			var prevMatchedEntities = this.entityTab[entityMatch.startIdx - 1]
			...
			for (var p = 0, prevMatchedEntitiesLen = prevMatchedEntities.length; p < prevMatchedEntitiesLen; ++p) {
				var prevEntityMatch = prevMatchedEntities[p]
				...
				if (prevEntityMatch.entity === entity) {
					...
					if (matchedTokensAreValid(entTokens, matchedTokens)) {
						...
						var newIdx = getMatchedEntityIndex(matchedEntities, entity, prevEntityMatch.startIdx, newDeletionsCost)
						if (newIdx !== -1) {
...
Parser.prototype.addMultiTokenEntityNodes = function (onlyNewDeletables) {
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			// Add nodes for the terminal rules that produce `terminalSym` (at this index), with `ruleProps` that include the entity id as the semantic argument and the display text which `entity` defines.
			this.addWordNodes(entityMatch.terminalSym, entityMatch.startIdx, endIdx, entityMatch.semanticArg, entity.text, costPenalty)
		}
	}
}
...
function isAmbigMultiTokenEntityMatch(matchedEntities, matchIdx) {
	var entityMatch = matchedEntities[matchIdx]
