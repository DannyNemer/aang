...
Parser.prototype.tokensLookup = function () {
	for (var curIdx = 0; curIdx < this.tokensLen; ++curIdx) {
		...
		// Also lookup integers for numeric terminal symbols that are not range-based; e.g., "1" -> "January".
		...
		if (isNaN(token)) {
			...
			// If token is a number, match with integer symbols for which `token` is within the specified range, if any.
