...
Parser.prototype.addTermRuleNodes = function (terminalSym, startIdx, endIdx, semanticArg, text, costPenalty) {
	...
	/**
	 * The unique node for `terminalSym`. There can only be one match per terminal symbol per index. Hence, there will be no duplicate terminal nodes at the same index (like nonterminal nodes) without checking existing nodes.
	 *
	 * There can be multiple matches at the same index to the same entity category, which is the same terminal symbol, though for different entities within the category. The node's parent subnode's `ruleProps` distinguishes these terminal nodes. Though those nodes are identical, they must be separate `Objedct` instance for `isNewSub()` to distinguish the subnodes (because `isNewSub()` does not check `ruleProps`).
	 */
	var terminalNode = {
		sym: terminalSym,
		size: wordSize,
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (semanticArg) {
			...
			terminalSub.ruleProps = {
				...
				// Maintain terminal symbol input tense following a deletion. For use by parent rules with `acceptedTense`, which specifies a tense to accept when input but not to enforce if otherwise (i.e., tense is optional because semantically meaningless).
				//   For example: "repos I do liked" -> "repos I liked"
