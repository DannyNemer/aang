...
Parser.prototype.addMultiTokenEntityNodes = function (onlyNewDeletables) {
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			// Check for matches to the same entity (id) via multiple (ambiguous) names/aliases, over the same token span, and keep the cheapest match. E.g., a match to "Alan" can be for either the alias "Alan Kay" or "Alan Curtis", both of which map to "Alan Kay".
			...
			// Keep multiple matches to the same entity (id) via multiple names/aliases over different token spans, just as multiple matches to the same entity object over different spans are kept, as all possible spans must be examined to explore possible insertions.
			// Note: It is likely the overhead from this additional check for this obscure edge case is more detrimental to performance than letting the `Parser` and `pfsearch` take on additional load, and allowing for the ambiguity to be caught at the end of `pfsearch`.
			if (entity.hasAmbiguousMultiTokenAlias) {
				for (var o = e + 1; o < matchedEntitiesLen; ++o) {
					...
					if (otherEntityObj.entity.id === entity.id && otherEntityObj.startIdx === entityObj.startIdx && otherEntityObj.deletionsCost <= entityObj.deletionsCost) {
						break
					}
				}

				// Do not add terminal node for entity match if there exists other matches for the same entity (id) across the same token span. If other matches have the same cost, then the last match will be added because this only comparison entities to those that follow withing `matchedEntities`.
				if (o < matchedEntitiesLen) {
					continue
				}
			}
