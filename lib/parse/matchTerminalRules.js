...
Parser.prototype.addTermRuleNodes = function (terminalSym, startIdx, endIdx, semanticArg, text, costPenalty) {
	 ...
	 *
	 * When re-parsing with all tokens marked deletable, there can be duplicate instances of a non-entity terminal node at the same index. Though, it is rare. For example:
	 *   "JavaScript Ruby repos mine mine"
	 * "mine" and "mine" will obviously be the same terminal rule. With all tokens marked deletable, there will be two sub-nodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine".
