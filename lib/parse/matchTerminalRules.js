...
/**
 * Performs terminal symbol, entity, and integer symbol lookups for each token in the input query. Adds nodes for each single-token terminal rule match and adds multi-token entity matches to `entityTab` for use by `Parser.prototype.mergeEntityMatches()`.
 *
 * @memberOf Parser
 */
...
/**
 * Finds the integer symbols for which `intToken` is within range, and adds nodes for those rules with `intToken` as a semantic argument and display text.
 *
 * @memberOf Parser
...
/**
 * Checks for entities that include `entToken`. Adds nodes for single-token entities, using the entity for for the semantic argument and display text, and adds matches for multi-token entities to `entityTab` for use by `Parser.prototype.mergeEntityMatches()`.
 *
 * @memberOf Parser
...
Parser.prototype.entityLookup = function (entToken, tokenIdx) {
	...
	if (entitySet) {
		...
		for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
			...
			this.findDeletions(tokenIdx, tokenIdx, function (startIdx, endIdx, deletionCost) {
				if (entity.size > 1) {
					// Add multi-token entities to `entityTab` at this index (instead of adding nodes here) for use by `Parser.prototype.mergeEntityMatches()`. This enables merging adjacent token matches for the same entity while avoiding duplicates (i.e., ambiguity).
					matchedEntities.push({
						entity: entity,
					...
					// Add nodes for the terminal rules that produce `terminalSym` (at this index), with `ruleProps` that include the entity id as the semantic argument and the display text which `entity` defines.
					this.addWordNodes(terminalSym, startIdx, endIdx, semanticArg, entity.text)
				}
			})
		}
	}
 ...
 * @memberOf Parser
 */
Parser.prototype.mergeEntityMatches = function () {
 ...
 *
 * Invoke this method after `Parser.prototype.tokensLookup()` to determine unrecognized input tokens, and before `Parser.prototype.addWordTabDeletions()`, which adds nodes with the deletions and the adjacent terminal symbol matches.
 *
 * @memberOf Parser
 */
