...
Parser.prototype.addTermRuleNodes = function (terminalSym, startIdx, endIdx, semanticArg, text, costPenalty) {
	 ...
	 *
	 * When re-parsing with all tokens marked deletable, there can be duplicate instances of a non-entity terminal node at the same index. For example:
	 *   "JavaScript Ruby repos mine mine"
	 * "mine" and "mine" will obviously be the same terminal rule. With all tokens marked deletable, there will be two subnodes for the same LHS symbol, spanning the last two input tokens: "mine <mine>", "<mine> mine".
	 ...
	 * • The duplicate terminal nodes are not problematic, because if the terminal rule is part of a term sequence, then `calcHeuristicCosts` chooses the cheapest match. Otherwise, `pfsearch` only keeps the cheapest resultant parse tree.
	 */
	var terminalNode = {
		sym: terminalSym,
		size: wordSize,
