...
Parser.prototype.matchTerminalRules = function (query) {
	...
	// Perform terminal symbol, integer symbol, and entity lookups for each token in the input query. Add nodes for each single-token terminal rule match and add multi-token entity matches to `entityTab` for use by `Parser.prototype.mergeEntityMatches()`.
	this.tokensLookup()
