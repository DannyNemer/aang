...
/**
 * Looks up entity matches in input and adds matches for multi-token entities to `entityTab` for use by `Parser.prototype.mergeEntityMatches()`.
 *
 * @memberOf Parser
 */
		...
		if (entitySet) {
			...
			for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
				...
				if (entity.size > 1) {
					...
					this.addWordNodes(terminalSym, curIdx, curIdx, semanticArg, entity.text)
				}
