...
Parser.prototype.matchTerminalRules = function (query) {
	...
	// Add nodes for multi-token entity matches by merging adjacent token matches for the same entity, including deletions, while avoiding ambiguity.
...
Parser.prototype.addDeletablesForAllTokens = function () {
	...
	// Adds nodes for multi-token entity matches that include the new deletions which `Parser.prototype.remainingTokensDeletablesLookup()` added.
...
/**
 * Adds nodes for multi-token entity matches by merging adjacent token matches for the same entity, including deletions, while avoiding ambiguity.
 *
 * @memberOf Parser
...
Parser.prototype.mergeEntityMatches = function (onlyNew) {
	...
	for (var endIdx = 1; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			for (var p = 0, prevMatchedEntitiesLen = prevMatchedEntities.length; p < prevMatchedEntitiesLen; ++p) {
				...
				if (onlyNew && !prevEntityObj.new && !entityObj.new) continue
	...
	// Add nodes for multi-token entity matches. To avoid duplicates (caused by deletions), this operation must occur after adding finding and merging all adjacent matches for the same entity, and keeping only the cheapest when there are duplicates.
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			if (onlyNew && !entityObj.new) continue
...
Parser.prototype.remainingTokensDeletablesLookup = function () {
	...
	for (var t = lastIdx; t > -1; --t) {
		...
		if (t < lastIdx) {
			...
			} else if (nextDeletion.new) {
				// Update existing deletables with for `followingCost` and specify `new` as `true` for spans of deletables from a token to the end of input.
				thisDeletion.followingCost = nextDeletion.cost + nextDeletion.followingCost
				thisDeletion.length = nextDeletion.length + 1
				thisDeletion.new = true
			}
...
Parser.prototype.addDeletions = function (onlyNew) {
	...
	for (var curIdx = 0; curIdx < this.tokensLen; ++curIdx) {
		...
		if ((deletion = this.deletions[curIdx + 1]) && curIdx + deletion.length === lastIdx) {
			...
			// When parsing after initially failing to reach the start node, only add deletions for new spans of deletables that extend to the end of input as denoted by `new`.
			seenNew = !onlyNew || deletion.new
			delStartIdx = curIdx
