...
Parser.prototype.matchTerminalRules = function (query) {
	...
	// Merges adjacent token matches for the same multi-token entity in `entityTab`, including deletions, while avoiding ambiguity.
...
/**
 * After failing to reach the start node or failing to generate legal parse trees (due to contradictory semantics) on the initial parse, as a last resort (and a significant performance hit), marks all tokens as deletable (except those already added to `deletions`) and adds new nodes for terminal rule matches that span and include those deletions.
 *
 * @memberOf Parser
 * @returns {Object[][]} Returns an array of an array of nodes for each terminal rule match at each index of `query`.
 */
Parser.prototype.addDeletablesForAllTokens = function () {
	...
	// Merges adjacent multi-token entity matches for the same entity that include the new deletions which `Parser.prototype.remainingTokensDeletablesLookup()` added.
	this.mergeEntityMatches(true)
	// Adds nodes for multi-token entities, including the merged entity matches, that include the new deletions which `Parser.prototype.remainingTokensDeletablesLookup()` added.
...
Parser.prototype.tokensLookup = function () {
	for (var curIdx = 0; curIdx < this.tokensLen; ++curIdx) {
		...
		if (isNaN(token)) {
			...
			// If token is a number, match with integer symbols for which `token` is within the specified value bounds, if any.
 ...
 *
 * @memberOf Parser
 * @param {string} intToken The integer input query token to match to integer symbols.
 * @param {number} tokenIdx The index of `intToken` in input.
 */
Parser.prototype.intSymbolLookup = function (intToken, tokenIdx) {
	// Convert the token to a number for comparing it to the numeric min and max value bounds of the terminal symbols. (It is faster to convert to a number with `parseFloat()` and compare than to compare the original string.)
	...
	for (var i = 0, intSymbolsLen = this.stateTable.intSymbols.length; i < intSymbolsLen; ++i) {
		...
		if (parsedFloat <= intSymbol.max) {
			...
			this.addWordNodes(terminalSym, tokenIdx, tokenIdx, semanticArg, intToken)
...
Parser.prototype.entityLookup = function (entToken, tokenIdx) {
	...
	if (entitySet) {
		var matchedEntities = this.entityTab[tokenIdx]
		...
		for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
			...
			if (entity.size > 1) {
				...
				// Save the entity match for adding deletions in `Parser.prototype.addDeleltionToAdjacentSymbolMatches()`. Include `entity` and `matchedTokens` for multi-token entities so that expansions of this match with adjacent deletables are also added to `matchedEntities`.
				this.matchedSymbols.push({
					terminalSym: terminalSym,
					semanticArg: semanticArg,
					text: entity.text,
				...
				this.addWordNodes(terminalSym, tokenIdx, tokenIdx, semanticArg, entity.text)
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			for (var p = 0, prevMatchedEntitiesLen = prevMatchedEntities.length; p < prevMatchedEntitiesLen; ++p) {
				...
				if (prevEntityObj.entity === entity) {
					...
					var newDeletionsCost = prevEntityObj.deletionsCost + entityObj.deletionsCost
					var newIdx = getMatchedEntityIndex(matchedEntities, entity, prevEntityObj.startIdx, newDeletionsCost)
					if (newIdx !== -1) {
						matchedEntities[newIdx] = {
							entity: entity,
							terminalSym: entityObj.terminalSym,
							semanticArg: entityObj.semanticArg,
							matchedTokens: matchedTokens,
...
Parser.prototype.addMultiTokenEntityNodes = function (onlyNewDeletables) {
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			if (onlyNewDeletables && !entityObj.hasNewDeletable) continue
			...
			// This check cannot occur earlier because all entity matches are needed for merging matches to the same entity object and determining which matches alias is the best match.
			...
			if (entity.hasAmbiguousMultiTokenAlias && isAmbigMultiTokenEntityMatch(matchedEntities, e)) {
				// Do not add terminal node for entity match if there exists other matches for the same entity (id and display text) across the same token span. If other matches have the same cost, then the last match will be added because this only compares entities to those that follow within `matchedEntities`.
				continue
			}
			...
			// Add nodes for the terminal rules that produce `terminalSym` (at this index), with `ruleProps` that include the entity id as the semantic argument and the display text which `entity` defines.
			this.addWordNodes(entityObj.terminalSym, entityObj.startIdx, endIdx, entityObj.semanticArg, entity.text, costPenalty)
		}
	}
}

/**
 * Checks if the multi-token entity match at index `matchIdx` in `matchedEntities` is ambiguous with another match in the array following index `matchIdx`, and this match should be discarded (i.e., not added as a terminal node) because it has a greater cost.
 *
 * Matches are ambiguous if they are for the same entity (id and display text), but with different names/aliases, and exist over the same token span. E.g., a match to "Alan" can be for either the alias "Alan Kay" or "Alan Curtis", both of which map to "Alan Kay".
 *
 * `Parser.prototype.addMultiTokenEntityNodes()` invokes this function, and only if the entity has the property `hasAmbiguousMultiTokenAlias`, which is `true` if this entity object is multi-token and shares a token with another multi-token name for same entity id (because this function could not return `true` otherwise).
 *
 * Note: It is likely the overhead from this additional check for this obscure edge case is more detrimental to performance than letting `Parser` and `pfsearch` absorb additional load, and allowing `pfsearch` to catch the ambiguity at the end.
 *
 * @private
 * @static
 * @param {Object[]} matchedEntities The multi-token entity matches to check for another (ambiguous) match to the same entity over the same token span via a different entity name/alias.
 * @param {number} matchIdx The index of the match within `matchedEntities` to compare against.
 * @returns {boolean} Returns `true` if the entity match at index `matchIdx` within `matchedEntities` is ambiguous with, and has a greater cost than, another match that follows it within the array and should be discarded, else `false`.
 */
function isAmbigMultiTokenEntityMatch(matchedEntities, matchIdx) {
	var entityObj = matchedEntities[matchIdx]
	var entityId = entityObj.entity.id

	for (var o = matchIdx + 1, matchedEntitiesLen = matchedEntities.length; o < matchedEntitiesLen; ++o) {
		var otherEntityObj = matchedEntities[o]

		// Check all other entities even if `onlyNewDeletables` because ambiguity can occur across both iterations of entity matches.
		if (otherEntityObj.entity.id === entityId && otherEntityObj.startIdx === entityObj.startIdx && otherEntityObj.deletionsCost <= entityObj.deletionsCost) {
			return true
		}
	}
...
/**
 * Gets the index of where to add the entity with the provided parameters in `matchedEntities` to avoid duplicates in `Parser.prototype.mergeEntityMatches()`.
 *
 * If `newDeletionsCost` is less than an existing match in `matchedEntities` for the same entity object, then returns the existing match's index with which to overwrite. Else if `newDeletionsCost` is greater than or equal to an an existing match for the same entity, then returns `-1` to discard the new entity match. Else, the entity is unique and returns the length of `matchedEntities` with which to append the new entity.
 *
 * @private
 * @static
 * @param {Object[]} matchedEntities The existing entity matches at this index.
 ...
 * @returns {number} Returns the index of `matchedEntities` where to add the specified entity. If `-1`, the entity is to be discarded (i.e., not added).
 */
function getMatchedEntityIndex(matchedEntities, entity, startIdx, newDeletionsCost) {
	for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
		var existingEntityObj = matchedEntities[e]

		if (existingEntityObj.entity === entity && existingEntityObj.startIdx === startIdx) {
			// Only compare `deletionsCost` because if it is the same token span (i.e., same `startIdx` and same end index because within the same `matchedEntities` array), then the only difference is deletions. If deletables ever have costs less than insertion costs for partial multi-token entity matches, then this check must be amended to account for such.
...
Parser.prototype.deletablesLookup = function () {
	...
	for (var t = this.tokensLen - 1; t > -1; --t) {
		...
		if (this.stateTable.deletables.indexOf(this.tokens[t]) !== -1) {
			...
			cost = 1
		} else if (this.wordTab[t].length === 0 && this.entityTab[t].length === 0) {
...
Parser.prototype.addDeletions = function (onlyNewDeletables) {
	...
	for (var curIdx = 0; curIdx < this.tokensLen; ++curIdx) {
		...
		while (deletion = this.deletions[--delStartIdx]) {
			...
			// When parsing after failing the initial parse, only add deletions that contain new deletables (where `cost` is 10).
		...
		if ((deletion = this.deletions[curIdx + 1]) && curIdx + deletion.length === lastIdx) {
			...
			this.nodeTab = this.nodeTabs[lastIdx]
			this.words = this.wordTab[lastIdx]
...
Parser.prototype.addDeleltionToAdjacentSymbolMatches = function (delStartIdx, delEndIdx, deletionCost, onlyNewDeletables) {
	// Add nodes for deletables that precede each terminal node and/or follow each terminal node if at the end of input. Traverses `matchedSymbols` instead of `wordTab`, because the latter will include the matches with deletions.
	for (var s = 0, matchedSymbolsLen = this.matchedSymbols.length; s < matchedSymbolsLen; ++s) {
		var symbolObj = this.matchedSymbols[s]

		// Add multi-token entity matches (denoted by presence of `symbolObj.entity`) to `entityTab` at `delEndIdx` (instead of adding nodes here) for use by `Parser.prototype.mergeEntityMatches()`. This enables merging adjacent token matches for the same entity while avoiding duplicates (i.e., ambiguity).
		var entity = symbolObj.entity
		if (entity) {
			var matchedEntities = this.entityTab[delEndIdx]
			// Add the match to `matchedEntities`. If `onlyNewDeletables`, check for existing matches in `matchedEntities`, which only occurs when `onlyNewDeletables` because otherwise there can not be a match for the same entity and length.
			var newIdx = onlyNewDeletables ? getMatchedEntityIndex(matchedEntities, entity, delStartIdx, deletionCost) : matchedEntities.length
			if (newIdx !== -1) {
				matchedEntities[newIdx] = {
					entity: entity,
					terminalSym: symbolObj.terminalSym,
					semanticArg: symbolObj.semanticArg,
					matchedTokens: symbolObj.matchedTokens,
			...
			this.addWordNodes(symbolObj.terminalSym, delStartIdx, delEndIdx, symbolObj.semanticArg, symbolObj.text, deletionCost)
		}
	}
}
