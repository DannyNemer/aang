...
Parser.prototype.entityLookup = function (entToken, tokenIdx) {
	...
	if (entitySet) {
		...
		for (var e = 0, entitySetLen = entitySet.length; e < entitySetLen; ++e) {
			...
			var semanticArg = this.getSemanticArg(entity.id, entity.isPerson)
 ...
 * @param {boolean} [isPerson] Specify this entity represents a person and can serve as the antecedent for an anaphoric expression.
 * @returns {Object[]} Returns the semantic argument for `semanticArgName`.
 */
Parser.prototype.getSemanticArg = function (semanticArgName, isPerson) {
	return this.semanticArgTab[semanticArgName] || (this.semanticArgTab[semanticArgName] = [ {
		semantic: {
			name: semanticArgName,
			// Specify this entity represents a person and can serve as the antecedent for an anaphoric expression (of matching grammatical person-number), for use in anaphora resolution in `pfsearch`.
			isPerson: isPerson,
		}
	} ])
}
