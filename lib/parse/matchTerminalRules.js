...
Parser.prototype.matchTerminalRules = function (query) {
	...
	// Merges adjacent token matches for the same multi-token entity, including deletions, while avoiding ambiguity.
	this.mergeEntityMatches()
	// Adds nodes for merged multi-token entity matches. To avoid duplicates (caused by deletions), invoke this method after `Parser.prototype.mergeEntityMatches()`, which finds and merges all adjacent matches for the same entity and keeps only the cheapest match when there are duplicates.
	this.addMergedEntityMatches()
...
/**
 * After failing to reach the start node on the initial parse, as a last resort (and a significant performance hit), marks all tokens as deletable (except those already added to `deletions`) and adds new nodes for terminal rule matches that span and include those deletions.
 *
 * @memberOf Parser
 * @returns {Object[][]} Returns an array of an array of nodes for each terminal rule match at each index of `query`.
 */
Parser.prototype.addDeletablesForAllTokens = function () {
	// Add deletables to `deletions` for every token that `Parser.prototype.deletablesLookup()` did not previously mark as deletable. Invoke this method before `Parser.prototype.addDeletions(true)`, which adds nodes with the new deletables and the adjacent terminal symbol matches.
	...
	// Adds nodes for merged multi-token entity matches that include the new deletions which `Parser.prototype.remainingTokensDeletablesLookup()` added.
	this.addMergedEntityMatches(true)
...
/**
 * Merges adjacent multi-token entity matches for the same entity, including deletions, while avoiding ambiguity.
 *
 * @memberOf Parser
 * @param {boolean} [onlyNew] Specify this invocation is after failing to reach the start node on the initial parse, and to only add nodes using the new deletables.
 */
Parser.prototype.mergeEntityMatches = function (onlyNew) {
	...
	for (var endIdx = 1; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			...
			for (var p = 0, prevMatchedEntitiesLen = prevMatchedEntities.length; p < prevMatchedEntitiesLen; ++p) {
				...
				// If `onlyNew`, require at least one of the two entity matches (to merge) to contain a new deletable.
				if (onlyNew && !prevEntityObj.isNew && !entityObj.isNew) continue

				// Check if `prevEntityObj`, which directly precedes `entityObj` in input, is a match for the same entity.
 ...
 * @param {boolean} [onlyNew] Specify this invocation is after failing to reach the start node on the initial parse, and to only add nodes using the new deletables.
 */
Parser.prototype.addMergedEntityMatches = function (onlyNew) {
	for (var endIdx = 0; endIdx < this.tokensLen; ++endIdx) {
		...
		for (var e = 0, matchedEntitiesLen = matchedEntities.length; e < matchedEntitiesLen; ++e) {
			var entityObj = matchedEntities[e]
			// If `onlyNew`, only add entity matches that contain deletables which `Parser.prototype.remainingTokensDeletablesLookup()` added.
			if (onlyNew && !entityObj.isNew) continue
...
/**
 * After failing to reach the start node on the initial parse, adds deletables to `deletions` for every token that `Parser.prototype.deletablesLookup()` did not previously mark as deletable.
 *
		...
		if (t < lastIdx) {
			...
			if (!thisDeletion) {
				this.deletions[t] = {
					...
					// The length of continuous deletables beginning at this index.
					length: nextDeletion.length + 1,
					// Specify this deletion was added on the second parse (after initially failing to reach the start node) to distinguish the deletable from existing deletions from which nodes were already created. This enables `Parser.prototype.addDeletions(true)` to avoid adding duplicate nodes.
					isNew: true,
				}
			} else if (nextDeletion.isNew) {
				// Update existing deletables with for `followingCost` and specify `isNew` as `true` for spans of deletables from a token to the end of input.
 ...
 * @param {boolean} [onlyNew] Specify this invocation is after failing to reach the start node on the initial parse, and to only add nodes using the new deletables.
 */
Parser.prototype.addDeletions = function (onlyNew) {
	...
	for (var curIdx = 0; curIdx < this.tokensLen; ++curIdx) {
		...
		while (deletion = this.deletions[--delStartIdx]) {
			...
			// When parsing after initially failing to reach the start node, only add deletions that contain new deletables (where `cost` is 10).
			if (seenNew || deletion.cost === 10) {
				seenNew = true
				...
				this.addDeleltionToAdjacentSymbolMatches(delStartIdx, curIdx, deletionCost, onlyNew)
			}
		}
		...
		if ((deletion = this.deletions[curIdx + 1]) && curIdx + deletion.length === lastIdx) {
			...
			// When parsing after initially failing to reach the start node, only add deletions for new spans of deletables that extend to the end of input as denoted by `isNew`.
			seenNew = !onlyNew || deletion.isNew
			delStartIdx = curIdx
			...
			do {
				...
				if (seenNew || deletion.cost === 10) {
					...
					this.addDeleltionToAdjacentSymbolMatches(delStartIdx, lastIdx, deletionCost, onlyNew)
				}
			} while (deletion = this.deletions[--delStartIdx])
		}
	}
 ...
 * @param {boolean} [onlyNew] Specify this invocation is after failing to reach the start node on the initial parse, and to mark entities added to `entityTab` as `new` (for use by `Parser.prototype.mergeEntityMatches()`).
 */
Parser.prototype.addDeleltionToAdjacentSymbolMatches = function (delStartIdx, delEndIdx, deletionCost, onlyNew) {
	...
	for (var s = 0, matchedSymbolsLen = this.matchedSymbols.length; s < matchedSymbolsLen; ++s) {
		...
		// Add multi-token entities to `entityTab` at `delEndIdx` (instead of adding nodes here) for use by `Parser.prototype.mergeEntityMatches()`. This enables merging adjacent token matches for the same entity while avoiding duplicates (i.e., ambiguity).
		var entity = symbolObj.entity
		if (entity && entity.size > 1) {
			var matchedEntities = this.entityTab[delEndIdx]
			// Add the match to `matchedEntities`. If `onlyNew`, check for existing matches in `matchedEntities`, which only occurs when `onlyNew` because otherwise there can not be a match for the same entity and length.
			var newIdx = onlyNew ? getMatchedEntityIndex(matchedEntities, entity, delStartIdx, deletionCost) : matchedEntities.length
			if (newIdx !== -1) {
				matchedEntities[newIdx] = {
					entity: entity,
