...
const semanticReduce = require('../grammar/semantic').reduce
...
Parser.prototype.terminalSymbolLookup = function (symToken, tokenIdx) {
	const terminalSym = this.stateTable.terminalSymTab[symToken]
...
Parser.prototype.intSymbolLookup = function (intToken, tokenIdx) {
	...
	const parsedFloat = parseFloat(intToken)
...
Parser.prototype.entityLookup = function (entToken, tokenIdx) {
	...
	const entitySet = this.stateTable.entitySets[entToken]

	if (entitySet) {
		const entityMatches = this.entityTab[tokenIdx]
...
function isAmbigMultiTokenEntityMatch(entityMatches, matchIdx) {
	const entityMatch = entityMatches[matchIdx]
	const entityId = entityMatch.entity.id
...
Parser.prototype.remainingTokensDeletablesLookup = function () {
	const lastIdx = this.tokensLen - 1
...
Parser.prototype.addTermRuleNodes = function (terminalSym, startIdx, endIdx, semanticArg, text, costPenalty) {
	...
	const wordSize = endIdx - startIdx + 1
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (node) termRuleNodes.push(node)
	}
