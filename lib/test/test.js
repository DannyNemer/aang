 ...
 *   correction` defining the expected display text of the parse\'s first result.
 *
 * Options
...
var argv = yargs
	.usage([
		...
		'  Parses the suite of test queries and checks output conforms to the test\'s specifications.',
		'',
			'  The test suite contains an array of `Object`s with a `query` to parse and the property `correction` defining the expected display text of the parse\'s first result.'
	].join('\n'))
	.updateStrings({
...
function checkTest(test, trees) {
	...
	if (correction) {
		if (correction === true && query === actual) {
			...
			// Parse's first result does not match expected display text.
