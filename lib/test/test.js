...
var forestSearch = require('../parser/forestSearch')
var printParseResults = require('../parser/printParseResults')

// Generate a `StateTable` from the grammar and instantiate a `Parser`.
var stateTable = require('../parser/buildStateTable')
...
// Print test results.
if (testsFailed) {
	util.log(util.colors.red('Error') + ':', 'Failed', testsFailed, 'of', testsLen, 'tests')
} else {
...
function parseTest(test) {
	...
	if (startNode) {
		...
		var trees = forestSearch(startNode, argv.k, argv.printTrees, !argv.quiet)
...
function failTest(test, expected, actual) {
	...
	util.log(util.colors.red('Error') + ':', 'Expected:', expected)
