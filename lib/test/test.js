 ...
 *
 *   For each provided <tag>, only runs tests in the suite with that tag. If none, parses the entire
 *   suite.
 *
 * Options
...
var argv = yargs
	.usage([
		...
		'',
		'  For each provided <tag>, only runs tests in the suite with that tag. If none, parses the entire suite.',
	].join('\n'))
	.updateStrings({
...
var argTags = argv._
if (argTags.length > 0) {
	tests = filterTestsByTags(tests, argTags)
	util.log('Parsing tests with tags:', argTags.map(util.stylize).join(' '))
}

// Will every test produce output.
...
function parseTest(test) {
	...
	if (printEveryQuery) printQuery(query)
	...
	if (startNode) {
		...
		var pfsearchVals = pfsearch(startNode, argv.k, {
			buildTrees: argv.printTrees,
			printStats: !argv.quiet || argv.benchmark,
			printAmbiguity: argv.printAmbiguity,
			returnPathCount: true,
		})

		var trees = pfsearchVals.trees
		pathsCreated += pfsearchVals.pathCount
...
/**
 * Creates a new array with all tests from `tests` that contain a tag in `argTags`.
 *
 * @param {Object[]} tests The test suite.
 * @param {string[]} argTags The tags provided as command line arguments.
 * @returns {Object[]} Returns the new, filtered array of tests.
 */
function filterTestsByTags(tests, argTags) {
	var filteredTests = []
	var foundArgTags = []
	var argTagsLen = argTags.length

	// Adds test to `filteredTests` that contain a tag in `argTags`. Maintain the same order as in `tests`.
	for (var t = 0, origTestLen = tests.length; t < origTestLen; ++t) {
		var test = tests[t]
		var testTags = test.tags

		if (testTags) {
			// Instead of checking for an intersection of at least one tag, check all tags to add every match to `foundArgTags` in case one tag is a complete subset of another tag and would not be marked as found otherwise.
			var containsMatchingTag = false
			for (var i = 0, testTagsLen = testTags.length; i < testTagsLen; ++i) {
				var testTag = testTags[i]

				for (var a = 0; a < argTagsLen; ++a) {
					if (testTag === argTags[a]) {
						containsMatchingTag = true

						if (foundArgTags.indexOf(testTag) === -1) {
							foundArgTags.push(testTag)
						}
					}
				}
			}
	...
	for (var a = 0; a < argTagsLen; ++a) {
		var argTag = argTags[a]
		if (foundArgTags.indexOf(argTag) === -1) {
			util.logError('Tag not found:', util.stylize(argTag))
			process.exit(1)
		}
	}
