 ...
 *   -a, --all-semantics            Check semantics of all parse results.                     [boolean]
...
var argv = yargs
	...
	.options({
		...
		'a': {
			alias: 'all-semantics',
			description: 'Check semantics of all parse results.',
			type: 'boolean',
		},
		'q': {
			alias: 'quiet',
...
function checkTestResults(test, trees) {
	...
	// If `--all-semanitcs` is passed, then compare the test's semantic results to the test's expected semantics, if defined. Checks for missing and new semantics, ignoring order.
	if (argv.allSemantics && test.suggestions) {
		var diff = diffSemanticSuggestions(test.suggestions, trees)
		if (diff) {
...
/**
 * Compares a test's semantic results to the test's expected semantics. Checks for missing and new semantics, ignoring order. Stylizes the differences for printing.
 *
 * @param {string[]} expectedSuggestions The test's expected semantic results to compare.
 * @param {Object[]} trees The test's output trees with the semantics to compare.
 * @returns {string|undefined} Returns a stylized string of the differences, if any, else `undefined`.
 */
function diffSemanticSuggestions(expectedSuggestions, trees) {
	// Map test result's semantic strings and disambiguations to a single array.
	var actualSuggestions = []

	for (var t = 0, treesLen = trees.length; t < treesLen; ++t) {
		var tree = trees[t]

		actualSuggestions.push(tree.semanticStr)

		if (tree.disambiguation) {
			Array.prototype.push.apply(actualSuggestions, tree.disambiguation)
		}
	}

	var diff = []

	// Check if up to the first `compareMax` results of one exists in all of the other set. This avoids reporting missing/new semantics because of slight result ordering changes where the last few similarly cost rules are cut off in one (i.e, > k-th result) and not the other and vice versa.
	var compareMax = 35
	var expectedLen = Math.min(expectedSuggestions.length, compareMax)
	var actualLen = Math.min(actualSuggestions.length, compareMax)

	// Check for missing semantics in test results.
	for (var e = 0; e < expectedLen; ++e) {
		var expected = expectedSuggestions[e]

		if (actualSuggestions.indexOf(expected) === -1) {
			diff.push(util.colors.red(expected))
		}
	}

	// Check for new semantics in test results.
	for (var a = 0; a < actualLen; ++a) {
		var actual = actualSuggestions[a]

		if (expectedSuggestions.indexOf(actual) === -1) {
			diff.push(util.colors.green(actual))
		}
	}
...
/**
 * Compares two strings word by word, ignoring whitespace, and stylizes the differences for printing.
 *
 ...
 *   - If `true`, then display text must be (any `string`) distinguishable from `test.query`.
 *   - If `false`, then display text must match `test.query` (i.e., no edits).
 *   - If a `string`, then display text must match `test.correction`.
 ...
 * @param {string[]} [test.suggestions] The optional expected semantics to compare, irrespective of order, to test output.
...
function validateTests(tests, testsFilePath) {
	var testSchema = {
		...
		correction: { type: [ Boolean, String ] },
		// The expected semantic of the parse's first result. Required when `test.correction` is `false` or a `string` (i.e., accepts a specific result), otherwise (i.e., accepts varying results) forbidden.
		semantic: { type: String, optional: true },
		// The optional note regarding the test printed when the test fails.
		note: { type: String, optional: true },
		// The optional expected semantics to compare, irrespective of order, to test output.
		suggestions: { type: Array, arrayType: String, optional: true },
	}
	...
	for (var t = 0, testsLen = tests.length; t < testsLen; ++t) {
		...
		var testQueryQuote = '"' + testQuery + '"'

		if (util.illFormedOpts(testSchema, test)) {
			util.log('  ' + util.firstPathAndLineNumberOf(testsFilePath, testQueryQuote))
		...
		for (var a = t + 1; a < testsLen; ++a) {
			if (testQuery === tests[a].query) {
				...
				util.pathAndLineNumbersOf(testsFilePath, testQueryQuote).forEach(function (path) {
					util.log('  ' + path)
				})
