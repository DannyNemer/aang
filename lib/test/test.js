 ...
 *   --count                   Print the number of tests in the test suite.                   [boolean]
...
var argv = yargs
	...
	.options({
		...
		'count': {
			description: 'Print the number of tests in the test suite.',
			type: 'boolean',
		},
	})
...
// Executes `--list-tests`, `--list-tags`, or `--count` when passed as command line arguments.
// Invoke after filtering tests by tags passed as command line arguments to filter `--list-tests` output.
if (processTestSuiteCommands(argv, tests)) {
	process.exit()
}
...
/**
 * Executes the following test suite commands when passed as command line arguments:
 *   `--list-tests` List the input queries in the test suite.
 *   `--list-tags`  List the tags in the test suite.
 ...
 * @param {Object[]} tests The tests in the test suite.
 * @returns {boolean} Returns `true` if a command is executed, else `false`.
 */
function processTestSuiteCommands(argv, tests) {
	// Print the input queries in the test suite. Invoke after filtering tests by tags passed as command line arguments, if any.
	if (argv.listTests) {
	...
	// Print the number of tests in the test suite.
	if (argv.count) {
...
function filterTestsByTagArgs(argv, tests) {
	...
	if (argTags.length > 0) {
		...
		util.log('Tests with the following tags:', argTags.map(util.unary(util.stylize)).join(', '))
	}
