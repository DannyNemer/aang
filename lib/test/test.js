...
function checkTestTopResult(test, actualTopResult) {
	...
	if (expectedTopResult) {
		...
		if (expectedTopResult.semantic !== actualTopResult.semanticStr) {
			...
			var expected = util.camelCase(expectedTopResult.semantic).replace(/[()]/g, ' $& ')
			var actual = util.camelCase(actualTopResult.semanticStr).replace(/[()]/g, ' $& ')

			var diff = util.diffStrings(expected, actual)

			// Restore kebab case and parentheses.
			expected = util.kebabCase(diff.expected).replace(/ /g, '')
			actual = util.kebabCase(diff.actual).replace(/ /g, '')
