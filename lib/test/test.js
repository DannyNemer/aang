...
var argv = yargs
	...
	.command('list', 'List the input queries in the test suite.', function (yargs) {
		...
		filterTestsByTagArgs(yargs.argv, tests).forEach(printTest)
		process.exit()
	})
	.command('search', 'Find and print the test in the test suite with `<query>`, if any.', function (yargs) {
		...
		var test = getTest(tests, testQuery)
		if (test) {
			printTest(test)
		} else {
 ...
 * @private
 * @static
 * @param {Object[]} tests The tests in which to search.
 * @param {string} query The test query for which to search.
 * @returns {Object|undefined} Returns the test with query `query` if exists, else `undefined`.
 */
function getTest(tests, query) {
