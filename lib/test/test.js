 ...
 *   -z, --check-semantics          Compare all semantic results to each test's expected semantics.
 *                                                                                            [boolean]
 ...
 *   -t, --print-trees              Print the parse trees.                                    [boolean]
 ...
 *   -s, --print-semantics          Print the semantic representation of each parse tree.
...
if (argTags.length > 0) {
	...
	util.log('Parsing tests with the following tags:', argTags.map(function (tagName) {
		return util.stylize(tagName)
	}).join(', '))
}
		...
		if (parseResults.trees.length > 0) {
			...
			if (!argv.quiet) {
				...
				printParseResults(parseResults, {
					...
					noSemantics: !argv.printSemantics,
				})
			}
		} else {
