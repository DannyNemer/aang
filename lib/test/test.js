...
var argv = yargs
	...
	.command('search', 'Find and print the test in the test suite with `<query>`, if any.', function (yargs) {
		var testQuery = yargs.argv._.slice(1).join(' ')
		var test = testUtil.getTest(tests, testQuery)
		if (test) {
			util.logSuccess('Test found.')
			testUtil.printTest(test)
		} else {
			util.logErrorSansNewLine('No test found:', util.stylize(testQuery))
		}
		process.exit()
	})
...
function printTestFailure(test, actual) {
	if (argv.mute) {
		...
		if (!argv.quiet) {
			util.logErrorSansNewLine(actual)
		}
		return
	}
...
function printTopResultTestFailure(test) {
	...
	for (var a = 1, argumentsLen = arguments.length; a < argumentsLen; ++a) {
		...
		util.logErrorSansNewLine('Expected:', testDiff.expected)
...
function checkTestSemantics(test, trees) {
	...
	if (diff) {
		...
		util.logErrorSansNewLine('Semantic results differ:')
		util.log(diff)
