...
function checkTestTopResult(test, actualTopResult) {
	...
	if (expectedTopResult) {
		...
		if (expectedTopResult.text !== actualTopResult.text) {
			...
			printTopResultTestFailure(test, diff.actual, diff.expected)
			testPassed = false
		}
		...
		if (expectedTopResult.semantic !== actualTopResult.semanticStr) {
			...
			printTopResultTestFailure(test, actual, expected)
			testPassed = false
		}
	}
 ...
 * @param {string} actual The test's actual value.
 * @param {string} [expected] The test's expected value. If omitted, uses `test.topResult` or '--not-input' (i.e., anything but `actual`).
 */
function printTopResultTestFailure(test, actual, expected) {
	if (argv.mute) {
