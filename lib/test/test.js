...
function checkTestResults(test, trees) {
	...
	var actualTopResult = trees[0]

	if (expectedTopResult) {
		// Check if expected top result text does not match actual text.
		...
		// Check if expected top result semantic does not match actual semantic.
	...
	// Check if parse incorrectly accepts input (i.e., no edits).
		...
		failTest(test, actualTopResult.text)
	}
	...
	if (argv.allSemantics && test.semantics) {
		var diff = diffSemantics(test.semantics, trees)
		if (diff) {
 ...
 *
 * @param {string[]} expectedSemantics The test's expected semantic results to compare.
 ...
 */
function diffSemantics(expectedSemantics, trees) {
	var diff = []

	// Get the semantic strings and disambiguated semantics of `trees`.
	var actualSemantics = getSemantics(trees)
	...
	var compareMax = 35
	var expectedLen = Math.min(expectedSemantics.length, compareMax)
	var actualLen = Math.min(actualSemantics.length, compareMax)

	// Check for missing semantics in test results.
	for (var e = 0; e < expectedLen; ++e) {
		var expected = expectedSemantics[e]

		if (actualSemantics.indexOf(expected) === -1) {
	...
	for (var a = 0; a < actualLen; ++a) {
		var actual = actualSemantics[a]

		if (expectedSemantics.indexOf(actual) === -1) {
 ...
 * @param {string[]} [test.semantics] The optional unsorted expected semantics to compare (irrespective of order) to test output.
...
function validateTests(tests, testsFilePath) {
	var testSchema = {
		...
		semantics: { type: Array, arrayType: String, optional: true },
	}
