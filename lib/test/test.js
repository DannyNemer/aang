 ...
 *   correction` defining whether the parse's first result's display text should match `query`.
 *
 * Options
...
var argv = yargs
	.usage([
		...
		'  Parses the suite of test queries and checks output conforms to the test\'s specifications.',
		'',
			'  The test suite contains an array of `Object`s with a `query` to parse and the property `correction` defining whether the parse\'s first result\'s display text should match `query`.',
	].join('\n'))
	.updateStrings({
