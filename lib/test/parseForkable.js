 ...
 *
 * Options:
 *   -q, --return-query  Include the parse query with the benchmark measurement in the
 *                       message returned to the parent process.               [boolean]
 ...
 */
 ...
 var argv = require('yargs')
	...
	.options({
		'q': {
			alias: 'return-query',
			description: 'Include the parse query with the benchmark measurement in the message returned to the parent process.',
			type: 'boolean',
		},
	})
...
process.send(argv.returnQuery ? { query: query, duration: duration } : duration)