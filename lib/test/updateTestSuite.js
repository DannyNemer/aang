	...
	for (var t = 0, testsLen = tests.length; t < testsLen; ++t) {
		...
		if (!util.arraysEqual(test.semantics, newTest.semantics) || !util.arraysEqual(test.tags, newTest.tags)) {
			...
			if (!util.objectsEqual(test.topResult, newTest.topResult)) {
				...
				promptTestUpdateDiffTopResult(tests, t, newTest)
			} else {
				// Prompt test update.
				promptTestUpdate(tests, t, newTest)
			}
		}
	}
}
 ...
 * @returns {boolean} Returns `true` if the user overwrote the existing test, else `false`.
 */
...
function actionPrompt(tests, testIndex, newTest, updatesTopResult) {
	...
	switch (actionKey) {
		case 'y':
			...
			updateTest(tests, testIndex, newTest)
			return true
		case 'e':
