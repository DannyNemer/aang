...
function updateTags(test, trees) {
	...
	var unionTag = 'union'
	var unionTagIndex = existingTags.indexOf(unionTag)
	if (testTagsUtil.hasUnion(test, trees)) {
		if (unionTagIndex === -1) {
			existingTags.push(unionTag)
		}
	} else if (unionTagIndex !== -1) {
		existingTags.splice(unionTagIndex, 1)
	}

	// Add/remove 'anaphora' tag.
	var anaphoraTag = 'anaphora'
	var anaphoraTagIndex = existingTags.indexOf(anaphoraTag)
	if (testTagsUtil.isAnaphoric(test, trees)) {
		if (anaphoraTagIndex === -1) {
			existingTags.push(anaphoraTag)
		}
	} else if (anaphoraTagIndex !== -1) {
		existingTags.splice(anaphoraTagIndex, 1)
	}
