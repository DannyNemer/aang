	...
	for (var t = 0, testsLen = tests.length; t < testsLen; ++t) {
		...
		if (!util.arraysEqual(test.semantics, newTest.semantics) || !util.arraysEqual(test.tags, newTest.tags)) {
			...
			if (!util.objectsEqual(test.topResult, newTest.topResult)) {
				...
				overwriteExisting = promptTestUpdateDiffTopResult(tests, t, newTest)
			} else {
				// Prompt test update.
				overwriteExisting = promptTestUpdate(tests, t, newTest)
			}

			if (overwriteExisting) {
				updateTest(tests, t, newTest)
			} else {
 ...
 */
function promptTestUpdateDiffTopResult(tests, testIndex, newTest) {
	...
	// Prompt user to overwrite a test change that changes `existingTest.topResult`.
	return actionPrompt(true, tests[testIndex], newTest)
}
 ...
 * @returns {boolean} Returns `true` if the user chooses to overwrite the test, else `false`.
 */
function promptTestUpdate(tests, testIndex, newTest) {
	if (argv.superForce || argv.force) {
		return true
	}

	// Prompt user to overwrite a test change.
	return actionPrompt()
}

/**
 * Prompts the user to overwrite a test change.
 *
 * @private
 * @static
 * @param {boolean} [updatesTopResult] Specify whether this change overwrites `existingTest.topResult`.
 * @param {Object} [existingTest] The existing test to overwrite (needed if `updatesTopResult`).
 * @param {Object} [newTest] The new text to replace `existingTest` (needed if `updatesTopResult`).8
 * @returns {boolean} Returns `true` if the user chooses to overwrite the test, else `false`.
 */
function actionPrompt(updatesTopResult, existingTest, newTest) {
	var actions = {
	...
	// If this update does not overwrite `topResult`, hide the `topResult` action in the action list.
	if (!updatesTopResult) {
		delete actions.e
	}
	...
	var actionKey = readlineSync.keyIn(util.colors.bold.blue('Overwrite this test [' + actionKeys + ']? '), {
		limit: actionKeys,
	})
 ...
 * @param {Object} newTest The new test to replace the existing test at `tests[testIndex]`.
 */
function updateTest(tests, testIndex, newTest) {
