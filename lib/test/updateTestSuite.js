...
for (var t = 0, testsLen = tests.length; t < testsLen; ++t) {
	...
	if (!util.arraysEqual(test.semantics, newTest.semantics) || !util.arraysEqual(test.tags, newTest.tags)) {
		...
		var testIndexPromptStr = '(' + (t + 1) + ' of ' + testsLen + ')'
		...
		if (!util.objectsEqual(test.topResult, newTest.topResult)) {
			...
			if (argv.superForce) {
				...
				var answerIndex = readlineSync.keyInSelect([
					...
					'Overwrite test except `topResult`.',
					'Skip test.'
				], testIndexPromptStr + ' Which would you like to do?', {
			...
			var promptMessage = testIndexPromptStr + ' Do you want to overwrite this test? '
			overwriteExisting = argv.superForce || argv.force || readlineSync.keyInYNStrict(promptMessage)
		}
