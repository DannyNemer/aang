	...
	for (var t = 0, testsLen = tests.length; t < testsLen; ++t) {
		...
		if (!util.arraysEqual(test.semantics, newTest.semantics) || !util.arraysEqual(test.tags, newTest.tags)) {
			...
			if (!util.objectsEqual(test.topResult, newTest.topResult)) {
				...
				overwriteExisting = promptTestUpdateDiffTopResult(test, newTest)
			} else {
				// Prompt test update.
				overwriteExisting = promptTestUpdate()
			}
 ...
 * @private
 * @static
 * @param {Object} existingTest The existing test to overwrite.
 * @param {Object} newTest The new text to replace `existingTest`.
 * @returns {boolean} Returns `true` if the user chooses to overwrite the test, else `false`.
 */
function promptTestUpdateDiffTopResult(existingTest, newTest) {
	...
	return actionPrompt(true, existingTest, newTest)
}
