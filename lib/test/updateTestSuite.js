...
for (var t = 0, testsLen = tests.length; t < testsLen; ++t) {
	...
	var newTest = {
		...
		tags: updateTags(test, parseResults),
		...
		semantics: parseResults.trees ? mapTreesToSemantics(parseResults.trees) : [],
	}
	...
	if (!util.arraysEqual(test.semantics, newTest.semantics) || !util.arraysEqual(test.tags, newTest.tags)) {
		...
		if (readlineSync.keyInYNStrict('(' + (t + 1) + ' of ' + testsLen + ') Do you want to overwrite this test? ')) {
			tests[t] = newTest
 ...
 */
function updateTags(test, parseResults) {
	// Clone tags array to enable checking for changes that require saving.
	var existingTags = test.tags.slice()

	// Add/remove 'union' tag.
	updateTag(test, parseResults, existingTags, 'union', testTagsUtil.hasUnion)

	// Add/remove 'anaphora' tag.
	updateTag(test, parseResults, existingTags, 'anaphora', testTagsUtil.isAnaphoric)

	// Add/remove 'start-sym-fail' tag.
	updateTag(test, parseResults, existingTags, 'start-sym-fail', testTagsUtil.didFailInitStartSym)

	// Add/remove 'no-legal-trees' tag.
	updateTag(test, parseResults, existingTags, 'no-legal-trees', testTagsUtil.didFailInitLegalTrees)

	return existingTags
}

/**
 * Adds or removes `tag` to/from `existingTags` based on whether `validator` returns truthy when invoked with the following two arguments: (test, parseResults).
 *
 * @private
 * @static
 * @param {Object} test The test to check.
 ...
 */
function updateTag(test, parseResults, existingTags, tag, validator) {
	var tagIndex = existingTags.indexOf(tag)
	if (validator(test, parseResults)) {
		if (tagIndex === -1) {
