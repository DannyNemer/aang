...
function promptTestUpdateDiffTopResult(tests, testIndex, newTest) {
	...
	return actionPrompt(tests, testIndex, newTest, true)
}
...
function promptTestUpdate(tests, testIndex, newTest) {
	...
	return actionPrompt(tests, testIndex, newTest)
}
 ...
 * @returns {boolean} Returns `true` if the user chooses to overwrite the test, else `false`.
 */
function actionPrompt(tests, testIndex, newTest, updatesTopResult) {
	var actions = {
	...
	switch (actionKey) {
		...
		case 'e':
			...
			newTest.topResult = tests[testIndex].topResult
			return true
		case 'n':
			// Skip this test.
			return false
	...
	throw new Error('actionPrompt: Should be unreachable.')
}
