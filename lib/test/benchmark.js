/**
 * A command line application that benchmarks the duration of parsing the queries in the test suite. For each parse, finds the 50-best parse trees and with no output.
 *
 ...
 * parse, finds the 50-best parse trees and does not output.
 *
 * Options:
 ...
 *
 * Examples:
 *   node benchmark.js -n=5  Benchmark the duration of parsing the queries in the test suite 5
 *                           times.
 */
...
var argv = yargs
	.usage(
		...
		'Description: Benchmark the duration of parsing the queries in the test suite. For each parse, finds the 50-best parse trees and does not output.'
	)
	.options({
		'n': {
			alias: 'num-runs',
