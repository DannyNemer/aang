...
function autoAddTags(newTest, trees) {
	if (hasUnion(newTest, trees)) {
		newTest.tags.push('union')
	}
}

/**
 * Checks if the 'union' tag applies to `newTest`.
 *
 * @param {Object} newTest The new test to check.
 * @param {Object[]} trees The parse trees from `newTest.query`.
 * @returns {boolean} Returns `true` if the 'union' tag applies to `newTest`, else `false`.
 */
function hasUnion(newTest, trees) {
	var reHasOr = /(^|\W)or(\W|$)/
	var reHasUnion = /(^|\W)union\((\W|$)/

	// Check if input query contains "or".
	if (reHasOr.test(newTest.query)) {
		return true
	}
	...
	var expectedTopResult = newTest.topResult
