 ...
 * @param {Object[]} trees The parse trees from `newTest.query`.
 */
function autoAddTags(newTest, trees) {
	// Check if 'union' tag applies.
	if (testTagsUtil.hasUnion(newTest, trees)) {
		newTest.tags.push('union')
	}

	// Check if 'anaphora' tag applies.
	if (testTagsUtil.isAnaphoric(newTest, trees)) {
		newTest.tags.push('anaphora')
	}

	// Check if 'start-sym-fail' tag applies.
	if (testTagsUtil.didFailInitStartSym(newTest, trees)) {
		newTest.tags.push('start-sym-fail')
	}

	// Check if 'no-legal-trees' tag applies.
	if (testTagsUtil.didFailInitLegalTrees(newTest, trees)) {
