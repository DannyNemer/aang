...
// Auto-add tags that apply to the new test.
autoAddTags(newTest, parseResults)
// Remove the matched tags from the choices.
for (var t = 0, tagsLen = newTest.tags.length; t < tagsLen; ++t) {
	tags.splice(tags.indexOf(newTest.tags[t]), 1)
}
if (tagsLen > 0) {
	util.log('Tags for new test:', newTest.tags)
}

// Prompt user to add tags to the new test.
tagsPrompt(newTest, tags)
...
/**
 * Adds tags that apply to `newTest`.
 *
 * @private
 * @static
 * @param {Object} newTest The new test to check.
 * @param {Object} parseResults The parse results from parsing `test.query`.
 */
function autoAddTags(newTest, parseResults) {
	// Check if 'union' tag applies.
	if (testTagsUtil.hasUnion(newTest, parseResults)) {
		newTest.tags.push('union')
	}

	// Check if 'anaphora' tag applies.
	if (testTagsUtil.isAnaphoric(newTest, parseResults)) {
		newTest.tags.push('anaphora')
	}

	// Check if 'start-sym-fail' tag applies.
	if (testTagsUtil.didFailInitStartSym(newTest, parseResults)) {
		newTest.tags.push('start-sym-fail')
	}

	// Check if 'no-legal-trees' tag applies.
	if (testTagsUtil.didFailInitLegalTrees(newTest, parseResults)) {
		newTest.tags.push('no-legal-trees')
	}
}
