 ...
 *
 * Displays the current `query` parse results, prompts the user for the new test `topResult` and description, and automatically assigns applicable test tags, if any.
 *
 * @private
 * @static
...
function addNewTest(tests, newTest) {
	...
	for (var t = 0, testsLen = tests.length; t < testsLen; ++t) {
		...
		if (existingTest.query === newTest.query) {
			// Prompt the user to confirm overwriting the existing test at index `t`.
			return promptOverwriteTest(tests, newTest, t)
		}
	}

	// Prompt the user to confirm appending `newTest` to the end of the test suite.
	return promptAddTest(tests, newTest, testsLen)
}

/**
 * Prompts the user to save `newTest` to `tests[testIndex]`.
 *
 * @private
 * @static
 * @param {Object[]} tests The test suite.
...
function promptAddTest(tests, newTest, testIndex) {
	...
	var actionKey = readlineSync.keyIn(util.colors.bold.blue('Add this test [' + actionKeys + ']? '), {
		limit: actionKeys,
	})
	...
	throw new Error('promptAddTest: Should be unreachable.')
}
