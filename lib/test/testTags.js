 ...
 * @property {Function} appliesToTest The function that returns truthy if the Tag applies to a given test. The function inspects the test's input query, expected output, and parse results returned by `Parser.prototype.parse()` for parsing `test.query`. The function is invoked with two arguments: (test, parseResults).
 */
var tagSchema = {
...
/**
 * The map of test suite tag names to `Tag` objects. Each defines a tag used in the test suite and can check its application to a provided test.
 *
 * @type {Object.<string, Tag>}
 */
module.exports = {
	'start-sym-fail': {
		description: 'Indicates the parse initially fails to reach the start symbol, which requires marking all input tokens as deletable and reparsing. The second parse may also fail.',
		appliesToTest: function (test, parseResults) {
			return parseResults.failedInitStartSym
		},
	},
	'no-legal-trees': {
		description: 'Indicates the parse initially fails to generate any legal parse trees (after reaching the start symbol) due to illegal semantics, which requires marking all input tokens as deletable and reparsing. The second parse may also fail.',
		appliesToTest: function (test, parseResults) {
			return parseResults.failedInitLegalTrees
		},
	},
	'reparse': {
		description: 'Indicates the parse requires marking all input tokens as deletable and reparsing. Hence, the union of tags \'start-sym-fail\' and \'no-legal-trees\'.',
		appliesToTest: function (test, parseResults) {
			return module.exports['start-sym-fail'].appliesToTest(test, parseResults) ||
				module.exports['no-legal-trees'].appliesToTest(test, parseResults)
		},
	},
	'union': {
