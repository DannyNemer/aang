 ...
 */
module.exports = function (startNode, k, buildTrees, printStats) {
	...
	heap.push({
		...
		node: startNode,
		// The reverse linked list of the second nodes of previously incomplete binary rules and yet-to-conjugate text objects from insertion rules. When `node` is undefined after reaching a terminal symbol, inspect `nextItems` to complete the binary rules and conjugate the text objects.
		nextItems: undefined,
		// The number of nodes in the `nextItems` that can produce a semantic. This excludes other nodes and yet-to-conjugate text. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
		nextNodesCount: 0,
...
function createPath(prevPath, sub, ruleProps) {
	...
	var newPath = {
		...
		// `prevPath.nextItems.minCost` is the total heuristic cost of incomplete branches from previous binary rules.
		cost: newCost + sub.minCost + (prevPath.nextItems ? prevPath.nextItems.minCost : 0),
	}

	// Nonterminal rule.
	if (sub.node.subs) {
		// Append `sub`'s semantics, if any, to `prevPath.semantics`.
		newPath.semantics = appendSemantic(prevPath.semantics, prevPath.nextNodesCount, ruleProps)
		...
		if (ruleProps.insertionIdx === undefined) {
			...
			if (subNext) {
				newPath.nextItems = {
					...
					minCost: subNext.minCost + (prevPath.nextItems ? prevPath.nextItems.minCost : 0),
		...
		// Insertions only exist on nonterminal rules because they can only be built from binary rules. This might iIf we enable terminal symbols to be in a RHS with another terminal or nonterminal symbol (or multiple).
			...
			if (ruleProps.insertionIdx === 1) {
				...
				newPath.nextItems = {
					text: ruleProps.text,
					minCost: prevPath.nextItems ? prevPath.nextItems.minCost : 0,
					prev: prevPath.nextItems,
				}
			} else {
	...
	// Terminal rule.
		...
		newPath.semantics = reduceSemanticTree(prevPath.semantics, prevPath.nextNodesCount, ruleProps.semantic)
 ...
 * @param {number} nextNodesCount The number of nodes in the previous path's `nextItems` that can produce a semantic. Used to determine if a RHS semantic is complete (no more semantics will follow it) and can be reduced with the preceding LHS semantic.
 ...
 */
function reduceSemanticTree(prevSemantic, nextNodesCount, newSemantic) {
