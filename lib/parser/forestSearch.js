...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		// Save this node whose subnodes this path can expand from.
		...
		// Insertions only exist on nonterminal rules because they can only be built from binary rules. This might iIf we enable terminal symbols to be in a RHS with another terminal or nonterminal symbol (or multiple).
			...
			if (ruleProps.insertionIdx === 1) {
				// Text in first index does not need conjugation.
				newPath.text = prevPath.text
	...
	// Terminal rule.
		...
		// Terminal rule. Any semantic on a terminal rule is completely reduced (i.e., RHS).
...
/**
 * Appends a new terminal rule's semantic to previous path's semantic list, and then reduces the path's semantic tree.
 *
