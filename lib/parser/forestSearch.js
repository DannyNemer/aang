...
function conjugateText(path, text) {
	...
	} else if (textConstructor === Array) {
		// Insertions: conjugate text within an array of text strings and text objects.
		// Includes a prepended space.
		return conjugateTextArray(path, text)
	} else {
		// No conjugation.
		return ' ' + text
	}
...
function conjugateTextArray(path, textArray) {
	...
	for (var t = 0, textArrayLen = textArray.length; t < textArrayLen; ++t) {
		...
		if (text.constructor === Object) {
			concatStr += ' ' + conjugateTextObject(path, text)
		} else {
			concatStr += ' ' + text
		}
	}
