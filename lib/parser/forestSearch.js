		...
		for (var s = 0, subs = node.subs, subsLen = subs.length; s < subsLen; ++s) {
			...
			if (ruleProps.constructor === Array) {
				for (var r = 0, rulePropsLen = ruleProps.length; r < rulePropsLen; ++r) {
					var newItem = createItem(item, sub, ruleProps[r])
				...
				var newItem = createItem(item, sub, ruleProps)
...
/**
 * Creates a new item by expanding `prevItem` with `sub`.
 *
 * @param {Object} prevItem The previous path item from which to expand.
 * @param {Object} sub The sub node that follows the `prevItem` path.
 * @param {Object} ruleProps The rule properties of `sub`.
 * @returns {Object|number} Returns the new path item if semantically legal, else `-1`.
 */
function createItem(prevItem, sub, ruleProps) {
	...
	var newItem = {
		...
		// `prevItemnextNodes.minCost` is the total heuristic cost of incomplete branches from previous binary rules.
 ...
 *
 * @param {Object} item The item containing the grammatical properties, `item.gramProps`.
...
/**
 * Conjugates `textObj` according to the grammatical properties in `item.gramProps`. Finds the most recent item in the reverse linked list `item.gramProps` that calls for a grammatical property in `textObj`. Removes the `gramProps` item from the list after conjugation. Does not allow for use of the same property in multiple places.
 *
 * @param {Object} item The item containing the grammatical properties, `item.gramProps`.
...
/**
 * Removes the element `gramPropsToRemove` from the linked list `item.gramProps`. The `gramProps` that will be used may not be the most recent, which requires the list to be rebuilt up to the `gramProps` used because the list elements are shared amongst paths.
 *
 * It is better to construct new portions of the linked list here, after finding the gramProps, instead of while traversing, because the list might not need splicing; e.g., a `gramCase` conjugation for a `gramProps` item that also contains a `personNumber`.
 *
 * @param {Object} item The item containing the reverse linked list, `item.gramProps`, to modify.
 * @param {Object} gramPropsToRemove The object to remove from `item.gramProps`.
 */
