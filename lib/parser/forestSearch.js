...
module.exports = function (startNode, k, buildDebugTrees, printStats) {
	...
	heap.push({
		...
		node: startNode,
		// The reverse linked list of the second nodes of previously incomplete binary rules and yet-to-be-conjugated text objects from insertion rules. When `node` is undefined after reaching a terminal symbol, inspect `nextNodes` to complete the binary rules and conjugate the text objects.
		nextNodes: undefined,
	...
	while (heap.content.length > 0) {
		...
		if (!node) {
			...
			while (nextNodes) {
				...
				// Append text from insertions of the second of two RHS nodes to the path's text, performing any necessary conjugation. Conjugation occurs in the nominative case, which relies on the person-number of the first branch (verb precedes subject).
...
/**
 * Appends a terminal rule's RHS semantics, if any, to the previous path's semantic list, and then reduces the path's semantic tree up to the first incompletely parsed node.
 *
 ...
 *
 * `text` can be a `string` not needing conjugation, an `Object` containing a term's inflected forms, or an `Array` of text strings and yet-to-be-conjugated text objects.
 *
 * If `text` is an `Array`, then it is being conjugated after completing an insertion rule's parse that determines the person-number required for the nominative case in the second (inserted) branch (verb precedes subject).
 *
