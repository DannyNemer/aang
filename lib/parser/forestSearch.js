...
module.exports = function (startNode, k, buildTrees, printStats) {
	...
	heap.push({
		...
		node: startNode,
		// The reverse linked list of the second nodes of previously incomplete binary rules and yet-to-conjugate text objects from insertion rules. When `node` is undefined after reaching a terminal symbol, inspect `nextItemList` to complete the binary rules and conjugate the text objects.
	...
	while (heap.content.length > 0) {
		...
		if (!node) {
			...
			if (!nextItemList) {
				...
				node = nextItemList.node
				path.nextItemList = nextItemList.prev
