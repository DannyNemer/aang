...
module.exports = function (startNode, k, buildDebugTrees, printStats) {
	...
	heap.push({
		...
		nextNodes: undefined,
		// The number of elements in `nextNodes` excluding yet-to-conjugate text. Marks when a LHS semantic is visited, to determine when the semantic can be reduced after completely reducing its rule's RHS.
		nextNodesCount: 0,
...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		newPath.semantics = appendSemantic(prevPath.semantics, prevPath.nextNodesCount, ruleProps.semantic, ruleProps.semanticIsRHS, ruleProps.insertedSemantic, ruleProps.rhsCanProduceSemantic)
 ...
 * @param {number} prevNextNodesCount The number of elements in the previous path's `nextNodes` excluding yet-to-conjugate text. Used to determine if the rule of a LHS semantic has been completely reduced and the semantic can be reduced.
 ...
 * @param {boolean} [rhsCanProduceSemantic] Specifies whether the new nonterminal rule's RHS symbol can produce a semantic.
 * @returns {Object|number} Returns the semantic linked list if appendage is semantically legal, else `-1`.
 */
function appendSemantic(prevSemantic, prevNextNodesCount, newSemantic, semanticIsRHS, insertedSemantic, rhsCanProduceSemantic) {
	// If `insertedSemantic` exists, then it is a RHS semantic and `newSemantic` also exists and is a LHS semantic.
	if (insertedSemantic) {
		...
		return {
			...
			prev: {
				...
				nextNodesCount: prevNextNodesCount,
				prev: prevSemantic,
			}
		}
	}
		...
		if (semanticIsRHS) {
			...
			} else if (prevSemantic && !prevSemantic.isRHS && !rhsCanProduceSemantic) {
				// RHS nodes cannot produce a semantic, then the rule's semantic can be reduced with the preceding LHS semantic before parsing the RHS nodes. This enables finding and discarding semantically illegal parses earlier than otherwise.
				return reduceSemanticTree(prevSemantic, prevNextNodesCount, newSemantic)
			} else {
				return {
					semantic: newSemantic,
					isRHS: true,
 ...
 */
function reduceSemanticTree(prevSemantic, prevNextNodesCount, newSemantic) {
	...
	while (prevSemantic) {
		...
		else if (prevNextNodesCount <= prevSemantic.nextNodesCount) {
