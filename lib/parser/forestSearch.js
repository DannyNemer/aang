	...
	while (heap.content.length > 0) {
		...
		if (!node) {
			...
			while (nextNodes) {
				...
				// Conjugate text of insertions of the second of two RHS nodes.
				// Used in nominative case, which relies on person-number in 1st branch (verb precedes subject).
				if (text.constructor === Array) {
					// Conjugate text within an array of text strings and text objects.
...
function createPath(prevPath, sub, ruleProps) {
	...
	if (sub.node.subs) {
		...
		// Insertions only exist on nonterminal rules because they can only be built from binary rules. This might iIf we enable terminal symbols to be in a RHS with another terminal or nonterminal symbol (or multiple).
			...
			if (ruleProps.insertionIdx === 1) {
				...
				if (text.constructor === Array) {
					// Conjugate text within an array of text strings and text objects.
					newPath.text += conjugateTextArray(newPath, text)
				} else if (text.constructor === Object) {
					// Conjugate text.
	...
	// Terminal rule.
		...
		// Save terminal rule's text, otherwise the terminal symbol is a stop word.
		var text = ruleProps.text
		if (text) {
