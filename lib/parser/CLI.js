...
var rl = require('readline').createInterface(process.stdin, process.stdout, function (line) {
	var completions = [ '.test', '.benchmark', '.logTest', '.conjugationTest', '.rebuild', '.repl', '.deleteCache', '.stateTable', '.history', '.help', '.k', '.out', '.trees', '.costs', '.time', '.query', '.stack', '.forest', '.graph' ]
...
function runCommand(input) {
	...
	// Run conjugation tests.
		...
		var origPrintOutput = config.printOutput
		var origPrintTrees = config.printTrees
		config.printOutput = false
		config.printTrees = false
		var failed = false

		testQueries.forEach(function (test) {
			if (test.exactMatch) {
				var query = test.query
				var trees = parse(query, 1)
				var actual = trees && trees[0].text

				if (query !== actual) {
					util.logError('Expected:', util.stylize(query))
					util.log('         Actual:', util.stylize(actual))
					failed = true
				}
			}
		})
		...
		config.printOutput = origPrintOutput
		config.printTrees = origPrintTrees
	}
