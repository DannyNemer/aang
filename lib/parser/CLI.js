...
function runCommand(input) {
	...
	// Output a run of test queries to file.
		...
		util.stdoutToFile('~/Desktop/out', function () {
			runCommand('.test')
		})
	...
	// Print help screen.
		...
		util.log('Commands:')
		util.log('.test             run test queries')
...
function spawnChildProcessAndRedirectStdio(command, args, callback) {
	...
	var child = child_process.spawn(command, args, { stdio: [ 'ignore', process.stdout, process.stderr ] })
