...
var rl = require('readline').createInterface(process.stdin, process.stdout, function (line) {
	var completions = [ '.test', '.logTest', '.conjugationTest', '.rebuild', '.repl', '.deleteCache', '.stateTable', '.history', '.help', '.k', '.out', '.trees', '.costs', '.time', '.query', '.stack', '.forest', '.graph' ]
	...
	util.tryCatchWrapper(function () {
		...
		if (query && !runCommand(query)) {
			spawnChildProcessAndRedirectStdio('node', [
				...
				'--benchmark=' + printTime,
				'--output=' + printOutput,
				'--print-costs=' + printCosts,
				'--print-trees=' + printTrees,
				'--print-stack=' + printStack,
				'--print-forest=' + printForest,
				'--print-forest-graph=' + printForestGraph,
			])
		}
	})
...
function runCommand(input) {
	...
	// Write parse output of test suite to file.
		...
		spawnChildProcessAndRedirectStdio('node', [
			...
			util.log('Test saved:', fs.realpathSync(path))
		})
	}
	...
	// Toggle printing input query.
		...
		util.log('Print input query:', printQuery)
	}
	...
	// Print help screen.
		...
		util.log('Commands:')
		util.log('.test             parse the suite of test queries')
		...
		util.log('.query   print input query:', printQuery)
