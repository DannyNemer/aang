...
/**
 * Converts a reverse linked list `path` output by `forestSearch` when ran with `buildTrees` as `true` (which links each `path` to the previous `path` and its new `ruleProps`) to a graph representation.
 *
 * @private
 * @static
...
function linkedListToGraph(path, options) {
	...
	while (true) {
		...
		// Terminal rule.
		if (!node) {
			var node = path.prev.node

			if (!node) {
				var prevNextItems = path.prev.prev.nextItems

				// Find last node, skipping the insertion text also stored in `nextNodes`.
				while (prevNextItems.text) {
					prevNextItems = prevNextItems.prev
				}
		...
		else if (path.nextItems && path.nextItems !== path.prev.nextItems && path.ruleProps.insertionIdx === undefined) {
			nodesStack.push({
				props: path.ruleProps,
				children: [
					formatNode(node, options, nodesStack.pop(), path.cost),
					formatNode(path.nextItems.node, options, nodesStack.pop())
				],
			})
		}
