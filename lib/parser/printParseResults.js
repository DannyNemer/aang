...
function linkedListToGraph(path, options) {
	...
	while (true) {
		...
		else if (path.nextNodes && path.nextNodes !== path.prev.nextNodes && path.ruleProps.insertionIdx === undefined) {
			nodesStack.push({
				props: path.ruleProps,
				children: [
					formatNode(node, options, nodesStack.pop(), path.cost),
					formatNode(path.nextNodes.node, options, nodesStack.pop())
				],
			})
		}
