...
/**
 * Creates an object holding all of the `rule`'s properties needing for forest-search but not needed for parsing. Creates a single `gramProps` object for all grammatical conjugation properties.
 *
...
function createRuleProps(rule) {
	...
	if (rule.verbForm !== undefined) {
		ruleProps.gramProps = {
			verbForm: rule.verbForm,
		}
	}

	if (rule.personNumber !== undefined) {
		if (ruleProps.gramProps) {
			ruleProps.gramProps.personNumber = rule.personNumber
		} else {
			ruleProps.gramProps = {
	...
	if (rule.gramCase !== undefined) {
		if (ruleProps.gramProps) {
			ruleProps.gramProps.gramCase = rule.gramCase
		} else {
			ruleProps.gramProps = {
				gramCase: rule.gramCase,
			}
		}
	}
