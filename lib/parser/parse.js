 ...
 *
 * Commands:
 *   benchmark  Benchmarks the duration of parsing the test suite.
 *   test       Parses the entire suite of test queries.
 *
 * <query> options:
...
if (command === 'benchmark') {
	yargs
		.usage(
			...
			util.format('Description: %s For each parse, finds the 50-best parses and does not output anything.', commandShortDescriptions[command])
		)
		.updateStrings({
...
// Parses the entire suite of test queries.
else if (command === 'test') {
	yargs
		.usage(
			util.format('Usage: node $0 %s [options]\n\n', command) +
			util.format('Description: %s', commandShortDescriptions[command])
		)
		.updateStrings({
		...
		.example(util.format('node $0 %s -k=30 -cb', command), 'Parses all of the test queries in the test suite, and includes the parse tree costs and time in the parse results.')
		.check(function (argv, options) {
			if (isNaN(argv.k)) {
...
// No command. Parses the input query.
	...
	yargs
		...
		.command('benchmark', commandShortDescriptions.benchmark)
		.command('test', commandShortDescriptions.test)
		.options({
			'k': {
...
var stateTable = require('./buildStateTable.js')('../aang.json', './StateTable.js')

// Benchmark the duration of parsing the test suite.
if (command === 'benchmark') {
	var testQueries = require('./testQueries.js').map(function (test) {
		return test.query
	})
...
// Parse the entire suite of test queries.
else if (command === 'test') {
	var testQueries = require('./testQueries.js').map(function (test) {
		return test.query
	})
	...
	// Cycle through every test query.
	for (var q = 0; q < testQueriesLen; ++q) {
		var testQuery = testQueries[q]

		// Print the query above the results.
		util.log((q ? '\n' : '') + 'query:', testQuery)
...
/**
 * Constructs a parse forest for `query`, and if it reaches the start symbol, then finds the k-best parse trees as well as their semantics and display texts
 *
