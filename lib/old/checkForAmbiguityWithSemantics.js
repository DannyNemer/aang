			...
			if (rule.insertionIdx === undefined && !rule.isTransposition) {
				...
				if (newSemantic) {
					if (rule.semanticIsRHS) {
						newPath.semantics.push(newSemantic)
					} else {
						newPath.semantics.push({ semantic: newSemantic, nextSymsLength: newPath.nextSymsLength })
					}
				}
		...
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rule.insertionIdx === undefined && !rule.isTransposition) {
				...
				var newPath = {
					...
					semantics: undefined,
					nextSymsLength: lastPath.nextSymsLength,
					text: lastPath.text,
				}
				...
				if (rule.isTerminal) {
					var skip = false
					for (var p = lastPath.semantics.length - 1; p > -1; --p) {
						var prevSemantic = lastPath.semantics[p]
						...
						else if (lastPath.nextSymsLength <= prevSemantic.nextSymsLength) {
					...
					if (newSemantic) {
						newPath.semantics = lastPath.semantics.slice(0, p + 1)
						newPath.semantics.push(newSemantic)
					} else {
						newPath.semantics = lastPath.semantics
					}
					...
					if (newSemantic) {
						...
						if (rule.semanticIsRHS) {
							...
							newPath.semantics = lastPath.semantics.slice()
							newPath.semantics.push(newSemantic)
						} else {
							// Discard if prevSemantic is RHS, is identical to newSemantic, and dups of the semantic are prevented
							var prevSemantic = lastPath.semantics[lastPath.semantics.length - 1]
							if (prevSemantic && prevSemantic.constructor === Array && semantic.isForbiddenMultiple(prevSemantic, newSemantic)) {
								continue
							}

							newPath.semantics = lastPath.semantics.slice()
							newPath.semantics.push({ semantic: newSemantic, nextSymsLength: lastPath.nextSymsLength })
					}
				} else {
						newPath.semantics = lastPath.semantics
					}
						...
						for (var o = 0; o < pathsBLen; ++o) {
							...
							if (pathB.nextSym === nextSym && listsEqual(pathB.nextSyms, nextSyms)) {
								...
								if (!options.findAll || !pairExists(ambigPairs, treeA, treeB)) {
									...
									var pathASemantics = completeSemantics(pathA.semantics)
									var pathBSemantics = completeSemantics(pathB.semantics)
									// util.dir(pathASemantics, pathBSemantics)
									util.log(pathA.text)
									util.log(pathB.text)
									util.log(pathASemantics ? semantic.toString(pathASemantics) : pathASemantics)
									util.log(pathBSemantics ? semantic.toString(pathBSemantics) : pathBSemantics)
...
function completeSemantics(semantics) {
	var semanticsLen = semantics.length
	if (semanticsLen === 0) return

	newSemantic = semantics[semanticsLen - 1]
	if (newSemantic.constructor !== Array) newSemantic = newSemantic.semantic

	for (var p = semanticsLen - 1; p > -1; --p) {
		var prevSemantic = semantics[p]
