...
module.exports = function (startNode, k, buildTrees, printStats, printAmbiguity) {
	...
	heap.push({
		...
		cost: 0,
		// The index of the next sub node to expand in `curNode.subs`.
		subIndex: 0,
	})
	...
	while (heap.content.length > 0) {
		...
		// Prepare this path to expand again by the next cheapest sub node.
		var nextSub = subs[index + 1]
		if (nextSub) {
			path.subIndex++

			// Determine the path's new `minCost` without the previously cheapest sub node.
...
function createPath(prevPath, sub, ruleProps) {
	...
	var newPath = {
		...
		// The index of the next sub node to expand in `newPath.curNode.subs`.
		subIndex: 0,
	}
