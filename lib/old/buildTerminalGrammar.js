...
/**
 * Creates rules that produce the tokens in `terminalSymbol`, and adds them to the terminal grammar, invoking `createTerminalRuleSetFunc` for each token set. `createTerminalRuleSetFunc` is invoked with one argument, `tokenSet`, and returns a `Symbol` which produces the rules in `tokenSet`.
 *
 ...
 */
function addSymbol(terminalSymbol, createTerminalRuleSetFunc) {
	// Exit if `terminalSymbol` is not unique.
	...
	forEachTokenSet(terminalSymbol, function (tokenSet) {
		...
		var newSymbol = createTerminalRuleSetFunc(tokenSet)
...
/**
 * Checks if start rule exists for `terminalSymbol`.
 *
 * @param {string} terminalSymbol The terminal symbol to check.
 * @returns {boolean} Returns `true` if a start rule exists for `terminalSymbol`, else `false`.
 */
