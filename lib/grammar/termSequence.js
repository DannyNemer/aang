...
function isIllFormedTermSequencePair(termSeqPair, isVerb, isNestedTermSequence) {
	...
	if (isVerb) {
		// Check if `termSeqPair` contains a term of the specified term sequence type. If `term` is a verb with grammatical tense (i.e., only produces verb forms for the specified tense), its compatability is checked like all other term sequence types because `verb-present` and `verb-past` are distinct types.
