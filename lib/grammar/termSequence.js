...
function newPastVerb(options) {
	...
	// `terminalRuleSetMethods.newVerb(options)` will extend `newVerb` with the properties `isTermSet` and `verbTense`, and will redfine `termSequenceType` as 'verb'.
...
var termSequenceSchema = {
	symbolName: { type: String, required: true },
	insertionCost: Number,
	type: { values: [ 'verb', 'pronoun', 'invariable' ], required: true },
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		 ...
		 */
			...
			if (isIllFormedTermSequenceItem(term, options.type, options.verbTense)) {
				throw new Error('Ill-formed term sequence')
			}
...
function termSequencePairToSymbol(options) {
	if (util.illFormedOpts(termSequencePairSchema, options, true)) {
	 ...
	 * • If `verbTense` is defined, checks every verb `termSeqPair` produces has matching grammatical tense.
	 */
	if (isIllFormedTermSequencePair(termSeqPair, options.type === 'verb', options.verbTense, options.isNestedTermSequence)) {
		throw new Error('Ill-formed term sequence pair')
	}
 ...
 */
function isIllFormedTermSequenceItem(term, type, verbTense) {
	if (!term.isTermSequence) {
		util.logErrorAndPath('Nonterminal symbol provided to term sequence is not a term sequence:', term)
		return true
	}

	if (verbTense && type !== 'verb') {
		util.logErrorAndPath('`verbTense` provided when `type` is not \'verb\':', util.stylize(verbTense))
		return true
	}
...
/**
 * Checks if `term` is either a verb terminal rule set (created by `terminalRuleSetMethods.newVerb()`) or a verb term sequence, which the calling function forbids. If so, prints an error message.
 *
