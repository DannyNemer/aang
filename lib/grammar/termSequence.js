...
function isIllFormedTermSequencePair(termSeqPair, termSequenceType, isNestedTermSequence, isSubstitution) {
	...
	// Check `termSeqPair` does not contain a term sequence that is neither of type `termSequenceType` nor invariable (i.e., neutral).
	for (var t = 0; t < termSeqLen; ++t) {
	...
	/**
	 * If `termSeqPair` is the base term sequence pair passed to `termSequence.newTermSequence()` or `termSequence.newTermSequenceBinarySymbol()` (i.e., is not a nested pair), check it is of type termSequenceType`.
	 *
	 * Only check the base pair, not the nested pairs, because `termSequenceType` only specifies if the entire base sequence contains one term of type `termSequenceType` (e.g., 'verb') and all others (at whatever nesting level) at of type 'invariable', not if every bigram within the sequence has a term of type `termSequenceType`.
	 * • For example, do not check if the bigram "in common" within the verb sequence "have in common" contains a verb. Hence, `termSequenceType` is only used when `termSeqPair` is the base pair.
	 */
