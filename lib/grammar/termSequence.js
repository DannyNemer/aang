		...
		options.substitutedTerms.forEach(function (term) {
			 ...
			 */
				...
				if (!exports.isTermSequenceType(term, options.type)) {
					// Allow invariable sequences as substitutions for pronoun sequences, and specify as such in error messages.
					...
					// Allow pronoun sequences as substitutions for invariable sequences, but do not specify in error messages because of its rarity. For example:
					//   `[1-sg-poss-det]` -> `[1-sg]`, text: "my"
					else if (options.type === exports.termTypes.INVARIABLE) {
						if (!exports.isTermSequenceType(term, exports.termTypes.PRONOUN)) {
							util.logErrorAndPath('Nested term sequence substitution', util.stylize(term.name), 'is not of type', util.stylize(options.type) + ':', options)
							throw new Error('Ill-formed term sequence substitution')
						}
					}

					// 'verb', 'verb-past', 'verb-present' must contain term sequences of matching type as substitutions.
					else {
						util.logErrorAndPath('Nested term sequence substitution', util.stylize(term.name), 'is not of type', util.stylize(options.type) + ':', options)
						throw new Error('Ill-formed term sequence substitution')
					}
