...
exports.newPronoun = function (options) {
	...
	return pronounSym._toTermSequence({
		isTermSet: true,
		type: exports.termTypes.PRONOUN,
		defaultText: pronounTextForms,
 ...
 * 2. A terminal rule set created by `verbTermSet.newVerb()` or `termSequence.newPronoun()`.
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		/**
		 * If `term` is a term sequence created by `termSequence.newTermSequence()` or `termSequence.newTermSequenceBinarySymbol()` (or a terminal rule set created by `verbTermSet.newVerb()` or `termSequence.newPronoun()`).
		 *
	...
	// Extend `termSeqSym` with term sequence properties. Prevents addition of further rules to `termSeqSym`, and enables nesting of `termSeqSym` in other term sequences of matching `termSeqSym.termSequenceType`.
 ...
 * 1. A terminal rule set created by `verbTermSet.newVerb()` or `termSequence.newPronoun()`.
