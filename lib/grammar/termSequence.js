...
function isIllFormedTermSequenceItem(term, type) {
	if (!term.isTermSequence) {
		util.logErrorAndPath('Non-term sequence nonterminal symbol', util.stylize(term.name), 'used in new term sequence:', term)
		return true
	}

	if (term.termSequenceType !== type) {
		util.logErrorAndPath('Nested term sequence', util.stylize(term.name), 'is not of type', type + ':', term)
		return true
	}
