...
function isIllFormedTermSequencePair(termSeqPair, termSequenceType, isNestedTermSequence) {
	...
	for (var t = 0; t < 2; ++t) {
		...
		if (termSequenceType === 'invariable') {
			if (term.termSequenceType !== termSequenceType) {
				util.logErrorAndPath('Term in provided term sequence pair not of type', util.stylize(termSequenceType) + ':', term, termSeqPair)
				return true
			}
		} else {
			if (term.termSequenceType !== 'invariable') {
				util.logErrorAndPath('Term in provided term sequence pair neither of type', util.stylize('invariable'), ' or the specified type', util.stylize(termSequenceType) + ':', term, termSeqPair)
				return true
			}
		}
	}
