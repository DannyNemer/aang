 ...
 *
 * If `options.tense` is `undefined`, each rule in the set has an object as its `text` value with the properties `oneSg`, `threeSg`, `pl`, and `past` for the different verb forms. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct verb form (i.e., display text) according to grammatical properties in the same tree:
 * • The person-number forms, `oneSg`, `threeSg`, and `pl`, are conjugated by the grammatical `personNumber` property in preceding nominative rules.
 * • The tense form, `past`, is conjugated by the `grammaticalForm` property in the (immediate) parent nonterminal rule. Also, if the parent rule has `acceptedTense` defined as 'past', it accepts the `past` form when input, but does not conjugate to it otherwise (unlike `grammaticalForm`).
 *
 * If `options.tense` is 'present', each rule in the set has an object as its `text` value just as when `options.tense` is `undefined`, but there is not `past` value on the `text` objects and there is no `grammaticalForm` conjugation.
 *
...
exports.newVerb = function (options) {
	 ...
	 *
	 * Else if `options.tense` is 'past', create a terminal rule set with `termSequence.newTermSequence()`, where each rule has the string `options.verbFormsTermSet.past` has its `text` value.
	 */
...
function baseNewVerb(options) {
	 ...
	 *
	 * For use when nesting `verbSym` in the first accepted rule of a term sequence with `termSequence.newTermSequence()`. If so, that new term sequence uses `verbSym.defaultText` in its default text, which it uses as the display text for its substitution rules, if any.
	 */
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		 ...
		 */
			...
			var termSeqPair = termSequencePairToSymbol(term, options.type)
		...
		options.substitutedTerms.forEach(function (term) {
			 ...
			 */
				...
				if (!exports.isTermSequenceType(term, options.type)) {
					/**
					 * PRONOUN: Allow invariable sequences as substitutions for pronoun sequences, and specify as such in error messages. For example:
					...
					if (options.type === exports.termTypes.PRONOUN) {
						if (!exports.isTermSequenceType(term, exports.termTypes.INVARIABLE)) {
							util.logErrorAndPath('Nested substituted term sequence', util.stylize(term.name), 'for', util.stylize(options.type), 'sequence is neither of', util.stylize(options.type), 'nor', util.stylize(exports.termTypes.INVARIABLE), 'type:', options)
							throw new Error('Ill-formed substituted term sequence')
						}
					}

					/**
					 * INVARIABLE: Allow pronoun sequences as substitutions for invariable sequences, but do not specify in error messages because of its rarity. For example:
					 *   `[1-sg-poss-det]` -> `[1-sg]`, text: "my"
					 *
					 * VERB, VERB_PAST, VERB_PRESENT: Must contain term sequences of matching types for substitutions.
					 */
					else if (options.type !== exports.termTypes.INVARIABLE || !exports.isTermSequenceType(term, exports.termTypes.PRONOUN)) {
						util.logErrorAndPath('Nested substituted term sequence', util.stylize(term.name), 'for', util.stylize(options.type), 'sequence is not of', util.stylize(options.type), 'type:', options)
						throw new Error('Ill-formed substituted term sequence')
					}
				}
...
exports.newTermSequenceBinarySymbol = function (options, _isNestedTermSequence) {
	 ...
	 */
	var termSeqPair = termSequencePairToSymbol(options.termPair, options.type, _isNestedTermSequence)
 ...
 */
function termSequencePairToSymbol(termSeqPair, termSequenceType, isNestedTermSequence) {
...
function isIllFormedTermSequencePair(termSeqPair, termSequenceType, isNestedTermSequence) {
	...
	if (termSeqPair.some(term => term.termSequenceType !== exports.termTypes.INVARIABLE && term.termSequenceType !== termSequenceType)) {
		util.logErrorAndPath('Term sequence pair of', util.stylize(termSequenceType), 'type contains term sequence of incorrect type:', termSeqPair)
		return true
	}
...
/**
 * Checks `term` is a term sequence or terminal rule set with term sequence type, `type`.
 *
 * @static
 * @memberOf termSequence
