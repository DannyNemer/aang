 ...
 * @property {number[]} [noInsertionIndexes] If `term` is a sequence pair (array), the index(es) of its nested term sequence(s) for which to forbid insertion rules.
 */
	...
	if (options.substitutedTerms) {
		...
		options.substitutedTerms.forEach(function (term) {
			...
			if (term.constructor === Object) {
				...
				if (term.noInsertionIndexes) {
					if (term.term.constructor !== Array) {
						util.logErrorAndPath('`noInsertionIndexes` used with a', util.colors.cyan(term.term.constructor.name), 'instead of a term sequence pair:', term)
						throw new Error('Ill-formed substituted term sequence')
					}

					if (term.noInsertionIndexes.some(idx => term.term[idx] === undefined)) {
						util.logErrorAndPath('`noInsertionIndexes` contains an out-of-bounds term sequence pair index:', term)
						throw new Error('Ill-formed substituted term sequence')
					}
				}
