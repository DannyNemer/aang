 ...
 *
 * Uses `termSequence.newTermSequence()` to create the invariable terminal rule set, where each rule has the string `options.verbFormsTermSet.past` as its `text` and 'past' as its `tense`. `pfsearch` can not conjugate these rules.
 *
 * Note: Each verb form in `options.verbFormsTermSet` becomes a terminal symbol and can not contain whitespace.
 *
 * For use by `termSequence.newVerb(options)` to create the past tense verb terminal rule set when `options.splitByTense` is `true`.
 *
 * The returned `NSymbol` has the following properties, which `termSequence.newVerb()` extends:
 * 1. name - `options.symbolName`.
...
function baseNewPastVerb(options) {
	...
	var pastVerbSym = g.newTermSequence(pastVerbTermSet)

	// Mark every terminal rule `tense` as 'past'.
	pastVerbSym.rules.forEach(function (rule) {
		rule.tense = 'past'
	})
