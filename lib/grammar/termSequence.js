...
function newPastVerb(options) {
	...
	// `termSequence.newVerb(options)` will extend `newVerb` with the properties `isTermSet` and `verbTense`, and will redfine `termSequenceType` as 'verb'.
...
function termSequencePairToSymbol(options) {
	if (util.illFormedOpts(termSequencePairSchema, options, true) || isIllFormedTermSequencePairOptions(options)) {
		throw new Error('Ill-formed term sequence pair')
	}

	var termSeqPair = options.termSeqPair
	for (var s = 0, termSeqLen = termSeqPair.length; s < termSeqLen; ++s) {
...
/**
 * Checks if `options`, which was passed to `termSequencePairToSymbol()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} options The `termSequencePairToSymbol()` options object to inspect.
 * @returns {boolean} Returns `true` if `options` is ill-formed, else `false`.
 */
function isIllFormedTermSequencePairOptions(options) {
	var termSeqPair = options.termSeqPair
