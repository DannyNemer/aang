 ...
 *
 * If `_isNestedTermSequence` is `true`, manually checks if `options.termPair` produces a verb, and the `verbTense` of that verb, to determine the `isVerb` and `verbTense` values for the returned `NSymbol`. For use when `options.termPair` was defined as a nested term sequence pair within another term sequence pair passed to this method or `termSequence.newTermSequence()`. `options.isVerb` only specifies if the entire base sequence contains or lacks a single verb, not every bigram within the sequence; e.g., do not check if "in common" within the verb phrase "have in common" contains a verb. Likewise, `options.verbTense` only specifies the grammatical tense of the entire base sequence.
 *
 * The returned `NSymbol` has the following properties:
...
exports.newTermSequenceBinarySymbol = function (options, _isSubstitution, _isNestedTermSequence) {
	 ...
	 *
	 * Pass `_isSubstitution` to specify `options.termPair` is a substituted term sequence pair with the substituted term sequence type exceptions specified in the `termSequence.newTermSequence()` description.
	 *
	 ...
	 * • If `true`, does not check if `options.termPair` contains a sequence of type `options.type` because `options.type` only specifies if the entire base sequence contains a single term of type `options.type`, not every bigram within the sequence. For example, do not check if the bigram "in common" within the verb sequence "have in common" contains a verb.
	 */
	...
	if (_isNestedTermSequence) {
		/**
		 * Manually check whether `termSeqSym` produces a verb, and the `verbTense` of that verb, when `options.termPair` was defined as a nested term sequence pair within another term sequence pair passed to this method or `termSequence.newTermSequence()`. This is necessary because `options.isVerb` only specifies if the entire base sequence (after flattening) contains or lacks a single verb, not every bigram within the sequence. Likewise, `options.verbTense` only specifies the grammatical tense of the entire base sequence.
		 *
		...
		// Define the `termSeqSym` term sequence type, which specifies every term sequence `termSeqSym` produces contains a single term of type `options.termSequenceType` and all other terms within the sequence are of type 'invariable'.
 ...
 * @private
 * @static
 * @param {NSymbol[]} termSeqPair The term sequence ordered pair to validate and flatten.
...
function flattenNestedTermSequencePairs(termSeqPair, termSequenceType, isSubstitution, isNestedTermSequence) {
	...
	for (var t = 0; t < termSeqLen; ++t) {
		...
		if (termSym.constructor === Array) {
			 ...
			 * • This is necessary because `termSequenceType` only specifies if the entire base sequence contains a single term of type `termSequenceType` (e.g., 'verb') and all others (at whatever nesting level) are of type 'invariable', not every bigram within the sequence (i.e., recursive invocations of this function). For example, do not check if the bigram "in common" within the verb sequence "have in common" contains a verb.
			 */
 ...
 * • For example, do not check if the bigram "in common" within the verb sequence "have in common" contains a verb.
 *
 * @private
 * @static
