...
exports.newTermSequence = function (options) {
	...
	// If `options.verbTense` is defined, specify every verb `termSeqSym` produces only contains verb forms for this grammatical tense (identified by `NSymbol` property `verbTense`); i.e., lacks terminal rules for the opposite tense.
...
exports.newTermSequenceBinarySymbol = function (options, _isNestedTermSequence) {
	if (util.illFormedOpts(binaryTermSequenceSchema, options) || isIllFormedNewTermSequenceBinarySymbolOptions(options)) {
...
/**
 * Checks if `options`, which was passed to `terminalRuleSetMethods.newTermSequenceBinarySymbol()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} options The `terminalRuleSetMethods.newTermSequenceBinarySymbol()` options object to inspect.
 * @returns {boolean} Returns `true` if `options` is ill-formed, else `false`.
 */
function isIllFormedNewTermSequenceBinarySymbolOptions(options) {
	if (options.verbTense && options.type !== 'verb') {
		util.logErrorAndPath('`verbTense` specified for term sequence without `tense` defined as \'verb\'`:', options)
		return true
	}
