...
function isIllFormedTermSequencePair(termSeqPair, termSequenceType, isNestedTermSequence) {
	...
	if (!isNestedTermSequence) {
		...
		// Check both term sequences in `termSeqPair` are not of type `termSequenceType` unless invariable.
	...
	for (var t = 0; t < 2; ++t) {
		...
		if (term.termSequenceType !== exports.termTypes.INVARIABLE) {
			if (termSequenceType === exports.termTypes.INVARIABLE) {
				...
				util.logErrorAndPath('Term in provided term sequence pair neither of (neutral) type', util.stylize(exports.termTypes.INVARIABLE), ' nor the specified type', util.stylize(termSequenceType) + ':', term, termSeqPair)
				return true
			}
		}
		...
		// Unsure if `isNestedTermSequence` is needed if we just specify `exports.termTypes.INVARIABLE`.
		if (!isNestedTermSequence && term.termSequenceType !== exports.termTypes.INVARIABLE) {
...
exports.isVerbTerm = function (term, tense) {
	...
	if (!(term.isTermSequence && term.termSequenceType === exports.termTypes.VERB)) {
		return false
	}
...
/**
 * Checks `term` is a term sequence with term sequence type, `type`.
 *
 * @static
 * @memberOf termSequence
