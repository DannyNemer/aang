...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		 ...
		 */
			 ...
			 *
			 * Ensure the resulting term term sequence pair produces only one terminal rule set of type `options.type` and all other term sequences are 'invariable'.
			 */
	...
	if (options.substitutedTerms) {
		...
		options.substitutedTerms.forEach(function (term) {
			 ...
			 */
				 ...
				 * Ensure the resulting term term sequence pair produces only one terminal rule set of type `options.type` and all other term sequences are 'invariable'.
				 *
				 * Pass `true` as third argument to specify `term` is a substituted term sequence pair, which has the same substitution exceptions as the substituted term sequences above (e.g., invariable term sequence pairs a pronoun substitutions).
				 */
