 ...
 * • The tense form, `past`, is conjugated by the `grammaticalForm` property in the (immediate) parent nonterminal rule. Also, if the parent rule has `acceptedTense` defined as 'past', it accepts the `past` form when input, but does not conjugate to it otherwise (unlike `grammaticalForm`).
 *
 * If `options.splitByTense` is `true`, creates two terminal rule sets from the verb forms in `options.verbFormsTermSet`, split by present and past grammatical tense. Each terminal rule set only has terminal symbols and display text for the specified tense. Returns the sets in an object of the form `TenseVerbSet`. For use with `Category.prototype.addTenseVerbRuleSet()` where tense is semantically meaningful, and input in one tense should never be substituted by the other.
 * • For `TenseVerbSet.present`, each rule in the set has an object as its `text` just as when `options.splitByTense` is falsey, but there is no `past` value on the `text` objects and there is no `grammaticalForm` conjugation.
 *   - The terminal rule set's `NSymbol` `name` is `options.symbolName` with the suffix '-present'.
 * • For ``TenseVerbSet.past`, each rule in this set is an invariable term with the  `options.verbFormsTermSet.past` string as its `text`. `pfsearch` can not conjugate these rules.
 *   - The terminal rule set's `NSymbol` `name` is `options.symbolName` with the suffix '-present'.
 *
 ...
 * • termSequenceType - 'verb' if `options.splitByTense` is falsey, else 'verb-present' and 'verb-past' for `TenseVerbSet.present` and `TenseVerbSet.past`, respectively.
 * • defaultText - If `options.splitByTense` is falsey, the conjugative `text` object for the verb forms. Else, the conjugative `text` object for the present verb forms `TenseVerbSet.present` and the invariable `text` string for the past verb form for `TenseVerbSet.past`.
...
exports.newVerb = function (options) {
	...
	if (options.splitByTense) {
		...
		// Return both symbols as a `TenseVerbSet`.
		return {
