...
exports.newVerb = function (options) {
	...
	// Assign the following `NSymbol` properties after adding all all `verbSym` rules above because `NSymbol.prototype.addRule()` prevents use on `NSymbol` with these properties.
...
exports.newPronoun = function (options) {
	...
	// Assign the following `NSymbol` properties after adding all all `pronounSym` rules above because `NSymbol.prototype.addRule()` prevents use on `NSymbol` with these properties.
...
function isIllFormedNewPronounOptions(options) {
	if (util.illFormedOpts(pronounFormsTermSetSchema, options.pronounFormsTermSet)) {
		return true
	}
