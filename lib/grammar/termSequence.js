 ...
 *
 * These methods create an `NSymbol` for the rule sets, as opposed to adding rules to an existing `NSymbol`, and forbid adding rules to the new `NSymbol` afterward to prevent external changes to the rule sets.
 */
...
/**
 * The present tense inflections of a verb, from which `termSequence.newVerb(options)` with `options.splitByTense` as `true`, creates the present tense verb terminal rule set.
 *
...
/**
 * The past tense inflections of a verb, from which `termSequence.newVerb(options)` with `options.splitByTense` as `true`, creates the past tense verb terminal rule set.
 *
...
/**
 * The object `termSequence.newVerb(options)` returns when invoked with `option.splitByTense` as `true`, which splits `options.verbFormsTermSet` into separate terminal rule sets for present and past grammatical tense.
 *
 * @typedef {Object} TenseVerbSet
 * @property {NSymbol} noTense The nonterminal term sequence of type 'verb' that produces `present` and `past`, with a conjugative text object for all verb forms in `options.verbFormsTermSet` as substitution text.
 ...
 *
 * If `options.splitByTense` is falsey, each rule in the set `options.verbFormsTermSet` has an object as its `text` with the properties `oneSg`, `threeSg`, `pl`, and `past` for the different verb forms. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct verb form (i.e., display text) according to grammatical properties in the same tree:
 * • The person-number forms, `oneSg`, `threeSg`, and `pl`, are conjugated by the grammatical `personNumber`
 *   property in preceding nominative rules.
 ...
 *
 * If `options.splitByTense` is `true`.
 * • Creates two terminal rule sets from the verb forms in `options.verbFormsTermSet`, split by present and
 *   past grammatical tense. Each terminal rule set only has terminal symbols and display text for the
 *   specified tense.
 *   - For `TenseVerbSet.present`, each rule in the set has an object as its `text` just as when
 *     `options.splitByTense` is falsey, but there is no `past` value on the `text` objects. The terminal rule
 *     set's `NSymbol` `name` is `options.symbolName` with the suffix '-present'.
 *   - For `TenseVerbSet.past`, each rule in this set is an invariable term with the
 *     `options.verbFormsTermSet.past` string as its `text`. `pfsearch` can not conjugate these rules. The
 *     terminal rule set's `NSymbol` `name` is `options.symbolName` with the suffix '-present'.
 * • Creates a term sequence that produces the two tense term sets, with a substitution conjugative `text`
 *   object on each nonterminal rule with all verb forms in `options.verbFormsTermSet`. Behaves identically to
 *   terminal rule sets created when `options.splitByTense` is falsey.
 * • Returns the three `NSymbol` instances in an object of the form `TenseVerbSet`.
 * • The terminal rule sets divided by tense are for use `Category.prototype.addTenseVerbRuleSet()` where
 *   tense is semantically meaningful, and input in one tense should never be substituted by the other. The
 *   term sequence parent set is for use in all other verb sets where tense does not influence semantics.
 *
 * Note: `options.splitByTense` and `options.noPastDisplayText` are mutually exclusive.
 *
 * Note: Each verb form in `options.verbFormsTermSet` becomes a terminal symbol and can not contain whitespace.
 *
 * Note: The boolean `options.splitByTense` parametric design is superior to `options.tense` definition of either 'present' or 'past', because the former enforces complete definitions of verbs. If only one tense terminal rule set is needed, the grammar generator deletes the unused set just as all unused grammar rules.
 *
 * The returned `NSymbol` (or, `NSymbol` pair if `options.splitByTense`) has the following properties:
 * • name - `options.symbolName` if defined, else the concatenation of 'verb' and the infinitive verb form, `options.verbFormsTermSet.pl`. If `options.splitByTense`, '-present' and '-past' are appended to this name for the present and past terminal rule sets, respectively.
 ...
 * • termSequenceType - 'verb' if `options.splitByTense` is falsey, else 'verb-present' and 'verb-past' for `TenseVerbSet.present` and `TenseVerbSet.past`, respectively, and 'verb' for `TenseVerbSet.noTense`.
 * • defaultText - If `options.splitByTense` is falsey, the conjugative `text` object for the verb forms. Else, the conjugative `text` object for the present verb forms `TenseVerbSet.present`, the invariable `text` string for the past verb form for `TenseVerbSet.past`, and the conjugative `text` object for the verb forms.
 * • insertionCost - `options.insertionCost`, if defined. If `options.splitByTense`, assigned to `TenseVerbSet.present`.
 *
 * @memberOf termSequence
 * @param {Object} options The options object.
 * @param {string} [options.symbolName] The name for the new `NSymbol`. If omitted, concatenates 'verb' and the infinitive verb form, `options.verbFormsTermSet.pl`.
 * @param {number} [options.insertionCost] The insertion cost for the terminal rule set, assigned to the first rule in the set (i.e., `verbFormsTermSet.oneSg`). Enables the creation of insertion rules using the new `NSymbol` that produces this set (i.e., the LHS symbol). If `options.splitByTense` is `true`, only assigned to `verbFormsTermSet.oneSg` in the present terminal rule set.
 * @param {boolean} [options.splitByTense] Specify splitting `options.verbFormsTermSet` into separate terminal rule sets for present and past grammatical tense.
 * @param {boolean} [options.noPastDisplayText] Specify excluding the past tense verb forms, `options.verbFormsTermSet.past`, from the display `text` object. Instead, substitute the `past` form when input with the verb's correct present tense form. Unlike the present tense set that `options.splitByTense` creates, this option creates terminal rules for the past tense verb forms as substitutions.
 * @param {VerbFormsTermSet} options.verbFormsTermSet The verb terminal symbol set with each verb form inflection.
 * @returns {NSymbol|TenseVerbSet} Returns the new `NSymbol` for the verb terminal rule set if `options.splitByTense` is falsey, else an object containing separate terminal rule sets for present and past tense.
 */
var verbSchema = {
	symbolName: String,
...
exports.newVerb = function (options) {
	...
	// If `options.symbolName` is omitted, generate the terminal rule set `NSymbol` name using the infinitive verb form, `options.verbFormsTermSet.pl`.
	if (!options.symbolName) {
		options.symbolName = g.hyphenate('verb', options.verbFormsTermSet.pl)
	}

	// If `options.splitByTense` is `true`, create two separate terminal rules sets for `options.verbFormsTermSet`, with the verb forms split by present and past tense.
	if (options.splitByTense) {
		...
		// Create a terminal rule set where each rule has the string `options.verbFormsTermSet.past` as its `text`.
		// For now, restrict `insertionCost` to the present tense verb set.
		...
		/**
		 * Create a term sequence that produces the two tense term sets, with identical substitution text on each nonterminal rule with all verb forms in `options.verbFormsTermSet`. Behaves identically to terminal rule sets created when `options.splitByTense` is falsey.
		 *
		 * For use when the same verb is needed in both a tense verb rule set (where tense yield different semantics) and a non-tense verb rule set. It is more efficient to create a parent rule then to duplicate terminal symbols for a separate non-split terminal rule set.
		 */

		// The nonterminal substitution rule conjugative `text` object containing the inflections for every verb form.
		var verbTextForms = {
			oneSg: verbFormsTermSet.oneSg,
			threeSg: verbFormsTermSet.threeSg,
			pl: verbFormsTermSet.pl,
		...
		var noTenseVerbSym = g.newSymbol(options.symbolName).addRule({
			rhs: [ presentVerbSym ],
			text: verbTextForms,
		}).addRule({
			// `pastVerbSym` is created with `tense` as 'past' on all terminal rules, whose values `noTenseVerbSym` can reach just as verb terminal rule sets created when `options.splitByTense` as falsey.
			rhs: [ pastVerbSym ],
			text: verbTextForms,
		})

		noTenseVerbSym.isTermSequence = true
		// Specify `noTenseVerbSym` contains every verb form. For inclusion within a term sequence of matching type.
		noTenseVerbSym.termSequenceType = exports.termTypes.VERB

		/**
		 * Save `verbTextForms`, which is identical for both nonterminal rules `noTenseVerbSym` produces, as the set's default text.
		 *
		 * For use when nesting `noTenseVerbSym` in the first accepted rule of a term sequence created with `termSequence.newTermSequence()`. If so, that new term sequence uses `noTenseVerbSym.defaultText` in its default text, which it uses as the display text for its substitution rules, if any.
		 */
		noTenseVerbSym.defaultText = verbTextForms
 ...
 * Creates an `NSymbol` that produces a terminal rule set for a verb with the necessary text forms for conjugation.
 *
 * If `termSequence.newVerb(options)` was invoked `options.splitByTense` as falsey, each rule in the set `options.verbFormsTermSet` has an object as its `text` with the properties `oneSg`, `threeSg`, `pl`, and `past` for the different verb forms. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct verb form (i.e., display text) according to grammatical properties in the same tree:
 * • The person-number forms, `oneSg`, `threeSg`, and `pl`, are conjugated by the grammatical `personNumber` property in preceding nominative rules.
 * • The tense form, `past`, is conjugated by the `grammaticalForm` property in the (immediate) parent nonterminal rule. Also, if the parent rule has `acceptedTense` defined as 'past', it accepts the `past` form when input, but does not conjugate to it otherwise (unlike `grammaticalForm`).
 *
 * If `termSequence.newVerb(options)` was invoked `options.splitByTense` as `true`, this function creates the terminal rule set for the present tense verb forms in `options.verbFormsTermSet`. Each rule in the set has an object as its `text` just as when `options.splitByTense` is falsey, but there is no `past` value on the `text` objects.
 *
 * Note: `options.splitByTense` and `options.noPastDisplayText` are mutually exclusive.
 *
 * Note: Each verb form in `options.verbFormsTermSet` becomes a terminal symbol and can not contain whitespace.
 ...
 * @param {boolean} [options.noPastDisplayText] Specify excluding the past tense verb forms, `options.verbFormsTermSet.past`, from the display `text` object. Instead, substitute the `past` form when input with the verb's correct present tense form. Unlike the present tense set that `options.splitByTense` creates, this option creates terminal rules for the past tense verb forms as substitutions.
 * @param {VerbFormsTermSet|PresentVerbFormsTermSet} options.verbFormsTermSet The verb terminal symbol set with each verb form inflection. Of type `VerbFormsTermSet` when `termSequence.newVerb()` was invoked with `options.splitByTense` as falsey, else `PresentVerbFormsTermSet` to create the present terminal rule set.
...
function baseNewVerb(options) {
	 ...
	 *
	 * NOTE: `options.noPastDisplayText` is no longer required for `[have]` following a redesign of `gramProps` conjugation, which requires nonterminal rules to specify to which RHS index the grammatical property applies.
	 */
	 ...
	 *
	 * If `termSequence.newVerb()` is invoked with `options.splitByTense` as `true` (which specifies splitting `options.verbFormsTermSet` into separate terminal rule sets for grammatical tense), then `verbSym` is a present verb term set and lacks `verbFormsTermSet.past`.
	 */
...
/**
 * Creates a `NSymbol.prototype.addRule()` options object for `terminalSymbol` with `displayText` as its `text` value.
 *
 * For use by `baseNewVerb()` and `baseNewPastVerb()`.
 * • If `baseNewVerb()`, `displayText` is a conjugative text object for all of the verbs forms, or the verb's present tense form.
 * • If `baseNewPastVerb()`, `displayText` is an invariable string for the past tense verb form.
 *
 * @private
 * @static
 ...
 */
function createVerbTerminalRule(terminalSymbol, displayText, tense) {
	...
	if (tense === 'past') {
		 ...
		 *
		 * If `displayText` is a conjugative text object and this rule is a past-participle form, is matched in input, and the parent rule's `acceptedTense` matches `tense`, `pfsearch` substitutes this symbol for the verb set's simple past form, `displayText.past`.
		 *
 ...
 *
 * For use by `termSequence.newVerb(options)` when `options.splitByTense` is `true`, to create the past tense verb terminal rule set. The set is an invariable, conjugative terminal rule set, where each rule has the string `options.verbFormsTermSet.past` as its `text` and 'past' as its `tense`. `pfsearch` can not conjugate these rules.
 *
 * Note: Each verb form in `options.verbFormsTermSet` becomes a terminal symbol and can not contain whitespace.
 *
 * The returned `NSymbol` has the following properties, which `termSequence.newVerb()` extends:
 ...
 * @param {Object} options The `termSequence.newVerb()` options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
 * @param {number} [options.insertionCost] The insertion cost for the terminal rule set, assigned to the first rule in the set (i.e., `verbFormsTermSet.past`). Enables the creation of insertion rules using the new `NSymbol` that produces this set (i.e., the LHS symbol).
...
function baseNewPastVerb(options) {
	// Create the `NSymbol` that produces the past tense verb terminal rule set.
	// `termSequence.newVerb()` has appended '-past' to `options.symbolName`.
	var pastVerbSym = g.newSymbol(options.symbolName)
	var pastVerbFormsTermSet = options.verbFormsTermSet
	// The invariable, non-conjugative `text` value of all rules in the terminal rule set.
	var defaultText = pastVerbFormsTermSet.past

	// The terminal rule for the simple past tense verb form (or, preterite). E.g., "worked", "lived".
	var simplePastTermRule = createVerbTerminalRule(pastVerbFormsTermSet.past, defaultText, 'past')
	if (options.insertionCost !== undefined) {
		// Assign the insertion cost, if any, to the first terminal rule in the set.
		// Currently unused because `termSequence.newVerb()` only passes `insertionCost` to the present verb terminal rule set that `baseNewVerb()` creates.
		simplePastTermRule.insertionCost = options.insertionCost
	}
	pastVerbSym.addRule(simplePastTermRule)

	// If provided, the terminal rule for the past-participle verb form, substituted when input with `defaultText`. E.g., "been", "done".
	if (pastVerbFormsTermSet.pastParticiple) {
		pastVerbSym.addRule(createVerbTerminalRule(pastVerbFormsTermSet.pastParticiple, defaultText, 'past'))
	}

	/**
	 * Save `defaultText`, which is identical for every terminal rule `pastVerbSym` produces, as the terminal rule set's default text.
	 *
	 * For use when nesting `pastVerbSym` in the first accepted rule of a term sequence created with `termSequence.newTermSequence()`. If so, that new term sequence uses `pastVerbSym.defaultText` in its default text, which it uses as the display text for its substitution rules, if any.
	 */
	pastVerbSym.defaultText = defaultText

	// The calling `termSequence.newVerb(options)` will extend `pastVerbSym` with the property `isTermSet` and will redefine `termSequenceType` as 'verb-past'.
	return pastVerbSym
}
...
function isIllFormedNewVerbOptions(options) {
	// Check `options.splitByTense` and `options.noPastDisplayText` are mutually exclusive.
	if (options.splitByTense && options.noPastDisplayText) {
		util.logErrorAndPath('Verb defines both `splitByTense` and `noPastDisplayText` as `true`:', options)
		return true
	}
 ...
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
 * @param {number} [options.insertionCost] The insertion cost for the term sequence, assigned to the first terminal symbol in `options.acceptedTerms`, if any. Enables the creation of insertion rules using the new `NSymbol` that produces this set (i.e., the LHS symbol). Only permitted if `options.type` is 'invariable'.
