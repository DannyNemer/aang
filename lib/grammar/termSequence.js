...
exports.newVerb = function (options) {
	 ...
	 *
	 * Before `pfsearch`, `flattenTermSequence` flattens term sequences to a single terminal parse node with a single display text. If the parse node associated with a root term sequence node can produce multiple parse trees, it is ambiguous and the cheapest is chosen, though this rarely occurs.
	 *
...
function newPastVerb(options) {
	...
	// `terminalRuleSetMethods.newVerb(options)` will extend `newVerb` with the properties `isTermSet`, `termSequenceType`, and `verbTense`.
