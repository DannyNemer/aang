 ...
 * @param {boolean} [options.noPastDisplayText] Specify excluding the past tense verb forms, `options.verbFormsTermSet.past`, from the display `text` object. Instead, substitute the `past` form when input with the verb's correct present tense form. Unlike the present tense set that `options.splitByTense` creates, this option creates terminal rules for the past tense verb forms as substitutions.
 * @param {VerbFormsTermSet|PresentVerbFormsTermSet} options.verbFormsTermSet The verb terminal rule set with each verb form inflection. Of type `VerbFormsTermSet` when `termSequence.newVerb()` was invoked with `options.splitByTense` as falsey, else `PresentVerbFormsTermSet` to create the present terminal rule set.
