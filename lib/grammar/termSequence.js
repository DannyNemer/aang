...
exports.newTermSequence = function (options) {
	...
	if (options.substitutedTerms) {
		// Create nonterminal substitution rules with `defaultText` as the `text` value for each rule. This instructs `pfsearch` to generate display text from these rules and discard the `text` values these rules produce.
		options.substitutedTerms.forEach(function (term) {
			 ...
			 */
				...
				util.logError('Substituted term is neither a terminal symbol, terminal rule set, term sequence, nor an ordered pair of term sequences:', term, options)
				throw new Error('Ill-formed substituted term sequence')
			}
		})
	}
