...
/**
 * The inflections of a verb, from which `terminalRuleSetMethods.newVerb()` creates a verb terminal rule set.
 *
...
/**
 * The present tense inflections of a verb, from which `terminalRuleSetMethods.newVerb(options)` with `options.tense` as 'present', creates a present tense verb terminal rule set.
 *
...
/**
 * The past tense inflections of a verb, from which `terminalRuleSetMethods.newVerb(options)` with `options.tense` as 'past', creates a past tense verb terminal rule set.
 *
 ...
 * • `isTermSet` - `true`. Prevents adding more rules to this `NSymbol`.
 * • `termSetType` - 'verb'.
...
exports.newVerb = function (options) {
	...
	/**
	 * If `options.tense` is 'past', create a terminal rule set with `terminalRuleSetMethods.newTermSequence()`, where each rule has the string `options.verbFormsTermSet.past` has its `text` value.
	 *
	 ...
	 */
	var verbSym = options.tense === 'past' ? newPastVerb(options) : baseNewVerb(options)

	/**
	 * Specify all possible `verbSym` reductions (i.e., subtrees) yield only display text, no semantics. Hence, all subtrees `verbSym` produces are semantically identical.
	 *
	 * Before `pfsearch`, `flattenTermSequence` flattens term sequences to a single terminal parser node with a single display text. If the parse node associated with a root term sequence node can produce multiple parse trees, it is due to ambiguity and the cheapest is chosen, though this rarely occurs.
	 *
	 * Assign `isTermSequence` after adding all `verbSym` rules above because `NSymbol.prototype.addRule()` prevents invocation on `NSymbol` instances with the property.
	 */
	verbSym.isTermSequence = true

	// Specify every rule `verbSym` produces is a uni-token terminal rule with the same `text` (display text) value. A subset of `NSymbol` instances marked `isTermSequence`.
	verbSym.isTermSet = true
	verbSym.termSetType = 'verb'

	// If defined, `verbSym` only produces terminal rules for the given grammatical tense; i.e., lacks terminal rules for the opposite tense.
	verbSym.verbTense = options.tense

	// Save `insertionCost` for convenience; not used internally.
 ...
 * Note: Each of the verb forms in `options.verbFormsTermSet` becomes a terminal symbol, and can not contain whitespace.
 *
 * For use by `terminalRuleSetMethods.newVerb(options)` when `options.tense` is 'present' or `undefined`.
 *
 * The returned `NSymbol` has the following properties, to be expended by `terminalRuleSetMethods.newVerb()`:
 ...
 * @private
 * @static
 * @param {Object} options The `terminalRuleSetMethods.newVerb()` options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
 * @param {number} [options.insertionCost] The insertion cost for the terminal rule set, assigned to the first rule in the set (i.e., `verbFormsTermSet.oneSg`). Enables the creation of insertion rules using the new `NSymbol` which produces this set (i.e., the LHS symbol).
 ...
 *
 * Uses `terminalRuleSetMethods.newTermSequence()` to create the invariable terminal rule set, where each rule has the string `options.verbFormsTermSet.past` as its `text` value. `pfsearch` can not conjugate the verb set.
 *
 * For use by `terminalRuleSetMethods.newVerb(options)` when `options.tense` is 'past'.
 *
 * The returned `NSymbol` has the following properties, to be expended by `terminalRuleSetMethods.newVerb()`:
 * 1. `name` - `options.symbolName`.
...
function newPastVerb(options) {
	...
	// `terminalRuleSetMethods.newVerb(options)` will extend `newVerb` with the properties `isTermSet`, `termSetType`, and `verbTense`.
	var newVerb = g.newTermSequence(pastVerbTermSet)
	delete newVerb.isTermSequence

	return newVerb
}

/**
 * Checks if `options`, which was passed to `terminalRuleSetMethods.newVerb()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} options The `terminalRuleSetMethods.newVerb()` options object to inspect.
...
/**
 * The inflections of a pronoun, from which `terminalRuleSetMethods.newPronoun()` creates a pronoun terminal rule set.
 *
...
exports.newPronoun = function (options) {
	...
	/**
	 * Specify all possible `pronounSym` reductions (i.e., subtrees) yield only display text, no semantics. Hence, all subtrees `pronounSym` produces are semantically identical.
	 *
	 * Before `pfsearch`, `flattenTermSequence` flattens term sequences to a single terminal parser node with a single display text. If the parse node associated with a root term sequence node can produce multiple parse trees, it is due to ambiguity and the cheapest is chosen, though this rarely occurs.
	 *
	 * Assign `isTermSequence` after adding all `pronounSym` rules above because `NSymbol.prototype.addRule()` prevents invocation on `NSymbol` instances with the property.
	 */
	pronounSym.isTermSequence = true

	// Specify every rule `pronounSym` produces is a uni-token terminal rule with the same `text` (display text) value. A subset of `NSymbol` instances marked `isTermSequence`.
	pronounSym.isTermSet = true
	pronounSym.termSetType = 'pronoun'
...
/**
 * Checks if `options`, which was passed to `terminalRuleSetMethods.newPronoun()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} options The `terminalRuleSetMethods.newPronoun()` options object to inspect.
 * @returns {boolean} Returns `true` if `options` is ill-formed, else `false`.
 */
...
/**
 * A term and cost pairing, from which `terminalRuleSetMethods.newTermSequence()` creates a term sequence substitution with a cost penalty.
 *
 ...
 * 2. A terminal rule set created by `terminalRuleSetMethods.newVerb()`.
 * 3. A terminal rule sequence created by this method or `terminalRuleSetMethods.newTermSequenceBinarySymbol()`.
 ...
 * The returned `NSymbol` has the following properties:
 * • `name` - `options.symbolName`.
 * • `isTermSequence` - `true`. Prevents adding more rules to this `NSymbol`.
 ...
 * @param {boolean} [options.verbTense] If `options.isVerb` is `true`, the grammatical tense, 'past' or 'present', of every verb the new `NSymbol` produces (each created by `terminalRuleSetMethods.newVerb(verbOptions)` with matching `verbOptions.tense`).
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		/**
		 * If `term` is a terminal rule set created by `terminalRuleSetMethods.newVerb()`, or a terminal rule sequence created by `terminalRuleSetMethods.newTermSetSet()` or `terminalRuleSetMethods.newTermSequenceBinarySymbol()`.
		 *
		 ...
		 *                                    -> `[on]`   -> "on",   text: "on"
		 */
		else if (term.isTermSet || term.isTermSequence) {
			...
			if (isIllFormedTermSequenceItem(term, options.isVerb, options.verbTense)) {
				throw new Error('Ill-formed term sequence')
			}
		 ...
		 */
			...
			var termSeqPair = flattenTermSequencePair(term, options.isVerb, options.verbTense)
	...
	// Assign `termSeqSym.isTermSequence` after adding all rules in the term sequence to prevent adding more rules to `termSeqSym`.
	...
	// Specify every term sequence `termSeqSym` produces, accepted or substituted, contains one verb terminal rule set (created by `terminalRuleSetMethods.newVerb()`).
	...
	termSeqSym.verbTense = options.verbTense

	return termSeqSym
}

/**
 * Checks if `options`, which was passed to `terminalRuleSetMethods.newTermSequence()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} options The `terminalRuleSetMethods.newTermSequence()` options object to inspect.
 * @returns {boolean} Returns `true` if `options` is ill-formed, else `false`.
 */
function isIllFormedNewTermSequenceOptions(options) {
	if (options.verbTense && !options.isVerb) {
		util.logErrorAndPath('`verbTense` specified for term sequence with falsey `isVerb`:', options)
		return true
	}
 ...
 * 1. A terminal rule set created by `terminalRuleSetMethods.newVerb()`.
 ...
 *
 * If `_isNestedTermSequence` is `true`, manually checks if `options.termPair` produces a verb, and the `verbTense` of that verb, to determine the `isVerb` and `verbTense` values for the returned `NSymbol`. For use when `options.termPair` was defined as a nested term pair within another term sequence pair passed to this method or `terminalRuleSetMethods.newTermSequence()`. `options.isVerb` only specifies if the entire base sequence contains or lacks a single verb, not every bigram within the sequence; e.g., do not check if "in common" within the verb phrase "have in common" contains a verb. Likewise, `options.verbTense` only specifies the grammatical tense of the entire base sequence.
 *
 * The returned `NSymbol` has the following properties:
 ...
 * • `isVerb` - `options.isVerb`, or if `_isNestedTermSequence` is `true` and `options.termPair` contains a verb.
...
exports.newTermSequenceBinarySymbol = function (options, _isNestedTermSequence) {
	...
	if (options.verbTense && !options.isVerb) {
		util.logErrorAndPath('`verbTense` defined for binary term sequence with falsey `isVerb`:', options)
		throw new Error('Ill-formed binary term sequence')
	}
	 ...
	 */
	var termSeqPair = flattenTermSequencePair(options.termPair, options.isVerb, options.verbTense, _isNestedTermSequence)
	...
	if (_isNestedTermSequence) {
		/**
		 * Manually check whether `termSeqSym` produces a verb, and the `verbTense` of that verb, when `options.termPair` was defined as a nested term pair within another term sequence pair passed to this method or `terminalRuleSetMethods.newTermSequence()`. This is necessary because `options.isVerb` only specifies if the entire base sequence (after flattening) contains or lacks a single verb, not every bigram within the sequence. Likewise, `options.verbTense` only specifies the grammatical tense of the entire base sequence.
		 *
		...
		termSeqSym.verbTense = termA.verbTense || termB.verbTense
	} else {
 ...
 * @param {boolean} verbTense If `isVerb` is `true`, specify every verb `termSeqPair` produces is of the grammatical tense 'past' or 'present' (each created by `terminalRuleSetMethods.newVerb(verbOptions)` with matching `verbOptions.tense`).
 ...
 */
function flattenTermSequencePair(termSeqPair, isVerb, verbTense, isNestedTermSequence) {
	...
	} else if (termSeqPair.indexOf(undefined) !== -1) {
		util.logErrorAndPath('Term sequence pair contains `undefined`:', termSeqPair)
		throw new Error('Ill-formed term sequence pair')
	}
	...
	for (var s = 0; s < termSeqLen; ++s) {
		...
		if (termSym.constructor === Array) {
			 ...
			 *
			 * Pass `true` for the method's second parameter, `isNestedTermSequence`, to instruct the method to manually check if the pair `termSym` contains a verb, and pass neither `isVerb` nor `verbTense`.
	 ...
	 * • If `verbTense` is defined, checks every verb `termSeqPair` produces has matching grammatical tense.
	 */
	if (isIllFormedTermSequencePair(termSeqPair, isVerb, verbTense, isNestedTermSequence)) {
		throw new Error('Ill-formed term sequence pair')
	}
 ...
 */
function isIllFormedTermSequenceItem(term, isVerb, verbTense) {
	if (isVerb) {
		if (!exports.isVerbTerm(term)) {
			util.logErrorAndPath('Non-verb provided to verb term sequence:', term)
			return true
		}

		if (verbTense) {
			if (!term.verbTense) {
				util.logErrorAndPath('Verb', util.stylize(term.name), 'lacks specified `verbTense` for verb term sequence:', util.stylize(verbTense), term)
				return true
			}

			if (verbTense !== term.verbTense) {
				util.logErrorAndPath('Verb tense of', util.stylize(term.name), '->', util.stylize(term.verbTense), 'does not match specified `verbTense` for verb term sequence:', util.stylize(verbTense), term)
				return true
			}
		} else if (term.verbTense) {
			util.logErrorAndPath('Verb with `verbTense` provided to verb term sequence without specified `verbTense`:', util.stylize(term.name), '->', util.stylize(term.verbTense), term)
			return true
		}
	} else if (verbTense) {
		util.logErrorAndPath('`verbTense` provided with falsey `isVerb`:', util.stylize(verbTense))
		throw new Error('Ill formed term sequence check')
 ...
 */
function isIllFormedTermSequencePair(termSeqPair, isVerb, verbTense, isNestedTermSequence) {
	...
	// Unlikely to be caught because of the preceding check, but this check will remain if the preceding limitation is removed.
	if (termSeqPair.every(term => term.verbTense)) {
		util.logErrorAndPath('Two instances of `verbTense` in a single term sequence pair:', termSeqPair)
		return true
	}

	if (isVerb) {
		if (!termSeqPair.some(exports.isVerbTerm)) {
			util.logErrorAndPath('No verbs provided in a verb term sequence pair:', termSeqPair)
			return true
		}

		var termSeqPairTense = termSeqPair[0].verbTense || termSeqPair[1].verbTense
		if (termSeqPairTense) {
			var termWithTense = termSeqPair[termSeqPair[0].verbTense ? 0 : 1]
			var tenseRuleStr = util.stylize(termWithTense.name) + ' -> ' + util.stylize(termSeqPairTense)
		}
		if (verbTense) {
			if (!termSeqPairTense) {
				util.logErrorAndPath('Term sequence pair lacks specified `verbTense` for verb term sequence:', util.stylize(verbTense), termSeqPair)
				return true
			}

			if (verbTense !== termSeqPairTense) {
				util.logErrorAndPath('Verb tense of', tenseRuleStr, 'does not match specified `verbTense` for verb term sequence pair:', util.stylize(verbTense), termSeqPair)
				return true
			}
		} else if (termSeqPairTense) {
			util.logErrorAndPath('Verb with `verbTense` provided to verb term sequence pair without specified `verbTense`:', tenseRuleStr, termSeqPair)
			return true
		}
	} else if (verbTense) {
		util.logErrorAndPath('`verbTense` provided with falsey `isVerb`:', util.stylize(verbTense))
...
function isForbiddenVerb(term) {
	if (term.isTermSet && term.termSetType === 'verb') {
		util.logErrorAndPath('Verb term set provided to non-verb term sequence (i.e., `isVerb` is falsey):', term)
		return true
	}

	if (term.isTermSequence && term.isVerb) {
		util.logErrorAndPath('Verb term sequence provided to non-verb term sequence (i.e., `isVerb` is falsey):', term)
		return true
	}

	if (term.verbTense) {
		util.logErrorAndPath('`verbTense` on non-verb term:', term)
		return true
	}
...
/**
 * Checks if `term` is a verb terminal rule set (created by `terminalRuleSetMethods.newVerb()`) or a verb term sequence (created by `terminalRuleSetMethods.newTermSequence()`).
 *
 * If `tense` is provided, checks if `term.verbTense` is of matching grammatical tense.
 * • Created by `terminalRuleSetMethods.newVerb(verbOptions)` with matching `verbOptions.tense`, or `terminalRuleSetMethods.newTermSequence(termSeqOptions)` with matching `termSeqOptions.verbTense`.
 *
 * @memberOf termSequence
...
exports.isVerbTerm = function (term, tense) {
	if (!(term.isTermSet && term.termSetType === 'verb') && !(term.isTermSequence && term.isVerb)) {
		return false
	}
 ...
 *
 * The following terminal symbols are permitted:
...
function isIllFormedTerminalSymbol(terminalSymbol) {
	 ...
	 * • apostrophe - for use in "followers'".
	 */
	...
	// Check if `terminalSymbol` contains adjacent permitted punctuation marks (defined above). Instances of any other punctuation mark are stopped above.
	var reAdjacentPuncMarks = RegExp('[' + permittedPuncMarks + ']{2,}')
	var adjacentPuncMarksMatch = reAdjacentPuncMarks.exec(terminalSymbol)
	if (adjacentPuncMarksMatch !== null) {
		util.logErrorAndPath('Terminal symbol', util.stylize(terminalSymbol), 'contains adjacent punctuation marks:', util.stylize(adjacentPuncMarksMatch[0]))
		return true
	}
