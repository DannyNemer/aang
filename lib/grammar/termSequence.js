 ...
 *
 * If `isNestedTermSequence` is falsey, specifies `termSeqPair` is the base term sequence passed to `termSequence.newTermSequenceBinarySymbol()` or `termSequence.newTermSequence()`. When so, checks `termSeqPair` contains one invariable term and one of type `termSequenceType` (which might also be 'invariable'). There are exceptions, documented internally.
 * • `isNestedTermSequence` must be falsey because because `termSequenceType` only specifies if the entire base sequence contains one term of type `termSequenceType` (e.g., 'verb') and all others (at whatever nesting level) are of type 'invariable', not if every bigram within the sequence has a term of type `termSequenceType`.
 ...
 * @param {boolean} isSubstitution Specify `termSeqPair` is substituted when matched in input. Necessary for term sequence pair type exceptions for substitutions of certain types.
 * @param {boolean} [isNestedTermSequence] Specify `termSeqPair` was defined as a nested term pair within another term sequence pair passed to `termSequence.newTermSequenceBinarySymbol()` or `termSequence.newTermSequence()`.
