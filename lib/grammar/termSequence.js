...
function isIllFormedTermSequencePair(termSeqPair, termSequenceType, isSubstitution, isNestedTermSequence) {
	...
	/**
	 * Check if both term sequences in `termSeqPair` are not non-invariable (i.e., conjugative). Always check irrespective of `isNestedTermSequence`.
	 *
	...
	// Check if `termSeqPair` does not contain a term sequence that is neither of type `termSequenceType` nor invariable (i.e., neutral).
	for (var t = 0; t < termSeqLen; ++t) {
