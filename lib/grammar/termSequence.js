	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		if (term.constructor === String) {
			...
			if (options.type !== exports.termTypes.INVARIABLE) {
				util.logErrorAndPath('Terminal symbol provided as accepted term for term sequence not of', util.stylize(exports.termTypes.INVARIABLE), 'type:', util.stylize(term), options)
				throw new Error('Ill-formed term sequence')
			}
		 ...
		 */
			...
			if (term.termSequenceType !== options.type) {
				util.logErrorAndPath('Term sequence', util.stylize(term.name), 'of', util.stylize(term.termSequenceType), 'type provided to parent term sequence of', util.stylize(options.type), 'type:', options)
				throw new Error('Ill-formed term sequence')
			}
		...
		options.substitutedTerms.forEach(function (term) {
			...
			if (term.constructor === String) {
				...
				if (options.type !== exports.termTypes.INVARIABLE && options.type !== exports.termTypes.PRONOUN) {
					util.logErrorAndPath('Terminal symbol provided as substituted term for term sequence of', util.stylize(options.type), 'type (only permitted for sequences of', util.stylize(exports.termTypes.INVARIABLE), 'or', util.stylize(exports.termTypes.PRONOUN), 'type):', util.stylize(term), options)
					throw new Error('Ill-formed substituted term sequence')
				}
				...
				if (!exports.isTermSequenceType(term, options.type)) {
					...
					if (options.type === exports.termTypes.PRONOUN) {
						if (!exports.isTermSequenceType(term, exports.termTypes.INVARIABLE)) {
							util.logErrorAndPath('Substituted term sequence', util.stylize(term.name), 'of', util.stylize(term.termSequenceType), 'type is neither of required', util.stylize(options.type), 'nor', util.stylize(exports.termTypes.INVARIABLE), 'type:', options)
							throw new Error('Ill-formed substituted term sequence')
						}
					}
					 ...
					 */
						...
						util.logErrorAndPath('Substituted term sequence', util.stylize(term.name), 'of', util.stylize(term.termSequenceType), 'type provided to parent term sequence of', util.stylize(options.type), 'type:', options)
						throw new Error('Ill-formed substituted term sequence')
					}
				}
			 ...
			 */
				 ...
				 * Ensure the resulting pair produces only one terminal rule set of type `options.type`, and all other term sequences are 'invariable'.
				 */
				var termSeqPair = flattenNestedTermSequencePairs(term, options.type, false, true)
 ...
 * @param {boolean} [isSubstitution] Specify `termSeqPair` is substituted when matched in input.
 * @returns {NSymbol} Returns the flattened term sequence ordered pair.
 */
function flattenNestedTermSequencePairs(termSeqPair, termSequenceType, isNestedTermSequence, isSubstitution) {
	 ...
	 */
	if (isIllFormedTermSequencePair(termSeqPair, termSequenceType, isNestedTermSequence, isSubstitution)) {
		throw new Error('Ill-formed term sequence pair')
	}
 ...
 */
function isIllFormedTermSequencePair(termSeqPair, termSequenceType, isNestedTermSequence, isSubstitution) {
	// Check if function invoked for a term sequence substitution but `termSeqPair` is not the root term sequence.
	if (isSubstitution && isNestedTermSequence) {
		util.logError('`isSubstitution` is `true` when `isNestedTermSequence` is falsey')
		throw new Error('Ill-formed term sequence pair check')
	}
	...
	// Check both term sequences in `termSeqPair` are not non-invariable (i.e., conjugative). Always check irrespective of `isNestedTermSequence`.
	// Check for two non-invariable term sequences instead of two type `termSequenceType` because there can be type exceptions where terms not of type `termSequenceType` are permitted, such as pronoun substitutions for invariable term sequences; e.g., `[1-sg-poss-det]` -> `[1-sg]`, text: "my".
	if (termSeqPair.every(term => term.termSequenceType !== exports.termTypes.INVARIABLE)) {
		util.logErrorAndPath('Term sequence pair of', util.stylize(termSequenceType), 'type contains two non-' + util.stylize(exports.termTypes.INVARIABLE), '(i.e., conjugative) term sequences:', termSeqPair)
		return true
	}
	...
	if (!isNestedTermSequence) {
		...
		for (var t = 0; t < 2; ++t) {
			...
			// Check if a term is neither invariable nor the specified type, `termSequenceType`
			if (termType !== exports.termTypes.INVARIABLE && termType !== termSequenceType) {
				// pronouns can be substituted terms for invariable sequences
				if (isSubstitution && termSequenceType === exports.termTypes.INVARIABLE && termType === exports.termTypes.PRONOUN) {
					continue
				}

				util.logErrorAndPath('Term sequence of', util.stylize(termType), 'type provided in term sequence pair of', util.stylize(termSequenceType), 'type:', termSeqPair)
				return true
			}
		}
		...
		// Check if `termSeqPair` lacks a term for the specified type
		if (termSeqPair.every(term => term.termSequenceType !== termSequenceType)) {
			// Pronoun substitutions can be of type 'invariable'
			// `[1-sg]` -> "myself", text: {1-sg-pronoun-forms}
			if (isSubstitution && termSequenceType === exports.termTypes.PRONOUN) {
				// Check if pronoun substitution neither has a pronoun nor is all invariable
				if (!termSeqPair.every(term => term.termSequenceType === exports.termTypes.INVARIABLE)) {
					util.logErrorAndPath('Substituted term sequence pair for', util.stylize(termSequenceType), 'sequence is neither of', util.stylize(termSequenceType), 'nor entirely', util.stylize(exports.termTypes.INVARIABLE), 'type:', termSeqPair)
					return true
				}
			}

			else {
				util.logErrorAndPath('Term sequence pair lacks term of specified', util.stylize(termSequenceType), 'type:', termSeqPair)
				return true
			}
		}
	}
