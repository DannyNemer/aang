 ...
 *
 * If `options.splitByTense` is falsey, each rule created from the terms in `options.verbFormsTermSet` has an object as its `text` with the properties `oneSg`, `threeSg`, `pl`, and `past` for the different verb forms. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct verb form (i.e., display text) according to grammatical properties in the same tree:
 ...
 *
 * If `options.splitByTense` is `true`, creates two terminal rule sets from the verb forms in `options.verbFormsTermSet`, split by present and past grammatical tense, and a third nonterminal substitution term sequence that produces both without tense restrictions. Returns the three `NSymbol` instances in an object of the form `TenseVerbSet`.
 ...
 *
 * If `options.splitByTense` is falsey, the returned `NSymbol` (or, `NSymbol` pair if `options.splitByTense`) has the following properties:
 ...
 *
 * If `options.splitByTense` is `true`, The returned `NSymbol` instances have the following properties:
 ...
 * @param {string} [options.symbolName] The name for the new `NSymbol`. If omitted, concatenates the prefix 'verb-' with the infinitive verb form, `options.verbFormsTermSet.pl`.
 ...
 * @returns {NSymbol|TenseVerbSet} If `options.splitByTense` is `true`, returns the object containing the separate present and past terminal rule sets and the parent substitution term sequence that combines the two. Else, returns the new `NSymbol` for the verb terminal rule set.
 */
var verbSchema = {
	symbolName: String,
...
exports.newVerb = function (options) {
	...
	if (options.splitByTense) {
		...
		// Specify every rule `presentVerbSym` and `pastVerbSym` each produce is a single-token terminal rule with the same `text` (display text) value as every rule in each separate set. A terminal rule set is subset of term sequences.
		...
		/**
		 * Save `displayText`, which is identical for both nonterminal rules `noTenseVerbSym` produces, as the set's default text.
		 *
		 ...
		 */
		noTenseVerbSym.defaultText = displayText
	...
	// Specify every rule `verbSym` produces is a single-token terminal rule with the same `text` (display text) value. A terminal rule set is subset of term sequences.
...
function baseNewVerb(options) {
	...
	if (!options.noPastDisplayText) {
		displayText.past = verbFormsTermSet.past
	}
	 ...
	 *
	 * Assign the insertion cost, if defined, to the first terminal rule in the set.
	 */
	verbSym.addRule(createVerbTerminalRule(verbFormsTermSet.oneSg, displayText, 'present', options.insertionCost))
	...
	if (verbFormsTermSet.past) {
		verbSym.addRule(createVerbTerminalRule(verbFormsTermSet.past, displayText, 'past'))
	}

	// If provided, the terminal rule for the present-subjunctive verb form, substituted when input with `displayText`. E.g., "be".
	...
	// If provided, the terminal rule for the present-participle verb form, substituted when input with `displayText`. E.g., "being", "liking".
	...
	// If provided, the terminal rule for the past-participle verb form, substituted when input with `displayText` (and substituted with `past` if the parent rule has matching `acceptedTense`). E.g., "been", "done".
	...
	/**
	 * Save `displayText`, which is identical for every terminal rule `verbSym` produces, as the terminal rule set's default text.
	 *
	 ...
	 */
	verbSym.defaultText = displayText
...
function createVerbTerminalRule(terminalSymbol, displayText, tense, insertionCost) {
	 ...
	 *
	 * If `displayText` is a conjugative text object with past and present tense verb forms, this rule is a past-participle form, `terminalSymbol` is matched in input, and the parent rule's `acceptedTense` matches `tense`, then `pfsearch` substitutes the verb set's simple past form, `VerbFormsTermSet.past`.
	 *
	...
	// Assign the insertion cost, if defined.
...
exports.newPronoun = function (options) {
	...
	// Specify every rule `pronounSym` produces is a single-token terminal rule with the same `text` (display text) value. A terminal rule set is subset of term sequences.
...
function isIllFormedNewPronounOptions(options) {
	...
	for (var pronounForm in pronounFormsTermSet) {
		if (termSequenceUtil.isIllFormedTerminalSymbol(pronounFormsTermSet[pronounForm])) {
			return true
		}
	}
	...
	if (options.substitutedTerms) {
		...
		options.substitutedTerms.forEach(function (term) {
			 ...
			 */
				 ...
				 */
				if (!termSequenceUtil.isTermSequenceType(term, options.type)) {
					/**
					 ...
					 */
					else if (options.type !== exports.termTypes.INVARIABLE || !termSequenceUtil.isTermSequenceType(term, exports.termTypes.PRONOUN)) {
