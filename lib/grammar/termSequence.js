...
function createVerbTerminalRule(terminalSymbol, verbTextForms, tense) {
	// Check every terminal symbol lacks whitespace and lacks forbidden characters.
...
exports.newPronoun = function (options) {
	...
	// Specify the `verbSym` term sequence type.
...
/**
 * Creates an `NSymbol` that produces a terminal rule sequence forming a term or phrase, comprised of terminal rules, terminal rule sets (e.g., `termSequence.newVerb()`, `termSequence.newPronoun()`), and nested term sequences.
 *
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		if (term.constructor === String) {
			...
			if (i === 0) {
				// Save the display text of the first item in `options.acceptedTerms` as the display text for `options.substitutedTerms`, if any.
				defaultText = term
			}
		}

		/**
		 * If `term` is a term sequence created by `termSequence.newTermSetSet()` or `termSequence.newTermSequenceBinarySymbol()` (or a terminal rule set created by `termSequence.newVerb()` or `termSequence.newPronoun()`).
		 *
		 ...
		 */
			...
			if (term.termSequenceType !== options.type) {
				util.logErrorAndPath('Nested term sequence', util.stylize(term.name), 'is not of type', util.stylize(type) + ':', term, options)
				throw new Error('Ill-formed term sequence')
			}
 ...
 * @private
 * @static
 * @param {NSymbol} term The term sequence (or terminal rule set) to inspect.
