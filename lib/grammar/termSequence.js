...
exports.newVerb = function (options) {
	...
	if (tense === undefined) {
		verbSym.termSequenceType = exports.types.VERB
	}	else if (tense === 'present') {
		verbSym.termSequenceType = exports.types.VERB_PRESENT
	}	else if (tense === 'past') {
		verbSym.termSequenceType = exports.types.VERB_PAST
	}
...
exports.newPronoun = function (options) {
	...
	pronounSym.termSequenceType = exports.types.PRONOUN
...
var termSequenceSchema = {
	symbolName: { type: String, required: true },
	insertionCost: Number,
	type: { values: Object.keys(exports.types).map(typeName => exports.types[typeName]), required: true },
...
exports.newTermSequenceBinarySymbol = function (options, _isNestedTermSequence) {
	...
	if (_isNestedTermSequence) {
		 ...
		 */
		termSeqSym.termSequenceType = termA.termSequenceType === exports.types.INVARIABLE ? termB.termSequenceType : termA.termSequenceType
	} else {
...
function isIllFormedTermSequencePair(termSeqPair, termSequenceType, isNestedTermSequence) {
	...
	for (var t = 0; t < 2; ++t) {
		...
		if (term.termSequenceType !== exports.types.INVARIABLE) {
			if (termSequenceType === exports.types.INVARIABLE) {
				...
				util.logErrorAndPath('Term in provided term sequence pair neither of (neutral) type', util.stylize(exports.types.INVARIABLE), ' nor the specified type', util.stylize(termSequenceType) + ':', term, termSeqPair)
				return true
			}
		}
		...
		// Unsure if `isNestedTermSequence` is needed if we just specify `exports.types.INVARIABLE`.
		if (!isNestedTermSequence && term.termSequenceType !== exports.types.INVARIABLE) {
...
exports.isVerbTerm = function (term, tense) {
	...
	if (!(term.isTermSequence && term.termSequenceType === exports.types.VERB)) {
		return false
	}
