 ...
 * • Requires `noInsert` also be `true` because an insertion of `symbol` would yield no text, and because it is a term sequence, no semantics. Hence, it would be identical to the rule `isOptional` creates, but with the insertion cost. If so desired, use `isOptional`.
 *
 ...
 * @property {boolean} [noText] Specify discarding the display text `symbol` and its descendants produce. Requires `symbol` be a term sequence and `noInsert` be `true`.
...
NSymbol.prototype._newNonterminalRule = function (options) {
	 ...
	 * • {Object|string|(Object|string)[]} [newRule.text] - If `newRule.isTermSequence` is `true`, assigns `options.text` to `newRule.text` for nonterminal substitutions, which instructs `flattenTermSequence` to use this rule's display text instead of text `newRule.rhs` produces for the terminal node it creates for this rule.
	 * • {number[]} newRule.rhsTermSequenceIndexes - If `newRule.isTermSequence` is not truthy because not every RHS symbol is a term sequence, `newRule.rhsTermSequenceIndexes` specifies the `newRule.rhs` indexes of which RHS symbol is a term sequence, if either. Only possible if `newRule.rhs` is binary. For use by `flattenTermSequence` to flatten a binary nonterminal node with one term sequence into an insertion node with the term sequence's text as the insertion text.
	 */
 ...
 * • {Object|string|(Object|string)[]} [newNonterminalRule.text] - If `newNonterminalRule.isTermSequence` is `true`, assigns `options.text` to `newNonterminalRule.text` for nonterminal substitutions, which instructs `flattenTermSequence` to use this rule's display text instead of text `newNonterminalRule.rhs` produces for the terminal node it creates for this rule.
 * • {number[]} newNonterminalRule.rhsTermSequenceIndexes - If `newNonterminalRule.isTermSequence` is not truthy because not every RHS symbol is a term sequence, `newNonterminalRule.rhsTermSequenceIndexes` specifies the `newNonterminalRule.rhs` indexes of which RHS symbol is a term sequence, if either. Only possible if `newNonterminalRule.rhs` is binary. For use by `flattenTermSequence` to flatten a binary nonterminal node with one term sequence into an insertion node with the term sequence's text as the insertion text.
 *
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	/**
	 * If `newNonterminalRule.isTermSequence` is not truthy because not every RHS symbol is a term sequence, `newNonterminalRule.rhsTermSequenceIndexes` specifies the `newNonterminalRule.rhs` indexes of which RHS symbol is a term sequence, if either. Only possible if `newNonterminalRule.rhs` is binary.
	 *
	 * For use by `flattenTermSequence` to flatten a binary nonterminal node with one term sequence into an insertion node with the term sequence's text as the insertion text.
	 *
	 * For use by `createInsertionRules` to mark insertion rules as term sequences for which the non-inserted RHS symbol is a term sequence when the original rule's entire RHS is not a term sequence.
	 */
	else {
		...
		// Only assign `newNonterminalRule.rhsTermSequenceIndexes` if it is not empty.
