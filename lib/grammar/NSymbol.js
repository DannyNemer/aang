 ...
 *
 * Does not include the violating function call line (i.e., file-path + line-number) in printed error messages. Excluded so that parent function may also print the options object that provided `text`.
 *
...
function isIllFormedText(text) {
	...
	if (textConstructor === Array) {
		...
		// Forbid `text` objects with only one form/property, for which a single string should have been used.
		...
		for (var textPropName in text) {
			...
			if (textPropName === 'infinitive') {
				util.logError('Conjugative text object property', util.stylize('infinitive'), 'is forbidden. Use', util.stylize('pl'), 'instead:', text)
				return true
			}

			// Recognize: 'oneSg', 'threeSg', 'pl'.
			if (nonterminalRuleSchema.personNumber.values.indexOf(textPropName) !== -1) {
				continue
			}

			// Recognize: 'nom', 'obj', 'past', 'participle'. ('infinitive' is manually stopped above.)
			if (rhsSymbolWrapperSchema.grammaticalForm.values.indexOf(textPropName) !== -1) {
				continue
			}

			// `textPropName` is unrecognized.
			util.logError('Unrecognized conjugative `text` object property:', util.stylize(textPropName), text)
