...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (rhs.every(rhsSym => rhsSym.isTermSequence)) {
		// Temporarily forbid a rule's RHS from producing multiple conjugative terminal rule sets (e.g., verbs, pronouns). Otherwise, `flattenTermSequence` can not track which input tense applies to which verb in the merged `text` array.
		if (rhs.length > 1 && rhs.every(term => term.termSequenceType !== g.termTypes.INVARIABLE)) {
			util.logError('RHS contains two conjugative term sequences:', ruleOptions.rhs)
