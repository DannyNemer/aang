...
NSymbol.prototype.addRule = function (options) {
	...
	if (this.isTermSequence && this.termSequenceType === g.termTypes.STOP) {
		util.logErrorAndPath('Attempting to add a rule to a completed stop sequence:', util.stylize(this.name), options)
		throw new Error('Adding rule to completed stop sequence')
	}
 ...
 *
 * `noText` instructs `flattenTermSequence` to discard the display text `symbol` and its descendants produce. Requires `symbol` be a term sequence.
 * • Directs term sequence `symbol` to behave as a stop sequence when matched in input by effectively removing the matched terminal symbols from input.
 *
 * `isOptional` instructs `NSymbol.prototype._newNonterminalRule()` to create a new unary rule with the other RHS symbol with which this `RHSSymbolWrapper` is paired and all of the rule's properties not specific to this symbol (i.e., the properties in this `RHSSymbolWrapper` instance).
 * • This is a shortcut for creating a rule with the same properties as the rule that owns the `RHSSymbolWrapper` instance with `isOptional`, but needs only match the other RHS symbol.
 * • Apt for stop sequences, to recognize the rule whether or not the stop sequence RHS symbol is matched.
 *
 ...
 * @property {boolean} [noText] Specify discarding the display text `symbol` and its descendants produce. Requires `symbol` be a term sequence.
 * @property {boolean} [isOptional] Specify creating a version of this symbol's nonterminal rule without this RHS symbol. Requires this rule be binary.
 */
 ...
 * @param {Object|string|(Object|string)[]} [options.text] The substitution display text that is used in place of any text `options.rhs` and its ancestors generate. For use when every symbol in `options.rhs` is a term sequence.
 * @param {number} [options.costPenalty] The rule's cost penalty.
 * @param {boolean} [options.isStopSequence] Specify `options.rhs` produces no display text. For use when every symbol in `options.rhs` is a term sequence.
...
NSymbol.prototype._newNonterminalRule = function (options) {
	...
	/**
	 * Iterate through binary `options.rhs`, and for `RHSSymbolWrapper` instances where `RHSSymbolWrapper.isOptional` is `true`, add a new unary nonterminal rule to this `NSymbol` with only the other RHS symbol and the `options` properties not specific to the optional symbol.
	 *
	 * Skip if this method's current invocation is a dry-run via `getExistingBinaryNSymbol()` (to create a temporary nonterminal rule for comparison to existing rules), which bound the method to `NSymbol.prototype` instead of an existing `NSymbol`.
	 */
	...
	var newRule = {
		...
		// The `rhs` indexes for which to instruct `flattenTermSequence` to discard `text` the term sequence at each `rhs` index produces when creating a new terminal `ruleProps` for `newRule`.
		noTextIndexes: getRHSNoTextIndexes(options.rhs),
		/**
	 ...
	 * • {Object|string|(Object|string)[]} [newRule.text] - If `newRule.isTermSequence` is `true`, assigns `options.text` to `newRule.text` for nonterminal multi-token substitutions, which instructs `flattenTermSequence` to use this rule's display text instead of text `newRule.rhs` produces for the terminal node it creates for this rule.
	 * • {boolean} [newRule.isStopSequence] - `options.isStopSequence`, which instructs `flattenTermSequence` to omit display text for the terminal node it creates for this rule, instead of using the text `newRule.rhs` produces. Therefore, stop sequences delete the matched input terminal symbols that `newRule.rhs` produces.
	 */
...
/**
 * Checks if `ruleOptions`, which was passed to `NSymbol.prototype._newNonterminalRule()` or `NSymbol.newBinaryRule()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} ruleOptions The nonterminal rule ruleOptions object to inspect.
 * @returns {boolean} Returns `true` if `ruleOptions` is ill-formed, else `false`.
 */
...
/**
 * Iterates through binary `options.rhs`, and for `RHSSymbolWrapper` instances where `RHSSymbolWrapper.isOptional` is `true`, adds a new unary nonterminal rule to this `NSymbol` with only the other RHS symbol and the `options` properties not specific to the optional symbol.
 *
 * Adds the new unary rule(s) before `NSymbol.prototype._newNonterminalRule(options)`, which invokes this method, adds the rule it will create from `options`. This order is immaterial.
 *
 * Invoke this method from `NSymbol.prototype._newNonterminalRule()` after flattening nested ordered pairs in RHS with `flattenNonterminalRHS()`.
 *
 * @memberOf NSymbol
 * @param {Object} options The `NSymbol.prototype._newNonterminalRule()` options object.
...
NSymbol.prototype._addOptionalRHSSymbolRules = function (options) {
	...
	for (var rhsIndex = 0, rhsLen = rhs.length; rhsIndex < rhsLen; ++rhsIndex) {
		...
		if (rhsSym.constructor === Object && rhsSym.isOptional) {
			...
			// Create new unary `rhs` with only the other RHS symbol. Includes grammatical properties, insertion restrictions, et cetera, specific to that RHS symbol.
...
function nonterminalRHSSymbolToName(rhsSym) {
	// If `rhsSym` is a string name of an `NSymbol`, checks if the respective `NSymbol` exists, else throws an exception. Only for use by `splitRegexTerminalSymbols`.
...
/**
 * Maps `rhsSym`, which was passed to `NSymbol.newBinaryRule(options)` in `options.rhs`, to a nonterminal symbol name with a distinguishing suffix if applicable, for use in the new binary symbol's name.
 *
 * For use by `NSymbol.newBinaryRule()` in the `name` of the new `NSymbol` it creates, for which it concatenates the mapped values this function returns for each of the new binary rule's RHS symbols.
 *
 * To distinguish binary symbols with identical RHS but differing properties, includes suffixes for the RHS symbol names using instances of the properties `RHSSymbolWrapper.noInsert` and `RHSSymbolWrapper.grammaticalForm`, if defined.
 *
 * @private
 * @static
 * @param {NSymbol|RHSSymbolWrapper|string} rhsSym The RHS nonterminal symbol to map.
 * @returns {string} Returns the name of `rhsSym`, distinguishing suffix if applicable.
 */
function binaryRHSSymbolToName(rhsSym) {
	...
	// Gets the nonterminal symbol name for all RHS items not instances of `RHSSymbolWrapper`: `NSymbol` instances, string names of `NSymbol` instances (for use by `splitRegexTerminalSymbols`), else throws an exception for unrecognized type.
...
/**
 * Generates an array of `rhs` indexes, from `RHSSymbolWrapper` instances with `RHSSymbolWrapper.noInsert` in `rhs`, for which to instruct `createInsertionRules` to prevent the creation of insertion rules at that index.
 *
 * Invoke this function after invoking `flattenNonterminalRHS(rhs)`.
 *
 * @private
...
/**
 * Generates an array of `rhs` indexes, from `RHSSymbolWrapper` instances with `RHSSymbolWrapper.noText` in `rhs`, for which to instruct `flattenTermSequence` to discard `text` the term sequence at that `rhs` index produces when creating a new terminal `ruleProps` for the associated nonterminal rule.
 *
 * Invoke this function after invoking `flattenNonterminalRHS(rhs)`.
 *
 * @private
 * @static
 * @param {(NSymbol|RHSSymbolWrapper|string)[]} rhs The nonterminal RHS array to iterate over.
 * @returns {number[]|undefined} Returns the array of RHS indexes for which to prevent display text, else `undefined`.
 */
function getRHSNoTextIndexes(rhs) {
	...
	for (var rhsIndex = 0, rhsLen = rhs.length; rhsIndex < rhsLen; ++rhsIndex) {
		...
		if (rhsSym.constructor === Object && rhsSym.noText) {
			noTextIndexes.push(rhsIndex)
		}
	}

	if (noTextIndexes.length > 0) {
		return noTextIndexes
	}
}
...
function assignSemanticAndCost(newRule, ruleOptions) {
	...
	if (ruleOptions.costPenalty !== undefined) {
		...
		// Restrict cost penalties to terminal rules, nonterminal substitutions, and nonterminal stop sequences.
		if (!ruleOptions.isTerminal && !newRule.text && !newRule.isStopSequence) {
			util.logErrorAndPath('`costPenalty` used on nonterminal rule that is neither a substitution term sequence nor a stop sequence:', ruleOptions)
			throw new Error('Ill-formed nonterminal rule')
		}
	}
 ...
 * • {Object|string|(Object|string)[]} [newNonterminalRule.text] - If `newNonterminalRule.isTermSequence` is `true`, assigns `options.text` to `newNonterminalRule.text` for nonterminal multi-token substitutions, which instructs `flattenTermSequence` to use this rule's display text instead of text `newNonterminalRule.rhs` produces for the terminal node it creates for this rule.
 * • {boolean} [newNonterminalRule.isStopSequence] - `ruleOptions.isStopSequence`, which instructs `flattenTermSequence` to omit display text for the terminal node it creates for this rule, instead of using the text `newNonterminalRule.rhs` produces. Therefore, stop sequences delete the matched input terminal symbols that `newNonterminalRule.rhs` produces.
 *
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (rhs.every(rhsSym => (rhsSym.constructor === Object ? rhsSym.symbol : rhsSym).isTermSequence)) {
		...
		if (ruleOptions.text) {
			...
			if (ruleOptions.isStopSequence) {
				util.logErrorAndPath('Nonterminal rule with substitution `text` has `isStopSequence`:', ruleOptions)
				throw new Error('Ill-formed nonterminal substitution rule')
			}

			/**
			 * Check the nonterminal substitution does not produce a RHS symbol with `grammaticalForm`, which should have conjugated `ruleOptions.text` and been discarded.
			 *
		...
		// Stop sequences yield no display text for the matched terminal symbols its RHS produces, effectively deleting them from input.
		else if (ruleOptions.isStopSequence) {
			// Check the stop sequence does not produce a RHS symbol with `grammaticalForm` or `acceptedTense`, which conjugate display text.
			if (rhs.some(rhsSym => rhsSym.constructor === Object && (rhsSym.acceptedTense || rhsSym.grammaticalForm))) {
				util.logErrorAndPath('Nonterminal rule with `isStopSequence` produces a RHS symbol with a conjugation property:', ruleOptions)
				throw new Error('Ill-formed nonterminal stop sequence')
			}

			// Instructs `flattenTermSequence` to not define `text` for the terminal node it creates from this rule's matched nonterminal node.
			newNonterminalRule.isStopSequence = true
		}
	} else if (ruleOptions.text) {
		util.logErrorAndPath('Nonterminal rule with `text` does not produce term sequence:', ruleOptions)
		throw new Error('Ill-formed nonterminal rule')
	} else if (ruleOptions.isStopSequence) {
		util.logErrorAndPath('Nonterminal rule with `isStopSequence` does not produce term sequence:', ruleOptions)
		throw new Error('Ill-formed nonterminal rule')
	}
...
/**
 * Checks if `rhsSym` is a conjugative verb that lacks a `grammaticalForm` value (defined on `RHSSymbolWrapper` instances) that can conjugate the `text` object the verb produces.
 *
 * Whether `rhsSym` has `acceptedTense` is immaterial because `grammaticalForm` will always conjugate when `acceptedTense` is absent. If `acceptedTense` is present, `pfsearch` will attempt to use its value to conjugate the associated `RHSSymbolWrapper.symbol` term sequence.
 *
 * For use by `assignTermSequence()` to determine if a provided binary rule contains two verbs, each of which produces a conjugative `text` object that the rule can not conjugate. Prevents `flattenTermSequence` from merging two conjugative verb `text` objects, each of which can have an input `tense`, and passing the merged text array up to a parent node without knowing to which text object each input `tense` applies.
 *
 * @private
 * @static
 * @param {RHSSymbolWrapper|NSymbol} rhsSym The RHS symbol to inspect.
 * @returns {boolean} Returns `true` if `rhsSym` is a conjugative verb that lacks a `grammaticalForm` value that can conjugate its `text` object, else `false`.
 */
...
NSymbol.newBinaryRule = function (options, suppressDupWarning) {
	 ...
	 *
	 * To distinguish binary symbols with identical RHS but differing properties, include suffixes for the RHS symbol names using instances of the properties `RHSSymbolWrapper.noInsert` and `RHSSymbolWrapper.grammaticalForm`, if defined.
	 */
...
function getExistingBinaryNSymbol(binaryRuleOptions) {
	...
	for (var s = 0, nontermSymsLen = nontermSyms.length; s < nontermSymsLen; ++s) {
		...
		// Check `NSymbol` instances with 1 rule, and those with > 1 rule that are marked `isBinarySymbol`, which resulted from binary rules whose RHS contained instances of `RHSSymbolWrapper.isOptional` that added additional rules to the binary symbol.
		if (existingNSymbol.rules.length === 1 || existingNSymbol.isBinarySymbol) {
			/**
			 * Check `existingNSymbol` has one rule, and that rule has properties identical to the binary rule `binaryRuleOptions` generates.
			 *
			 * Use `NSymbol.prototype._newNonterminalRule(binaryRuleOptions)` to create a temporary nonterminal rule to compare to existing rules of `existingNSymbol`.
			 * • Invoke the method bound to `NSymbol.prototype` rather than an existing `NSymbol` so that the method may internally distinguish that the invocation is a dry run; e.g., prevents the method from creating unary rules for instances of `RHSSymbolWrapper.isOptional`.
			 * • If this invocation throws an exception because `binaryRuleOptions` is ill-formed, then `binaryRuleOptions` is unique anyway because the existing rule must have passed the same checks.
			 * • Does not check (nor throw an exception) if this temporary rule is a duplicate of an existing rule, because that occurs in `NSymbol.prototype.addRule()`).
			 */
 ...
 * @param {boolean} [options.isStopTerm] Specify `options.rhs` is a stop term and `options.text` is `undefined`, such that `options.rhs` is produces no display text when matched in input. (This parameter is only used to enforce complete `NSymbol` rule definitions and is not saved to the output grammar.)
...
function isIllFormedTerminalRuleOptions(options) {
	...
	if (options.rhs === g.emptySymbol || options.isPlaceholder || options.isStopWord || options.isStopTerm) {
		...
		} else if (options.isStopTerm) {
			msgStart += 'a stop term'
		}
		msgStart += ', has '
