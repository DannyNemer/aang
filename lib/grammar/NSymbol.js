...
NSymbol.prototype._newNonterminalRule = function (options) {
	 ...
	 * • `rhsProducesOnlyTermSets` - Defines `newRule.rhsProducesOnlyTermSets` as `true` if the RHS produces only term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by the RHS, assign the merged `text` to this rule, and prevent `pfsearch` from traversing past this rule's node.
	 *
	 * • `text` - If `newNonterminalRule.rhsProducesOnlyTermSets` is `true`, assigns `options.text` to `newRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of text the RHS symbols produce.
	 */
	assignTermSetSequence(newRule, options)
 ...
 *
 *Defines `newRule.rhsProducesOnlyTermSets` as `true` if the RHS produces only term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by the RHS, assign the merged `text` to this rule, and prevent `pfsearch` from traversing past this rule's node.
 *
 * If `newNonterminalRule.rhsProducesOnlyTermSets` is `true`, assigns `ruleOptions.text` to `newNonterminalRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of the `text` the RHS symbols produce.
 *
 ...
 */
function assignTermSetSequence(newNonterminalRule, ruleOptions) {
	// Check if `rhs` contains only terminal rule sets (each created by either `g.newVerb()` or `g.newInvariableTerm()`).
	if (ruleOptions.rhs.every(rhsSym => rhsSym.isTermSet)) {
		/**
		 ...
		 */
		newNonterminalRule.rhsProducesOnlyTermSets = true
