...
/**
 * Generates a map of `rhs` indexes, from `RHSSymbolWrapper` instances in `rhs` with `RHSSymbolWrapper.noText`, for which to instruct `flattenTermSequence` to discard `text` the term sequence at that `rhs` index produces when creating a new `ruleProps` for the associated nonterminal rule.
 *
 * Invoke this function after invoking `flattenNonterminalRHS(rhs)`.
 *
 * @private
 * @static
 * @param {(NSymbol|RHSSymbolWrapper|string)[]} rhs The nonterminal RHS array to iterate over.
 * @returns {Object.<number, boolean>|undefined} Returns the map of RHS indexes for which to prevent display text, if any, else `undefined`.
 */
function getRHSNoTextIndexes(rhs) {
	...
	if (Object.keys(rhsNoTextIndexes).length > 1) {
