...
NSymbol._ruleSets = {}
// The map of `NSymbol` names to creation-lines (file-path + line-number); used for error reporting.
...
function NSymbol() {
	...
	if (NSymbol._ruleSets.hasOwnProperty(this.name)) {
		util.logErrorAndPath('Duplicate symbol name:', util.stylize(this.name))
		util.log('  Other', util.stylize(this.name), 'definition:')
...
var nontermRuleSchema = {
	...
	// The grammatical forms (case, tense) for which to conjugate terminal rules produced by this rule's RHS. E.g., "me" vs. "I" (case), "like" vs. "liked" (tense).
...
NSymbol.prototype.newNonterminalRule = function (options) {
	...
	var newRule = {
		...
		rhs: options.rhs.map(function (sym) {
			if (sym.constructor === String) {
				if (!NSymbol._ruleSets.hasOwnProperty(sym)) {
	...
	if (options.semantic) {
		...
		newRule.cost = this.newRuleCost(semantic.sumCosts(options.semantic))
	} else {
		// Calculate rule cost.
		newRule.cost = this.newRuleCost()
	}
...
NSymbol.prototype.newTerminalRule = function (options) {
	...
	// Require semantics to be completely reduced (i.e., a RHS semantic), except placeholder symbols.
	...
	var newRule = {
		...
		tense: options.tense,
		// `splitRegexTerminalSymbols` uses the following two properties to define nonterminal rules created from splitting multi-token terminal symbols as `rhsDoesNotProduceText`, which tells `pfsearch` to get the `text` from those nonterminal rules and to not traverse their child nodes (i.e., RHS symbols). Both are excluded in the output grammar.
	...
	if (options.semantic) {
		...
		newRule.semanticIsReduced = semantic.isReduced(options.semantic)
		// Calculate rule cost which includes the semantic's cost.
		newRule.cost = this.newRuleCost(semantic.sumCosts(options.semantic) + costPenalty)
	} else {
		// Calculate rule cost.
		newRule.cost = this.newRuleCost(costPenalty)
	}
...
/**
 * Calculates the cost of a new rule for this `NSymbol`. Increments cost of each new rule by an epsilon value of `1e-7` and adds `costPenalty` for the rule's semantics, if any.
 *
 * @memberOf NSymbol
 * @param {number} [costPenalty] The penalty added to the base cost for semantics.
 * @returns {number} Returns the cost of the new rule.
 */
NSymbol.prototype.newRuleCost = function (costPenalty) {
	// The cost of semantics on nonterminal rules, if any.
