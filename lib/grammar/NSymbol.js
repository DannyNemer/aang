...
NSymbol.prototype._newNonterminalRule = function (options) {
	...
	if (options.rhs.length === 2 && options.rhs.every(rhsSym => rhsSym.isTermSet)) {
		/**
		...
		if (options.acceptedTense && options.rhs.every(rhsSym => rhsSym.termSetType === 'verb')) {
			util.logError('Binary term set contains two verbs:', options.rhs)
		...
		util.logErrorAndPath('Nonterminal rule has `text` does not produce a binary term set:', options)
		throw new Error('Ill-formed nonterminal rule')
	}
