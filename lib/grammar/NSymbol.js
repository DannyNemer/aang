...
NSymbol.prototype._newNonterminalRule = function (options) {
	 ...
	 * • {number[]} newRule.termSequenceRHSIndexes - The `newRule.rhs` indexes of which RHS symbols are term sequences, irrespective of `newRule.isTermSequence`. For use by `createInsertionRules` to mark insertion rules as term sequences for which the non-inserted RHS symbol is a term sequence when the original rule's entire RHS is not a term sequence. `removeTempRulesAndProps` removes this property upon completion of grammar generation.
	 */
 ...
 * • {number[]} newNonterminalRule.termSequenceRHSIndexes - The `newNonterminalRule.rhs` indexes of which RHS symbols are term sequences, irrespective of `newNonterminalRule.isTermSequence`. For use by `createInsertionRules` to mark insertion rules as term sequences for which the non-inserted RHS symbol is a term sequence when the original rule's entire RHS is not a term sequence. `removeTempRulesAndProps` removes this property upon completion of grammar generation.
 *
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	 ...
	 */
	newNonterminalRule.termSequenceRHSIndexes = {}
	rhs.forEach(function (rhsSym, rhsIndex) {
		newNonterminalRule.termSequenceRHSIndexes[rhsIndex] = (rhsSym.constructor === Object ? rhsSym.symbol : rhsSym).isTermSequence
	})
