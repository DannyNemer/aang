 ...
 * 3. A nested ordered pair containing any combination of #1 or #2 from which
 *    to recursively create a new binary symbol and rule.
 ...
 * @param {boolean} [options.noInsert] Specify `createInsertionRules` can
 * not create insertion rules that insert `options.rhs` entirely, but can
 * create rules that insert one of two symbols in `options.rhs`.
 ...
 * reverse order of the ordered pair `options.rhs` in input, and corrects
 * the order of the display text the two RHS symbols produce when parsing.
 * @param {Object[]} [options.semantic] The semantic from which a semantic
 * tree is constructed in association with parse trees constructed with this
 * rule.
 * @param {string} [options.personNumber] The grammatical person-number for
 * which to conjugate verbs that either `options.rhs` produces or follow
 * this rule within the same parse subtree.
 * @param {string} [options.anaphoraPersonNumber] The grammatical
 * person-number for anaphoric rules with which to match and copy an
 * antecedent semantic of the same person-number. E.g., "his|her" refers to
 * semantics of third-person-singular representations.
...
NSymbol.prototype._newNonterminalRule = function (options) {
	...
	var newRule = {
		 ...
		 * objects the term sequences in `rhs` produce; i.e., `ruleProps.text` of
		 * immediate child nodes (post flattening via `flattenTermSequence`.
		 *
		 ...
		 * the empty object at the end of grammar generation.
		 *
		 * Note: It is much better to have a single terminal rule set with dynamic
		 * grammatical conjugation than to define separate rule sets for each
		 * grammatical case (depending on the rule) with the same
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (rhs.every(rhsSym => (rhsSym.constructor === Object ? rhsSym.symbol : rhsSym).isTermSequence)) {
		 ...
		 *
		 * Whether this rule has `acceptedTense` is immaterial because
		 * `grammaticalForm` will always conjugate when `acceptedTense` is
		 * absent. If `acceptedTense` is present, `pfsearch` will attempt to use
		 * its value to conjugate the associated `RHSSymbolWrapper.symbol` term
		 * sequence.
		 */
		if (rhs.length > 1 && rhs.every(isUnconjugatedVerb)) {
		...
		/**
		 * The `isTermSequence` property instructs `flattenTermSequence` to merge
		 * the `text` properties of the rules produced by each of this rule's RHS
		 * symbols (i.e., the child nodes) and assign the merged `text` to this
		 * rule's parse node. `pfsearch` will use this merged `text` and not
		 * traverse past this rule's node (to its child nodes).
		 * `flattenTermSequence` also checks the child nodes' input tense, if any,
		 * for use by `pfsearch` when conjugating the merged `text`.
		 *
		 ...
		 * Depending on the matched terminal rules, `flattenTermSequence` will
		 * create the following `text` value to assign to the `[work-at]` node,
		 * which `pfsearch` uses.
		 *   `text: [ { oneSg: 'work', threeSg: 'works', pl: 'work', past: 'worked' }, 'at' ]`
		 *   `tense: 'past'` // If "worked" is matched.
		 *
		 * Merging these adjacent `text` terminal rule values is more efficient
		 * than requiring `pfsearch` to traverse the same subtree to produce
		 * identical display text for multiple parse tree.
		 *
		 * In addition, as shown by this example, without this merging of `text`
		 * values, the `text` value on the  matched `[work-verb]` terminal rule
		 * will be too far for any `grammaticalForm` conjugation property, which
		 * only conjugates its direct child nodes, on the nonterminal rule that
		 * contains `[work-at]`.
		 *
		 * This rule can be unary, and neither a substitution no insertion. For
		 * example:
		 *   `[like]` -> `[love]` -> "love", "loves", "loved", text: `{love-verb-forms}`
		 *
		 * If the parse node associated with a root term sequence node can produce
		 * multiple parse trees, it is ambiguous and the cheapest is chosen,
		 * though this rarely occurs.
		 */
		newNonterminalRule.isTermSequence = true

		/**
		 * Display text for multi-token substitutions. The term sequences
		 * produced by the RHS symbols are matched in input,
		 * `flattenTermSequence` checks the matched terminal rules for input
		 * `tense` (to maintain in verb substitutions), while `pfsearch` uses
		 * this rule's `text`.
		 *
		 * For example, the following nonterminal substitution uses the `text`
		 * on the parent nonterminal rule while maintaining the input `tense` of
		 * the matched verb terminal rule.
		...
		if (ruleOptions.text) {
			...
			/**
			 * Check the nonterminal substitution does not produce a RHS symbol
			 * with `grammaticalForm`, which should have conjugated
			 * `ruleOptions.text` and been discarded.
			 *
			 * Do not check for `acceptedTense`, which should remain for
			 * (unconjugated) substitution `text` to allow `pfsearch` to conjugate
			 * `text` to match input tense.
			 *
			 * Note: This check should be extended to exempt `grammaticalForm` if
			 * `acceptedTense` also defined. Will implement later upon
			 * encountering this error in development.
			 */
			if (rhs.some(rhsSym => rhsSym.constructor === Object && rhsSym.grammaticalForm)) {
				util.logErrorAndPath('Nonterminal rule with substitution `text` produces a RHS symbol with `grammaticalForm`:', ruleOptions)
				throw new Error('Ill-formed nonterminal substitution rule')
			}

			// For use by nonterminal substitutions instead of the `text` produced
			// by this rule's RHS (i.e., child nodes).
	...
	/**
	 * If `newNonterminalRule.isTermSequence` is not truthy because not every
	 * RHS symbol is a term sequence,
	 * `newNonterminalRule.rhsTermSequenceIndexes` specifies the
	 * `newNonterminalRule.rhs` indexes of which RHS symbol is a term
	 * sequence, if either. Only possible if `newNonterminalRule.rhs` is
	 * binary.
	 *
	 * For use by `flattenTermSequence` to flatten a binary nonterminal node
	 * with one term sequence into an insertion node with the term sequence's
	 * text as the insertion text.
	 *
	 * For use by `createInsertionRules` to mark insertion rules as term
	 * sequences for which the non-inserted RHS symbol is a term sequence when
	 * the original rule's entire RHS is not a term sequence.
	 */
	else {
		var rhsTermSequenceIndexes = {}
		rhs.forEach(function (rhsSym, rhsIndex) {
			if ((rhsSym.constructor === Object ? rhsSym.symbol : rhsSym).isTermSequence) {
				// Only map the RHS index if `true`, skip if `false`.
				rhsTermSequenceIndexes[rhsIndex] = true
			}
		})

		// Only assign `newNonterminalRule.rhsTermSequenceIndexes` if it is not
		// empty.
		if (Object.keys(rhsTermSequenceIndexes).length > 0) {
			newNonterminalRule.rhsTermSequenceIndexes = rhsTermSequenceIndexes
		}
	}
...
/**
 * Checks if `rhsSym` is a conjugative verb that lacks a `grammaticalForm`
 * value (defined on `RHSSymbolWrapper` instances) that can conjugate the
 * `text` object the verb produces.
 *
 * Whether `rhsSym` has `acceptedTense` is immaterial because
 * `grammaticalForm` will always conjugate when `acceptedTense` is absent.
 * If `acceptedTense` is present, `pfsearch` will attempt to use its value
 * to conjugate the associated `RHSSymbolWrapper.symbol` term sequence.
 *
 * For use by `assignTermSequence()` to determine if a provided binary rule
 * contains two verbs, each of which produces a conjugative `text` object
 * that the rule can not conjugate. Prevents `flattenTermSequence` from
 * merging two conjugative verb `text` objects, each of which can have an
 * input `tense`, and passing the merged text array up to a parent node
 * without knowing to which text object each input `tense` applies.
 *
 * @private
 * @static
 * @param {RHSSymbolWrapper|NSymbol} rhsSym The RHS symbol to inspect.
 * @returns {boolean} Returns `true` if `rhsSym` is a conjugative verb that
 * lacks a `grammaticalForm` value that can conjugate its `text` object,
 * else `false`.
 */
function isUnconjugatedVerb(rhsSym) {
	var term = rhsSym.constructor === Object ? rhsSym.symbol : rhsSym

	/**
	 * Check if `term` produces a verb conjugative `text` object.
	 * • `[be-past]` is the only term sequence of type 'verb-past' that
	 *   produces a conjugative `text` object.
	 */
	...
	/**
	 * Check if `rhsSym` is an instance of `RHSSymbolWrapper` and has a
	 * `grammaticalForm` value that will conjugate the `text` object
	 * `rhsSym.symbol` produces.
	 * • Map 'infinitive' -> 'pl' as `getGramProps()` does upon creating the
	 *   `gramProps` object that the output grammar uses.
	 */
	...
	/**
	 * `true` if `rhsSym` produces a verb conjugative `text` object that its
	 * `rule (which will have the associated `gramProps`) can not conjugate,
	 * `thereby depending on a parent rule for conjugation.
	 */
	return isVerb && !canConjugateVerb
}

/**
 * Instantiates a new `NSymbol` with a single binary nonterminal rule for
 * `options`.
 *
 * If an identical binary rule for `options` already exists, prints a
 * warning and returns the associated, existing `NSymbol` instead of
 * creating a new `NSymbol` and rule.
 *
 * For the new `NSymbol` `name`, concatenates the names of the `options.rhs`
 * symbols (after recursively converting any nested arrays to binary
 * symbols).
 * • To distinguish binary symbols with identical `options.rhs` but
 *   differing properties, the new `NSymbol` `name` includes suffixes for the
 *   RHS symbol names to which `options.noInsertionIndexes` and/or
 *   `options.grammaticalForm` applies, if defined.
 *
 * After recursively converting any nested `options.rhs` arrays to binary
 * symbols, and instantiating the `NSymbol` with the name as explained,
 * creates the binary nonterminal rule with
 * `NSymbol.prototype._newNonterminalRule(options)`.
 *
 * Assigns `isBinarySymbol` to the returned `NSymbol` to prevent the
 * addition of any more rules to the symbol.
 *
 * @static
 * @memberOf NSymbol
 * @param {Object} options The options object for the nonterminal binary
 * rule, to pass to `NSymbol.prototype._newNonterminalRule()`.
 * @param {boolean} [suppressDupWarning] Specify suppressing the warning if
 * an identical binary rule and `NSymbol` already exists.
...
NSymbol.newBinaryRule = function (options, suppressDupWarning) {
	...
	// Recursively convert instances of nested ordered pairs in `options.rhs` to
	// binary symbols.
	flattenNonterminalRHS(options.rhs)

	/**
	 * Get the existing `NSymbol`, if any, that produces a single binary rule
	 * with properties identical to the rule generated from `options`.
	 *
	 * Perform this check after recursively creating binary rules for any
	 * nested RHS arrays in `options.rhs`.
	 */
	...
	/**
	 * For the new `NSymbol` `name`, concatenate the names of the
	 * `options.rhs` symbols (after recursively converting any nested arrays
	 * to binary symbols).
	 *
	 * To distinguish binary symbols with identical RHS but differing
	 * properties, include suffixes for the RHS symbol names using instances
	 * of the properties `RHSSymbolWrapper.noInsert` and
	 * `RHSSymbolWrapper.grammaticalForm`, if defined.
	 */
	var symbolNameTokens = options.rhs.map(binaryRHSSymbolToName)
	...
	// Mark `binaryRuleSym` as binary rule to prevent the addition of any more
	// rules.
...
/**
 * Gets the existing `NSymbol`, if any, that produces a single binary rule
 * with properties identical to the rule generated from `binaryRuleOptions`.
 *
 * For use by `NSymbol.newBinaryRule()` to check if a binary rule matching
 * `binaryRuleOptions` already exists. This is useful for nested arrays in
 * the `rhs` of nonterminal rule definitions, which is the shorthand to
 * create binary sub-rules, thereby removing the need to reference `NSymbol`
 * instances when used multiple rule definitions.
 *
 * The returned `NSymbol` need not have an identical name as the `NSymbol`
 * that `NSymbol.newBinaryRule()` would have created had no existing
 * identical rule been found. Though, this is currently impossible because
 * all binary symbol names are generated automatically.
 *
 * Invoke this function after recursively creating binary rules for any
 * nested RHS arrays in `binaryRuleOptions.rhs`.
 *
 * @private
 * @param {Object} binaryRuleOptions The nonterminal, binary rule options to
 * check, passed to `NSymbol.newBinaryRule()`.
 * @returns {NSymbol|undefined} Returns the matching `NSymbol` if it exists,
 * else `undefined`.
 */
function getExistingBinaryNSymbol(binaryRuleOptions) {
	...
	/**
	 * Get the existing `NSymbol`, if any, that produces a single binary rule
	 * with properties identical to the rule generated from
	 * `binaryRuleOptions`.
	 * Ignore LHS symbol names.
	 */
	...
	for (var s = 0, nontermSymsLen = nontermSyms.length; s < nontermSymsLen; ++s) {
		...
		/**
		 * Check `NSymbol` instances with 1 rule, and those with > 1 rule that
		 * are marked `isBinarySymbol`, which resulted from binary rules whose
		 * RHS contained instances of `RHSSymbolWrapper.isOptional` that added
		 * additional rules to the binary symbol.
		 */
		if (existingNSymbol.rules.length === 1 || existingNSymbol.isBinarySymbol) {
			/**
			 * Check `existingNSymbol` has one rule, and that rule has properties
			 * identical to the binary rule `binaryRuleOptions` generates.
			 *
			 * Use `NSymbol.prototype._newNonterminalRule(binaryRuleOptions)` to
			 * create a temporary nonterminal rule to compare to existing rules of
			 * `existingNSymbol`.
			 * • Invoke the method bound to `NSymbol.prototype` rather than an
			 *   existing `NSymbol` so that the method may internally distinguish
			 *   that the invocation is a dry run; e.g., prevents the method from
			 *   creating unary rules for instances of
			 *   `RHSSymbolWrapper.isOptional`.
			 * • If this invocation throws an exception because
			 *   `binaryRuleOptions` is ill-formed, then `binaryRuleOptions` is
			 *   unique anyway because the existing rule must have passed the same
			 *   checks.
			 * • Does not check (nor throw an exception) if this temporary rule is
			 *   a duplicate of an existing rule, because that occurs in
			 *   `NSymbol.prototype.addRule()`).
			 */
			if (existingNSymbol._ruleExists(NSymbol.prototype._newNonterminalRule(binaryRuleOptions))) {
				// Alert if the found, existing binary rule was not created by
				// `NSymbol.newBinaryRule()`.
...
/**
 * Checks if this `NSymbol` has a rule identical to `newRule` by comparing
 * all rule properties.
 *
 * @private
 * @memberOf NSymbol
 * @param {Object} newRule The new rule to check.
 * @returns {boolean} Returns `true` if this `NSymbol` has a rule identical
 * to `newRule`, else `false`.
 */
NSymbol.prototype._ruleExists = function (newRule) {
	return this.rules.some(function (existingRule) {
		for (var prop in existingRule) {
			// Do not compare the property `line`, which is a temporary property
			// used for debugging and which `removeTempRulesAndProps` removes at the
			// conclusion of grammar generation.
			if (prop === 'line') {
				continue
			}
 ...
 * @param {string} options.rhs The single-token terminal symbol this rule
 * produces.
 * @param {Object[]} [options.semantic] The completely reduced semantic from
 * which a semantic tree is constructed in association with parse trees
 * constructed with this rule.
 * @param {number} [options.insertionCost] Specify `createEditRules` can
 * create insertion rules from nonterminal rules that contain this (LHS)
 * `NSymbol` in their `rhs`, with this cost penalty and inserts
 * `options.text`.
 * @param {number} [options.costPenalty] The rule's cost penalty (separate
 * from `options.inserionCost`).
 * @param {Object|string|(Object|string)[]} [options.text] The display text
 * when `options.rhs` is matched in input. Can be omitted for stop-words or
 * placeholders that generate display text from input.
 * @param {string} [options.tense] The grammatical tense of `options.rhs`,
 * compared to the parent nonterminal rule's `acceptedTense` property, to
 * determine if the associated verb is accepted (for display text) in the
 * input tense, but not enforced if not input in that tense.
 * @param {boolean} [options.isPlaceholder] Specify `options.rhs` is a
 * placeholder to replace with input when parsing; e.g., entity category,
 * integer symbol. Prevents `Parser` from matching the literal form of
 * `options.rhs` in input (e.g., "{user}"). Placeholder symbols generate
 * display text and a semantic argument from matched input.
 * @param {boolean} [options.isSubstitution] Specify `options.text`
 * substitutes `options.rhs` when matched in input. or use by
 * `splitRegexTerminalSymbols` to define nonterminal rules created from
 * splitting multi-token terminal symbols as `rhsDoesNotProduceText`, which
 * tells `pfsearch` to get the `text` from those nonterminal rules and to
 * not traverse their child nodes (i.e., RHS symbols).
 * @param {boolean} [options.isStopWord] Specify `options.rhs` is a
 * stop-word and `options.text` is `undefined`, such that `options.rhs` is
 * excluded from display text when matched in input. For use by
 * `splitRegexTerminalSymbols`, just as `options.isSubstitution`.
...
NSymbol.prototype._newTerminalRule = function (options) {
	...
	var newRule = {
		// Specify this is a terminal rule (i.e., `options.rhs` does not produce
		// rules).
		isTerminal: true,
		/**
		 * Convert `options.rhs` to lowercase just as all input. This does not
		 * alter capitalization of `options.text`, even if identical to
		 * `options.rhs`.
		 */
		rhs: [ options.rhs.toLowerCase() ],
		/**
		 * Placeholder terminal symbols generate display text and a semantic
		 * argument from input; e.g., integer symbols, entities. Define
		 * `isPlaceholder` to prevent `Parser` from matching the literal
		 * terminal symbol in input.
		 */
		isPlaceholder: options.isPlaceholder,
		tense: options.tense,
		/**
		 * `splitRegexTerminalSymbols` uses the following two properties to
		 * define `nonterminal rules created from splitting multi-token terminal
		 * symbols `as `rhsDoesNotProduceText`, which tells `pfsearch` to get
		 * the display `text, `text`, from those nonterminal rules and to not
		 * traverse their `child nodes (i.e., RHS symbols). I.e., `pfsearch`
		 * only needs (and `uses) `rhsDoesNotProduceText`. Both are excluded in
		 * the output `grammar.
		 */
		...
		text: options.text,
		/**
		 * Enable `createEditRules` to create insertion rules from nonterminal
		 * rules that contain this (LHS) `NSymbol` in their `rhs`, with
		 * `options.insertionCost` as cost penalty and inserts `options.text`.
		 */
 ...
 * `NSymbol.prototype._newTerminalRule()`, is ill-formed. If so, prints an
 * error.
 *
 * @private
 * @static
 * @param {Object} options The `NSymbol.prototype._newTerminalRule()`
...
function isIllFormedTerminalRuleOptions(options) {
	...
	// Prevent terminal rules with placeholder symbols from being assigned a
	// reduced (RHS) semantic because placeholder symbols generate and use a
	// (reduced) semantic argument from input.
	...
	// Require semantics on terminal rules to be completely reduced (i.e., a RHS
	// semantic), except placeholder symbols.
 ...
 *
 * Printed errors exclude the violating file-path + line-number so that the
 * calling function can also print the options object that provided `text`.
 *
 * FIXME: Extend checks with `termSequence.isIllFormedTerminalSymbol()`,
 * even if redundant.
 *
 * @private
 * @static
 * @param {Object|string|(Object|string)[]} text The display text to
 * inspect.
 ...
 * @private
 * @static
 * @param {Object} textObject The conjugative display text object to
 * inspect.
 * @returns {boolean} Returns `true` if `textObject` is ill-formed, else
 * `false`.
 */
function isIllFormedTextObject(textObject) {
	// Forbid `text` objects with only one property/form, for which a single
	// string should have been used.
	...
	for (var textPropName in textObject) {
		/**
		 * Though 'infinitive' is a recognized `grammaticalForm` value, it is
		 * not used on text objects.
		 *
		 * For `grammaticalForm`, `NSymbol` maps 'infinitive' -> 'pl' to share the
		 * text form with `personNumber` defined as 'pl'`.
		 */
		 ...
		 * • gramProps: 'nom', 'obj', 'past', 'participle'.
		 *   ('infinitive' is manually stopped above.)
		 */
...
/**
 * Diversifies the costs of the grammar's non-edit rules by incrementing the
 * cost of each nonterminal symbol's rules by an increasing epsilon value.
 * This ensures no nonterminal symbol produces multiple non-edit rules of
 * identical cost.
 *
 * The cost modification introduces entropy to the grammar via multiplicity,
 * which significantly reduces the probability of a given parse node
 * producing multiple subnodes with identical cost or identical cost
 * heuristic. This enables consistent determination of the cheapest path for
 * `pfsearch` to follow or use in its minimum cost heuristic calculations.
 * Otherwise, `pfsearch` would choose whichever path/subnode is arbitrarily
 * first.
 *
 * Invoke this method after invoking `removeUnusedComponents`, which removes
 * unused rules from the grammar, and before adding edit-rules in
 * `createEditRules`. Diversify the costs after removing the unused rules,
 * instead of incrementing each rule's cost upon each
 * `NSymbol.prototype.addRule()` invocation, to evenly distribute the
 * increasing epsilon value for each nonterminal symbol's rules. Though
 * edit-rules are excluded from this operation, they will inherit the
 * diversified costs of their base rules and not lessen the rule cost
 * multiplicity.
 *
 * @static
 * @memberOf NSymbol
...
NSymbol.diversifyRuleCosts = function () {
	// The arbitrarily small positive quantity with which to increment each
	// rule's cost by an increasing value.
	var EPSILON = 1e-7
...
/**
 * Sorts the grammar's nonterminal symbols alphabetically and each symbols'
 * rules by increasing cost.
 *
 * It is essential to sort rules by increasing cost so that when
 * `StateTable` groups `ruleProps` instances for edit rules with the same
 * RHS symbols, the cheapest rule is the first `ruleProps` instance in the
 * set. This enables `calcHeuristicCosts` to determine the minimum cost for
 * these `ruleProps` sets by checking the cost of the first object in each
 * set.
 *
 * `sortGrammar()` in `grammar` invokes this method at the end of grammar
 * generation.
 *
 * @static
 * @memberOf NSymbol
...
// Extend `NSymbol` with methods for predefined sets of rules (e.g., verbs,
// stop words).
