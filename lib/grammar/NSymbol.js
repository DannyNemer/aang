...
/**
 * Checks if `newNonterminalRuleOptions`, which was passed to `NSymbol.prototype._newNonterminalRule()`, is ill-formed. If not, prints an error.
 *
 * @private
 * @static
 * @param {Object} newNonterminalRuleOptions The `NSymbol.prototype._newNonterminalRule()` options object to inspect.
 * @returns {boolean} Returns `true` if `newNonterminalRuleOptions` is ill-formed, else `false`.
 */
function isIllFormedNewNonterminalRuleOptions(newNonterminalRuleOptions) {
	if (newNonterminalRuleOptions.rhs.length > 2) {
		util.logErrorAndPath('Nonterminal rule has > 2 RHS symbols:', newNonterminalRuleOptions)
		return true
	}

	if (newNonterminalRuleOptions.noInsertionIndexes && newNonterminalRuleOptions.noInsertionIndexes.some(idx => newNonterminalRuleOptions.rhs[idx] === undefined)) {
		util.logErrorAndPath('\'noInsertionIndexes\' contains an index for which there is no RHS symbol:', newNonterminalRuleOptions)
		return true
	}

	if (newNonterminalRuleOptions.transpositionCost !== undefined && newNonterminalRuleOptions.rhs.length !== 2) {
		util.logErrorAndPath('Nonterminal rule with transposition cost does not have 2 RHS symbols:', newNonterminalRuleOptions)
		return true
	}
...
function assignSemanticAndCost(newRule, ruleOptions) {
	...
	if (ruleOptions.costPenalty !== undefined) {
		...
		if (!ruleOptions.isTerminal && !newRule.text && !newRule.isTermSequence) {
 ...
 * • {boolean} [newNonterminalRule.isTermSequence] - Defines `true` if the `newNonterminalRule.rhs` produces only term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by ``newNonterminalRule.rhs``, assign the merged `text` to this rule, and prevent `pfsearch` from traversing past this rule's node.
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (ruleOptions.rhs.every(rhsSym => rhsSym.isTermSet || rhsSym.isTermSequence)) {
		...
		// Temporarily forbid a term sequence from containing multiple verbs. Otherwise, `calcHeuristicCosts` can not track which input tense applies to which verb in the merged `text` array.
		if (ruleOptions.rhs.length > 1 && ruleOptions.rhs.every(g.isVerbTerm)) {
			util.logError('Term sequence contains multiple verbs:', ruleOptions.rhs)
 ...
 *
 * Also checks objects in `textArray` contain at least two term forms.
 *
 * @private
 * @static
 ...
 * @private
 * @static
 * @param {string|Object} textItem The text value to check.
...
/**
 * Checks if `newTerminalRuleOptions`, which was passed to `NSymbol.prototype._newTerminalRule()`, is ill-formed. If not, prints an error.
 *
 * @private
 * @static
 * @param {Object} newTerminalRuleOptions The `NSymbol.prototype._newTerminalRule()` options object to inspect.
 * @returns {boolean} Returns `true` if `newTerminalRuleOptions` is ill-formed, else `false`.
 */
function isIllFormedNewTerminalRuleOptions(newTerminalRuleOptions) {
	if (/[^\S ]/.test(newTerminalRuleOptions.rhs)) {
		util.logError('Terminal symbol contains a whitespace character other than a space:', util.stylize(newTerminalRuleOptions.rhs), newTerminalRuleOptions)
		return true
	}

	if (/ {2,}/.test(newTerminalRuleOptions.rhs)) {
		util.logError('Terminal symbol contains a sequence of multiple spaces:', util.stylize(newTerminalRuleOptions.rhs), newTerminalRuleOptions)
		return true
	}

	if (newTerminalRuleOptions.text === '') {
		util.logError('\'text\' is an empty string, \'\':', newTerminalRuleOptions)
		return true
	}

	if (newTerminalRuleOptions.isSubstitution && newTerminalRuleOptions.text === undefined) {
		util.logError('Substitution lacks \'text\':', newTerminalRuleOptions)
		return true
	}
	...
	if (newTerminalRuleOptions.isPlaceholder && newTerminalRuleOptions.semantic && semantic.isReduced(newTerminalRuleOptions.semantic)) {
		util.logErrorAndPath('Placeholder symbols has reduced (RHS) semantic:', newTerminalRuleOptions)
		return true
	}

	// Forbid semantics on `<empty>`.
	if (newTerminalRuleOptions.rhs === g.emptySymbol && newTerminalRuleOptions.semantic) {
		util.logErrorAndPath('Rule with `<empty>` has semantic:', newTerminalRuleOptions)
		return true
	}

	// Require semantics on terminal rules to be completely reduced (i.e., a RHS semantic), except placeholder symbols.
	if (newTerminalRuleOptions.semantic && !semantic.isReduced(newTerminalRuleOptions.semantic) && !newTerminalRuleOptions.isPlaceholder) {
		util.logError('Terminal rules has non-reduced (LHS) semantic:', newTerminalRuleOptions.semantic)
		util.logPathAndObject(newTerminalRuleOptions)
		return true
	}

	if (newTerminalRuleOptions.rhs === g.emptySymbol || newTerminalRuleOptions.isPlaceholder || newTerminalRuleOptions.isStopWord) {
		var msgStart = '\'' + newTerminalRuleOptions.rhs + '\', '
		if (newTerminalRuleOptions.rhs === g.emptySymbol) {
			msgStart += 'an empty string'
		} else if (newTerminalRuleOptions.isPlaceholder) {
			msgStart += 'a placeholder symbol'
		} else if (newTerminalRuleOptions.isStopWord) {
			msgStart += 'a stop-word'
		}
		msgStart += ', has '

		// Forbid display text on stop-words, `<empty>` and placeholder symbols.
		if (newTerminalRuleOptions.text !== undefined) {
			util.logErrorAndPath(msgStart + '\'text\':', newTerminalRuleOptions)
			return true
		}

		// Forbid insertions of stop-words, `<empty>`, and placeholder symbols.
		if (newTerminalRuleOptions.insertionCost !== undefined) {
			util.logErrorAndPath(msgStart + '\'insertionCost\':', newTerminalRuleOptions)
			return true
		}
	}

	// Forbid `text` objects with only one form, for which a single string should have been used.
	else if (isIllFormedTextObject(newTerminalRuleOptions.text)) {
		return true
	}
