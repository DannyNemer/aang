 ...
 * @property {NSymbol} symbol THe RHS symbol.
 * @property {string} [acceptedTense] The grammatical tense for which the verb `symbol`, created with `g.newVerb()`, is accepted when input in that tense, but the form is not enforced if not input in that tense.
 */
...
var nonterminalRuleSchema = {
	/**
	 * The array of `NSymbol` instances, objects of the form `RHSSymbolWrapper`, `NSymbol` names (strings), and/or nested arrays of any the preceding for new binary rules to recursively create. These (nested) sub-rules, however, can only contain a RHS and no other rule properties.
	 *
	 * Accepts string `NSymbol` names for passing an existing rule's `rhs` array for a new rule, as in `user.js`.
	 */
	rhs: { type: Array, arrayType: [ NSymbol, Object, String, Array ], required: true },
...
NSymbol.newBinaryRule = function (options, suppressDupWarning) {
	...
	var symbolNameTokens = options.rhs.map(function (sym, i) {
		...
		// Specify in name if an insertion rules are forbidden for the RHS symbol.
		if (options.noInsertionIndexes && options.noInsertionIndexes.indexOf(i) !== -1) {
			name = grammarUtil.hyphenate(name, 'no', 'insert')
		}

		// Specify in name if the RHS symbol has an associated `options.grammaticalForm` property.
		if (options.grammaticalForm && options.grammaticalForm[i]) {
			name = grammarUtil.hyphenate(name, options.grammaticalForm[i])
		}
