...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (rhs.every(rhsSym => rhsSym.isTermSequence)) {
		 ...
		 *
		 * Prevents `flattenTermSequence` from merging two conjugative verb text objects, each of which can have an input `tense`, passing the merged text array up to a parent node without knowing to which text object which input `tense` applies.
		 *
