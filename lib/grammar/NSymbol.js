 ...
 *
 * Defines `newNonterminalRule.rhsIsBinaryTermSet` as `true` if the RHS contains only term sets, which instructs `calcHeuristicCosts` to merge the `text` values the RHS produces and prevent `pfsearch` from traversing past this rule's node.
 *
 * If `newNonterminalRule.rhsIsBinaryTermSet` is `true`, assigns `ruleOptions.text` to `newNonterminalRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of the `text` the RHS symbols produce.
 *
