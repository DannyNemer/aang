...
NSymbol.prototype._newNonterminalRule = function (options) {
	 ...
	 * • {number[]} newRule.termSequenceRHSIndexes - The `newRule.rhs` indexes of which RHS symbols are term sequences, irrespective of `newRule.isTermSequence`. For use by `createInsertionRules` to mark insertion rules as term sequences for which the original rule is not because the inserted RHS symbol is not a term sequence. `removeTempRulesAndProps` removes this property upon completion of grammar generation.
	 */
 ...
 * • {number[]} newNonterminalRule.termSequenceRHSIndexes - The `newNonterminalRule.rhs` indexes of which RHS symbols are term sequences, irrespective of `newNonterminalRule.isTermSequence`. For use by `createInsertionRules` to mark insertion rules as term sequences for which the original rule is not because the inserted RHS symbol is not a term sequence. `removeTempRulesAndProps` removes this property upon completion of grammar generation.
 *
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	// Temporarily save the `newNonterminalRule.rhs` indexes of which RHS symbols are term sequences, irrespective of `newNonterminalRule.isTermSequence`. For use by `createInsertionRules` to mark insertion rules as term sequences for which the original rule is not a term sequence because the inserted RHS symbol is not a term sequence. `removeTempRulesAndProps` removes this rule property upon completion of grammar generation.
