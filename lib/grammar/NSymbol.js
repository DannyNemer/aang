...
NSymbol.prototype._newNonterminalRule = function (options) {
	 ...
	 * • {boolean} [isTermSequence] - Defines `newRule.isTermSequence` as `true` if the RHS produces only term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by the RHS, assign the merged `text` to this rule, and prevent `pfsearch` from traversing past this rule's node.
	 *
	 * • {string} [text] - If `newNonterminalRule.isTermSequence` is `true`, assigns `options.text` to `newRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of text the RHS symbols produce.
	 */
	...
	var costPenalty = 0
	if (options.costPenalty !== undefined) {
		costPenalty = options.costPenalty

		// Restrict cost penalties to non-terminal substitutions
		if (!newRule.text && !newRule.isTermSequence) {
			util.logErrorAndPath('`costPenalty` used on non-substitution nonterminal term sequence:', options)
			throw new Error('Ill-formed nonterminal rule')
		}
	}
	...
	if (options.semantic) {
		if (options.anaphoraPersonNumber) {
			util.logErrorAndPath('Anaphoric rule hss semantic:', options)
			throw new Error('Ill-formed nonterminal rule')
		}

		// Assign semantic used in semantic trees that correspond to parse trees that contain this rule.
		...
		// The rule's semantic may be reduced and have its RHS produce additional semantics, reduced or not. If so, `pfsearch` will merge this rule's reduced semantic (`newRule.semantic`) with the semantic(s) its RHS produces, forming a semantic array of RHS semantic arguments.
...
NSymbol.prototype._newTerminalRule = function (options) {
	...
	else if (isIllFormedTextObject(options.text)) {
		throw new Error('Ill-formed terminal text')
	}
