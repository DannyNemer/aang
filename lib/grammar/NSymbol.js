...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (rhs.every(rhsSym => rhsSym.isTermSequence)) {
		// Temporarily forbid a rule's RHS from producing multiple verbs. Otherwise, `flattenTermSequence` can not track which input tense applies to which verb in the merged `text` array.
		if (rhs.length > 1 && rhs.every(term =>
			...
			util.logError('RHS contains two verb sequences:', ruleOptions.rhs)
