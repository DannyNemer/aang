...
NSymbol.prototype.addRule = function (options) {
	...
	if (this.isTermSetSequence) {
		util.logErrorAndPath('Attempting to add a rule to a completed term set sequence:', options)
		throw new Error('Adding rule to completed rule set')
	}
...
NSymbol.prototype._newNonterminalRule = function (options) {
	 ...
	 * • `isTermSetSequence` - Defines `newRule.isTermSetSequence` as `true` if the RHS produces only term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by the RHS, assign the merged `text` to this rule, and prevent `pfsearch` from traversing past this rule's node.
	 *
	 * • `text` - If `newNonterminalRule.isTermSetSequence` is `true`, assigns `options.text` to `newRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of text the RHS symbols produce.
	 */
 ...
 *
 *Defines `newRule.isTermSetSequence` as `true` if the RHS produces only term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by the RHS, assign the merged `text` to this rule, and prevent `pfsearch` from traversing past this rule's node.
 *
 * If `newNonterminalRule.isTermSetSequence` is `true`, assigns `ruleOptions.text` to `newNonterminalRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of the `text` the RHS symbols produce.
 *
...
function assignTermSetSequence(newNonterminalRule, ruleOptions) {
	...
	if (ruleOptions.rhs.every(rhsSym => rhsSym.isTermSet)) {
		 ...
		 */
		newNonterminalRule.isTermSetSequence = true
