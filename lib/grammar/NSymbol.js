 ...
 */
NSymbol.new = function () {
	const newNSymbol = Object.create(NSymbol.prototype)
...
NSymbol.prototype.addRule = function (options) {
	const newRule = options.isTerminal ? this.newTerminalRule(options) : this.newNonterminalRule(options)
...
NSymbol.newBinaryRule = function (options) {
	...
	const symbolNameTokens = options.rhs.map(function (sym, i) {
	...
	return NSymbol.new.apply(null, symbolNameTokens).addRule(options)
}
...
NSymbol.prototype.newTerminalRule = function (options) {
	...
	// The cost penalty added to this rule's cost.
	const costPenalty = options.costPenalty === undefined ? 0 : options.costPenalty
...
NSymbol.sortRules = function () {
	Object.keys(NSymbol._ruleSets).sort().forEach(function (symbolName) {
		...
		const rules = NSymbol._ruleSets[symbolName].sort(function (ruleA, ruleB) {
