...
function getExistingBinaryNSymbol(binaryRuleOptions) {
	...
	for (var s = 0, nontermSymsLen = nontermSyms.length; s < nontermSymsLen; ++s) {
		...
		if (existingNSymbol.rules.length === 1) {
			 ...
			 *
			 * Use `existingNSymbol._newNonterminalRule()`, bound to the existing `NSymbol`, to create a temporary rule to compare.
			 * • The bound `NSymbol` is only needed for mapping RHS symbols and including the symbol's name in error messages.
