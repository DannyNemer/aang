...
/**
 * Generates an array of `rhs` indexes, from `RHSSymbolWrapper` instances in `rhs` with `RHSSymbolWrapper.noInsert`, for which to instruct `createInsertionRules` to prevent the creation of insertion rules at that index.
 *
 * Invoke this function after invoking `flattenNonterminalRHS(rhs)`.
 *
 * @private
...
/**
 * Generates an array of `rhs` indexes, from `RHSSymbolWrapper` instances in `rhs` with `RHSSymbolWrapper.noText`, for which to instruct `flattenTermSequence` to discard `text` the term sequence at that `rhs` index produces when creating a new `ruleProps` for the associated nonterminal rule.
 *
 * Invoke this function after invoking `flattenNonterminalRHS(rhs)`.
 *
 * @private
