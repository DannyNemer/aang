...
function isIllFormedGramProps(newNonterminalRule) {
	...
	for (var propName in gramProps) {
		...
		if (gramProp && NSymbol.isFutileGramProp(newNonterminalRule.rhs, gramProp)) {
			util.logErrorAndPath('RHS produces no rules with conjugative `text` applicable to the grammatical property', util.stylize(propName), '->', util.stylize(gramProp) + ':', newNonterminalRule)
			return true
		}
	}
