...
NSymbol.prototype.addRule = function (options) {
	if (this.isTermSet) {
		...
		throw new Error('Adding rule to completed rule set')
	}

	if (this.isVerbSet) {
		util.logErrorAndPath('Attempting to add a rule to a completed verb set:', options)
		throw new Error('Adding rule to completed rule set')
	}
...
NSymbol.prototype._newNonterminalRule = function (options) {
	 ...
	 * • `rhsIsBinaryTermSet` - Defines `newRule.rhsIsBinaryTermSet` as `true` if the RHS contains two term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by the RHS and prevent `pfsearch` from traversing past this rule's node.
	 *
	 * • `text` - If `newNonterminalRule.rhsIsBinaryTermSet` is `true`, assigns `options.text` to `newRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of text the RHS symbols produce.
	 */
	assignBinaryTermSet(newRule, options)
...
function isIllFormedGramProps(newNonterminalRule) {
	...
	for (var propName in gramProps) {
		...
		if (gramProp && !NSymbol.rhsAcceptsGramProp(newNonterminalRule.rhs, gramProp)) {
			...
			util.logPathAndObject(grammarUtil.stringifyRule(this.name, newNonterminalRule))
			return true
		}
	}
...
/**
 * Checks if `rhs` produces at least one rule with conjugative `text` applicable to the non-person-number grammatical property, `gramProp`. Determines whether `gramProp` is useless on the rule that produces `rhs`, and therefore should be discarded.
 *
 ...
 *
 * `rhs` can produce either terminal rules or nonterminal substitution rules.
 *
 * @static
 * @memberOf NSymbol
 ...
 * @returns {boolean} Returns `true` if `rhs` produces a rule with conjugative `text` applicable to `gramProp`, else `false`.
 */
NSymbol.rhsAcceptsGramProp = function (rhs, gramProp) {
	for (var s = 0, rhsLen = rhs.length; s < rhsLen; ++s) {
		...
		for (var r = 0, rulesLen = rhsRules.length; r < rulesLen; ++r) {
			var ruleText = rhsRules[r].text

			if (ruleText) {
				if (ruleText.constructor === Array) {
					// `rule` is a nonterminal multi-token substitution.
 ...
 *
 * Defines `newNonterminalRule.rhsIsBinaryTermSet` as `true` if the RHS contains two term sets, which instructs `calcHeuristicCosts` to merge the `text` values produced by the RHS and prevent `pfsearch` from traversing past this rule's node.
 *
 * If `newNonterminalRule.rhsIsBinaryTermSet` is `true`, assigns `ruleOptions.text` to `newNonterminalRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of text the RHS symbols produce.
 *
 ...
 */
function assignBinaryTermSet(newNonterminalRule, ruleOptions) {
	// Check if `rhs` contains two terminal rule sets (each created by either `g.newVerb()` or `g.newInvariableTerm()`).
	if (ruleOptions.rhs.length === 2 && ruleOptions.rhs.every(rhsSym => rhsSym.isTermSet)) {
		/**
		 ...
		 */
		newNonterminalRule.rhsIsBinaryTermSet = true
		...
		if (ruleOptions.text) {
			...
			// For use by nonterminal substituions instead of the `text` produced by this rule's RHS (i.e., child nodes).
...
function isIllFormedTextArray(textArray) {
	...
	for (var t = 0; t < textArrayLen; ++t) {
		...
		if (textItem.constructor !== Object && textItem.constructor !== String) {
			util.logError('Item in nonterminal substitution text array is neither String nor Object:', typeof textItem)
			util.log('  ', textItem)
			return true
		}
	}
