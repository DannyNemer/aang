...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (ruleOptions.rhs.every(rhsSym => rhsSym.isTermSet || rhsSym.isTermSequence)) {
		// Temporarily forbid a term sequence from containing multiple verbs. Otherwise, `flattenTermSequence` can not track which input tense applies to which verb in the merged `text` array.
