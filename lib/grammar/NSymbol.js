...
function isIllFormedNonterminalRuleOptions(ruleOptions) {
	...
	for (var r = 0; r < rhsLen; ++r) {
		...
		if (rhsSym.constructor === Object) {
			...
			if (rhsSym.isOptional && rhsLen !== 2) {
				util.logErrorAndPath('RHS symbol with `isOptional` used with non-binary rule:', rhsSym, ruleOptions)
				return true
			}
		}
	}
...
NSymbol.prototype._addOptionalRHSSymbolRules = function (options) {
	...
	// `NSymbol.prototype._newNonterminalRule()` already ensured only binary rules have instances of `RHSSymbolWrapper.isOptional` via `isIllFormedNonterminalRuleOptions()`; i.e., no need to throw an exception here.
	if (rhsLen === 2) {
		for (var rhsIndex = 0; rhsIndex < rhsLen; ++rhsIndex) {
			var rhsSym = rhs[rhsIndex]
			if (rhsSym.constructor === Object && rhsSym.isOptional) {
				...
				optionsClone.rhs = [ rhs[Number(!rhsIndex)] ]
