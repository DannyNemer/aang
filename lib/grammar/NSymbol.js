...
// The grammatical form values supported by the `nonterminalRuleSchema.grammaticalForm` map.
var grammaticalFormValues = [ 'nom', 'obj', 'past', 'infinitive' ]
// The accepted tense values supported by the `nonterminalRuleSchema.acceptedTense` map.
var acceptedTenseValues = [ 'past' ]
...
var nonterminalRuleSchema = {
	/**
	 * The array of `NSymbol`s, `NSymbol` names (strings), and/or nested arrays of either for new binary rules to recursively create. These (nested) sub-rules, however, can only contain a RHS and no other rule properties.
	 *
	 * Accepts string `NSymbol` names for passing an existing rule's `rhs` array for a new rule, as in `user.js`.
	 */
	...
	/**
	 * The map of `rhs` index to the grammatical form to which to conjugate the term sequence in that `rhs` index.
	 *
	 * For example, the following conjugates the term sequence at `rhs[1]` to its 'infinitive' grammatical form:
	 *   `grammaticalForm: { 1: 'infinitive' }`
	 *
	 * The following grammatical form values are supported:
	 ...
	 *
	 * `pfsearch` uses the `grammaticalForm` property to conjugate `text` on rules `rhs` immediately produces (i.e., term sequences in `rhs`), but does not conjugate any subsequent rules.
	 ...
	 */
	grammaticalForm: Object,
	/**
	 * The map of `rhs` index to the grammatical tense for which the verb at that `rhs` index, created with `g.newVerb()`, is accepted when input in that tense, but the form is not enforced if not input in that tense.
	 *
	 * For example, the following accepts the 'past' tense form of the verb at `rhs[0]` if input in past tense, but the form is not enforced otherwise:
	 *   `acceptedTense: { 0: 'past' }`
	 *
...
NSymbol.prototype._newNonterminalRule = function (options) {
	...
	var newRule = {
		 ...
		 */
		gramProps: genGramProps(options),
...
/**
 * Generates the `gramProps` object from `options`, which was passed to `NSymbol.prototype._newTerminalRule()`, for a new nonterminal rule, formatted for use by `pfsearch` and `conjugateText`.
 *
 ...
 */
function genGramProps(options) {
	var gramProps = {}
	for (var rhsIndex = 0, rhsLen = options.rhs.length; rhsIndex < rhsLen; ++rhsIndex) {
		// The grammatical properties with which to conjugate the term sequence at `options.rhs[rhsIndex]`.
		var symGramProps = gramProps[rhsIndex] = {}

		var grammaticalFormVal = options.grammaticalForm && options.grammaticalForm[rhsIndex]
		if (grammaticalFormVal) {
			if (isUnrecognizedVal(grammaticalFormVal, grammaticalFormValues, options, 'grammaticalForm')) {
				throw new Error('Ill-formed nonterminal rule')
			}

			// Map 'infinitive' -> 'pl', to use the `pl` property on text objects created by `g.newVerb()`.
			symGramProps.form = grammaticalFormVal === 'infinitive' ? 'pl' : grammaticalFormVal
		}

		var acceptedTenseVal = options.acceptedTense && options.acceptedTense[rhsIndex]
		if (acceptedTenseVal) {
			if (isUnrecognizedVal(acceptedTenseVal, acceptedTenseValues, options, 'acceptedTense')) {
				throw new Error('Ill-formed nonterminal rule')
			}

			symGramProps.acceptedTense = acceptedTenseVal
		}
	}
...
function isIllFormedGramProps(newNonterminalRule) {
	...
	for (var rhsIndex = 0, rhsLen = newNonterminalRule.rhs.length; rhsIndex < rhsLen; ++rhsIndex) {
		...
		for (var propName in symGramProps) {
			...
			if (NSymbol.isFutileGramProp(rhsSym, gramProp)) {
				util.logErrorAndPath('The RHS symbol at index', rhsIndex, ',', util.stylize(rhsSym), ', produces no rules with conjugate `text` applicable to the grammatical property', util.stylize(propName), '->', util.stylize(gramProp) + ':', newNonterminalRule)
				return true
			}
		}
	}
 ...
 */
NSymbol.isFutileGramProp = function (rhsSym, gramProp) {
	...
	for (var r = 0, rulesLen = rhsRules.length; r < rulesLen; ++r) {
		...
		if (ruleText) {
			...
			for (var s = 0, subRHSLen = subRHS.length; s < subRHSLen; ++s) {
				if (!NSymbol.isFutileGramProp(subRHS[s], gramProp)) {
...
function isUnrecognizedGramProp(gramProp) {
	...
	if (gramProp !== 'pl' && grammaticalFormValues.indexOf(gramProp) === -1) {
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (ruleOptions.rhs.every(rhsSym => rhsSym.isTermSequence)) {
		/**
		 ...
		 *
		 * Whether this rule has `acceptedTense` is immaterial because `grammaticalForm` will always conjugate when `acceptedTense` is absent. If `acceptedTense` is present, will attempt to conjugate each sequence separately, where the input `tense` value of each is on separate nodes.
		 */
		var grammaticalForm = ruleOptions.grammaticalForm
		if (ruleOptions.rhs.length > 1 && ruleOptions.rhs.every(term =>
				...
				// Alert if `ruleOptions` does not specify `grammaticalForm` conjugation for both verbs.
				(!grammaticalForm || (
					(grammaticalForm[0] !== 'past' && grammaticalForm[0] !== 'infinitive') &&
					(grammaticalForm[1] !== 'past' && grammaticalForm[1] !== 'infinitive')
				))) {
...
var termRuleSchema = {
	...
	// The tense of this terminal symbol that is checked if matches the parent nonterminal rules' `acceptedTense` property to determine if this symbol is an acceptable form of the associated verb (though not enforced by default).
	tense: { values: acceptedTenseValues },
