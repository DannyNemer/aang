...
// The grammatical form values supported by the `nontermRuleSchema.grammaticalForm` map.
var grammaticalFormValues = [ 'nom', 'obj', 'past', 'infinitive' ]
// The accepted tense values supported by the `nontermRuleSchema.acceptedTense` map.
...
var nontermRuleSchema = {
	...
	/**
	 * The map of `rhs` index to the grammatical form to which to conjugate the term sequence in `rhs` at that index.
	 *
	...
	/**
	 * The map of `rhs` index to the grammatical tense for which the verb in `rhs` at the index, created with `g.newVerb()`, is accepted when input in that tense, but the form is not enforced if not input in that tense.
	 *
 ...
 *
 * For use to check instances of `nontermRuleSchema.grammaticalForm` and `nontermRuleSchema.acceptedTense`.
 *
 * @private
 * @static
