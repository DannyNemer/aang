...
NSymbol.prototype._newNonterminalRule = function (options) {
	...
	var newRule = {
		...
		noInsertionIndexes: getNoInsertionIndexes(options.rhs),
		/**
	 ...
	 * • {string} [newRule.text] - If `newRule.isTermSequence` is `true`, assigns `options.text` to `newRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of text its RHS produces.
	 */
...
function isIllFormedNewNonterminalRuleOptions(options) {
	...
	if (options.text) {
		if (!options.rhs.every(sym => (sym.constructor === Object ? sym.symbol : sym).isTermSequence)) {
 ...
 */
function getNoInsertionIndexes(rhs) {
 ...
 * • {string} [newNonterminalRule.text] - If `newNonterminalRule.isTermSequence` is `true`, assigns `options.text` to `newNonterminalRule.text` for nonterminal multi-token substitutions, which instructs `pfsearch` to use this rule's display text instead of text its RHS produces.
 *
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (rhs.every(rhsSym => (rhsSym.constructor === Object ? rhsSym.symbol : rhsSym).isTermSequence)) {
		 ...
		 *
		 * Whether this rule has `acceptedTense` is immaterial because `grammaticalForm` will always conjugate when `acceptedTense` is absent. If `acceptedTense` is present, will attempt to conjugate the associated `RHSSymbolWrapper.symbol` term sequence.
		 */
		if (rhs.length > 1 && rhs.every(function (rhsSym) {
					...
					var canConjugateVerb = rhsSym.grammaticalForm === 'past' || rhsSym.grammaticalForm === 'infinitive'
					// Alert if `ruleOptions.rhs` does not specify `grammaticalForm` conjugation for both verbs. Okay if this rule can only conjugate one verb and not the other, because it still prevents two unused instances of `tense` and two verb conjugative `text` objects.
...
function isIllFormedTextArray(textArray) {
	...
	for (var t = 0; t < textArrayLen; ++t) {
		...
		if (t > 0 && textItem.constructor === String && textArray[t - 1].constructor === String) {
			util.logError('Text array contains adjacent (non-concatenated) strings:', textArray)
			return true
		}
 ...
 *
 * FIXME: Extend function's checks with `termSequence.isIllFormedTerminalSymbol()`, even if redundant.
 *
 * @private
 * @static
...
function isIllFormedText(text) {
	...
	else if (textConstructor !== Object && textConstructor !== String) {
		util.logError('`text` is neither string, nor object, nor array:', text)
		return true
	}

	if (textConstructor === Object && Object.keys(text).length < 2) {
		util.logError('Text object contains only one term form:', text)
		return true
	}
