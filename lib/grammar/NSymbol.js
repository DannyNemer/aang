 ...
 *
 * `acceptedTense` specifies the grammatical tense for which the verb `symbol`, created with `g.newVerb()`, is accepted when input in that tense, but the tense verb form is not enforced when not input in that tense nor for insertion rules that use `symbol`. The property is for use when tense is semantically meaningless. For example, consider the following semantically identical queries:
 *   past: "repos I liked"
 ...
 * @property {string} [acceptedTense] The grammatical tense form for which the verb `symbol`, created with `g.newVerb()`, is accepted when input in that tense, but the tense verb form is not enforced when not input in that tense nor for insertion rules that use `symbol`
...
/**
 * Maps `rhsSym`, which is either an `NSymbol` or a string name of an `NSymbol`, to its name.
 *
 ...
 * @private
 * @memberOf NSymbol
 * @param {NSymbol|RHSSymbolWrapper|string} rhsSym The RHS symbol to map, passed in an array of RHS symbols for a new nonterminal rule.
		...
		if (rhsSym.constructor === Object) {
			if (rhsSym.acceptedTense) {
				...
				if (isFutileGramProp(rhsSym.symbol.name, symGramProps.acceptedTense)) {
					util.logErrorAndPath('The RHS symbol,', util.stylize(rhsSym.symbol.name) + ', produces no rules with conjugative `text` applicable to the grammatical property', util.stylize('acceptedTense'), '->', util.stylize(rhsSym.acceptedTense) + ':', rhsSym)
					throw new Error('Ill-formed nonterminal rule')
				}
			}
			...
			if (rhsSym.grammaticalForm) {
				...
				if (isFutileGramProp(rhsSym.symbol.name, symGramProps.form)) {
					util.logErrorAndPath('The RHS symbol,', util.stylize(rhsSym.symbol.name) + ', produces no rules with conjugative `text` applicable to the grammatical property', util.stylize('grammaticalForm'), '->', util.stylize(rhsSym.grammaticalForm) + ':', rhsSym)
					throw new Error('Ill-formed nonterminal rule')
				}
			}
		}
