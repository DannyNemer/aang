...
var nontermRuleSchema = {
	 ...
	 *       "people who `[verb-have]` been ..." -> "people who have been ..."
...
/**
 * Checks if `rhs` produces no rules with a conjugative `text` object applicable to the grammatical form property, `gramProp`. If so, `gramProp` is useless on the rule that produces `rhs`, and therefore should be discarded.
 *
 * Grammatical form properties, defined on the nonterminal rule property `grammaticalForm`, can only conjugate `text` on term sequence which their rule's RHS symbols immediately produce (i.e., its child nodes).
 *
 * • The `grammaticalForm` value 'infinitive' is mapped to 'pl' so that it may use the `pl` property on text objects created by `g.newVerb()`.
 *
 * @static
 * @memberOf NSymbol
 * @param {string[]} rhs The RHS symbols to check if produce a rule with conjugative `text` object applicable to `gramProp`.
 * @param {string} gramProp The grammatical form property name for which to check if can conjugate `rhs`; e.g., 'past', 'obj'.
...
/**
 * Checks if `gramProp` is ill-formed. If so, prints an error.
 *
...
function isUnrecognizedGramProp(gramProp) {
	if (gramProp === 'infinitive') {
		util.logError('Grammatical form property', util.stylize('infinitive'), 'should have been mapped to', util.stylize('pl') + ':', util.stylize(gramProp))
		return true
	}

	if (gramProp !== 'pl' && nontermRuleSchema.grammaticalForm.values.indexOf(gramProp) === -1) {
		util.logError('Unrecognized `gramProps` property:', util.stylize(gramProp))
		return true
	}
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	if (rhs.every(rhsSym => rhsSym.isTermSequence)) {
		// Temporarily forbid a term sequence from containing multiple conjugative terminal rule sets (e.g., verbs, pronouns). Otherwise, `flattenTermSequence` can not track which input tense applies to which verb in the merged `text` array.
		if (rhs.length > 1 && rhs.every(term => term.termSequenceType !== g.termTypes.INVARIABLE)) {
