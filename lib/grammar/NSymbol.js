...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	 ...
	 * `newNonterminalRule.rhs` indexes of which RHS symbol is a term sequence,
	 * if either. Only possible if `newNonterminalRule.rhs` is binary.
	 *
...
/**
 * Checks if `rhsSym` is a conjugative verb that lacks a `grammaticalForm` value
 * (defined on `RHSSymbolWrapper` instances) that can conjugate the `text`
 * object the verb produces.
 *
 * Whether `rhsSym` has `acceptedTense` is immaterial because `grammaticalForm`
 * will always conjugate when `acceptedTense` is absent. If `acceptedTense` is
 * present, `pfsearch` will attempt to use its value to conjugate the associated
 * `RHSSymbolWrapper.symbol` term sequence.
 *
 * For use by `assignTermSequence()` to determine if a provided binary rule
 * contains two verbs, each of which produces a conjugative `text` object that
 * the rule can not conjugate. Prevents `flattenTermSequence` from merging two
 * conjugative verb `text` objects, each of which can have an input `tense`, and
 * passing the merged text array up to a parent node without knowing to which
 * text object each input `tense` applies.
 *
 * @private
 * @static
 ...
 * lacks a `grammaticalForm` value that can conjugate its `text` object, else
 * `false`.
 */
...
/**
 * Gets the existing `NSymbol`, if any, that produces a single binary rule with
 * properties identical to the rule generated from `binaryRuleOptions`.
 *
 * For use by `NSymbol.newBinaryRule()` to check if a binary rule matching
 * `binaryRuleOptions` already exists. This is useful for nested arrays in the
 * `rhs` of nonterminal rule definitions, which is the shorthand to create
 * binary sub-rules, thereby removing the need to reference `NSymbol` instances
 * when used multiple rule definitions.
 *
 * The returned `NSymbol` need not have an identical name as the `NSymbol` that
 * `NSymbol.newBinaryRule()` would have created had no existing identical rule
 * been found. Though, this is currently impossible because all binary symbol
 * names are generated automatically.
 *
 * Invoke this function after recursively creating binary rules for any nested
 * RHS arrays in `binaryRuleOptions.rhs`.
 *
 * @private
 ...
 * @param {number} [options.insertionCost] Specify `createEditRules` can create
 * insertion rules from nonterminal rules that contain this (LHS) `NSymbol` in
 * their `rhs`, with this cost penalty and inserts `options.text`.
 * @param {number} [options.costPenalty] The rule's cost penalty (separate from
 * `options.inserionCost`).
 * @param {Object|string|(Object|string)[]} [options.text] The display text when
 * `options.rhs` is matched in input. Can be omitted for stop-words or
 ...
 * determine if the associated verb is accepted (for display text) in the input
 * tense, but not enforced if not input in that tense.
 ...
 * `options.rhs` in input (e.g., "{user}"). Placeholder symbols generate display
 * text and a semantic argument from matched input.
 * @param {boolean} [options.isSubstitution] Specify `options.text` substitutes
 * `options.rhs` when matched in input. or use by `splitRegexTerminalSymbols` to
 * define nonterminal rules created from splitting multi-token terminal symbols
 * as `rhsDoesNotProduceText`, which tells `pfsearch` to get the `text` from
 * those nonterminal rules and to not traverse their child nodes (i.e., RHS
 * symbols).
 * @param {boolean} [options.isStopWord] Specify `options.rhs` is a stop-word
 * and `options.text` is `undefined`, such that `options.rhs` is excluded from
 * display text when matched in input. For use by `splitRegexTerminalSymbols`,
 * just as `options.isSubstitution`.
 ...
 * @private
 * @static
 * @param {Object} options The `NSymbol.prototype._newTerminalRule()` options
 * object to inspect.
 ...
 *
 * FIXME: Extend checks with `termSequence.isIllFormedTerminalSymbol()`, even if
 * redundant.
 *
 * @private
 * @static
 ...
 *
 * Checks `textObject` for the following illegal conditions: • Has only one
 * property/form. • Has an unrecognized property.
 *
 * @private
 * @static
 ...
 * cost of each nonterminal symbol's rules by an increasing epsilon value. This
 * ensures no nonterminal symbol produces multiple non-edit rules of identical
 * cost.
 *
 * The cost modification introduces entropy to the grammar via multiplicity,
 * which significantly reduces the probability of a given parse node producing
 * multiple subnodes with identical cost or identical cost heuristic. This
 * enables consistent determination of the cheapest path for `pfsearch` to
 * follow or use in its minimum cost heuristic calculations. Otherwise,
 * `pfsearch` would choose whichever path/subnode is arbitrarily first.
 *
 ...
 * `NSymbol.prototype.addRule()` invocation, to evenly distribute the increasing
 * epsilon value for each nonterminal symbol's rules. Though edit-rules are
 * excluded from this operation, they will inherit the diversified costs of
 * their base rules and not lessen the rule cost multiplicity.
 *
 * @static
 * @memberOf NSymbol
 */
 ...
 *
 * It is essential to sort rules by increasing cost so that when `StateTable`
 * groups `ruleProps` instances for edit rules with the same RHS symbols, the
 * cheapest rule is the first `ruleProps` instance in the set. This enables
 * `calcHeuristicCosts` to determine the minimum cost for these `ruleProps` sets
 * by checking the cost of the first object in each set.
 *
