...
/**
 * Checks if `options`, which was passed to `NSymbol.prototype._newNonterminalRule()`, is ill-formed. If not, prints an error.
 *
 * @private
 * @static
...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	// Check if `rhs` contains only terminal rule sets (created by `g.newVerb()` or) or terminal rule sequences (created by `g.newTermSequence()`).
...
NSymbol.prototype._newTerminalRule = function (options) {
	...
	var newRule = {
		...
		// Specify insertions that include this rule, and where the inserted segment is in the right (second) position of a binary rule, can only occur at the end of an input query.
		restrictInsertion: options.restrictInsertion,
	}
...
/**
 * Checks if `options`, which was passed to `NSymbol.prototype._newTerminalRule()`, is ill-formed. If not, prints an error.
 *
 * @private
 * @static
...
function isIllFormedNewTerminalRuleOptions(options) {
	...
	if (options.insertionCost === undefined && options.restrictInsertion) {
