...
NSymbol.prototype._newNonterminalRule = function (options) {
	...
	// Generate the `gramProps` object from instances of `RHSSymbolWrapper` in `options.rhs`. Map such instances to their `RHSSymbolWrapper.symbol` (`NSymbol`) values before invoking `NSymbol.prototype._rhsSymbolToName()` below.
	var gramProps = genGramProps(options)
 ...
 *
 * Maps instances of `RHSSymbolWrapper` in `options.rhs` to their `RHSSymbolWrapper.symbol` (`NSymbol`) values. Invoke this function on `options.rhs` before invoking with `NSymbol.prototype._rhsSymbolToName` in `NSymbol.prototype._newNonterminalRule()`.
 *
...
function genGramProps(options) {
	...
	for (var rhsIndex = 0, rhsLen = options.rhs.length; rhsIndex < rhsLen; ++rhsIndex) {
		...
		var rhsSym = options.rhs[rhsIndex]
		if (rhsSym.constructor === Object) {
			...
			// Replace `rhsSym` in `options.rhs` with its `NSymbol`.
			// Modify before invoking `NSymbol.prototype._rhsSymbolToName()` with `options.rhs` in `NSymbol.prototype._newNonterminalRule()`.
			options.rhs[rhsIndex] = rhsSym.symbol
		}
	}
