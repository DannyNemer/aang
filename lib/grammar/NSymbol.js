...
function assignTermSequence(newNonterminalRule, ruleOptions) {
	...
	// Temporarily save the `newNonterminalRule.rhs` indexes of which RHS symbols are term sequences, irrespective of `newNonterminalRule.isTermSequence`. For use by `createInsertionRules` to mark insertion rules as term sequences for which the non-inserted RHS symbol is a term sequence when the original rule's entire RHS is not a term sequence. `removeTempRulesAndProps` removes this rule property upon completion of grammar generation.
	newNonterminalRule.termSequenceRHSIndexes = rhs.map(function (rhsSym, rhsIndex) {
		if ((rhsSym.constructor === Object ? rhsSym.symbol : rhsSym).isTermSequence) {
			return rhsIndex
		}
	}).filter(rhsIndex => rhsIndex !== undefined)
