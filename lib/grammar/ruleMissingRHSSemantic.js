 ...
 */
function ruleMissingRHSSemantic(ruleSets, nontermSym, rule, _symsSeen) {
	...
	if (!ruleHasRHSSemantic(rule)) {
		...
		if (_symsSeen.indexOf(nontermSym) === -1) {
			...
			// Check if `nontermSym` has an ancestor rule with a LHS semantic, which requires `nontermSym` to produce a RHS semantic, by finding rules with `nontermSym` in their RHS.
			for (var parentNontermSym in ruleSets) {
				...
				for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
					...
					if (rhsIdx !== -1) {
						...
						var demandingRule = ruleMissingRHSSemantic(ruleSets, parentNontermSym, parentRule, _symsSeen)
						if (demandingRule) {
							// Format parsing stack for printing and debugging.
							var node = {}
							node[nontermSym] = rule
