...
function ruleMissingRHSSemantic(ruleSets, nontermSym, rule, printFailedParsePath) {
	...
	if (failedParsePath) {
		if (printFailedParsePath) {
			util.logError('Rule will not produce needed RHS semantic:', util.stylize(nontermSym), '->', rule.rhs.map(util.unary(util.stylize)).join(' '))
	...
	for (var parentNontermSym in ruleSets) {
		...
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rhsIdx !== -1) {
				...
				if (_symsSeen.indexOf(parentNontermSym) === -1) {
					ancestorNode = getAncestorWithLHSSemantic(ruleSets, parentNontermSym, parentRule, _symsSeen)
				}
...
/**
 * Checks if `rule` contains a RHS semantic, has an inserted (RHS) semantic, is a placeholder which generates a RHS semantic argument from input, or is anaphoric and will copy the antecdent semantic.
 *
 * @private
 * @static
