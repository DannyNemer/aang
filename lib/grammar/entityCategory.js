	...
	for (var e = 0, newEntitiesLen = newEntities.length; e < newEntitiesLen; ++e) {
		...
		if (newEntity.constructor === Object) {
			...
			for (var n = 0, entityNamesLen = entityNames.length; n < entityNamesLen; ++n) {
				addEntity(newEntity.display, entityNames[n], entityId, categoryName, entityCategory.isPerson, getAmbiguousUniTokenAliases(entityNames, n), hasAmbiguousMultiTokenAlias(entityNames, n))
			}
		} else {
 ...
 */
function addEntity(displayText, name, id, categoryName, isPerson, ambiguousUniTokenAliases, hasAmbiguousMultiTokenAlias) {
	...
	var entity = {
		...
		id: id,
		// Note: It is likely the following two properties introduce overhead that should be avoided given the obscurity of the edge cases they prevent, and that it would be better to have `Parser` and `pfsearch` absorb additional load, and allowing `pfsearch` to catch the ambiguity at the end.
		...
		// `undefined` if none exist to avoid invoking `indexOf()` for every multi-token entity match (on empty arrays), given this is such an obscure case.
		ambiguousUniTokenAliases: ambiguousUniTokenAliases,
	}
 ...
 *
 * These tokens instruct `Parser.prototype.addMultiTokenEntityNodes()` in `matchTerminalRules` to avoid adding a parse node for a single-token entity match via a token for which a uni-token alias for the same entity was already added. I.e., avoids multiple matches for the same entity (id and display) via different names/aliases over the same one-token span.
 *
 * A single-token match to a multi-token entity name for a token returned by this function will not be added to the parse forest, but only kept for merging with adjacent matches to the same entity object (to form a multi-token match), because the ambiguous uni-token alias will be cheapest.
 *
 ...
 *
 * This property supplements `hasAmbiguousMultiTokenAlias`, which only checks for possible ambiguous matches with other multi-token names because `Parser.prototype.addMultiTokenEntityNodes()` does not have access to the uni-token entity matches to check for ambiguity.
 *
 ...
 */
function getAmbiguousUniTokenAliases(entityNames, entityIdx) {
	var nameTokens = tokenizeEntityName(entityNames[entityIdx])
	var ambiguousUniTokenAliases = []
		...
		for (var o = 0, entityNamesLen = entityNames.length; o < entityNamesLen; ++o) {
			...
			if (otherNameTokens.length === 1) {
				...
				if (nameTokens.indexOf(uniTokenAlias) !== -1) {
					ambiguousUniTokenAliases.push(uniTokenAlias)
				}
			}
		}
	}

	// Return `undefined` if there are no ambiguous uni-token aliases to avoid invoking `indexOf()` for every multi-token entity match (on empty arrays), given this is such an obscure case.
	if (ambiguousUniTokenAliases.length > 0) {
		return ambiguousUniTokenAliases
	}
}
