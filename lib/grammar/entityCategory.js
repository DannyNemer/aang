...
exports.new = function (options) {
	...
	for (var i = 0, newEntitiesLen = newEntities.length; i < newEntitiesLen; ++i) {
		...
		// Check if this entity represents a person and can serve as the antecedent for an anaphoric expression (of matching grammatical person-number).
		if (options.isPerson) {
			// The person-number property to assign to the semantic argument created from this entity (when matched in input), for use in anaphora resolution in `pfsearch`.
