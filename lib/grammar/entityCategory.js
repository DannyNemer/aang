	...
	for (var e = 0, newEntitiesLen = newEntities.length; e < newEntitiesLen; ++e) {
		...
		if (newEntity.constructor === Object) {
			...
			for (var n = 0, entityNamesLen = entityNames.length; n < entityNamesLen; ++n) {
				addEntity(newEntity.display, entityNames[n], entityId, categoryName, entityCategory.isPerson)
			}
		} else {
 ...
 */
function addEntity(displayText, name, id, categoryName, isPerson) {
	// Tokenize entity name to enable partial entity matches, deletables within entities, and out of order token matches. Sort tokens alphabetically to prevent duplicate token index matches in input.
	var nameTokens = name.toLowerCase().split(/\s+/).sort()
