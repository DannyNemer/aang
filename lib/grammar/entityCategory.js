...
var entityCategorySchema = {
	...
	// Specify an instance of this entity category represents a person and can serve as the antecedent for an anaphoric expression (of matching grammatical person-number). Semantic arguments created from these entities (when matched in input) will inherit this property, for use in anaphora resolution in `pfsearch`.
	isPerson: Boolean,
}
...
exports.new = function (options) {
	...
	for (var i = 0, newEntitiesLen = newEntities.length; i < newEntitiesLen; ++i) {
		...
		var entity = {
			...
			// Specify this entity represents a person and can serve as the antecedent for an anaphoric expression (of matching grammatical person-number). The semantic argument created from this entity (when matched in input) will inherit this property, for use in anaphora resolution in `pfsearch`.
