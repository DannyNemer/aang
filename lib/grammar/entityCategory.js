		...
		if (newEntity.constructor === Object) {
			...
			for (var n = 0, entityNamesLen = entityNames.length; n < entityNamesLen; ++n) {
				addEntity({
					...
					ambiguousUniTokenAliases: getAmbiguousUniTokenAliases(entityNames, n),
					hasAmbiguousMultiTokenAlias: hasAmbiguousMultiTokenAlias(entityNames, n),
				})
			}
		} else {
			addEntity({
 ...
 * @param [string[]] [options.ambiguousUniTokenAliases] The uni-token aliases of the same `id` (with a different `name`) that this multi-token `name` contains.
 * @param {boolean} [options.hasAmbiguousMultiTokenAlias] Specify `name` is multi-token and shares a token with another multi-token name for the same `id`.
 */
...
var entitySchema = {
	...
	ambiguousUniTokenAliases: { type: Array, arrayType: String },
	...
	hasAmbiguousMultiTokenAlias: Boolean,
}

function addEntity(options) {
	if (util.illFormedOpts(entitySchema, options)) {
	...
	var entity = {
		...
		// Specify `name` is multi-token and shares a token with another multi-token name for the same `id`. This instructs `matchTerminalRules` to check for multiple matches to the same entity id via different aliases over the same token span, and thereby avoid ambiguity.
		hasAmbiguousMultiTokenAlias: options.hasAmbiguousMultiTokenAlias,
		...
		ambiguousUniTokenAliases: options.ambiguousUniTokenAliases,
	}
