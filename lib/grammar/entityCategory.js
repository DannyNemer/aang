 ...
 * @private
 * @static
 * @param {string[]} entityNames The other entity names to check for potentially ambiguous multi-token aliases.
 * @param {number} n The index of the name within `entityNames`
 * @returns {boolean|undefined} Returns `true` if the name at index `n` within `entityNames` shares a token with another multi-token name within `entityNames` and can lead to ambiguous, multiple matches to the same entity via aliases, else `undefined`.
 */
function hasAmbiguousMultiTokenAlias(entityNames, n) {
	var nameTokens = tokenizeEntityName(entityNames[n])
	var nameTokensLen = nameTokens.length
