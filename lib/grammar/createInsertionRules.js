...
module.exports = function (ruleSets, stopAmbiguity) {
	...
	addInsertionRules(ruleSets, insertionSets, stopAmbiguity)
}
		...
		grammarUtil.forEachRule(ruleSets, function (rule, nontermSym) {
			if (isInsertableRule(insertionSets, rule)) {
				...
				.forEach(function (insertion) {
					// Create a new insertion by merging `rule` with the insertion (i.e., parse tree) created from `rule.rhs`.
					var newInsertion = {
						cost: rule.cost + insertion.cost,
						// The text array for the insertion created from `rule.rhs`. If `rule` is not a nonterminal substitution, conjugate conjugative text objects in `insertion.text`, if any, according to conjugation properties `rule.gramProps` or `insertion.personNumber`, if defined.
						...
						// The created from `rule.semantic` and/or `insertion.semantic`.
						semantic: getInsertionSemantic(rule, insertion),
						tree: insertion.tree,
					}
...
function isInsertableRule(insertionSets, rule) {
	...
	/**
	 * Forbid insertions of stop sequences (i.e., `noText`) because they contain no text or semantics.
	 *
	 * This check is redundant because `NSymbol` already requires stop sequence symbols to have `noInsert`.
	 *
	 * If this rule is binary with one stop sequence, mark the stop sequence RHS symbol as `isOptional` to create a unary rule with just the other RHS symbol (which is identical to inserting the stop sequence symbol that lacks text and semantics).
	 */
...
function mergeInsertionSets(ruleSets, nontermSym, rule, insertionsA, insertionsB) {
	...
	insertionsA.forEach(function (a) {
		insertionsB.forEach(function (b) {
			 ...
			 *
			 * Stop these insertions here, in addition to `addInsertionRules()`, to prevent their inclusion in larger insertions for which the logic in `isSemanticlessClauseInsertion()` can not detect their meaninglessness.
...
/**
 * Gets the semantic for the insertion `findNonterminalRuleInsertions()` creates from `rule` and `insertion`.
 *
 * Returns `undefined` if neither `rule` nor `insertion` has a semantic (because only text is being inserted).
 *
 * @private
 * @static
 * @param {Object} rule The rule from which the `insertion` is created.
 * @param {Object} insertion The insertion created from `rule.rhs`.
 * @returns {Object[]|undefined} Returns the semantic for the new insertion, if any.
 */
...
function addInsertionRules(ruleSets, insertionSets, stopAmbiguity) {
	grammarUtil.forEachRule(ruleSets, function (origRule, nontermSym) {
		...
		for (var insertedSymIdx = 0; insertedSymIdx < rhsLen; ++insertedSymIdx) {
			...
			if (origRule.noTextIndexes && origRule.noTextIndexes.indexOf(insertedSymIdx) !== -1) {
				continue
			}
			...
			if (insertions) {
				for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
					...
					if (origRule.noTextIndexes && origRule.noTextIndexes.indexOf(nonInsertedRHSSymIdx) !== -1) {
						// Remake the `noTextIndexes` map because `nonInsertedRHSSymIdx` may not be 0 (even though the map is guaranteed to only contain this RHS index and not the inserted symbol index).
						newRule.noTextIndexes = [ 0 ]
					}
...
function conjugateText(insertionText, gramProps, personNumber) {
	return insertionText.reduce(function (textArray, text) {
		...
		// Text can not yet be conjugated. Occurs when invoked via `addInsertionRules()` and only conjugating the text of one of the branches, where there exists a verb dependent on the person-number property to be defined in the other branch.
		else {
			textArray.push(text)
		}
