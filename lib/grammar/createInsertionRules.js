		...
		for (var insertedSymIdx = 0; insertedSymIdx < rhsLen; ++insertedSymIdx) {
			...
			if (insertions) {
				for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
					...
					/**
					 * If `origRule` is a binary term set has `text`, then `origRule` is a multi-token nonterminal substitution. `pfsearch` uses the `origRule.text` instead of the `text` of the matched terminal rules `origRule.rhs` produces. For example:
					...
					if (origRule.text && origRule.rhsIsBinaryTermSet) {
						...
						/**
						 * If `origRule` is a binary term set without `text`, mark it as `rhsIsBinaryTermSet` to instruct `calcHeuristicCosts` to traverse the single child node `origRule` produces to get the matched terminal rule `text`. `calcHeuristicCosts` merges this `text` with the insertion `text` according to `newRule.insertedSymIdx`. For example:
