		...
		for (var insertedSymIdx = 0; insertedSymIdx < rhsLen; ++insertedSymIdx) {
			...
			if (insertions) {
				for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
					...
					/**
					 * If `origRule` is a term set sequence and has `text`, then `origRule` is a multi-token nonterminal substitution.
					 *
					 ...
					 */
					if (origRule.text && origRule.isTermSetSequence) {
						newRule.isTermSetSequence = true
						...
						/**
						 * If `origRule` is a term set sequence without `text`, mark it as `isTermSetSequence`.@async
						 *
