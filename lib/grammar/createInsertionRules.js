		...
		for (var insertedSymIdx = 0; insertedSymIdx < rhsLen; ++insertedSymIdx) {
			...
			if (insertions) {
				for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
					...
					/**
					 * Check for `origRule.text` to determine if `origRule` is a multi-token nonterminal substitution, for which `pfsearch` uses the rule's `text` instead of the `text` of the matched terminal rules it produces. For example:
