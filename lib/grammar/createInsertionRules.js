			...
			if (isInsertableRule(insertionSets, rule)) {
				...
				.forEach(function (insertion) {
					...
					if (rule.text) {
						 ...
						 *
						 * Do not conjugate because nonterminal substitution rules can not have `gramProps` that conjugate their text (otherwise the text would not have been saved to those rules unconjugated), and term sequences can neither have nor produce rules with `personNumber`.
						 */
						newInsertion.text = [ rule.text ]
					} else if (rule.rhs.length === 2) {
						/**
						 * `insertion` is a merger of a pair of insertions that binary `rule.rhs` produces (generated by the `mergeInsertionSets()` invocation above).
						 *
						 * `insertion.text` is the merged `text` values of the two insertions, for which `mergeInsertionSets()` conjugated each insertion's `text` separately using the `gramProps` value for the respective RHS index.
						 *
						 * Conjugate any resulting conjugative text objects in `newInsertion.text` with `newInsertion.personNumber`, if any. (The grammatical properties in `gramProps` have conjugative precedence over `personNumber.)
						 */
						newInsertion.text = conjugateText(insertion.text, {}, newInsertion.personNumber)
					} else if (rule.gramProps && rule.gramProps[0]) {
						/**
						 * `rule` is a unary, non-substitution rule.
						 *
						 * Conjugate insertion text using the rule's grammatical properties and the most recent person-number property.
						 */
						newInsertion.text = conjugateText(insertion.text, rule.gramProps[0], newInsertion.personNumber)
					} else {
						/**
