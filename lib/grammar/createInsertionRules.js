...
function isInsertableRule(insertionSets, rule) {
	...
	// Prevent insertion of stop sequences, which lack display text and semantics.
		...
		for (var insertedSymIdx = 0; insertedSymIdx < rhsLen; ++insertedSymIdx) {
			...
			if (insertions) {
				for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
					...
					// Unsure whether to allow partial insertions of multi-symbol stop sequences, but they have yet to be seen. Will likely permit them because can not prevent non-stop term insertions within stop sequences not at the sequence's root rule.
					if (origRule.isStopSequence) {
						util.logError('Attempted insertion of stop sequence:')
						util.log('  Insertion:', util.stylize(insertedRHSSym), '->', insertion)
						util.log('  Original rule:', origRule)
