...
/**
 * Adds new rules to `ruleSets` created from sequences of inserted terminal
 * symbols or empty symbols.
 *
 * @static
...
module.exports = function (ruleSets, stopAmbiguity) {
	...
	// Add new rules to the grammar created from sequences of inserted terminal
	// symbols or empty symbols in `insertionSets`.
		...
		grammarUtil.forEachRule(ruleSets, function (rule, nontermSym) {
			if (isInsertableRule(insertionSets, rule)) {
				rule.rhs.map(function (insertedRHSSym) {
					// Map the insertions for each RHS symbol to new insertions
					// with an extended tree.
				...
				// When `rule` is binary and both RHS symbols have insertions,
				// merge every possible pair of insertions the RHS produces.
				.reduce(mergeInsertionSets.bind(null, ruleSets, nontermSym, rule))
				.forEach(function (insertion) {
					// Create a new insertion by merging `rule` with the
					// insertion (i.e., parse tree) created from `rule.rhs`.
					var newInsertion = {
						cost: rule.cost + insertion.cost,
						// The text array for the insertion created from
						// `rule.rhs`. If `rule` is not a nonterminal
						// substitution, conjugate conjugative text objects in
						// `insertion.text`, if any, according to conjugation
						// properties `rule.gramProps` or
						// `insertion.personNumber`, if defined.
						...
						// The created from `rule.semantic` and/or
						// `insertion.semantic`.
						semantic: getInsertionSemantic(rule, insertion),
						tree: insertion.tree,
					}

					// Add insertion if unique (i.e., unambiguous) and below the
					// cost upper bound.
...
function appendSemantic(existingRule, newRule, insertionSemantic) {
	...
	if (insertionSemantic) {
		...
		// Existing rule has a LHS semantic.
			...
			// If the other symbol does not produce a semantic, then reduce the
			// existing rule's LHS semantic with the inserted (reduced)
			// semantic.
			...
			// Else save the existing semantic and inserted semantic separately.
				...
				// When `newRule.semantic` and `newRule.insertedSemantic` are
				// both defined, the former is a LHS semantic and the latter is
				// a reduced semantic.
