		...
		grammarUtil.forEachRule(ruleSets, function (rule, nontermSym) {
			if (isInsertableRule(insertionSets, rule)) {
				...
				.forEach(function (insertion) {
					// Create new insertion by merging `rule` with the insertion (i.e., parse tree) created from `rule.rhs`.
					var newInsertion = {
						...
						// Get the semantic for the insertion created from `rule` and `insertion`.
