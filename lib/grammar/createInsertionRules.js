		...
		grammarUtil.forEachRule(ruleSets, function (rule, nontermSym) {
			if (isInsertableRule(insertionSets, rule)) {
				...
				.forEach(function (insertion) {
					...
					// Use `rule.text`, if defined, for nonterminal term sequence substitutions.
...
function mergeInsertionSets(ruleSets, nontermSym, rule, insertionsA, insertionsB) {
	...
	insertionsA.forEach(function (a) {
		insertionsB.forEach(function (b) {
			...
			/**
			 * Conjugate each insertion's text separately using the `gramProps` for the respective RHS index. Merge the resultant text arrays.
			 *
			 * Do not yet perform `personNumber` conjugation, which occurs after in `findNonterminalRuleInsertions()`.
			 */
			var mergedTextArray
			if (rule.gramProps) {
				var textA = a.text
 ...
 * @returns {(Object|string)[]} Returns an array of conjugated strings and conjugative text objects (which rely on yet-to-parse person-number properties).
 */
...
function genGramProps(nonInsertedRHSSym, origGramProps) {
	...
	for (var propName in origGramProps) {
		...
		if (gramProp && !isFutileGramProp(nonInsertedRHSSym, gramProp)) {
