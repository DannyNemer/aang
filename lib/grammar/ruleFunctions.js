...
/**
 * Creates a new `Symbol` with terminal rules that produce the provided terminal symbols and do not yield display text, but rather uses text defined on a parent nonterminal rule.
 *
 * @param {...string} [terms] The terminal symbols from which to create terminal rules.
 * @returns {Symbol} Returns the new `Symbol` that produces the terminal rules for `terms` without display text.
 */
g.newBlankSet = function () {
	var terms = Array.prototype.slice.call(arguments)

	return g.newSymbol(terms.join('|'), 'blank').addBlankSet({
		terms: terms,
	})
}
