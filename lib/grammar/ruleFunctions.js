...
var terminalSymbolSchema = {
	symbol: { type: [ String, Symbol ], required: true },
 ...
 *
 * @memberOf Symbol
 * @param {string|Object} symbols The terminal symbols as either strings or objects with properties `symbol` (string or Symbol) and `costPenalty`.
 */
...
var wordSchema = {
	...
	// The terminal symbols to substitute when seen in input, defined as either strings, objects with properties `symbol` (string or Symbol) and `costPenalty`, or an array of a pair of Symbols. The first of `accepted` terminal symbols replaces `substitutions` when seen in input.
	...
	if (options.substitutions) {
		...
		options.substitutions.forEach(function (termSym) {
			if (termSym.constructor === Array) {
				...
				if (termSym.symbol.constructor === Symbol) {
					this.addRule({
						RHS: [ termSym.symbol ],
						text: correctedText,
