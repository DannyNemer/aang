...
var wordSchema = {
	...
	// Enable creation of edit rules using `Symbol` for this cost by inserting the first of the `accepted` terminal symbols.
	insertionCost: Number,
	// The terminal symbols that are accepted when seen in input. Defined as either strings or objects with properties `symbol` and `costPenalty`.
	accepted: { type: Array, arrayType: [ String, Object ], required: true },
	// The terminal symbols to substitute when seen in input. The first of `accepted` terminal symbols replaces `substitutions` when seen in input. Defined as either strings or objects with properties `symbol` and `costPenalty`.
...
Symbol.prototype.addWord = function (options) {
	...
	if (options.substitutions) {
		...
		var firstAcceptedTerm = options.accepted[0]
		var correctedText = firstAcceptedTerm.constructor === Object ? firstAcceptedTerm.symbol : firstAcceptedTerm

		options.substitutions.forEach(function (termSym, i) {
