...
var wordSchema = {
	...
	// The terminal symbols to substitute when seen in input, defined as either strings, Symbols, an array of a pair of Symbols, or objects with properties `symbol` (string, Symbol, or array of Symbols) and `costPenalty`. The first of `accepted` terminal symbols replaces `substitutions` when seen in input.
	substitutions: { type: Array, arrayType: [ String, Symbol, Array, Object ] },
}
...
Symbol.prototype.addWord = function (options) {
	...
	if (options.substitutions) {
		...
		options.substitutions.forEach(function (termSym) {
			if (termSym.constructor === String) {
				this.addRule({
					isTerminal: true,
					RHS: termSym,
					text: correctedText,
				})
			} else if (termSym.constructor === Symbol) {
				this.addRule({
					RHS: [ termSym ],
					text: correctedText,
				})
			} else if (termSym.constructor === Array) {
