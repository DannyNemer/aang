...
exports.newVerb = function (options) {
	...
	var verbSym = createVerbTerminalRuleSet({
...
exports.newTenseVerb = function (options) {
	...
	var presentVerbSym = createVerbTerminalRuleSet({
		symbolName: g.newSymbol(options.symbolName, 'present'),
	...
	var pastVerbSym = createVerbTerminalRuleSet({
		symbolName: g.newSymbol(options.symbolName, 'past'),
	...
	// Create a term sequence that produces the two tense term sets, `presentVerbSym` and `pastVerbSym`, with identical substitution text on each nonterminal rule with all verb forms in `verbFormsTermSet`. Behaves identically to terminal rule sets that `verbTermSet.newVerb()` creates with `verbFormsTermSet`.
	var noTenseVerbSym = createNoTenseVerb(options.symbolName, verbFormsTermSet, presentVerbSym, pastVerbSym)
}

/**
 * Creates a term sequence that produces the two tense term sets, `presentVerbSym` and `pastVerbSym`, with identical substitution text on each nonterminal rule with all verb forms in `verbFormsTermSet`. The returned `NSymbol` behaves identically to terminal rule sets that `verbTermSet.newVerb()` creates with `verbFormsTermSet`.
 *
...
function createNoTenseVerb(symbolName, verbFormsTermSet, presentVerbSym, pastVerbSym) {
	...
	/**
	 * Specify all possible `noTenseVerbSym` reductions (i.e., subtrees) yield only display text, no semantics. Hence, all subtrees `noTenseVerbSym` produces are semantically identical. Enables nesting within term sequences and instructs `flattenTermSequence` to flatten instances of `noTenseVerbSym` to a single terminal parse node with a single display text.
	 *
	 * Assign `isTermSequence` after adding all `noTenseVerbSym` nonterminal rules above because `NSymbol.prototype.addRule()` prevents invocation on `NSymbol` instances with `isTermSequence`.
	 */
	...
	/**
	 * Save `noTenseVerbDisplayText`, which is identical for both nonterminal rules `noTenseVerbSym` produces, as the set's default text.
	 *
	 ...
	 */
	noTenseVerbSym.defaultText = noTenseVerbDisplayText

	return noTenseVerbSym
}
...
function createVerbTerminalRuleSet(options) {
	if (util.illFormedOpts(verbTerminalRuleSetSchema, options) || isIllFormedTerminalRuleSetOptions(options)) {
	 ...
	 *
	 * Assign `isTermSequence` after adding all `verbSym` rules in `addVerbTerminalRuleSet()` because `NSymbol.prototype.addRule()` prevents invocation on `NSymbol` instances with `isTermSequence`.
	 */
	...
	/**
	 * Save `options.displayText`, which is identical for every terminal rule `verbSym` produces, as the terminal rule set's default text.
	 *
