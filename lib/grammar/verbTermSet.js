...
function isIllFormedTerminalRuleSetOptions(options) {
	if (options.termSequenceType === g.termTypes.VERB) {
		...
		if (options.displayText.constructor !== Object) {
			util.logErrorAndPath('`displayText` for terminal rule set of term sequence type', util.stylize(options.termSequenceType), 'is not an object:', util.stylize(options.displayText))
			return true
		}
		...
		if (options.displayText.constructor !== String) {
			util.logErrorAndPath('`displayText` for terminal rule set of term sequence type', util.stylize(options.termSequenceType), 'is not a string:', util.stylize(options.displayText))
			return true
		}
	}
