...
exports.newVerb = function (options) {
	...
	/**
	 * If `options.noPastDisplayText` is `true`, exclude the past tense form, `verbFormsTermSet.past`, from the display `text` object. Instead, substitute the `past` form when input with the verb's correct present tense form.
	 *
	...
	// Create a verb terminal rule set with every verb inflection in `verbFormsTermSet`, where each terminal rule has a conjugative `text` object with the verb forms `oneSg`, `threeSg`, `pl`, and `past`.
	var verbSym = createVerbTerminalRuleSet({
		// If `options.symbolName` is `undefined`, generate the terminal rule set `NSymbol.prototype.name` using the infinitive verb form, `verbFormsTermSet.pl`.
		symbolName: options.symbolName || genVerbTermSetName(verbFormsTermSet),
		termSequenceType: g.termTypes.VERB,
		insertionCost: options.insertionCost,
		verbFormsTermSet: verbFormsTermSet,
		displayText: verbDisplayText,
	})
}
