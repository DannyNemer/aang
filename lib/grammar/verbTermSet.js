...
/**
 * The inflections of a verb, from which `verbTermSet.newVerb()` creates a verb terminal rule set and `verbTermSet.newTenseVerb()` creates separate past and present tense verb terminal rule sets.
 *
...
/**
 * The present tense inflections of a verb, which `verbTermSet.newTenseVerb()` constructs internally when creating the present tense verb terminal rule set.
 *
...
/**
 * The past tense inflections of a verb, which `verbTermSet.newTenseVerb()` constructs internally when creating the past tense verb terminal rule set.
 *
...
exports.newTenseVerb = function (options) {
	...
	// If `options.symbolName` is `undefined`, generate the terminal sequence `NSymbol.prototype.name` using the infinitive verb form, `options.verbFormsTermSet.pl`.
	if (!options.symbolName) {
		options.symbolName = genVerbTermSetName(options.verbFormsTermSet)
	}
	...
	var presentVerbFormsTermSet = {
		...
		threeSg: options.verbFormsTermSet.threeSg,
		pl: options.verbFormsTermSet.pl,
		presentSubjunctive: options.verbFormsTermSet.presentSubjunctive,
		presentParticiple: options.verbFormsTermSet.presentParticiple,
	}
 ...
 *
 * If provided, assigns `insertionCost` to the first terminal rule in the set (i.e., `verbFormsTermSet.oneSg`). Does not assign `insertionCost` if `verbFormsTermSet.oneSg` is `undefined` (i.e., when `verbFormsTermSet` is of the form `PastVerbFormsTermSet`, for a past tense verb terminal rule set).
 *
 * @private
 * @static
...
function addVerbTerminalRuleSet(verbSym, verbFormsTermSet, displayText, insertionCost) {
	 ...
	 *
	 * Assign `insertionCost`, if defined, to the first terminal rule in the set.
	 */
