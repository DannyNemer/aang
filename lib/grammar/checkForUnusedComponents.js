...
module.exports = function (ruleSets, supressWarnings) {
	...
	for (var categorySymbol in entityCategoryCreationLines) {
	...
	for (var integerSymbol in intSymbolsCreationLines) {
	...
	var semanticCreationLines = semantic.creationLines

	for (var semanticName in semanticCreationLines) {
		...
		if (!supressWarnings) {
			...
			util.log('  ' + semanticCreationLines[semanticName])
		}
	}
}
