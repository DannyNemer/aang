...
/**
 * The term wrapper parameterization, for use in `termSequenceSchema.acceptedTerms` by `termSequence.newTermSequence()` and `termSequenceSchema.substitutedTerms` by `NSymbol.prototype.addTermSequenceSubstitutedTerm()`, to specify properties specific to the wrapped `term`.
 *
 * @typedef {Object} TermWrapper
 * @property {string|NSymbol|NSymbol[]} term The terminal symbol, terminal rule set, term sequence, or term sequence pair.
 ...
 *
 * In addition, items in `options.acceptedTerms` and `options.substitutedTerms` may also be the following:
 * 5. `TermWrapper` - A term with a cost penalty and/or insertion restriction, parametrized as follows:
 *   • {string|NSymbol|NSymbol[]} TermWrapper.term - Any of #1-4 above.
 *   • {number} [TermWrapper.costPenalty] - The cost penalty added to the rule's base cost.
 *   • {number[]} [TermWrapper.noInsertionIndexes] - If `TermWrapper.term` is a sequence pair (array), the
 *     index(es) of its term sequence(s) for which to forbid insertion rules.
 *
 ...
 * @param {(string|NSymbol|NSymbol|TermWrapper[])[]} options.acceptedTerms The terminal symbols, terminal rule sets, term sequences, and term sequence pairs (with a cost penalty and/or insertion restriction if `TermWrapper`) to accept when input, as explained above.
 * @param {(string|NSymbol|NSymbol[]|TermWrapper)[]} [options.substitutedTerms] The terminal symbols, terminal rule sets, term sequences, and term sequence pairs (with a cost penalty and/or insertion restriction if `TermWrapper`) to substitute when matched with the first item in `options.acceptedTerms`, as explained above.
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		var costPenalty = 0
		// If `term` is an ordered pair, the pair's indexes for which to instruct `createInsertionRules` to prevent the creation of insertion rules at that index.
		 ...
		 */
			 ...
			 *
			 * Get `defaultText` before modifying `termSeqPair` below for `noInsertionIndexes`.
			 */
			if (i === 0) {
			 ...
			 *
			 * Temporarily map `termSeqPair` to use `RHSSymbolWrapper`. Will later extend `termSequence.newTermSequence()` to accept the `RHSSymbolWrapper` format.
			 */
			...
			// `flattenTermSequence` will traverse this rule's child nodes, merge the `text` values of the matched terminal rules it produces, and create a new, terminal `ruleProps` for the rule's node that instructs `pfsearch` to use as display text and not traverse further.
			termSeqSym.addRule({
				rhs: termSeqPair,
				costPenalty: costPenalty,
			})
...
/**
 * Checks if `termWrapper`, which is a `TermWrapper` instance passed to `termSequence.newTermSequence(termSeqOptions)` in `termSeqOptions.acceptedTerms` or `termSeqOptions.substitutedTerms`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {TermWrapper} termWrapper The substituted term wrapper to inspect.
 * @returns {boolean} Returns `true` if `termWrapper` is ill-formed, else `false`.
 */
 ...
 *
 * @memberOf NSymbol
 * @param {string|NSymbol|NSymbol[]|TermWrapper} term The terminal symbol, terminal rule set, term sequence, or term sequence pair (with a cost penalty and/or insertion restriction if `TermWrapper`) to substitute when matched with `this.defaultText`.
...
NSymbol.prototype.addTermSequenceSubstitutedTerm = function (term) {
	...
	// The cost penalty incurred when `term` is matched in input (and substituted).
	var costPenalty = 0
	// If `term` is an ordered pair, the pair's indexes for which to instruct `createInsertionRules` to prevent the creation of insertion rules at that index.
	var noInsertionIndexes
	if (term.constructor === Object) {
 ...
 * @param {string|NSymbol|NSymbol[]|TermWrapper} term The terminal symbol, terminal rule set, term sequence, or term sequence pair to add as substitute rule to `termSequenceSym`, for use in error messages.
 ...
 *
 * The returned `NSymbol` has the following properties:
 * • name - The concatenation of the `NSymbol` names in `options.termPair` (after flattening nested ordered
 *   pairs).
 * • isTermSequence - `true`. Enables nesting within term sequences and flattening in `flattenTermSequence`.
