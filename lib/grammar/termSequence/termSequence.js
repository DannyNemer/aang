 ...
 *   • {number} [SubstitutedTermWrapper.costPenalty] - The substitution cost penalty added to the rule's cost.
 *   • {number[]} [SubstitutedTermWrapper.noInsertionIndexes] - If `SubstitutedTermWrapper.term` is a sequence pair (array), the index(es) of its term sequence(s) for which to forbid insertion rules.
 *
 ...
 *
 * @memberOf NSymbol
 * @param {string|NSymbol|NSymbol[]|SubstitutedTermWrapper} term The terminal symbol, terminal rule set, term sequence, or term sequence pair to substitute when matched (with a cost penalty and/or insertion restriction if `SubstitutedTermWrapper`) with `this.defaultText`.
...
function isIllFormedSubstitutedTermWrapper(substitutedTerm) {
	if (substitutedTerm.noInsertionIndexes) {
		if (substitutedTerm.term.constructor !== Array) {
			util.logErrorAndPath('`noInsertionIndexes` used with a', util.colors.cyan(substitutedTerm.term.constructor.name), 'instead of a term sequence pair:', substitutedTerm)
			return true
		}

		if (substitutedTerm.noInsertionIndexes.some(idx => substitutedTerm.term[idx] === undefined)) {
			util.logErrorAndPath('`noInsertionIndexes` contains an out-of-bounds term sequence pair index:', substitutedTerm)
			return true
		}
	}
