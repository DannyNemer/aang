...
NSymbol.prototype.addTermSequenceSubstitutedTerm = function (term) {
	if (!this.isTermSequence) {
		util.logErrorAndPath('Attempting to add a substituted term to an `NSymbol` that is not a term sequence (created with `g.newTermSequence()`:', term)
	...
	if (term.constructor === String) {
		...
		if (this.termSequenceType !== exports.termTypes.INVARIABLE && this.termSequenceType !== exports.termTypes.PRONOUN) {
			util.logErrorAndPath('Terminal symbol provided as substituted term for term sequence of type', util.stylize(this.termSequenceType), '(only permitted for sequences of type', util.stylize(exports.termTypes.INVARIABLE), 'or', util.stylize(exports.termTypes.PRONOUN) + '):', util.stylize(term))
			throw new Error('Ill-formed substituted term sequence')
		}
		...
		if (!termSequenceUtil.isTermSequenceType(term, this.termSequenceType)) {
			...
			if (this.termSequenceType === exports.termTypes.PRONOUN) {
				if (!termSequenceUtil.isTermSequenceType(term, exports.termTypes.INVARIABLE)) {
					util.logErrorAndPath('Substituted term sequence', util.stylize(term.name), 'of type', util.stylize(term.termSequenceType), 'neither of required type', util.stylize(this.termSequenceType), 'nor', util.stylize(exports.termTypes.INVARIABLE) + ':', term)
					throw new Error('Ill-formed substituted term sequence')
				}
			}
			 ...
			 */
				...
				util.logErrorAndPath('Substituted term sequence', util.stylize(term.name), 'of type', util.stylize(term.termSequenceType), 'provided to parent term sequence of type', util.stylize(this.termSequenceType) + ':', term)
				throw new Error('Ill-formed substituted term sequence')
			}
		}
