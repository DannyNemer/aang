 ...
 * 2. A terminal rule set created by `verbTermSet.newVerb()` or `pronounTermSet.newPronoun()`.
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		/**
		 * If `term` is a term sequence created by `termSequence.newTermSequence()` or `termSequence.newTermSequenceBinarySymbol()` (or a terminal rule set created by `verbTermSet.newVerb()` or `pronounTermSet.newPronoun()`).
		 *
		...
		options.substitutedTerms.forEach(function (term) {
			 ...
			 */
				...
				if (noInsertionIndexes) {
					...
					newTermSequencePairRule.rhs = newTermSequencePairRule.rhs.map(function (rhsSym, r) {
						return noInsertionIndexes[r] ? { symbol: rhsSym, noInsert: true } : rhsSym
					})
				}
 ...
 * 1. A terminal rule set created by `verbTermSet.newVerb()` or `pronounTermSet.newPronoun()`.
