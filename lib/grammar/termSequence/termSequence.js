...
NSymbol.prototype.addTermSequenceSubstitutedTerm = function (term) {
	if (!this.isTermSequence) {
		util.logErrorAndPath('Attempting to add a substituted term to', util.stylize(this.name), ', which is not a term sequence (created with `g.newTermSequence()`:', util.deleteUndefinedObjectProps(term))
