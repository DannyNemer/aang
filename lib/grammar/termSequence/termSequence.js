 ...
 *
 * These methods create an `NSymbol` for the rule sets, instead of adding rules to an existing `NSymbol`, and forbid adding rules to the returned `NSymbol` afterward to prevent external changes to the rule sets.
 */
 ...
 *
 * @typedef {Object} SubstitutedTerm
 ...
 * 3. 'noun'
 *   • Every term sequence (or terminal rule set) in `options.acceptedTerms` and `options.substitutedTerms` has
 ...
 *   • These restrictions ensure every term sequence produces exactly one noun terminal rule set, created by
...
exports.newTermSequence = function (options) {
	...
	if (options.substitutedTerms) {
		// Create nonterminal substitution rules with `defaultText` as the `text` value for each rule. This instructs `pfsearch` to use display text from these rules and discard the `text` values these rules' descendants produce.
		options.substitutedTerms.forEach(function (term) {
			addTermSequenceSubstitutedTerm(termSeqSym, options, defaultText, term)
		})
	}

	// Extend `termSeqSym` with term sequence properties. Enables nesting of `termSeqSym` in other term sequences with matching term sequence type, and prevents addition of further rules to `termSeqSym`.
...
/**
 * Adds a nonterminal term sequence substitution rule to `termSeqSym` for RHS `term` and `defaultText` as its `text` value.
 *
 * This substitution rule instructs `pfsearch` to use display text from this rule and discard the `text` values this rule's descendants produce.
 *
 * For use by `termSequence.newTermSequence()`.
 *
 * @private
 * @static
 * @param {NSymbol} termSeqSym The term sequence `NSymbol` to add the new rule.
 * @param {Object} options The `termSequence.newTermSequence()` options object to reference the term sequence's type and for inclusion in error messages.
 * @param {Object|string|(Object|string)[]} defaultText The `text` value of the substituted `term`.
 * @param {string|NSymbol|NSymbol[]|SubstitutedTerm} term The terminal symbol, terminal rule set, term sequence, or term sequence pair to substitute when matched (with a cost penalty and/or insertion restriction if `SubstitutedTerm`) with `defaultText`.
 */
function addTermSequenceSubstitutedTerm(termSeqSym, options, defaultText, term) {
 ...
 * 2. 'pronoun'
 *   • Every term sequence `options.termPair` produces includes one term with `termSequenceType` of 'pronoun'
 ...
 * 3. 'noun'
 *   • Every term sequence `options.termPair` produces includes one term with `termSequenceType` of 'noun' and
...
/**
 * Checks if `termSequence` is an ill-formed ordered term sequence pair. If so, prints an error.
 *
 * @private
 * @static
 * @param {NSymbol[]} termSequence The term sequence array to inspect.
 * @returns {boolean} Returns `true` if `termSequence` is ill-formed, else `false`.
 */
function isIllFormedTermSequenceOrderedPair(termSequence) {
	if (termSequence.length !== 2) {
		util.logErrorAndPath('Term sequence array is not an ordered pair:', termSequence)
		return true
	}

	if (termSequence.indexOf(undefined) !== -1) {
		util.logErrorAndPath('Term sequence array contains `undefined`:', termSequence)
		return true
	}
...
/**
 * Extends this symbol with term sequence properties. The properties enable nesting of this symbol in other term sequences with matching `options.type`, and prevents addition of further rules to this symbol.
 *
 ...
 * @memberOf NSymbol
 * @param {Object} options The options object.
 * @param {boolean} [options.isTermSet] Specify this symbol produces only terminal rules, each with the same `text` value.
 * @param {string} options.type The term sequence type.
 * @param {Object|string|(Object|string)[]} options.defaultText The text used for substitution display text when nesting this symbol in the first accepted rule of a term sequence created with `termSequence.newTermSequence()`.
...
NSymbol.prototype._toTermSequence = function (options) {
	 ...
	 *
	 * Assign after adding all rule because `NSymbol.prototype.addRule()` prevents invocation on `NSymbol` instances with `isTermSequence`.
	 */
	...
	/**
	 * Define the term sequence type, which specifies every term sequence this symbol produces is of the specified type, as described in the `termSequence.newTermSequence()` method description.
	 *
...
function isIllFormedToTermSequenceOptions(symbol, options) {
	if (symbol.isTermSequence) {
		util.logError('`NSymbol.prototype._toTermSequence()` invoked on', util.stylize(symbol.name), 'which is already a term sequence:', options)
		return true
	}

	if (symbol.rules.length === 0) {
		util.logError('`NSymbol.prototype._toTermSequence()` invoked on', util.stylize(symbol.name), 'which has no rules:', options)
		return true
	}
