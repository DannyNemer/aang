...
NSymbol.prototype.addTermSequenceSubstitutedTerm = function (term) {
	if (!this.isTermSequence) {
		util.logErrorAndPath('Attempting to add a substituted term to', util.stylize(this.name), ', which is not a term sequence (created with `g.newTermSequence()`:', term.constructor === String ? util.stylize(term) : util.deleteUndefinedObjectProps(term))
		throw new Error('Ill-formed substituted term sequence')
	}

	if (this.isTermSet) {
		util.logErrorAndPath('Attempting to add a substituted term to terminal rule set', util.stylize(this.name) + ':', term.constructor === String ? util.stylize(term) : util.deleteUndefinedObjectProps(term))
		throw new Error('Ill-formed substituted term sequence')
	}

	if (this.termSequenceType === exports.termTypes.STOP) {
		util.logErrorAndPath('Attempting to add a substituted term to', util.stylize(exports.termTypes.STOP), 'sequence', util.stylize(this.name) + ':', term.constructor === String ? util.stylize(term) : util.deleteUndefinedObjectProps(term))
		throw new Error('Ill-formed substituted term sequence')
	}
