...
exports.newTermSequence = function (options) {
	...
	if (options.substitutedTerms) {
		// Create nonterminal substitution rules with `defaultText` as the `text` value for each rule. This instructs `pfsearch` to use display text from these rules and discard the `text` values the rules' descendants produce.
		options.substitutedTerms.forEach(function (term) {
			termSeqSym.addTermSequenceSubstitutedTerm(term, options)
		})
	}
 ...
 *
 * @memberOf NSymbol
 * @param {string|NSymbol|NSymbol[]|SubstitutedTerm} term The terminal symbol, terminal rule set, term sequence, or term sequence pair to substitute when matched (with a cost penalty and/or insertion restriction if `SubstitutedTerm`) with `this.defaultText`.
 ...
 */
NSymbol.prototype.addTermSequenceSubstitutedTerm = function (term, options) {
	if (!this.isTermSequence) {
		util.logErrorAndPath('Attempting to add a substituted term to an `NSymbol` that is not a term sequence (created with `g.newTermSequence()`:', term, options)
	...
	if (term.constructor === String) {
		...
		if (this.termSequenceType !== exports.termTypes.INVARIABLE && this.termSequenceType !== exports.termTypes.PRONOUN) {
			util.logErrorAndPath('Terminal symbol provided as substituted term for term sequence of type', util.stylize(this.termSequenceType), '(only permitted for sequences of type', util.stylize(exports.termTypes.INVARIABLE), 'or', util.stylize(exports.termTypes.PRONOUN) + '):', util.stylize(term), options)
			throw new Error('Ill-formed substituted term sequence')
		}
		...
		if (!termSequenceUtil.isTermSequenceType(term, this.termSequenceType)) {
			...
			if (this.termSequenceType === exports.termTypes.PRONOUN) {
				if (!termSequenceUtil.isTermSequenceType(term, exports.termTypes.INVARIABLE)) {
					util.logErrorAndPath('Substituted term sequence', util.stylize(term.name), 'of type', util.stylize(term.termSequenceType), 'neither of required type', util.stylize(this.termSequenceType), 'nor', util.stylize(exports.termTypes.INVARIABLE) + ':', options)
					throw new Error('Ill-formed substituted term sequence')
				}
			}
			 ...
			 */
				...
				util.logErrorAndPath('Substituted term sequence', util.stylize(term.name), 'of type', util.stylize(term.termSequenceType), 'provided to parent term sequence of type', util.stylize(this.termSequenceType) + ':', options)
				throw new Error('Ill-formed substituted term sequence')
			}
		}
