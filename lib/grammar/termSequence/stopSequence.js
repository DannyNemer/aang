 ...
 * @property {string|NSymbol} term The terminal symbol, terminal rule set, or term sequence to remove when matched in input.
 * @property {number} [costPenalty] The cost penalty added to the rule's base cost.
 */
var stopTermWrapperSchema = {
	term: { type: [ String, NSymbol ], required: true },
 ...
 * 4. `StopTermWrapper` - A term with a cost penalty, parametrized as follows:
 *   • {string|NSymbol|} StopTermWrapper.term - Any of #1-3 above.
 ...
 * @param {(string|NSymbol|StopTermWrapper)[]} options.stopTerms The terminal symbols, terminal rule sets, and term sequences (with a cost penalty if `StopTermWrapper`) to remove when matched in input.
...
var stopSequenceSchema = {
	...
	stopTerms: { type: Array, arrayType: [ String, NSymbol, Object ], required: true },
}
