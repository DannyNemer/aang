 ...
 * 4. An ordered pair containing any combination of #2, #3, or nested ordered pairs from which to recursively create new term sequences of type 'stop'.
 *   • There are no term sequence type restrictions for the base term sequence pair or any nested pairs because these rules produce no display text and therefore need no structure
 *
	...
	for (var t = 0, termSeqLen = termSeqPair.length; t < termSeqLen; ++t) {
		...
		if (termSym.constructor === Array) {
			...
			termSeqPair[t] = g.newBinaryRule({
				...
				rhs: flattenNestedTermSequencePairs(termSym),
