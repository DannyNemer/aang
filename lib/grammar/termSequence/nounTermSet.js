...
/**
 * The inflections of a count noun, from which `nounTermSet.newCountNoun()` creates a noun terminal rule set.
 *
 * Count nouns represent distinct objects, with separate singular and plural forms for modification by numerals or co-occurrence with quantificational determiners (e.g., "every", "each"); e.g., "follower(s)".
 *
 ...
 *
 * @typedef {Object} CountNounFormsSet
 ...
 */
var countNounFormsSetSchema = {
	sg: { type: String, required: true },
	pl: { type: String, required: true },
}

/**
 * Creates an `NSymbol` that produces a terminal rule set for a count noun.
 *
 * Count nouns represent distinct objects, with separate singular and plural forms for modification by numerals or co-occurrence with quantificational determiners (e.g., "every", "each"); e.g., "follower(s)". In contrast, mass nouns (created by `nounTermSet.newMassSet()`) do not have separate forms for plurality nor can occur with numerals or quantificational determiners.
 *
 ...
 * • defaultText - The invariable `text` string for the plural noun form, `options.nounFormsSet.pl`, used as
 *   display text for every (terminal) rule this `NSymbol` produces.
 ...
 * @param {CountNounFormsSet} options.nounFormsSet The count noun terminal symbol set with each noun form.
 * @returns {NSymbol} Returns the new `NSymbol` for the noun terminal rule set.
 */
var countNounSchema = {
	insertionCost: Number,
	nounFormsSet: { type: Object, schema: countNounFormsSetSchema, required: true },
}

exports.newCountNoun = function (options) {
	if (util.illFormedOpts(countNounSchema, options)) {
		throw new Error('Ill-formed count noun')
	}
 ...
 *
 * Mass nouns, or uncountable nouns, do not represent distinct objects, and therefore do not have separate singular and plural forms like count nouns (created by `nounTermSet.newCountSet()`). I.e., each has the syntactic property that any quantity of it is treated as an undifferentiated unit, rather than as something with discrete subsets. E.g., "funding".
 *
 ...
 *
 * For use by `nounTermSet.newCountNoun()` and `nounTermSet.newMassNoun()`.
 *
 * @private
 * @static
