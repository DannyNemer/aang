 ...
 *
 * @typedef {Object} PronounFormsTermSet
 ...
 *
 * Each rule in the set, created from the terms in `options.pronounFormsTermSet`, has an object as its `text` with the properties `nom` and `obj` for the different personal pronoun case forms. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct grammatical case (i.e., display text) according to the `grammaticalForm` property on the (immediate) parent rule.
 *
 ...
 * • defaultText - The conjugative `text` object for the pronoun forms, used as display text for every (terminal) rule this `NSymbol` produces.
 ...
 * @param {number} [options.insertionCost] The insertion cost for the terminal rule set, assigned to the first rule in the set (i.e., `pronounFormsTermSet.nom`). Enables the creation of insertion rules using the new nonterminal symbol that produces this set.
 * @param {PronounFormsTermSet} options.pronounFormsTermSet The pronoun terminal symbol set with each pronoun form.
...
function createPronounTerminalRule(terminalSymbol, displayText, insertionCost) {
	...
	if (termSequenceUtil.isIllFormedTerminalSymbol(terminalSymbol)) {
		throw new Error('Ill-formed pronoun terminal symbol')
	}
