 ...
 *
 * Formats the rule as follows: '[lhs-sym]' -> '[rhs-a]' '[rhs-b]'.
 *
 * @static
 * @memberOf grammarUtil
...
exports.stringifyRule = function (nontermSym, rule) {
	var rhs = rule.rhs.slice()

	// Include insertion text, if any.
	if (rule.insertedSymIdx !== undefined) {
		rhs.splice(rule.insertedSymIdx, 0, rule.text)
	}

	return util.stylize(nontermSym) + ' -> ' + rhs.map(util.unary(util.stylize)).join(' ')
}
...
/**
 * Check if `name`, the name of a new grammar component (e.g., symbol, semantic), already exists in `creationLines`. If so, prints an error with the creation-line (file-path + line-number) of the conflicting component.
 *
 * @static
 * @memberOf grammarUtil
 * @param {string} name The name of the new grammar component to check.
 * @param {Object.<string, string>} creationLines The map of unique component names to creation-lines (file-path + line-number).
 * @param {stirng} componentType The grammarical component type of `name` and `creationLines` (e.g., symbol, semantic).
 * @returns {boolean} Returns `true` if `name` exists in `creationLines`, else `false`.
 */
exports.isDuplicateName = function (name, creationLines, componentType) {
	if (creationLines.hasOwnProperty(name)) {
