...
/**
 * Iterates over rules `nontermSym` produces, invoking `iteratee` for each rule. Invokes `iteratee` with one argument: (rule).
 *
 * @static
 * @memberOf grammarUtil
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
...
/**
 * Checks if `predicate` returns truthy for any rule in `ruleSets`. Stops iteration once `predicate` returns truthy. Invokes `predicate` with one argument: (rule).
 *
 * @static
 * @memberOf grammarUtil
...
/**
 * Creates a string representation of `nontermSym` and `rule` for printing to the console.
 *
...
/**
 * Concatenates variadic string arguments with dashes, and replaces instances of whitespace in the strings with dashes.
 *
 * @static
 * @memberOf grammarUtil
...
/**
 * Converts `string`, the name of a new grammar element, to lowercase, removes any characters the grammar reserves for special purpose (e.g., brackets in `NSymbol.prototype.name`), and checks for whitespace.
 *
 * @static
 * @memberOf grammarUtil
...
/**
 * Checks if `string`, a name of a grammar element, contains reserved characters. If `true`, prints an error indicating the reserved character.
 *
 * @static
 * @memberOf grammarUtil
...
/**
 * Checks if `name`, the name of a new grammar component (e.g., symbol, semantic), already exists in `defLines`. If so, prints an error with the definition line (file-path + line-number) of the conflicting component.
 *
 * @static
 * @memberOf grammarUtil
...
/**
 * Checks if `symA` and `symB` each produce a rule with identical `rhs` arrays. If so, prints an error.
 *
 * @static
 * @memberOf grammarUtil
 * @param {NSymbol} symA The symbol whose rules to check.
 * @param {NSymbol} symB The other symbol whose rules to check.
 * @returns {boolean} Returns `true` if `symA` and `symB` each produce a rule with identical `rhs`, else `false`.
 */
...
/**
 * Merges `leftText` and `rightText`, each of which is a rule `text` property, in the order provided. For use as a rule's `text` value.
 *
 * If both `leftText` and `rightText` are strings, merges the two into a single string separated by a space. Else, at least one is a conjugative object (with different forms of a term) or an array with a conjugative object and at least one string, and creates a new array containing `leftText` and `rightText` (in that order).
 *
 * Have only ever seen text arrays during grammar generation when constructing the `defaultText` values of long (i.e., deeply nested) term sequences (though no such sequences are in use). Have never seen when parsing (via `flattenTermSequence`).
 *
 * @private
 * @static
...
exports.mergeTextPair = function (leftText, rightText) {
	...
	if (leftText.constructor === Array) {
		// Creates a new array consisting of the elements of `leftText` followed by the elements of `rightText` (if `rightText` is an array) or `rightText` itself (if `rightText` is not an array).
