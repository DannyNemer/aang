 ...
 *
 * `grammar.newVerb(options)` - Creates an `NSymbol` that produces a terminal rule set for a pronoun with the necessary text forms for conjugation.
 *
 * `grammar.newPronoun(options)` - Creates an `NSymbol` that produces a terminal rule set for a verb with the necessary text forms for conjugation.
 *
 * `grammar.newTermSequence(options)` - Creates an `NSymbol` that produces a terminal rule sequence forming a term or phrase, comprised of terminal rules, terminal rule sets (e.g., `g.newVerb()`), and nested term sequences.
 *
 ...
 *
 * Invoked a second time in `removeTempRulesAndProps` to ensure the module correctly removed all unused components when initially invoked before `createEditRules`.
 */
...
exports.compileGrammar = function () {
	 ...
	 * Invoke this module before `createEditRules` to avoid creating edit-rules from these components.
	 */
	exports.removeUnusedComponents()
