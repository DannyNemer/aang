 ...
 *
 * `grammar.newTerm()` - Creates an `NSymbol` that produces terminal rules for a set of uninflicted terms.
 *
 ...
 *
 * `grammar.newInvariableTerm()` - Creates an `NSymbol` that produces a terminal rule set for uninflected (i.e., invariable) terms.
 *
 * `grammar.newVerbSet2()` - Creates an `NSymbol` that produces a set of verbs and verb-phrases.
 *
 * `grammar.newTermSetSequence()` - Creates an `NSymbol` that produces a terminal rule sequence forming a term or phrase, comprised of terminal rule sets and nested terminal rule set sequences.
 *
 * `grammar.newBinaryTermSetSequence()` - Creates an `NSymbol` with a single binary rule that produces the two provided terms. Together, those symbols produce a terminal sequence forming a phrase comprised of terminal rule sets and nested terminal rule set sequences.
 */
