 ...
 * `grammar.newVerb()` - Creates an `NSymbol` that produces terminal rule sets for a verb with the necessary text forms for conjugation.
 *
 ...
 *
 * `grammar.isVerb()` - Check if an `NSymbol` is a verb created by `grammar.newVerb()` that has inflected text for past tense.
 *
 * `grammar.isPresentVerb()` - Check if an `NSymbol` is a verb created by `grammar.newVerb()` that lacks inflected text for past tense.
 *
