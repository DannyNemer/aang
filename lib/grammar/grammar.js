...
// Checks `ruleSets` for errors after constructing all rules and edit-rules. Such errors occur across multiple rules and therefore are only evident after rule construction completes. Invoke this module at the conclusion of grammar generation.
...
exports.getGrammar = function () {
	return {
		...
		// Export `<blank>` instead of `[blank-inserted]` because nonterminal symbols in `StateTable` do not have access to the terminal rules they produce. Ergo, `<blank>` would be unaccessable to create its terminal node.
