...
// Splits the grammar's multi-token terminal symbols into rules that yield only uni-token terminal symbols.
exports.splitMutliTokenTerminals = require('./splitMutliTokenTerminals').bind(null, ruleSets)
 ...
 *
 * Sorts entity tokens alphabetically.
 *
...
exports.sortGrammar = function () {
	...
	var entityTokenMap = entityCategory.entities
	Object.keys(entityTokenMap).sort().forEach(function (entityToken) {
		// Sort entities alphabetically.
		var entities = entityTokenMap[entityToken].sort(function (entityA, entityB) {
			return entityA.text - entityB.text
		})

		// Sort entity tokens alphabetically.
		delete entityTokenMap[entityToken]
		entityTokenMap[entityToken] = entities
	})
