...
// Instantiates a new `NSymbol` with a single binary nonterminal rule for the provided options object.
...
/**
 * Extend `grammar` with the following methods:
 * `grammar.termTypes` - The enumerated type of term sequence types.
 *
 * `grammar.newTermSequence(options)` - Creates an `NSymbol` that produces a terminal rule sequence forming a term or phrase, comprised of terminal rules, terminal rule sets, and nested term sequences.
 *
 * `grammar.newTermSequenceBinarySymbol(options)` - Creates an `NSymbol` with a single binary rule with `options.termPair` as its `rhs`, which produces a terminal sequence forming a phrase comprised of terminal rule sets and nested term sequences.
 *
 * `grammar.newCountNoun(options)` - Creates an `NSymbol` that produces a terminal rule set for a count noun.
 *
 * `grammar.newMassNoun(options)` - Creates an `NSymbol` that produces a terminal rule set for a mass (uncountable) noun.
 *
 * `grammar.newPronoun(options)` - Creates an `NSymbol` that produces a terminal rule set for a pronoun with the necessary text forms for conjugation.
 *
 * `grammar.newVerb(options)` - Creates an `NSymbol` that produces a terminal rule set for a verb with the necessary text forms for conjugation.
 *
 * `grammar.newTenseVerb(options)` - Creates two terminal rule sets from the verb forms in `options.verbFormsTermSet`, split by present and past grammatical tense, and a third nonterminal substitution term sequence that produces both without tense restrictions.
 *
 * `grammar.newStopSequence(options)` - Creates an `NSymbol` that produces a stop sequence, which is a term sequence that yields no display text when matched in input.
 *
 * `grammar.isTermSequenceType(term, type)` - Checks if `term` is a term sequence or terminal rule set with term sequence type, `type`.
 *
 * `grammar.isIllFormedTerminalSymbol(terminalSymbol)` - Checks if `terminalSymbol` is ill-formed. If so, prints an error message.
 */
 ...
 * 1. Converts the grammar's regex-style terminal symbols into rules that yield only single-token terminal symbols.
 * 2. Removes ill-formed and unused instances of nonterminal symbols, nonterminal rules, entity categories, integer symbols, and semantics from the grammar.
 * 3. Diversifies the costs of the grammar's non-edit rules.
 * 4. Creates grammar edit-rules derived from insertion and transposition costs, and empty strings in existing rules.
 * 5. Checks the grammar's rules for errors after constructing all non-edit and edit rules.
 * 6. Removes the temporary rules and rule properties used internally for grammar generation.
...
exports.compileGrammar = function () {
	/**
	 * Convert the grammar's regex-style terminal symbols into rules that yield only single-token terminal symbols. This is necessary to enable partial matches and deletions within what would otherwise be regex-style terminal symbols.
	 *
	...
	/**
	 * Remove ill-formed and unused instances of nonterminal symbols, nonterminal rules, entity categories, integer symbols, and semantics from the grammar.
	 *
	...
	/**
	 * Diversify the costs of the grammar's non-edit rules by incrementing the cost of each nonterminal symbol's rules by an increasing epsilon value.
	 *
	 * Invoke this method after invoking `removeUnusedComponents` to evenly distribute the increasing epsilon value, and before adding edit-rules in `createEditRules`.
	 */
	NSymbol.diversifyRuleCosts()

	/**
	 * Create grammar edit-rules derived from insertion and transposition costs, and empty strings in existing rules.
	 *
	 * Invoke this module and after invoking `removeIllFormedRulesAndSyms()` to remove ill-formed nonterminal rules and symbols.
	 */
	require('./createEditRules')(NSymbol._ruleSets)

	/**
	 * Check `ruleSets` for errors after constructing all non-edit and edit rules.
	 *
	 * Invoke this module at the conclusion of grammar generation because the errors occur across multiple rules and therefore are only evident after rule construction completes.
	 */
	require('./checkGrammar')(NSymbol._ruleSets)

	/**
	 * Remove the temporary rules and rule properties used internally for grammar generation from `ruleSets` to exclude them from the output grammar.
	 *
	 * Invoke this module after `checkGrammar` to include line definitions, which `removeTempRulesAndProps` removes, in its error messages.
	 */
