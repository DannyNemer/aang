 ...
 *
 * `grammar.newInvariableTerm()` - Creates an `NSymbol` that produces a terminal rule set for invariable terms (i.e., `pfsearch` can not inflect the terms during parsing).
 *
 ...
 *
 * `grammar.isVerbTerm()` - Check if an `NSymbol` is either a verb terminal rule set (created by `g.newVerb()`) or a verb term sequence (created by `g.newTermSequence()`).
 */
