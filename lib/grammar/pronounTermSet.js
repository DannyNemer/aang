/**
 * Methods, which `grammar` inherits, that create `NSymbol` instances that produce terminal rule sets for pronoun.
 *
 * These methods create an `NSymbol` for pronoun form term sets, as opposed to adding rules to an existing `NSymbol`, and forbid adding rules to the new `NSymbol` afterward to prevent external changes to the rule sets.
 */
...
exports.newPronoun = function (options) {
	...
	pronounSym.addRule(createPronounTerminalRule(pronounFormsTermSet.nom, pronounTextForms, options.insertionCost))
	...
	pronounSym.addRule(createPronounTerminalRule(pronounFormsTermSet.obj, pronounTextForms)
...
/**
 * Checks if `options`, which was passed to `pronounTermSet.newPronoun()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} options The `pronounTermSet.newPronoun()` options object to inspect.
