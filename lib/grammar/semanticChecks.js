	...
	for (var s = 0, nontermSymsLen = _nontermSyms.length; s < nontermSymsLen; ++s) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				...
				// Get ancestor paths of `parentRule` if all have a LHS semantic, which requires `nontermSym` to produce a reduced semantic. Skip if at least one ancestor path does not require a reduced semantic.
