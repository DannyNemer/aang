...
/**
 * Checks if `rule` lacks and cannot produce a reduced semantic required for itself or an ancestor rule in `ruleSets`. Removes `rule` from the grammar if it exists in `ruleSets` (i.e., `rule` is a non-edit rule).
 *
 ...
 *
 * Invoke `removeUnusedNonterminalSyms` after this module to remove any rule-less or unreachable nonterminal symbols that result from the removal of rules here.
 *
 * If a `Category` is created, which adds the essential base rules, but has neither entities nor rules specific to the category, then this module (followed by `removeUnusedNonterminalSyms`) will successfully remove all rules and symbols for that category because none can produce valid parse trees.
 *
 * @static
 ...
 * @param {boolean} [printWarning] Specify printing a warning if a required semantic is missing.
 * @param {boolean} [printFailedParsePath] If `printWarning` is truthy, specify printing a graph representation of the parse tree path if a required semantic is missing.
 ...
 * @param {Object} rule The rule produced by `nontermSym` to check.
 * @param {boolean} [printWarning] Specify printing a warning if a required semantic is missing in recursive `semanticChecks.ruleMissingReducedSemantic()` invocations.
 * @param {boolean} [printFailedParsePath] If `printWarning` is truthy, specify printing a graph representation of the parse tree path if a required semantic is missing in recursive `semanticChecks.ruleMissingReducedSemantic()` invocations.
...
function getAncestorWithLHSSemantic(ruleSets, nontermSym, rule, printWarning, printFailedParsePath, _rulesSeen) {
	...
	for (var parentNontermSym in ruleSets) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				// Check parent rules first for missing a required reduced semantic in order to only remove the problematic rules that always fail, and not the descendant rules that only fail sometimes because of the problematic ancestor rule.
