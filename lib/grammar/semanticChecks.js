...
exports.removeRulesMissingRequiredSemantics = function (ruleSets, suppressWarnings) {
	...
	grammarUtil.forEachRuleSet(ruleSets, function (rules, nontermSym) {
		...
		for (var r = 0; r < rules.length; ++r) {
			if (exports.isRuleMissingReducedSemantic(ruleSets, nontermSym, rules[r], suppressWarnings, true)) {
 ...
 */
exports.isRuleMissingReducedSemantic = function (ruleSets, nontermSym, rule, suppressWarnings, printFailedPaths) {
	...
	var failedParsePaths = getAncestorsWithLHSSemantic(ruleSets, nontermSyms, nontermSym, rule, suppressWarnings, printFailedPaths)
	if (failedParsePaths) {
 ...
 * @param {Object} rule The rule `nontermSym` produces to check.
 * @param {boolean} [suppressWarnings] Specify suppressing warnings from output, in recursive `semanticChecks.isRuleMissingReducedSemantic()` invocations, if `rule` (or ancestor rules) can not produce a required reduced semantic.
 ...
 */
function getAncestorsWithLHSSemantic(ruleSets, nontermSyms, nontermSym, rule, suppressWarnings, printFailedPaths, _rulesSeen) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				...
				if (!parentRule.semanticSafe) {
					if (exports.isRuleMissingReducedSemantic(ruleSets, parentNontermSym, parentRule, suppressWarnings, printFailedPaths)) {
				...
				var parAncestorPathsWithLHSSemantics = getAncestorsWithLHSSemantic(ruleSets, nontermSyms, parentNontermSym, parentRule, suppressWarnings, printFailedPaths, _rulesSeen)
