...
exports.ruleMissingReducedSemantic = function (ruleSets, nontermSym, rule, printWarning, printFailedPath) {
	...
	if (failedParsePath) {
		...
		var ruleExists = ruleIdx !== -1

		// Remove problematic rule if already in grammar (i.e., only non-edit rules).
		if (ruleExists) {
			rules.splice(ruleIdx, 1)
		}

		if (printWarning) {
			util.logWarning((ruleExists ? 'Rule' : 'Edit rule') + ' can not produce required reduced semantic:')
