	...
	for (var s = 0, nontermSymsLen = _nontermSyms.length; s < nontermSymsLen; ++s) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				...
				// Examining parent rules first does not change which rules are removed because all ancestor rules must require a reduced semantic for the base rule to qualify for removal. If a parent rule is removed, it too did not produce a required reduced semantic and would have still required this rule to produce a reduced semantic. This procedure only serves to simplify the output parse tree paths by excluding from others sets of paths those which will be removed on their own.
