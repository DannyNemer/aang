...
function getAncestorsWithLHSSemantic(ruleSets, nontermSym, rule, printWarning, printFailedPaths, _rulesSeen, _nontermSyms) {
	...
	for (var s = 0, nontermSymsLen = _nontermSyms.length; s < nontermSymsLen; ++s) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				// Skip if `parentRule` has a reduced semantic and can satisfy any LHS semantic requirement that precedes it.
				...
				if (isBinary) {
					...
					if (symCanProduceReducedSemantic(ruleSets, otherSym)) {
						// If this binary parent rule can produce a reduced semantic via the other RHS symbol, then return `false` because any LHS semantic in an ancestor rule will be satisfied by `otherSym` and thereby provide a semantically legal tree for the base rule. (Only exclude base rule from the grammar if it always fails.)
						return false
					}
				}
