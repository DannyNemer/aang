 ...
 */
exports.removeRulesMissingRequiredSemantics = function (ruleSets) {
	...
	grammarUtil.forEachRuleSet(ruleSets, function (rules, nontermSym) {
		...
		for (var r = 0; r < rules.length; ++r) {
			if (exports.ruleMissingReducedSemantic(ruleSets, nontermSym, rules[r], true)) {
 ...
 *
 * Invoke `removeIllFormedRulesAndSyms` after this module to remove any rule-less or unreachable nonterminal symbols that result from the removal of rules here.
 *
 * If a `Category` is created, which adds the essential base rules, but has neither entities nor rules specific to the category, then this module (followed by `removeIllFormedRulesAndSyms`) will successfully remove all rules and symbols for that category because none can produce valid parse trees.
 *
 * @static
 ...
 */
function getAncestorWithLHSSemantic(ruleSets, nontermSym, rule, printWarning, printFailedParsePath, _rulesSeen) {
	...
	for (var parentNontermSym in ruleSets) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				...
				var ancestorNode = getAncestorWithLHSSemantic(ruleSets, parentNontermSym, parentRule, printWarning, printFailedParsePath, _rulesSeen)
				if (ancestorNode) {
...
/**
 * Deletes instances of the (temporary) rule property, `semanticSafe`, used by `semanticChecks.ruleMissingReducedSemantic()`.
 *
 * Invoke this method after adding edit-rules, which relies on these properties to avoid edit-rules that can not produce a required reduced semantic.
 *
 * @static
 * @memberOf semanticChecks
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 */
exports.deleteSemanticChecksProps = function (ruleSets) {
