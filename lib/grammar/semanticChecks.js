...
exports.ruleMissingReducedSemantic = function (ruleSets, nontermSym, rule, printWarning, printFailedPaths) {
	...
	if (failedParsePaths) {
		...
		if (printWarning) {
			// Check properties `rule.tree` and `rule.isTransposition` to determine if `rule` is an edit-rule, not whether `rule` is in the grammar, in case the method is improperly removing a parent edit-rule (i.e., not the new rule passed to the method).
