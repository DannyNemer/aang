...
function getAncestorsWithLHSSemantic(ruleSets, nontermSym, rule, printWarning, printFailedPaths, _rulesSeen, _nontermSyms) {
	...
	// The parse tree ancestor paths of `rule` that require a reduced semantic, of which every parent rule of `rule` contributes at least one of the paths. If at least one parent rule does not produce a path requiring a reduced semantic, then the base rule is semantically legal in that path, and return `false`.
