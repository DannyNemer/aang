 ...
 * @param {boolean} [printFailedPath] If `printWarning` is truthy, specify printing a graph representation of the parse tree path if `rule` can not produce a required semantic.
 ...
 */
exports.ruleMissingReducedSemantic = function (ruleSets, nontermSym, rule, printWarning, printFailedPath) {
	...
	var failedParsePath = getAncestorWithLHSSemantic(ruleSets, nontermSym, rule, printWarning, printFailedPath)
	if (failedParsePath) {
 ...
 * @param {boolean} [printFailedPath] If `printWarning` is truthy, specify printing, in recursive `semanticChecks.ruleMissingReducedSemantic()` invocations, a graph representation of the parse tree path if `rule` can not produce a required semantic.
 ...
 */
function getAncestorWithLHSSemantic(ruleSets, nontermSym, rule, printWarning, printFailedPath, _rulesSeen, _nontermSyms) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				...
				if (!parentRule.semanticSafe) {
					if (exports.ruleMissingReducedSemantic(ruleSets, parentNontermSym, parentRule, printWarning, printFailedPath)) {
				...
				if (isBinary) {
					...
					if (symCanProduceReducedSemantic(ruleSets, otherSym)) continue
				}

				// Check if `parentRule` or its ancestors has a LHS semantic (which requires `nontermSym` to produce a reduced semantic).
				var ancestorNode = getAncestorWithLHSSemantic(ruleSets, parentNontermSym, parentRule, printWarning, printFailedPath, _rulesSeen, _nontermSyms)
				if (ancestorNode) {
					// Get the parent node of `rule`, which is the previous node added to the parse tree path.
					var parentNode = ancestorNode.parentNode || ancestorNode
					...
					if (isBinary) {
						...
						parentNode.children = rhsIdx === 0 ? [ node, otherNode ] : [ otherNode, node ]
					} else {
