...
function symCanProduceReducedSemantic(ruleSets, nontermSym, _symsSeen) {
	...
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (ruleHasReducedSemantic(rule)) {
			return true
		} else if (rhsCanProduceReducedSemantic(ruleSets, rule, _symsSeen)) {
			return true
		}
	}
 ...
 */
function rhsCanProduceReducedSemantic(ruleSets, rule, _symsSeen) {
	if (!rule.isTerminal) {
		var rhsSyms = rule.rhs
		for (var s = 0, rhsSymsLen = rhsSyms.length; s < rhsSymsLen; ++s) {
			if (symCanProduceReducedSemantic(ruleSets, rhsSyms[s], _symsSeen)) {
				return true
			}
		}
	}
