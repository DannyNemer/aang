 ...
 */
exports.ruleMissingRHSSemantic = function (ruleSets, nontermSym, rule, printFailedParsePath) {
...
/**
 * Checks if `rule` has a semantic or can produce a rule with a semantic.
 *
 * @static
 * @memberOf semanticChecks
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 * @param {string} rule The nonterminal rule to check.
 * @returns {boolean} Returns `true` if `rule` has a semantic or can produce a rule with a semantic, else `false`.
 */
function ruleCanProduceSemantic(ruleSets, rule, _rulesSeen) {
	...
	// Check if `rule` has a semantic.
	if (rule.semantic || rule.isPlaceholder || rule.anaphoraPersonNumber) {
		return true
	} else if (!rule.isTerminal) {
		// Check if `rule` can produce a rule with a semantic.
		return rule.rhs.some(function (rhsSym) {
			return ruleSets[rhsSym].some(function (subRule) {
				return ruleCanProduceSemantic(ruleSets, subRule, _rulesSeen)
			})
		})
	}
}
