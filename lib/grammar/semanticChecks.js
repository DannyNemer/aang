...
function getAncestorsWithLHSSemantic(ruleSets, nontermSym, rule, printWarning, printFailedPaths, _rulesSeen, _nontermSyms) {
			...
			if (rhsIdx !== -1) {
				...
				if (isBinary) {
					...
					if (symCanProduceReducedSemantic(ruleSets, otherSym)) {
						// Exit function and accept base rule if this binary parent rule can produce a reduced semantic via the other RHS symbol, because any LHS semantic in an ancestor rule will be satisfied by `otherSym` and thereby provide a semantically legal path for the base rule. (Only exclude base rule from the grammar if it always fails.)
						return false
					}
				}
				...
				if (parAncestorPathsWithLHSSemantics) {
					// A node formatted for printing in a parse tree path graph representation. Does not matter than this node will be shared by multiple ancestor paths, as the portion of the path that precedes `node` varies while the remainder as it unwinds the recursive calls to return to the base rule is identical.
	...
	// Check if somehow fails to find any rule that produces `nontermSym`.
