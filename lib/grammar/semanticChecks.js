...
function getAncestorsWithLHSSemantic(ruleSets, nontermSyms, nontermSym, rule, suppressWarnings, printFailedPaths, _rulesSeen) {
	...
	// If `ancestorPathsWithLHSSemantics` is empty, then no parent rules remain for `nontermSym` due to recursive `semanticChecks.ruleMissingReducedSemantic()` invocations that removed the parent rules.
