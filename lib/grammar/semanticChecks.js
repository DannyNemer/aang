 ...
 * @param {boolean} [supressWarnings] Specify suppressing warnings for rules that can not produce a required reduced semantic.
 * @returns {boolean} Returns `true` if any rules were removed, else `false`.
 */
exports.removeRulesMissingRequiredSemantics = function (ruleSets, supressWarnings) {
	...
	grammarUtil.forEachRuleSet(ruleSets, function (rules, nontermSym) {
		...
		for (var r = 0; r < rules.length; ++r) {
			if (exports.ruleMissingReducedSemantic(ruleSets, nontermSym, rules[r], !supressWarnings, true)) {
 ...
 * @param {boolean} [supressWarnings] Specify suppressing warnings if `rule` (or ancestor rules) can not produce a required reduced semantic.
 * @param {boolean} [printFailedPaths] If `supressWarnings` is falsey and `rule` (or ancestor rules) can not produce a reduced semantic, specify printing graph representations of the parse tree paths that require `rule` (or ancestor rules) to produce a reduced semantic.
 ...
 */
exports.ruleMissingReducedSemantic = function (ruleSets, nontermSym, rule, supressWarnings, printFailedPaths) {
	...
	var failedParsePaths = getAncestorsWithLHSSemantic(ruleSets, nontermSym, rule, supressWarnings, printFailedPaths)
	if (failedParsePaths) {
 ...
 * @param {Object} rule The rule `nontermSym` produces to check.
 * @param {boolean} [supressWarnings] Specify suppressing warnings, in recursive `semanticChecks.ruleMissingReducedSemantic()` invocations, if `rule` (or ancestor rules) can not produce a required reduced semantic.
 * @param {boolean} [printFailedPaths] If `supressWarnings` is falsey and `rule` (or ancestor rules) can not produce a reduced semantic, specify printing graph representations, in recursive `semanticChecks.ruleMissingReducedSemantic()` invocations, of the parse tree paths that require `rule` (or ancestor rules) to produce a reduced semantic.
 ...
 */
function getAncestorsWithLHSSemantic(ruleSets, nontermSym, rule, supressWarnings, printFailedPaths, _rulesSeen, _nontermSyms) {
		...
		for (var p = 0; p < parentRules.length; ++p) {
			...
			if (rhsIdx !== -1) {
				...
				if (!parentRule.semanticSafe) {
					if (exports.ruleMissingReducedSemantic(ruleSets, parentNontermSym, parentRule, supressWarnings, printFailedPaths)) {
				...
				// Get ancestor paths of `parentRule` if all have a LHS semantic, which requires `rule` produce a reduced semantic. Exit function if at least one ancestor path does not require a reduced semantic (i.e., provides a legal path).
				var parAncestorPathsWithLHSSemantics = getAncestorsWithLHSSemantic(ruleSets, parentNontermSym, parentRule, supressWarnings, printFailedPaths, _rulesSeen, _nontermSyms)
