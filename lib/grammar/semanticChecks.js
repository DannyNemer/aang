 ...
 */
function getAncestorWithLHSSemantic(ruleSets, nontermSym, rule, _rulesSeen) {
	var isBaseRule = !_rulesSeen

	// Track visited rules to prevent infinite recursion. Track rules instead of symbols to not skip recursive rules.
	if (!_rulesSeen) {
		_rulesSeen = [ rule ]
	...
	for (var parentNontermSym in ruleSets) {
		...
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rhsIdx !== -1) {
				...
				var ancestorNode = getAncestorWithLHSSemantic(ruleSets, parentNontermSym, parentRule, _rulesSeen)
				if (ancestorNode) {
