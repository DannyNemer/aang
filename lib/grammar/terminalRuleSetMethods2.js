 ...
 *
 * All rules which the new `NSymbol` produces are marked `rhsIsBinaryTermSet`, which instructs `calcHeuristicCosts` to do the following:
 ...
 * @returns {NSymbol} Returns the `NSymbol` for the new verb set.
 */
var verbSetSchema = {
	symbolName: { type: String, required: true },
	insertionCost: Number,
...
exports.newVerbSet2 = function (options) {
	...
	options.acceptedVerbs.forEach(function (verb, i) {
		 ...
		 *
		 * For example:
		 *   `[create]` -> `[make]` -> "make", "makes", "made", text: `{make-verb-forms}`
		 */
		if (verb.isTermSet && verb.termSetType === 'verb') {
			/**
			 * Even though this rule only produces a single matched terminal rule, it is still assigned `rhsIsBinaryTermSet` to instruct `calcHeuristicCosts` to bring the text-verb-forms object up to this rule's node level, so that `gramProps` may conjugate it (`gramProps` only conjugate the immediate child nodes).
			 *
	...
	if (options.substitutedVerbs)	{
		options.substitutedVerbs.forEach(function (verb) {
			 ...
			 *
			 * For example:
			 *   `[like]` -> `[love]`, text: `{like-verb-forms}` -> "love", "loves", "loved"
			 */
			if (verb.isTermSet && verb.termSetType === 'verb') {
	...
	// Assign rule set property to the `NSymbol` after adding all rules in the verb set, to prevent further adding rules.
	verbSetSym.isVerbSet = true
