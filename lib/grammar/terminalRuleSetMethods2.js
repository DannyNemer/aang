 ...
 * 3. `termSetType` - 'verb'.
 * 4. `verbTense` - `options.tense`, if specified.
...
exports.newVerb = function (options) {
	...
	if (options.tense === 'past') {
		...
		var verbSym = this.newInvariableTerm(pastVerbInvariableTermSet)
...
function createVerbTerminalRule(terminalSymbol, verbTextForms, tense) {
	...
	if (verbSchema.tense.indexOf(tense) === -1) {
 ...
 * • If `options.verbTense` is specified, checks every verb is of matching grammatical tense.
 *
 * The returned `NSymbol` has the following properties:
 * 1. `name` - `options.symbolName`.
 ...
 * 5. `verbTense` - `options.tense`, if specified.
 *
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
 ...
 * @param {boolean} [options.verbTense] If `options.isVerb` is `true`, specify the grammatical tense, 'past' or 'present', of every verb the new `NSymbol` produces (each created by `terminalRuleSetMethods.newVerb(verbOptions)` with matching `verbOptions.tense`).
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		if (term.isTermSet || term.isTermSequence) {
			...
			// If `options.verbTense` is defined and `term` is a verb or verb sequence, check `term` is of matching grammatical tense.
	...
	// If `options.verbTense` is specified, every verb `termSeqSym` produces only contains verb forms for the given grammatical tense; i.e., lacks terminal rules for the opposite tense.
 ...
 * • If `options.isVerb` is falsey, checks every term sequence contains zero verb terminal rule sets.
 * • If `options.verbTense` is specified, checks every verb `options.termPair` produces is of matching grammatical tense.
 *
...
exports.newTermSequenceBinarySymbol = function (options, _isNestedTermSequence) {
	...
	if (options.verbTense && !options.isVerb) {
		util.logErrorAndPath('`verbTense` specified for binary term sequence with falsey `isVerb`:', options)
		throw new Error('Ill-formed binary term sequence')
	}
 ...
 * • If `verbTense` is defined, checks every verb `termSeqPair` produces is of matching grammatical tense.
 *
