 ...
 *
 * If `_isNestedTermSequence` is `true`, manually checks if `options.termPair` contains a verb to determine the `isVerb` value for the returned `NSymbol`. For use when `options.termPair` was defined as a nested term pair within another term sequence pair passed to this method or `terminalRuleSetMethods.newTermSequence()`. `options.isVerb` exists only to check if an entire sequence contains or lacks a single verb, not every bigram within the sequence; e.g., "in common" within the verb-phrase "have in common" lacks a verb.
 *
 * The returned `NSymbol` has the following properties:
...
exports.newBinaryTermSequence = function (options, _isNestedTermSequence) {
	...
	if (_isNestedTermSequence) {
		 ...
		 *
		 * The `flattenTermSequencePair()` invocation above prevents both items in `termSeqPair` from both being verbs.
		 */
...
function flattenTermSequencePair(termSeqPair, isVerb) {
	...
	for (var s = 0; s < termSeqLen; ++s) {
		...
		if (termSym.constructor === Array) {
			...
			termSeqPair[s] = exports.newBinaryTermSequence({ termPair: termSym })
