...
exports.newVerbSet2 = function (options) {
	...
	options.acceptedVerbs.forEach(function (verb, i) {
		// If `verbSym` is a verb terminal rule set created by `terminalRuleSetMethods.newVerb()`.
		if (verbSym.isTermSet && verbSym.type === 'verb') {
			// Even though the `text` object of every terminal rule in `verbSym` is identical, do not (yet) assign the `text` to this rule. `calcHeuristicCosts` will still need to traverse down this rule's respective node to get the input tense.
			verbSetSym.addRule({
				rhs: [ verbSym ],
			})
			...
			if (i === 0) {
				...
				// The `text` object of every terminal rule in `verbSym`, a verb terminal rule set, is identical.
				defaultText = verbSym.defaultText
			}
		}
	})
