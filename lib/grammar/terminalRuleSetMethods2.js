...
exports.newVerb = function (options) {
	...
	/**
	 * If `options.tense` is 'past', create a terminal rule set with `terminalRuleSetMethods.newInvariableTerm()`, where each rule has the string `options.verbFormsTermSet.past` has its `text` value.
	 *
	 ...
	 *
	 * Overwrite `termSetType`, set by `terminalRuleSetMethods.newInvariableTerm()` in `newPastVerb()`.
	 */
 ...
 *
 * Uses `terminalRuleSetMethods.newInvariableTerm()` to create the terminal rule set, where each rule has the string `options.verbFormsTermSet.past` has its `text` value, and `pfsearch` does not conjugate the verb set.
 *
...
function newPastVerb(options) {
	...
	var pastVerbInvariableTermSet = {
		symbolName: options.symbolName,
		acceptedTerms: [ verbFormsTermSet.past ],
	}

	if (options.insertionCost !== undefined) {
		pastVerbInvariableTermSet.insertionCost = options.insertionCost
	}

	if (verbFormsTermSet.pastParticiple) {
		pastVerbInvariableTermSet.substitutedTerms = [ verbFormsTermSet.pastParticiple ]
	}

	// `terminalRuleSetMethods.newVerb(options)` will overwrite the `termSetType` property to 'verb', which `terminalRuleSetMethods.newInvariableTerm()` sets to 'term'.
	return g.newInvariableTerm(pastVerbInvariableTermSet)
}

/**
 * Checks if `newVerbOptions`, which was passed to `terminalRuleSetMethods.newVerb()`, is ill-formed. If not, prints an error.
 *
 * @private
 * @static
 * @param {Object} newVerbOptions The `terminalRuleSetMethods.newVerb()` options object to inspect.
 * @returns {boolean} Returns `true` if `newVerbOptions` is ill-formed, else `false`.
 */
function isIllFormedNewVerbOptions(newVerbOptions) {
	var verbFormsTermSet = newVerbOptions.verbFormsTermSet

	if (newVerbOptions.tense) {
		// Properties `verbSchema.tense` and `verbSchema.noPastDisplayText` are mutually exclusive.
		if (newVerbOptions.noPastDisplayText) {
			util.logErrorAndPath('Verb with `tense` defines `noPastDisplayText` as `true`:', newVerbOptions)
			return true
		}

		// Print specific errors for verb of a tense containing the forms of the other tense.
		if (newVerbOptions.tense === 'present') {
			for (var verbForm in pastVerbFormsTermSetSchema) {
				if (verbFormsTermSet.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Present tense verb has forbidden \`${verbForm}\` verb form:`, newVerbOptions)
					return true
				}
			}
		...
		} else if (newVerbOptions.tense === 'past') {
			for (var verbForm in presentVerbFormsTermSetSchema) {
				if (verbFormsTermSet.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Past tense verb has forbidden \`${verbForm}\` verb form:`, newVerbOptions)
					return true
				}
			}
 ...
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
 * @param {number} [options.insertionCost] The insertion cost for the terminal rule set, assigned to the first term in `options.acceptedTerms`. Enables the creation of insertion rules using the new `NSymbol` which produces this set (i.e., the LHS symbol).
...
/**
 * Checks if `terminalSymbol` contains any non-alphabetic characters. If so, prints an error message.
 *
 * @private
 * @static
 * @param {string} terminalSymbol The terminal symbol to check.
 * @returns {boolean} Returns `true` if `terminalSymbol` contains any non-alphabetic characters, else `false`.
 */
function isIllFormedTerminalSymbol(terminalSymbol) {
	...
	if (invalidCharIndex !== -1) {
		util.logErrorAndPath('Terminal symbol', util.stylize(terminalSymbol), 'contains non-alphabetic character:', util.stylize(terminalSymbol[invalidCharIndex]))
		return true
	}
 ...
 * 5. A terminal symbol with a (substitution) cost penalty.
 *
 ...
 * 3. `defaultText` - The `defaultText` value (or merger of `defaultText` values) of the first term set or term sequence in `options.acceptedTerms`. For use when nesting this `NSymbol` in another term sequence.
 * 4. `isVerb` - `options.isVerb`.
 * 5. `verbTense` - `options.tense`, if defined.
 *
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
 ...
 * @param {(string|SubstitutedTerm|NSymbol|NSymbol[])[]} [options.substitutedTerms] The terminal symbols, terminal symbols with cost penalties, term sets, and term sequences to substitute when input with the first item in `options.acceptedTErms`, parameterized as described above.
...
exports.newTermSequence = function (options) {
	...
	if (options.insertionCost !== undefined && !options.acceptedTerms.some(term => term.constructor === String)) {
		util.logErrorAndPath('Term sequence', util.stylize(options.symbolName), 'has an `insertionCost` but no terminal symbol (i.e., string) in `options.acceptedTerms` to which to assign it:', options)
		throw new Error('Ill-formed term sequence')
	}
	...
	if (options.substitutedTerms) {
		...
		options.substitutedTerms.forEach(function (term) {
			...
			/**
			 * If `term` is a terminal symbol with a cost penalty.
			 *
			 * A substitution example:
			 *   `[prep-participant]` -> "at", text: "of", cost: 0.5
			 */
				...
				termSeqSym.addRule({
					...
					rhs: term.term,
					text: defaultText,
					// The substitution cost penalty incurred when this terminal rule is matched in input (and substituted).
					costPenalty: term.costPenalty
				})
			}
 ...
 * 2. `isTermSequence` - `true`. Prevents adding more rules to this `NSymbol`.
 * 3. `defaultText` - The merger of the `defaultText` values of the terms in `options.termPair`. For use when nesting this `NSymbol` in another term sequence.
 * 4. `isVerb` - `options.isVerb`, or if `_isNestedTermSequence` is `true` and `options.termPair` contains a verb.
 * 5. `verbTense` - `options.tense`, or if `_isNestedTermSequence` is `true` and `options.termPair` contains a verb with `verbTense`.
 *
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object.
...
function isIllFormedTermSequenceItem(term, isVerb, verbTense) {
	if (isVerb) {
		...
		if (verbTense) {
			if (!term.verbTense) {
				util.logError('Verb', util.stylize(term.name), 'lacks specified `verbTense` for verb term sequence:', util.stylize(verbTense), term)
				return true
			}

			if (verbTense !== term.verbTense) {
				util.logError('Verb tense of', util.stylize(term.name), '->', util.stylize(term.verbTense), 'does not match specified `verbTense` for verb term sequence:', util.stylize(verbTense), term)
				return true
			}
		} else if (term.verbTense) {
			util.logError('Verb with `verbTense` provided to verb term sequence without specified `verbTense`:', util.stylize(term.name), '->', util.stylize(term.verbTense), term)
			return true
		}
	} else if (verbTense) {
		util.logError('`verbTense` provided with falsey `isVerb`:', util.stylize(verbTense))
...
function isIllFormedTermSequencePair(termSeqPair, isVerb, verbTense, isNestedTermSequence) {
	...
	if (termSeqPair.every(exports.isVerbTerm)) {
		util.logError('Two verbs provided in a single term sequence pair:', termSeqPair)
		return true
	}
	...
	if (termSeqPair.every(term => term.verbTense)) {
		util.logError('Two instances of `verbTense` in a single term sequence pair:', termSeqPair)
		return true
	}
	...
	if (isVerb) {
		...
		if (verbTense) {
			if (!termSeqPairTense) {
				util.logError('Term sequence pair lacks specified `verbTense` for verb term sequence:', util.stylize(verbTense), termSeqPair)
				return true
			}

			if (verbTense !== termSeqPairTense) {
				util.logError('Verb tense of', tenseRuleStr, 'does not match specified `verbTense` for verb term sequence pair:', util.stylize(verbTense), termSeqPair)
				return true
			}
		} else if (termSeqPairTense) {
			util.logError('Verb with `verbTense` provided to verb term sequence pair without specified `verbTense`:', tenseRuleStr, termSeqPair)
			return true
		}
	} else if (verbTense) {
		util.logError('`verbTense` provided with falsey `isVerb`:', util.stylize(verbTense))
...
function isForbiddenVerb(term) {
	...
	if (term.verbTense) {
		util.logError('`verbTense` on non-verb term:', term)
		return true
	}
