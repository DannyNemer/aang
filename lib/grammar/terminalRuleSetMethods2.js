...
exports.newVerbSet2 = function (options) {
	...
	options.acceptedVerbs.forEach(function (verb, i) {
		...
		if (verb.isTermSet && verb.type === 'verb') {
			// Even though the `text` object of every terminal rule in `verb` is identical, do not (yet) assign the `text` to this rule. `calcHeuristicCosts` will still need to traverse down this rule's respective node to get the input tense.
			verbSetSym.addRule({
				rhs: [ verb ],
			})
