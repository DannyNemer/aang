	...
	if (options.tense) {
		...
		if (options.tense === 'present') {
			for (var verbForm in pastVerbFormsTermSetSchema) {
				if (options.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Present tense verb has forbidden \`${verbForm}\`:`, options)
					throw new Error('Ill-formed present tense verb')
				}
			}

			if (util.illFormedOpts(presentVerbFormsTermSetSchema, options.verbFormsTermSet)) {
			...
			for (var verbForm in presentVerbFormsTermSetSchema) {
				if (options.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Past tense verb has forbidden \`${verbForm}\`:`, options)
					throw new Error('Ill-formed past tense verb')
				}
			}

			if (util.illFormedOpts(pastVerbFormsTermSetSchema, options.verbFormsTermSet)) {
				throw new Error('Ill-formed past tense verb forms term set')
			}
		}
	} else if (util.illFormedOpts(verbFormsTermSetSchema, options.verbFormsTermSet)) {
