 ...
 * 3. `termSetType` - 'verb'.
 * 4. `defaultText` - The conjugative `text` object for the forms of this verb, which is for every terminal rule this `NSymbol` produces. For use when nesting this `NSymbol` in a term sequence.
 ...
 * @param {string} [options.tense] Specify the tense, 'present' or 'past', for which to limit the verb terminal rule set; i.e., exclude all verb forms for the opposite tense. If 'present', `options.verbFormsTermSet` must adhere to `PresentVerbFormsTermSet`. If 'past', `options.verbFormsTermSet` must adhere to `PastVerbFormsTermSet`.
...
exports.newVerb = function (options) {
	...
	// If specified, `verbSym` only produces terminal rules for the given tense; i.e., lacks terminal rules for the opposite tense.
 ...
 * @param {boolean} [options.verbTense] If `options.isVerb` is `true`, specify the tense, 'past' or 'present', of every verb the new `NSymbol` produces (each created by `terminalRuleSetMethods.newVerb(verbOptions)` with matching `verbOptions.tense`).
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		if (term.isTermSet || term.isTermSequence) {
			...
			// If `options.verbTense` is defined and `term` is a verb or verb sequence, check `term` is of matching tense.
	...
	// If `options.verbTense` is specified, every verb `termSeqSym` produces only contains verb forms for the given tense; i.e., lacks terminal rules for the opposite tense.
...
exports.newTermSequenceBinarySymbol = function (options, _isNestedTermSequence) {
	...
	if (_isNestedTermSequence) {
		/**
		 * Manually check whether `termSeqSym` produces a verb, and whether that verb has `verbTense`, when `options.termPair` was defined as a nested term pair within another term sequence pair passed to this method or `terminalRuleSetMethods.newTermSequence()`. This is necessary because `options.isVerb` only specifies if the entire base sequence (after flattening) contains or lacks a single verb, not every bigram within the sequence.
		 *
		 ...
		 *
		 * Perform this check after invoking `flattenTermSequencePair()` with `termSeqPair` above to ensure the term sequence is flattened and valid (e.g., `termSeqPair` items are not both verbs, `termSeqPair` do not both have a tense).
		 */
 ...
 * @param {boolean} verbTense If `isVerb` is `true`, specify every verb `termSeqPair` produces is of the tense 'past' or 'present' (each created by `terminalRuleSetMethods.newVerb(verbOptions)` with matching `verbOptions.tense`).
 ...
 * @param {string} [verbTense] If `isVerb` is `true`, specify `term` should be of the tense 'past' or 'present'.
