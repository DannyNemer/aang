...
function isIllFormedTermSequenceItem(term, isVerb, verbTense) {
	if (isVerb) {
		...
		if (verbTense) {
			if (!term.verbTense) {
				util.logError('Verb', util.stylize(term.name), 'lacks specified verb term sequence `verbTense`:', util.stylize(verbTense), term)
				return true
			}

			if (verbTense !== term.verbTense) {
				util.logError('Verb tense of', util.stylize(term.name), '->', util.stylize(term.verbTense), 'does match specified verb term sequence `verbTense`:', util.stylize(verbTense), term)
				return true
			}
...
function isIllFormedTermSequencePair(termSeqPair, isVerb, verbTense, isNestedTermSequence) {
	...
	if (isVerb) {
		...
		if (verbTense) {
			if (!termSeqPairTense) {
				util.logError('Term sequence pair lacks specified verb term sequence `verbTense`:', util.stylize(verbTense), termSeqPair)
				return true
			}

			if (verbTense !== termSeqPairTense) {
				util.logError('Verb tense of', tenseRuleStr, 'does match specified verb term sequence pair `verbTense`:', util.stylize(verbTense), termSeqPair)
				return true
			}
