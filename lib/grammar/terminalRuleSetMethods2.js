	...
	if (isVerb) {
		...
		if (verbTense) {
			if (verbTense !== term.verbTense) {
				util.logError('Verb tense of', util.stylize(term.verbTense), 'does match match verb term sequence `verbTense`:', util.stylize(verbTense), term)
				return true
			}
		} else if (term.verbTense) {
			util.logError('Verb with `verbTense` provided to verb term sequence without specified `verbTense`:', util.stylize(term.verbTense), term)
			return true
		}
