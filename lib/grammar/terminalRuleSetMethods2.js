...
exports.newVerbSet2 = function (options) {
	...
	options.acceptedVerbs.forEach(function (verb, i) {
		...
		/**
		 * If `verb` is a verb sequence containing a verb terminal rule set created by `terminalRuleSetMethods.newVerb()` and an invariable terminal rule set created by `terminalRuleSetMethods.newInvariableTerm()`.
		 *
		 * For example:
 ...
 *
 * If `isVerb` is `true`, checks every term sequence `termSeqPair` produces contains exactly one verb terminal rule set. If `options.isVerb` is falsey and `_isNestedTermSequence` is falsey, checks every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
 *
 * `_isNestedTermSequence` must be falsey for the second `isVerb` check because `isVerb` only specifies if the entire base sequence contains or lacks a single verb, not every bigram within the sequence.  For example, the function does not check if "in common" within the verb sequence "have in common" contains verb; ergo, `isVerb` is unavailable should the nested pair be "have in", though the pair contains a verb.
 *
 * @private
 * @static
 ...
 * @param {boolean} [_isNestedTermSequence] Specify `termSeqPair` was defined as a nested term pair within another term sequence pair passed to `terminalRuleSetMethods.newBinaryTermSequence()` or `terminalRuleSetMethods.newTermSequence()`.
 * @returns {NSymbol} Returns the flattened term set sequence ordered pair.
 */
function flattenTermSequencePair(termSeqPair, isVerb, _isNestedTermSequence) {
	...
	// Always check if both terms in the pair are verbs, irrespective of `isVerb` and `_isNestedTermSequence`.
	if (termSeqPair.every(isVerbTerm)) {
		util.logError('Two verbs provided in a single verb term sequence pair:', termSeqPair)
		throw new Error('Ill-formed term sequence pair')
	}

	if (isVerb) {
		if (!termSeqPair.some(isVerbTerm)) {
			util.logError('No verbs provided in a verb term sequence pair:', termSeqPair)
			throw new Error('Ill-formed term sequence pair')
		}
	} else if (!_isNestedTermSequence) {
		/**
		 * Only alert if there are verbs when `isVerb` is falsey if `_isNestedTermSequence` is also falsey. This is necessary `isVerb` only specifies if the entire base sequence contains or lacks a single verb, not every bigram within the sequence.
		 *
