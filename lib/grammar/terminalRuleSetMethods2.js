 ...
 * • If `isVerb` is falsey and `isNestedTermSequence` is falsey, checks every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
 *
 ...
 * @param {boolean} [isNestedTermSequence] Specify `termSeqPair` was defined as a nested term pair within another term sequence pair passed to `terminalRuleSetMethods.newBinaryTermSequence()` or `terminalRuleSetMethods.newTermSequence()`.
...
function flattenTermSequencePair(termSeqPair, isVerb, isNestedTermSequence) {
	 ...
	 * • If `isVerb` is falsey and `isNestedTermSequence` is falsey, check every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
	 */
 ...
 *
 * If `isVerb` is `true`, every term sequence `termSeqPair` produces must contain exactly one verb terminal rule set.
 *
 * If `isVerb` is falsey and `isNestedTermSequence` is falsey, checks every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
 ...
 * @param {falsey} [isNestedTermSequence] Specify `termSeqPair` was defined as a nested term pair within another term sequence pair passed to `terminalRuleSetMethods.newBinaryTermSequence()` or `terminalRuleSetMethods.newTermSequence()`.
