...
exports.newVerb = function (options) {
	...
	// Saved for convenience when defining verb sets; not used internally.
...
/**
 * Checks if `terminalSymbol` contains any forbidden characters. If so, prints an error message.
 *
...
function isIllFormedTerminalSymbol(terminalSymbol) {
	/**
	 * Match any permitted non-alphabetic symbol: <, >
	 * `terminalSymbol` must match these symbols exactly; can not contain them as substring.
	 */
	var reSpecialSymbol = /^[<>]$/
	...
	/**
	 * Match any non-alphabetic or unspecified punctuation character: a-z, A-Z, '
	 * • apostrophe - for use in "followers'".
	 */
	var permittedPuncMarks = '\''
	var reForbiddenChar = RegExp('[^a-zA-Z' + permittedPuncMarks + ']')
	var forbiddenCharIndex = terminalSymbol.search(reForbiddenChar)
	if (forbiddenCharIndex !== -1) {
		util.logErrorAndPath('Terminal symbol', util.stylize(terminalSymbol), 'contains forbidden character:', util.stylize(terminalSymbol[forbiddenCharIndex]))
		return true
	}

	// Check if `terminalSymbol` contains adjacent permitted punctuation marks (defined above).
