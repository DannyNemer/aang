 ...
 *
 * Each rule in this set has as an object as its `text` with the properties `oneSg`, `threeSg`, `pl`, and `past`. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct verb form (i.e., display text) according to grammatical properties in preceding nonterminal rules in the same tree.
 *
 ...
 *
 * Each rule in this set has as an object as its `text` with the properties `oneSg`, `threeSg`, and `pl`. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct verb form (i.e., display text) according to grammatical properties in preceding nonterminal rules in the same tree.
 *
 * The person-number forms, `oneSg`, `threeSg`, and `pl`, are conjugated by the `personNumber` property in preceding nominative rules.
 *
 ...
 *
 * If `options.tense` is not "past", each terminal rule in the set has as an object as its `text` with the properties `oneSg`, `threeSg`, `pl`, and `past` for the various verb forms. When constructing parse trees, `pfsearch` conjugates the `text` object to the correct form (i.e., display text) according to grammatical properties in preceding nonterminal rules in the same tree.
 *
 ...
 *
 * If `options.tense` is specified, `options.verbFormsTermSet` must adhere to the following verb terminal rule set schemas:
 * • 'present' - `PresentVerbFormsTermSet`.
 * • 'past' - `PastVerbFormsTermSet`.
 *
 ...
 * @param {string} [options.tense] The grammatical tense, 'present' or 'past', for which to limit the verb terminal rule set; i.e., exclude all verb forms for the opposite tense. If 'present', `options.verbFormsTermSet` must adhere to `PresentVerbFormsTermSet`. If 'past', `options.verbFormsTermSet` must adhere to `PastVerbFormsTermSet`.
...
exports.newVerb = function (options) {
	...
	// Check `options` is well-formed, else throw an exception.
	checkVerbOptions(options)

	if (options.tense === 'past') {
		// Create a terminal rule set with `terminalRuleSetMethods.newInvariableTerm()`, where each terminal rule has the string `options.verbFormsTermSet.past` has its `text` value.
		var verbSym = newPastVerb(options)

		// Overwrite `termSetType` set by `terminalRuleSetMethods.newInvariableTerm()`.
	...
	// If specified, `verbSym` only produces terminal rules for the given grammatical tense; i.e., lacks terminal rules for the opposite tense.
	verbSym.verbTense = options.tense
 ...
 *
 * Uses `terminalRuleSetMethods.newInvariableTerm()` to create the terminal rule set, where each terminal rule has the string `options.verbFormsTermSet.past` has its `text` value, and `pfsearch` does not conjugate the verb set.
 *
 ...
 * @param {string} options.tense Limit the verb terminal rule set to its past tense verb forms; i.e., exclude all present tense verb forms.
...
/**
 * Checks `verbOptions` is well-formed. If not, prints an error and throws an exception.
 *
 * For use by `terminalRuleSetMethods.newVerb()`.
 *
 * @private
 * @static
 * @param {Object} verbOptions The options object passed to `terminalRuleSetMethods.newVerb()`.
 */
function checkVerbOptions(verbOptions) {
	var verbFormsTermSet = verbOptions.verbFormsTermSet

	if (verbOptions.tense) {
		// Properties `verbSchema.tense` and `verbSchema.noPastDisplayText` are mutually exclusive.
		if (verbOptions.noPastDisplayText) {
			util.logErrorAndPath('Verb with `tense` defines `noPastDisplayText` as `true`:', verbOptions)
			throw new Error('Ill-formed verb')
		}

		// Print specific errors for verb of a tense containing the forms of the other tense.
		if (verbOptions.tense === 'present') {
			for (var verbForm in pastVerbFormsTermSetSchema) {
				if (verbFormsTermSet.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Present tense verb has forbidden \`${verbForm}\`:`, verbOptions)
					throw new Error('Ill-formed present tense verb')
				}
			}

			if (util.illFormedOpts(presentVerbFormsTermSetSchema, verbFormsTermSet)) {
				throw new Error('Ill-formed present tense verb forms term set')
			}
		} else if (verbOptions.tense === 'past') {
			for (var verbForm in presentVerbFormsTermSetSchema) {
				if (verbFormsTermSet.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Past tense verb has forbidden \`${verbForm}\`:`, verbOptions)
					throw new Error('Ill-formed past tense verb')
				}
			}

			if (util.illFormedOpts(pastVerbFormsTermSetSchema, verbFormsTermSet)) {
				throw new Error('Ill-formed past tense verb forms term set')
			}
		}
