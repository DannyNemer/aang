...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		 ...
		 */
			...
			var termSeqPair = flattenTermSequencePair(term)

			// If `options.isVerb`, check exclusively `termSeqPair[0]` or `termSeqPair[1]` is either a verb terminal rule set or a verb term sequence, else both `termSeqPair[0]` and `termSeqPair[1]` are neither a verb terminal rule set nor a verb term sequence.
			if (isIllFormedVerbTermSequencePair(termSeqPair[0], termSeqPair[1], options.isVerb)) {
 ...
 */
function flattenTermSequencePair(termSeqPair) {
