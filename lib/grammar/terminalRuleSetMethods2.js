 ...
 * 2. `isTermSetSequence` - `true`. Prevents adding more rules to this `NSymbol`.
 * 3. `defaultText` - The merger of the `defaultText` values of the terms in `options.termPair`. For use when nesting this `NSymbol` in another term set sequence.
 *
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object.
...
exports.newBinaryTermSetSequence = function (options) {
	if (util.illFormedOpts(binaryTermSetSequenceSchema, options)) {
		throw new Error('Ill-formed binary term set sequence')
	}

	// Recursively flattens any nested ordered pairs to `NSymbol` instances that produce term set sequences.
	var termSetSeqPair = flattenTermSetSequencePair(options.termPair)
...
function flattenTermSetSequencePair(termSetSeqPair) {
	...
	for (var s = 0; s < termSetSeqLen; ++s) {
		...
		if (termSym.constructor === Array) {
			...
			termSetSeqPair[s] = exports.newBinaryTermSetSequence({ termPair: termSym })
