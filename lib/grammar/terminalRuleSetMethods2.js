 ...
 * For all three, `calcHeuristicCosts` creates a new, terminal `ruleProps` for the rule with the `text` value as specified, which `pfsearch` uses to generate display text. `calcHeuristicCosts` also always traverses the matched terminal rules to get the input tense of any matched verb terminal rules, which `pfsearch` uses if the parent rule has matching `acceptedTense`.
 *
 * If `options.isVerb` is `true`, checks every term sequence `options.acceptedTerms` and `options.substitutedTerms` produces contains exactly one verb terminal rule set. If `options.isVerb` is falsey, check every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
 *
 * The returned `NSymbol` has the following properties:
 * 1. `name` - `options.symbolName`.
 * 2. `isTermSetSequence` - `true`. Prevents adding more rules to this `NSymbol`.
...
exports.newTermSequence = function (options) {
	...
	termSeqSym.isTermSetSequence = true
 ...
 *
 * If `options.isVerb` is `true`, checks every term sequence `options.termPair` produces contains exactly one verb terminal rule set. If `options.isVerb` is falsey, check every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
 *
 * If `_isNestedTermSequence` is `true`, manually checks if `options.termPair` contains a verb to determine the `isVerb` value for the returned `NSymbol`. For use when `options.termPair` was defined as a nested term pair within another term sequence pair passed to this method or `terminalRuleSetMethods.newTermSequence()`. `options.isVerb` exists only to check if an entire sequence contains or lacks a single verb, not every bigram within the sequence; e.g., do not check if "in common" within the verb-phrase "have in common" contains a verb.
 *
 * The returned `NSymbol` has the following properties:
...
exports.newBinaryTermSequence = function (options, _isNestedTermSequence) {
	...
	if (_isNestedTermSequence) {
		/**
		 * Manually check whether `termSeqSym` produces a verb when `options.termPair` was defined as a nested term pair within another term sequence pair passed to this method or `terminalRuleSetMethods.newTermSequence()`. This is because `options.isVerb` exists only to check if an entire sequence (after flattening) contains or lacks a single verb, not every bigram within the sequence.
		 *
		 ...
		 */
		termSeqSym.isVerb = termSeqPair.some(isVerbTerm)
	} else {
		termSeqSym.isVerb = options.isVerb
	}
...
/**
 * Checks `termSeqPair` is a valid term set sequence pair and recursively flattens any nested ordered pairs to `NSymbol` instances that produce term set sequences.
 *
 ...
 *
 * If `isVerb` is `true`, checks every term sequence `termSeqPair` produces contains exactly one verb terminal rule set. If `options.isVerb` is falsey, check every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
 *
 * **Note:** This function mutates `termSeqPair`.
 *
 * @private
 * @static
 ...
 */
function flattenTermSequencePair(termSeqPair, isVerb) {
	...
	for (var s = 0; s < termSeqLen; ++s) {
		...
		if (termSym.constructor === Array) {
			 ...
			 *
			 * Pass `true` for the method's second parameter, `_isNestedTermSequence`, to instruct the method to manually check if the pair `termSym` contains a verb, instead of checking `options.isVerb`, when defining `isVerb` on the returned `NSymbol`. The parameter `isVerb` exists only to check if an entire sequence (i.e., the base invocation of this function) contains or lacks a single verb, not every bigram within the sequence (i.e., recursive invocations of this function). E.g., do not check if "in common" within the verb-phrase "have in common" contains a verb.
			 */
			termSeqPair[s] = exports.newBinaryTermSequence({ termPair: termSym }, true)
	...
	// After flattening nested term sequence pairs, if `isVerb` is `true`, check every term sequence `termSeqPair` produces contains exactly one verb terminal rule set, else check every term sequence contains zero verb terminal rule sets.
	if (isIllFormedVerbTermSequencePair(termSeqPair[0], termSeqPair[1], isVerb)) {
		throw new Error('Ill-formed term sequence pair')
	}
 ...
 *
 * If `isVerb` is `true`, exclusively `termA` or `termB` must be either a verb terminal rule set or a verb term sequence, else both `termA` and `termB` are neither a verb terminal rule set nor a verb term sequence.
 *
 * @private
 * @static
