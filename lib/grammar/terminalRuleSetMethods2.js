...
/**
 * Checks if `newVerbOptions` is well-formed. If not, prints an error and throws an exception.
 *
 * For use by `terminalRuleSetMethods.newVerb()`.
 *
 * @private
 * @static
 * @param {Object} newVerbOptions The options object passed to `terminalRuleSetMethods.newVerb()`.
 */
function checkNewVerbOptions(newVerbOptions) {
		...
		if (newVerbOptions.tense === 'present') {
			for (var verbForm in pastVerbFormsTermSetSchema) {
				if (verbFormsTermSet.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Present tense verb has forbidden \`${verbForm}\`:`, newVerbOptions)
			...
			for (var verbForm in presentVerbFormsTermSetSchema) {
				if (verbFormsTermSet.hasOwnProperty(verbForm)) {
					util.logErrorAndPath(`Past tense verb has forbidden \`${verbForm}\`:`, newVerbOptions)
