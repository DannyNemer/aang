...
exports.newVerbSet2 = function (options) {
	...
	// Save default text, the `text` value of the first accepted term set or term set sequence in this sequence. For use when nesting `verbSetSym` in another term set sequence, which uses `verbSetSym.defaultText` to generate its default text to use as the display text for its substitution rules.
 ...
 *
 * The terms and term set sequences in `options.acceptedTerms` and `options.substitutedTerms` must be one of the following:
 * 1. A terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`.
 * 2. A terminal rule set sequence created by this method.
 ...
 *
 * The `defaultText` value (or merger of `defaultText` values) of the first term in `options.acceptedTerms` is used as the `text` value of the rules for the items in `options.substitutedTerms`, if any, with which to substitute their `text` values when input.
 *
 * All rules which the new `NSymbol` produces are marked `isTermSetSequence`, which instructs `calcHeuristicCosts` to do the following:
 * 1. For non-edit rules, `calcHeuristicCosts` merges the `text` values of the matched terminal rules it produces.
 * 2. For insertion rules, `calcHeuristicCosts` traverses the single child node, gets the `text` values of the matched terminal rules, and merges those `text` values with the rule's insertion `text` according to its `insertedSymIdx` property.
 ...
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
 * @param {(NSymbol|NSymbol[])[]} options.acceptedTerms The terms and term set sequences to accept when input, parameterized as described above.
 * @param {(NSymbol|NSymbol[])[]} [options.substitutedTerms] The terms and term set sequences to substitute when input with the first item in `options.acceptedTErms`, parameterized as described above.
