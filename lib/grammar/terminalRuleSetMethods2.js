 ...
 * 5. `tense` - `options.tense`, if specified.
 *
...
function checkVerbOptions(verbOptions) {
	if (verbOptions.tense) {
		...
		if (verbOptions.tense === 'present') {
			...
			if (util.illFormedOpts(pastVerbFormsTermSetSchema, verbOptions.verbFormsTermSet)) {
				throw new Error('Ill-formed past tense verb forms term set')
			}
		}
	} else if (util.illFormedOpts(verbFormsTermSetSchema, verbOptions.verbFormsTermSet)) {
