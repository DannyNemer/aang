 ...
 *
 * If `isVerb` is `true`, checks every term sequence `termSeqPair` produces contains exactly one verb terminal rule set. If `options.isVerb` is falsey and `isNestedTermSequence` is falsey, checks every term sequence contains zero verb terminal rule sets. Throws an exception if either check fails.
 *
 * `isNestedTermSequence` must be falsey for the second `isVerb` check because `isVerb` only specifies if the entire base sequence contains or lacks a single verb, not every bigram within the sequence.  For example, the function does not check if "in common" within the verb sequence "have in common" contains verb; ergo, `isVerb` is unavailable should the nested pair be "have in", though the pair contains a verb.
 *
 * @private
 * @static
