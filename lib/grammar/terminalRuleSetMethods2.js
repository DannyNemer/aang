 ...
 *
 * The term sets and term set sequences in `options.acceptedTerms` and `options.substitutedTerms` must be one of the following:
 * 1. A terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`.
 * 2. A terminal rule set sequence created by this method or `terminalRuleSetMethods.newBinaryTermSetSequence()`.
 ...
 * 1. `name` - `options.symbolName`.
 * 2. `isTermSetSequence` - `true`. Prevents adding more rules to this `NSymbol`.
 * 3. `defaultText` - The `defaultText` value (or merger of `defaultText` values) of the first term set or term set sequence in `options.acceptedTerms`. For use when nesting this `NSymbol` in another term set sequence.
 *
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the new `NSymbol`.
...
exports.newTermSetSequence = function (options) {
	if (util.illFormedOpts(termSetSequenceSchema, options)) {
		throw new Error('Ill-formed term set sequence')
	}
	...
	options.acceptedTerms.forEach(function (term, i) {
		/**
		 * If `term` is a terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`, or a terminal rule set sequence created by `terminalRuleSetMethods.newTermSetSet()`.
		 *
		...
		if (term.isTermSet || term.isTermSetSequence) {
			...
			if (i === 0) {
				 ...
				 * • If `term` is an invariable term, `defaultText` is the display text of the first accepted terminal rule.
				 * • If `term` is a verb, `defaultText` is the identical `text` of all terminal rules in the set.
				 * • If `term` is a term set sequence, `defaultText` is the display text of the first accepted term set or term set sequence it produces.
				 */
		 ...
		 */
			...
			var termSetSeqLen = term.length
			if (termSetSeqLen !== 2) {
				util.logError('Term set sequence array is not an ordered pair:', term)
				throw new Error('Ill-formed term set sequence')
			}

			for (var t = 0; t < termSetSeqLen; ++t) {
				var termSym = term[t]
				if (termSym.constructor === Array) {
					if (termSym.length !== 2) {
						util.logError('Nested term set sequence is not an ordered pair:', termSym)
						throw new Error('Ill-formed nested term set sequence')
					}

					// Recursively create an `NSymbol` that produces a single binary rule for a term set sequence of this nested ordered pair.
					term[t] = exports.newBinaryTermSetSequence.apply(null, termSym)
				} else if (!termSym.isTermSet || !termSym.isTermSetSequence) {
					util.logError('Term is neither a term set nor a term set sequence:', termSym)
					throw new Error('Ill-formed term set sequence')
				}
			}
		}
			...
			util.logError('Term is neither a term set, term set sequence, nor an ordered pair of the two:', term)
			throw new Error('Ill-formed term set sequence')
		}
	})
...
/**
 * Creates an `NSymbol` with a single binary rule that produces `termA` and `termB`, in that order. Together, these symbols produce a terminal sequence forming a phrase comprised of terminal rule sets and nested terminal rule set sequences.
 *
 * `termsA` and `termB` must be one of the following:
 * 1. A terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`.
 * 2. A terminal rule set sequence created by `terminalRuleSetMethods.newTermSetSequence()` or this method.
 * 3. An ordered pair containing any combination of #1, #2, or nested ordered pairs from which to recursively create new term set sequences.
 *
 * The single rule the new `NSymbol` produces is marked `isTermSetSequence`.
 *
 * The returned `NSymbol` has the following properties:
 * 1. `name` - The concatenation of `termA.name` and `termB.name`.
 * 2. `isTermSetSequence` - `true`. Prevents adding more rules to this `NSymbol`.
 * 3. `defaultText` - The merger of `termA.defaultText` and `termB.defaultText`. For use when nesting this `NSymbol` in another term set sequence.
 *
 * @memberOf terminalRuleSetMethods
 * @param {NSymbol|NSymbol[]} termA The first term set or term set sequence in this sequence.
 * @param {NSymbol|NSymbol[]} termB The second term set or term set sequence in this sequence.
 * @returns {NSymbol} Returns the new `NSymbol` for the terminal rule set sequence.
 */
exports.newBinaryTermSetSequence = function (termA, termB) {
	if (termA.constructor === Array) {
		if (termA.length !== 2) {
			util.logError('Nested term set sequence is not an ordered pair:', termA)
			throw new Error('Ill-formed nested term set sequence')
		}

		termA = exports.newBinaryTermSetSequence.apply(null, termA)
	}

	if (termB.constructor === Array) {
		if (termB.length !== 2) {
			util.logError('Nested term set sequence is not an ordered pair:', termB)
			throw new Error('Ill-formed nested term set sequence')
		}

		termB = exports.newBinaryTermSetSequence.apply(null, termB)
	}

	if (!isValidTermSetSequencePair(termA, termB)) {
...
/**
 * Checks if both `termA` and `termB` can form a valid term set sequence. Each must be either a term set or term set sequence.
 *
 * @private
 * @static
 * @param {NSymbol} termA The term to check.
 * @param {NSymbol} termB The other term to check.
 * @returns {boolean} Returns `true` if `termA` and `termB` can form a valid term set sequence, else `false`.
 */
function isValidTermSetSequencePair(termA, termB) {
	if (!termA.isTermSet && !termA.isTermSetSequence) {
		util.logError('Term is neither a term set nor a term set sequence:', termA)
		return false
	}

	if (!termB.isTermSet && !termB.isTermSetSequence) {
		util.logError('Term is neither a term set nor a term set sequence:', termB)
		return false
	}
