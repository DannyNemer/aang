...
/**
 * Creates an `NSymbol` that produces a terminal rule sequence forming a term or phrase, comprised of terminal rule sets and nested term sequences.
 *
 ...
 * 2. A terminal rule set sequence created by this method or `terminalRuleSetMethods.newTermSequenceBinarySymbol()`.
...
exports.newTermSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		/**
		 * If `term` is a terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`, or a terminal rule set sequence created by `terminalRuleSetMethods.newTermSetSet()` or `terminalRuleSetMethods.newTermSequenceBinarySymbol()`.
		 *
		...
		if (term.isTermSet || term.isTermSetSequence) {
			...
			/**
			 * Even though this rule is unary and does not require `text` merging, the rule is still assigned `isTermSetSequence` to instruct `calcHeuristicCosts` to bring the `text` up to this rule's node level, allowing `gramProps` to conjugate the `text` (`gramProps` only conjugates the immediate child nodes).
			 *
...
/**
 * Checks `termSeqPair` is a valid term set sequence pair and recursively flattens any nested ordered pairs to `NSymbol` instances that produce the nested term sequence.
 *
 * Each item in `termSeqPair` must be one of the following:
 * 1. A terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`.
 * 2. A terminal rule set sequence created by `terminalRuleSetMethods.newTermSequence()` or `terminalRuleSetMethods.newTermSequenceBinarySymbol()`.
 ...
 * • Temporarily forbids a term sequence to produce multiple verbs to prevent multiple instances of `tense` within a single sequence. `calcHeuristicCosts` does not support mapping a terminal rule's input tense to a particular item in the merged text when flattening the a term sequence's subtree.
 *
