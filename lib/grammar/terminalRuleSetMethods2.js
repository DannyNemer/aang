...
exports.newTermSetSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		 ...
		 */
			...
			var termSetSeqPair = flattenTermSetSequencePair(term)
			...
			if (i === 0) {
				 ...
				 * • If an item in the pair an invariable term, its `defaultText` is the display text of the first accepted terminal rule.
				 * • If an item in the pair is a verb, its `defaultText` is the identical `text` of all terminal rules in the set.
				 * • If an item in the pair is a term set sequence, its `defaultText` is the display text of the first accepted term set or term set sequence it produces.
				 */
	...
	if (options.substitutedTerms) {
		options.substitutedTerms.forEach(function (term) {
			if (term.isTermSet || term.isTermSetSequence) {
				...
				termSetSeqSym.addRule({
					rhs: flattenTermSetSequencePair(term),
					text: defaultText,
				})
			} else {
				util.logError('Term is neither a term set, term set sequence, nor an ordered pair of the two:', term)
...
exports.newBinaryTermSetSequence = function (termA, termB) {
	// Recursively flattens any nested ordered pairs to `NSymbol` instances that produce term set sequences.
	var termSetSeqPair = flattenTermSetSequencePair([ termA, termB ])
	termA = termSetSeqPair[0]
	termB = termSetSeqPair[1]
 ...
 *
 * Each element in `termSetSeqPair` must be one of the following:
 * 1. A terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`.
 * 2. A terminal rule set sequence created by `terminalRuleSetMethods.newTermSetSequence()` or `terminalRuleSetMethods.newBinaryTermSetSequence()`.
 * 3. An ordered pair containing any combination of #1, #2, or nested ordered pairs from which to recursively create new term set sequences with `terminalRuleSetMethods.newBinaryTermSetSequence()`.
 *
 ...
 */
function flattenTermSetSequencePair(termSetSeqPair) {
	var termSetSeqLen = termSetSeqPair.length
	if (termSetSeqLen !== 2) {
		util.logError('Term set sequence is not an ordered pair:', termSetSeqPair)
		throw new Error('Ill-formed term set sequence pair')
	}
	...
	for (var s = 0; s < termSetSeqLen; ++s) {
		...
		if (termSym.constructor === Array) {
			if (termSym.length !== 2) {
				util.logError('Term set sequence is not an ordered pair:', termSym)
				throw new Error('Ill-formed term set sequence pair')
			}

			// Recursively create an `NSymbol` that produces a single binary rule for the term set sequence of this nested ordered pair.
			termSetSeqPair[s] = exports.newBinaryTermSetSequence.apply(null, termSym)
