...
exports.newTermSetSequence = function (options) {
	...
	options.acceptedTerms.forEach(function (term, i) {
		/**
		 * If `term` is a terminal rule set created by `terminalRuleSetMethods.newVerb()` or `terminalRuleSetMethods.newInvariableTerm()`, or a terminal rule set sequence created by `terminalRuleSetMethods.newTermSetSet()` or `terminalRuleSetMethods.newBinaryTermSetSequence()`.
		 *
	...
	if (options.substitutedTerms) {
		...
		options.substitutedTerms.forEach(function (term) {
			 ...
			 */
			if (term.isTermSet || term.isTermSetSequence) {
				// `pfsearch` uses this rule's `text` property instead of the `text` values of the matched terminal rules this rule produces.
				termSetSeqSym.addRule({
					rhs: [ term ],
					text: defaultText,
				})
