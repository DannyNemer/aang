						...
						AInsertions.forEach(function (A) {
							BInsertions.forEach(function (B) {
								...
								newInsertion.gramProps = {
									...
									// `personNumber` only needed for first symbol in a binary rule: nominative case (verb precedes subject).
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== lhsSym) {
						insertions[sym].forEach(function (insertion) {
							...
							newRule.gramProps = {
								...
								gramCase: rule.gramProps.gramCase || insertionGramProps.gramCase,
								verbForm: rule.gramProps.verbForm || insertionGramProps.verbForm,
								// Person-number is only needed for first symbol in a binary rule: nominative case (verb precedes subject).Otherwise, the property is for when this lhsSym is used in a reduction (not this text).
