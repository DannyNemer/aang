 ...
 *
 * Recursively removes nonterminal symbols without rules, rules whose RHS containing those symbols or the `<empty>` symbol, and any rule-less nonterminal symbols that result.
 *
...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Recursively remove nonterminal symbols without rules, rules whose RHS symbols include those symbols, and any rule-less nonterminal symbols that result. Invoke this function before `createInsertionRules()`, which would otherwise create insertion rules using the nonterminal rules this removes.
	removeUnusedNonterminalSyms(ruleSets)

	// Define if each nonterminal rule's RHS symbols can produce a semantic. This can be determined before adding edit rules because any semantics in edit rules will be found in the original rules from which they are derived. Invoke after removing unused rules in `removeUnusedNonterminalSyms()`.
	assignSemanticPotentials(ruleSets)

	// Check for default (non-edit) rules that lack and cannot produce a needed RHS semantic. Throws an exception if finds any. Invoke after removing rules with rule-less symbols in `removeUnusedNonterminalSyms()`.
 ...
 *
 * Invoke this function after removing unused rules in `removeUnusedNonterminalSyms()`.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 ...
 *
 * Invoke this function after removing rules with rule-less symbols in `removeUnusedNonterminalSyms()`.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
