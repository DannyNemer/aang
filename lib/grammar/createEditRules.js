 ...
 *
 * The edits exist to enable `pfsearch` to return results that expand on an
 * input query and handle ill-formed input, and not to offer alternatives to an
 * input query.
 *
 * Eventually, costs will be improved via machine learning using feedback data
 * of what users continue typing or which suggestion uses select versus the
 * actual generated suggestions.
 *
 * Invoke this module after invoking `removeIllFormedRulesAndSyms()` to remove
 * ill-formed nonterminal rules and symbols.
 *
 * @static
 ...
 *
 * When reducing matched transposition rules, `Parser.prototype.reduce()` flips
 * the child nodes of the binary node, and the resulting node and `ruleProps`
 * are indistinguishable from non-transposition binary rules.
 *
 * Note: Transposition rules slow the test suite by 3.5x by significantly
 * increasing the parse table size. `Parser` accounts for nearly all of this
 * added time. Transposition edits provide little benefit because they rarely
 * offer superior query suggestions for which no other edit rules provide
 * semantically equivalent suggestions (i.e., ambiguous suggestions). Ergo,
 * transposition rules may be removed from the grammar.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
	...
	grammarUtil.forEachRule(ruleSets, function (origRule, nontermSym) {
		if (origRule.transpositionCost !== undefined) {
			var newRule = {
				...
				// Need not flip the RHS indexes to which `origRule.gramProps`
				// applies because `Parser` reverses matched transposition
				// rules to their original order before `pfsearch` conjugates.
