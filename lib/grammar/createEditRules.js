...
var oneSgSymName = require('./rules/oneSg').plain.name
			...
			if (rule.isTerminal) {
				...
				if (termSym === g.emptySymbol) {
					...
					addInsertion(insertions, nontermSym, {
						...
						hasOneSg: nontermSym === oneSgSymName,
						...
						insertionsA.forEach(function (a) {
							insertionsB.forEach(function (b) {
								...
								mergedInsertions.push({
									...
									hasOneSg: a.hasOneSg || b.hasOneSg,
				...
				RHS.forEach(function (sym, insertedSymIdx) {
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							var newRule = {
								// Rules where the inserted segment is in the right (second) position of a binary rule and includes the `[1-sg]` symbol in the insertion, only insert at the end of an input query.
								...
								// 2. This limits the number of such insertions at the end the input query to one.
								// 3. Most importantly, this prioritizes suggestions that are easier for the user to read by only inserting at the end. In contrast, arbitrarily positioning edits anywhere in the query disorients the user as they must discern the suggestion's differences from input.
								...
								// The `<blank>` symbol also enables deletions (with costs) for input tokens that precede it (i.e., the last token(s) of input).
								// In `pfsearch`, the insertion text is handled just as normal insertions because the second branch for `<blank>` is not traversed, but also count any deletion costs `<blank>` may absorb.
								RHS: insertedSymIdx === 1 && insertion.hasOneSg ? [ otherSym, blankInsertedSymbol.name ] : [ otherSym ],
