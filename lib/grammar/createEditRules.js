...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Define if each nonterminal rule's RHS symbols can produce a semantic. This can be determined before adding edit rules because any semantics in edit rules will be found in the original rules from which they are derived. Invoke after removing rule-less nonterminal symbols in `removeIllFormedRulesAndSyms()`.
	assignSemanticPotentials(ruleSets)

	// Check for default (non-edit) rules that lack and can not produce a needed RHS semantic. Throws an exception if finds any. Invoke after removing rule-less nonterminal symbols in `removeIllFormedRulesAndSyms()`.
 ...
 *
 * Invoke this function after removing rule-less nonterminal symbols in `removeIllFormedRulesAndSyms()`.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 ...
 *
 * Do not need to invoke `removeIllFormedRulesAndSyms()` because rule-less symbols can not result from the removal of `<empty>` because nonterminal symbols are forbidden from producing `<empty>` as their only rule, else throws an exception. Also, it is not possible the rule removals render other symbols unreachable via the start symbol, which would also require invoking `removeIllFormedRulesAndSyms()`, because `<empty>` only exists on terminal (unary) rules.
 *
 ...
 *
 * Invoke this function after removing rule-less nonterminal symbols in `removeIllFormedRulesAndSyms()` and before creating any edit-rules. Does not check edit-rules because if an edit-rules lacks a needed semantic, then that condition will also be true for the original rule from which it was derived.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
...
function removeRulesMissingRequiredSemantics(ruleSets) {
	...
	// Also invoke `removeIllFormedRulesAndSyms()` because can not monitor if `semanticChecks.ruleMissingReducedSemantic()` remove ancestor rules (which are not the rules passed here).
	...
	removeIllFormedRulesAndSyms(ruleSets)
