				...
				RHS.forEach(function (sym, symIdx) {
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== lhsSym) {
						insertions[sym].forEach(function (insertion) {
							...
							appendSemantic(ruleSets, rule, newRule, insertion.semantic)
 ...
 */
function appendSemantic(ruleSets, existingRule, newRule, insertionSemantic) {
	...
	if (insertionSemantic) {
		...
		// Existing rule has a LHS semantic.
			...
			if (!symbolProducesSemantic(ruleSets, newRule.RHS[0])) {
