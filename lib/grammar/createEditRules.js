...
var rhsAcceptsGramProp = require('./symbol').rhsAcceptsGramProp
			...
			ruleSets[nontermSym].forEach(function (rule) {
				...
				if (rhsCanBeInserted(insertions, rule)) {
					rule.rhs.map(function (sym) {
						...
						if (addInsertion(insertions, nontermSym, newInsertion, stopAmbiguity)) {
							insertionAdded = true
						}
					})
				}
...
function createInsertionRules(ruleSets, insertions, stopAmbiguity) {
	// The nonterminal symbol used in the second position of insertion rules where the inserted segment is in the right (second) position of a binary rule and includes the `[1-sg]` symbol in the insertion.
				...
				rhs.forEach(function (sym, insertedSymIdx) {
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							...
							if (g.ruleMissingRHSSemantic(nontermSym, newRule)) return

							if (rule.rhsDoesNotProduceText) {
								// The display text for nonterminal rules created from regex-style terminal rules (including substitutions) and nonterminal stop-words (which lack `text`).
								newRule.text = rule.text
								newRule.tense = rule.tense
								newRule.rhsDoesNotProduceText = true

								// Flatten some rules to enable `ruleIsUnique()` to detect instances of ambiguity and properly discard those rules. For example: X -> "x" vs. X -> Y -> "x".
								...
								// Avoid insertion rules with `[blank-inserted]`.
								if (newRule.rhs.length === 1) {
									var rhsRules = ruleSets[otherSym]

									if (rhsRules.length === 1) {
										var rhsRule = rhsRules[0]

										newRule.rhs = rhsRule.rhs
										...
										if (rhsRule.isTerminal) {
											newRule.isTerminal = true

											// Delete properties only needed for nonterminal rules.
											delete newRule.rhsDoesNotProduceText
											delete newRule.rhsCanProduceSemantic
										} else {
											newRule.rhsCanProduceSemantic = rhsRule.rhsCanProduceSemantic
											newRule.secondRHSCanProduceSemantic = rhsRule.secondRHSCanProduceSemantic
										}
									}
								}

								// The grammar generator badly needs a complete redesign of its handling of multi-token terminal rules. This work has long been avoided due to frustration from not finding a perfect solution: automatically split rules (using regex-style definitions) to enable an easy developer interface vs. require the developer to manually define all terminal symbols, each limited to one token vs. both. In addition, the grammar needs a terminal rule optimizer that reduces the grammar by sharing or flattening rules.
							...
							addRule(symRules, newRule, nontermSym, stopAmbiguity)
						})
					}
				})
			}
 ...
 * @param {Object} gramProps The insertion's parent rule's grammatical properties (`tense` and `case`) for conjugating text objects in `insertionText`.
...
function conjugateText(insertionText, gramProps, personNumber) {
	return insertionText.reduce(function (textArray, text) {
		...
		else if (text[gramProps.case]) {
			textArray.push(text[gramProps.case])
		}
 ...
 */
function addInsertion(insertions, nontermSym, newInsertion, stopAmbiguity) {
...
function insertionIsUnique(symInsertions, newInsertion, nontermSym, stopAmbiguity) {
	for (var s = 0, symInsertionsLen = symInsertions.length; s < symInsertionsLen; ++s) {
		...
		if (areAmbiguous(existingInsertion, newInsertion, nontermSym, stopAmbiguity)) {
 ...
 */
function addRule(rules, newRule, nontermSym, stopAmbiguity) {
...
function ruleIsUnique(rules, newRule, nontermSym, stopAmbiguity) {
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (util.arraysEqual(existingRule.rhs, newRule.rhs)) {
			...
			if (areAmbiguous(existingRule, newRule, nontermSym, stopAmbiguity)) {
 ...
 */
function areAmbiguous(a, b, nontermSym, stopAmbiguity) {
	...
	for (var nontermSym in ruleSets) {
		...
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rule.insertedSymIdx === undefined) {
				// If `rule` lacks and cannot produce a RHS semantic required for itself or an ancestor rule, then print a graph representation of the parse tree path from the rule requiring the RHS semantic to `rule`, and throw an exception.
				if (g.ruleMissingRHSSemantic(nontermSym, rule, true)) {
					throw new Error('Semantic error')
				}
			}
		}
