...
/**
 * Checks if `newInsertion` is unique compared to existing insertions. This prevents ambiguity created by insertions that generate identical display text or semantics. Returns `true if insertions exists`. If `newInsertion` is cheaper than an existing insertion with identical text or semantics, removes the existing insertion and returns `true`.
 *
...
function insertionIsUnique(symInsertions, newInsertion) {
	for (var s = 0, symInsertionsLen = symInsertions.length; s < symInsertionsLen; ++s) {
		...
		// Check if `newInsertion` and `existingInsertion` create ambiguity using identical display text or semantics.
		if (textsEqual(existingInsertion.text, newInsertion.text) || semantic.arraysEqual(existingInsertion.semantic, newInsertion.semantic)) {
