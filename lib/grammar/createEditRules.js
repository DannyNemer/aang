...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Remove non-edit rules that lack and can not produce a reduced semantic required for themselves or ancestor rules. Invoke after removing rule-less nonterminal symbols in `removeIllFormedRulesAndSyms()`.
	semanticChecks.removeRulesMissingRequiredSemantics(ruleSets, true)

	// Define if each nonterminal rule's RHS symbols can produce a semantic. This can be determined before adding edit rules because any semantics in edit rules will be found in the original rules from which they are derived. Invoke after removing rule-less nonterminal symbols and rules in `removeIllFormedRulesAndSyms()` and `semanticChecks.removeRulesMissingRequiredSemantics()`.
 ...
 *
 * Do not need to invoke `semanticChecks.removeRulesMissingRequiredSemantics()` after removing rules with `<empty>` because rules with `<empty>` are forbidden from having a semantic.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
