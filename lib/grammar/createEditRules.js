...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Check for default (non-edit) rules that lack and cannot produce a needed RHS semantic. Throws an exception if finds any. Invoke after removing rule-less nonterminal symbols in `removeUnusedNonterminalSyms()`.
...
/**
 * Removes non-edit rules that lack and cannot produce a reduced semantic required for themselves or ancestor rules.
 *
...
function removeRulesMissingRequiredSemantics(ruleSets) {
	...
	// Also invoke `removeUnusedNonterminalSyms()` because cannot monitor if `semanticChecks.ruleMissingReducedSemantic()` remove ancestor rules (which are not the rules passed here).
