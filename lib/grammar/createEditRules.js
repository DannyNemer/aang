					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							var newRule = {
								...
								// Create a new grammatical properties object for an insertion rule containing only the original rule's grammatical properties applicable to the insertion rule's non-inserted symbol, `otherSym`.
								gramProps: genGramProps(otherSym, rule.gramProps),
								...
								rhsCanProduceSemantic: semanticChecks.symCanProduceSemantic(ruleSets, otherSym),
...
/**
 * Creates a new grammatical properties object for an insertion rule containing only the original rule's grammatical properties applicable to the insertion rule's non-inserted symbol, `nonInsertedSym`.
 *
 ...
 * @private
 * @static
 * @param {string} nonInsertedSym The insertion rule's (non-inserted) RHS symbol for which to determine the applicable grammatical properties.
 ...
 */
function genGramProps(nonInsertedSym, origGramProps) {
	var newRuleRHS = [ nonInsertedSym ]
	var newGramProps = {}

	for (var propName in origGramProps) {
		var gramProp = origGramProps[propName]

		// Add the grammatical property if it can conjugate `nonInsertedSym`.
