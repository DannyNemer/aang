					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							...
							if (rule.rhsDoesNotProduceText) {
								...
								// Avoid insertion rules with `[blank-inserted]`. Though, the same flattening procedure should still be possible.
								// FIXME: This should not be done here, because other rules might still be added to `ruleSets[otherSym]`, though ambiguity is being prevented here.
								if (newRule.rhs.length === 1) {
									var rhsRules = ruleSets[otherSym]

									for (var r = 0, rhsRulesLen = rhsRules.length; r < rhsRulesLen; ++r) {
										var newRuleClone = {}
										for (var prop in newRule) {
											newRuleClone[prop] = newRule[prop]
										}
										...
										addRule(symRules, newRuleClone, nontermSym, stopAmbiguity)
									}
