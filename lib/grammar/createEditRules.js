 ...
 *
 * Discards insertions that `isUselessInsertion()` determines produce a meaningless clause.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
...
function mergeInsertionSets(ruleSets, nontermSym, rule, insertionsA, insertionsB) {
	...
	insertionsA.forEach(function (a) {
		insertionsB.forEach(function (b) {
			/**
			 * Discard insertion if produces a meaningless clause.
			 *
			 * Stop these insertions here, in addition to `createInsertionRules()`, to prevent their inclusion in larger insertions for which the logic in `isUselessInsertion()` can not detect their meaninglessness.
			 ...
			 */
			if (isUselessInsertion(ruleSets, a, a.tree[0].symbol, nontermSym, rule, true)) {
				return
			}

			if (isUselessInsertion(ruleSets, b, b.tree[0].symbol, nontermSym, rule, true)) {
				return
			}
...
/**
 * Checks if `insertion` produces a meaningless clause, which should be avoided. For use by `createInsertionRules()` and `mergeInsertionSets()` to discard useless insertions.
 *
 ...
 *   • `semanticChecks.ruleMissingReducedSemantic()` in `editRulesUtil.addRule()` would stop this rule anyway for being unable to produce a semantic required for the parent semantic, `not()`.
 *
...
function isUselessInsertion(ruleSets, insertion, insertedRHSSym, nontermSym, origRule, printWarning) {
	...
	if (insertion.text[0] && !insertion.semantic) {
		 ...
		 *   • Given `nonInsertedRHSSym` can not produce a semantic and `insertedRHSSym` can, then `origRule` forms a clause where `nonInsertedRHSSym` produces display text for sentence structure and `insertedRHSSym` semantics contribute the meaning.
		...
		/*
		 * • When neither `insertedRHSSym` nor `nonInsertedRHSSym` can produce semantics, then `origRule` does not form a clause but rather provides necessary sentence structure to a clause of which it is a subset. Such insertions should not be discarded. For example:
		 *   - "repos are ..." -> "repos (that) are ..."
		 *   - "repos by my" -> "repos (created by) me"
		 *
		 * • If there is a non-insertion rule with `nonInsertedRHSSym` as its only RHS symbol and the same semantic (or lack thereof) as `origRule`, then `rulesAreAmbiguous()` will catch the ambiguity and discard `insertion` because its display text assures it is more expensive.
		 ...
		 *
		 * • Another possible meaningless clause insertion, though not checked here because such an insertion's construction is avoided by invoking this function in `mergeInsertionSets()`, is demonstrated by the following rules:
