		...
		for (var nontermSym in ruleSets) {
			ruleSets[nontermSym].forEach(function (rule) {
				...
				if (rhsCanBeInserted(insertionSets, rule)) {
					rule.rhs.map(function (insertedSym) {
						// Map the insertions for each RHS symbol to new insertions with an extended tree.
						return insertionSets[insertedSym].map(function (insertion) {
							return {
								cost: insertion.cost,
								text: insertion.text,
								...
								tree: [ { symbol: insertedSym, children: insertion.tree } ],
							}
						})
...
function createInsertionRules(ruleSets, insertionSets, stopAmbiguity) {
	...
	for (var nontermSym in ruleSets) {
		ruleSets[nontermSym].forEach(function (rule, ruleIdx, rules) {
			var rhs = rule.rhs

			if (rhs.length > 1) {
				rhs.forEach(function (insertedSym, insertedSymIdx) {
					...
					if (insertionSets.hasOwnProperty(insertedSym) && nonInsertedRHSSym !== nontermSym) {
						insertionSets[insertedSym].forEach(function (insertion) {
							var newRule = {
								...
								rhs: insertedSymIdx === 1 && (rule.restrictInsertion || insertion.restrictInsertion) ? [ nonInsertedRHSSym, blankInsertedSymbol.name ] : [ nonInsertedRHSSym ],
								...
								gramProps: genGramProps(nonInsertedRHSSym, rule.gramProps),
