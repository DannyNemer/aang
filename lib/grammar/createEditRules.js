...
module.exports = function (ruleSets) {
	...
	// Recursively removes nonterminal symbols with no rules, rules whose RHS containing those symbols or `<empty>`, and any rule-less nonterminal symbols that result. Called after finding insertions from rules with '<empty>' in `findTermRuleInsertions()`.
	...
	// Define whether each nonterminal rule's RHS symbols can produce a semantic. If `false`, then the rule's semantic can be reduced with a parse tree's preceding LHS semantic before parsing the RHS symbols. This enables finding and discarding semantically illegal parses earlier than otherwise.
	for (var nontermSym in ruleSets) {
		ruleSets[nontermSym].forEach(function (rule) {
			if (!rule.isTerminal) {
				rule.rhsCanProduceSemantic = rhsCanProduceSemantic(ruleSets, rule.RHS)
			}
		})
	}
	...
	createInsertionRules(ruleSets, insertions)
...
/**
 * Checks if `newInsertion` is unique compared to existing insertions. Returns `true if insertions exists`. If `newInsertion` is cheaper than an existing insertion with identical text, removes the existing insertion and returns `true`.
 *
 ...
 * @param {Object} insertions The map of symbols to array of inserted symbol sequences.
 */
function createInsertionRules(ruleSets, insertions) {
	Object.keys(ruleSets).forEach(function (nontermSym) {
				...
				RHS.forEach(function (sym, symIdx) {
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							...
							newRule.rhsCanProduceSemantic = rhsCanProduceSemantic(ruleSets, newRule.RHS)
							...
							// Add new rule to grammar if it is unique and below the cost upper bound. This complexity bound, which reduces the possible insertions, is necessary because when only the k-best parse results are output, then having > k insertions will construct paths that are never output.
...
/**
 * Checks if `newRule` has unique LHS and RHS symbols compared to existing rules. Throws an exception if `newRule` has identical RHS and display text to an existing non-insertion rule. If `newRule` has identical RHS symbols, semantics, but different display text and cheaper than an other insertion rule, then removes the other insertion rule and return `true`.
 *
		...
		if (util.arraysEqual(existingRule.RHS, newRule.RHS)) {
			...
			if (textsEqual(existingRule.text, newRule.text)) {
				if (existingRule.insertedSyms) {
					util.logError('Two identical rules produced by insertions:')
				} else {
					util.logError('New rule produced with edits identical to original rule:')
				}

				util.dir(nontermSym, existingRule, newRule)
...
/**
 * Checks if a nonterminal rule's RHS symbols, `rhs`, contains a symbol that can produce a rule with a semantic.
 *
 * If not, then the rule's semantic can be reduced with a parse tree's preceding LHS semantic before parsing `rhs`. This enables finding and discarding semantically illegal parses earlier than otherwise.
 *
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 * @param {string[]} rhs The nonterminal rule's RHS symbols.
 * @returns {boolean} Returns `true` if `rhs` contains a `symbol` that can produce a rule with a semantic, else `false`.
 */
function rhsCanProduceSemantic(ruleSets, rhs, _symsSeen) {
	_symsSeen = _symsSeen || []

	return rhs.some(function (nontermSym) {
		if (_symsSeen.indexOf(nontermSym) === -1) {
			...
			return ruleSets[nontermSym].some(function (rule) {
				if (rule.semantic || rule.isPlaceholder) {
					...
					return rhsCanProduceSemantic(ruleSets, rule.RHS, _symsSeen)
				}
			})
		}
	})
