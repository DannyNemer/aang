...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Define if each nonterminal rule's RHS symbols can produce a semantic. This can be determined before adding edit rules because any semantics in edit rules will be found in the original rules from which they are derived. Invoke after removing ill-formed nonterminal symbols and rules in `removeIllFormedRulesAndSyms()`.
	...
	// Remove rules from that produce the empty symbol, `<empty>`. Invoke this function after creation insertion rules from `<empty>`.
	...
	// Remove instances of the rule properties, `rule.isSubstitution` and `rule.isStopWord`, used by `splitRegexTerminalSymbols`.
	removeTextSubstitutionProps(ruleSets)

	if (!includeTrees) removeTrees(ruleSets)

	// Delete instances of the (temporary) rule property, `semanticSafe`, used by `semanticChecks.ruleMissingReducedSemantic()` in `removeIllFormedRulesAndSyms`. Invoke after adding edit-rules, which relies on these properties to avoid edit-rules that can not produce a required reduced semantic.
				...
				if (insertions && nonInsertedRHSSym !== nontermSym) {
					for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
						...
						if (origRule.rhsDoesNotProduceText) {
							...
							// Do not need `isSubstitution` or `isStopWord` because the propery `rhsDoesNotProduceText` handles both cases.
 ...
 *
 * Invoke this function after removing ill-formed nonterminal symbols and rules in `removeIllFormedRulesAndSyms()`.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 ...
 * - Rule-less symbols can not result from the removal of `<empty>` because nonterminal symbols are forbidden from producing `<empty>` as their only rule, else throws an exception
 * - It is not possible the rule removals render other symbols unreachable via the start symbol because `<empty>` only exists on terminal (unary) rules.
 * - It is not possible the rule removals prevent other rules from producing required semantics because rules with `<empty>` are forbidden from having a semantic.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
...
/**
 * Removes instances of the rule properties, `rule.isSubstitution` and `rule.isStopWord`, used by `splitRegexTerminalSymbols`.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 */
function removeTextSubstitutionProps(ruleSets) {
	grammarUtil.forEachRule(ruleSets, function (rule) {
