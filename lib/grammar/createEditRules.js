...
var removeUnusedNonterminalSyms = require('./removeUnusedNonterminalSyms')
...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Remove rules from that produce the empty symbol, `<empty>`. Invoke this function after finding and saving insertions for rules with `<empty>` in `findTerminalRuleInsertions()`.
	removeEmptySymbolRules(ruleSets)

	// Define if each nonterminal rule's RHS symbols can produce a semantic. This can be determined before adding edit rules because any semantics in edit rules will be found in the original rules from which they are derived. Invoke after removing rule-less nonterminal symbols in `removeUnusedNonterminalSyms()`.
	assignSemanticPotentials(ruleSets)

	// Check for default (non-edit) rules that lack and can not produce a needed RHS semantic. Throws an exception if finds any. Invoke after removing rule-less nonterminal symbols in `removeUnusedNonterminalSyms()`.
	removeRulesMissingRequiredSemantics(ruleSets)
	...
	// Delete instances of the (temporary) rule property, `semanticSafe`, used by `semanticChecks.ruleMissingReducedSemantic()`. Invoke after adding edit-rules, which relies on these properties to avoid edit-rules that can not produce a required reduced semantic.
	semanticChecks.deleteSemanticChecksProps(ruleSets)
}
...
function createInsertionRules(ruleSets, insertionSets, stopAmbiguity) {
	...
	// Add this symbol to the grammar here to prevent `removeUnusedNonterminalSyms()` from removing it earlier.
 ...
 *
 * Invoke this function after removing rule-less nonterminal symbols in `removeUnusedNonterminalSyms()`.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 ...
 *
 * Do not need to invoke `removeUnusedNonterminalSyms()` because rule-less symbols can not result from the removal of `<empty>` because nonterminal symbols are forbidden from producing `<empty>` as their only rule, else throws an exception. Also, it is not possible the rule removals render other symbols unreachable via the start symbol, which would also require invoking `removeUnusedNonterminalSyms()`, because `<empty>` only exists on terminal (unary) rules.
 *
 * Do not need to invoke `removeRulesMissingRequiredSemantics()` after removing rules with `<empty>` because rules with `<empty>` are forbidden from having a semantic.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
	...
	grammarUtil.forEachRuleSet(ruleSets, function (rules, nontermSym) {
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rule.rhs[0] === g.emptySymbol) {
				...
				// Check for nonterminal symbols whose only rule is `<empty>`. This check must occur here, after removing any rules in `removeUnusedNonterminalSyms()`.
				if (rulesLen === 0) {
...
/**
 * Removes non-edit rules that lack and can not produce a reduced semantic required for themselves or ancestor rules.
 *
 ...
 *
 * Invoke this function after removing rule-less nonterminal symbols in `removeUnusedNonterminalSyms()` and before creating any edit-rules. Does not check edit-rules because if an edit-rules lacks a needed semantic, then that condition will also be true for the original rule from which it was derived.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 */
function removeRulesMissingRequiredSemantics(ruleSets) {
	...
	grammarUtil.forEachRuleSet(ruleSets, function (rules, nontermSym) {
		...
		for (var r = 0; r < rules.length; ++r) {
			if (semanticChecks.ruleMissingReducedSemantic(ruleSets, nontermSym, rules[r], true)) {
	...
	// Remove any rule-less or unreachable nonterminal symbols that result from the removal of rules above.
	// Also invoke `removeUnusedNonterminalSyms()` because can not monitor if `semanticChecks.ruleMissingReducedSemantic()` remove ancestor rules (which are not the rules passed here).
	...
	removeUnusedNonterminalSyms(ruleSets)

	// Keep `semanticSafe` properties, added by `semanticChecks.ruleMissingReducedSemantic()`, until after adding edit-rules because the properties are necessary to check if each edit-rule can produce any required semantic.
}
