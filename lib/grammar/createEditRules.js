...
/**
 * Determines whether `newInsertion` already exists. Returns `true if insertions exists`. If `newInsertion` is cheaper than an existing insertion with identical text, removes the existing insertion and returns `false.
 *
 * @param {Object[]} symInsertions The existing insertions with the same LHS as `newInsertion`.
 * @param {Object} newInsertion The new insertion to add.
 * @returns {boolean} Returns `true` if `newInsertion` is identical to an existing insertion, else `false`.
 */
...
/**
 * Determines if `newRule` has identical LHS and RHS symbols as an existing rule. Throws an exception if `newRule` has identical RHS and display text to an existing non-insertion rule. If `newRule` has identical RHS symbols, semantics, but different display text and cheaper than an other insertion rule, then remove the other insertion rule and return `false`.
 *
 ...
 * @returns {boolean} Returns `true` if `newRule` is identical to an existing rule, else `false`.
 */
