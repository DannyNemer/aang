...
/**
 * Removes rules from `ruleSets` that produce the empty symbol, `<empty>`. Recursively removes any rule-less nonterminal symbols that result.
 *
	...
	grammarUtil.forEachRuleSet(ruleSets, function (rules) {
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rule.rhs[0] === g.emptySymbol) {
				...
				if (rulesLen === 0) {
					createdRulelessSym = true
				}
			}
		}
	})

	// After removing rules that produce the empty symbol, recursively remove any rule-less nonterminal symbols that result. If no rule-less symbols resulted above, it is not possible the removals rendered other symbols unreachable via the start symbol, which would also require invoking `removeUnusedNonterminalSyms()`, because `<empty>` only exists on terminal (unary) rules.
	if (createdRulelessSym) {
