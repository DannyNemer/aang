				...
				if (insertions && nonInsertedRHSSym !== nontermSym) {
					for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
						...
						if (origRule.rhsDoesNotProduceText) {
							...
							// Flatten `newRule`, the insertion rule created from a nonterminal rule derived from a multi-token terminal symbol in `splitRegexTerminalSymbols`, into several insertion rules, and add those rules to the grammar. The flattening, which merges `newRule` with the rules `newRule.rhs[0]` produces, enables `ruleIsUnique()` in `editRulesUtil` to properly discard ambiguous rules. E.g., X -> "x" vs. X -> Y -> "x".
