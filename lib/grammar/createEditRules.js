...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Remove instances of the rule property, `rule.semanticSafe`, used by `semanticChecks.ruleMissingReducedSemantic()` in `removeIllFormedRulesAndSyms`. Invoke after adding edit-rules, which relies on these properties to avoid edit-rules that can not produce a required reduced semantic.
	semanticChecks.removeSemanticChecksProps(ruleSets)
}
