					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							var newRule = {
								...
								RHS: insertedSymIdx === 1 && insertion.restrictInsertion ? [ otherSym, blankInsertedSymbol.name ] : [ otherSym ],
								...
								// Specify an insertion created using a rule that was originally a regex-style substitution, whose `RHS` `pfsearch` should not traverse becuase it produces no text.
								isSubstitution: rule.isSubstitution,
								// Specify an insertion created using a rule that was originally a regex-style stop word, whose `RHS` `pfsearch` should not traverse becuase it produces no text.
							...
							if (newRule.isSubstitution || newRule.isStopWord) {
								// The display text for nonterminal substitutions and nonterminal stop-words.
