 ...
 * @param {boolean} [ignoreAmbiguity] Specify not throwing exceptions for ambiguous rules. This is used to create the `ambiguityCheck` test rules.
 */
module.exports = function (ruleSets, includeTrees, ignoreAmbiguity) {
	...
	findNonterminalRulesProducingInsertions(ruleSets, insertions, ignoreAmbiguity)

	// Add new rules to the grammar created by sequences of inserted terminal symbols in `insertions`.
	createInsertionRules(ruleSets, insertions, includeTrees, ignoreAmbiguity)
	...
	// Remove instances of `gramProps` with no defined values.
 ...
 * @param {boolean} ignoreAmbiguity Specify not throwing an exception when attempting to add an ambiguous insertion.
 */
 ...
 * @param {boolean} ignoreAmbiguity Specify not throwing an exception when attempting to add an ambiguous insertion.
 */
function findNonterminalRulesProducingInsertions(ruleSets, insertions, ignoreAmbiguity) {
			...
			ruleSets[nontermSym].forEach(function (rule) {
				...
				if (rhsCanBeInserted(insertions, rule)) {
					rule.RHS.map(function (sym) {
						...
						addInsertion(insertions, nontermSym, newInsertion, ignoreAmbiguity)
					})
				}
			})
		})
 ...
 * @param {boolean} ignoreAmbiguity Specify not throwing an exception when attempting to add an ambiguous insertion.
 */
function createInsertionRules(ruleSets, insertions, includeTrees, ignoreAmbiguity) {
	Object.keys(ruleSets).forEach(function (nontermSym) {
				...
				RHS.forEach(function (sym, symIdx) {
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							...
							var parsePath = g.ruleMissingRHSSemantic(nontermSym, newRule)
							if (parsePath) {
							...
							// Add new rule to grammar if it is unique (i.e., unambiguous) and below the cost upper bound.
							if (ruleIsUnique(symRules, newRule, nontermSym, ignoreAmbiguity) && newRule.cost < maxCost) {
								if (!includeTrees) {
...
function addInsertion(insertions, nontermSym, newInsertion, ignoreAmbiguity) {
	...
	if (insertionIsUnique(symInsertions, newInsertion, nontermSym, ignoreAmbiguity) && newInsertion.cost < maxCost) {
...
function insertionIsUnique(symInsertions, newInsertion, nontermSym, ignoreAmbiguity) {
	for (var s = 0, symInsertionsLen = symInsertions.length; s < symInsertionsLen; ++s) {
		...
		// Throw an exception if semantics and/or display texts are identical.
		if (areAmbiguous(existingInsertion, newInsertion, nontermSym, ignoreAmbiguity)) return false
	}
...
function ruleIsUnique(rules, newRule, nontermSym, ignoreAmbiguity) {
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (util.arraysEqual(existingRule.RHS, newRule.RHS)) {
			// Throw an exception if semantics and/or display texts are identical.
			if (areAmbiguous(existingRule, newRule, nontermSym, ignoreAmbiguity)) return false
		}
	}
...
function areAmbiguous(a, b, nontermSym, ignoreAmbiguity) {
	...
	if (semanticsEquivalent || textsEquivalent) {
		...
		util.dir(util.stylize(nontermSym), a, b, '\n')
	...
	for (var nontermSym in ruleSets) {
		...
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rule.insertionIdx === undefined) {
				var parsePath = g.ruleMissingRHSSemantic(nontermSym, rule)
				if (parsePath) {
