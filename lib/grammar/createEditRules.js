					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							var newRule = {
								...
								cost: rule.cost + insertion.cost,
								// The parent rule's grammatical properties. Do not save grammatical properties from insertions because they only apply to direct terminal rule descendants.
								gramProps: rule.gramProps,
								...
								personNumber: symIdx === 0 ? (rule.personNumber || insertion.personNumber) : rule.personNumber,
							...
							if (insertion.text[0]) {
								// Only save `insertionIdx` for rules with text, otherwise the new rule (produced via `<empty>`) is an ordinary unary nonterminal rule.
								newRule.insertionIdx = symIdx

								// Conjugate text using the rule's grammatical properties and the most recent person-number property.
								var text = conjugateText(insertion.text, newRule.gramProps, newRule.personNumber)
 ...
 * @param {Object} gramProps The insertion's parent rule's grammatical properties (`verbForm` and `gramCase`) for conjugating text objects in `insertionText`.
...
function conjugateText(insertionText, gramProps, personNumber) {
	return insertionText.reduce(function (textArray, text) {
		...
		else if (text[gramProps.verbForm]) {
