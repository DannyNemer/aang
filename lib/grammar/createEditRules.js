 ...
 * @param {boolean} [stopAmbiguity] Specify printing an error and throwing an exception for a pair of ambiguous rules, else save the cheapest of the two to the grammar.
 */
		...
		for (var nontermSym in ruleSets) {
			ruleSets[nontermSym].forEach(function (rule) {
				if (rhsCanBeInserted(insertionSets, rule)) {
					...
					// When `rule` is binary and both RHS symbols have insertions, merges every possible pair of insertions produced by both RHS symbols.
					.reduce(mergeInsertionSets)
...
/**
 * Creates a set of insertions constructed by merging every possible pair of insertions in sets `insertionsA` and `insertionsB`. For use by `findNonterminalRuleInsertions()` when creating an insertion from a binary rule for which both RHS symbols have insertions.
 *
 * @private
 * @static
 * @param {Object[]} insertionsA The set of insertions to merge.
 * @param {Object[]} insertionsB The other set of insertions to merge.
 * @returns {Object[]} Returns a set of insertions constructed by merging every possible pair of insertions in `insertionsA` and `insertionsB`.
 */
function mergeInsertionSets(insertionsA, insertionsB) {
...
function createInsertionRules(ruleSets, insertionSets, stopAmbiguity) {
	...
	// Add this symbol to the grammar here to prevent `removeIllFormedRulesAndSyms()` from removing it earlier.
			...
			for (var insertedSymIdx = 0; insertedSymIdx < rhsLen; ++insertedSymIdx) {
				...
				if (insertions && nonInsertedRHSSym !== nontermSym) {
					for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
						...
						// Discard insertion if lacks semantics and produces superfluous text.
						if (isUselessInsertion(ruleSets, insertion, insertedRHSSym, nontermSym, origRule, true)) {
							continue
						}
...
/**
 * Checks if `insertion` lacks semantics and produces superfluous text. For use by `createInsertionRules()` to discard useless insertions.
 *
 * Examples:
 ...
 *
 * Note: The grammar currently does not produce rules with this characteristic because of particular rules defined with insertion restrictions (e.g., `noInsert`).
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
 ...
 * @param {string} insertedRHSSym The `origRule` RHS symbol to insert that produces `insertion`.
 * @param {string} nontermSym The nonterminal symbol that produces `origRule`.
 * @param {Object} origRule The nonterminal rule from which to create an insertion rule using `insertion`.
 * @param {boolean} [printWarning] Specify printing a warning if `insertion` is useless.
 * @returns {boolean} Returns `true` if `insertion` is useless and should be discarded, else `false`.
 */
function isUselessInsertion(ruleSets, insertion, insertedRHSSym, nontermSym, origRule, printWarning) {
	/**
	 * First check `insertion` for the following:
	 * 1. Produces text; i.e., terminal symbols are not only `<empty>`(s).
	 * 2. Lacks a semantic when `insertedRHSSym` could produce a semantic, unlike symbols that never produce a semantic, such those for terminal rule sets.
	 */
	if (insertion.text[0] && !insertion.semantic && semanticPotential.symCanProduceSemantic(ruleSets, insertedRHSSym)) {
		var nonInsertedRHSSym = origRule.rhs[Number(!origRule.rhs.indexOf(insertedRHSSym))]

		/**
		 * Check if the non-inserted sibling symbol can not produce a semantic.
		 *
		 * For example, stops the following insertions:
		 * • "are" -> "... people"
		 * • "that" -> "... are people"
		 * • "are not" -> "... people"
		 *   - `semanticChecks.ruleMissingReducedSemantic()` in `editRulesUtil.addRule()` would stop this rule anyway for being unable to produce a semantic required for the parent `not()` semantic.
		 */
		if (!semanticPotential.symCanProduceSemantic(ruleSets, nonInsertedRHSSym)) {
			if (printWarning) {
				printUselessInsertionWarning(insertion, insertedRHSSym, nontermSym, origRule)
			}
		...
		/**
		 * Check if `insertion` is for a complete clause (which is meaningless because it lacks semantics), formed from the rule:
		 *   [cat-plural] -> [cat-no-relative] [cat-relative-clause], intersect()
		 *
		 * 1. Insert: [cat-no-relative] -> issues
		 *    - Enables "that are open" -> "issues (that are repos)", which is desirable and not discarded.
		 *
		 * 2. Insert: [cat-relative-clause]
		 *    - Yields "repos" -> "... that are repos", which should be discarded.
		 *    - Stopped by `rulesAreAmbiguous()` in `editRulesUtil.addRule()`, which finds the rule ambiguous with the non-edit rule: [cat-plural] -> [cat-no-relative], intersect()
		 *    - The existence of this other non-edit rule demonstrates `[cat-relative-clause]` is optional and `[cat-no-relative]` can form a complete clause independently. Hence, `[cat-relative-clause]` can produce subtrees that are also complete clauses that can be excluded without affecting the meaning of the `[cat-no-relative]` branch (except in case #1). Hence, if the `[cat-relative-clause]` insertion lacks semantics, yet is a branch for a complete clause, then the clause is meaningless and should be discarded.
		 *    - This condition can also exist for a similar binary rule where one of the RHS symbols is optional (i.e., produces `<empty>`), which would also be caught by rulesAreAmbiguous()` (because both rules produce no semantics for the same RHS). The function would replace the useless insertion with the `<empty>` insertion, which is guaranteed to be cheaper because it lacks display text.
		 */
		else if (origRule.semantic) {

		}

		/**
		 * Check if `insertion`, though semantic-less with the potential for semantics, completes a rule with a semantic that can not exist without completing the rule. This should not be discarded.
		 *
		 * For example:
		 * • "liked by me" -> "repos (liked by me)"
		 * • "I like" -> "repos (I like)"
		 * • "open" -> "(open) issues"
		 *   - Without this semantic-less insertion, only the following would be possible:
		 *       "open" -> "my (open) issues".
		 */
		else {
...
/**
 * Prints a warning for the useless insertion, `insertion`, which lacks semantics and produces superfluous text. For use by `isUselessInsertion()` upon determining `insertion` should be discarded.
 *
 * @private
 * @static
 * @param {Object} insertion The inspected insertion for which to print a warning.
 * @param {string} insertedRHSSym The `origRule` RHS symbol that produces `insertion`.
 * @param {string} nontermSym The nonterminal symbol that produces `origRule`.
 * @param {Object} origRule The nonterminal rule from which to create an insertion rule using `insertion`.
 */
function printUselessInsertionWarning(insertion, insertedRHSSym, nontermSym, origRule) {
	util.logWarning('Discarded useless insertion:')
	util.log('  Original rule:', grammarUtil.stringifyRule(nontermSym, origRule))
 ...
 *
 * Invoke this function after removing ill-formed nonterminal symbols and rules in `removeIllFormedRulesAndSyms()`, and before adding edit-rules which inherit the semantic properties of their original (non-edit) rules.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
	...
	grammarUtil.forEachRule(ruleSets, function (rule) {
		if (!rule.isTerminal) {
			...
			if (rule.rhs.length === 2) {
				...
				rule.secondRHSCanProduceSemantic = semanticPotential.symCanProduceSemantic(ruleSets, rule.rhs[1])
			}

			rule.rhsCanProduceSemantic = rule.secondRHSCanProduceSemantic || semanticPotential.symCanProduceSemantic(ruleSets, rule.rhs[0])
		}
	})
}
