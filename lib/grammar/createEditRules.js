...
var ruleMissingRHSSemantic = require('./ruleMissingRHSSemantic')
...
module.exports = function (ruleSets, includeTrees, stopAmbiguity) {
	...
	// Recursively remove nonterminal symbols with no rules, rules whose RHS contains those symbols or `<empty>`, and any rule-less nonterminal symbols that result. Invoked after finding and creating insertions from rules with `<empty>` in `findTerminalRuleInsertions()`, and before `createInsertionRules()` which would otherwise create insertion rules using the nonterminal rules this removes.
	removeNullNonterminalSymbols(ruleSets)

	// Check for default (non-edit) rules that lack and cannot produce a needed RHS semantic. Throws an exception if finds any. Invoked after removing rules with rule-less symbols in `removeNullNonterminalSymbols()`.
				...
				rhs.forEach(function (sym, insertedSymIdx) {
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							...
							if (ruleMissingRHSSemantic(ruleSets, nontermSym, newRule)) return
							...
							if (rule.rhsDoesNotProduceText) {
								...
								// Flatten some rules to enable `ruleIsUnique()` in `editRulesUtil` to detect instances of ambiguity and properly discard those rules. For example: X -> "x" vs. X -> Y -> "x".
...
/**
 * Checks for default (non-edit) rules that lack and cannot produce a needed RHS semantic. Throws an exception if finds any.
 *
	...
	for (var nontermSym in ruleSets) {
		...
		for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
			...
			if (rule.insertedSymIdx === undefined) {
				...
				if (ruleMissingRHSSemantic(ruleSets, nontermSym, rule, true)) {
					throw new Error('Semantic error')
				}
			}
		}
 ...
 *
 * Invoked at the conclusion of `createEditRules` when invoked with falsey `includeTrees`.
 *
