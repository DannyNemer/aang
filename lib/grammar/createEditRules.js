			...
			ruleSets[nontermSym].forEach(function (rule) {
				...
				if (rhsCanBeInserted(insertionSets, rule)) {
					rule.rhs.map(function (sym) {
						...
						return insertionSets[sym].map(function (insertion) {
							return {
								cost: insertion.cost,
								text: insertion.text,
		...
		ruleSets[nontermSym].forEach(function (rule, ruleIdx, symRules) {
			...
			if (rhs.length > 1) {
				rhs.forEach(function (sym, insertedSymIdx) {
					...
					if (insertionSets.hasOwnProperty(sym) && nonInsertedRHSSym !== nontermSym) {
						insertionSets[sym].forEach(function (insertion) {
							var newRule = {
