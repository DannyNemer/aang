				...
				if (insertions && nonInsertedRHSSym !== nontermSym) {
					for (var i = 0, insertionsLen = insertions.length; i < insertionsLen; ++i) {
						...
						var newRule = {
							...
							// Save parse trees graph for `ruleIsUnique()` errors in `editRulesUtil`. `removeTempGrammarPropsAndRules` removes this property at the end of grammar generation if `buildGrammar` was invoked without '--trees'.
							tree: [ { symbol: insertedRHSSym, children: insertion.tree } ],
						}
