...
function insertionIsUnique(symInsertions, newInsertion) {
	for (var s = 0, symInsertionsLen = symInsertions.length; s < symInsertionsLen; ++s) {
		...
		if (textIsEqual(existingInsertion.text, newInsertion.text)) {
...
function ruleIsUnique(rules, newRule, lhsSym) {
	for (var r = 0, rulesLen = rules.length; r < rulesLen; ++r) {
		...
		if (util.arraysEqual(existingRule.RHS, newRule.RHS)) {
			...
			if (textIsEqual(existingRule.text, newRule.text)) {
...
/**
 * Performs a comparison between two rules' text to determine if they are equivalent.
 *
...
function textIsEqual(a, b) {
	...
	if (constructorA === Array && constructorB === Array) {
		return util.arraysEqual(a, b, textIsEqual)
	}
	...
	// Input text are of different type or are different strings.
	return false
}
