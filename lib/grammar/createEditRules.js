			...
			if (rule.isTerminal) {
				...
				if (termSym === g.emptySymbol) {
					...
					addInsertion(insertions, nontermSym, {
						...
						hasOneSg: /\[1-sg\]|\[obj-users\]/.test(nontermSym),
					...
					if (insertions.hasOwnProperty(sym) && otherSym !== nontermSym) {
						insertions[sym].forEach(function (insertion) {
							var newRule = {
								...
								// Specify an insertion created using a rule that was originally a multi-token substitution, whose `RHS` `pfsearch` should not traverse becuase it produces no text.
								isSubstitution: rule.isSubstitution,
								// Specify an insertion created using a rule that was originally a multi-token stop word, such as a multi-token symbol that contained an `<empty>`, whose `RHS` `pfsearch` should not traverse becuase it produces no text.
