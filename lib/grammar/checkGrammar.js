...
/**
 * Checks `ruleSets` for errors after constructing all non-edit and edit rules. Such errors occur across multiple rules and therefore are only evident after rule construction completes.
 *
 * @static
 ...
 *
 * Term sequences can not produce semantics because `flattenTermSequence` does not check for semantics in in child node when flattening term sequence nodes, only the sequence nodes themselves.
 *
 * @private
 * @static
 * @param {Object} ruleSets The map of the grammar's nonterminal symbols to rules.
...
/**
 * Checks if `rule.text` is an ill-formed `text` array. If so, prints an error and thorws an exception.
 *
 ...
 * @returns {boolean} Throws an exception if `rule.text` is ill-formed, else returns `false`.
 */
