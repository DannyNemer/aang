...
Symbol.prototype.newNonterminalRule = function (options) {
	...
	if (options.RHS.length > 2) {
		util.logErrorAndPath('Nonterminal rule has > 2 RHS symbols:', util.stylize(this.name), '->', newRule.RHS)
		throw new Error('Ill-formed nonterminal rule')
	}

	if (options.transpositionCost !== undefined) {
		if (options.RHS.length !== 2) {
			util.logErrorAndPath('Nonterminal rules with transposition cost does not have 2 RHS symbols:', util.stylize(this.name), '->', newRule.RHS)
			throw new Error('Ill-formed nonterminal rule')
		}
...
var termRuleSchema = {
	...
	isStopWord: Boolean,
	// Specify this rule substitutes the input terminal symbol as display text. This is a temporary solution used by `splitMutliTokenTerminals`.
