...
var nontermRuleOptionsSchema = {
	// An array of `Symbol`s, `Symbol` names (as `string`s), and/or nested arrays of RHS for new binary rules to recursively create. However, these sub-rules can only contain a RHS and not other rule properties.
	RHS: { type: Array, arrayType: [ Symbol, Array, String ] },
...
Symbol.prototype.newNonterminalRule = function (options) {
	...
	var newRule = {
		// RHS can contain `Symbol`s, `Symbol` names (as `string`s), and/or nested arrays of RHS for new binary rules to recursively create.
		RHS: options.RHS.map(function (sym) {
			if (sym.constructor === Array) {
				...
				// Check `Symbol` with name exists.
				if (!exports.ruleSets.hasOwnProperty(sym)) {
					util.logErrorAndPath('Unrecognized Symbol name in \'RHS\':', util.stylize(sym), options)
					throw new Error('Ill-formed nonterminal rule')
				}
...
exports.newBinaryRule = function (options) {
	...
	var symbolNameTokens = options.RHS.map(function (sym, i) {
		// `sym` can be a `Symbol` or `Symbol` name.
