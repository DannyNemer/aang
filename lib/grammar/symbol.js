...
var nontermRuleSchema = {
	...
	// The display text for multi-symbol text substitution; e.g., "about","which" -> "that". This rule's RHS does not produce any text, but rather uses this rule's `text`.
	text: { type: [ String, Object ] },
	// The cost penalty for multi-symbol text substitution.
	costPenalty: Number,
}
...
Symbol.prototype.newNonterminalRule = function (options) {
	...
	// The cost penalty for multi-symbol text substitution.
	var costPenalty = options.costPenalty ? options.costPenalty : 0
	...
	if (options.semantic) {
		...
		// Calculate rule cost which include's the semantic's cost.
...
var termRuleSchema = {
	...
	// Use `options.text: ''` for a terminal rule with no display text (e.g., symbols used in multi-symbol substitution, stop-words).
	text: { type: [ String, Object ] },
