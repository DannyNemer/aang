 ...
 *
 * @constructor
 * @param {...string} [nameTokens] The tokens to hyphenate for the new `Symbol`'s name.
 */
function Symbol() {
	this.name = exports.genName.apply(null, arguments)
	...
	if (exports.ruleSets.hasOwnProperty(this.name)) {
		...
		util.log('  ' + exports.creationLines[this.name])
...
var nontermRuleSchema = {
	// The array of `Symbol`s, `Symbol` names, and/or nested arrays of RHS for new binary rules to recursively create. However, these sub-rules can only contain a RHS and not other rule properties.
	rhs: { type: Array, arrayType: [ Symbol, String, Array ] },
	...
	// The person-number for anaphoric rules with which to match to an antecedent semantic of the same person-number. E.g., "his/her" refers to semantics of third-person-singular representations.
	anaphoraPersonNumber: { values: [ 'threeSg' ] },
}
...
Symbol.prototype.newNonterminalRule = function (options) {
	...
	var newRule = {
		// `Symbol`s, `Symbol` names, and/or nested arrays of RHS for new binary rules to recursively create.
		rhs: options.rhs.map(function (sym) {
			if (sym.constructor === String) {
				if (!exports.ruleSets.hasOwnProperty(sym)) {
	...
	for (var propName in newRule.gramProps) {
		...
		if (gramProp && !exports.rhsAcceptsGramProp(newRule.rhs, gramProp)) {
 ...
 */
exports.rhsAcceptsGramProp = function (rhs, gramProp) {
...
exports.newBinaryRule = function (options) {
	...
	var symbolNameTokens = options.rhs.map(function (sym, i) {
		var name = sym.constructor === Symbol ? sym.name : sym
...
// Extend `Symbol` with methods for predefined sets of rules (e.g., verbs, stop words).
