...
/**
 * The inflections of a verb, from which `terminalRuleSetMethods.newVerb()` creates a terminal rule set where each rule of this set has as an object as its `text` with the properties `oneSg`, `threeSg`, `pl`, and `past`. When constructing parse trees, `pfsearch` conjugates this `text` object to the correct verb form (i.e., display text) according to grammatical properties in preceding nonterminal rules in the same tree.
 *
...
exports.newVerb = function (options) {
	...
	options.acceptedVerbTermSets.forEach(function (verbTermSet, i) {
		...
		// Add all remaining unique verb forms as terminal rules to be substituted when input by the appropriate verb form added as a terminal rule above.
 ...
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the `NSymbol`.
 * @param {number} [options.insertionCost] The insertion cost for the terminal rule set, assigned to the first rule of the first noun set in `options.acceptedNounTermSets`. Enables the creation of insertion rules using the `NSymbol` that produces these sets.
...
exports.newNoun = function (options) {
	...
	options.acceptedNounTermSets.forEach(function (nounTermSet, i) {
		...
		if (i === 0) {
			...
			// Assign the insertion cost, if any, to the first terminal rule in the first set.
			if (options.insertionCost !== undefined) {
				plTermRule.insertionCost = options.insertionCost
			}
		}

		nounSym.addRule(plTermRule)
 ...
 *
 * @type {Object} SubstitutionTerm
 * @property {string} term The terminal symbol to substitute when seen in input.
 * @property {number} costPenalty The substitution cost penalty.
 */
var substitutionTermSchema = {
	term: { type: String, required: true },
	costPenalty: { type: Number, required: true },
}

/**
 * Creates an `NSymbol` that produces terminal rules for a set of uninflected terms.
 *
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object.
 * @param {string} options.symbolName The name for the `NSymbol`.
 * @param {number} [options.insertionCost] The insertion cost for the terminal rule set, assigned to the first term in `options.acceptedTerms`. Enables the creation of insertion rules using the `NSymbol` that produces this set.
 * @param {string[]} options.acceptedTerms[] The uninflected terms accepted when input.
 * @param {(string|SubstitutionTerm)[]} [options.substitutedTerms[]] The uninflected terms substituted when input by the first term in `options.acceptedTerms`, defined with cost penalties (`SubstitutionTerm`) or without (`string`).
 * @returns {NSymbol} Returns the `NSymbol` for the terminal rule set.
 */
var termSchema = {
	symbolName: { type: String, required: true },
...
exports.newTerm = function (options) {
	...
	// Add the terminal rules that are substituted when input by the first term in `options.acceptedTerms`.
	if (options.substitutedTerms) {
		// The display text for the first terminal rule substitutes the terminal rules in `options.substitutedTerms` when input.
		var defaultText = options.acceptedTerms[0]

		options.substitutedTerms.forEach(function (term) {
			var costPenalty = 0
			if (term.constructor === Object) {
				if (util.illFormedOpts(substitutionTermSchema, term)) {
...
/**
 * Checks `options[paramName]` is a verb created by `terminalRuleSetMethods.newVerb()` that has inflected text for past tense. If not, prints an error message.
 *
 * @static
 * @memberOf terminalRuleSetMethods
...
/**
 * Checks `options[paramName]` is a verb created by `terminalRuleSetMethods.newVerb()` that lacks inflected text for past tense. If not, prints an error message.
 *
 * @static
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object with the property to inspect.
...
/**
 * Checks `options[paramName]` is a verb created by `terminalRuleSetMethods.newVerb()` that has inflected text for past tense if `hasPastTense` is truthy or lacks past tense if `hasPastTense` is falsey. If not, prints an error message.
 *
 * @private
 * @static
 * @param {Object} options The options object with the property to inspect.
...
function baseIsVerb(options, paramName, hasPastTense) {
	...
	// Check `options[paramName]` is a verb created by `terminalRuleSetMethods.newVerb()`.
	if (textForms.constructor !== Object || !textForms.oneSg || !textForms.threeSg || !textForms.pl) {
		printTerminalRuleSetErr(options, paramName, terminalRule, 'is not a verb created by `g.newVerb()`')
		return false
	}

	if (hasPastTense) {
		// Check `options[paramName]` has inflected text for past tense, even though optional for `terminalRuleSetMethods.newVerb()` (e.g., `[be]`).
		if (!textForms.past) {
 ...
 *
 * Such terminal rule sets are created by `terminalRuleSetMethods.newNoun()` or `terminalRuleSetMethods.newTerm()`.
 *
 * @static
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object with the property to inspect.
 ...
 * @param {Object} terminalRule The terminal rule at faul in `options[paramName].rules` to print.
