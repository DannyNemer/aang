		...
		options.substitutedVerbTermSets.forEach(function (verbTermSet) {
			...
			if (verbTermSet.past) {
				...
				verbSym.addRule({
					...
					rhs: verbTermSet.past,
					/**
					 * Define `tense` for the parent nonterminal rule property `acceptedTense`, which uses the verb form of the same tense when the rule is matched in input, but will only conjugate to this form (via any rule in the set) for the nonterminal rule property `grammaticalForm`.
					 *
