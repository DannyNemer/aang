...
/**
 * Checks `options[paramName]` is a verb created by `terminalRuleSetMethods.newVerb()` and has inflected text for past tense. If not, prints an error message.
 *
 * @static
 * @memberOf terminalRuleSetMethods
...
/**
 * Checks `options[paramName]` is a verb created by `terminalRuleSetMethods.newVerb()` and lacks inflected text for past tense. If not, prints an error message.
 *
 * @static
 * @memberOf terminalRuleSetMethods
 * @param {Object} options The options object with the property to inspect.
 * @param {string} paramName The `options` property to inspect.
 * @returns {boolean} Returns `true` if `options[paramName]` is a verb without past tense, `false`.
 */
...
/**
 * Checks `options[paramName]` is a verb created by `terminalRuleSetMethods.newVerb()` and has inflected text for past tense if `hasPastTense` is truthy or lacks past tense if `hasPastTense` is falsey. If not, prints an error message.
 *
 * @private
 * @static
 * @param {Object} options The options object with the property to inspect.
 ...
 * @returns {boolean} Returns `true` if `options[paramName]` is a verb with past tense if `hasPastTense` is truthy or without past tense if `hasPastTense` is falsey, `false`.
 */
function baseIsVerb(options, paramName, hasPastTense) {
	...
	if (hasPastTense) {
		...
		if (textForms.past) {
			printTerminalRuleSetErr(options, paramName, 'has past tense inflection')
			return false
		}
	}
 ...
 * @param {string[]} [options.substitutedTerms[]] The uninflected terms substituted when input by the first term in `options.acceptedTerms`.
 * @returns {NSymbol} Returns the `NSymbol` for the terminal rule set.
 */
var termSchema = {
	symbolName: { type: String, required: true },
	...
	substitutedTerms: { type: Array, arrayType: String },
}
