...
exports.newTerm = function (options) {
	...
	// The terminal rule `text` string from the first term in `options.acceptedTerms`, used as the display text for all terms in `options.substitutedTerms`, if any.
	var defaultText
	...
	options.acceptedTerms.forEach(function (term, i) {
		...
		if (i === 0) {
			// Save the display text for the first terminal rule , which will substitute the terminal rules in `options.substitutedTerms`, if any, when input.
			defaultText = term

			// Assign the insertion cost, if any, to the first terminal rule.
			if (options.insertionCost !== undefined) {
