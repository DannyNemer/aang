 ...
 * @property {string} [sg] The singular noun form, substituted when input with `pl`.
 ...
 * @param {NounTermSet[]} [options.substitutedNounTermSets[]] The noun terminal rule sets substituted when input with the appropriate form in the first noun set in `options.acceptedNounTermSets`. The parametrization of each form, as opposed to an array of terminal symbols, enforces complete definition of a noun set.
 * @returns {NSymbol} Returns the `NSymbol` for the terminal rule set.
 */
var nounSchema = {
	symbolName: { type: String, required: true },
...
exports.newNoun = function (options) {
	...
	// Add the terminal rule noun sets that are substituted when input with the first noun set in `options.acceptedNounTermSets`.
 ...
 * @param {(string|SubstitutedTerm)[]} [options.substitutedTerms[]] The uninflected terms substituted when input with the first term in `options.acceptedTerms`, defined with cost penalties (`SubstitutedTerm`) or without (`string`).
 * @returns {NSymbol} Returns the `NSymbol` for the terminal rule set.
 */
var termSchema = {
	symbolName: { type: String, required: true },
...
exports.newTerm = function (options) {
	...
	// Add the terminal rules that are substituted when input with the first term in `options.acceptedTerms`.
