...
/**
 * The inflections of a verb, from which `g.newVerb()` forms a terminal rule set where each terminal rule has as its `text` an object with the properties `oneSg`, `threeSg`, `pl`, and `past`. When constructing parse trees, `pfsearch` conjugates the `text` object to the correct form (i.e., display text) according to grammatical properties in preceding nonterminal rules in the same tree.
 *
 ...
 *
 * The grammar generator and `pfsearch` do not use `presentSubjunctive`, `presentParticiple`, and `pastParticiple` for conjugation. Rather, their definitions serve only to provide complete substitution sets, replaced when input by one of the forms in the set with conjugation support.
 *
 ...
 * @property {string} oneSg The first-person-singular form, chosen by the nonterminal rule property `personNumber`. E.g., "am", "was", "like".
 * @property {string} threeSg The third-person-singular form, chosen by the nonterminal rule property `personNumber`. E.g., "is", "was", "likes".
 * @property {string} pl The plural form, chosen by the nonterminal rule property `personNumber`. E.g., "are", "were" "like".
 * @property {string} [past] The past-tense form, chosen by the parent nonterminal rule property `grammaticalForm` and accepted when input by `acceptedTense`. E.g, "was", "liked", "did".
 * @property {string} [presentSubjunctive] The present-subjunctive form, substituted when input by one of the first four forms. E.g., "be".
 * @property {string} [presentParticiple] The present-participle form, substituted when input by one of the first four forms. E.g., "being", "liking".
 * @property {string} [pastParticiple] The past-participle form, substituted when input by one of the first four forms. E.g., "been", "done".
 */
 ...
 * @param {VerbTermSetSchema[]} [options.acceptedVerbTermSets[]] The verb terminal rule sets accepted when input. Add the terminal rules in set, so that the incorrect inflection is conjugated to the correct form of the same word.
...
exports.newVerb = function (options) {
	...
	options.acceptedVerbTermSets.forEach(function (verbTermSet, i) {
		...
		// The terminal rule for first-person-singular. E.g., "am", "was", "like".
		var newRule = {
			isTerminal: true,
		...
		verbSym.addRule(newRule)
		addedInlectionTermSyms.push(verbTermSet.oneSg)

		// If the terminal symbol is unique, add the terminal rule for third-person-singular, for use in the present indicative tense. E.g, "is", "was", "likes".
		...
		// If the terminal symbol is unique, add the terminal rule for plural. E.g, "are", "were", "like".
	...
	// Add the terminal rule verb sets that are substituted when input by the appropriate form in first verb set in `options.acceptedVerbTermSets`.
