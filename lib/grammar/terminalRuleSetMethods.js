	...
	options.acceptedVerbTermSets.forEach(function (verbTermSet, i) {
		...
		if (verbTermSet.past) {
			...
			verbSym.addRule({
				...
				// Define `tense` for the parent nonterminal rule property `acceptedTense`, which uses the verb form of the same tense when a terminal rule of that tense is matched in input, but does not conjugate to that tense if not input unless the parent rule property `grammaticalForm` dictates as such.
				tense: 'past',
				text: verbSetTextForms,
			})
		...
		if (verbTermSet.presentSubjunctive && addedInlectionTermSyms.indexOf(verbTermSet.presentSubjunctive) === -1) {
			verbSym.addRule({
				isTerminal: true,
				rhs: verbTermSet.presentSubjunctive,
				text: verbSetTextForms,
			})
		...
		if (verbTermSet.presentParticiple && addedInlectionTermSyms.indexOf(verbTermSet.presentParticiple) === -1) {
			verbSym.addRule({
				isTerminal: true,
				rhs: verbTermSet.presentParticiple,
				text: verbSetTextForms,
			})
		...
		if (verbTermSet.pastParticiple && addedInlectionTermSyms.indexOf(verbTermSet.pastParticiple) === -1) {
			verbSym.addRule({
				isTerminal: true,
				rhs: verbTermSet.pastParticiple,
				// Define `tense` for the parent nonterminal rule property `acceptedTense`, which uses the verb form of the same tense when a terminal rule of that tense is matched in input, but does not conjugate to that tense if not input unless the parent rule property `grammaticalForm` dictates as such.
				// For nonterminal rules for which this true, when this past-participle is matched in input, it is replaced with `verbSetTextForms.past`.
				tense: 'past',
				text: verbSetTextForms,
			})
		...
		options.substitutedVerbTermSets.forEach(function (verbTermSet) {
			...
			if (verbTermSet.past) {
				...
				verbSym.addRule({
					 ...
					 * Define `tense` for the parent nonterminal rule property `acceptedTense`, which uses the verb form of the same tense when a terminal rule of that tense is matched in input, but does not conjugate to that tense if not input unless the parent rule property `grammaticalForm` dictates as such.
					 *
					 * This input tense is maintained for verb substitutions. For example:
			...
			if (verbTermSet.pastParticiple && addedInlectionTermSyms.indexOf(verbTermSet.pastParticiple) === -1) {
				verbSym.addRule({
					...
					rhs: verbTermSet.pastParticiple,
					// Define `tense` for the parent nonterminal rule property `acceptedTense`, which uses the verb form of the same tense when a terminal rule of that tense is matched in input, but does not conjugate to that tense if not input unless the parent rule property `grammaticalForm` dictates as such.
					// For nonterminal rules for which this true, when this past-participle is matched in input, it is replaced with `defaultTextForms.past`.
					tense: 'past',
					text: defaultTextForms,
				})
				addedInlectionTermSyms.push(verbTermSet.pastParticiple)
