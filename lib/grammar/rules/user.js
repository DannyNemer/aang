...
// Identical rule structure to rules `conjunctions.create()` produces, though defined here because several of the sub-symbols are needed elsewhere in this module.
...
// (people I and/or) `[nom-pl-users]` (follow)
nomPlUsersPlus.addRule({
	RHS: [ nomPlUsers ],
})

var nomPlUsersAnd = g.newBinaryRule({
	RHS: [ nomPlUsers, conjunctions.and ],
	// Could allow insertion of `[and]`, but its insertion cost is too high to be effective.
	noInsertionIndexes: [ 0, 1 ],
})

var nomPlUsersPlusNoUnion = g.newSymbol(nomPlUsersPlus.name, 'no', 'union')
nomPlUsersPlusNoUnion.addRule({
	RHS: [ nomPlUsers ],
}).addRule({
	RHS: [ nomPlUsersAnd, nomPlUsersPlusNoUnion ],
	noInsertionIndexes: [ 0, 1 ],
})

// (people I and/or) `[nom-pl-users]` and `[nom-pl-users]` [and `[nom-pl-users]` ...] (follow)
nomPlUsersPlus.addRule({
	RHS: [ nomPlUsersAnd, nomPlUsersPlusNoUnion ],
	noInsertionIndexes: [ 0, 1 ],
})
...
var nomPlUsersPlusNoUnionOr = g.newBinaryRule({
	RHS: [ nomPlUsersPlusNoUnion, conjunctions.union ],
	noInsertionIndexes: [ 0, 1 ],
	semantic: conjunctions.intersectSemantic,
})
var nomPlUsersPlusIntersect = g.newSymbol(nomPlUsersPlus.name, 'intersect').addRule({
	RHS: [ nomPlUsersPlus ],
	semantic: conjunctions.intersectSemantic,
})
// (people I and/or) `[nom-pl-users]` [and `[nom-pl-users]` ...] or `[nom-pl-users+]` (follow)
nomPlUsersPlus.addRule({
	RHS: [ nomPlUsersPlusNoUnionOr, nomPlUsersPlusIntersect ],
	noInsertionIndexes: [ 0, 1 ],
	semantic: conjunctions.unionSemantic,
})


// Do not use `conjunctions.create()` because conjunctions must use rules without person-number properties to enable proper conjunctions.
...
// (people) `[nom-pl-users]` [and `[nom-pl-users]` ...] (follow)
user.nomUsersPlus.addRule({
	RHS: [ nomPlUsersAnd, nomPlUsersPlusNoUnion ],
	noInsertionIndexes: [ 0, 1 ],
	personNumber: 'pl',
...
user.nomPlUsersSubj = g.newSymbol('nom', 'pl', user.namePl, 'subj')
// (followers) `[user-plural]` (share)
user.nomPlUsersSubj.addRule({
	RHS: [ user.plural ],
	personNumber: 'pl',
})
// (followers) `[nom-pl-users]` [and `[nom-pl-users]` ...] (share)
user.nomPlUsersSubj.addRule({
	RHS: [ nomPlUsersAnd, nomPlUsersPlusNoUnion ],
	noInsertionIndexes: [ 0, 1 ],
	personNumber: 'pl',
...
// Following logic's order of operations, "and" takes precedence over "or". Hence, segments between "or" are grouped together with `intersect()`, which are then grouped together in `union()`.
var userNomPlUsersSubjNoUnion = g.newSymbol(user.nomPlUsersSubj.name, 'no', 'union')
// (followers) my followers (or I and {user} share)
userNomPlUsersSubjNoUnion.addRule({
	RHS: [ user.plural ],
})
...
var userNomPlUsersSubjNoUnionOr = g.newBinaryRule({
	RHS: [ userNomPlUsersSubjNoUnion, conjunctions.union ],
	semantic: conjunctions.intersectSemantic,
	noInsertionIndexes: [ 0, 1 ],
})
var userNomPlUsersSubjIntersect = g.newSymbol(user.nomPlUsersSubj.name, 'intersect').addRule({
...
user.nomPlUsersSubj.addRule({
	RHS: [ userNomPlUsersSubjNoUnionOr, userNomPlUsersSubjIntersect ],
	semantic: conjunctions.unionSemantic,
	noInsertionIndexes: [ 0, 1 ],
	personNumber: 'pl',
})
...
user.objUsers.addRule({ RHS: [ user.term ], semantic: allUsersSemantic, noInsertionIndexes: [ 0 ] })
