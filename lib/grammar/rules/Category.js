...
function Category(options) {
	...
	reducedNoTense.addRule({ rhs: [ conjunctions.create(this.passive) ] })
	...
	// (repos) that I like
	// (people) who are followed by me
	var filterPlus = createFilterPlusRuleSet(filter, options.isPerson)
 ...
 * @returns {NSymbol} Returns the `[cat-filter+]` `NSymbol`.
 */
function createFilterPlusRuleSet(filter, isPerson) {
 ...
 *
 * `options.userVerbSemantic` must the property `isPeople` defined as `true` to enable the verb's use as an antecedent for anaphora. For example:
...
Category.prototype.addAgentNoun = function (options) {
	...
	// Check `options.userVerbSemantic` has the property `isPeople` defined as `true` to enable the noun's use as an antecedent for anaphora.
	if (!options.userVerbSemantic[0].semantic.anaphoraPersonNumber) {
