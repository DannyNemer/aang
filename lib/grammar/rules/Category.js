...
Category.prototype.addVerbRuleSet = function (options) {
	...
	// Check `options.userVerbSemantic` was contains a semantic defined with `isPeople`, which enables the verb's use as an antecedent for anaphora.
	if (!semantic.hasAntecedent(options.userVerbSemantic)) {
...
/**
 * Adds nonterminal rules to this `Category` that represent instances of this `Category` on which specified users performed this action.
 *
 * Adds the following rules for `options.catVerbSemantic`:
 * 1. `[cat-passive]`    -> `[verb]` `[by-obj-users+]`           => (repos) liked by `[obj-users+]`
 * 2. `[cat-obj-filter]` -> `[nom-users+]` `[verb]`              => (repos) `[nom-users+]` like(s)
...
Category.prototype.addAgentNoun = function (options) {
	...
	// Check `options.agentNounSemantic` was contains a semantic defined with `isPeople`, which enables the noun's use as an antecedent for anaphora.
	if (!semantic.hasAntecedent(options.agentNounSemantic)) {
