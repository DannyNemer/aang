...
Category.prototype.addVerbRuleSet = function (options) {
	...
	// (people who) like(d) `[repos+]`
	// (people who) have liked `[repos+]`
	// (people who) do not like `[repos+]`
	// (people who) have not liked `[repos+]`
	user.addSubjectVerbRuleSet({
		verbTerm: options.verbTerm,
		// If `options.onlyPastTense` is `true`, following rules from above will be in past tense:
		// (people who) forked `[repos+]`
		// (people who) did not fork `[repos+]`
 ...
 *
 * Adds the following rules for `options.catVerbSemantic`:
 * 1. `[cat-subj-filter]` -> `[verb]` `[cats+]`             => (people who) like(d) `[repos+]`
 * 2. `[cat-subj-filter]` -> `[have]` `[verb]` `[cats+]`    => (people who) have liked `[repos+]`
 * 3. `[cat-subj-filter]` -> `[do-not] `[verb]` `[cats+]`   => (people who) do not like `[repos+]`
 * 4. `[cat-subj-filter]` -> `[have-not] `[verb]` `[cats+]` => (people who) have not liked `[repos+]`
 *
 * If `options.onlyPastTense` is `true`, then all nonterminal rules in this set will be in past tense. Specifically, the following rules from above will be different:
 * 1. `[cat-subj-filter]` -> `[verb]` `[cats+]`              => (people who) forked `[repos+]`
 * 2. `[cat-subj-filter]` -> `[do-not] `[verb]` `[cats+]`    => (people who) did not fork `[repos+]`
 *
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbTerm The symbol that produces the terminal rule set for the verb that represents this action, created by `NSymbol.prototype.addVerb()`.
...
/**
 * Adds nonterminal rules to this `Category` that represent instances of this `Category` with the specified quantity of `itermTerm`.
 *
...
Category.prototype.addCountRuleSet = function (options) {
	...
	var notCatCountSemantic = g.reduceSemantic(auxVerbs.notSemantic, options.catCountSemantic)
	...
	if (options.verbTerm) {
		...
		this.subjFilter.addRule({
			// The grammatical person-number in the parent rule that produces `[cat-subj-filter]` is not used.
			rhs: [ options.verbTerm, itemCount ],
			// Dictates inflection of `options.verbTerm`:
			//   "(companies that) `[raise]` `<int>` in funding" -> "(people who) forked `<int>` in funding"
			grammaticalForm: 'past',
			semantic: options.catCountSemantic,
		})
		...
		var verbPastTenseItemCount = g.newBinaryRule({
			rhs: [ options.verbTerm, itemCount ],
			// Dictates inflection of `options.verbTerm`:
			//   "(companies that have) `[like]`" -> "(companies that have) raised"
			//   "(companies that have not) `[like]`" -> "(companies that have not) raised"
			grammaticalForm: 'past',
		})
		...
		this.subjFilter.addRule({
			// The grammatical person-number in the parent rule that produces `[cat-subj-filter]`, defined as `pl`, dictates the inflection of `[have]`:
			//   "(companies that) `[have]` raised" -> "(companies that) have raised"
			rhs: [ auxVerbs.have, verbPastTenseItemCount ],
			// No insertion for `[have]` to prevent creating multiple semantically identical trees, where this suggestion would be discarded for its higher cost. For example, were this insertion enabled, it would enable the following additional, wasteful suggestion:
			//   Stop: "(companies that) raised" ->  "(companies that) have raised"
			noInsertionIndexes: [ 0 ],
			semantic: options.catCountSemantic,
		})
		...
		this.subjFilter.addRule({
			// The grammatical person-number in the parent rule that produces `[cat-subj-filter]`, defined as `pl`, dictates the inflection of (only) `options.verbTerm`:
			//   "(companies that) did not `[raise]`" -> "(companies that) did not raise"
			rhs: [ [ auxVerbs.doPastNegation, options.verbTerm ], itemCount ],
			semantic: notCatCountSemantic,
		})

		// (companies that) have not raised `<int>` in funding
		this.subjFilter.addRule({
			// The grammatical person-number in the parent rule that produces `[cat-subj-filter]`, defined as `pl`, dictates the inflection of `[have]`:
			//   "(companies that) `[have]` not raised" -> "(companies that) have not raised"
			// No insertion for `[have]` to prevent creating multiple semantically identical trees. For example, were this insertion enabled, it would (wastefully) enable the following semantically duplicate suggestions:
			//   Stop:  "(cos that) not raise" -> "(cos that) did not raise", "(cos that) have not raised"
			rhs: [ auxVerbs.haveNoInsertNegation, verbPastTenseItemCount ],
			semantic: notCatCountSemantic,
		})
	} else {
