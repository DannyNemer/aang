...
function Category(options) {
	...
	this.lhs.addRule({ rhs: [ this.lhs, this.preModifier ], noInsert: true, transpositionCost: 0.1 })
	...
	// `noInsert` is temporarily enabled to prevent recursive sequences of unary rules, which the following insertion of this rule enables:
	...
	// Though, as all categories use `[obj-users]`, this recursive rule damages parse performance of queries for all categories.
	// Such rules are forbidden because they enable recursive parse nodes, which the parse-forest search heuristic calculator does not yet support. See "Recursive Node Restriction" in `calcHeuristicCosts` for a detailed explanation.
	var lhsHead = g.newBinaryRule({ rhs: [ this.lhs, this.head ], noInsert: true, transpositionCost: 1 })
	...
	rhs.addRule({ rhs: [ rhs, stopWords.sentenceAdverbial ], noInsert: true, transpositionCost: 0 })


	var baseNoRelative = g.newSymbol(this.nameSg, 'base', 'no', 'relative')
	// people I follow; female people followed by me
	baseNoRelative.addRule({ rhs: [ lhsHead, rhs ], noInsert: true, transpositionCost: 1 })
	...
	// (people who) <stop> follow me, I follow
	filter.addRule({ rhs: [ stopWords.sentenceAdverbial, filter ], noInsert: true, transpositionCost: 0 })
