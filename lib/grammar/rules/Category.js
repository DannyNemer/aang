...
Category.prototype.addTenseVerbRuleSet = function (options) {
	...
	// Check `options.verbPresentTerm` produces terminal rules for present tense verb forms and lacks past tense verb forms.
	if (!g.isVerbTerm(options.verbPresentTerm, 'present')) {
		util.logError('`verbPresentTerm` is not a present tense verb set or present tense verb term sequence:', options.verbPresentTerm)
		throw new Error('Ill-formed present verb')
	}

	// Check `options.verbPastTerm` produces terminal rules for past tense verb forms and lacks present tense verb forms.
	if (!g.isVerbTerm(options.verbPastTerm, 'past')) {
		util.logError('`verbPastTerm` is not a past tense verb set or past tense verb term sequence:', options.verbPastTerm)
