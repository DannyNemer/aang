...
const relPronouns = require('./relativePronouns')
...
function Category(options) {
	...
	const relPronoun = options.isPerson ? relPronouns.who : relPronouns.that
	...
	const lhsHead = g.newBinaryRule({ rhs: [ this.lhs, this.head ], transpositionCost: 1 })

	// (people) followed by me
	this.passive = g.newSymbol(this.nameSg, 'passive')
	// (repos) liked by me and/or created by {user}
	const passivePlus = conjunctions.create(this.passive)


	const reducedNoTense = g.newSymbol(this.nameSg, 'reduced', 'no', 'tense')
	...
	const reduced = g.newSymbol(this.nameSg, 'reduced')
	// (people) followed by me
	...
	const objFilterPlus = conjunctions.create(this.objFilter, true)


	const rhsExt = g.newSymbol(this.nameSg, 'rhs', 'ext')
	// (people) I follow
	...
	const rhs = g.newSymbol(this.nameSg, 'rhs')
	...
	const baseNoRelative = g.newSymbol(this.nameSg, 'base', 'no', 'relative')
	...
	const filter = g.newSymbol(this.nameSg, 'filter')
	// (people who) follow me
	...
	const filterPlus = g.newSymbol(filter.name + '+')
	// (people who) `[filter]`
	filterPlus.addRule({
		rhs: [ filter ],
	})

	const filterAnd = g.newBinaryRule({
		rhs: [ filter, conjunctions.and ],
		noInsertionIndexes: [ 0 ],
	})
	const filterAndRelPronoun = g.newBinaryRule({
		rhs: [ filter, options.isPerson ? conjunctions.andWho : conjunctions.andThat ],
		noInsertionIndexes: [ 0 ],
	})

	const filterPlusNoUnion = g.newSymbol(filterPlus.name, 'no', 'union')
	...
	const filterPlusNoUnionOr = g.newBinaryRule({
		rhs: [ filterPlusNoUnion, conjunctions.union ],
		noInsertionIndexes: [ 0 ],
		semantic: conjunctions.intersectSemantic,
	})
	const filterPlusNoUnionOrRelPronoun = g.newBinaryRule({
		rhs: [ filterPlusNoUnion, options.isPerson ? conjunctions.unionWho : conjunctions.unionThat ],
		noInsertionIndexes: [ 0 ],
		semantic: conjunctions.intersectSemantic,
	})
	const filterPlusIntersect = g.newSymbol(filterPlus.name, 'intersect').addRule({
		rhs: [ filterPlus ],
		semantic: conjunctions.intersectSemantic,
	})
	...
	const relativeClause = g.newSymbol(this.nameSg, 'relative', 'clause')
	// (repos) that I like; (people) who are followed by me
