...
function createFilterConjunctionRuleSet(filter, isPerson) {
	...
	filterPlus.addRule({
		rhs: [
			...
			{ symbol: filterPlusIntersect, noInsert: true },
		],
 ...
 *
 * If `options.catVerbSemantic` has the property `forbidsMultiple` defined as `true`, then forbids conjunctions of subjects (via `intersect()`) and restricts `options.catVerbSemantic` to disjunctions (i.e., only `union()`) with the following changes to the preceding rules:
 * 1.   `[obj-users+-no-intersect]` replaces `[obj-users+].
 * 2-5. `[nom-users+-no-intersect]` replaces `[nom-users+].
 ...
 * 5. Stop: "(repos) `[nom-users+-no-intersect]` have/has not created"
...
Category.prototype.addVerbRuleSet = function (options) {
	...
	this.addObjectVerbRuleSet({
		...
		//   Stop: "(repos) `[nom-users+-no-intersect]` have/has not created"
...
Category.prototype.addObjectVerbRuleSet = function (options) {
	...
	/**
	 * When `options.catVerbSemantic` has `forbidsMultiple` defined as `true`, the parser only permits one instance of `options.catVerbSemantic` within the semantic arguments of a parent `intersect()`, irrespective of the semantic children of each `options.catVerbSemantic`. This restriction is necessary for database object properties for which each object can only have one value for the property.
	 ...
	 *
	 * `pfsearch` -> `semantic.reduce()` detects such `intersect()` instances with illegal RHS semantic arrays that contain multiple instances of a semantic defined with `forbidsMultiple`, and discards the associated parse trees.
	 *
	 * Below, grammar rules with semantics that have `forbidsMultiple` are designed to limit the possible constructions of such semantically illegal trees, which prevents `pfsearch` from having to construct (and then discard) the trees in the first place.
	 * • By using `[obj-users+-no-intersect]` and `[nom-users+-no-intersect]` below in place of `[obj-users+]` and `[nom-users+]`, respectively, it prevents subject conjunctions that use `intersect()` with `options.verbTerm`, which prevents the illegal query example above, and only enable logical disjunctions by using `union()`. In addition, the disjunction sets enable the substitution of "and" with "or" when matched in input.
	...
	this.passive.addRule({
		rhs: [ {
			...
			noSubjectConjunctions ? user.byObjUsersPlusNoIntersect : user.byObjUsersPlus,
		],
	...
	if (options.onlyPastTense) {
		...
		this.objFilter.addRule({
			rhs: [
				...
				noSubjectConjunctions ? user.nomUsersPlusNoIntersect : user.nomUsersPlus,
				{
					symbol: options.verbTerm,
					// Dictates inflection of `options.verbTerm`:
		...
		this.objFilter.addRule({
			...
			//   "(repos) I `[verb-like]`" -> "(repos) I like"
			rhs: [ noSubjectConjunctions ? user.nomUsersPlusNoIntersect : user.nomUsersPlus, options.verbTerm ],
			semantic: options.catVerbSemantic,
		})
	}
	...
	if (!options.noPresentPerfect) {
		...
		this.objFilter.addRule({
			rhs: [
				...
				noSubjectConjunctions ? user.nomUsersPlusNoIntersectHaveNoInsert : user.nomUsersPlusHaveNoInsert,
				{
					symbol: options.verbTerm,
					// Dictates inflection of `options.verbTerm`:
	...
	if (!options.noPresentPerfect && !options.noPresentPerfectNegative) {
		...
		this.objFilter.addRule({
			rhs: [
				...
				noSubjectConjunctions ? user.nomUsersPlusNoIntersectHaveNoInsertNegation : user.nomUsersPlusHaveNoInsertNegation,
				{
					symbol: options.verbTerm,
					// Dictates inflection of `options.verbTerm`:
