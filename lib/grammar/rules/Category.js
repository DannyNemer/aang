...
Category.prototype._addPossesssiveRules = function (possSemantic) {
	...
	this.possessable = g.newSymbol(this.nameSg, 'possessable').addRule({
		rhs: [ this.lhs, this.headMayPoss ],
		// For now, prevent this rule's insertion, which would enable  "my" -> "my repos", because it halves performance. (Measurement exclude `no-legal-trees`, which remain disproportionately slow until fixed.)
		noInsert: true,
		transpositionCost: 1,
	})
