...
function Category(options) {
	 ...
	 * man I saw".
	 * • Unreduced forms of this relative clause would be, "This
...
Category.prototype.addObjectVerbRuleSet = function (options) {
	 ...
	 * property.
	 * • For example, a repository database object can only have one value for
	 ...
	 * first place.
	 * • By using `[obj-users+-disjunction]` and `[nom-users+-disjunction]`
	 ...
	 *   • In addition, the disjunction sets enable the substitution of
	 *     "and" -> "or" when matched in input.
	 * • Semantically illegal parse trees with such semantics can still be
	 ...
	 * • It is difficult to compare the performance of these trade-offs.
	 *   `Parser` shift-reduce operations consume 67% of total parse time,
	 *   while `pfsearch` consumes 20% (10% garbage collection, 3% other).
	 * • Might need to restructure these rules to pair `[do-negation]` with the
	 ...
	 * • `options.noPresentPerfect` - Verbs that represent ongoing present
	 ...
	 * • `options.noPresentPerfectNegative` - Verbs that represent past actions
 ...
 * the opposite tense:
 * • Each is created by `g.newTenseVerb(verbOptions)` or
