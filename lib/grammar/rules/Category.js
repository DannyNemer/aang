	...
	if (options.onlyPastTense) {
		...
		this.objFilter.addRule({
			rhs: [
				 ...
				 *   `[nom-users+] [ [do] [verb] ]`
				 * In contrast, the following structure yields less rules by sharing the leftmost rule:
				 *   `[ [nom-users+] [do] ] [verb]`
				 *
				 * Despite the former structure requiring more grammar rules, which yields more parser reductions, it enables more term sequence flattening, which reduces the number of paths `pfsearch` creates and text objects it conjugates.
