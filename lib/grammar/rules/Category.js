...
Category.prototype.addObjectVerbRuleSet = function (options) {
	...
	if (options.onlyPastTense) {
		...
		this.objFilter.addRule({
			rhs: [
				// The grammatical person-number in `[nom-users+]` is not used.
				noSubjectConjunctions ? user.nomUsers : user.nomUsersPlus,
				/**
				 * This structure requires creating a unique binary rule for every verb:
				 *   `[nom-users+] [ [do] [verb] ]`
				 * In constrast, the following structure yields less rules by sharing by sharing the leftmost rule:
				 *   `[ [nom-users+] [do] ] [verb]`
				 *
				 * Despite more rules, which require more parser reduction, the former structure enables more term sequence flattening, which reduces the number of paths `pfsearch` creates and the number of conjugation operations.
				 */
				g.newBinaryRule({
					rhs: [ auxVerbs.doNegation, options.verbTerm ],
					// Dictates inflection of `[do]` and `options.verbTerm`:
					//   "(repos) I `[do]` not `[verb-fork]`" -> "(repos) I did not fork".
					grammaticalForm: { 0: 'past', 1: 'infinitive' },
		...
		this.objFilter.addRule({
			...
			rhs: [ noSubjectConjunctions ? user.nomUsersDoNegation : user.nomUsersPlusDoNegation, options.verbTerm ],
			// Dictates inflection of `options.verbTerm`:
		...
		this.subjFilter.addRule({
			rhs: [
				g.newBinaryRule({
					rhs: [ auxVerbs.doNegation, options.verbTerm ],
					// Dictates inflection of `[do]` and `options.verbTerm`:
					//   "(people who) `[do]` not `[verb-fork]` (...)" -> "(people who) did not fork (...)"
					grammaticalForm: { 0: 'past', 1: 'infinitive' },
				// Pass `true` to suppress warning if `NSymbol` uses an existing, identical binary rule.
				}, true),
				options.objectSym,
