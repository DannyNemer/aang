 ...
 * @param {boolean} [options.isPerson] Specify the category is person. This is used for relative pronouns (i.e., "that" vs. "who").
 * @param {NSymbol} options.headNoun The term sequence of type 'noun' for the category head.
 ...
 * @param {(Object|string)[]} [options.entities] The entities with which to create an associated entity category. Defined as either strings or objects with properties `display` (string) and `names` (`string[]`) for entities with multiple names (e.g., "JavaScript", "JS").
 */
var categorySchema = {
...
function Category(options) {
	...
	// Do not use `isOptional` with `stopWords.sentenceAdverbial`, because it would yield the unary, recursive rule: `rhs` -> `rhs`.
	...
	// (people who) <stop> follow me, I follow
	// Do not use `isOptional` with `stopWords.sentenceAdverbial`, because it would yield the unary, recursive rule: `filter` -> `filter`.
 ...
 * 2. `[cat-head]` -> `[cat-head-possessable]` `[of-poss-users+]` => (repos) of `[poss-users]`
 *
...
Category.prototype._addPossessiveRules = function (possSemantic) {
	...
	this.head.addRule({
		rhs: [
			...
			/**
			 * Use a separate rule for `possSemantic` to ensure only semantics `ofPossUsersPlus` produces, and not semantics `this.headPossessable` produces (e.g., `repositories-type(fork)` for "forks"), are added to `possSemantic`.
			 *
			 * Note: Not all categories have `this.headPossessable` produce any semantics, rendering this additional unary rule unnecessary for those categories. If `RHSSymbolWrapper` (in `NSymbol`) is extended to enable specifying semantics for a specific symbol in a rule's RHS, this extra rule can be removed.
			 */
 ...
 * @param {NSymbol} options.headNoun The term sequence of type 'noun' for the subcategory head, created by `g.newCountNoun()`, `g.newMassNoun()`, or `g.newTermSequence()` with `type` 'noun'.
 * @returns {Category} Returns this `Category` instance.
 */
...
/**
 * Checks if `subcategoryOptions`, which was passed to `Category.prototype.addSubcategory()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} subcategoryOptions The `Category.prototype.addSubcategory()` options object to inspect.
 * @returns {boolean} Returns `true` if `subcategoryOptions` is ill-formed, else `false`.
 */
 ...
 * @param {NSymbol} [options.agentNoun.agentNounTerm] The term sequence of type 'noun' for the agent noun.
...
/**
 * Checks if `verbRuleSetOptions`, which was passed to `Category.prototype.addObjectVerbRuleSet()` or `Category.prototype.addSubjectVerbRuleSet()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} verbRuleSetOptions The verb rule set options object to inspect.
 * @returns {boolean} Returns `true` if `verbRuleSetOptions` is ill-formed, else `false`.
 */
...
/**
 * Adds nonterminal rules to this `Category` that represent instances of this `Category` (i.e., the indirect object) associated with users (i.e., the direct object) who received this action.
 *
 * Adds the following rules for `options.catVerbSemantic`, which represent instances of this `Category` associated with this action received by the specified users:
 ...
 *
 * Adds the following rule to `options.agentSetSym`, which represents users who received this action associated with the specified instances of this `Category`:
 ...
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbTerm The term sequence of type 'verb' that represents this action.
 * @param {NSymbol} options.prepTerm The term sequence of type 'invariable' for the preposition that follows `options.verbTerm` in the rules documented in the method description.
 * @param {Object[]} options.catVerbSemantic The semantic that returns instances of this `Category` on which specified users perform this action.
 * @param {NSymbol} options.agentSetSym The symbol for the set of categories associated with users who received this action.
 * @returns {Category} Returns this `Category` instance.
 */
...
/**
 * Checks if `indirectObjectVerbRuleSetOptions`, which was passed to `Category.prototype.addIndirectObjectVerbRuleSet()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} indirectObjectVerbRuleSetOptions The `Category.prototype.addIndirectObjectVerbRuleSet()` options object to inspect.
 * @returns {boolean} Returns `true` if `indirectObjectVerbRuleSetOptions` is ill-formed, else `false`.
 */
 ...
 * • Each is created by `g.newTenseVerb(verbOptions)` or `g.newTermSequence(termSeqOptions)` with matching `termSeqOptions.type`.
 *
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbPresentTerm The term sequence of type 'verb-present' that produces the present tense verb forms, created by `g.newTenseVerb()` or `g.newTermSequence()` with `type` 'verb-present'.
 * @param {NSymbol} options.verbPastTerm The term sequence of type 'verb-past' that produces the past tense verb forms, created by `g.newTenseVerb()` or `g.newTermSequence()` with `type` 'verb-past'.
 * @param {Object[]} options.catVerbSemantic The semantic that returns instances of this `Category` on which specified users perform this action.
 * @returns {Category} Returns this `Category` instance.
 */
...
/**
 * Checks if `tenseVerbRuleSetOptions`, which was passed to `Category.prototype.addTenseVerbRuleSet()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} tenseVerbRuleSetOptions The `Category.prototype.addTenseVerbRuleSet()` options object to inspect.
 * @returns {boolean} Returns `true` if `tenseVerbRuleSetOptions` is ill-formed, else `false`.
 */
function isIllFormedTenseVerbRuleSetOptions(tenseVerbRuleSetOptions) {
	// Check `tenseVerbRuleSetOptions.verbPresentTerm` is a present tense verb terminal rule set created by `g.newTenseVerb()` or a present tense verb sequence created by `g.newTermSequence()`.
	if (isIllFormedTermSequence(tenseVerbRuleSetOptions, 'verbPresentTerm', g.termTypes.VERB_PRESENT)) {
		return true
	}

	// Check `tenseVerbRuleSetOptions.verbPastTerm` is a past tense verb terminal rule set created by `g.newTenseVerb()` or a past tense verb sequence created by `g.newTermSequence()`.
	if (isIllFormedTermSequence(tenseVerbRuleSetOptions, 'verbPastTerm', g.termTypes.VERB_PAST)) {
		return true
	}

	// Check `tenseVerbRuleSetOptions.verbPresentTerm` and `tenseVerbRuleSetOptions.verbPastTerm` have distinct terminal rules. This is necessary to prevent multiple similar suggestions, scarcely distinguished by tense (though different semantics), for the same input. If not, `grammarUtil.haveIdenticalRules()` prints an error message.
 ...
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.agentNounTerm The term sequence of type 'noun' for the agent noun.
...
/**
 * Adds nonterminal rules to this `Category` that represent instances of this `Category` with a specified quantity of `options.itemNoun`.
 *
 ...
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.itemNoun  The term sequence of type 'noun' for the item to quantify, created by `g.newCountNoun()`, `g.newMassNoun()`, or `g.newTermSequence()` with `type` 'noun'.
 ...
 * @param {Object[]} options.catCountSemantic The semantic that returns instances of this `Category` with a specified quantity of `options.itemNoun`.
 * @returns {Category} Returns this `Category` instance.
 */
...
Category.prototype.addCountRuleSet = function (options) {
	...
	// If an `NSymbol` for a count set already exists for `options.itemNoun`, gets the existing `NSymbol` instead of throwing an exception for duplicity. `
...
/**
 * Checks if `countRuleSetOptions`, which was passed to `Category.prototype.addCountRuleSet()`, is ill-formed. If so, prints an error.
 *
 * @private
 * @static
 * @param {Object} countRuleSetOptions The `Category.prototype.addCountRuleSet()` options object to inspect.
 * @returns {boolean} Returns `true` if `countRuleSetOptions` is ill-formed, else `false`.
 */
