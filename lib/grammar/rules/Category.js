 ...
 * 6. `[user-subj-filter]` -> `[verb]` `[cats+]`             => (people who) like(d) `[repositories+]`
 ...
 *
 * If `options.onlyPastTense` is `true`, all nonterminal rules in this set will be in past tense. Specifically, the following rules from above will be different:
 ...
 *
 * If `options.noPresentPerfectNegative` is `true`, it specifies omitting the present-perfect-negative rules #5 and #9 from the verb rule set. For use by verbs that represent past actions that can not reoccur for the same objects, which these rules would otherwise suggest. Consider the following examples:
 * • For rule #5, `repositories-created()` returns a list of repositories which the specified users did not create; however, the rule's present-perfect tense suggests the specified users can create those same repositories in the future. This implication is false because a repository's creation can not reoccur.
 * • For rule #9, `repository-creators()` returns a list of users that did not create the specified repositories; however, the rule's present-perfect tense suggests those same users can create the specified repositories in the future. This implication is false because a repository's creation can not reoccur.
 *
 ...
 * @returns {Category} Returns this `Category` instance for method chaining.
 */
...
Category.prototype.addVerbRuleSet = function (options) {
	...
	/**
	 * When `options.catVerbSemantic` has `forbidsMultiple` defined as `true`, it specifies to the parser to only permit one instance of `options.catVerbSemantic` within a parent semantic's RHS semantic array (irrespective of child semantics). This restriction is necessary for database object properties for which each object can only have one value for the property.
	...
	// Check `options.onlyPastTense` is `true` when `options.noPresentPerfectNegative` is `true` because `options.noPresentPerfectNegative` is a restriction for verbs that represent past actions.
	if (options.noPresentPerfectNegative && !options.onlyPastTense) {
		util.logErrorAndPath('`noPresentPerfectNegative` is `true` but `onlyPastTense` is not for the verb rule set', util.stylize(options.verbTerm.name) + '. `noPresentPerfectNegative` is a restriction for verbs that represent past actions.', options)
		throw new Error('Ill-formed verb rule set')
	}
	...
	// Do not assign the present-perfect-negative rule to verbs that represent past actions that can not reoccur for the same objects. For example, avoid the following rule, which has the false implication that the associated action (repository creation) can reoccur for the objects it returns:
	//   Stop: "(repos) I have not created"
	if (!options.noPresentPerfectNegative) {
 ...
 *
 * Adds the following rules for `options.catVerbSemantic`:
 * 1. `[cat-subj-filter]` -> `[verb]` `[cats+]`             => (people who) like(d) `[repositories+]`
 ...
 *
 * If `options.noPresentPerfectNegative` is `true`, it specifies omitting the present-perfect-negative rule #4 from the verb rule set. For use by verbs that represent past actions that can not reoccur for the same objects, which these rules would otherwise suggest. For example:
 * • For rule #4, `repository-creators()` returns a list of users that did not create the specified repositories; however, the rule's present-perfect tense suggests those same users can create the specified repositories in the future. This implication is false because a repository's creation can not reoccur.
 *
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbTerm The symbol that produces the terminal rule set for the verb that represents this action, created by `g.newVerbSet()`.
...
Category.prototype.addSubjectVerbRuleSet = function (options) {
	if (util.illFormedOpts(subjectVerbRuleSetSchema, options)) {
	...
	if (options.onlyPastTense) {
		...
		this.subjFilter.addRule({
			...
			rhs: [ options.verbTerm, options.objectSym ],
			// Accept past tense form of `options.verbTerm` if input, otherwise conjugate according to `[cat-subj-filter]` grammatical person-number:
			//   "(people who) liked `[repositories+]`"
			// Also, default to `[cat-subj-filter]` person-number for insertions created from this rule.
			acceptedTense: 'past',
			semantic: options.catVerbSemantic,
		})
	}
	...
	// Do not assign the present-perfect-negative rule to verbs that represent past actions that can not reoccur for the same objects. For example, avoid the following rule, which has the false implication that the associated action (repository creation) can reoccur for the objects it returns:
	//   Stop: "(people who) have not created `[repositories+]`"
	// Though, for this example, it *is* possible for the same users which the query returns to meet the same specified criteria in the future (e.g., "repos `{user}` likes"), it will not be for the same objects (i.e., "`{user}`" must "like" different repositories in the future). Moreover, this rule's past-perfect implication can be absolutely false for other queries; e.g., "people who have not created `{repo}`". Hence, avoid this rule to prevent the queries that are absolutely false. Also, the same query semantics remain possible via the simple-present-negative rule; e.g., "(people who) did not create `[repositories+]`".
	if (!options.noPresentPerfectNegative) {
		// (people who) have not liked `[repositories+]`
		this.subjFilter.addRule({
			// The grammatical person-number in the parent rule that produces `[cat-subj-filter]`, defined as `pl`, dictates the inflection of `[have]`:
...
/**
 * Adds nonterminal rules to this `Category` that represent relationships characterized by an action whose time is meaningful, represented by a verb, between users and instances of this `Category`.
 *
 * Adds the following rules for `options.catVerbSemantic` :
 ...
 *
 * Demands the verb be two separate terminal rule sets with distinct rules (i.e., not intersection of rule sets) to prevent multiple similar suggestions, scarcely distinguished by tense (though different semantics), for the same input. For example, this prevents the following:
 *   "companies I work at"
 ...
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbPresentTerm The symbol that produces the present-tense terminal rule set for the verb that represents this action, created by `g.newVerbSet()`.
 * @param {NSymbol} options.verbPastTerm The symbol that produces the past-tense terminal rule set for the verb that represents this action, created by `g.newTerm()`.
 * @param {NSymbol} [options.prepTerm] The symbol that produces the terminal rule set for the preposition that follows the verb, if any, likely created by `g.newTerm()`.
 * @param {Object[]} options.catVerbSemantic The semantic that returns instances of this `Category` on which the specified users performed this action.
 * @returns {Category} Returns this `Category` instance for method chaining.
 */
...
Category.prototype.addTenseVerbRuleSet = function (options) {
	if (util.illFormedOpts(tenseVerbRuleSetSchema, options)) {
		throw new Error('Ill-formed tense verb rule set')
	}

	/**
	 * Check `options.verbPresentTerm` is either:
	 * 1. A present verb terminal rule set created by `g.newVerb()` (i.e., lacks past tense verb forms).
	 * 2. A present verb sequence created by `g.newTermSequence()` that contains present verbs created by `g.newVerb()`.
	 * 3. A verb created by `g.newVerbSet()` or `NSymbol.prototype.addVerb()` that lacks inflected text for past tense.
	 */
	var presentVerb = options.verbPresentTerm
	if (!(presentVerb.isTermSet && presentVerb.termSetType === 'verb' && presentVerb.verbTense === 'present')
		&& !(presentVerb.isTermSequence && presentVerb.isVerb && presentVerb.verbTense === 'present')
		&& !g.isPresentVerb(options, 'verbPresentTerm')) {
		throw new Error('Ill-formed present verb')
	}

	/**
	 * Check `options.verbPastTerm` is either:
	 * 1. A past verb terminal rule set created by `g.newVerb()` (i.e., lacks present tense verb forms).
	 * 2. A past verb sequence created by `g.newTermSequence()` that contains past verbs created by `g.newVerb()`.
	 * 3. A terminal rule set that lacks inflections.
	 */
	var pastVerb = options.verbPastTerm
	if (!(pastVerb.isTermSet && pastVerb.termSetType === 'verb' && pastVerb.verbTense === 'past')
		&& !(pastVerb.isTermSequence && pastVerb.isVerb && pastVerb.verbTense === 'past')
		&& !g.isInvariableTerm(options, 'verbPastTerm')) {
		throw new Error('Ill-formed past verb')
	}

	// Check `options.verbPresentTerm` and `options.verbPastTerm` have distinct terminal rules. This is necessary to prevent multiple similar suggestions, scarcely distinguished by tense (though different semantics), for the same input.
	if (grammarUtil.haveIdenticalRules(options.verbPresentTerm, options.verbPastTerm)) {
		throw new Error('Ill-formed tense verb rule set')
	}
	...
	this.passive.addRule({
		rhs: [
			options.prepTerm ? [ options.verbPastTerm, options.prepTerm ] : options.verbPastTerm,
			user.byObjUsersPlus,
		],
	...
	this.objFilter.addRule({
		...
		rhs: [
			user.nomUsersPlus,
			options.prepTerm ? [ options.verbPresentTerm, options.prepTerm ] : options.verbPresentTerm,
		],
