...
/**
 * Adds nonterminal rules to this `Category` that represent relationships characterized by an action whose time is meaningful, represented by a verb with meaningful tense, between users and instances of this `Category`.
 *
 * Adds the following rules for `options.catVerbSemantic`:
 * 1. `[cat-passive]`    -> `[verb]` `[by-obj-users+]` => (companies) worked at by `[obj-users+]`, `ever()`
 * 2. `[cat-obj-filter]` -> `[nom-users+]` `[verb]`    => (companies) `[nom-users+]` work(s) at, `present()`
 *
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbTerm The symbol that produces the terminal rule set for the verb that represents this action, created by `g.newVerb()`.
 * @param {NSymbol} [options.prepTerm] The symbol that produces the terminal rule set for the preposition that follows `options.verbTerm`, if any, likely created by `g.newTerm()`.
 * @param {Object[]} options.catVerbSemantic The semantic that returns instances of this `Category` on which the specified users performed this action.
 * @returns {Category} Returns this `Category` instance for method chaining.
 */
...
Category.prototype.addTenseVerbRuleSet = function (options) {
	...
	// Check `options.verbTerm` is a verb created by `g.newVerb()` and has inflected text for past tense.
	if (!g.isVerb(options, 'verbTerm')) {
		throw new Error('Ill-formed verb')
	}
	...
	this.passive.addRule({
		rhs: [
			options.verbTerm,
			options.prepTerm ? [ options.prepTerm, user.byObjUsersPlus ] : user.byObjUsersPlus,
		],
		// Dictates inflection of `options.verbTerm`:
		//   "(companies) `[work]` at by me" -> "(companies) worked at by me"
		grammaticalForm: 'past',
	...
	this.objFilter.addRule({
		...
		rhs: [ user.nomUsersPlus, options.prepTerm ? [ options.verbTerm, options.prepTerm ] : options.verbTerm ],
