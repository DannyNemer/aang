 ...
 * @param {NSymbol} options.headTerm The `NSymbol` that produces the term sequence for the category head noun, created by `g.newTermSequence()`.
 * @param {(string|Object)[]} [options.entities] The entities for this category with which to create an associated entity category. Defined as either strings or objects with properties `display` (string) and `names` (`string[]`) for entities with multiple names (e.g., "JavaScript", "JS").
 */
var categorySchema = {
...
function Category(options) {
	...
	// The `user` category does not use this symbol (and its subrules) because `user` is not possessable (unlike subcategory "followers").
	...
	// FIXME: The three following rules that include `personNumber` are inneficiently conjugating display text via the `personNumber` property when conjugation via a `gramProps`-style property, which only conjugates a rule's immediate children, would work.
	...
	filter.addRule({ rhs: [ auxVerbs.haveSentenceAdverbialBeen, reducedNoTense ], noInsertionIndexes: [ 0 ], personNumber: 'pl' })
	// (people who) <stop> follow me, I follow
	...
	filter.addRule({ rhs: [ auxVerbs.haveNegationBeen, this.reduced ], semantic: auxVerbs.notSemantic, personNumber: 'pl' })
...
function isIllFormedCategoryOptions(categoryOptions) {
	if (!categoryOptions.headTerm.isTermSequence) {
		util.logError('`headTerm` is not a term sequence (created by `g.newTermSequence()`):', categoryOptions.headTerm)
		util.logPathAndObject(categoryOptions)
		return true
	}
 ...
 *
 * Adds the following rules for `options.catVerbSemantic`, which represent instances of `Category` on which specified users perform this action:
 ...
 * 5. Stop: "(repos) `[nom-users]` have/has not created"
...
Category.prototype.addSubjectVerbRuleSet = function (options) {
	...
	var verbPastTenseObjectSym = g.newBinaryRule({
		...
		grammaticalForm: 'past',
		// Do not print warning if `NSymbol` uses an existing, identical binary rule.
	}, true)
	...
	if (options.onlyPastTense) {
		...
		this.subjFilter.addRule({
			...
			// NOTE: If the nested binary rule `[ [do-not] [verb] ]` already exists, `NSymbol.prototype.addRule()` finds and uses the existing rule.
...
function isIllFormedVerbRuleSetOptions(ruleSetOptions) {
	 ...
	 */
	if (!(ruleSetOptions.verbTerm.isTermSequence && ruleSetOptions.verbTerm.termSequenceType === 'verb')
		&& !g.isVerb(ruleSetOptions, 'verbTerm')) {
 ...
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbPresentTerm The term sequence of type 'verb-present' that produces the present tense verb forms (created by `g.newTermSequence()` with `type` 'verb-present' or `g.newVerb()` with `tense` 'present').
 * @param {NSymbol} options.verbPastTerm The term sequence of type 'verb-past' that produces the past tense verb forms (created by `g.newTermSequence()` with `type` 'verb-past' or `g.newVerb()` with `tense` 'past').
 * @param {Object[]} options.catVerbSemantic The semantic that returns instances of this `Category` on which specified users perform this action.
 * @returns {Category} Returns this `Category` instance.
 */
...
function isIllFormedTenseVerbRuleSetOptions(ruleSetOptions) {
	...
	if (!g.isTermSequenceType(ruleSetOptions.verbPresentTerm, g.termTypes.VERB_PRESENT)) {
		util.logErrorAndPath('`verbPresentTerm` is not a term sequence of type', util.stylize(g.termTypes.VERB_PRESENT) + ':', util.stylize(ruleSetOptions.verbPresentTerm.name), ruleSetOptions)
		return true
	}
	...
	if (!g.isTermSequenceType(ruleSetOptions.verbPastTerm, g.termTypes.VERB_PAST)) {
		util.logErrorAndPath('`verbPastTerm` is not a term sequence of type', util.stylize(g.termTypes.VERB_PAST) + ':', util.stylize(ruleSetOptions.verbPastTerm.name), ruleSetOptions)
		return true
	}
...
Category.prototype.addAgentNoun = function (options) {
	...
	// Check `options.agentNounSemantic` represents a set of people.
	if (!isPeopleSemantic(options.agentNounSemantic)) {
 ...
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbTerm The verb term sequence that represents this action, created by `g.newVerb()` or `g.newTermSequence()`.
...
Category.prototype.addDateRuleSet = function (options) {
	...
	// Check `options.verbTerm` is a tense-less verb terminal rule set created by `g.newVerb()` or a verb sequence created by `g.newTermSequence()`.
	if (!g.isTermSequenceType(options.verbTerm, g.termTypes.VERB)) {
		util.logErrorAndPath('`verbTerm` is not a term sequence of type', util.stylize(g.termTypes.VERB) + ':', util.stylize(options.verbTerm.name), options)
		throw new Error('Ill-formed verb')
	}
...
function isIllFormedTermSequence(options, paramName, termSequenceType) {
	if (!g.isTermSequenceType(options[paramName], termSequenceType)) {
		util.logErrorAndPath(`\`${paramName}\` is not a term sequence of type ${util.stylize(termSequenceType)}:`, util.stylize(options[paramName].name), options)
		return true
	}
