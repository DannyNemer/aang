...
Category.prototype.addTenseVerbRuleSet = function (options) {
	...
	/**
	 * Check `options.verbPresentTerm` only has terminal symbols for present tense verb forms (i.e., lacks past tense verb forms):
	 * 1. A present verb terminal rule set, created by `g.newVerb(verbOptions)` where `verbOptions.tense` is 'present'.
	 * 2. A present tense verb sequence, created by `g.newTermSequence(termSeqOptions)` where `termSeqOptions.verbTense` is 'present'.
	 */
	if (!g.isVerbTerm(options.verbPresentTerm, 'present')) {
		throw new Error('Ill-formed present verb')
	}

	/**
	 * Check `options.verbPastTerm` only has terminal symbols for past tense verb forms (i.e., lacks present tense verb forms):
	 * 1. A past verb terminal rule set created by `g.newVerb(verbOptions)` where `verbOptions.tense` is 'past'.
	 * 2. A past tense verb sequence created by `g.newTermSequence(termSeqOptions)` where `termSeqOptions.verbTense` is 'past'.
	 */
	if (!g.isVerbTerm(options.verbPastTerm, 'past')) {
