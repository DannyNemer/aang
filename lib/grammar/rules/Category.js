...
/**
 * Adds the following nonterminal rules to the `user` `Category` for the agent noun, `agentNounTerm`, associated with this `Category`:
 ...
 * @param {Object[]} options.actionSemantic The semantic that identifies instances of this `Category` associated with the action.
 */
var agentNounSchema = {
...
/**
 * Adds the following nonterminal rules to this `Category` for a verb:
 * 1. `[cat-passive]` -> `[verb]` `[by-obj-users+]` => (repos) liked by me/`{user}`/people-who...
 * 2. `[cat-obj-filter]` -> `[nom-users+]` `[verb]` => (repos) I/`{user}`/people-who... like(s)
 * 3. `[cat-obj-filter]` -> `[nom-users+]` `[have]` `[verb]` => (repos) I/`{user}`/people-who... have/has liked
 * 4. `[cat-obj-filter]` -> `[nom-users+]` `[do-not]` `[verb]` => (repos) I/`{user}`/people-who... do not like
 * 5. `[cat-obj-filter]` -> `[nom-users+]` `[have-not]` `[verb]` => (repos) I/`{user}`/people-who... have not liked
 *
 * @memberOf Category
 * @param {Object} options The options object.
 ...
 * @param {Object[]} options.verbSemantic The semantic that identifies instances of this `Category` associated with the verb.
 */
...
Category.prototype.addVerbRuleSet = function (options) {
	...
	this.objFilter.addRule({
		...
		semantic: options.verbSemantic,
		// Accept past-tense form of `options.verbTerm` if input, otherwise conjugate according to `[nom-users+]` grammatical person-number (and when creating insertion rules).
		acceptedTense: 'past',
	})
	...
	// (repos) I/`{user}`/people-who... [and/or `[nom-users+]`] have not liked
	this.objFilter.addRule({
		// The grammatical person-number in `[nom-users+]` dictates the inflection of `[have]`.
