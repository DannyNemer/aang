...
Category.prototype.addObjectVerbRuleSet = function (options) {
	 ...
	 *
	 * Below, grammar rules with semantics that have `forbidsMultiple` are designed to limit the possible constructions of such semantically illegal trees, which prevents `pfsearch` from having to construct (and then discard) the trees in the first place. By using `[obj-users+-no-intersect]` and `[nom-users+-no-intersect]` below in place of `[obj-users+]` and `[nom-users+]`, respectively, it prevents subject conjunctions that use `intersect()` with `options.verbTerm`, which prevents the illegal query example above, and only enable disjunctions with use `union()`. In addition, the disjunction sets enable the substitution of "and" with "or" when matched in input.
