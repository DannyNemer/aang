...
Category.prototype.addVerbRuleSet = function (options) {
	 ...
	 * 1. A verb created by `g.newVerbSet()` that has inflected text for past tense.
	 * 2. A verb created by `g.newVerb()` (which has inflected text for past tense).
	 * 3. A verb set created by `g.newVerbSet2()` (i.e., `options.verbTerm.isTermSetSequence && options.verbTerm.hasVerb`) that contains verbs created by `g.newVerb()` (each of which has inflected text for past tense).
	 */
	if (!(options.verbTerm.isTermSetSequence && options.verbTerm.hasVerb) && !g.isVerb(options, 'verbTerm')) {
...
Category.prototype.addTenseVerbRuleSet = function (options) {
	...
	// Check `options.verbPresentTerm` is a verb created by `g.newVerbSet()` that lacks inflected text for past tense.
	if (!g.isPresentVerb(options, 'verbPresentTerm')) {
