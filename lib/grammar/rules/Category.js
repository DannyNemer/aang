...
function Category(options) {
	...
	this.rhs.addRule({ isTerminal: true, rhs: g.emptySymbol })
	// (people) followed by me
	...
	this.rhs.addRule({ rhs: [ auxVerbs.negation, reduced ], semantic: auxVerbs.notSemantic })
	...
	baseNoRelative.addRule({ rhs: [ lhsHead, this.rhs ], transpositionCost: 1 })
	...
	this.noRelative.addRule({ rhs: [ this.noRelativePossessive, this.rhs ], transpositionCost: 1 })
	...
	this.relativeClause = g.newSymbol(this.nameSg, 'relative', 'clause')
	// (repos) that I like; (people) who are followed by me
	this.relativeClause.addRule({ rhs: [ relPronoun, filterPlus ] })
	...
	this.plural.addRule({ rhs: [ this.noRelative, this.relativeClause ], semantic: conjunctions.intersectSemantic })
