...
Category.prototype.addVerbRuleSet = function (options) {
	 ...
	 * 3. A verb set created by `g.newVerbSet2()` (i.e., `options.verbTerm.isVerbSet`) that contains verbs created by `g.newVerb()` (each of which has inflected text for past tense).
	 */
	if (!options.verbTerm.isVerbSet && !g.isVerb(options, 'verbTerm')) {
...
Category.prototype.addDateRuleSet = function (options) {
	...
	// Check `options.verbTerm` is a verb created by `g.newVerbSet()` that has inflected text for past tense.
	if (!g.isVerb(options, 'verbTerm')) {
		throw new Error('Ill-formed verb')
	}
