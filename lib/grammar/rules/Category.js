 ...
 * @param {Object[]} options.catVerbSemantic The semantic that returns instances of this `Category` on which specified users performed this action.
 ...
 * @param {Object[]} options.userVerbSemantic The semantic that returns users who performed this action on specified instances of this `Category`.
...
/**
 * Adds nonterminal rules to this `Category` that represent instances of this `Category` (i.e., the object) on which specified users (i.e., the subject) performed this action.
 *
 * Adds the following rules for `options.catVerbSemantic`:
 * 1. `[cat-passive]`    -> `[verb]` `[by-obj-users+]`           => (repos) liked by `[obj-users+]`
 * 2. `[cat-obj-filter]` -> `[nom-users+]` `[verb]`              => (repos) `[nom-users+]` like(s)
 ...
 * The latter property is unlikely the user's intent, and is also unsatisfiable because this data is unavailable.
 *
 * If `options.noPresentPerfectNegative` is `true`, omits only the present perfect negative rule #5 from the verb rule set. For use by verbs that represent past actions that can not reoccur for the same objects, which these rules would otherwise suggest. For example:
 * • For rule #5, "(repos) `[nom-users]` have/has not created", `not(repositories-created())` would return a list of repositories which the specified users did not create; however, the rule's present perfect negative tense suggests the specified users can create those same repositories in the future. This implication is false because a repository's creation can not reoccur.
 *
 * @memberOf Category
 * @param {Object} options The options object.
...
/**
 * Adds nonterminal rules to this `Category` that represent instances of this `Category` (i.e., the subject) that performed this action on `options.objectSym` (i.e., the object).
 *
 * Adds the following rules for `options.catVerbSemantic`:
 ...
 * @param {Object[]} options.catVerbSemantic The semantic that returns instances of this `Category` that performed this action on `options.objectSym`.
 * @returns {Category} Returns this `Category` instance.
 */
...
function isIllFormedVerbRuleSetOptions(ruleSetOptions) {
	 ...
	 */
	if (!(ruleSetOptions.verbTerm.isTermSet && ruleSetOptions.verbTerm.termSetType === 'verb')
		&& !(ruleSetOptions.verbTerm.isTermSequence && ruleSetOptions.verbTerm.isVerb)
 ...
 *
 * `options.agentNounSemantic` must have been defined with the property `isPeople` to enable the agent noun's use as an antecedent for anaphora. For example:
 ...
 * @param {Object[]} options.agentNounSemantic The semantic that returns users who performed this action on specified instances of this `Category`.
 * @returns {Category} Returns this `Category` instance.
 */
var agentNounSchema = {
 ...
 *
 * To represent a set of people (as an agent noun), a semantic function must have been defined with the property `isPeople`.
 *
 * @private
 * @static
