 ...
 *
 * If `options.agentNoun` is provided, adds the following rules for `options.agentNounSemantic`:
 ...
 * @param {Object} [options.agentNoun.agentNounTerm] The symbol that produces the terminal rule set for the agent noun, likely created by `NSymbol.prototype.addWord()`.
 * @param {Object} [options.agentNoun.prepTerm] The symbol that produces the terminal rule set for the preposition that follows `options.agentNounTerm` in rule #10 of the second section of this method description, likely created by `NSymbol.prototype.addWord()`.
 */
...
/**
 * Adds nonterminal rules to the `user` `Category` for an agent noun which represents relationships characterized by an action between users and instances of this `Category`.@async
 *
 * Adds the following rules for `options.agentNounSemantic`:
 ...
 *
 * Adds the following rule for `options.dateVerbSemantic`:
 * 1. `[cat-inner]` -> `[verb]` `[date-general]` => (companies) founded `[date-general]`
 *
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbTerm The symbol that produces the terminal rule set for the verb, created by `NSymbol.prototype.addVerb()`.
 * @param {Object[]} options.dateVerbSemantic The semantic that identifies instances of this `Category` for which this action occurred within a specified date range.
 */
var dateVerbSchema = {
	verbTerm: { type: NSymbol, required: true },
	dateVerbSemantic: { type: Array, arrayType: Object, required: true },
}

Category.prototype.addDateVerb = function (options) {
	if (util.illFormedOpts(dateVerbSchema, options)) {
	...
	// (companies) founded `[date-general]`
	this.inner.addRule({
		rhs: [ options.verbTerm, date.general ],
		// Dictates inflection of `options.verbTerm`:
		//   "(companies) `[found]` in `[year]`" -> "(companies) founded in `[year]`"
		grammaticalForm: 'past',
		semantic: options.dateVerbSemantic,
	})
}
...
function isNotVerb(options, paramName) {
	...
	if (textForms.constructor !== Object || !textForms.oneSg || !textForms.threeSg || !textForms.pl) {
		var errMsg = '\'' + paramName + '\' is not a verb created by `NSymbol.prototype.addVerb()`:'
		util.logError(errMsg, util.stylize(verbTerm.name), '->', util.stylize(textForms))
		util.logPathAndObject(options)
		return true
	}

	// Check `options[paramName]` has inflected text for past tense, even though optional for `NSymbol.prototype.addVerb()` (e.g., `[be-general]`).
	if (!textForms.past) {
		var errMsg = '\'' + paramName + '\' lacks past tense inflection:'
		util.logError(errMsg, util.stylize(verbTerm.name), '->', util.stylize(textForms))
		util.logPathAndObject(options)
		return true
	}
