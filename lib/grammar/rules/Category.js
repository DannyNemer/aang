...
function Category(options) {
	...
	if (options.possSemantic) {
		...
		// If `options.possSemantic` is defined with `forbidsMultipleIntersection`, use `[of-poss-users+-disjunction]` instead of `[of-poss-users+]` to prevent multiple instances of the semantic function within an `intersect()`.
		var ofPossUsersPlus = options.possSemantic[0].semantic.forbidsMultipleIntersection
		...
		this.head.addRule({
			rhs: [
				this.headMayPoss,
				// Use a separate rule for `options.possSemantic` to ensure only semantics `ofPossUsersPlus` produces, and not semantics `this.headMayPoss` produces (e.g., `repositories-type(fork)` for "forks"), are added to `options.possSemantic`.
