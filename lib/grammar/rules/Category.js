...
Category.prototype.addVerbRuleSet = function (options) {
	...
	this.passive.addRule({
		...
		// Conjugate "like by me" -> "liked by me".
	...
	this.objFilter.addRule({
		...
		semantic: options.verbSemantic,
		// Accept past-tense form of `options.verbTerm` if input, otherwise conjugate according to `[nom-users+]` grammatical person-number. (Also, default to `[nom-users+]` person-number for insertion rules).
		acceptedTense: 'past',
	})
	...
	this.objFilter.addRule({
		...
		// No insertion for `[have]` to prevent creating multiple semantically identical trees, where this suggestion would be discarded for its higher cost. For example, were this insertion enabled, it would enable the following suggestion: "repos I like" -> "repos I have liked".
		rhs: [ user.nomUsersPlusHaveNoInsert, options.verbTerm ],
		semantic: options.verbSemantic,
		// Conjugate "have like" -> "have liked".
		grammaticalForm: 'past',
	})
	...
	this.objFilter.addRule({
		...
		// Conjugate "do not likes" -> "do not like".
		personNumber: 'pl',
	})
	...
	this.objFilter.addRule({
		// The grammatical person-number in `[nom-users+]` dictates the inflection of `[have]`.
		// No insertion for `[have]` to prevent creating multiple semantically identical trees. For example, were this insertion enabled, it would enable the following semantically duplicate suggestions:
		//   "repos I not" -> "repos I have not", "repos I do not"
		rhs: [ user.nomUsersPlusHaveNoInsertNegation, options.verbTerm ],
		semantic: notVerbSemantic,
		// Conjugate "have not like" -> "have not liked".
		grammaticalForm: 'past',
	})
	...
	if (!options.agentNounSemantic[0].semantic.anaphoraPersonNumber) {
		util.logError('The semantic agent noun semantic associated with', util.stylize(options.verbTerm.name), 'was defined without `isPeople`:', options.agentNounSemantic)
		util.logPathAndObject(options)
	...
	user.subjFilter.addRule({
		// The grammatical person-number in the parent rule that produces `[user-subj-filter]` dictates the inflection of `options.verbTerm`.
		rhs: [ options.verbTerm, this.plPlus ],
		semantic: options.agentNounSemantic,
		// Accept past-tense form of `options.verbTerm` if input, otherwise conjugate according to `[user-subj-filter]` grammatical person-number. (Also, default to `[user-subj-filter]` person-number for insertion rules).
		acceptedTense: 'past'
	})
}
 ...
 * 1. `[agent-noun]` `[prep]` `[cats+]` => founders of `[companies+]`
 * 2. `[cat]` `[agent-noun]`            => `{company}` founders
 ...
 * @param {Object[]} options.actionSemantic The semantic that identifies users who performed this action on the specified instances of this `Category`.
 */
var agentNounSchema = {
...
Category.prototype.addAgentNoun = function (options) {
	...
	// Check `options.actionSemantic` was defined with `isPeople` to enable the noun's use as an antecedent in anaphora.
	if (!options.actionSemantic[0].semantic.anaphoraPersonNumber) {
		util.logError('The semantic associated with the agent noun', util.stylize(options.agentNounTerm.name), 'was defined without `isPeople`:', options.actionSemantic)
		util.logPathAndObject(options)
		throw new Error('Ill-formed agent noun')
	}
	...
	user.head.addRule({
		...
		noInsertionIndexes: [ 0 ],
		transpositionCost: 1,
		semantic: options.actionSemantic,
	})
