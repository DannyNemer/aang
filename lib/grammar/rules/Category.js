...
Category.prototype.addSubjectVerbRuleSet = function (options) {
	...
	if (options.onlyPastTense) {
		...
		this.subjFilter.addRule({
			rhs: [
				g.newBinaryRule({
					rhs: [ auxVerbs.doPastNegation, options.verbTerm ],
					// Dictates inflection of `options.verbTerm`:
					//   "(people who) did not `[fork]`" -> "(people who) did not fork"
					grammaticalForm: 'infinitive',
					// Pass `true` to suppress warning for duplicate binary rule definition, which uses an existing identical binary rule.
				}, true),
				options.objectSym,
			],
			semantic: notCatVerbSemantic,
