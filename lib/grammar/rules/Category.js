 ...
 * @param {Object[]} [options.possSemantic] The semantic for rules that represent ownership/possession of instances of this category.
...
function Category(options) {
	...
	if (options.possSemantic) {
		 ...
		 *
		 * Use `[of-poss-users+-disjunction]` instead of `[of-poss-users+]` because `options.possSemantic`, which is defined with `forbidsMultipleIntersection`, prevents multiple instances of the semantic function within an `intersect()`.
		 */
		this.head.addRule({
			rhs: [
				this.headMayPoss,
				// Use a separate rule for `options.possSemantic` to ensure only semantics `[of-poss-users+-disjunction]` produces are added to `options.possSemantic` and not the semantics which `this.headMayPoss` produces (e.g., `repositories-type(fork)` for "forks").
				g.newSymbol(this.nameSg, user.ofPossUsersPlusDisjunction.name).addRule({
					rhs: [ user.ofPossUsersPlusDisjunction ],
					semantic: options.possSemantic,
				}),
			],
		})
 ...
 *
 * If `options.catVerbSemantic` has the property `forbidsMultipleIntersection` defined as `true`, then forbids conjunctions of subjects (via `intersect()`) and restricts `options.catVerbSemantic` to disjunctions (i.e., only `union()`) with the following changes to the preceding rules:
