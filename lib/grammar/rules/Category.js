...
function Category(options) {
	...
	this.rhsExt.addRule({ rhs: [ relPronoun, this.objFilter ], noInsertionIndexes: [ 0 ] })
...
/**
 * Adds a nonterminal rule to this `Category` that represents of this `Category` with the specified quantity of `itermTerm`.
 *
 * Adds the following rule for `options.catCountSemantic`:
 * 1. `[cat-inner]` -> `[prep]` `[item]` => (companies) with `<int>` employees
 *
 * @memberOf Category
 * @param {Object} options The options object.
 * @param {NSymbol} options.itemTerm The symbol that produces the terminal rule set for the items being quantified.
 * @param {NSymbol} options.prepTerm The symbol that produces the terminal rule set for the verb, created by `NSymbol.prototype.addVerb()`.
...
Category.prototype.addCountRuleSet = function (options) {
	...
	this.inner.addRule({
		rhs: [ options.prepTerm, count.create(options.itemTerm) ],
