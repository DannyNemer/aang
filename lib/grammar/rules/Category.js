...
Category.prototype.addSubjectVerbRuleSet = function (options) {
	...
	if (options.onlyPastTense) {
		...
		this.subjFilter.addRule({
			...
			//   "(people who) `[like]` (`[repositories+]`)" -> "(people who) like (`[repositories+]`)".
			grammaticalForm: 'infinitive',
	...
	var verbPastTenseObjectSym = g.newBinaryRule({
		...
		grammaticalForm: 'past',
	// Pass `true` to suppress warning if `NSymbol` uses an existing, identical binary rule.
	}, true)
	...
	if (options.onlyPastTense) {
		...
		this.subjFilter.addRule({
			rhs: [
				g.newBinaryRule({
					// Dictates inflection of both `[do]` and `options.verbTerm`:
					//   "(people who) `[do]` not `[like]`" -> "(people who) do not like"
					rhs: [ auxVerbs.doPresentNegation, options.verbTerm ],
					grammaticalForm: 'infinitive',
				// Pass `true` to suppress warning if `NSymbol` uses an existing, identical binary rule.
				}, true),
				options.objectSym,
	...
	if (!options.noPresentPerfect && !options.noPresentPerfectNegative) {
		...
		this.subjFilter.addRule({
			...
			// No insertion for `[have]` to prevent `pfsearch` from wastefully creating multiple semantically identical trees, which it ultimately discard. For example, were this insertion enabled, it would yield the following semantically duplicate suggestions:
