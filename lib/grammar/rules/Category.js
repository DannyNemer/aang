...
function isIllFormedTenseVerbRuleSetOptions(ruleSetOptions) {
	// Check `ruleSetOptions.verbPresentTerm` is a present tense verb terminal rule set created by `g.newTenseVerb()` or a present tense verb sequence created by `g.newTermSequence()`.
	if (isIllFormedTermSequence(ruleSetOptions, 'verbPresentTerm', g.termTypes.VERB_PRESENT)) {
		return true
	}

	// Check `ruleSetOptions.verbPastTerm` is a past tense verb terminal rule set created by `g.newTenseVerb()` or a past tense verb sequence created by `g.newTermSequence()`.
	if (isIllFormedTermSequence(ruleSetOptions, 'verbPastTerm', g.termTypes.VERB_PAST)) {
		return true
	}

	// Check `ruleSetOptions.verbPresentTerm` and `ruleSetOptions.verbPastTerm` have distinct terminal rules. This is necessary to prevent multiple similar suggestions, scarcely distinguished by tense (though different semantics), for the same input. If not, `grammarUtil.haveIdenticalRules()` prints an error message.
