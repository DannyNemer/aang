 ...
 * • For rule #9, "(people who) have not liked `[obj-users+]`", `not(followers())` would return a list of users that do not currently follow the specified users; however, the rule's present-perfect negative tense suggests the list is the following:
 *   1. Users that do not currently follow *and* have never followed the specified users.
 * The latter property is unlikely the user's intent, and is also unsatisfiable because this data is unavailable.
 *
 * If `options.noPresentPerfectNegative` is `true`, it specifies omitting only the present-perfect-negative rules #5 and #9 from the verb rule set. For use by verbs that represent past actions that can not reoccur for the same objects, which these rules would otherwise suggest. For example:
 ...
 * @param {boolean} [options.noPresentPerfect] Specify omitting all present-perfect rules (i.e., rules #3, #5, #7, and #9) from this verb rule set, which otherwise express both the current state and past occurrences of this action/state, the latter of which is unlikely the user's intent.
...
Category.prototype.addVerbRuleSet = function (options) {
	...
	// Check `options.noPresentPerfect` and `options.noPresentPerfectNegative` are not both `true`, which is redundant because the former option encompasses the latter.
	if (options.noPresentPerfect && options.noPresentPerfectNegative) {
		util.logErrorAndPath('Both `noPresentPerfect` and `noPresentPerfectNegative` are `true`, which is redundant because the former option encompasses the latter. `noPresentPerfect` prevents both positive and negative present-perfect rules.', options)
		throw new Error('Ill-formed verb rule set')
	}

	// Check `options.noPresentPerfect` is not `true` while `options.noPresentPerfectNegative` is explicitly `false`, which is contradictory because the former option encompasses the latter.
	if (options.noPresentPerfect && options.noPresentPerfectNegative === false) {
		util.logErrorAndPath('`noPresentPerfect` is `true` while `noPresentPerfectNegative` is explicitly `false`, which is contradictory because the former option encompasses the latter. `noPresentPerfect` prevents both positive and negative present-perfect rules.')
		throw new Error('Ill-formed verb rule set')
	}
	...
	if (options.noPresentPerfectNegative && !options.onlyPastTense) {
		util.logErrorAndPath('`noPresentPerfectNegative` is `true` while `onlyPastTense` is falsey for the verb rule set', util.stylize(options.verbTerm.name) + '. Latter must also be `true` if former is `true` because `noPresentPerfectNegative` is a restriction for verbs that represent past actions.', options)
		throw new Error('Ill-formed verb rule set')
	}
 ...
 *
 * If `options.noPresentPerfect` is `true`, it specifies omitting all present-perfect rules from the verb rule set: #2, #4. For use by verbs that represent ongoing present actions or states, for which present-perfect tense would express both the current state and past occurrences of the state, the latter of which is unlikely the user's intent. Further, there is no data for past occurrences of such actions or states. For example:
 * • For rule #2, "(people) `[nom-users+]` have/has not followed", `not(users-followed())` would return a list of users which the specified users do not currently follow; however, the rule's present-perfect negative tense suggests the list is the following:
 *   1. Users which the specified users do not currently follow *and* have never followed.
 * The latter property is unlikely the user's intent, and is also unsatisfiable because there is no data for accounts a user previously follows and no longer follows.
 * • For rule #4, "(people who) have not liked `[obj-users+]`", `not(followers())` would return a list of users that do not currently follow the specified users; however, the rule's present-perfect negative tense suggests the list is the following:
 *   1. Users that do not currently follow *and* have never followed the specified users.
 * The latter property is unlikely the user's intent, and is also unsatisfiable because this data is unavailable.
 *
 * If `options.noPresentPerfectNegative` is `true`, it specifies omitting only the present-perfect-negative rule #4 from the verb rule set. For use by verbs that represent past actions that can not reoccur for the same objects, which these rules would otherwise suggest. For example:
...
/**
 * Checks if `ruleSetOptions`, which was passed to `Category.prototype.addTenseVerbRuleSet()`, is ill-formed. If not, prints an error.
 *
 * @private
 * @static
