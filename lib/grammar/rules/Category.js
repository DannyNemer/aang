...
Category.prototype.addVerbRuleSet = function (options) {
	...
	if (options.noPresentPerfect && options.noPresentPerfectNegative) {
		util.logErrorAndPath('Both `noPresentPerfect` and `noPresentPerfectNegative` are `true`, which is redundant because `noPresentPerfect` encompasses `noPresentPerfectNegative`. `noPresentPerfect` prevents both positive and negative present perfect rules.', options)
		throw new Error('Ill-formed verb rule set')
	}

	if (options.noPresentPerfectNegative && !options.onlyPastTense) {
		util.logErrorAndPath('`noPresentPerfectNegative` is `true` while `onlyPastTense` is falsey. `onlyPastTense` must also be `true` because `noPresentPerfectNegative` is a restriction for verbs that represent past actions.', options)
		throw new Error('Ill-formed verb rule set')
	}
