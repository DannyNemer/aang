...
Category.prototype.addVerbRuleSet = function (options) {
	...
	if (options.acceptPastTenseIfInput && options.onlyPastTense) {
		util.logErrorAndPath('Both `acceptPastTenseIfInput` and `onlyPastTense` are `true`, which is contradictory because `acceptPastTenseIfInput` enables accepting both present and past tense when input (defaulting to present), while `onlyPastTense` forbids present tense.', options)
		throw new Error('Ill-formed verb rule set')
	}
