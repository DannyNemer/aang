 ...
 *
 * If `options.dateVerbSemantic` is provided, add the following rule for the semantic:
...
var verbRuleSetSchema = {
	...
	agentNoun: Object,
	dateVerbSemantic: { type: Array, arrayType: Object },
}
...
Category.prototype.addVerbRuleSet = function (options) {
	...
	if (options.dateVerbSemantic) {
		// (companies) founded `[date]`
		this.addDateVerb({
			verbTerm: options.verbTerm,
			dateVerbSemantic: options.dateVerbSemantic,
		})
	}
 ...
 * @param {Object} options The options object.
 * @param {NSymbol} options.verbTerm The symbol that produces the terminal rule set for the verb, created by `NSymbol.prototype.addVerb()`.
 * @param {Object[]} options.dateVerbSemantic The semantic that returns instances of this `Category` for which this action occurred within a specified date range.
 */
var dateVerbSchema = {
	verbTerm: { type: NSymbol, required: true },
