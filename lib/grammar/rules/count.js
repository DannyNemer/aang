...
const number = g.newSymbol('number').addRule({
	isTerminal: true,
...
const andPrepOver = g.newBinaryRule({ rhs: [ conjunctions.and, preps.over ] })

// (issues with over <int> comments) and under (<int> comments)
const andPrepUnder = g.newBinaryRule({ rhs: [ conjunctions.and, preps.under ] })
...
exports.createForItems = function (itemsSymbol) {
	const itemsCountSemantic = g.newSemantic({
	...
	const itemsCountOverSemantic = g.newSemantic({
	...
	const itemsCountUnderSemantic = g.newSemantic({
	...
	const itemsCount = g.newSymbol(itemsSymbol.name, 'count')

	// (issues with) <int> comments
	const numberItems = g.newBinaryRule({ rhs: [ number, itemsSymbol ] })
	// (issues with between) <int> /comments/ (and <int> comments)
	const numberItemsOpt = g.newBinaryRule({ rhs: [ number, itemsSymbol.createNonterminalOpt() ] })
