...
// Identical to the rules `[nom-users]` produces sans the `personNumber` properties (and without the rule for just "user" - `all(user)`). For use in conjunctions, for which the root conjunction rules have `personNumber: 'pl'`, because conjunctions are always plural.
var nomUsersPlural = g.newSymbol('nom', user.namePl, 'plural')
// (people) people who follow me (and/or I follow)
nomUsersPlural.addRule({
	rhs: [ user.plural ],
})
...
var nomUsersPluralPlus = conjunction.create(nomUsersPlural, true)
var nomUsersPluralPlusConjunctionRule = nomUsersPluralPlus.rules[1]
var nomUsersPluralPlusDisjunctionRule = nomUsersPluralPlus.rules[2]

// Do not use `conjunction.create(user.nomUsers)` because the conjunction rules must use `[nom-users-plural]`, which lacks `personNumber` properties on the sub-rules to enable proper conjugation. This set's conjunction rules, which produces at least two subjects, have `personNumber: 'pl'` at the conjunction roots.
...
user.nomUsersPlus.addRule({
	// Use `[nom-users]`, which has the appropriate `personNumber` values on the sub-rules, unlike `[nom-users-plural]`, which has no `personNumber` properties (only the base `[nom-users+]` has the `personNumber` properties).
	rhs: [ user.nomUsers ],
})
/**
 ...
 */
Array.prototype.push.apply(user.nomUsersPlus.rules, nomUsersPluralPlus.rules.slice(1).map(function (rule) {
	var ruleClone = util.clone(rule)
	ruleClone.personNumber = 'pl'
	return ruleClone
}))

// Manually create a disjunction rule set manually, instead of using `conjunction.createDisjunctionSet(user.nomUsers)`, to enable proper `personNumber` conjugation for the same reasons as `[nom-users+]`.
...
// (repos) `[nom-users-plural]` or `[nom-users-plural]` [or `[nom-users-plural]` ...] (created)
// (repos) `[nom-users-plural]` and `[nom-users-plural]` (...) -> ... or `[nom-users-plural]` (...)
var nomUsersPluralPlusNoIntersect = conjunction.createDisjunctionSet(nomUsersPlural)
Array.prototype.push.apply(user.nomUsersPlusNoIntersect.rules, nomUsersPluralPlusNoIntersect.rules.slice(1).map(function (rule) {
	var ruleClone = util.clone(rule)
	ruleClone.personNumber = 'pl'
	return ruleClone
}))
...
// Note: Should refactor this operation to avoid manually manipulating `NSymbol.rules`.
var nomUsersPluralPlusConjunctionRuleClone = util.clone(nomUsersPluralPlusConjunctionRule)
nomUsersPluralPlusConjunctionRuleClone.personNumber = 'pl'
user.nomUsersPluralSubj.rules.push(nomUsersPluralPlusConjunctionRuleClone)
 ...
 * Note: Should refactor this operation to avoid manually manipulating `NSymbol.rules`.
 */
nomUsersPluralSubjNoUnion.rules.push(util.clone(nomUsersPluralPlusConjunctionRule))
...
user.nomUsersHaveNoInsertNegation = g.newBinaryRule({
	...
	//   "repos I not" -> "repos I have not", "repos I do not"
	rhs: [ user.nomUsers, auxVerbs.haveNoInsertNegation ],
})
...
user.byObjUsersPlusNoIntersect = g.newBinaryRule({
	rhs: [ preps.agent, conjunction.createDisjunctionSet(user.objUsers) ],
})
