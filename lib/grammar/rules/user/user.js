 ...
 * • No insertion for `[have]` to prevent creating multiple semantically identical trees, where this suggestion would be discarded for its higher cost. For example, were `[have]` insertable, it would enable the following semantically duplicate suggestion:
...
user.nomUsersDoPastNegation = g.newBinaryRule({
	rhs: [ user.nomUsers, auxVerbs.doPastNegation ],
})
// (repos) `[nom-users+]` did not (fork)
user.nomUsersPlusDoPastNegation = g.newBinaryRule({
	rhs: [ user.nomUsersPlus, auxVerbs.doPastNegation ],
})

/**
 * Regarding the following two rules:
 * • No insertion for `[have]` to prevent creating multiple semantically identical trees, where this suggestion would be discarded for its higher cost. For example, were `[have]` insertable, it would enable the following semantically duplicate duplicate suggestions:
