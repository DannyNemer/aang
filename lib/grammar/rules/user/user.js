...
// Do not use `conjunction.create()` because conjunctions must use `[nom-users-plural]`, which lack person-number properties, to enable proper conjugation.
...
var nomUsersPluralSubjNoUnionOr = g.newBinaryRule({
	rhs: [ nomUsersPluralSubjNoUnion, conjunction.or ],
...
/**
 * NOTE: The following rules that combine auxiliary verbs with `user` rules from above are inefficient and should be removed. These auxiliary verbs should be paired with the verbs they precede as part of a single term sequence to optimize term sequence flattening. However, this yields more rules in the grammar because it requires a unique binary for each verb (i.e., `[ [do], [verb] ]`), which yields more `Parser` reductions, which may yield worse performance.
 */
...
user.nomUsersDoNegation = g.newBinaryRule({
	rhs: [ user.nomUsers, auxVerbs.doNegation ],
})
// (repos) `[nom-users+]` do/does not (like)
user.nomUsersPlusDoNegation = g.newBinaryRule({
	rhs: [ user.nomUsersPlus, auxVerbs.doNegation ],
})

/**
 * NOTE: The use of `auxVerbs.doNegation` in the following two rules, instead of creating a separate binary rule with the `grammaticalForm` property closer to the term sequence for `[do]`, yields more parse-time conjugation (i.e., worse performance). With the other structure, `[do]` is conjugated to past tense when flattening, which does not occur here because this rule (with the conjugative property) is not a term sequence. This structure's only benefit is it reduces the number of parser reductions. It is difficult to compare the performance of the two implementations.
 */
// (repos) `[nom-users]` did not (create)
user.nomUsersDoPastNegation = g.newSymbol(user.nomUsersDoNegation.name, 'past').addRule({
	rhs: [ user.nomUsers, auxVerbs.doNegation ],
	// Dictates inflection of `[do]`:
	//   "(repos) I [do] not (create)" -> "(repos) I did not (create)"
	grammaticalForm: { 1: 'past' },
})
// (repos) `[nom-users+]` did not (fork)
user.nomUsersPlusDoPastNegation = g.newSymbol(user.nomUsersPlusDoNegation.name, 'past').addRule({
	rhs: [ user.nomUsersPlus, auxVerbs.doNegation ],
	// Dictates inflection of `[do]`:
	//   "(repos) my followers [do] not (fork)" -> "(repos) my followers did not (fork)"
	grammaticalForm: { 1: 'past' },
})
