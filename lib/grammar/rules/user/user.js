...
// Do not use `conjunction.create()` because the conjunction must use `[nom-users-plural]`, which lacks person-number properties, to enable proper conjugation. The conjunction rules, which produces at least two subjects, have `personNumber: 'pl'` at the conjunction roots.
...
/**
 * Define `personNumber` for conjunctions to enable "people I and Danny follow", using sub-rules that lack person-number properties. As a result, `personNumber` exists only at the root of conjunctions.
 *
 ...
 *
 * Note: Should refactor this operation to avoid manually manipulating `NSymbol.prototype.rules`.
 */
