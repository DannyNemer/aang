...
const user = module.exports = new Category({
	sg: 'user',
	pl: 'users',
	isPerson: true,
	entities: [
...
const allUsersSemantic = g.reduceSemantic(
...
const nomUsersPlural = g.newSymbol('nom', user.namePl, 'plural')
...
nomUsersPlural.addRule({ rhs: [ anaphora.threeSg ], case: 'nom', anaphoraPersonNumber: 'threeSg' })
// (my followers' repos liked by people) they (and/or I follow)
nomUsersPlural.addRule({ rhs: [ anaphora.threePl ], case: 'nom', anaphoraPersonNumber: 'threePl' })
...
const nomUsersPluralPlus = conjunctions.create(nomUsersPlural, true)
const nomUsersPluralAndnomUsersPluralPlusNoUnion = nomUsersPluralPlus.rules[1]
const nomUsersPluralPlusNoUnionOrnomUsersPluralPlus = nomUsersPluralPlus.rules[2]
...
const nomUsersPluralSubjNoUnion = g.newSymbol(user.nomUsersPluralSubj.name, 'no', 'union')
...
// Following logic's order of operations, "and" takes precedence over "or". Hence, segments between "or" are grouped together with `intersect()`, which are then grouped together in `union()`.
const nomUsersPluralSubjNoUnionOr = g.newBinaryRule({
	rhs: [ nomUsersPluralSubjNoUnion, conjunctions.union ],
	noInsertionIndexes: [ 0, 1 ],
	semantic: conjunctions.intersectSemantic,
})
const nomUsersPluralSubjIntersect = g.newSymbol(user.nomUsersPluralSubj.name, 'intersect').addRule({
...
user.objUser.addRule({ rhs: [ anaphora.threeSg ], case: 'obj', anaphoraPersonNumber: 'threeSg' })
...
user.objUsers.addRule({ rhs: [ anaphora.threePl ], case: 'obj', anaphoraPersonNumber: 'threePl' })
