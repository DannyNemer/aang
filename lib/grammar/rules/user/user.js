...
// The grammatical person-number property exists only for the nominative case to conjugate the verbs that follow.
...
Array.prototype.push.apply(nomUsersPlural.rules, nomUsers.rules.map(function (rule) {
...
// Do not use `conjunction.create(nomUsers)` because the conjunction rules must use `[nom-users-plural]`, which lacks `personNumber` properties on the sub-rules to enable proper conjugation. This set's conjunction rules, which produces at least two subjects, have `personNumber: 'pl'` at the conjunction roots.
