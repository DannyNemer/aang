...
/**
 * NOTE: The following rules that combine auxiliary verbs with `user` rules from above are inefficient and should be removed. These auxiliary verbs should be paired with the verbs they precede as part of a single term sequence to optimize term sequence flattening.
 */
...
user.nomUsersDoPresentNegation = g.newBinaryRule({
	rhs: [ user.nomUsers, auxVerbs.doPresentNegation ],
})
// (repos) `[nom-users+]` do/does not (like)
user.nomUsersPlusDoPresentNegation = g.newBinaryRule({
	rhs: [ user.nomUsersPlus, auxVerbs.doPresentNegation ],
})
