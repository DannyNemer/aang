...
var possUserPluralNoIntersect = g.newSymbol(possUser.name, 'plural', 'no', 'intersect')
Array.prototype.push.apply(possUserPluralNoIntersect.rules, user.objUsersPlusNoIntersect.rules.slice(1).map(util.clone))
// (repos of) `[obj-users]` or `[obj-users+-no-intersect]` -> (...) me or `{user}`
exports.ofPossUsersPlusNoIntersect.addRule({
	rhs: [
		preps.possessor,
		possUserPluralNoIntersect
	],
	// Note: Could redesign to define `noInsert` within the parent rule's `rhs` instead of here.
	noInsert: true
})
