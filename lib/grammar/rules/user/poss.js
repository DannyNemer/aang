...
exports.ofPossUsersPlus = g.newSymbol(preps.possessor.name, possUsersPlus.name).addRule({
	...
	// Note: Could redesign to define `noInert` within the parent rule's `rhs` instead of here. Will do so after abstracting the creation of possession rules for each `Category`.
	noInsert: true,
})
...
exports.ofPossUsersPlusNoIntersect = g.newSymbol(preps.possessor.name, possUsersPlusNoIntersect.name).addRule({
	rhs: [
		preps.possessor,
		// Does not prevent any insertions because `possUsersPlusNoIntersect` does not produce any insertables.
		{ symbol: possUsersPlusNoIntersect, noInsert: true },
	],
})
 ...
 *
 * Note: Ambiguous with `[poss-users+]` for  "followers of `{user}` and`{user}`" and "followers of `{user}` and people who...". Can not restructure rules to avoid the ambiguity because "me" is a substitute for "mine". Though this ambiguity is rare and minute and can be ignored.
 *
