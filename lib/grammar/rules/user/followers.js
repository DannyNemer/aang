...
var followersPossDeterminer = g.newSymbol(followers.name, poss.possDeterminer.name).addRule({
...
user.noRelativePossessive.addRule({
	rhs: [
		followersPossDeterminer,
		{ symbol: userFollowersPossessable, noInsert: true },
	],
})
...
var followersPossessiveDeterminerSg = g.newSymbol(followers.name, 'possessive', poss.possDeterminerSg.name).addRule({
	// Limit to `[poss-determiner-sg]`, instead of `[poss-determiner]`, to limit semantic complexity.
	rhs: [ poss.possDeterminerSg ],
	semantic: followersSemantic,
})
poss.possDeterminerPl.addRule({
