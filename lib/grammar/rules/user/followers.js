...
const followersSemantic = g.newSemantic({
	name: 'followers',
	cost: 0.5,
	minParams: 1,
	maxParams: 1,
...
const usersFollowedSemantic = g.newSemantic({
...
const follow = g.newSymbol('follow').addVerb({
	insertionCost: 1,
	oneSg: 'follow',
	threeSg: 'follows',
	pl: 'follow',
...
const followersTerm = g.newSymbol('followers', 'term').addWord({
	insertionCost: 2.5,
...
const userFollowersHead = g.newSymbol(user.nameSg, 'followers', 'head').addRule({
	rhs: [ user.companyOpt, followersTerm ],
	noInsert: true
})

// my/{user:'s}/my-followers' (followers)
const followersPossDeterminer = g.newSymbol('followers', poss.determiner.name).addRule({
	rhs: [ poss.determiner ],
	semantic: followersSemantic,
})
// (my) female followers
const userFollowersPossessable = g.newSymbol(user.nameSg, 'followers', 'possessable').addRule({
...
const followersPossessiveTerm = g.newSymbol('followers', 'possessive', 'term').addWord({
	accepted: [ 'followers\'', 'subscribers\'' ],
})
// (my/{user:'s}) followers' (repos)
const userFollowersPossessiveHead = g.newSymbol(user.nameSg, 'followers', 'possessive', 'head').addRule({
	rhs: [ user.companyOpt, followersPossessiveTerm ],
})
// my/{user:'s} followers' (repos); my/{user:'s} female followers' (repos)
const followersPossessiveDeterminerSg = g.newSymbol('followers', 'possessive', poss.determinerSg.name).addRule({
...
// SHARE:
const share = g.newSymbol('share').addVerb({
	insertionCost: 2,
	oneSg: 'share',
	threeSg: 'shares',
	pl: 'share',
