...
var follow = g.newSymbol('follow').addVerb({
	...
	substitutions: [
		...
		// No "have followed" becuase it can imply people the user no longer follows.
		'have followed',
		'has followed',
	],
}).addVerb({
	oneOrPl: [ 'subscribe to' ],
...
userFollowersHead.addRule({ RHS: [ user.companyOpt, followersTerm ], noInsert: true })
...
// Enables certain insertions, with a low insertion cost, while preventing drawbacks which would result from removing the `noInsertionIndexes` restriction above.
user.objUsers.addRule({
	RHS: [ g.newSymbol('my', 'followers', 'special').addRule({
		isTerminal: true,
		RHS: 'my followers',
		insertionCost: 0.1,
	}) ],
