...
// PHRASE
const datePhrase = g.newSymbol('date', 'phrase')

// (repos created) this month/year
const thisDate = g.newSymbol('this', 'date').addWord({
	insertionCost: 1.5,
	accepted: [ 'this' ],
...
const lastDate = g.newSymbol('last', 'date').addWord({
	insertionCost: 0.5,
	accepted: [ 'last' ],
	substitutions: [ { symbol: 'in|within the last|past', costPenalty: 1 } ],
})

const yearTerm = g.newSymbol('year', 'term').addWord({
	accepted: [ 'year' ],
})

const yearPhrase = g.newSymbol('year', 'phrase')
...
const monthTerm = g.newSymbol('month', 'term').addWord({
	accepted: [ 'month' ],
})
...
const weekTerm = g.newSymbol('week', 'term').addWord({
	insertionCost: 1.5,
	accepted: [ 'week' ],
})
...
const year = g.newSymbol('year').addRule({
	isTerminal: true,
	rhs: g.newIntSymbol({ min: 1950, max: 2050 }),
	isPlaceholder: true,
})

const month = g.newSymbol('month').addRule({
...
// INTERVAL
const datePhraseOrValue = g.newSymbol('date', 'phrase', 'or', 'value')
...
const dateBeforeSemantic = g.newSemantic({
	name: g.hyphenate('date', 'before'),
	minParams: 1,
	maxParams: 3,
	cost: 0.5,
})
const dateAfterSemantic = g.newSemantic({
...
// Semantic reduces the pair of semantics to a single semantic, because the two cannot exist independently, unlike `date-before()` and `date-after()`.
const dateIntervalSemantic = g.newSemantic({
	name: g.hyphenate('date', 'interval'),
	minParams: 2,
	maxParams: 2,
	cost: 0,
})
const dateSemantic = g.newSemantic({
	name: 'date',
	minParams: 1,
	maxParams: 3,
	cost: 0.5,
...
const dateIntervalStopWord = g.newSymbol('date', 'interval', 'stop', 'word').addStopWords('from', 'in')

// (repos created) <from> before (this year)
const prepBefore = g.newBinaryRule({ rhs: [ dateIntervalStopWord, preps.before ] })
const prepBeforeDatePhraseOrValue = g.newBinaryRule({ rhs: [ prepBefore, datePhraseOrValue ], semantic: dateBeforeSemantic })
// (repos created) <from> after (this year)
const prepAfter = g.newBinaryRule({ rhs: [ dateIntervalStopWord, preps.after ] })
const prepAfterDatePhraseOrValue = g.newBinaryRule({ rhs: [ prepAfter, datePhraseOrValue ], semantic: dateAfterSemantic })

const dateInterval = g.newSymbol('date', 'interval')
