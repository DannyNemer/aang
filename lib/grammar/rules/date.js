...
var inWithinBlank = g.newBlankSet('in', 'within')
var theLastPastBlank = g.newBinaryRule({ RHS: [ g.newBlankSet('the'), g.newBlankSet('last', 'past') ] })
var thisDate = g.newSymbol('this', 'date').addWord({
	insertionCost: 1.5,
	accepted: [ 'this' ],
	substitutions: [
		[ inWithinBlank, g.newBlankSet('this') ],
		[ inWithinBlank, theLastPastBlank ],
	],
})
...
var lastDate = g.newSymbol('last', 'date').addWord({
	...
	accepted: [ 'last' ],
	substitutions: [
		{ symbol: [ inWithinBlank, theLastPastBlank ], costPenalty: 1 }
	],
})
...
var dateIntervalSemantic = g.newSemantic({ name: g.hyphenate('date', 'interval'), minParams: 2, maxParams: 2, cost: 0 })
...
dateIntervalStopWord.addStopWord('from', 'in')
...
dateInterval.addRule({ RHS: [ prepAfterDatePhraseOrValue, [ conjunctions.and, prepBeforeDatePhraseOrValue ] ], noInsertionIndexes: [ 1 ] })
// (repos created) <from> before [year] and <from>  after [year]
dateInterval.addRule({ RHS: [ prepBeforeDatePhraseOrValue, [ conjunctions.and, prepAfterDatePhraseOrValue ] ], noInsertionIndexes: [ 1 ] })
