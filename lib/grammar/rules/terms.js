...
// Do not use insertion costs for the following two substituted terms because
// the vast majority of suggestions output by the insertions (which become
// substitutions) are too different from input and unhelpful. (repos created in)
// this (week) -> (repos created) this (week)
...
exports.thisDate = g.newTermSequence({
	...
	insertionCost: 1,
	acceptedTerms: [ 'this' ],
	// Note: Could add a substitution for "last" -> "this" to output more
	// diverse suggestions. Though, better to optimize for additive suggestions
	// rather than substitutive suggestions which can disorient the user by
	// disregarding the input's intent.
	substitutedTerms: [
		// in|within|during this|the Prevent `[prep-origin]` insertion which
		// would otherwise enable an ambiguous insertion for this substitution:
		// "this" -> "in this" -> "this".
...
exports.lastDate = g.newTermSequence({
	...
	acceptedTerms: [ 'last' ],
	substitutedTerms: [
		// in|within|during this|the last|past Note: Could restructure as a
		// stop-word for "in|within|during this|the" and a substitution for
		// "past". Prevent `[prep-origin]` insertion which would otherwise
		// enable an ambiguous insertion for this substitution: "last" -> "in
		// this last" -> "last".
