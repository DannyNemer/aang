...
var pullRequest = g.newCategory({
	...
	headNoun: g.newTermSequence({
		symbolName: g.hyphenate('pull', 'request', 'head', 'noun'),
		...
		acceptedTerms: [
			[
				...
				g.newCountNoun({
					insertionCost: 1.75,
					nounFormsSet: { sg: 'request', pl: 'requests' },
				}),
			],
		],
		substitutedTerms: [
			g.newCountNoun({
				nounFormsSet: { sg: 'PR', pl: 'PRs' },
			}),
		],
	}),
})
...
pullRequest.headPossessable.addRule({ rhs: [ pullRequest.headNoun ] })
pullRequest.headPossessable.addRule({ rhs: [ github.term, pullRequest.headNoun ] })
