...
const pullRequest = new Category({ sg: 'pull-request', pl: 'pull-requests' })
...
const pullRequestsCreatedSemantic = g.newSemantic({
...
// my pull requests
const pullRequestPossDeterminer = g.newSymbol(pullRequest.nameSg, poss.determiner.name).addRule({
	rhs: [ poss.determiner ],
...
const pullRequestCreatorsSemantic = g.newSemantic({
...
user.subjFilter.addRule({ rhs: [ github.haveOptCreatePast, pullRequest.catPlPlus ], semantic: pullRequestCreatorsSemantic })
// (people who) did not create [pull-requests+]
user.subjFilter.addRule({ rhs: [ github.doPastNegationCreatePresent, pullRequest.catPlPlus ], semantic: g.reduceSemantic(auxVerbs.notSemantic, pullRequestCreatorsSemantic) })

// creators of [pull-requests+]
user.head.addRule({
	rhs: [ github.creators, [ preps.participant, pullRequest.catPlPlus ] ],
...
// MENTION:
const pullRequestsMentionedSemantic = g.newSemantic({
	name: g.hyphenate(pullRequest.namePl, 'mentioned'),
	cost: 0.5,
	minParams: 1,
	maxParams: 1,
})
const notPullRequestsMentionedSemantic = g.reduceSemantic(auxVerbs.notSemantic, pullRequestsMentionedSemantic)
...
// ASSIGNED-TO:
const pullRequestsAssignedSemantic = g.newSemantic({
