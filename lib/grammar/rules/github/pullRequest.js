...
// my/`{user:'s}`/my-followers' pull requests; ... closed pull requests
var pullRequestPossDeterminer = g.newSymbol(pullRequest.nameSg, user.possDeterminer.name).addRule({
 ...
 *
 * Use `[of-poss-users+-disjunction]` instead of `[of-poss-users+]` because `pull-requests-created()`, which is defined with `forbidsMultipleIntersection`, prevents multiple instances of the semantic function within an `intersect()`.
 */
 pullRequest.head.addRule({
	rhs: [ pullRequest.headMayPoss, user.ofPossUsersPlusDisjunction ],
	semantic: pullRequestsCreatedSemantic,
})
