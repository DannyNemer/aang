...
// my/`{user:'s}`/my-followers' issues; ... closed issues
var issuePossDeterminer = g.newSymbol(issue.nameSg, user.possDeterminer.name).addRule({
 ...
 *
 * Use `[of-poss-users+-disjunction]` instead of `[of-poss-users+]` because `issues-opened()`, which is defined with `forbidsMultipleIntersection`, prevents multiple instances of the semantic function within an `intersect()`.
 */
issue.head.addRule({
	rhs: [ issue.headMayPoss, user.ofPossUsersPlusDisjunction ],
	semantic: issuesOpenedSemantic,
})
