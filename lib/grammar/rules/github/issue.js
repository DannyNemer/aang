...
issue.addVerbRuleSet({
	verbTerm: g.newTermSequence({
		...
		isVerb: true,
		acceptedTerms: [
			// Use identical `insertionCost` for `[verb-open]` as `[verb-create]` to ensure insertion rules are created using `[verb-open]` instead of its accepted synonym `[verb-create]`.
			verbs.open,
			verbs.create,
		],
		substitutedTerms: [
