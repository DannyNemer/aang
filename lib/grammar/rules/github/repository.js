...
const oneSg = require('../user/oneSg')
...
const repositoriesCreatedSemantic = g.newSemantic({
...
const repositoryPossDeterminer = g.newSymbol(repository.nameSg, poss.determiner.name).addRule({
	rhs: [ poss.determiner ],
...
// Use a separate rule to allow semantics on `[repository-head-may-poss]` that are merged (instead of reduced) with `repositories-created()`.
const repositoryOfPossUsers = g.newSymbol(repository.nameSg, 'of', 'poss', 'users').addRule({
...
const repositoryCreatorsSemantic = g.newSemantic({
...
const ofRepositoriesPlus = g.newBinaryRule({ rhs: [ preps.participant, repository.catPlPlus ] })
user.head.addRule({
...
// LIKE:
const like = g.newSymbol('like').addVerb({
	insertionCost: 0.8,
	oneSg: 'like',
	threeSg: 'likes',
	pl: 'like',
...
const repositoriesLikedSemantic = g.newSemantic({
...
const notRepositoriesLikedSemantic = g.reduceSemantic(auxVerbs.notSemantic, repositoriesLikedSemantic)
// (repos) I do not like
...
const repositoryLikersSemantic = g.newSemantic({
...
const likedRepositoriesPlus = g.newBinaryRule({ rhs: [ like, repository.catPlPlus ], tense: 'past' })
...
const notRepositoryLikersSemantic = g.reduceSemantic(auxVerbs.notSemantic, repositoryLikersSemantic)
...
const likers = g.newSymbol('likers').addWord({
	insertionCost: 2,
	accepted: [ 'likers' ],
})
...
// CONTRIBUTE-TO:
const contributeTo = g.newSymbol('contribute', 'to').addVerb({
	insertionCost: 1.2,
	oneSg: 'contribute to',
...
const repositoriesContributedSemantic = g.newSemantic({
...
const notRepositoriesContributedSemantic = g.reduceSemantic(auxVerbs.notSemantic, repositoriesContributedSemantic)
...
const repositoryContributorsSemantic = g.newSemantic({
...
const contributeToPastRepositoriesPlus = g.newBinaryRule({ rhs: [ contributeTo, repository.catPlPlus ], tense: 'past' })
user.subjFilter.addRule({
...
const notRepositoryContributorsSemantic = g.reduceSemantic(auxVerbs.notSemantic, repositoryContributorsSemantic)
...
const contributors = g.newSymbol('contributors').addWord({
	insertionCost: 2.4,
...
// FORK:
const fork = g.newSymbol('fork').addVerb({
	insertionCost: 2,
	oneSg: 'fork',
	threeSg: 'forks',
	pl: 'fork',
...
const repositoriesForkedSemantic = g.newSemantic({
...
const notRepositoriesForkedSemantic = g.reduceSemantic(auxVerbs.notSemantic, repositoriesForkedSemantic)
...
const repositoryForksSemantic = g.newSemantic({
	name: g.hyphenate(repository.nameSg, 'forks'),
	cost: 0.5,
	minParams: 1,
	maxParams: 1,
})
const repositoryCreatorsForksSemantic = g.reduceSemantic(repositoryCreatorsSemantic, repositoryForksSemantic)
...
const forkPastRepositoriesPlus = g.newBinaryRule({ rhs: [ fork, repository.catPlPlus ], tense: 'past' })
user.subjFilter.addRule({
...
const notRepositoryCreatorsForksSemantic = g.reduceSemantic(auxVerbs.notSemantic, repositoryCreatorsForksSemantic)
...
// FORKS/SOURCES:
const repositoriesTypeSemantic = g.newSemantic({
...
const sources = g.newSymbol('sources').addWord({
	insertionCost: 3.5,
	accepted: [ 'sources' ],
	substitutions: [ 'source' ],
})
// sources (of {user:'s}/mine/[users]); (my/{user:'s}/my-followers') sources; (repos that are) sources
const sourceSemanticArg = g.newSemantic({ isArg: true, name: 'source', cost: 0 })
...
// PUBLIC/PRIVATE:
const repositoriesVisibilitySemantic = g.newSemantic({
...
const publicSemanticArg = g.newSemantic({ isArg: true, name: 'public', cost: 0 })
...
const privateSemanticArg = g.newSemantic({ isArg: true, name: 'private', cost: 0 })
repository.adjective.addRule({
	isTerminal: true,
	rhs: 'private',
...
// LANGUAGE:
const languageEntity = g.newEntityCategory({
	name: 'language',
	entities: [
		'ActionScript',
		'C',
...
const language = g.newSymbol('language').addRule({
	isTerminal: true,
	rhs: languageEntity,
	isPlaceholder: true,
...
const written = g.newSymbol('written').addWord({
	insertionCost: 1.5,
	accepted: [ 'written' ],
})
...
// WITH N STARS:
const stars = g.newSymbol('stars').addWord({
	insertionCost: 3.25,
	accepted: [ 'stars' ],
...
// SIZE:
const size = g.newSymbol('size').addWord({
	insertionCost: 3.5,
	accepted: [ 'KB' ],
})
...
const pushed = g.newSymbol('pushed').addWord({
	accepted: [ 'pushed' ],
})
const repositoriesPushedSemantic = g.newSemantic({
	name: g.hyphenate(repository.namePl, 'pushed'),
	cost: 0.5,
	minParams: 1,
	maxParams: 2,
})
