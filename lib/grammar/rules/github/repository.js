...
var repositoryPossDeterminer = g.newSymbol(repository.nameSg, poss.possDeterminer.name).addRule({
	rhs: [ poss.possDeterminer ],
 ...
 *
 * Use `[of-poss-users+-disjunction]` instead of `[of-poss-users+]` because `repositories-created()`, which is defined with `forbidsMultiple`, prevents multiple instances of the semantic function within an `intersect()`.
 */
var repositoryOfPossUsersPlusDisjunction = g.newSymbol(repository.nameSg, poss.ofPossUsersPlusDisjunction.name).addRule({
	rhs: [ poss.ofPossUsersPlusDisjunction ],
	semantic: repositoriesCreatedSemantic,
})
