...
// `forbidsMultipleIntersection` because repos only have one author, so an intersection of repos from different authors is empty.
...
var repositoryPossDeterminer = g.newSymbol(repository.nameSg, user.possDeterminer.name).addRule({
 ...
 *
 * Use a separate rule to ensure only semantics `[of-poss-users+-disjunction]` produces are added to `repositories-created()`, and not those which `[repository-head-may-poss]` produces, like `repositories-type(fork)`.
 *
 * Use `[of-poss-users+-disjunction]` instead of `[of-poss-users+]` because `repositories-created()`, which is defined with `forbidsMultipleIntersection`, prevents multiple instances of the semantic function within an `intersect()`.
 */
var repositoryOfPossUsersPlusDisjunction = g.newSymbol(repository.nameSg, user.ofPossUsersPlusDisjunction.name).addRule({
	rhs: [ user.ofPossUsersPlusDisjunction ],
	semantic: repositoriesCreatedSemantic,
})
repository.head.addRule({ rhs: [ repository.headMayPoss, repositoryOfPossUsersPlusDisjunction ] })
