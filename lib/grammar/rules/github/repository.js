...
like.addVerb({
	...
	threeSg: [ 'likes' ],
	past: [ 'liked' ],
	alwaysAcceptPast: true,
})

// (repos I do not) like
var likePresent = g.newSymbol('like', 'present')
likePresent.addVerb({
	insertionCost: 0.8,
	oneOrPl: [ 'like' ],
	threeSg: [ 'likes' ],
...
// (repos) I do not like
repository.objFilter.addRule({ RHS: [ user.nomUsersPlusDoPresentNegation, likePresent ], semantic: notRepositoriesLikedSemantic, personNumber: 'pl' })
...
var doPresentNegationLike = g.newBinaryRule({ RHS: [ auxVerbs.doPresentNegation, likePresent ], personNumber: 'pl' })
...
// (repos I do not) contribute to
var contributeToPresent = g.newSymbol('contribute', 'to', 'present')
contributeToPresent.addVerb({
	insertionCost: 1.2,
	oneOrPl: [ 'contribute to' ],
	threeSg: [ 'contributes to' ],
	substitutions: [ 'contributed to' ],
})
...
repository.objFilter.addRule({ RHS: [ user.nomUsersPlusDoPresentNegation, contributeToPresent ], semantic: notRepositoriesContributedSemantic, personNumber: 'pl' })
...
var doPresentNegationContributeTo = g.newBinaryRule({ RHS: [ auxVerbs.doPresentNegation, contributeToPresent ], personNumber: 'pl' })
...
repository.inner.addRule({ RHS: [ writtenIn, language ] })
