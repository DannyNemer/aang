...
// LIKE:
repository.addVerbRuleSet({
	// Omitted synonyms: "starred", "bookmarked". To include these terms, `[verb-like]` must use `onlyPastTense` here (i.e., "repos I star->starred"), however, the verb remains as is for use in tests and documentation.
	verbTerm: g.newTermSequence({
		symbolName: g.hyphenate(repository.nameSg, 'like'),
		isVerb: true,
		acceptedTerms: [ verbs.like ],
		substitutedTerms: [ verbs.star ],
	}),
