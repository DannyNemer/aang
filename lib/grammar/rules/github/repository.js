...
var repository = g.newCategory({
	...
	headTerm: g.newTermSequence({
		symbolName: g.hyphenate('repositories', 'term'),
		type: g.termTypes.INVARIABLE,
		insertionCost: 3,
		acceptedTerms: [ 'repositories', 'repos' ],
		substitutedTerms: [
			{ term: sourcesTerm, costPenalty: 2.5 },
			{ term: forksTerm, costPenalty: 2.75 },
		],
	}),
	entities: [
...
repository.headMayPoss.addRule({ rhs: [ repository.term ] })
repository.headMayPoss.addRule({ rhs: [ github.term, repository.term ] })
 ...
 *
 * Use a separate rule to ensure only semantics `[of-poss-users+-no-intersect]` produces are added to `repositories-created()`, and not those which `[repository-head-may-poss]` produces, like `repositories-type(fork)`.
 *
 * Use `[of-poss-users+-no-intersect]` instead of `[of-poss-users+]` because `repositories-created()`, which is defined with `forbidsMultiple`, prevents multiple instances of the semantic function within an `intersect()`.
 */
var repositoryOfPossUsersPlusNoIntersect = g.newSymbol(repository.nameSg, poss.ofPossUsersPlusNoIntersect.name).addRule({
	rhs: [ poss.ofPossUsersPlusNoIntersect ],
	semantic: repositoriesCreatedSemantic,
})
repository.head.addRule({ rhs: [ repository.headMayPoss, repositoryOfPossUsersPlusNoIntersect ] })
...
repository.addVerbRuleSet({
	...
	//   Stop: (repos) `[nom-users+-no-intersect]` create(s)
	//   Stop: (repos) `[nom-users+-no-intersect]` do/does not create
	...
	//   (repos) created by `[obj-users+-no-intersect]`
	//   (repos) `[nom-users+-no-intersect]` created
	//   (repos) `[nom-users+-no-intersect]` have/has created
	//   (repos) `[nom-users+-no-intersect]` did not open
