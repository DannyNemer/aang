...
repository.addAgentNoun({
	...
	agentNounSemantic: repositoryCreatorsSemantic,
})
...
repository.addVerbRuleSet({
	...
	// Verb rules for `repositories-likers()`:
	...
	// Agent noun rules for `repositories-likers()`:
	...
	agentNoun: {
		...
		agentNounPrepTerm: preps.participant,
	}
})
...
repository.addVerbRuleSet({
	...
	// Verb rules for `repositories-contributors()`:
	...
	// Agent noun rules for `repositories-contributors()`:
	...
	agentNoun: {
		...
		agentNounPrepTerm: preps.receiver,
	}
})
...
repository.objFilter.addRule({ rhs: [ user.nomUsersPlus, fork ], semantic: repositoriesForkedSemantic, grammaticalForm: 'past' })
// (repos) `[nom-users+]` have forked
repository.objFilter.addRule({ rhs: [ user.nomUsersPlusHaveNoInsert, fork ], semantic: repositoriesForkedSemantic, grammaticalForm: 'past' })
