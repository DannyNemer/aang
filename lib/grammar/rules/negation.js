...
/**
 * Creates a `Symbol` with rules for "not" while avoiding ambiguity by excluding the stop-words which `termRuleSetSymbol` also produces as terminal symbols. E.g., exclude the stop-word "have" when the new symbol is for the rule "have not" to avoid "<have> not" -> "not" -> "have not" (last step is insertion).
 *
 ...
 *
 * @param {Symbol} [termRuleSetSymbol] The `Symbol` that produces a terminal rule set for terminal symbols for which to exclude from the stop-words that precede "not" (to avoid ambiguity). `termRuleSetSymbol.name` serves as the prefix to distinguish the name of the new symbol (and sub-symbols) in the format '[${termRuleSetSymbol.name}-negation]'.
