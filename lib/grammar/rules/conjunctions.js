...
exports.create = function (symbol, noConjunctionInsert) {
	const symbolPlus = g.newSymbol(symbol.name + '+')
	...
	const symbolAnd = g.newBinaryRule(newRule)

	const symbolPlusNoUnion = g.newSymbol(symbolPlus.name, 'no', 'union')
	...
	const symbolPlusNoUnionOr = g.newBinaryRule({
	...
	const symbolPlusIntersect = g.newSymbol(symbolPlus.name, 'intersect').addRule({
		rhs: [ symbolPlus ],
		// Use `intersect()` to group semantics for the parent `union()` semantic.
		semantic: exports.intersectSemantic,
	})
