...
exports.createDisjunctionSet = function (symbol) {
	...
	symbolPlusNoIntersect.addRule({
		rhs: [
			...
			// Note: Could redesign rule structure such that `symbolPlusNoIntersect` is repalced with an identical rule set without `union()`, to remove the need for `union()` flattening, however, it introduces overhead for the aditional parser reductions and state table size.
			symbolPlusNoIntersect,
		],
		semantic: exports.unionSemantic,
	})
