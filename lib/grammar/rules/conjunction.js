...
// Conjunction.
exports.and = g.newInvariableTerm({
	symbolName: 'and',
	insertionCost: 2,
	acceptedTerms: [ 'and' ],
...
// Disjunction.
exports.or = g.newInvariableTerm({
	symbolName: 'or',
	acceptedTerms: [ 'or' ],
...
exports.orWho = g.newBinaryRule({ rhs: [ exports.or, relPronouns.who ], noInsertionIndexes: [ 1 ] })
...
exports.orThat = g.newBinaryRule({ rhs: [ exports.or, relPronouns.that ], noInsertionIndexes: [ 1 ] })
...
exports.create = function (symbol, noConjunctionInsert) {
	...
	var symbolPlusNoUnionOr = g.newBinaryRule({
		rhs: [ symbolPlusNoUnion, exports.or ],
	...
	var symbolPlusIntersect = g.newSymbol(symbolPlus.name, 'intersect').addRule({
		rhs: [ symbolPlus ],
		// Use `intersect()` to group semantics for the parent `union()` semantic.
		// When `symbolPlus` returns a semantic tree with `intersect()` at its root, it indicates to `reduceUnion()` in `semantic` to not distribute the LHS semantic that preceds the `union()` semantic that precedes this semantic among that `intersect()` semantic's arguments.
		semantic: exports.intersectSemantic,
	})
