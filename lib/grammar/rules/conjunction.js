...
// For use by `conjunction.createDisjunctionSet()` with rule sets whose associated semantic forbids multiple instances of that semantic within an `intersect()` using `forbidsMultiple`, and therefore forbids logical conjunctions.
 ...
 *
 * For use by rule sets whose associated semantic forbids multiple instances of that semantic within an `intersect()` using `forbidsMultiple` (e.g., `repositories-created()`), and therefore forbids logical conjunctions. That semantic is to use the semantics this new disjunction rule set produces as its semantic arguments (except for `union()`, which is moved up the semantic tree to distribute the outer LHS semantic among the `union()` semantic arguments).
 *
 * Substitutes "and" -> "or" in input.
 *
 * @memberOf conjunction
...
exports.createDisjunctionSet = function (symbol) {
	var symbolPlusNoIntersect = g.newSymbol(symbol.name + '+', 'no', g.getSemanticName(exports.intersectSemantic))

	// (repos liked by) `[obj-users]`
	symbolPlusNoIntersect.addRule({
		rhs: [ symbol ],
	})
	...
	var symbolOr = g.newBinaryRule({
		// Use of `symbol` is identical to `symbolPlusNoIntersect` without `union()` (i.e., without the last `symbolPlusNoIntersect` rule).
		rhs: [ symbol, orSubstitutedAnd ],
		// Use `intersect()` to group semantics `symbol` produces for the parent `union()` semantic.
		semantic: exports.intersectSemantic,
	})
	var symbolPlusDisjunctionIntersect = g.newSymbol(symbol.name + '+', 'disjunction', 'intersect').addRule({
		rhs: [ symbolPlusNoIntersect ],
		// Use `intersect()` to group semantics for the parent `union()` semantic.
		// When `symbolPlusNoIntersect` returns a semantic tree with `intersect()` at its root, it indicates to `reduceUnion()` to not distribute the LHS semantic that precedes the `union()` that precedes this semantic among the arguments of that `intersect()`.
		semantic: exports.intersectSemantic,
	})
