...
function createBeConjugativeVerbTerminalRuleSet(options) {
	...
	// Extend `beVerbSym` with terminal rules for every terminal symbol in `options.verbFormsTermSet`, with `options.displayText` as each rule's `text` value, and `options.insertionCost` assigned to the first terminal rule in the set.
	addVerbTerminalRuleSet(beVerbSym, verbFormsTermSet, displayText, tense, options.insertionCost)
