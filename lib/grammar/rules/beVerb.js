 ...
 *
 * Match only the present tense forms in input. For use by rule sets where tense is semantically meaningful, and therefore must not substitute to the past tense, which must have a different semantic.
 */
 ...
 *
 * Match only the past tense forms in input. For use by rule sets where tense is semantically meaningful, and therefore must not substitute to the present tense, which must have a different semantic.
 *
 * Every terminal rule has `tense` defined as 'past', though only used to maintain input tense when `[be-past]` is a substitution within a term sequence defined on a rule with matching `acceptedTense`. `[be-past]` itself does not work with `acceptedTense` because its conjugative `text` object has no `past` value.
 */
 ...
 *
 * Match every form of "be" in input and conjugate to the correct form in the matched tense.
 */
