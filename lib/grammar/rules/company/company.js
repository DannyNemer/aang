...
company.addVerbRuleSet({
	verbTerm: g.newTermSequence({
		...
		substitutedTerms: [
			verbs.start,
			verbs.create,
			// Exclude past tense verb form "found" to avoid ambiguity with `[verb-found]`.
			verbs.findPresent,
		],
	}),
...
company.addTenseVerbRuleSet({
	...
	verbPastTerm: g.newInvariableTerm({
		...
		// No insertion cost to prevent multiple similar suggestions, with display text scarcely distinguished by tense (though different semantics). For example:
		//   "companies"
