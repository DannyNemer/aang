...
var founders = g.newSymbol('founders').addWord({
	insertionCost: 2.5,
	accepted: [ 'founders' ],
	substitutions: [ 'starters', 'creators' ],
})
// founders of `[companies+]`
var ofCompaniesPlus = g.newBinaryRule({ rhs: [ preps.participant, company.plPlus ] })
user.head.addRule({
	rhs: [ founders, ofCompaniesPlus ],
	noInsertionIndexes: [ 0 ],
	transpositionCost: 1,
	semantic: companyFoundersSemantic,
})
// `{company}` founders
user.head.addRule({ rhs: [ company.sg, founders ], semantic: companyFoundersSemantic })
...
user.head.addRule({
	rhs: [ boardMembers, ofCompaniesPlus ],
	noInsertionIndexes: [ 0 ],
	transpositionCost: 1,
