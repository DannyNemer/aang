...
module.exports = function (ruleSets) {
	...
	do {
		...
		// Check for rule-less and unreachable (via the start symbol) nonterminal symbols.
		for (var nontermSym in ruleSets) {
			if (nontermSym === startSymbol) continue

			if (ruleSets[nontermSym].length === 0 || !reachesSymbol(ruleSets, startSymbol, nontermSym)) {
