...
exports.addRule = function (ruleSets, nontermSym, newRule, stopAmbiguity) {
	 ...
	 *
	 * Perform `semanticChecks.isRuleMissingReducedSemantic()` check before the ambiguity check, which could otherwise throw exceptions for ambiguity with rules that would be discarded anyway for failing to produce a required semantic.
	 */
 ...
 * @param {boolean} [stopAmbiguity] Specify throwing an exception for a pair of ambiguous insertions, else save the cheapest of the two.
...
function rulesAreAmbiguous(ruleA, ruleB, nontermSym, stopAmbiguity) {
	...
	// Check if display texts, including conjugative text objects, are identical.
	...
	if (semanticsEquivalent || textsEquivalent) {
		 ...
		 *
		 * Need not account for an insertion rule being ambiguous with a transposition rule because they can not have identical RHS symbols, which `isUniqueRule()` requires before invoking this function.
		 */
 ...
 * @param {Object|string|(Object|string)[]} textB The other display text to compare.
