...
/**
 * Adds a new rule to the grammar if it is unique (i.e., unambiguous), below the cost upper bound, and satisfies any semantic requirement (i.e., has or can produce a reduced semantic required for itself or all ancestor rules).
 *
 * @static
...
exports.addRule = function (ruleSets, newRule, nontermSym, stopAmbiguity) {
	...
	if (ruleIsUnique(rules, newRule, nontermSym, stopAmbiguity) && newRule.cost < MAX_COST) {
		// Discard `newRule` if lacks and can not produce a reduced semantic required for itself or all ancestor rule; i.e., if `newRule` only produces semantically illegal paths.
