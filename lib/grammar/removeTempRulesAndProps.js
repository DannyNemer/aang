...
function removeTempRuleProps(ruleSets, includeTrees) {
	grammarUtil.forEachRule(ruleSets, function (rule) {
		...
		// `createInsertionRules` uses the following property to mark insertion rules as term sequences for which the original rule is not a term sequence because the inserted RHS symbol is not a term sequence.
		delete rule.termSequenceRHSIndexes
