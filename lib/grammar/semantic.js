	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen === 1) {
			...
			if (rhsSemanticNode.semantic === intersectSemantic) {
				// Check if `rhsSemanticNode already has `intersectCompleted`, which could only occur if there are three consecutive `intersect()` semantics.
				if (rhsSemanticNode.intersectCompleted) {
					util.logError('`rhsSemanticNode` already has `intersectCompleted`')
					util.dir(lhs, rhs)
					throw new Error('Semantic reduction error')
				}
...
function markRHSUnionCompleted(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		var rhsSemanticNode = rhsSemanticNodeArray[s]
		// Check `rhsSemanticNode` lacks `unionCompleted` to avoid unnecessarily recreating the semantic node when it already has the property.
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.unionCompleted) {
			// Copy `rhsSemanticNodeArray` to avoid mutating the shared resource.
			if (!rhsSemanticNodeArrayCopy) {
				rhsSemanticNodeArrayCopy = rhsSemanticNodeArray.slice(0, s)
