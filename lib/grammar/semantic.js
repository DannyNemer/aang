...
exports.reduce = function (lhs, rhs) {
	...
	rhs = flattenUnion(lhsSemantic, rhs)
 ...
 * For example:
 *   `lhsSemantic`: `lhsFunc()`
 *   `unionSemanticNode`: `union(0,intersect(1,followers(me)))`
...
function reduceUnion(lhsSemantic, unionSemanticNode) {
	...
	for (var u = 0, unionChildrenLen = unionChildren.length; u < unionChildrenLen; ++u) {
		...
		if (unionChildNode.semantic === intersectSemantic) {
			...
			// Distribute the LHS semantic function among children of `intersect()`, created from "and" pair between instaces of "or". For example:
...
/**
 * If` lhsSemantic` is a `union()` semantic node, replaces instances of `union()` semantic nodes within the first (most shallow) level of `rhsSemanticNodeArray` with their child semantic nodes, if any.
 *
 ...
 * @private
 * @static
 * @param {Object} lhsSemantic The `lhsSemantic` to check if is `union()`.
 * @param {Object[]} rhsSemanticNodeArray The semantic array of which to replace instances of `union()` with the semantic arguments if `lhsSemantic` is also `union()`.
 * @returns {Object[]} Returns a copy of `rhsSemanticNodeArray` without `union()` in the most shallow level if `lhsSemantic` is `union()`, else returns `rhsSemanticNodeArray` unchanged.
 */
function flattenUnion(lhsSemantic, rhsSemanticNodeArray) {
	if (lhsSemantic === unionSemantic) {
		...
		for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
			...
			if (rhsSemanticNode.semantic === unionSemantic) {
				// Copy `rhsSemanticNodeArray` if changes are needed to avoid mutating the shared resource.
				if (!copied) {
					rhsSemanticNodeArray = rhsSemanticNodeArray.slice()
					copied = true
				}

				// Insert the `union()` child semantic nodes in place of the `union()` node.
				var nodeChildren = rhsSemanticNode.children
				Array.prototype.splice.apply(rhsSemanticNodeArray, [ s, 1 ].concat(nodeChildren))
...
/**
 * Converts a semantic to a string representation.
 *
...
/**
 * Coverts a semantic to a string representation like `semantic.toString()`, but with syntax highlighting (for printing).
 *
 * @static
 * @memberOf semantic
 * @param {Object[]} semanticArray The semantic to convert.
...
/**
 * Converts a semantic string output by `semantic.toString()` to the original object array format.
 *
 * @static
 * @memberOf semantic
...
exports.stringToObject = function (semanticString) {
	...
	if (firstLeftParen === -1) {
		return [ {
			// Map to semantic definition or create an object for semantics created from input (e.g., entities). This enables parameter checks in `printParseResults`.
			semantic: exports._semantics[semanticString] || {
				name: semanticString,
				isArg: true,
			},
		} ]
	...
	var semanticNode = {
		semantic: exports._semantics[semanticString.slice(0, firstLeftParen)],
		children: [],
	}
