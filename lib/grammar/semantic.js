 ...
 * @param {boolean} [options.forbidsMultipleIntersection] Specify forbidding multiple instantces of this semantic function in the arguments of an `intersect()`, irrespective of this semantic's arguments.
...
/**
 * Checks for multiple instances of the same semantic function marked `forbidsMultipleIntersection` in `rhsSemanticNodeArray`.
 *
 ...
 * @returns {boolean} Returns `true` if `rhsSemanticNodeArray` contains multiple instances of the same semantic function marked `forbidsMultipleIntersection`, else `false`.
 */
...
exports.reduce = function (lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		/**
		 * When LHS is `intersect()`, prevent multiple instances of the same semantic function marked `forbidsMultipleIntersection` (e.g., `users-gender()`) within its arguments, `rhsSemanticNodeArray`.
		 *
		 * Only perform `forbidsMultipleIntersection` check when LHS is `intersect()`, instead of for any semantic array (via `semantic.mergeRHS()`) to allow for when LHS is `union()`, which is logically valid.
		 */
	 ...
	 *
	 * Use `else if` because if the preceding conditional check passed (i.e., `lhsSemantic` is `intersect()`), the method would have returned if `rhsLen` is 1.
	 */
	else if (rhsLen === 1) {
...
function completeSingleNodeConjunctionSemantic(semanticNodeArray) {
	...
	if (semanticNode.semantic === unionSemantic) {
		 ...
		 *                 followers(me)
		 *                 users-followed(me) ) ) ) ) ) ) )
		 * Invoking `completeMultiNodeConjunctionSemantic()` with the `union()` arguments, as annotated in the diagram, enables the following proper reduction and `union()` distribution:
		...
		if (semanticNode.isComplete) {
			...
			if (modifiedChildren) {
				// Copy and extend `semanticNode` (with modified `semanticNode.children`) to avoid mutating the shared resource.
				return [ {
			...
			// If `completeMultiNodeConjunctionSemantic()` makes no changes to `semanticNode.children`, return `semanticNodeArray` unchanged. This occurs for 69% of invocations.
...
/**
 * Copies and extends instances of `intersect()` in `semanticNodeArray`, if any, with the property `isComplete` to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows the parent `union()` among the arguments of its `intersect()` children in `semanticNodeArray`. If `semanticNodeArray` contains no `intersect()` instances, returns `undefined` to instruct the calling function to not wastefully recreate the parent `union()` node.
 *
 ...
 * @returns {Object[]|undefined} Returns a copy of `semanticNodeArray` with its `intersect()` nodes extended with the `isComplete` property, if any, else `undefined`.
 */
function completeMultiNodeConjunctionSemantic(semanticNodeArray) {
	...
	// Return `undefined` if made no changes to instruct `completeSingleNodeConjunctionSemantic()` to not wastefully recreate the parent `union()` node.
 ...
 *
 * Even with an alternative grammar rule design that uses only one `union()` semantic for multiple "or" instances within a single rule set (an implementation of which is located at `lib/deprecated/conjunctionsOneRootUnion`), this function's operation will be needed to detect semantic duplicity across nested conjunction rule sets. For example, the following two queries are semantically identical, but the second query requires removal of a nested `union()` added from a second conjunction rule set.
...
function flattenUnion(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		...
		if (rhsSemanticNode.semantic === unionSemantic) {
			...
			/**
			 * Check for illegal semantics after removing redundant, nested `union()` semantics; i.e., duplicates or `not()` contradictions. For example:
			 *   "repos I created or Danny likes or I created"
			 *   `union(repositories-created(me),union(repositories-created(me),repositories-liked(0)))`
			 *   -> `union(repositories-created(me),repositories-created(me),repositories-liked(0))`
			 *   -> -1
			 *
			 * Do not check for `forbidsMultipleIntersection` violations, which only occur within `intersect()`.
			 */
 ...
 *
 * `semantic.getName(semanticArray)` - Gets the name of the semantic in `semanticArray`.
 *
 * `semantic.forbidsMultipleIntersection(semanticArray)` - Checks if the root semantic of `semanticArray` has the property `forbidsMultipleIntersection` defined as `true`.
 */
