...
function copyLHSAndReduce(lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	// Check if `lhsSemantic` forbids multiple instances, yet the rules' design improperly enabled the semantic to receive multiple semantic arguments.
	if (lhsSemantic.forbidsMultiple) {
	...
	for (var s = 0; s < rhsLen; ++s) {
		...
		// Check for `union()` without `isCompletedUnion` in the RHS, as opposed to being passed to `reduceUnion()` when RHS has length of 1.
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.isCompletedUnion) {
