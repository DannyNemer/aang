...
/**
 * Reduces `lhs` with `rhs`, whose number of semantic nodes is within the LHS semantic node's `maxParams` bound. Ignores any children `lhs` may have, using only the LHS semantic node's function.
 *
 * For example:
 *   LHS: `intersect()`
 ...
 * @private
 * @static
 * @param {Object[]} lhs The LHS semantic node array.
 * @param {Object[]} rhs The RHS semantic node array to reduce with `lhs`.
 * @returns {Object[]|number} Returns the new semantic array from reducing `lhs` with `rhs`, if legal, else -1.
 */
function baseReduce(lhs, rhs) {
	if (lhs.length !== 1) {
		util.logError('Semantic reduction: lhs.length !== 1')
		util.dir(lhs, rhs)
		throw new Error('Semantic reduction error')
	...
	// Check this function was invoked when `rhs` has fewer semantic nodes than the `lhs` `maxParams` bound. Else, `copyLHSAndReduce()` should have been used.
	if (rhs.length > lhsSemantic.maxParams) {
	...
	return [ {
		semantic: lhsSemantic,
		// Mutating the shared `rhs` array is inconsequential because all instances must be sorted (identically) eventually.
