...
exports.reduce = function (lhs, rhs) {
	...
	/**
	 * Insert `rhs` at innermost semantic function by recursively reducing `lhsSemanticNode.children` with `rhs`, and replacing `rhs` with the resulting semantic tree. This rebuilds the `lhs` semantic tree while unwrapping,duplicating LHS semantic functions if necessary. For example:
	 *   LHS: `not(repos-liked())`  ->  LHS: `not()`
	 *   RHS: `[ 1, 2 ]`            ->  RHS: `repos-liked(1),repos-liked(2)`
	 *   -> `not(repos-liked(1)),not(repos-liked(2))`
	 *
	 * After replacing `rhs` with `lhsSemanticNode.children` reduced with the original `rhs`, this function caries on reducing the LHS semantic node with the new `rhs`, ignoring the original children on `lhsSemanticNode`.
	 *
	 ...
	 */
	if (lhsSemanticNode.children.length > 0) {
		// Replace `rhs` with reduction of `lhsSemanticNode.children` with `rhs`. Going forward, ignore `lhsSemanticNode.children` by only references `lhsSemanticNode.semantic`.
	...
	if (rhsLen < lhsSemantic.minParams) {
		util.logError('Semantic reduction: rhs.length < lhsSemantic.minParams')
		util.dir(lhs, rhs)
		throw new Error('Semantic reduction error')
	}
	...
	if (rhsLen > lhsSemantic.maxParams) {
		/**
		 * When `lhs` has `maxParams` of 1 and `rhs` contains > 1 semantic nodes, reduce a duplicate of the LHS semantic node with each RHS semantic node, and create a merged semantic node array.
		 *
		 * Ignore any children `lhs` might have, because if any, they were reduced with `rhs` above.
		 */
		return copyLHSAndReduce(lhs, rhs)
	}

	/**
	 * Reduce `lhs` with `rhs`, whose number of semantic nodes is within the LHS semantic node's `maxParams` bound.
	 *
	 * Ignore any children `lhs` may have, using only the LHS semantic node's function.
	 *
	 * If `lhs` is a `union()` semantic node, first invokes `flattenUnion(rhs)` to remove redundant instances of nested `union()` semantics to enable proper semantic duplicity detection.
	 */
	return baseReduce(lhs, rhs)
}
