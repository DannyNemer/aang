...
/**
 * Reduces `lhs`, which has `maxParams` of 1, with `rhs`, which contains > 1 semantic nodes by reducing a duplicate of the LHS semantic node with each RHS semantic node, and creating a merged semantic node array. Ignores any children `lhs` may have, using only the LHS semantic node's function.
 *
 * For example:
 *   LHS: `repos-liked()`
 *   RHS: `[ 1, 2 ]`
 ...
 * @private
 * @static
 * @param {Object[]} lhs The LHS semantic node array with `maxParams` 1.
 * @param {Object[]} rhs The RHS semantic node array with > 1 semantic nodes.
 * @returns {Object[]} Returns the new semantic array from reducing `lhs` with `rhs`.
 */
function copyLHSAndReduce(lhs, rhs) {
	if (lhs.length !== 1) {
		util.logError('Semantic reduction: lhs.length !== 1')
		util.dir(lhs, rhs)
		throw new Error('Semantic reduction error')
	...
	// Check this function was invoked when `rhs` has more semantic nodes than the `lhs` `maxParams` bound, requiring `lhs` be copied for each `rhs` semantic node. Else, `baseReduce()` should have been used.
	if (rhsLen <= lhsSemantic.maxParams) {
