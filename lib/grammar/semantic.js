...
exports.reduce = function (lhs, rhs) {
	...
	// Reduce `lhs` with the `rhs` `union()` semantic node by distributing the LHS semantic, `lhs`, among the `union()` semantic arguments, `rhsSemanticNode.children`, creating a new semantic tree with `union()` as the root. Invoke `reduceUnion()` after the `intersect()` check.
	if (rhsLen === 1) {
...
function reduceUnion(lhsSemanticNodeArray, unionSemanticNode) {
	...
	var newLHSNode = {
		...
		children: [],
		// Define `unionCompleted` as `true` to mark the conjunction as complete to prevent `reduceUnion()` from moving the semantic node any further up the tree.
		unionCompleted: true,
	}
	...
	for (var u = 0, unionChildrenLen = unionChildren.length; u < unionChildrenLen; ++u) {
		...
		if (unionChildNode.semantic === intersectSemantic) {
			/**
			 * Having reached an `intersect()` semantic, assign `unionCompleted` to instances of `union()` semantic nodes in `rhs`, if any, to prevent `reduceUnion()` from moving the semantic node any further up the tree.
			 * NOTE: This operation is only to avoid the `semantic.reduce()` error for incomplete `union()` semantic nodes in a RHS of length > 1.
			 */
			var intersectChildren = markRHSUnionCompleted(unionChildNode.children)
			...
			var newIntersectNode = {
				...
				children: exports.reduce(lhsSemanticNodeArray, intersectChildren),
			}
