...
exports.reduce = function (lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	if (lhsSemantic === intersectSemantic) {
		 ...
		 *
		 * If `rhsSemanticNodeArray` is a `union()` semantic node, having reached its parent `intersect()` semantic, assign `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from moving the `union()` semantic node too far up its semantic tree.
		 *
		 ...
		 */
		if (rhsLen === 1) {
			return markMultiNodeRHSConjunctionComplete(rhsSemanticNodeArray)
		}
		 ...
		 */
			...
			rhsSemanticNodeArray = markMultiNodeRHSConjunctionComplete(rhsSemanticNodeArray)
		}
	}
	 ...
	 *
	 * Invoke `reduceUnion()` after the `intersect()` check, which invokes `markMultiNodeRHSConjunctionComplete()` to assign properties indicating reaching a conjunction's root. Invoke before traversing `lhsSemanticNodeArray` children, if any, to properly distribute all of `lhsSemanticNodeArray` among the `union()` arguments.
	 */
	if (rhsLen === 1) {
 ...
 *
 * Upon reducing `intersect()` with a RHS semantic array of length 1, `rhsSemanticNodeArray`, copies and extends the RHS semantic node with an applicable property, if any, to indicate to `reduceUnion()` having reached the parent `intersect()`. If `rhsSemanticNodeArray` contains no applicable semantics, returns it unchanged.
 *
 * If `rhsSemanticNodeArray` is a `union()` semantic node, having reached its parent `intersect()` semantic, assign `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from moving the `union()` semantic node too far up its semantic tree.
 *
 * Else if `rhsSemanticNodeArray` is an `intersect()` semantic node, having reached its parent `intersect()` semantic, assign `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows a `union()` semantic too deep among the arguments of instances of `intersect()` within the `union()` semantic.
 ...
 *
 *   Following this invocation, and in the next `semantic.reduce()` invocation:
 ...
 */
function markMultiNodeRHSConjunctionComplete(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		...
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.isComplete) {
			...
			rhsSemanticNodeArrayCopy.push(copyAndMarkSemanticNodeComplete(rhsSemanticNode))
		}
...
/**
 * Creates a copy of `semanticNode` extended with the boolean property `isComplete` defined as `true`.
 *
 * For use to mark `intersect()` and `union()` semantics within conjunctions as complete having reached their parent `intersect()` semantic.
 *
 * It is necessary to copy `semanticNode` to not mutate the shared resource.
 *
 * @private
 * @static
 * @param {Object} semanticNode The semantic node to copy and extend.
 * @returns {Object} Returns a copy of `semanticNode` extended with the boolean property `isComplete` defined as `true`.
 */
function copyAndMarkSemanticNodeComplete(semanticNode) {
	var semanticDef = semanticNode.semantic
	if (semanticDef !== unionSemantic && semanticDef !== intersectSemantic) {
		util.logError('`copyAndMarkSemanticNodeComplete()` invoked with a semantic that is neither `union()` nor `intersect()`:', semanticNode)
		throw new Error('Semantic reduction error')
	}
...
function flattenUnion(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		 ...
		 *   `union(func(a),union(func(b),func(c)))` -> `union(func(a),func(b),func(c))`
		 *
		 * Do not check these nested `union()` nodes for `isComplete`, which indicates the root of a conjunction set to `reduceUnion()`. `markMultiNodeRHSConjunctionComplete()` assigns `isComplete` upon reaching the parent `intersect()` that follows `union()`, however, every rule with `union()` (in conjunctions) has RHS symbols that only produce rules with `intersect()` to group the `union()` semantic arguments on each side of an "or".
		 *
