...
exports.reduce = function (lhs, rhs) {
	...
	if (lhsSemanticNode.anaphoric) {
			var firstRHSSemantic = [ rhs[0] ]

			if (lhsSemanticNode.children.length > 0) throw 'oops'

			var lhsSemanticSansProp = [ {
				semantic: lhsSemantic,
				children: [],
			} ]

			// Copy the antecedent in the RHS over to the anaphor's location.
			var lhsSemanticAntecedent = exports.reduce(lhsSemanticSansProp, firstRHSSemantic)

			// It would be simpler to add `intersect()` around the anaphor and `rhs` arguments excluding the antecedent, however, they are semantically identical because intersection is an associative operation.
			return exports.mergeRHS(rhs, lhsSemanticAntecedent)
	}
