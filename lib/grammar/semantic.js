...
exports.reduce = function (lhs, rhs) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen === 1) {
			/**
			 * If `rhs` is an `intersect()` semantic node, having reached its parent `intersect()` semantic, assign `isCompletedIntersect` to the semantic node to prevent `reduceUnion()` from incorrectly distributing the LHS semantic that follows a `union()` semantic too far amongst instances of `intersect()` within the `union()` semantic.
			 *
			 * Else if `rhs` is a `union()` semantic node, having reached its parent `intersect()` semantic, assign `isCompletedUnion` to the semantic node to prevent `reduceUnion()` from moving the `union()` semantic node too far up its semantic tree.
			 *
 ...
 *
 * If `rhsSemanticNodeArray` is an `intersect()` semantic node, having reached its parent `intersect()` semantic, assign `isCompletedIntersect` to the semantic node to prevent `reduceUnion()` from incorrectly distributing the LHS semantic that follows a `union()` semantic too far amongst instances of `intersect()` within the `union()` semantic.
 *
 * Else if `rhsSemanticNodeArray` is a `union()` semantic node, having reached its parent `intersect()` semantic, assign `isCompletedUnion` to the semantic node to prevent `reduceUnion()` from moving the `union()` semantic node too far up its semantic tree.
 *
...
/**
 * Creates a semantic node array with a copy of `rhsIntersectSemanticNode` extended with the boolean property `isCompletedIntersect` defined as `true`. This property prevents `reduceUnion()` from incorrectly distributing the LHS semantic that follows a `union()` semantic too far amongst instances of `intersect()` within the `union()` semantic. Invoke this function when reducing the LHS semantic `intersect()` with a RHS semantic array containing a single `intersect()` semantic node, `rhsIntersectSemanticNode`.
 *
 ...
 * @private
 * @static
 * @param {Object} rhsIntersectSemanticNode The RHS `intersect()` semantic node being reduced with `intersect()`, for which to extend with `isCompletedIntersect`.
 * @returns {Object[]} Returns a semantic array with a copy of `rhsIntersectSemanticNode` extended with the boolean property `isCompletedIntersect` defined as `true`.
 */
function markIntersectComplete(rhsIntersectSemanticNode) {
	// Check `rhsIntersectSemanticNode` is an `intersect()` semantic node.
	if (rhsIntersectSemanticNode.semantic !== intersectSemantic) {
		util.logError('markIntersectComplete: `rhsIntersectSemanticNode` is not an `intersect()` semantic node:', rhsIntersectSemanticNode)
		throw new Error('Semantic reduction error')
	}

	// Recreate RHS `intersect()` semantic node with `isCompletedIntersect` as `true`. Copy to not mutate the shared resource.
	return [ {
		semantic: rhsIntersectSemanticNode.semantic,
		children: rhsIntersectSemanticNode.children,
		isCompletedIntersect: true,
	} ]
}

/**
 * Creates a semantic node array with a copy of `rhsUnionSemanticNode` extended with the boolean property `isCompletedUnion` defined as `true`. This property prevents reduceUnion()` from moving the `union()` semantic node too far up its semantic tree. Invoke this function when reducing the LHS semantic `intersect()` with a RHS semantic array containing a single `union()` semantic node, `rhsUnionSemanticNode`.
 *
 ...
 * @private
 * @static
 * @param {Object} rhsUnionSemanticNode The RHS `union()` semantic node being reduced with `intersect()`, for which to extend with `isCompletedUnion`.
 * @returns {Object[]} Returns a semantic array with a copy of `rhsUnionSemanticNode` extended with the boolean property `isCompletedUnion` defined as `true`.
 */
function markUnionComplete(rhsUnionSemanticNode) {
	// Check `rhsUnionSemanticNode` is a `union()` semantic node.
	if (rhsUnionSemanticNode.semantic !== unionSemantic) {
		util.logError('markUnionComplete: `rhsUnionSemanticNode` is not a `union()` semantic node:', rhsUnionSemanticNode)
		throw new Error('Semantic reduction error')
	}

	// Recreate RHS `union()` semantic node with `isCompletedUnion` as `true`. Copy to not mutate the shared resource.
	return [ {
		semantic: rhsUnionSemanticNode.semantic,
		children: rhsUnionSemanticNode.children,
		isCompletedUnion: true,
	} ]
}
