...
exports.reduce = function (lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	if (lhsSemantic === intersectSemantic) {
		 ...
		 *
		 * Copy and extend instances of `intersect()` and `union()` in `rhsSemanticNodeArray`, if any, with the property `isComplete` to indicate to `reduceUnion()` having reached the parent `intersect()`. If `rhsSemanticNodeArray` contains no applicable semantics, leave as is.
		 * • For instances of `union()`, assign `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from moving the `union()` semantic node too far up its semantic tree.
		 * • For instances of `intersect()`, assign `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows a `union()` semantic too deep among the arguments of instances of `intersect()` within the `union()` semantic.
		 *
...
/**
 * Copies and extends semantic node in `semanticNodeArray` if it is either `union()` or `intersect()` with the property `isComplete`, which indicates to `reducedUnion()` that `semanticNodeArray` reached its parent `intersect()`. If `semanticNodeArray` does not contain either specified semantic function, returns it unchanged.
 *
 ...
 *
 * If `semanticNodeArray` contains `intersect()`, assigns `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows a `union()` semantic among the arguments of instances of `intersect()` within the `union()` semantic.
 *
