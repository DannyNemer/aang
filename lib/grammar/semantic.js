...
exports.reduce = function (lhs, rhs) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen === 1) {
			...
			// If `rhs` is a `union()` semantic node, having reached its parent `intersect()` semantic, assign `isCompletedUnion` to the semantic node to prevent `reduceUnion()` from moving the semantic node any further up the tree.
...
/**
 * Defines the boolean property `isCompletedUnion` as `true` for each `union()` semantic node in `rhsSemanticNodeArray`, if any. Invoke this function when reducing with `intersect()` as the LHS semantic.
 *
 ...
 *
 * To determine having completed a `union()` conjunction, and when to stop moving `union()` up the semantic tree, `semantic.reduce()` invokes this function upon reaching the parent `intersect()` semantic to mark `union()` semantic nodes, if any, with the property `isCompletedUnion`. `reduceUnion()` checks for the `isCompletedUnion` property.
 *
 * This function does not mutate `rhsSemanticNodeArray`; rather, it copies the array if changes are needed, else returns the original array.
 *
 * @private
 * @static
 * @param {Object[]} rhsSemanticNodeArray The RHS semantic node array, being reduced with `intersect()`, of which to assign `isCompletedUnion` to each `union()` semantic node, if any.
 * @returns {Object[]} Returns a copy of `rhsSemanticNodeArray` with `isCompletedUnion` assigned to each `union()` semantic node, if any, else `rhsSemanticNodeArray` unchanged.
 */
function markRHSUnionCompleted(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		...
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.isCompletedUnion) {
			...
			// Recreate RHS `union()` semantic node with `isCompletedUnion` as `true`.
