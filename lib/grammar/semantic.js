...
function reduceUnion(lhsSemanticNodeArray, unionSemanticNode) {
	...
	for (var u = 0, unionChildrenLen = unionChildren.length; u < unionChildrenLen; ++u) {
		...
		if (unionChildNode.semantic === intersectSemantic && !unionChildNode.isCompletedIntersect) {
			...
			// Check if reducing `lhsSemanticNodeArray` with the `intersect()` semantic arguments is semantically illegal. This has yet to be seen, though the check exists for good measure.
			...
			// Check if reducing `lhsSemanticNodeArray` with the `union()` semantic argument is semantically illegal. This has yet to be seen, though the check exists for good measure.
