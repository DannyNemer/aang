...
exports.reduce = function (lhs, rhs) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen === 1) {
			// Recreate `union()` semantic nodes with `sorted` as `true` to mark the conjunction as complete (having reached the parent `intersect()`) and prevent other instances of `union()` from distributing too far.
			var rhsSemanticNode = rhs[0]
			if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.sorted) {
				return [ {
	...
	if (rhsLen === 1) {
		...
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.sorted) {
			var newLHSNode = {
				semantic: unionSemantic,
				children: [],
				// Define `sorted` as `true` to mark the conjunction as complete and prevent other instances of `union()` from distributing too far.
				sorted: true,
			}
...
exports.isReduced = function (semanticArray) {
	return semanticArray.every(function (semanticNode) {
		...
		if (semanticChildren) {
			return semanticChildren.length === 0 ? false : exports.isReduced(semanticChildren)
		}
