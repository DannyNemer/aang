...
exports.reduce = function (lhs, rhs) {
	...
	// Reduce `lhsSemantic` with the `rhs` `union()` semantic node by distributing the LHS semantic function, `lhsSemantic`, among the `union()` semantic arguments, `rhsSemanticNode.children`, creating a new semantic tree with `union()` as the root. Invoke `reduceUnion()` after the `intersect()` check.
	if (rhsLen === 1) {
...
/**
 * Reduces `lhsSemantic` with `unionSemanticNode` by distributing the LHS semantic function, `lhsSemantic`, among the `union()` semantic arguments, `unionSemanticNode.children`, and creating a new semantic tree with `union()` as the root.
 *
 * For example:
 *   LHS: `lhsFunc()`
 ...
 *
 * For use by `semantic.reduce(lhs, rhs)`, when `lhs` is an unreduced semantic array and `rhs` is a semantic array with a single semantic node for `union()` and the boolean property `unionSemanticNode.unionCompleted` is falsey. Invoke this function after the `intersect()` check in `semantic.reduce()`.
 *
 * @private
 * @static
