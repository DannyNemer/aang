...
/**
 * Checks if `semanticNodeArray` has a semantic in its first level that can be an antecedent.
 *
 * Semantics that can serve as antecedents for anaphora have the property `anaphoraPersonNumber`, which defines the grammatical person-number of the person or group for which the semantic represents.
 *
 * @static
 * @memberOf semantic
 * @param {Object[]} semanticNodeArray The semantic node array to inspect.
 * @returns {boolean} Returns `true` if `semanticNodeArray` has an antecedent semantic, else `false`.
 */
