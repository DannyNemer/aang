...
exports.reduce = function (lhs, rhs) {
	...
	if (lhsSemanticNode.children.length > 0) {
		/**
		 * Insert `rhs` at innermost semantic function and then rebuild the trees as unwrapping (duplicating if necessary). For example:
		 *   LHS: `not(repos-liked())`
		 *   RHS: `[ 1, 2 ]`
	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen === 1) {
			// If `rhs` is a `union()` semantic node, having reached its parent `intersect()` semantic, assign `unionCompleted` to the semantic node to prevent `reduceUnion()` from moving the semantic node any further up the tree.
			return markRHSUnionCompleted(rhs)
		}
	}

	// Reduce `lhsSemantic` with the `rhs` `union()` semantic node by distributing the LHS semantic function, `lhsSemantic`, among the `union()` semantic arguments, `unionSemanticNode.children`, creating a new semantic tree with `union()` as the root. Invoke `reduceUnion()` after the `intersect()` check.
	if (rhsLen === 1) {
		var rhsSemanticNode = rhs[0]
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.unionCompleted) {
			return reduceUnion(lhsSemantic, rhsSemanticNode)
		}
	}
	...
	if (rhsLen > lhsSemantic.maxParams) {
		...
		for (var s = 0; s < rhsLen; ++s) {
			...
			// Check for `union()` without `unionCompleted` in the RHS, as opposed to being caught above and passed to `reduceUnion()` when RHS length of 1.
			if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.unionCompleted) {
	...
	if (lhsSemantic === unionSemantic) {
		// If `lhsSemantic` is a `union()` semantic node, replace instances of `union()` semantic nodes within the first (most shallow) level of `rhs` with their child semantic nodes, if any.
...
/**
 * Reduces `lhsSemantic` with `unionSemanticNode` by distributing the LHS semantic function, `lhsSemantic`, among the `union()` semantic arguments, `unionSemanticNode.children`, creating a new semantic tree with `union()` as the root.
 *
 * For example:
 *   LHS: `lhsFunc()`
 ...
 *
 * For use by `semantic.reduce(lhs, rhs)`, when `lhs` is an unreduced semantic array and `rhs` is a semantic array of with a single semantic node for `union()` and the boolean property `unionSemanticNode.unionCompleted` is falsey. Invoke this function after the `intersect()` check near the top of `semantic.reduce()`.
 *
 * @private
 * @static
 * @param {Objct} lhsSemantic The LHS semantic function to reduce with `unionSemanticNode.children`.
 ...
 */
function reduceUnion(lhsSemantic, unionSemanticNode) {
	...
	if (unionSemanticNode.unionCompleted) {
		util.logError('reduceUnion: `unionSemanticNode.unionCompleted` is `true`:', unionSemanticNode)
		throw new Error('Semantic reduction error')
	}
	...
	// Distribute the LHS semantic function, `lhsSemantic`, among the semantic arguments of `union()`.
	...
	for (var u = 0, unionChildrenLen = unionChildren.length; u < unionChildrenLen; ++u) {
		...
		if (unionChildNode.semantic === intersectSemantic) {
			var newIntersectNode = {
				semantic: intersectSemantic,
				children: [],
			}

			// Distribute the LHS semantic function among children of `intersect()`, created from "and" pair between instances of "or". For example:
			...
			var intersectChildren = unionChildNode.children
			for (var i = 0, intersectChildrenLen = intersectChildren.length; i < intersectChildrenLen; ++i) {
			...
			// Need not sort `intersect()` children because they were already sorted before distributing the LHS semantic function, and the order will not change because all are prefixed with the same function.
			newLHSNode.children.push(newIntersectNode)
		} else {
			newLHSNode.children.push({
				semantic: lhsSemantic,
				children: [ unionChildNode ],
			})
		}
	}
...
/**
 * Replaces instances of `union()` semantic nodes within the first (most shallow) level of `rhsSemanticNodeArray`, if any, with their child semantic nodes. Invoke this function from `semantic.reduce(lhs, rhs)` when `lhs` is `union()`.
 *
 ...
 *
 * NOTE: An unfinished implementation of grammar rules where `union()` only exists at the root the parse subtree, removing the need for this operation, exists at `'aang/notes/ignore/incomplete/conjunctionsOneRootUnion.js`.
 *
 * @private
 * @static
...
function flattenUnion(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		 ...
		 *   `union(func(a),union(func(b),func(c)))` -> `union(func(a),func(b),func(c))`
		 *
		 * Do not check for `unionCompleted` because there is an `intersect()` for each RHS symbol of rules with `union()` to group the semantics between instances of "or". Hence, those `intersect()` instances are not separating sections of the tree, which `reduceUnion()` looks for with `unionCompleted`. Rather, because reducing `union()` with a RHS that also has `union()`, the semantics are part of the same conjunction parse subtree.
		 */
...
/**
 * Defines the boolean property `unionCompleted` as `true` for each `union()` semantic node in `rhsSemanticNodeArray`, if any. Invoke this function when reducing with `intersect()` as the LHS semantic.
 *
 * In the grammar, rules with the `union()` semantic come after the LHS semantic function to which they apply. `reduceUnion()` moves instances of `union()` up the tree and distributes the outer LHS semantic function(s) among the `union()` semantic children. For example:
 *   LHS: `lhsFunc()`
 *   RHS: `union(0,intersect(1,followers(me)))`
 *   -> `union(lhsFunc(0),intersect(lhsFunc(1),lhsFunc(followers(me))))`
 *
 * To determine having completed a `union()` conjunction, and when to stop moving `union()` up the semantic tree, `semantic.reduce()` invokes this function upon reaching the parent `intersect()` semantic to mark `union()` semantic nodes, if any, with the property `unionCompleted`. `reduceUnion()` checks for the `unionCompleted` property.
 *
 * This function does not mutate `rhsSemanticNodeArray`; rather, it copies the array if changes are needed, else returns the original array.
 *
 * @private
 * @static
 * @param {Object[]} rhsSemanticNodeArray The RHS semantic node array, being reduced with `intersect()`, of which to assign `unionCompleted` to each `union()` semantic node, if any.
 * @returns {Object[]} Returns a copy of `rhsSemanticNodeArray` with `unionCompleted` assigned to each `union()` semantic node, if any, else `rhsSemanticNodeArray` unchanged.
 */
function markRHSUnionCompleted(rhsSemanticNodeArray) {
	var rhsSemanticNodeArrayCopy
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		var rhsSemanticNode = rhsSemanticNodeArray[s]
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.unionCompleted) {
			...
			// Recreate RHS `union()` semantic node with `unionCompleted` as `true`.
