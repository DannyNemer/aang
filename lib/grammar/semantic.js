...
exports._semantics = {}
// The map of semantic names to creation-lines (file-path + line-number); used for error reporting.
...
exports.new = function (options) {
	options.name = grammarUtil.formatStringForName(options.name)

	if (exports._semantics.hasOwnProperty(options.name)) {
		var nameStylized = util.stylize(options.name)
		util.logErrorAndPath('Duplicate semantic name:', nameStylized)
		util.log('  Other', nameStylized, 'definition:')
		util.log('    ' + exports._creationLines[options.name], '\n')
	...
	exports._creationLines[options.name] = util.getModuleCallerPathAndLineNumber()

	// Save semantics used by `semantic` functions.
	if (options.name === 'intersect') {
		intersectSemantic = semantic[0].semantic
	} else if (options.name === 'union') {
		unionSemantic = semantic[0].semantic
	} else if (options.name === 'not') {
		notSemantic = semantic[0].semantic
	}
...
function newSemanticFunc(options) {
	...
	var semantic = exports._semantics[options.name] = {
		name: options.name,
		cost: options.cost,
