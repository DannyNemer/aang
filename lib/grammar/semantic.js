...
exports.reduce = function (lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen > 1) {
			 ...
			 */
			rhsSemanticNodeArray = markRHSConjunctionComplete(rhsSemanticNodeArray)
		} else {
			/**
			 * Copy and extend instances of `intersect()` and `union()` in `rhsSemanticNodeArray`, if any, with the property `isComplete`, as above.
			 *
