...
/**
 * Checks if `prevLHSSemanticNodeArray` is `intersect()`, and `newLHSSemanticNodeArray`, a new LHS semantic function (yet to reduce), exists in `rhsSemanticNodeArray`, which contains the previous RHS semantics (already reduced) that will become the semantic arguments of the parent `intersect()`, when multiple instances of the semantic function in `newLHSSemanticNodeArray` (irrespective of children) are forbidden within an `intersect()`.
 *
 ...
 * @static
 * @memberOf semantic
 * @param {Object[]} prevLHSSemanticNodeArray The most recent unreduced semantic in a tree that will be reduced with the merge of `rhsSemanticNodeArray` and `newLHSSemanticNodeArray` upon the latter's completion.
 * @param {Object[]} rhsSemanticNodeArray The most recent set of reduced semantics that follows `prevLHSSemanticNodeArray` in a tree that has yet to be reduced into its parent semantic, `prevLHSSemanticNodeArray`.
 ...
 * @returns {boolean} Returns `true` if `prevLHSSemanticNodeArray` is `intersect()` and `rhsSemanticNodeArray` contains an instance of `newLHSSemanticNodeArray`'s semantic function for which multiple instances are forbidden, else `false`.
 */
exports.isForbiddenMultiple = function (prevLHSSemanticNodeArray, rhsSemanticNodeArray, newLHSSemanticNodeArray) {
	// The outer LHS semantic must be `intersect()` for its contents (which will be a merge of `rhsSemanticNodeArray` and `newLHSSemanticNodeArray`) to be in logical violation.
	if (prevLHSSemanticNodeArray[0].semantic === intersectSemantic) {
...
exports.reduce = function (lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen === 1) {
			...
			/**
			 * When LHS is `intersect()`, prevent multiple instances of the same semantic if marked `forbidsMultiple` (e.g., 'users-gender()').
			 *
			...
			for (var s = 0; s < rhsLen; ++s) {
				...
				if (semantic.forbidsMultiple) {
					for (var j = s + 1; j < rhsLen; ++j) {
						if (semantic === rhsSemanticNodeArray[j].semantic) {
							return -1
						}
					}
				}
