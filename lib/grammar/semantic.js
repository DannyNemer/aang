...
// The operator semantics used throughout this module, including in `semantic.reduce()`, `reduceUniion()`, `semantic.isIllegalRHS()`, and `hasSemantic()`.
var intersectSemantic
var unionSemantic
var notSemantic
// If not currently building the grammar via `buildGrammar`, then load the `intersect()`, `union()`, and `not()` semantics from the grammar file, which works for comparisons because of `initSemantics`.
...
/**
 * Creates and adds a new semantic function or argument to the grammar.
 *
 * @static
 * @memberOf semantic
...
/**
 * Sums the costs of all semantics in `semanticNodeArray`.
 *
 * @static
 * @memberOf semantic
 * @param {Object[]} semanticNodeArray The semantic tree to sum.
...
/**
 * Checks if reduced (RHS) semantic node arrays `a` and `b` are forbidden to merge as a single set of RHS semantic arguments. This is `true` for the following cases:
 * - There are duplicate semantics.
 * - Violates a semantic's `forbidsMultiple` property.
 * - There are contradictory negations.
 *
 * It is necessary to prevent contradictory and duplicate semantics, even when explicitly input as such by the user, to prevent poor semantics as suggestions. In addition, completely simplifying the semantics's logic enables `pfsearch` to correctly identify semantically identical parse trees (whose logical equivalence would be undetectable in their unsimplified forms).
 *
 * @static
 * @memberOf semantic
...
/**
 * Checks if `newLHSSemanticNodeArray`, a new LHS semantic function (yet to be reduced), exists in `rhsSemanticNodeArray`, the previous RHS semantics (already reduced) when multiple instances of that semantic function (irrespective of children) are forbidden.
 *
 * This check enables its parse tree's rejection without having to complete `newLHSSemanticNodeArray` and wait for `semantic.mergeRHS()` to reject the tree for this reason.
 *
 * @static
 * @memberOf semantic
 * @param {Object[]} rhsSemanticNodeArray The most recent set of reduced semantics in a tree that have yet to be reduced into their parent semantic.
 * @param {Object[]} newLHSSemanticNodeArray The new semantic, yet to be reduced, which will eventually be concatenated with `rhsSemanticNodeArray` as semantic arguments.
 * @returns {boolean} Returns `true` if `rhsSemanticNodeArray` contains an instance of `newLHSSemanticNodeArray`'s semantic function for which multiple instances are forbidden, else `false`.
 */
exports.isForbiddenMultiple = function (rhsSemanticNodeArray, newLHSSemanticNodeArray) {
	// `newLHSSemanticNodeArray` can only ever have one semantic (which has yet to be reduced).
	var lhsSemantic = newLHSSemanticNodeArray[0].semantic

	// Check if multiple instances of `lhsSemantic` are forbidden within the same set of semantic arguments irregardless of the LHS semantic (e.g., 'users-gender()'').
 ...
 * @static
 * @memberOf semantic
 * @param {Object[]} lhsSemanticNodeArray The LHS non-reduced semantic.
 * @param {Object[]} rhsSemanticNodeArray The array of RHS reduced semantics.
...
exports.reduce = function (lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		/**
		 * Reject if `rhsSemanticNodeArray` contains a semantic that `requires` another semantic not within this semantic tree, stopping before instances of the `intersect()` semantic.
		 *
		 ...
		 *
		 * This check must occur before the check below (for when the `lhsSemanticNodeArray` is `intersect()` and `rhsSemanticNodeArray` is a single semantic) because it is possible for a semantic tree to lack `intersect()`.
		 */
		if (isMissingRequiredSemantic(rhsSemanticNodeArray)) {
			return -1
		}

		// Do not reduce `intersect()` with `rhsSemanticNodeArray` if `rhsSemanticNodeArray` only contains one semantic node.
		if (rhsLen === 1) {
			/**
	 ...
	 *
	 * Invoke `reduceUnion()` after the `intersect()` check, which invokes `markRHSConjunctionComplete()` to assign properties indication reaching a conjunction's root. Invoke before traversing `lhsSemanticNodeArray` children, if any, to properly distribute all of `lhsSemanticNodeArray` among the `union()` arguments.
	 */
	if (rhsLen === 1) {
	...
	/**
	 * Insert `rhsSemanticNodeArray` at innermost semantic function by recursively reducing `lhsSemanticNode.children` with `rhsSemanticNodeArray`, and replacing `rhsSemanticNodeArray` with the resulting semantic tree. This rebuilds the `lhsSemanticNodeArray` semantic tree while unwrapping,duplicating LHS semantic functions if necessary. For example:
	 ...
	 *
	 * Invoke after invoking `reduceUnion()`, which properly moves a `union()` semantic up a semantic tree and distributes the entire `lhsSemanticNodeArray` among the `union()` arguments. For example:
	 ...
	 */
	if (lhsSemanticNode.children.length > 0) {
		// Replace `rhsSemanticNodeArray` with reduction of `lhsSemanticNode.children` with `rhsSemanticNodeArray`. Going forward, ignore `lhsSemanticNode.children` by only references `lhsSemanticNode.semantic`.
		...
		// Update `rhsLen`, which might have increased from duplicating semantic children for multiple nodes in `rhsSemanticNodeArray`.
...
/**
 * Reduces `lhsSemanticNodeArray`, which has `maxParams` of 1, with `rhsSemanticNodeArray`, which contains > 1 semantic nodes by reducing a duplicate of the LHS semantic node with each RHS semantic node, and creating a merged semantic node array. Ignores any children `lhsSemanticNodeArray` may have, using only the LHS semantic node's function.
 *
 * For example:
...
function copyLHSAndReduce(lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	// Check this function was invoked when `rhsSemanticNodeArray` has more semantic nodes than the `lhsSemanticNodeArray` `maxParams` bound, requiring `lhsSemanticNodeArray` be copied for each `rhsSemanticNodeArray` semantic node. Else, `baseReduce()` should have been used.
	if (rhsLen <= lhsSemantic.maxParams) {
		util.logError('`copyLHSAndReduce()` invoked when RHS semantic within LHS `maxParams` bound. Use `baseReduce()` instead.')
		util.dir(lhsSemanticNodeArray, rhsSemanticNodeArray)
		throw new Error('Semantic reduction error')
	}

	// Check `lhsSemantic.maxParams` is 1. Otherwise, would require dividing RHS semantic nodes into groups among multiple LHS semantic nodes; e.g., 4 RHS semantic nodes halved for 2 LHS semantic nodes. This is not supported because it enables a grammar designed to expect a specific positioning of RHS semantic nodes within an array before the array's reduction (and sorting). Though possible, such a design is too complex and should be discouraged because an alternative, straightforward semantic rule structure is always possible.
	...
	// Check if `lhsSemantic` forbids multiple instances, yet the rules' design improperly enabled the function to receive multiple semantic arguments.
	if (lhsSemantic.forbidsMultiple) {
		util.logError('Semantic reduction: rhsSemanticNodeArray.length > lhsSemanticNodeArray.maxParams and lhsSemanticNodeArray.forbidsMultiple')
		util.dir(lhsSemanticNodeArray, rhsSemanticNodeArray)
		throw new Error('Semantic reduction error')
	}
	...
	for (var s = 0; s < rhsLen; ++s) {
		...
		// Check for `union()` without `isCompletedUnion` in the RHS, as opposed to being caught in `semantic.reduce()` and passed to `reduceUnion()` when RHS has length of 1.
		if (rhsSemanticNode.semantic === unionSemantic && !rhsSemanticNode.isCompletedUnion) {
	...
	// Need not sort `newLHSSemanticNodeArray`, because the array will be sorted when reduced as a RHS semantic array.
	return newLHSSemanticNodeArray
}

/**
 * Reduces `lhsSemanticNodeArray` with `rhsSemanticNodeArray`, whose number of semantic nodes is within the LHS semantic node's `maxParams` bound. Ignores any children `lhsSemanticNodeArray` may have, using only the LHS semantic node's function.
 *
 * For example:
 *   LHS: `intersect()`
...
function baseReduce(lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	// Check this function was invoked when `rhsSemanticNodeArray` has fewer semantic nodes than the `lhsSemanticNodeArray` `maxParams` bound. Else, `copyLHSAndReduce()` should have been used.
	if (rhsSemanticNodeArray.length > lhsSemantic.maxParams) {
		util.logError('`baseReduce()` invoked when RHS semantic exceeds LHS `maxParams` bound. Use `copyLHSAndReduce()` instead.')
		util.dir(lhsSemanticNodeArray, rhsSemanticNodeArray)
		throw new Error('Semantic reduction error')
	}

	if (lhsSemantic === unionSemantic) {
		// If `lhsSemantic` is a `union()` semantic node, replace instances of `union()` semantic nodes within the first (most shallow) level of `rhsSemanticNodeArray` with their child semantic nodes, if any. This removes logically redundant instances of nested `union()` semantics, which enables detecting duplicate semantics.
...
function markRHSConjunctionComplete(rhsSemanticNodeArray) {
	...
	if (rhsSemanticNode.semantic === intersectSemantic) {
		// Throw an exception if `rhsSemanticNode already has `isCompletedIntersect`, which could only occur if there are three consecutive `intersect()` semantics.
 ...
 *
 * When an `intersect()` follows another `intersect()` (as is when this function is invoked), then there were no intermediate semantics, which only occurs for `union()` rules in a conjunction rule set. This LHS `intersect()` is used to separate the two RHS branches on either side of "or", to group the `union()` semantic arguments. This RHS `intersect()` is the top of a subtree, beginning at `[cat-plural]`.
 *
 ...
 *
 * For example, `isCompletedIntersect` prevents the following incorrect distribution of a LHS semantic function among `union(intersect())` semantic arguments:
 ...
 *   -> union(not(repos-liked(0)),not(repos-liked(1)))`
 *
 * For use by `semantic.reduce(lhsSemanticNodeArray, rhsSemanticNodeArray)`, when `lhsSemanticNodeArray` is an unreduced semantic array and `rhsSemanticNodeArray` is a semantic array with a single semantic node for `union()` and the boolean property `unionSemanticNode.isCompletedUnion` is falsey. Invoke this function after the `intersect()` check in `semantic.reduce()`.
 *
 ...
 * Though, this semantic is not illogical. It merely can be simplified. Nevertheless, reject this semantic even if input as such by the user to prevent such unsimplified semantics as suggestions. In addition, it is necessary to fully simplify the semantics for `pfsearch` to correctly identify semantically identical parse trees (whose logical equivalence would be undetectable in their unsimplified forms).
 *
 * @private
 * @static
...
function reduceUnion(lhsSemanticNodeArray, unionSemanticNode) {
	...
	for (var u = 0, unionChildrenLen = unionChildren.length; u < unionChildrenLen; ++u) {
		...
		if (unionChildNode.semantic === intersectSemantic && !unionChildNode.isCompletedIntersect) {
			 ...
			 * This occurs when an `intersect()` follows another `intersect()`, where the `union()` conjunction set produced the former function and a `[cat-plural]` produced the latter.
			 *
			 ...
			 *
			 * This semantic distribution does not catch all duplicate semantics. For example, the third `union()` semantic argument below is logically identical to the union of the firs two:
			 ...
			 * Ideally, this semantic would be fully logically simplified to catch duplicates for the reasons explained in this function's description, however, it is far too rare and complex to justify the algorithmic overhead to this procedure.
			 */
			var intersectChildNodes = exports.reduce(lhsSemanticNodeArray, unionChildNode.children)

			// Check if reducing `lhsSemanticNodeArray` with the `intersect()` semantic arguments is semantically illegal. Throws an exception because not yet seen, though could return -1 without issue.
			...
			// Check if reducing `lhsSemanticNodeArray` with the `union()` semantic argument is semantically illegal. Throws an exception because not yet seen, though could return -1 without issue.
...
/**
 * Replaces instances of `union()` semantic nodes within the first (most shallow) level of `rhsSemanticNodeArray`, if any, with their child semantic nodes. This removes logically redundant instances of nested `union()` semantics, which enables detecting duplicate semantics. Invoke this function from `semantic.reduce(lhsSemanticNodeArray, rhsSemanticNodeArray)` when `lhsSemanticNodeArray` is `union()`.
 *
...
function flattenUnion(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		...
		if (rhsSemanticNode.semantic === unionSemantic) {
			...
			/**
			 * Check for illegal semantics after removing redundant, nested `union()` semantics; i.e., no duplicates, `not()` contradictions, or `forbidsMultiple` conflicts. For example:
			 *   "repos I created or Danny likes or Aang created"
			 *   `union(repos-created(me),union(repos-created(1),repos-liked(0)))`
			 *   -> `union(repos-created(me),repos-created(1),repos-liked(0))`
			 *   -> -1
			 ...
			 *
			 * Use `Array.prototype.push()` instead of `semantic.mergeRHS()`, which uses `Array.prototype.concat()`, because `rhsSemanticNodeArrayCopy` was already copied.
			 */
 ...
 * @static
 * @memberOf semantic
 * @param {Object[]} semanticNodeArray The semantic tree to inspect.
