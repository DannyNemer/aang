 ...
 * • Duplicate semantics.
 * • Violations of a semantic's `forbidsMultiple` property.
 * • Contradictory negations.
 *
 * It is necessary to prevent contradictory and duplicate semantics, even when explicitly input by the user, to prevent undesirable semantics as suggestions. In addition, completely simplifying the semantics's logic enables `pfsearch` to correctly identify semantically identical parse trees (whose logical equivalence would be undetectable in their unsimplified forms).
 *
 * @static
 * @memberOf semantic
...
/**
 * Checks if `newLHSSemanticNodeArray`, a new LHS semantic function (yet to reduce), exists in `rhsSemanticNodeArray`, the previous RHS semantics (already reduced) when multiple instances of that semantic function (irrespective of children) are forbidden.
 *
