...
/**
 * Copies and extends the semantic node in `semanticNodeArray` if it is either `union()` or `intersect()` with the property `isComplete`, which indicates to `reducedUnion()` that `semanticNodeArray` reached its parent `intersect()`. If `semanticNodeArray` does not contain either specified semantic function, returns it unchanged.
 *
 ...
 * @returns {Object[]} Returns a copy of `semanticNodeArray` extended with the property `isComplete`, if applicable,  else `semanticNodeArray` unchanged.
 */
function completeSingleNodeConjunctionSemantic(semanticNodeArray) {
	...
	// Return `semanticNodeArray` as is.
