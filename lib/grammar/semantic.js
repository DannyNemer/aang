...
function flattenUnion(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		...
		if (rhsSemanticNode.semantic === unionSemantic) {
			...
			rhsSemanticNodeArrayCopy = exports.mergeRHS(rhsSemanticNodeArrayCopy, [ rhsSemanticNode ])

			/**
			 * Check for illegal semantics after removing redundant, nested `union()` semantics. For example:
