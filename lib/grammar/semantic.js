...
if (require.main.filename !== require.resolve('./buildGrammar.js')) {
	// If not currently building the grammar, then load the `intersect()` semantic from the grammar file, which works for comparisons because of 'initSemantics.js'.
...
exports.new = function (opts) {
	...
	if (exports.semantics.hasOwnProperty(opts.name)) {
		util.logErrorAndPath('Duplicate semantic name:', opts.name)
...
var semanticFuncOptsSchema = {
	...
	maxParams: Number,
	// Specify only one instance of this semantic function is permitted in another semantic's RHS.
...
var semanticArgOptsSchema = {
	// Specify this semantic is a semantic argument.
	isArg: Boolean,
	// The unique name.
	name: String,
	// The parsing cost.
...
/**
 * Merges two semantic arrays into a single array.
 *
 * Returns `-1` when semantically illegal:
 * - There are duplicate semantics.
 * - A `forbidsMultiple` rule is violated.
 * - There are contradictory negations.
 *
 * Note: it is faste to return `-1` and check the return value up the stack than it is to throw an exception and catch it up the stack.
 *
 * @static
 * @memberOf semantic
 ...
 *
 * Requires invoking 'initSemantics.js' on the semantics beforehand, which enables comparing semantics by reference instead of checking their `name` properties. In addition, 'initSemantics.js' accelerates semantic comparisons by replacing identical semantics in the grammar with references to a single object.
 *
 * @static
 ...
 * @returns {Object[]} Returns the new, reduced semantic.
 */
