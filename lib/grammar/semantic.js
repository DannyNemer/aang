...
function markRHSConjunctionComplete(rhsSemanticNodeArray) {
	...
	// If `rhsSemanticNodeArray` is an `intersect()` semantic node, assign `isCompletedIntersect` to the semantic node to prevent `reduceUnion()` from incorrectly distributing the LHS semantic that follows a `union()` semantic too deep among the arguments of instances of `intersect()` within the `union()` semantic.
...
/**
 * Creates a semantic node array with a copy of `unionSemanticNode` extended with the boolean property `isCompletedUnion` defined as `true`.
 *
 * The `isCompletedUnion` property prevents reduceUnion()` from moving the `union()` semantic node too far up its semantic tree. Invoke this function when reducing the LHS semantic `intersect()` with a RHS semantic array containing a single `union()` semantic node, `unionSemanticNode`.
 *
