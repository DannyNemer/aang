 ...
 * @static
 * @param {Object} options The options object.
 * @param {string} options.name The unique semantic function name.
 * @param {number} options.cost The cost penalty added to rules that use the function.
 ...
 * @param {boolean} [options.forbidsMultiple] Specify permitting only one instance of this semantic function in another semantic's arguments (i.e., RHS).
 * @param {Object[]} [options.requires] A separate semantic that `psearch` requires is within the same instance of `intersect()` as this semantic, else rejects the semantic tree.
 ...
 * @private
 * @static
 * @param {Objct} lhsSemanticNodeArray The LHS semantic to reduce with `unionSemanticNode.children`.
...
function reduceUnion(lhsSemanticNodeArray, unionSemanticNode) {
	...
	var newLHSNode = {
		...
		children: [],
		// Define `isCompletedUnion` as `true` to mark the conjunction as complete to prevent `reduceUnion()` from moving this `union()` node any further up the tree.
		isCompletedUnion: true,
	}
	...
	for (var u = 0, unionChildrenLen = unionChildren.length; u < unionChildrenLen; ++u) {
		...
		if (unionChildNode.semantic === intersectSemantic && !unionChildNode.isCompletedIntersect) {
			 ...
			 *
			 * Check `isCompletedIntersect` on the `intersect()` semantic node to ensure `lhsSemanticNodeArray` distribution only occurs for `intersect()` instances for the same conjunction rule set as `unionSemanticNode`, and not a subtree. `isCompletedIntersect` identifies the latter, which occurs when one `intersect()` follows another `intersect()`, where the `union()` conjunction set produced the former and a `[cat-plural]` produced the latter.
			 *
...
function flattenUnion(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		 ...
		 *   `union(func(a),union(func(b),func(c)))` -> `union(func(a),func(b),func(c))`
		 *
		 * Do not check these nested `union()` nodes for `isCompletedUnion`, which indicates the root of a conjunction set to `reduceUnion()`. `markUnionComplete()` assigns `isCompletedUnion` upon reaching the parent `intersect()` that follows `union()`, however, every rule with `union()` (in conjunctions) has RHS symbols that only produce rules with `intersect()` to group the `union()` semantic arguments on each side of an "or".
		 *
		 * Hence, `isCompletedUnion` does not indicate separate conjunction sets, because any conjunction with multiple instances of "or" will assign `isCompletedUnion` to every `union()`. Rather, it is this function's job to remove the redundant, nested `union()` semantics, leaving the only true root `union()`. `reduceUnion()` uses this root `union()` to distinguish multiple conjunction sets within a tree.
		 */
