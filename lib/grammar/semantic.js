...
exports.reduce = function (lhs, rhs) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		if (rhsLen === 1) {
			/**
			 * When an `intersect()` follows an `intersect()`, then there were no intermediate semantics, which only occurs for `union()` rules in a conjunction rule set.
			 *
...
function reduceUnion(lhsSemanticNodeArray, unionSemanticNode) {
	...
	for (var u = 0, unionChildrenLen = unionChildren.length; u < unionChildrenLen; ++u) {
		...
		if (unionChildNode.semantic === intersectSemantic && !unionChildNode.intersectCompleted) {
			/**
			 ...
			 *   With `intersectCompleted` check:    Without check (incorrect):
			 *   union(                              union(
			 *     likers(                             intersect(
