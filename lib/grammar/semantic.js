...
exports.reduce = function (lhsSemanticNodeArray, rhsSemanticNodeArray) {
	...
	if (lhsSemantic === intersectSemantic) {
		...
		/**
		 * Discard LHS `intersect()` when `rhsSemanticNodeArray` contains only one semantic node.
		 *
		 * Copy and extend the semantic node in `rhsSemanticNodeArray` if it is either either `union()` or `intersect()` with the property `isComplete`,  which indicates to `reducedUnion()` that `rhsSemanticNodeArray` reached its parent `intersect()`. If `rhsSemanticNodeArray` does not contain either specified semantic function, return the array unchanged.
		 *
		 * • If `rhsSemanticNodeArray` contains `union()`, assign `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from moving the `union()` semantic node too far up its semantic tree. If the node already has `isComplete`, mark instances of `intersect()` among its children with `isComplete` to prevent `reduceUnion()` from incorrectly distributing the LHS semantic that follows the `union()` among the arguments of its `intersect()` children.
		 *
		 * • If `rhsSemanticNodeArray` contains `intersect()`, assign `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows a future `union()` semantic among the arguments of instances of `intersect()` within the `union()` semantic.
		 *
	 ...
	 *
	 * Invoke `reduceUnion()` after the `intersect()` check, which invokes `completeSingleNodeConjunctionSemantic()` to assign properties indicating reaching a conjunction's root. Invoke before traversing `lhsSemanticNodeArray` children, if any, to properly distribute all of `lhsSemanticNodeArray` among the `union()` arguments.
	 */
	if (rhsLen === 1) {
...
/**
 * Copies and extends the semantic node in `semanticNodeArray` if it is either `union()` or `intersect()` with the property `isComplete`, which indicates to `reducedUnion()` that `semanticNodeArray` reached its parent `intersect()`. If `semanticNodeArray` does not contain either specified semantic function, returns the array unchanged.
 *
 * Invoke from `semantic.reduce()` upon reducing the LHS `intersect()` with the single-node RHS `semanticNodeArray`.
 *
 * If `semanticNodeArray` contains `union()`, assigns `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from moving the `union()` semantic node too far up its semantic tree. If the node already has `isComplete`, marks instances of `intersect()` among its children with `isComplete` to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows the `union()` among the arguments of its `intersect()` children.
 *
 * If `semanticNodeArray` contains `intersect()`, assigns `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows a future `union()` semantic among the arguments of instances of `intersect()` within the `union()` semantic.
 *
 * This function does not mutate `semanticNodeArray`, which is a shared resource; rather, it copies the array if changes are needed, else returns the original array.
 *
 * @private
 * @static
 * @param {Object[]} semanticNodeArray The single node semantic array in which to copy and mark complete an instance of `union()` or `intersect()`, if any.
 * @returns {Object[]} Returns a copy of `semanticNodeArray` with its node extended with the `isComplete` property, if applicable, else `semanticNodeArray` unchanged.
 */
function completeSingleNodeConjunctionSemantic(semanticNodeArray) {
	...
	/**
	 * If `semanticNode` is a `union()` semantic, assign `isComplete` to the node to prevent future `reduceUnion()` invocations from moving the `union()` semantic node too far up its semantic tree.
	 *
	 * If the node already has `isComplete`, mark instances of `intersect()` among `semanticNode.children` with `isComplete` to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows the `union()` among the arguments of `intersect()` children in `semanticNode.children`.
	 *
	...
	if (semanticNode.semantic === unionSemantic) {
		// Copy and extend `semanticNode` (with `isComplete`) to avoid mutating the shared resource.
		return [ {
			semantic: semanticNode.semantic,
			/**
			 * If `semanticNode.isComplete` is `true`, then `intersect()` is following an already complete `union()`. Ergo, the succession of semantics must have been (at least) the following, where the middle `intersect()` previously marked `union()` complete and was discarded (for having only one argument).
			 *   `intersect()` -> `intersect()` -> `union()`
			 *
			 * When this occurs, invoke `completeMultiNodeConjunctionSemantic(semanticNode.children)` to mark all instances of `intersect()` within `union()` complete to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows the `union()` among the arguments of its `intersect()` children.
			 *
			 * For example, consider the following query:
			 *   "repos of mine or people who follow Aang or are my followers I follow"
			 * This query yields the following unmodified semantic tree; i.e., without removal of redundant `intersect()` semantics, distribution of `union()` parent semantics, and `union()` flattening:
			 *   intersect(
			 *     repos-created(
			 *       union(
			 *         intersect(me)
			 ...
			 *                 followers(me)
			 *                 users-followed(me) ) ) ) ) ) ) )
			 * With the `completeMultiNodeConjunctionSemantic()` invocation with the `union()` arguments, as annotated in the diagram, it enables the following proper reduction and `union()` distribution:
			 ...
			 *         users-followed(me) ) ) )
			 * In this example, the `completeMultiNodeConjunctionSemantic(semanticNode.children)` invocation prevented `reduceUnion()` from distributing `repositories-created()` among the `intersect()` arguments.
			 */
			children: semanticNode.isComplete ? completeMultiNodeConjunctionSemantic(semanticNode.children) : semanticNode.children,
			isComplete: true,
		} ]
	}

	/**
	 * If `semanticNode` is an `intersect()` semantic, assign `isComplete` to the node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows a future `union()` semantic among the arguments of instances of `intersect()` within the `union()` semantic.
	 *
	 * When an `intersect()` follows another `intersect()` (as is when this function is invoked and the following condition is true), then there were no intermediate semantics, which only occurs for `union()` rules in a conjunction rule set.
	 ...
	 *
	 * In this case, the LHS semantic that precedes the `union()` semantic in the tree, which precedes this LHS `intersect()` semantic, should not be distributed among this RHS `intersect()` semantic arguments. Hence, recreate `semanticNode` to define `isComplete` as `true` to prevent `reduceUnion()` from distributing the LHS semantic that follows `union()` among the arguments of this `intersect()`.
	 *
	...
	if (semanticNode.semantic === intersectSemantic) {
		// Throw an exception if `semanticNode` already has `isComplete`, which can only occur if there are three consecutive `intersect()` semantics.
...
/**
 * Copies and extends instances of `intersect()` in `semanticNodeArray`, if any, with the property `isComplete` to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows the parent `union()` among the arguments of its `intersect()` children in `semanticNodeArray`. If `semanticNodeArray` contains no `intersect()` instances, returns the array unchanged.
 *
 * Invoke from `completeMultiNodeConjunctionSemantic()` with the semantic children of an already complete `union()` semantic that the function is attempting to mark complete again.
 * • `semanticNodeArray` will always contain multiple semantic nodes because its parent semantic function, `union()`, requires a minimum of two semantic arguments.
 *
 * This function does not mutate `semanticNodeArray`, which is a shared resource; rather, it copies the array if changes are needed, else returns the original array.
 *
 * @private
 * @static
 * @param {Object[]} semanticNodeArray The multi-node semantic array in which to copy and mark complete instances of `intersect()`, if any.
 * @returns {Object[]} Returns a copy of `semanticNodeArray` with its `intersect()` nodes extended with the `isComplete` property, if any, else `semanticNodeArray` unchanged.
 */
function completeMultiNodeConjunctionSemantic(semanticNodeArray) {
	...
	for (var s = 0, rhsLen = semanticNodeArray.length; s < rhsLen; ++s) {
		...
		/**
		 * If `semanticNode` is an `intersect()` semantic, assign `isComplete` to the node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows the parent `union()` among the arguments of its `intersect()` children in `semanticNodeArray`.
		 *
		 * See `completeSingleNodeConjunctionSemantic()` in-line documentation for detailed explanation.
		 *
		 * Check if `semanticNode.isComplete` is already `true` to prevent unnecessary duplication.
		 */
	...
	return semanticNodeArrayCopy || semanticNodeArray
}

/**
 * Copies and extends instances of `intersect()` and `union()` in `rhsSemanticNodeArray`, if any, with the property `isComplete` to indicate to `reduceUnion()` having reached the parent `intersect()`. If `rhsSemanticNodeArray` contains no applicable semantics, returns it unchanged.
 *
 * Invoke from `semantic.reduce()` upon reducing the LHS `intersect()` with `rhsSemanticNodeArray`.
 *
 * For instances of `union()`, assigns `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from moving the `union()` semantic node too far up its semantic tree.
 *
 * For instances of `intersect()`, assigns `isComplete` to the semantic node to prevent future `reduceUnion()` invocations from incorrectly distributing the LHS semantic that follows a `union()` semantic too deep among the arguments of instances of `intersect()` within the `union()` semantic.
 *
 * This function does not mutate `rhsSemanticNodeArray`, which is a shared resource; rather, it copies the array if changes are needed, else returns the original array.
 *
 * @private
 * @static
 * @param {Object[]} rhsSemanticNodeArray The RHS semantic array to iterate and in which to copy and extend semantic nodes with an applicable property, if any.
...
function markRHSConjunctionComplete(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		...
		/**
		 * If `rhsSemanticNode` is a `union()` semantic, assign `isComplete` to the node to prevent `reduceUnion()` from moving the `union()` semantic node too far up its semantic tree.
		 *
		 * Copy `rhsSemanticNode` to avoid mutating a shared resource. Check `rhsSemanticNode` lacks `isComplete` to avoid unnecessarily recreating the semantic node.
		 *
		 * In the grammar, rules with the `union()` semantic follow the LHS semantic function to which they apply. `reduceUnion()` moves instances of `union()` up a semantic tree and distributes the outer LHS semantic function(s) among the `union()` semantic arguments. For example:
		 *   LHS: `lhsFunc()`
		 *   RHS: `union(0,intersect(1,followers(me)))`
		 ...
		 * To determine having completed the semantic reduction for a `union()` conjunction, and when to stop moving `union()` up the semantic tree as described, `semantic.reduce()` invokes this function upon reaching the parent `intersect()` semantic (at `[cat-plural]`) to mark `union()` semantic nodes, if any, with the property `isComplete`. `reduceUnion()` checks for this `isComplete` property.
		 *
		 * Most often, `rhsSemanticNodeArray` contains a single semantic node, though it can contain multiple. For example, the following marks `union()` complete within the RHS array:
		...
		if (rhsSemanticNode.semantic === unionSemantic) {
			...
			// Copy and extend `rhsSemanticNode` (with `isComplete`) to avoid mutating the shared resource.
			rhsSemanticNodeArrayCopy.push({
				semantic: rhsSemanticNode.semantic,
				/**
				 * If `rhsSemanticNode.isComplete` is `true`, then `intersect()` is following an already complete `union()`. Ergo, the succession of semantics must have been (at least) the following, where the middle `intersect()` previously marked `union()` complete and was discarded (for having only one argument).
				 *   `intersect()` -> `intersect()` -> `union()`
				 *
				 * When this occurs, recursively invoke `markRHSConjunctionComplete(rhsSemanticNode.children)` to mark all instances of `intersect()` within `union()` complete to prevent `reduceUnion()` from distributing semantics too far.
				 * • There will never be multiple recursive calls because `flattenUnion()` prevents a `union()` within a `union()`.
				 *
				 * For example, consider the following query:
				 *   "repos of mine or people who follow Aang or are my followers I follow"
				 * This query yields the following unmodified semantic tree; i.e., without removal of redundant `intersect()` semantics, distribution of `union()` parent semantics, and `union()` flattening:
				 ...
				 *               intersect(
				 *                 followers(1) )
				 *               intersect( // Marked `isComplete` in recursive invocation.
				 *                 followers(me)
				 *                 users-followed(me) ) ) ) ) ) ) )
				 * With the recursive invocation with the `union()` arguments, as annotated in the diagram, it enables the following proper reduction and `union()` distribution:
				 *   With recursive operation:                Without recursive operation (incorrect):
				 *   union(                                   union(
				 *     repositories-created(me),                repositories-created(me),
				 *     repositories-created(followers(1)),      repositories-created(followers(1)),
				 *     repositories-created(                    intersect(
				 ...
				 *         followers(me),                         repositories-created(users-followed(me)) ) )
				 *         users-followed(me) ) ) )
				 * In this example, the `markRHSConjunctionComplete(rhsSemanticNode.children)` invocation prevented `reduceUnion()` from distributing `repositories-created()` among the `intersect()` arguments.
				 */
				children: rhsSemanticNode.isComplete ? markRHSConjunctionComplete(rhsSemanticNode.children) : rhsSemanticNode.children,
				isComplete: true,
			})
		}

		/**
		 * If `rhsSemanticNode` is an `intersect()` semantic, assign `isComplete` to the node to prevent `reduceUnion()` from incorrectly distributing the LHS semantic that follows a `union()` semantic too deep among the arguments of instances of `intersect()` within the `union()` semantic.
		 *
		 * Copy `rhsSemanticNode` to avoid mutating a shared resource.
		 *
		 ...
		 *     likers(20) )
		 *
		 * Most often, `rhsSemanticNodeArray` contains a single semantic node, though it can contain multiple. For example, the following marks `intersect()` complete within the RHS array:
		 *   "people who follow Danny or Aang and my followers I follow"
		 *   LHS: `intersect()`
		 *   RHS: `1,intersect(followers(me),users-followed(me))`
		 *
		 *   After this function's invocation, and in the next `semantic.reduce()` invocation:
		 ...
		 *
		 * `rhsSemanticNode.isComplete` can be `true` in a recursive invocations on `union()` children above.
		 */
		else if (rhsSemanticNode.semantic === intersectSemantic && !rhsSemanticNode.isComplete) {
			if (!rhsSemanticNodeArrayCopy) {
				rhsSemanticNodeArrayCopy = rhsSemanticNodeArray.slice(0, s)
			}
...
function flattenUnion(rhsSemanticNodeArray) {
	...
	for (var s = 0, rhsLen = rhsSemanticNodeArray.length; s < rhsLen; ++s) {
		 ...
		 *   `union(func(a),union(func(b),func(c)))` -> `union(func(a),func(b),func(c))`
		 *
		 * Do not check these nested `union()` nodes for `isComplete`, which indicates the root of a conjunction set to `reduceUnion()`. `completeSingleNodeConjunctionSemantic()` assigns `isComplete` upon reaching the parent `intersect()` that follows `union()`, however, every rule with `union()` (in conjunctions) has RHS symbols that only produce rules with `intersect()` to group the `union()` semantic arguments on each side of an "or".
		 *
 ...
 *
 * `semantic.getName(semanticString)` - Gets the name of the semantic in `semanticString`.
 */
