...
/**
 * Converts the grammar's regex-style terminal symbols into rules that yield only single-token terminal symbols. This is necessary to enable partial matches and deletions within what would otherwise be regex-style terminal symbols.
 *
 * Invoke this module before `createEditRules` because it adds `<empty>` rules to the grammar.
 *
 * NOTE: Module is currently only in use for the verb set "follow", which when upgraded to using `verbTermSet` will allow this module to be deprecated.
 *
 * @static
...
function convertNonterminalRuleToTerminal(rule, createTerminalRuleSetFunc) {
	...
	rule.rhsDoesNotProduceText = rule.isSubstitution || rule.isStopWord || rule.text instanceof Object || reNoRegexLogicalOr.test(rule.text)

	// Delete `rule.text` if `rhs` symbols produce text.
...
function splitRegexTerminalSymbol(terminalSymbol, createTerminalRuleSetFunc) {
	...
	return forEachTokenSet(terminalSymbol, function (tokenSet, isLastSet) {
		...
		if (isLastSet) {
			// Return the array of `NSymbol` instances that produces `terminalSymbol` for use as a nonterminal rule's `rhs`.
			if (prevSymbolName) {
			...
			if (prevSymbolName) {
				...
				if (parentSymbolName) {
					...
					// Use `name` property because can not get existing `NSymbol` instances, only their names.
...
function createTerminalRuleSet(insertionCosts, tokenSet) {
	...
	var newSymbol = g.newSymbol(tokenSet, 'term')
 ...
 * @returns {number} Returns the total insertion cost that the symbol named `symbolName` produces.
 */
...
/**
 * Gets the name of an existing nonterminal symbol that produces a single binary nonterminal rule that yields the terminal symbols `a` and `b` produce, if any.
 *
 * @private
 * @static
...
/**
 * Creates a regex-style string representation of the terminal symbols the nonterminal symbol `symbolName` produces. Used to determine if two nonterminal symbols accept the same terminal symbols, irrespective of the rules' structure (e.g., terminal rule set order).
 *
 ...
 * @returns {string} Returns the regex-style string representation of the terminal symbols `symbolName` produces.
 */
...
function calculateInsertionCosts(nonterminalRuleSets) {
	...
	// Get the insertion cost, if any, of each single-token terminal symbol.
	forEachTerminalRule(function (rule) {
 ...
 * @private
 * @static
 * @param {Function} iteratee The function invoked per terminal rule.
 */
 ...
 * @param {Function} predicate The function invoked per token set.
 ...
 * @param {Function} iteratee The function invoked per token.
 */
