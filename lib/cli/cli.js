...
rl.addCommands({
	...
	func: function archiveTest(filepath) {
		...
		var outPath = util.expandHomeDir(filepath || '../../out/out_test')
	...
	func: function diffTest() {
		...
		rl.spawnAsyncProcess('node', [ '../test/test.js', '--mute' ], [ 'ignore', outFD, outFD ], function () {
			...
			childProcess.spawnSync('vimdiff', [ '../../out/out_test', tempPath ], { stdio: 'inherit' })
	...
	name: 'archiveGrammar',
	argNames: [ '[<path>]' ],
	description: 'Rebuild the grammar and archive it to a separate file [at <path>].',
	func: function archiveGrammar(filepath) {
		rl.spawnAsyncProcess('node', [
			'../grammar/buildGrammar.js',
			'--output=' + (filepath || '../../out/grammar_old.json'),
		// Pass all arguments to the command line program.
		].concat(Array.prototype.slice.call(arguments)))
	},
}, {
	...
	func: function diffGrammar() {
		...
		childProcess.spawnSync('vimdiff', [ '../../out/grammar_old.json', '../grammar.json' ], { stdio: 'inherit' })
	},
}, {
	name: 'ambigCheck',
	argNames: [ '[<n>]' ],
	description: 'Find ambiguity in the grammar [with tree-symbol limit <n>].',
	func: function ambigCheck(treeSymsLimit) {
		rl.spawnAsyncProcess('node', [
