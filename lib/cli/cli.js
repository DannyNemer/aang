...
rl.addCommands({
	...
	func: function archiveGrammar(filename, callback) {
		...
		rl.spawnAsyncProcess('node', [
			...
			// Color RLI prompt character green if the archived grammar does not match the last grammar `.buildGrammar` generated. Invoke after `callback()`.
			updateRLIPrompt(rl)
		})
	},
}, {
	...
	name: 'archiveAll',
	description: 'Invokes `.archiveGrammar`, `.archiveTest`, and `.archiveTestQuiet`.',
	func: function archiveAll() {
		// Do not bother rewriting as asynchronous because the change does not shorten processing time enough for the user to not background the process as before.
		// Invoke `buildGrammar` to update grammar for use in tests.
		rl.commands['.buildGrammar'](function () {
			// Copy `buildGrammar` output instead of invoking `archiveGrammar` to save 3.5 seconds (by not regenerating the grammar). Write stream operation can be asynchronous because its output is not used by the following operations.
			fs.createReadStream(paths.grammar).pipe(fs.createWriteStream(paths.grammarOld)).on('finish', function () {
				// Reset RLI prompt character after copying grammar to archive.
				updateRLIPrompt(rl)
			})
			// Do not write console messages within the asynchorous 'finish' event above, otherwise they will write after the following command (`.archiveTest`) redirects output.
			util.log('File saved:', fs.realpathSync(paths.grammarOld))
			util.log()
