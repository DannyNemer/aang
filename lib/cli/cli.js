...
rl.addCommands({
	...
	// Copies output of the test suite to the pasteboard.
	copyTest: function (k) {
		...
		rl.spawnAsyncProcess('node', [
			...
			// Pass all arguments to the command line program.
			Array.prototype.join.call(arguments, ' '),
		], [ 'ignore', pbcopy.stdin, process.stderr ], function () {
	...
	// Writes output of the test suite to a file.
	saveTest: function (filepath) {
	...
	// Toggles suppressing parse results from output.
	quiet: function () {
		config.quiet = !config.quiet
		util.log('Suppress parse results:', config.quiet)
	},

	// Toggles benchmarking duration of the parse.
	time: function () {
	...
	help: function () {
		util.log([
			...
			'  .copyTest [<k>]     copy output of the test suite to the pasteboard',
			'  .saveTest [<path>]  write output of the test suite to a file [at <path>]',
			...
			'  .time         benchmark parse duration: ' + util.colors.yellow(config.benchmark),
