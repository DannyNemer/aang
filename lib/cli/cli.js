...
/**
 * Listens for `\n` in input stream, usually received when the user hits `return`. Executed when input is not a recognized command (set by `rl.setCommands()`).
 */
...
rl.addCommands({
	...
	// Print state table.
	stateTable: function () {
		rl.spawnAsyncProcess('node', [ '../parser/printStateTable.js' ])
	},
	...
	// Toggle supressing parse results from output.
	quiet: function () {
