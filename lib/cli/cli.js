...
rl.onLine(function (query) {
	this.spawnAsyncProcess('node', [
		...
		'--costs=' + config.printCosts,
		'--ambiguity=' + config.printAmbiguity,
		'--trees=' + config.printTrees,
		'--tree-node-costs=' + config.printTreeNodeCosts,
		'--tree-token-ranges=' + config.printTreeTokenRanges,
		'--semantics=' + config.printSemantics,
		'--object-semantics=' + config.printObjectSemantics,
		'--parse-stack=' + config.printParseStack,
		'--parse-forest=' + config.printForest,
		'--parse-forest-graph=' + config.printForestGraph,
...
rl.addCommands({
	...
	func: function quiet() {
		util.log('Suppress parse results:', config.quiet = !config.quiet)
	},
}, {
	name: 'time',
	description: 'Toggle benchmarking duration of parse and parse forest search.',
	func: function time() {
		util.log('Print parse time:', config.benchmark = !config.benchmark)
	},
}, {
	name: 'costs',
	description: 'Toggle printing parse costs.',
	func: function costs() {
		util.log('Print parse costs:', config.printCosts = !config.printCosts)
	},
}, {
	name: 'ambiguity',
	description: 'Toggle printing instances of semantic ambiguity.',
	func: function ambiguity() {
		util.log('Print ambiguity:', config.printAmbiguity = !config.printAmbiguity)
	},
}, {
	name: 'trees',
	description: 'Toggle printing parse trees.',
	func: function trees() {
		util.log('Print parse trees:', config.printTrees = !config.printTrees)
	},
}, {
	name: 'nodeCosts',
	description: 'Toggle including in parse trees each node\'s path cost.',
	func: function nodeCosts() {
		util.log('Include in parse trees each node\'s path cost:', config.printTreeNodeCosts = !config.printTreeNodeCosts)
	},
}, {
	name: 'tokenRanges',
	description: 'Toggle including in parse trees each node\'s token range.',
	func: function tokenRanges() {
		util.log('Include in parse trees each node\'s token range:', config.printTreeTokenRanges = !config.printTreeTokenRanges)
	},
}, {
	name: 'semantics',
	description: 'Toggle printing semantics.',
	func: function semantics() {
		util.log('Print semantics:', config.printSemantics = !config.printSemantics)
	},
}, {
	name: 'objectSemantics',
	description: 'Toggle printing object representations of semantics.',
	func: function objectSemantics() {
		util.log('Print object representations of semantics:', config.printObjectSemantics = !config.printObjectSemantics)
	},
}, {
	name: 'stack',
	description: 'Toggle printing the parse stack.',
	func: function stack() {
		util.log('Print parse stack:', config.printParseStack = !config.printParseStack)
	},
}, {
	name: 'forest',
	description: 'Toggle printing an equational representation of the parse forest.',
	func: function forest() {
		util.log('Print parse forest:', config.printForest = !config.printForest)
	},
}, {
	name: 'graph',
	description: 'Toggle printing a graph representation of the parse forest.',
	func: function graph() {
		util.log('Print parse forest graph:', config.printForestGraph = !config.printForestGraph)
	},
})