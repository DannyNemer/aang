...
rl.addCommands({
	...
	func: function archiveAll() {
		...
		rl.commands['.buildGrammar'](function () {
			// Copy `buildGrammar` output instead of invoking `archiveGrammar` to save 3.5 seconds. Write stream operation can be asynchronous because its output is not used here.
			fs.createReadStream(paths.grammar).pipe(fs.createWriteStream(paths.grammarOld))
			util.log('File saved:', fs.realpathSync(paths.grammarOld))
			util.log()

			// Reset RLI prompt character after archiving grammar.
			updateRLIPrompt(rl)
