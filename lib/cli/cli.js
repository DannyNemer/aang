...
rl.addCommands({
	...
	func: function copyTest() {
		// Pass all arguments to the command line program.
		copyOutput([ paths.test ].concat(Array.prototype.slice.call(arguments)))
	},
}, {
	name: 'archiveTest',
	argNames: [ '[<filename>]' ],
	...
	func: function archiveGrammar(filename, callback) {
		...
		rl.spawnAsyncProcess('node', [
			...
			// Remove green color from RLI prompt character after archiving all grammar changes, if any.
			setRLIPrompt(rl, false)
...
function updateRLIPrompt(rl) {
	setRLIPrompt(rl, filesDiff(paths.grammar, paths.grammarOld))
}

/**
 * Colors the RLI prompt character green if `grammarHasChanges` is truthy, otherwise removes its color.
 *
 * @private
 * @static
 * @param {Interface} rl The RLI of which to color the prompt character.
 * @param {boolean} [grammarHasChanges] Specify if there are unarchived grammar changes, which colors the `rl`. prompt character green, else removes its color.
 */
function setRLIPrompt(rl, grammarHasChanges) {
	rl.setPrompt(util.colors[ grammarHasChanges ? 'green' : 'stripColor' ](rl._prompt))
}