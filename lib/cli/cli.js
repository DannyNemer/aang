...
// Output file paths.
var paths = (function () {
	var outDirectory = '../../out/'

	return {
		out: outDirectory,
		testOut: outDirectory + 'out_test',
		grammar: '../grammar.json',
		grammarOld: outDirectory + 'grammar_old.json',
		ambigCheckOut: outDirectory + 'ambig',
		stateTableOut: outDirectory + 'st',
	}
}())
...
rl.addCommands({
	...
	name: 'archiveTest',
	argNames: [ '[<filename>]' ],
	description: 'Archive output of the test suite to \'out\' directory [with <filename>].',
	func: function archiveTest(filename) {
		...
		var outPath = util.expandHomeDir(filename ? paths.out + filename : paths.testOut)
	...
	name: 'diffTest',
	description: 'Compare the last test saved by `.archiveTest` to current test output.',
	func: function diffTest() {
		util.log('Processing test...')

		// Run test and temporarily save output.
		var tempPath = '../../out/out_test_temp'

		var outPath = util.expandHomeDir(tempPath)
		var outFD = fs.openSync(outPath, 'w')

		rl.spawnAsyncProcess('node', [ '../test/test.js', '--mute' ], [ 'ignore', outFD, outFD ], function () {
	...
	func: function archiveGrammar(filename) {
		rl.spawnAsyncProcess('node', [
			'../grammar/buildGrammar.js',
			'--output=' + (filename ? paths.out + filename : paths.grammarOld),
		// Pass all arguments to the command line program.
		].concat(Array.prototype.slice.call(arguments)))
	},
}, {
	name: 'diffGrammar',
	description: 'Compare the last archived grammar to the current grammar.',
	...
	func: function archiveAmbigCheck(filename) {
		// Create file if does not exist, truncate file to zero length if it does exist, or throw an exception if `outPath` is a directory.
		var outPath = util.expandHomeDir(filename ? paths.out + filename : paths.ambigCheckOut)
		var outFD = fs.openSync(outPath, 'w')
	...
	argNames: [ '[<filename>]' ],
	description: 'Archive StateTable equational representation to \'out\' directory [with <filename>].',
	func: function archiveStateTable(filename) {
		// Create file if does not exist, truncate file to zero length if it does exist, or throw an exception if `outPath` is a directory.
		var outPath = util.expandHomeDir(filename ? paths.out + filename : paths.stateTableOut)
		var outFD = fs.openSync(outPath, 'w')
		...
		// Pass all arguments to the command line program.
			...
			util.log('StateTable output saved:', fs.realpathSync(outPath))
		})
	},
}, {
	name: 'repl',
