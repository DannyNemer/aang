...
function createNewTest(query) {
	// Parse query and find 60-best parse trees.
	var parseResults = parse(query, 60)
	...
	// Pass `description` of existing test for `query`, if any, as default input for the prompt.
	var existingTest = testUtil.findTest(tests, query)
	newTest.description = getDescription(existingTest && existingTest.description)
 ...
 * @private
 * @static
 * @param {string} [defaultDescription] The default description to use if user input is empty.
 * @returns {string} Returns the test description.
 */
function getDescription(defaultDescription) {
	while (true) {
		var description = readlineSync.question(
			'description: ' + (defaultDescription ? '($<defaultInput>) ' : ''),
