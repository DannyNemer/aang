 ...
 *
 * Description
 *   Parses and finds the 60-best parse trees of the tests queries in the test
...
var argv = yargs
	.usage([
		...
		'  Parses and finds the 60-best parse trees of the tests queries in the test suite, updates the tests\' existing expected `semantics` with those of the parse trees, and updates the tests\' tags. If the new test differs from the existing test, outputs the new test for the user to confirm before overwriting.',
	].join('\n'))
	.updateStrings({
...
// Reparse `tests` and updates the expected values of each if there are parse result changes.
updateTests(tests)
...
/**
 * Reparses `tests` and updates the expected values of each if there are parse result changes.
 *
 * @private
 * @static
...
/**
 * Creates a new test from `existingTest` with updated expected values (by reparsing `existingTest.test` and examining the parse results).
 *
 * @private
 * @static
 * @param {Object} existingTest The existing test to reparse and update.
 * @returns {Object} Returns the new test with updated expected values, if any.
 */
function createUpdatedTest(existingTest) {
	// Parse test query and find 60-best parse trees.
	var parseResults = parse(existingTest.query, 60)
